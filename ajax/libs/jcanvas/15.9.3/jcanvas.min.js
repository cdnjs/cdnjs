...
q.height)):(Q(l,v,q,q.width,q.height),v.drawImage(I,q.x-q.width/2,q.y-q.height/2,q.width,q.height));v.beginPath();v.rect(q.x-q.width/2,q.y-q.height/2,q.width,q.height);T(l,v,q);v.closePath();q._transformed&&v.restore();ya(v,p,q);q.layer?O(z,p,r,"load"):q.load&&q.load.call(z[0],r);q.layer&&(r._masks=p.transforms.masks.slice(0),q._next&&z.drawLayers({clear:!1,resetFire:!0,index:q._next}))}}var l,A,D,r,G,R,I,F,L,P=ca.imageCache;for(A=0;A<this.length;A+=1)if(l=this[A],D=K(this[A]))r=H(this[A]),G=new J(z),
R=N(this[A],G,z,v),G.visible&&(L=G.source,F=L.getContext,L.src||F?I=L:L&&(P[L]&&P[L].complete?I=P[L]:(I=new Ta,L.match(/^data:/i)||(I.crossOrigin=G.crossOrigin),I.src=L,P[L]=I)),I&&(I.complete||F?p(l,D,r,G,R)():(I.onload=p(l,D,r,G,R),I.src=I.src)));return this};g.fn.createPattern=function(v){function z(){r=l.createPattern(D,A.repeat);A.load&&A.load.call(p[0],r)}var p=this,l,A,D,r,G;(l=K(p[0]))?(A=new J(v),G=A.source,da(G)?(D=g("<canvas />")[0],D.width=A.width,D.height=A.height,v=K(D),G.call(D,v),
