...
(function (global, factory) {
  ...
  const FetchersContext = /*#__PURE__*/React__namespace.createContext(null);
  {
  ...
  */
  ...
  function useFetcherDataLayer() {
    let fetcherRefs = React__namespace.useRef(new Map());
    let fetcherData = React__namespace.useRef(new Map());
    let registerFetcher = React__namespace.useCallback(key => {
    ...
    let unregisterFetcher = React__namespace.useCallback(key => {
      let count = fetcherRefs.current.get(key);
    ...
    let fetcherContext = React__namespace.useMemo(() => ({
  ...
  function useFetcher(_temp3) {
    ...
    let fetchersContext = React__namespace.useContext(FetchersContext);
    let route = React__namespace.useContext(reactRouter.UNSAFE_RouteContext);
    let routeId = (_route$matches = route.matches[route.matches.length - 1]) == null ? void 0 : _route$matches.route.id;
    !fetchersContext ? router.UNSAFE_invariant(false, "useFetcher must be used inside a FetchersContext")  : void 0;
    ...
    }, [router$1, fetcherKey, register, unregister]);

    // Fetcher additions
    let load = React__namespace.useCallback(href => {
