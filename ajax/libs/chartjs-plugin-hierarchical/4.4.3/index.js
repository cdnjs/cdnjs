...
import { defaults, CategoryScale, registry } from 'chart.js';
import { valueOrDefault, toFont, merge } from 'chart.js/helpers';
...
function getMaxDepth(rootNodes) {
    const levels = [];
...
function handleClickEvents(chart, _event, elem, offsetDelta, inRange, reverse = false) {
    var _a, _b;
    const cc = chart;
    let { offset } = elem;
    const { index } = elem;
    ...
    const parents = parentsOf(label, flat);
    const maxDepth = getMaxDepth((cc.data.rootNodes || []));
    if (reverse)
        offset += maxDepth * offsetDelta;
...
const hierarchicalPlugin = {
    ...
    beforeDatasetsDraw(chart) {
        ...
        const renderHorLevel = (node, maxDepth = 0) => {
            if (node.children.length === 0) {
                return false;
            }
        ...
        const renderVertLevel = (node, maxDepth = 0) => {
            if (node.children.length === 0) {
                return false;
            }
        ...
        const maxLevel = getMaxDepth(roots);
        if (hor) {
