  ...
  */
   ...
   */
		...
		function stylis(selector, styles, animations, compact, middleware) {
			...
			// middleware
			if (use) {
				var uses = (typeof middleware === 'undefined' ? 'undefined' : _typeof(middleware)).charCodeAt(0);
			...
			if ((plugins = stylis.plugins).length !== 0) {
				middleware = function middleware(ctx, str, line, col) {
					var output = str;
										...
										if (animations === true && (buff.charCodeAt(9) || 0) !== 45) {
											...
											for (var j = 0, length = anims.length; j < length; j++) {
												...
												for (var k = 0, l = props.length; k < l; k++) {
													...
													!(frst === 115 && third === 101 && (last === 116 && len === 10 || last === 100 && len === 8)) &&

													// ease/ease-in/ease-out/ease-in-out, e, s, e
													!(frst === 101 && third === 115 && (last === 101 && len === 4 || (len === 11 || len === 7 || len === 8) && prop.charCodeAt(4) === 45)) &&
																							...
																							if (firstChar === 58) {
																								...
																								if (secondChar === 104 && selector.charCodeAt(4) === 116) {
																									...
																									// :host-context(selector)
																											...
																											selector = selector.substring(9, indexOf) + ' ' + prefix + selector.substring(indexOf + 1);
    ...
    var StyledComponent = function (_ParentComponent) {
      ...
      createClass(StyledComponent, [{
        ...
        value: function render() {
          ...
          propsForElement.className = [className, componentId, generatedClassName].filter(function (x) {
            return x;
          }).join(' ');
          if (innerRef) {
