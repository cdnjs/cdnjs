						...
						// property/selector
								...
								if (first === 97 && second === 110 && third === 105) {
									...
									// explicit syntax, anims array should have only one elemenet
											...
											// n
											build += ((buff.charCodeAt(10) || 0) !== 110 ? '' : animns) + buff.substring(colon).trim().trim();
										}
																		...
																		if (special === 0 || type === 2) {
																			...
																			// top-level selector
																					...
																					for (var j = 0, length = selectors.length; j < length; j++) {
																						...
																						if (use) {
																							temp = middleware(1.5, j === length - 1 ? selector.substring(0, selector.length - 1).trim() : selector, line, column, prefix);
