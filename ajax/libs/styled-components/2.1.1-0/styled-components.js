...
var stylis = createCommonjsModule(function (module, exports) {
  ...
  */
	/* eslint-disable */
	(function (factory) {
		(typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? module['exports'] = factory() : typeof undefined === 'function' && undefined['amd'] ? undefined(factory()) : window['stylis'] = factory();
	})(function () {
...
var BrowserTag = function () {
  ...
  BrowserTag.prototype.flush = function flush(memoryTag) {
    var _this = this;

    Object.keys(memoryTag.components).forEach(function (componentId) {
      if (!_this.components[componentId]) _this.addComponent(componentId);
      var css = memoryTag.components[componentId].css;
      var comp = _this.components[componentId];
      _this.inject(componentId, css.slice(comp.index).join(''), memoryTag.names);
  ...
  BrowserTag.prototype.addComponent = function addComponent(componentId) {
    ...
    var comp = { componentId: componentId, textNode: document.createTextNode(''), index: 0 };
  ...
  BrowserTag.prototype.inject = function inject(componentId, css, names) {
    if (!css) return;
    if (!this.ready) this.replaceElement();
  ...
  BrowserTag.prototype.replaceElement = function replaceElement() {
    ...
    Object.keys(this.components).forEach(function (key) {
      var comp = _this2.components[key];
...
var InMemoryTag = function () {
  function InMemoryTag(onBrowser, isLocal, browserTag) {
    classCallCheck(this, InMemoryTag);
    ...
    this.components = !browserTag ? {} : Object.keys(browserTag.components).reduce(function (accum, componentId) {
      var _babelHelpers$extends;

      var browserTagComp = browserTag.components[componentId];
      browserTagComp.index = 1;
      return _extends({}, accum, (_babelHelpers$extends = {}, _babelHelpers$extends[componentId] = { componentId: componentId, css: [browserTagComp.cssFromDOM] }, _babelHelpers$extends));
    }, {});
  }

  InMemoryTag.prototype.isFull = function isFull() {
    return this.onBrowser && this.size >= COMPONENTS_PER_TAG;
  };

  InMemoryTag.prototype.addComponent = function addComponent(componentId) {
    if (this.components[componentId]) throw new Error('Trying to add Component \'' + componentId + '\' twice!');
    this.components[componentId] = { componentId: componentId, css: [] };
    this.size += 1;
  };

  InMemoryTag.prototype.inject = function inject(componentId, css, name) {
    var comp = this.components[componentId];
  ...
  InMemoryTag.prototype.flush = function flush() {
  ...
  InMemoryTag.prototype.toHTML = function toHTML() {
    var _this = this;
    ...
    var css = Object.keys(this.components).map(function (key) {
      return _this.components[key].css.join('');
    }).join('');

    return '<style type="text/css" ' + namesAttr + ' ' + localAttr + '>\n' + css + '\n</style>';
  };

  InMemoryTag.prototype.toReactElement = function toReactElement(key) {
    var _attributes,
        _this2 = this;
  ...
  InMemoryTag.prototype.clone = function clone() {
    var _this3 = this;

    if (this.browserTag) throw new Error('BrowserTag cannot be cloned!');
    var copy = new InMemoryTag(this.onBrowser, this.isLocal);
    copy.names = [].concat(this.names);
    copy.size = this.size;
    copy.components = Object.keys(this.components).reduce(function (acc, key) {
      var _components$key = _this3.components[key],
          componentId = _components$key.componentId,
          css = _components$key.css;

      acc[key] = { componentId: componentId, css: [].concat(css) }; // eslint-disable-line no-param-reassign
      return acc;
    }, {});
...
var BrowserStyleSheet = {
  create: function create() {
    ...
    for (var i = 0; i < nodesLength; i += 1) {
      ...
      var isLocal = el.getAttribute(LOCAL_ATTR) === 'true';
...
var StyleSheet = function () {
  ...
  StyleSheet.prototype.createNewTag = function createNewTag(isLocal) {
    this.dirty = true;
    var newTag = new InMemoryTag(this.onBrowser, isLocal);
  ...
  StyleSheet.clone = function clone(oldSheet) {
    var newSheet = new StyleSheet(oldSheet.onBrowser, oldSheet.tags.map(function (tag) {
      return tag.clone();
...
var _StyledComponent = (function (ComponentStyle, constructWithOptions) {
  ...
  var BaseStyledComponent = function (_AbstractStyledCompon) {
    ...
    BaseStyledComponent.prototype.getStyleSheetInstance = function getStyleSheetInstance() {
    ...
    BaseStyledComponent.prototype.generateAndInjectStyles = function generateAndInjectStyles(theme, props) {
      ...
      var className = componentStyle.generateAndInjectStyles(executionContext, this.getStyleSheetInstance());
    ...
    BaseStyledComponent.prototype.componentDidUpdate = function componentDidUpdate() {
      this.getStyleSheetInstance().flush();
    };
...
var wrapWithTheme = function wrapWithTheme(Component$$1) {
  ...
  var WithTheme = function (_React$Component) {
    ...
    WithTheme.prototype.render = function render() {
      ...
      return React$1__default.createElement(Component$$1, _extends({
        theme: theme
      }, this.props, {
