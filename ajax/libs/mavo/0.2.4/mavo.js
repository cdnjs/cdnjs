...
var _ = Mavo.Collection = class Collection extends Mavo.Node {
	...
	static getClosestAcceptingCollection (target, property) {
		let collection = target.closest(`[mv-list]:is([property="${property}"], [mv-accepts~="${property}"])`);
	...
	getDragula () {
		...
		this.dragula.on("drop", (el, target, source) => {
			...
			if (collection && item.collection.isCompatible(collection)) {
				let next = Mavo.Node.get(el.nextElementSibling?.closest(`[mv-list-item][property="${collection.property}"]`));
				let previous = Mavo.Node.get(el.previousElementSibling?.closest(`[mv-list-item][property="${collection.property}"]`));
