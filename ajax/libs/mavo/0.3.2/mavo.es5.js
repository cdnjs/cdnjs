...
(async function ($, $$) {
  ...
  let _ = self.Mavo = $.Class((_temp = _class = class Mavo {
    constructor(element) {
      ...
      if (this.primaryBackend) {
        ...
        requestAnimationFrame(() => {
          ...
          this.element.dispatchEvent(new Event("mv-load", {
            bubbles: true
          }));
        });
      } // Dynamic ids
    ...
    unobserve(o, callback) {
      ...
      return (_$observers2 = _.observers) === null || _$observers2 === void 0 ? void 0 : _$observers2.unobserve(options, callback);
    }
    ...
    updateBackend(role) {
      ...
      if (!backend) {
        backend = _.Functions.url("".concat(this.id, "-").concat(role));
      }
    ...
    async load({
      ...
      await Mavo.defer();
      this.dataLoaded.resolve();
      this.element.dispatchEvent(new CustomEvent("mv-load", {
        detail: backend,
        bubbles: true
      }));
    }
  ...
  }, _defineProperty(_class, "version", "v0.3.2"), _defineProperty(_class, "all", {}), _defineProperty(_class, "superKey", navigator.platform.indexOf("Mac") === 0 ? "metaKey" : "ctrlKey"), _defineProperty(_class, "base", ["blob:", "about:"].includes(location.protocol) ? ((_document$currentScri = document.currentScript) === null || _document$currentScri === void 0 ? void 0 : _document$currentScri.src) || "https://mavo.io" : location), _defineProperty(_class, "dependencies", [// Plugins.load() must be run after DOM load to pick up all mv-plugins attributes
  $.ready().then(() => _.Plugins.load())]), _defineProperty(_class, "defer", delay => new Promise(resolve => delay === undefined ? requestAnimationFrame(resolve) : setTimeout(resolve, delay))), _defineProperty(_class, "UI", {}), _defineProperty(_class, "hooks", new $.Hooks()), _defineProperty(_class, "properties", new Set()), _defineProperty(_class, "attributes", ["mv-app", "mv-storage", "mv-source", "mv-init", "mv-path", "mv-format", "mv-attribute", "mv-default", "mv-mode", "mv-edit", "mv-editor", "mv-permisssions", "mv-rel", "mv-value"]), _temp), {
    live: {
      inProgress(value) {
...
(function ($, $$) {
  var _ = $.extend(Mavo, {
    ...
    load: (url, base = (() => {
      ...
      return (_document$currentScri2 = (_document$currentScri3 = document.currentScript) === null || _document$currentScri3 === void 0 ? void 0 : _document$currentScri3.src) !== null && _document$currentScri2 !== void 0 ? _document$currentScri2 : location;
    })()) => {
    ...
    getProperAttributeCase(element, attribute) {
      ...
      (_$properlyCasedAttrib2 = (_$properlyCasedAttrib = _.properlyCasedAttributesCache)[root]) !== null && _$properlyCasedAttrib2 !== void 0 ? _$properlyCasedAttrib2 : _$properlyCasedAttrib[root] = {};
      ...
      let doc = new DOMParser().parseFromString("<".concat(root, "><").concat(tag, " ").concat(attribute, "=\"\"></").concat(tag, "></").concat(root, ">"), "text/html");
  ...
  let _ = Mavo.Plugins = {
    ...
    async load() {
      ...
      $$("[mv-plugins]").forEach(element => {
        element.getAttribute("mv-plugins").trim().split(/\s+/).forEach(plugin => {
          let [id, version] = plugin.split("@");

          _.plugins.add(id); // If an author requested several versions of the same plugin (with the same id),
      ...
      return Mavo.thenAll(plugin.filter(plugin => _.plugins.has(plugin.id)).map(async plugin => {
        ...
        if (plugin.repo) {
          ...
          url = "https://cdn.jsdelivr.net/gh/".concat(plugin.repo, "@").concat(version, "/").concat(filename); // Try to load the requested version of a plugin

          try {
            return await $.include(_.loaded[plugin.id], url);
          } catch (e) {
            // If there is no such version, fallback to the latest one (or the latest commit)
            url = "https://cdn.jsdelivr.net/gh/".concat(plugin.repo, "/").concat(filename);
          }
        } else {
          // Plugin hosted in the mavo-plugins repo
          url = "".concat(_.url, "/").concat(plugin.id, "/").concat(filename);
        }
...
(async function ($, $$) {
  var _ = Mavo.Primitive = class Primitive extends Mavo.Node {
      ...
      if (this.editor) {
        if (_.isFormControl(this.editor)) {
          if (this.editor.matches("select")) {
            ...
            let text = (_find = [...this.editor.options].find(o => Mavo.toArray(value).map(v => v.toString()).includes(o.value))) === null || _find === void 0 ? void 0 : _find.textContent; // We have a local editor, do we need to add/remove temp options?
    ...
    editorUpdated() {
      ...
      if ("placeholder" in this.editor && !this.editor.placeholder) {
        this.editor.placeholder = this.editor.type === "number" ? this.editor.min || 0 : "(".concat(this.label, ")");
      }
    ...
    edit(o = {}) {
      ...
      if (this.closestCollection && this.editType === "inline" && (_this$editor3 = this.editor) !== null && _this$editor3 !== void 0 && _this$editor3.matches(Mavo.selectors.textInput)) {
      ...
      if ((_this$editor4 = this.editor) !== null && _this$editor4 !== void 0 && _this$editor4.matches("select")) {
    ...
    if (node instanceof Mavo.Primitive && node.config && !node.pausedObserver) {
      if (attribute === "mv-default" && !node.defaultExpression) {
        ...
        if (Mavo.in("placeholder", node.editor)) {
          node.editor.placeholder = node.editor.type === "number" ? node.editor.min || 0 : "(".concat(node.label, ")");
        }
  ...
  let oldMvEdit = Mavo.attributeStartsWith("mv-edit-").filter(a => (a.name !== "mv-edit-type" || inputTypes.includes(a.value)) && !["mv-edit-as"].includes(a.name)).map(a => a.name);
...
(function ($, $$) {
  ...
  _.register({
    ...
    "meter": {
      ...
      datatype: "number",
      edit: function () {
        var _ref, _this$element$min, _ref2, _this$element$max, _ref3, _ref4, _this$element$step;

        let min = (_ref = (_this$element$min = this.element.min) !== null && _this$element$min !== void 0 ? _this$element$min : this.element.getAttribute("min")) !== null && _ref !== void 0 ? _ref : 0;
        let max = (_ref2 = (_this$element$max = this.element.max) !== null && _this$element$max !== void 0 ? _this$element$max : this.element.getAttribute("max")) !== null && _ref2 !== void 0 ? _ref2 : 1;
        min = +min;
        max = +max;
        let range = max - min;
        let step = (_ref3 = (_ref4 = (_this$element$step = this.element.step) !== null && _this$element$step !== void 0 ? _this$element$step : this.element.getAttribute("step")) !== null && _ref4 !== void 0 ? _ref4 : this.element.getAttribute("mv-editor-step")) !== null && _ref3 !== void 0 ? _ref3 : range > 1 ? 1 : range / 100;
        step = +step;
    ...
    "time": {
      ...
      init: function () {
        if (!this.fromTemplate("dateType")) {
          ...
          if (!dateFormat) {
            var _this$config$defaultF, _this$config$defaultF2, _this$config$defaultF3;

            // TODO what about mv-expressions?
            this.element.textContent = (_this$config$defaultF = (_this$config$defaultF2 = (_this$config$defaultF3 = this.config.defaultFormats)[this.dateType]) === null || _this$config$defaultF2 === void 0 ? void 0 : _this$config$defaultF2.call(_this$config$defaultF3, this.property)) !== null && _this$config$defaultF !== void 0 ? _this$config$defaultF : "";
...
(function ($, $$) {
  ...
  var _ = Mavo.Collection = class Collection extends Mavo.Node {
    ...
    isCompatible(c) {
      return c && this.itemTemplate.constructor == c.itemTemplate.constructor && (c === this || c.template == this || this.template == c || this.template && this.template == c.template || c.accepts.has(this.property));
...
(function ($, $$) {
  ...
  var _ = Mavo.DOMExpression = $.Class({
    async constructor(o = {}) {
      ...
      if (/^mv-(value$|attr-)/.test(this.attribute)) {
        ...
        if (this.attribute == "mv-value") {
          ...
          this.attribute = this.attribute.replace("mv-attr-", ""); // Get proper attribute case if in a case sensitive environment
        ...
        (_this$fallback = this.fallback) !== null && _this$fallback !== void 0 ? _this$fallback : this.fallback = Mavo.Primitive.getValue(this.element, {
          attribute: this.attribute
        });
    ...
    update: function (o) {
      ...
      if (!changed && !(o !== null && o !== void 0 && o.force)) {
...
(function ($, $$) {
  ...
  var _ = Mavo.Expressions = $.Class({
    ...
    extract: function (node, attribute, path, syntax = Mavo.Expression.Syntax.default) {
      let attributeName = attribute === null || attribute === void 0 ? void 0 : attribute.name;

      if (_.directives.some(d => {
        var _d$test;

        return ((_d$test = d.test) === null || _d$test === void 0 ? void 0 : _d$test.call(d, attributeName)) || d === attributeName;
    ...
    traverse: function (node, path = [], syntax) {
      ...
      if (node.nodeType === 3) {
        ...
        let ignoredAttributes = new Set([// Globally ignored attributes (for all elements)
        ..._.skip, // Locally ignored attributes (for this element)
        ...((_node$getAttribute$tr = (_node$getAttribute = node.getAttribute("mv-expressions-ignore")) === null || _node$getAttribute === void 0 ? void 0 : _node$getAttribute.trim().split(/\s*,\s*/)) !== null && _node$getAttribute$tr !== void 0 ? _node$getAttribute$tr : [])]);
        let specifiedAttributes = new Set(node.getAttributeNames()); // Remove ignored attributes
  ...
  let $u = {
    ...
    deprecatedFunction(name, oldName, fn) {
      return function (...args) {
        ...
        (_fn = fn) !== null && _fn !== void 0 ? _fn : fn = Mavo.Functions[name];
        Mavo.warn("".concat(oldName, "() is deprecated and will be removed in the next version of Mavo. Please use ").concat(name, "() instead."));
        return fn(...args);
      };
    }
  ...
  let _ = Mavo.Functions = {
    ...
    url: (id, ...options) => {
      ...
      if (id) {
        ...
        if (type === "path" || !type) {
          let path = url.pathname.split("/"); // FIXME support multiple here?
          ...
          if (index > -1) {
            ...
            let ret = (_path = path[index + 1]) !== null && _path !== void 0 ? _path : "";
    ...
    sort(list, by = list, ...options) {
      ...
      options = Object.assign({}, ...options);
      let collatorOptions = Object.assign({
        numeric: true
      }, options);
      ...
      let desc = (_options$order = options.order) === null || _options$order === void 0 ? void 0 : _options$order.startsWith("desc");
    ...
    phrase: $.extend(function (id, vars, lang) {
      ...
      let locale = lang ? Mavo.Locale.get(lang) : (_this$Mavo$mavo$local = this === null || this === void 0 ? void 0 : (_this$Mavo$mavo = this[Mavo.mavo]) === null || _this$Mavo$mavo === void 0 ? void 0 : _this$Mavo$mavo.locale) !== null && _this$Mavo$mavo$local !== void 0 ? _this$Mavo$mavo$local : Mavo.Locale.default;
...
(function ($, val, _, $u = _.util) {
  ...
  $.extend(_, {
    ...
    datetime: $.extend((date, time, precision) => {
      ...
      (_precision2 = precision) !== null && _precision2 !== void 0 ? _precision2 : precision = "minutes";
      ...
      if (separateTime) {
        ...
        ret += Mavo.value(time) ? "T".concat(_.time(time, precision)) : "";
      } else {
        ret += "T".concat(_.time(date, precision));
      }
  ...
  _.duration = $.extend(function (ms, terms) {
    ...
    if (terms && isNaN(terms)) {
      ...
      if (!(unitPlural in s)) {
        throw new TypeError("Unknown duration unit ".concat(terms, ". Please use one of ").concat(Object.keys(s).join(", ")));
      }
  ...
  var _ = Mavo.Script = {
    $fn: self.Proxy ? new Proxy({
      ...
      get: (data, property) => {
        ...
        var propertyL = (_property$toLowerCase = property === null || property === void 0 ? void 0 : (_property$toLowerCase2 = property.toLowerCase) === null || _property$toLowerCase2 === void 0 ? void 0 : _property$toLowerCase2.call(property)) !== null && _property$toLowerCase !== void 0 ? _property$toLowerCase : property,
  ...
  let _ = Mavo.Actions = {
    ...
    run: (code, element, evt) => {
      if (code) {
        ...
        if (node) {
          let expression = new Mavo.Expression(code, {
            actions: true
          });
    ...
    nope: () => {
      let actions = Object.keys(_.Functions).map(name => "".concat(name, "()"));
    ...
    Functions: {
      ...
      move: (from, to, index) => {
        ...
        if ($.type(to) == "number" && !((_toNode = toNode) !== null && _toNode !== void 0 && _toNode.collection)) {
          // If to is a number and not a collection item, it's an index
          [index, to] = [to];
          toNode = undefined;
        }

        let fromNodes = Mavo.toArray(from).map(_.getNode).filter(n => n === null || n === void 0 ? void 0 : n.closestCollection);
    ...
    async put(serialized, path = this.path, o = {}) {
      ...
      if (!repoInfo || repoInfo.owner.login !== this.username || repoInfo.name !== this.repo) {
        ...
        try {
          ...
          (_this$branch = this.branch) !== null && _this$branch !== void 0 ? _this$branch : this.branch = repoInfo.default_branch;
        } catch (e) {
          if (e.status === 404) {
            var _this$options$private;

            // Create repo if it doesn’t exist
            repoInfo = this.repoInfo = await this.request("user/repos", {
              name: this.repo,
              private: !!((_this$options$private = this.options.private) !== null && _this$options$private !== void 0 ? _this$options$private : o.private)
            }, "POST");
          }
        }
      }
      ...
      if (!this.canPush()) {
        ...
        let forkInfo = await this.request("".concat(repoCall, "/forks"), {
          name: this.repo
        }, "POST");
        fileCall = "repos/".concat(forkInfo.full_name, "/contents/").concat(path);
        this.forkInfo = forkInfo; // Ensure that fork is created (they take a while)

        let fetchedForkInfo;

        do {
          await delay(1000); // If we can get a list of commits, the fork is created

          fetchedForkInfo = await this.request("repos/".concat(forkInfo.full_name, "/commits"), {
        ...
        if (this.user) {
          ...
          if (this.repo) {
            return this.request("repos/".concat(this.username, "/").concat(this.repo)).then(repoInfo => {
              ...
              (_this$branch2 = this.branch) !== null && _this$branch2 !== void 0 ? _this$branch2 : this.branch = repoInfo.default_branch;
