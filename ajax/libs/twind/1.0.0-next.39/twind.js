...
function mql(screen, prefix = '@media ') {
    return prefix + asArray(screen).map((screen)=>{
        ...
        return screen.raw || Object.keys(screen).map((feature)=>`(${feature}-width:${screen[feature]})`).join(' and ');
    }).join(',');
}
...
/* d: 16 */ function pseudoPrecedence(selector) {
...
/** The name to use for `&` expansion in selectors. Maybe empty for at-rules like `@import`, `@font-face`, `@media`, ... */ /** The calculated precedence taking all variants into account. */ /** The rulesets (selectors and at-rules). expanded variants `@media ...`, `@supports ...`, `&:focus`, `.dark &` */ /** Is this rule `!important` eg something like `!underline` or `!bg-red-500` or `!red-500` */ function convert({ n: name , i: important , v: variants = []  }, context, precedence, conditions) {
    ...
    for (const variant of variants){
        ...
        for (const condition of asArray(screen && mql(screen) || context.v(variant))){
            ...
            precedence |= screen ? 1 << 26 | /* Shifts.screens */ atRulePrecedence(condition) : variant == 'dark' ? 1 << 30 : /* Shifts.darkMode */ condition[0] == '@' ? atRulePrecedence(condition) : pseudoPrecedence(condition);
        }
    }
    return {
        n: name,
...
function stringify$1(rule) {
    if (rule.d) {
        const groups = [];
        const selector = replaceEach(// merge all conditions into a selector string
...
function translate$(rule, context, precedence, conditions, important) {
    rule = {
        ...rule,
        i: rule.i || important
    };
    ...
    if (typeof resolved == 'string') {
        ...
        return merge(translate(parse$1(resolved), context, precedence, conditions, rule.i), rule.n);
    }
...
function translateWith(name, layer, rules, context, precedence, conditions, important, useOrderOfRules) {
    return merge((useOrderOfRules ? rules.flatMap((rule)=>translate([
            rule
        ], context, precedence, conditions, important)) : translate(rules, context, precedence, conditions, important)).map((rule)=>// do not move defaults
...
function serialize$(style, { n: name , p: precedence , r: conditions = [] , i: important  }, context) {
    ...
    for(let key in style || {}){
        ...
        if (key[0] == '@') {
            ...
            // @apply ...;
            if (key[1] == 'a') {
                rules.push(...translateWith(name, precedence, parse$1('' + value), context, precedence, conditions, important, true));
                continue;
            }
            // @layer <layer>
            if (key[1] == 'l') {
                for (const css of asArray(value)){
            ...
            if (key[0] == '@' || key.includes('&')) {
                ...
                if (key[0] == '@') {
                    ...
                    key = key.replace(/\bscreen\(([^)]+)\)/g, (_, screenKey)=>{
                        ...
                        if (screen) {
                            rulePrecedence |= 1 << 26;
                            /* Shifts.screens */ return mql(screen, '');
                        }
                        return _;
                    });
...
function astish(strings, interpolations) {
    return Array.isArray(strings) ? astish$(interleave(strings, interpolations, (interpolation)=>interpolation != null && typeof interpolation != 'boolean' ? interpolation : '')) : typeof strings == 'string' ? astish$(strings) : [
        strings
    ];
}
...
function css(strings, ...interpolations) {
    ...
    return register(className, (rule, context)=>merge(ast.flatMap((css)=>serialize(css, rule, context, Layer.o)), className));
}

const animation = /* #__PURE__ */ new Proxy(function animation(animation1, waypoints) {
    return animation$('animation', animation1, waypoints);
}, {
    get (target, name) {
        if (name in target) return target[name];
        return function namedAnimation(animation, waypoints) {
...
function cx(strings, ...interpolations) {
    return format(parse$1(interpolate(strings, interpolations)), ' ');
}
...
function createContext({ theme , darkMode , darkColor , variants , rules , hash: hash$1 , stringify , ignorelist  }) {
    ...
    variants.push([
        'dark',
        Array.isArray(darkMode) || darkMode == 'class' ? `${asArray(darkMode)[1] || '.dark'} &` : typeof darkMode == 'string' && darkMode != 'media' ? darkMode : // a custom selector
        '@media (prefers-color-scheme:dark)'
    ]);
    const h = typeof hash$1 == 'function' ? (value)=>hash$1(value, hash) : hash$1 ? hash : identity;
    return {
    ...
    return Object.defineProperties(function tw(tokens) {
        if (!cache.size) {
            for (let preflight of asArray(config.preflight)){
                ...
                if (preflight) {
                    (typeof preflight == 'string' ? translateWith('', Layer.b, parse$1(preflight), context, Layer.b, [], false, true) : serialize(preflight, {}, context, Layer.b)).forEach(insert);
                }
            }
        }
        tokens = '' + tokens;
        ...
        if (!className) {
            ...
            for (const rule of translate(parse$1(tokens), context)){
...
function observe(tw$1 = tw, target = typeof document != 'undefined' && document.documentElement) {
...
function dom(element) {
    ...
    return {
        ...
        snapshot () {
            // collect current rules
            const rules = Array.from(target.childNodes, (node)=>node.textContent);
            return ()=>{
                // remove all existing rules
                this.clear();
                // add all snapshot rules back
 ...
 */ const tw = /* #__PURE__ */ new Proxy(// just exposing the active as tw should work with most bundlers
...
noop, {
    ...
    get (target, property) {
        const value = (active || target)[property];
        if (typeof value === 'function') {
 ...
 */ function setup(config = {}, sheet = getSheet, target) {
    active?.destroy();
    active = observe(twind(config, typeof sheet == 'function' ? sheet() : sheet), target);
    return active;
}
...
function nested(marker) {
    ...
    function nested$(name, strings, interpolations) {
        return format(parse$1(name + marker + '(' + interpolate(strings, interpolations) + ')'));
    }
}

/** The found theme value */ function fromTheme(/** Theme section to use (default: `$1` — The first matched group) */ section, /** The css property (default: value of {@link section}) */ resolve, convert) {
    ...
    return (match, context)=>{
        const themeSection = camelize(section || match[1]);
...
/** Theme section to use (default: `$0.replace('-', 'Color')` — The matched string with `Color` appended) */ /** The css property (default: value of {@link section}) */ /** `--tw-${$0}opacity` -> '--tw-text-opacity' */ /** `section.replace('Color', 'Opacity')` -> 'textOpacity' */ function colorFromTheme(options = {}, resolve) {
    return (match, context)=>{
        // text- -> textColor
        // ring-offset(?:-|$) -> ringOffsetColor
        const { section =camelize(match[0]).replace('-', '') + 'Color'  } = options;
        ...
        opacityVariable =`--tw-${match[0].replace(/-$/, '')}-opacity` , opacitySection =section.replace('Color', 'Opacity') , property =section , selector ,  } = options;
...
function normalize(value) {
    ...
    return value.// Convert `_` to ` `, except for escaped underscores `\_`
...
function createStyle(config = {}, parent) {
    ...
    function register(mq, token, layer) {
        ...
        (parent ? parent.className.replace(/#.+$/, '~') : '') + label + mq + id).replace(/[: ,()[\]]/, ''), layer, token && parse$1(token));
    }
    return Object.defineProperties(function style(allProps) {
        ...
        for(const variantKey in variants){
            ...
            if (propsValue === Object(propsValue)) {
                ...
                if (token) {
                    classNames += ' ' + register('--' + variantKey + '-' + mq, token, 0b011 << 27);
                }
            } else /* Shifts.layer */ if (token = variant[propsValue]) {
                classNames += ' ' + register('--' + variantKey + '-' + propsValue, token, 0b011 << 27);
            }
        }
        /* Shifts.layer */ when.forEach((match, index)=>{
            let mq = '';
            ...
            if (mq && (token = match[1])) {
                classNames += ' ' + register('-' + index + '--' + mq, token, 0b100 << 27);
            }
        });
        /* Shifts.layer */ return classNames;
    }, Object.getOwnPropertyDescriptors({
        className,
        defaults,
...
export { animation, apply, arbitrary, asArray, auto, autoDarkColor, colorFromTheme, consume, css, cssom, cx, defineConfig, dom, escape, extract, fromTheme, getSheet, hash, identity, injectGlobal, inline, install, keyframes, mql, noop, normalize, observe, setup, shortcut, stringify, style, toCSS, toColorValue, tw, twind, tx, virtual };
