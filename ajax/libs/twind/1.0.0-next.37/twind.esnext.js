...
function resolve(rule, context) {
    ...
    return factory ? factory(rule, context) : context.r(rule.n, rule.v[0] == 'dark');
}
 ...
 */ function autoDarkColor(section, key, { theme  }) {
    ...
    key = key.replace(/\d+$/, (shade)=>// ~~(parseInt(shade, 10) / 100): 50 -> 0, 900 -> 9
        // (9 - 0) -> 900, (9 - 9) -> 50
        ((9 - ~~(parseInt(shade, 10) / 100) || 0.5) * 100)
    );
...
function resolveThemeFunction(value3, context) {
    ...
    return value3.replace(/theme\((["'`])?(.+?)\1(?:\s*,\s*(["'`])?(.+?)\3)?\)/g, (_, __, key, ___, defaultValue)=>{
...
function createContext({ theme , darkMode , darkColor , variants , rules , hash: hash$1 , stringify , ignorelist  }) {
    ...
    const h = typeof hash$1 == 'function' ? (value)=>hash$1(value, hash)
    ...
    return {
        ...
        r (className, isDark) {
            ...
            if (!ruleCache.has(key)) {
                ruleCache.set(key, // TODO console.warn(`[twind] unknown rule "${value}"`),
                !ignored(className, this) && find(className, rules, ruleResolvers, getRuleResolver, this, isDark));
            }
...
function defineConfig({ presets =[] , ...userConfig }) {
    ...
    for (const preset of asArray([
        ...
        const { preflight , darkMode =config.darkMode , darkColor =config.darkColor , theme , variants , rules , hash =config.hash , ignorelist , stringify =config.stringify ,  } = typeof preset == 'function' ? preset(config) : preset;
        config = {
...
function twind(userConfig, sheet) {
    ...
    function insert(rule) {
        ...
        const cssText = stringify$1(name ? {
            ...rule,
            n: name
        } : rule);
    ...
    function handleMutationRecords(records) {
        for (const { type , target  } of records){
            if (type[0] == 'a' /* attribute */ ) {
                ...
                /* childList */ // some nodes have been added — find all with a class attribute
                for (const el of target.querySelectorAll('[class]')){
...
function virtual(includeResumeData) {
    ...
    return {
        ...
        insert (css, index, rule) {
            target.splice(index, 0, includeResumeData ? `/*!${rule.p.toString(36)},${(rule.o * 2).toString(36)}${rule.n ? ',' + rule.n : ''}*/${css}` : css);
        },
        resume: noop
    };
}
...
function resume(addClassName, insert) {
    ...
    if (RE.test(textContent)) {
        ...
        while((function commit(match) {
            if (lastMatch) {
                insert(// grep the cssText from the previous match end up to this match start
 ...
 */ function consume(markup, tw$1 = tw) {
    ...
    extract$1(markup, (startIndex, endIndex, quote)=>{
        ...
        const token = quote == `"` ? value.replace(/(\[)&#x27;|&#x27;(])/g, `$1'$2`) : quote == `'` ? value.replace(/(\[)&quot;|&quot;(])/g, `$1"$2`) : value;
        const className = tw$1(token);
...
function install(config, isProduction) {
...
function colorFromTheme(options1 = {}, resolve) {
    return (match1, context)=>{
        ...
        const { section =camelize(match1[0]).replace('-', '') + 'Color'  } = options1;
        ...
        opacityVariable =`--tw-${match1[0].replace(/-$/, '')}-opacity` , opacitySection =section.replace('Color', 'Opacity') , property =section , selector ,  } = options1;
        ...
        let properties1 = create(match1, context);
        ...
        if (!match1.dark) {
            ...
            if (darkColorValue && darkColorValue !== colorValue) {
                ...
                properties1 = {
                    '&': properties1,
                    [context.v('dark')]: create(match1, context)
                };
            }
        }
        return properties1;
...
export { animation, apply, arbitrary, asArray, auto, autoDarkColor, colorFromTheme, consume, css, cssom, cx, defineConfig, dom, escape, extract, fromTheme, getSheet, hash, identity, injectGlobal, inline, install, keyframes, mql, noop, observe, setup, shortcut, stringify, style, toColorValue, tw, twind, tx, virtual };
