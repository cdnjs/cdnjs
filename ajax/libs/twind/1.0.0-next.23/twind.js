...
function css(strings, ...interpolations) {
    const ast = Array.isArray(strings) ? astish(interleave(strings, interpolations, (interpolation)=>interpolation != null && typeof interpolation != 'boolean' ? interpolation : ''
 ...
 */ function astish(css2) {
    css2 = removeComments(css2);
    const rules = [];
    const tree = [];
    let block1;
    while(block1 = newRule.exec(css2)){
        ...
        if (block1[3]) {
            ...
            rules.push(tree.reduceRight((css3, block)=>({
                    [block]: css3
                })
            , {
                [block1[1]]: block1[2]
...
export { apply, arbitrary, asArray, auto, colorFromTheme, consume, css, cssom, cx, defineConfig, dom, escape, extract, fromTheme, hash, identity, inline, mql, noop, observe, setup, shortcut, stringify, style, toColorValue, tw, twind, tx, virtual };
