    ...
    ***************************************************************************** */
    ...
    function fromAjax(config) {
        return new Observable(function (destination) {
            var _a, _b;
            var params = config.params, configuredBody = config.body, configuredHeaders = config.headers, remainingConfig = __rest(config, ["params", "body", "headers"]);
            ...
            if (params) {
                ...
                if (url.includes('?')) {
                    ...
                    new URLSearchParams(params).forEach(function (value, key) { return searchParams_1.set(key, value); });
                    url = parts[0] + '?' + searchParams_1;
                }
                else {
                    searchParams_1 = new URLSearchParams(params);
