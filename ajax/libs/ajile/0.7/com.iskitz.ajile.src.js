...
| Modified: Thursday,  September 28, 2006    [2006.09.28 - 05:30:50 EDT]
|+-----------------------------------------------------------------------------+
|
...
//continue paths;
   ...
   function handleImport(shortName, fullName, url, notation, version, module, owner)
   {
      ...
      addAsSupporter(fullName, (shortName == '*' ? fullName : shortName));
   ...
   function handleImportLoaded(shortName, fullName, module, owner)
   {
      ...
      if((shortName != IMPORTED) && hasNamingConflict(shortName, fullName, owner))
      {
...
//            if(!processed.has($fullName)                    continue;
            if(hasNamingConflict(member, $fullName, owner)) continue;
   ...
   *--------------------------------------------------------------------------*/
   function ImportAs(shortName, fullName, url, notation, owner)
   {
      ...
      if((importInfo = getImportInfo(shortName, fullName)))
      {
         ...
         shortName = (shortName != IMPORTED) ? importInfo[0] : IMPORTED;
      ...
      if((i >= j && shortName != '*'))// || ((shortName == IMPORTED) && (i == j-2)))
      {
      ...
      {
         if(shortName == '*' || shortName == IMPORTED) shortName = fullName;
...
//            updateUsers(fullName);
      ...
      if(!(dependencies && (dependencies = dependencies.getAll())))
