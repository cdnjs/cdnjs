...
| Modified: Wednesday, August     2, 2006    [2006.08.02 - 20:03:06 EDT]
|+-----------------------------------------------------------------------------+
|
   ...
   *--------------------------------------------------------------------------*/
   ...
   function AddImportListener(moduleName, listener)
   {
      ...
      if(  ( moduleName && GetModule(moduleName))
        || (!moduleName && pendingImports.getSize() == 0))
         return setTimeout(listener, 62.25);
...
//continue paths;
   ...
   *--------------------------------------------------------------------------*/
   ...
   function ImportThread(fullName, ttl)
   {
      ...
      function start()
      {
         if(timesChecked >= 500) return stop();
...
//       log("LoadCode [ " + code + " ]...DONE!", arguments);
   ...
   | loadController
   |+--------------------------------------------------------------------------+
   | Attempts to load the current page's JavaScript controller (MVC) file.
   *--------------------------------------------------------------------------*/
   function loadController()
   {
      ...
      var iDot = name.indexOf('\x2e', ++iEnd);
      iDot     = (iDot == -1) ? iEnd : iDot;

      if((name = name.substring(iEnd, iDot)) == "")
         name  = CONTROLLER;
      else
         CONTROLLER = name;
  
      var code = STEALTH+'\x28"'+CONTROLLER+'"\x29';
   ...
   *--------------------------------------------------------------------------*/
   ...
   function RemoveImportListener(moduleName, listener)
   {
      ...
      var listenerList = importListeners.get((moduleName || ''));

      if(!listenerList) return false;
      
      var listeners = listenerList.getAll();
