...
(function/*com.iskitz.ajile*/(VERSION, undefined)
{
   ...
   function AddImportListener(moduleName, listener)
   {
      ...
      (!moduleName && this != window [ALIAS]) && (moduleName = INTERNAL);

      var $listener = listener;

      if (moduleName && (processed.has (moduleName) || GetModule (moduleName)))
      {
         window.setTimeout (function onNamedImport (moduleName) { $listener (moduleName); }, 0);
         return true;
      }
      ...
      if (!moduleName)
         if (processed.getSize() > 0 || pendingImports.getSize() == 0)
            for (var importee in processed.getAll())
               window.setTimeout (function onAnyImport() { $listener (importee); }, 0);
...
paths:for(var path in paths)
   ...
   function handleImported(shortName, fullName, module, owner)
   {
      ...
      (shortName != fullName)
      &&    (importListeners.has('') || importListeners.has (shortName))
      &&    notifyImportListeners (shortName);

      (importListeners.has('') || importListeners.has (fullName))
   ...
   function ImportThread (fullName, ttl, maxCheckCount)
   {
      ...
      function stop()
      {
         (undefined != threadID)     && window.clearTimeout  (threadID);
         (undefined != terminatorID) && window.clearInterval (terminatorID);
      }
