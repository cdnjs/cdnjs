...
new function /*com.iskitz.ajile*/()
{
   ...
   {
      cloakObject((THIS = instance).constructor);

      if(!(INFO = getNamespaceInfo(QNAME)))
         INFO = new NamespaceInfo();
   ...
   function cloak(moduleName)
   {
      ...
      if (module)
         cloakObject((typeof module == "function" ? module : module.constructor));
...
paths:for(var path in paths)
   ...
   function ImportThread(fullName, ttl, maxCheckCount)
   {
      ...
      function $ImportThread(THIS)
      {
         ...
         ttl           = ttl || 60000;
         terminatorID  = window.setInterval(stop, ttl);
         THIS.start    = start;
         THIS.stop     = stop;
      ...
      if(this.constructor != ImportThread)   // FBI - Fix Broken Instantiation
...
*/
   ...
   function LoadSimple(src, container, code, defer, title, type, language)
   {
      ...
      if(src)
      {
//alert("LoadSimple():: src="+src+", code="+code);
      ...
      var scriptTag = '<'+"script"
                    ...
                    + "<\/script>\n";
/*
      if(HEAD && typeof HEAD.innerHTML != "undefined")   // DEBUG - Might be read-only!!!
      {
         var headHTML;

         if((headHTML = HEAD.innerHTML) == (HEAD.innerHTML += scriptTag))
            container.write(scriptTag);  // HEAD.innerHTML is read-only!
      }
      else
*/
   ...
   function log(message, _caller, showLog)
   {
      ...
      if(typeof message != "undefined")
      {
         var _LOG = LOG;
         var now  = new Date();
         LOG      = [now.getFullYear(), now.getMonth(),   now.getDate()].join('.') + ','
   ...
   function NamespaceInfo(path, notation, fullName, shortName, version, extension, options)
   {
      ...
      if(this.constructor != NamespaceInfo)   //FBI: Fix broken instantiations
         return new NamespaceInfo( path,     notation,  fullName, shortName
   ...
   function SimpleSet()
   {
      ...
      if(this.constructor != SimpleSet)   // If creating via mySet = SimpleSet()
         return new SimpleSet();          // guarantee proper instantiation.
   ...
   var currentModuleName = QNAME
     ...
     , modulePaths       = /*new */SimpleSet()  // Test improper instantiation
