...
(function/*com.iskitz.ajile*/(VERSION, global, undefined)
{
   ...
   function AddImportListener (moduleName, listener, notifyOriginal)        // API: ajile 1.6.1...1.7.3
   {
      ensureFailSafe();
   ...
   function AddImportListener$handleGroup (names, notify, notified) {
      function isGroupImported (listener) {
         for (var item, items, name, i=0, j=names.length; i < j; i++) {
            name = names[i];
            item = GetModule (name);
   ...
   function destroyModules (fullName)
   {
      ...
      var module = fullName ? GetModule(fullName) : shortName ? global : undefined;
      ...
      cloakModule (fullName||shortName);
   }
...
paths:for(var path in paths)
   ...
   {
      LEGACY && (notation = notation === false ? '\x2f' : '\x2e');          // API: JSPackaging 2.0+
   ...
   function handleImported (shortName, fullName, module, owner) {
      ...
      if (!isSupported (fullName, shortName)) return undefined;
   ...
   function ImportAs(shortName, fullName, url, notation, owner)
   {
      ...
      if (pendingImports.has (fullName)) {
         (shortName == '*' || shortName == LOADED) && (shortName = fullName);
...
//         return isInlineImportReady (fullName, shortName);
   ...
   function Listener (map) {
      if (!map || ("function" != typeof map.notify))
         return;
      this.error  = undefined;
      this.name   = map.name || GENERIC;
      this.item   = map.item;
   ...
   function RemoveImportListener (moduleName, listener)
   {
      ...
      for(var callback=listener, listeners, i=listenerList.length; --i >= 0;) {
         if (!listenerList[i]) continue;
         ...
         for (var id in listeners)
            if ("undefined" == typeof Object.prototype[id]) {
               ...
               delete listener.notifyOriginal;

               for (var notified=listener.notified, n=0, nl=notified.length; n < nl; n++) {
