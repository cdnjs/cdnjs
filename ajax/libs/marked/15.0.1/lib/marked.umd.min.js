...
`}strong({tokens:e}){return`<strong>${this.parser.parseInline(e)}</strong>`}em({tokens:e}){return`<em>${this.parser.parseInline(e)}</em>`}codespan({text:e}){return`<code>${z(e,!0)}</code>`}br(e){return"<br>"}del({tokens:e}){return`<del>${this.parser.parseInline(e)}</del>`}link({href:e,title:t,tokens:n}){var n=this.parser.parseInline(n),r=v(e);if(null===r)return n;let s='<a href="'+(e=r)+'"';return t&&(s+=' title="'+z(t)+'"'),s+=">"+n+"</a>"}image({href:e,title:t,text:n}){var r=v(e);if(null===r)return z(n);let s=`<img src="${e=r}" alt="${n}"`;return t&&(s+=` title="${z(t)}"`),s+=">"}text(e){return"tokens"in e&&e.tokens?this.parser.parseInline(e.tokens):"escaped"in e&&e.escaped?e.text:z(e.text)}}class L{strong({text:e}){return e}em({text:e}){return e}codespan({text:e}){return e}del({text:e}){return e}html({text:e}){return e}text({text:e}){return e}link({text:e}){return""+e}image({text:e}){return""+e}br(){return""}}class B{options;renderer;textRenderer;constructor(e){this.options=e||n.defaults,this.options.renderer=this.options.renderer||new I,this.renderer=this.options.renderer,this.renderer.options=this.options,(this.renderer.parser=this).textRenderer=new L}static parse(e,t){const n=new B(t);return n.parse(e)}static parseInline(e,t){const n=new B(t);return n.parseInline(e)}parse(n,r=!0){let s="";for(let t=0;t<n.length;t++){var e=n[t];if(this.options.extensions?.renderers?.[e.type]){var i=e,l=this.options.extensions.renderers[i.type].call({parser:this},i);if(!1!==l||!["space","hr","heading","code","table","blockquote","list","html","paragraph","text"].includes(i.type)){s+=l||"";continue}}var a=e;switch(a.type){case"space":s+=this.renderer.space(a);continue;case"hr":s+=this.renderer.hr(a);continue;case"heading":s+=this.renderer.heading(a);continue;case"code":s+=this.renderer.code(a);continue;case"table":s+=this.renderer.table(a);continue;case"blockquote":s+=this.renderer.blockquote(a);continue;case"list":s+=this.renderer.list(a);continue;case"html":s+=this.renderer.html(a);continue;case"paragraph":s+=this.renderer.paragraph(a);continue;case"text":{var o;let e=this.renderer.text(a);for(;t+1<n.length&&"text"===n[t+1].type;)o=n[++t],e+="\n"+this.renderer.text(o);s+=r?this.renderer.paragraph({type:"paragraph",raw:e,text:e,tokens:[{type:"text",raw:e,text:e,escaped:!0}]}):e;continue}default:var c='Token with "'+a.type+'" type was not found.';if(this.options.silent)return console.error(c),"";throw new Error(c)}}return s}parseInline(t,n=this.renderer){let r="";for(let e=0;e<t.length;e++){var s=t[e];if(this.options.extensions?.renderers?.[s.type]){var i=this.options.extensions.renderers[s.type].call({parser:this},s);if(!1!==i||!["escape","html","link","image","strong","em","codespan","br","del","text"].includes(s.type)){r+=i||"";continue}}var l=s;switch(l.type){case"escape":r+=n.text(l);break;case"html":r+=n.html(l);break;case"link":r+=n.link(l);break;case"image":r+=n.image(l);break;case"strong":r+=n.strong(l);break;case"em":r+=n.em(l);break;case"codespan":r+=n.codespan(l);break;case"br":r+=n.br(l);break;case"del":r+=n.del(l);break;case"text":r+=n.text(l);break;default:var a='Token with "'+l.type+'" type was not found.';if(this.options.silent)return console.error(a),"";throw new Error(a)}}return r}}class C{options;block;constructor(e){this.options=e||n.defaults}static passThroughHooks=new Set(["preprocess","postprocess","processAllTokens"]);preprocess(e){return e}postprocess(e){return e}processAllTokens(e){return e}provideLexer(){return this.block?_.lex:_.lexInline}provideParser(){return this.block?B.parse:B.parseInline}}class M{defaults=e();options=this.setOptions;parse=this.parseMarkdown(!0);parseInline=this.parseMarkdown(!1);Parser=B;Renderer=I;TextRenderer=L;Lexer=_;Tokenizer=S;Hooks=C;constructor(...e){this.use(...e)}walkTokens(e,t){let n=[];for(const s of e)switch(n=n.concat(t.call(this,s)),s.type){case"table":var r=s;for(const i of r.header)n=n.concat(this.walkTokens(i.tokens,t));for(const l of r.rows)for(const a of l)n=n.concat(this.walkTokens(a.tokens,t));break;case"list":r=s;n=n.concat(this.walkTokens(r.items,t));break;default:{const o=s;this.defaults.extensions?.childTokens?.[o.type]?this.defaults.extensions.childTokens[o.type].forEach(e=>{e=o[e].flat(1/0);n=n.concat(this.walkTokens(e,t))}):o.tokens&&(n=n.concat(this.walkTokens(o.tokens,t)))}}return n}use(...e){const w=this.defaults.extensions||{renderers:{},childTokens:{}};return e.forEach(e=>{const t={...e};if(t.async=this.defaults.async||t.async||!1,e.extensions&&(e.extensions.forEach(n=>{if(!n.name)throw new Error("extension name required");if("renderer"in n){const r=w.renderers[n.name];r?w.renderers[n.name]=function(...e){let t=n.renderer.apply(this,e);return t=!1===t?r.apply(this,e):t}:w.renderers[n.name]=n.renderer}if("tokenizer"in n){if(!n.level||"block"!==n.level&&"inline"!==n.level)throw new Error("extension level must be 'block' or 'inline'");const e=w[n.level];e?e.unshift(n.tokenizer):w[n.level]=[n.tokenizer],n.start&&("block"===n.level?w.startBlock?w.startBlock.push(n.start):w.startBlock=[n.start]:"inline"===n.level&&(w.startInline?w.startInline.push(n.start):w.startInline=[n.start]))}"childTokens"in n&&n.childTokens&&(w.childTokens[n.name]=n.childTokens)}),t.extensions=w),e.renderer){const i=this.defaults.renderer||new I(this.defaults);for(const l in e.renderer){if(!(l in i))throw new Error(`renderer '${l}' does not exist`);if(!["options","parser"].includes(l)){var n=l;const a=e.renderer[n],o=i[n];i[n]=(...e)=>{let t=a.apply(i,e);return(t=!1===t?o.apply(i,e):t)||""}}}t.renderer=i}if(e.tokenizer){const c=this.defaults.tokenizer||new S(this.defaults);for(const h in e.tokenizer){if(!(h in c))throw new Error(`tokenizer '${h}' does not exist`);if(!["options","rules","lexer"].includes(h)){var r=h;const p=e.tokenizer[r],u=c[r];c[r]=(...e)=>{let t=p.apply(c,e);return t=!1===t?u.apply(c,e):t}}}t.tokenizer=c}if(e.hooks){const g=this.defaults.hooks||new C;for(const k in e.hooks){if(!(k in g))throw new Error(`hook '${k}' does not exist`);if(!["options","block"].includes(k)){var s=k;const d=e.hooks[s],f=g[s];C.passThroughHooks.has(k)?g[s]=e=>{if(this.defaults.async)return Promise.resolve(d.call(g,e)).then(e=>f.call(g,e));e=d.call(g,e);return f.call(g,e)}:g[s]=(...e)=>{let t=d.apply(g,e);return t=!1===t?f.apply(g,e):t}}}t.hooks=g}if(e.walkTokens){const x=this.defaults.walkTokens,b=e.walkTokens;t.walkTokens=function(e){let t=[];return t.push(b.call(this,e)),t=x?t.concat(x.call(this,e)):t}}this.defaults={...this.defaults,...t}}),this}setOptions(e){return this.defaults={...this.defaults,...e},this}lexer(e,t){return _.lex(e,t??this.defaults)}parser(e,t){return B.parse(e,t??this.defaults)}parseMarkdown(l){return(n,e)=>{e={...e};const r={...this.defaults,...e},t=this.onError(!!r.silent,!!r.async);if(!0===this.defaults.async&&!1===e.async)return t(new Error("marked(): The async option was set to true by an extension. Remove async: false from the parse options object to return a Promise."));if(null==n)return t(new Error("marked(): input parameter is undefined or null"));if("string"!=typeof n)return t(new Error("marked(): input parameter is of type "+Object.prototype.toString.call(n)+", string expected"));r.hooks&&((r.hooks.options=r).hooks.block=l);const s=r.hooks?r.hooks.provideLexer():l?_.lex:_.lexInline,i=r.hooks?r.hooks.provideParser():l?B.parse:B.parseInline;if(r.async)return Promise.resolve(r.hooks?r.hooks.preprocess(n):n).then(e=>s(e,r)).then(e=>r.hooks?r.hooks.processAllTokens(e):e).then(e=>r.walkTokens?Promise.all(this.walkTokens(e,r.walkTokens)).then(()=>e):e).then(e=>i(e,r)).then(e=>r.hooks?r.hooks.postprocess(e):e).catch(t);try{r.hooks&&(n=r.hooks.preprocess(n));let e=s(n,r),t=(r.hooks&&(e=r.hooks.processAllTokens(e)),r.walkTokens&&this.walkTokens(e,r.walkTokens),i(e,r));return t=r.hooks?r.hooks.postprocess(t):t}catch(e){return t(e)}}}onError(n,r){return e=>{var t;if(e.message+="\nPlease report this to https://github.com/markedjs/marked.",n)return t="<p>An error occurred:</p><pre>"+z(e.message+"",!0)+"</pre>",r?Promise.resolve(t):t;if(r)return Promise.reject(e);throw e}}}const E=new M;function q(e,t){return E.parse(e,t)}q.options=q.setOptions=function(e){return E.setOptions(e),t(q.defaults=E.defaults),q},q.getDefaults=e,q.defaults=n.defaults,q.use=function(...e){return E.use(...e),t(q.defaults=E.defaults),q},q.walkTokens=function(e,t){return E.walkTokens(e,t)},q.parseInline=E.parseInline,q.Parser=B,q.parser=B.parse,q.Renderer=I,q.TextRenderer=L,q.Lexer=_,q.lexer=_.lex,q.Tokenizer=S,q.Hooks=C;x=(q.parse=q).options,m=q.setOptions,i=q.use,y=q.walkTokens,c=q.parseInline,g=q,r=B.parse,w=_.lex;n.Hooks=C,n.Lexer=_,n.Marked=M,n.Parser=B,n.Renderer=I,n.TextRenderer=L,n.Tokenizer=S,n.getDefaults=e,n.lexer=w,n.marked=q,n.options=x,n.parse=g,n.parseInline=c,n.parser=r,n.setOptions=m,n.use=i,n.walkTokens=y});