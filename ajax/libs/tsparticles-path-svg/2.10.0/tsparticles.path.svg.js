  ...
  generate(particle, delta) {
    ...
    if (path) {
      const pathElement = path.element,
        ...
        isPercent = offset.mode === "percent",
        scale = this._scale * pxRatio;
      particle.position.x = (pos.x - this._size.width / 2) * scale + particle.svgInitialPosition.x + (isPercent ? canvasSize.width * offset.x / 100 : offset.x) + particle.svgOffset.width;
      particle.position.y = (pos.y - this._size.height / 2) * scale + particle.svgInitialPosition.y + (isPercent ? canvasSize.height * offset.y / 100 : offset.y) + particle.svgOffset.height;
    }
