...
${am}#variant.${e.reason}`), t;
  ...
  if (r)
    switch (Q(r.val).toHex()) {
      case nm: {
        n = `The transaction reverted because a "require" statement has thrown ${t.length ? br(t[0]) : "an error."}.`;
        break;
      }
      case rm: {
      ...
      case sm:
        n = 'The transaction reverted because an "assert" statement failed to evaluate to true.';
        break;
      case Ed:
        n = `The transaction reverted because it's missing an "OutputChange".`;
        break;
      default:
