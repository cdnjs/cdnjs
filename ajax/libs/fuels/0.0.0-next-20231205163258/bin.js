...
var createTempChainConfig = (coreDir) => {
  const chainConfigPath = (0, import_path3.join)(coreDir, "chainConfig.json");
  const chainConfigJson = JSON.stringify(defaultChainConfig, null, 2);
  (0, import_fs3.mkdirSync)((0, import_path3.dirname)(chainConfigPath), { recursive: true });
  (0, import_fs3.writeFileSync)(chainConfigPath, chainConfigJson);
...
var startFuelCore = async (config) => {
  ...
  const coreDir = (0, import_path3.join)(config.basePath, ".fuels");
...
// src/cli/utils/findPackageRoot.ts
var import_fs4 = require("fs");
var import_path4 = require("path");
function findPackageRoot(currentDir = __dirname) {
  if ((0, import_fs4.existsSync)((0, import_path4.join)(currentDir, "package.json"))) {
    return currentDir;
  }
  return findPackageRoot((0, import_path4.join)(currentDir, ".."));
}
...
var buildSwayProgram = async (config, path) => {
  ...
  return new Promise((resolve4, reject) => {
    ...
    const builtInForcPath = (0, import_path5.join)(pkgRootDir, "node_modules", ".bin", "fuels-forc");
...
async function generateTypesForProgramType(config, paths, programType) {
  ...
  (0, import_runTypegen.runTypegen)({
    ...
    cwd: config.basePath,
    filepaths,
    output: (0, import_path6.join)(config.output, pluralizedDirName),
...
async function generateTypes(config) {
  ...
  (0, import_fs5.writeFileSync)((0, import_path6.join)(config.output, "index.ts"), indexFile);
}
...
async function loadConfig(cwd) {
  ...
  const configPath = await configJoycon.resolve({
    ...
    cwd,
    stopDir: (0, import_path7.parse)(cwd).root
  });
  if (!configPath) {
  ...
  config.output = (0, import_path7.resolve)(cwd, config.output);
  ...
  if (!userConfig.workspace) {
    ...
    config.contracts = (contracts || []).map((c) => (0, import_path7.resolve)(cwd, c));
    config.scripts = (scripts || []).map((s) => (0, import_path7.resolve)(cwd, s));
    config.predicates = (predicates || []).map((p) => (0, import_path7.resolve)(cwd, p));
  } else {
    const workspace = (0, import_path7.resolve)(cwd, userConfig.workspace);
  ...
  ${configPath}`;
    ...
    const swayMembers = forcToml.workspace.members.map((member) => (0, import_path7.resolve)(workspace, member));
...
function init(program) {
  ...
  if (options.workspace) {
    absoluteWorkspace = (0, import_path8.resolve)(path, options.workspace);
    workspace = `./${(0, import_path8.relative)(path, absoluteWorkspace)}`;
  }
  const absoluteOutput = (0, import_path8.resolve)(path, options.output);
  const output = `./${(0, import_path8.relative)(path, absoluteOutput)}`;
  const [contracts, scripts, predicates] = ["contracts", "scripts", "predicates"].map(
    (optionName) => {
      ...
      const relatives = expanded.map((e) => (0, import_path8.relative)(path, e));
      return relatives;
    }
  );
  ...
  const fuelsConfigPath = (0, import_path8.join)(path, "fuels.config.ts");
...
var configureCli = () => {
  ...
  const binDir = (0, import_path9.join)(findPackageRoot(), "node_modules", ".bin");
  program.command("core", "Wrapper around Fuel Core binary", {
    executableFile: (0, import_path9.join)(binDir, "fuels-core")
  });
  program.command("forc", "Wrapper around Forc binary", {
    executableFile: (0, import_path9.join)(binDir, "fuels-forc")
  });
  return program;
};
var run = async (argv) => {
