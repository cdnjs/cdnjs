...
const VCalendar = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    useRender(() => {
      ...
      }, [chunkArray(daysInMonth.value.filter(day => props.weekdays.includes(getDay(adapter, day.date))), props.weekdays.length).map((week, wi) => [!props.hideWeekNumber ? createVNode("div", {
        "class": "v-calendar-month__weeknumber"
      }, [weekNumbers.value[wi]]) : '', week.filter(day => props.weekdays.includes(getDay(adapter, day.date))).map(day => createVNode(VCalendarMonthDay, {
