  ...
  const VCalendarMonthDay = genericComponent()({
    ...
    setup(props, _ref) {
      ...
      useRender(() => {
        ...
        }, [props.events?.filter(event => event.allDay).map(event => vue.createVNode(VCalendarEvent, {
          "day": props.day,
          "event": event,
          "allDay": true
        }, null))]), vue.createVNode("div", {
  ...
  const VCalendar = genericComponent()({
    ...
    setup(props, _ref) {
      ...
      useRender(() => {
        ...
        }, null))])]), props.viewMode === 'week' && daysInWeek.value.map((day, i) => vue.createVNode(VCalendarDay, vue.mergeProps(calendarDayProps, {
          "day": day,
          "dayIndex": i,
