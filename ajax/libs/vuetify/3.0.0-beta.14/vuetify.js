...
(function (global, factory) {
  ...
  function useProvided(props, prop, provided) {
    const internal = useProxiedModel(props, prop, props[prop] ?? provided.value); // TODO: Remove when defaultValue works
  ...
  function createVuetifyAdapter(options) {
    ...
    const messages = vue.ref((options == null ? void 0 : options.messages) ?? {
      en
    });
    return {
      name: 'vuetify',
  ...
  const allowedDensities = [null, 'default', 'comfortable', 'compact']; // typeof allowedDensities[number] evalutes to any
    ...
    setup(props, _ref) {
      ...
      vue.watch(model, value => {
        if (!props.multiple) {
          ...
          search.value = ((_value$ = value[0]) == null ? void 0 : _value$.title) ?? '';
        }
      });
      const {
        filteredItems
