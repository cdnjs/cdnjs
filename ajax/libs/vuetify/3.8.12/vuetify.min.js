...
if(d(e.at(-1),e.at(-3))&&!d(e.at(-1),e.at(-2)))return}const e=y()
e&&m.push(e.flipped)}))
...
function T(){cancelAnimationFrame(A),A=requestAnimationFrame(D)}function D(){if(!c.value||!g.value||!n.value)return
...
if(null==e)continue
if(r=!0,e)return e}if(!r&&(u instanceof Date&&c instanceof Date&&(u=u.getTime(),c=c.getTime()),[u,c]=[u,c].map((e=>null!=e?e.toString().toLocaleLowerCase():e)),u!==c))return J(u)&&J(c)?0:J(u)?-1:J(c)?1:isNaN(u)||isNaN(c)?o.compare(u,c):Number(u)-Number(c)}return 0})).map((e=>{let[t]=e
...
const n=t.ref(),{daysInMonth:r,model:i,weekNumbers:s,weekdayLabels:u}=function(e){const a=Iu(),l=na(e,"modelValue",[],(e=>B(e).map((e=>a.date(e))))),o=t.computed((()=>e.displayValue?a.date(e.displayValue):l.value.length>0?a.date(l.value[0]):e.min?a.date(e.min):Array.isArray(e.allowedDates)?a.date(e.allowedDates[0]):a.date())),n=na(e,"year",void 0,(e=>{const t=null!=e?Number(e):a.getYear(o.value)
...
return a.getWeekdays(e.firstDayOfWeek).filter(((a,l)=>e.weekdays.includes((l+t)%7)))})),s=t.computed((()=>{const t=a.getWeekArray(r.value,e.firstDayOfWeek),l=t.flat()
...
if(a&&e in a)return a[e]}Sm.version="3.8.12"
const xm=function(){return Sm({components:pm,directives:wm,...arguments.length>0&&void 0!==arguments[0]?arguments[0]:{}})},Cm="3.8.12"
