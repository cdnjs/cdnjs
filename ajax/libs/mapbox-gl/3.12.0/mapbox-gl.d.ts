...
declare class UniformColor extends Uniform<RenderColor> implements IUniform<RenderColor> {
	constructor(context: Context);
...
interface WorkerSourceConstructor {
	new (actor?: Actor, layerIndex?: StyleLayerIndex, availableImages?: ImageId[], availableModels?: StyleModelMap, isSpriteLoaded?: boolean, loadData?: (params: {
		source: string;
		scope: string;
	}, callback: Callback<unknown>) => () => void | undefined, brightness?: number): WorkerSource;
}
...
declare class SymbolStyleLayer extends StyleLayer {
	...
	getValueAndResolveTokens<T extends keyof LayoutProps$4>(name: T, feature: Feature, canonical: CanonicalTileID, availableImages: ImageId[]): string;
	...
	static hasPaintOverride(layout: PossiblyEvaluated<LayoutProps$4>, propertyName: string): boolean;
	getProgramIds(): ProgramName[];
	getDefaultProgramParams(name: string, zoom: number, lut: LUT$1 | null): CreateProgramParams | null;
	hasElevation(): boolean;
}
...
type ProceduralBuildingDepthUniformsType = {
	["u_matrix"]: UniformMatrix4f;
};
...
type DynamicDefinesType = GlobalDefinesType | CircleDefinesType | SymbolDefinesType | LineDefinesType | FillDefinesType | FillExtrusionDefinesType | HeatmapDefinesType | GlobeDefinesType | RasterDefinesType | RasterParticleDefinesType | FogDefinesType | HillshadeDefinesType | TerrainDepthAccessDefinesType | ModelDefinesType;
declare const programUniforms: {
	...
	readonly buildingDepth: (context: Context) => ProceduralBuildingDepthUniformsType;
...
type ShadowsUniformsType = ShadowUniformsType | GroundShadowUniformsType | SymbolUniformsType | ModelUniformsType | ModelDepthUniformsType | FillExtrusionDepthUniformsType | FillExtrusionPatternUniformsType;
type ShadowCascade = {
...
declare class ShadowRenderer {
	...
	setupShadowsFromMatrix(worldMatrix: mat4, program: Program$1<ShadowUniformsType | ModelUniformsType>, normalOffset?: boolean): void;
...
declare class IndoorManager extends Evented<IndoorEvents> {
	...
	_updateLevels(selectedLevel: SelectedLevel | undefined, animated: boolean): void;
