...
var RuleEngine = class {
  ...
  execute(fact, callback) {
    ...
    let complete = false;
    const session = (0, import_lodash.cloneDeep)(fact);
    let lastSession = (0, import_lodash.cloneDeep)(fact);
    ...
    function FnRuleLoop(x) {
      const API = {
        ...
        next: () => {
          if (!ignoreFactChanges && !(0, import_lodash.isEqual)(lastSession, session)) {
            lastSession = (0, import_lodash.cloneDeep)(session);
            thisHolder.nextTick(() => {
              API.restart();
            });
          } else {
