    ...
    return __async(this, null, function* () {
      ...
      try {
        if (!this.ctx.conf.imgUploader) {
          ...
          resp = yield this.ctx.conf.imgUploader(file);
        }
      } catch (err) {
        console.error(err);
...
class ListLite extends Component {
  ...
  onLoad(data, offset) {
    ...
    if (this.ctx.conf.versionCheck && this.versionCheck("frontend", feMinVersion, "2.3.0"))
      return;
...
class List extends ListLite {
  constructor(ctx) {
    ...
    this.$el.querySelector(".atk-copyright").innerHTML = `Powered By <a href="https://artalk.js.org" target="_blank" title="Artalk v${"2.3.0"}">Artalk</a>`;
  }
  initListActionBtn() {
...
const _Artalk = class {
  constructor(customConf) {
    ...
    this.conf.el = this.conf.el || document.createElement("div");
    if (this.conf.el instanceof HTMLElement) {
      ...
      try {
        ...
        if (!$root)
          throw Error(`Target element "${this.conf.el}" was not found.`);
        this.$root = $root;
      } catch (e) {
        console.error(e);
  ...
  initCountAndPV() {
    return __async(this, null, function* () {
      ...
      const curtPagePvNum = yield this.ctx.getApi().pv();
  ...
  handleStatCount(args) {
    return __async(this, null, function* () {
      ...
      if (queryPageKeys.length > 0) {
        const counts = yield this.ctx.getApi().stat(args.api, queryPageKeys);
