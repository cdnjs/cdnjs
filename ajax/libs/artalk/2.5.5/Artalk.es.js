...
block.paragraph = edit$1(block._paragraph).replace("hr", block.hr).replace("heading", " {0,3}#{1,6} ").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", block._tag).getRegex();
...
block.gfm.paragraph = edit$1(block._paragraph).replace("hr", block.hr).replace("heading", " {0,3}#{1,6} ").replace("|lheading", "").replace("table", block.gfm.table).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", block._tag).getRegex();
...
inline.reflink = edit$1(inline.reflink).replace("label", inline._label).replace("ref", block._label).getRegex();
inline.nolink = edit$1(inline.nolink).replace("ref", block._label).getRegex();
...
class Parser {
  ...
  parse(tokens, top = true) {
    let out = "", i, j, k, l2, l3, row, cell, header2, body, token, ordered, start, loose, itemBody, item, checked, task, checkbox, ret;
...
function parseMarkdown(lexer, parser2) {
  return (src, opt, callback) => {
    ...
    opt = __spreadValues(__spreadValues({}, marked$1.defaults), origOpt);
    ...
    if (opt.async) {
      return Promise.resolve(opt.hooks ? opt.hooks.preprocess(src) : src).then((src2) => lexer(src2, opt)).then((tokens) => opt.walkTokens ? Promise.all(marked$1.walkTokens(tokens, opt.walkTokens)).then(() => tokens) : tokens).then((tokens) => parser2(tokens, opt)).then((html) => opt.hooks ? opt.hooks.postprocess(html) : html).catch(throwError);
    }
    try {
      if (opt.hooks) {
        src = opt.hooks.preprocess(src);
...
marked$1.options = marked$1.setOptions = function(opt) {
  marked$1.defaults = __spreadValues(__spreadValues({}, marked$1.defaults), opt);
...
marked$1.use = function(...args) {
  ...
  args.forEach((pack) => {
    ...
    opts.async = marked$1.defaults.async || opts.async || false;
    if (pack.extensions) {
    ...
    if (pack.hooks) {
      const hooks = marked$1.defaults.hooks || new Hooks();
      for (const prop in pack.hooks) {
        const prevHook = hooks[prop];
        if (Hooks.passThroughHooks.has(prop)) {
          hooks[prop] = (arg) => {
            if (marked$1.defaults.async) {
...
marked$1.parseInline = parseMarkdown(Lexer.lexInline, Parser.parseInline);
