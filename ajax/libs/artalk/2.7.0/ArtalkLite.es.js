...
function Fetch$1(opts, input, init, timeout) {
  return __async(this, null, function* () {
    var _a, _b;
    ...
    const resp = yield timeoutFetch(input, timeout || opts.timeout || 15e3, init);
    ...
    const recall = (resolve, reject) => {
      Fetch$1(opts, input, init).then((d) => {
        resolve(d);
      }).catch((e) => {
        reject(e);
      });
...
function POST(opts, url, data) {
  return __async(this, null, function* () {
    ...
    const json = yield Fetch$1(opts, url, init);
    return json.data || {};
  });
}
...
class ApiBase {
  ...
  Fetch(path, init, timeout) {
    return __async(this, null, function* () {
      return Fetch$1(this.options, this.options.baseURL + path, init, timeout);
    });
  }
}
...
function initCountWidget(opt) {
  return __async(this, null, function* () {
    ...
    const curtPagePvNum = opt.pvAdd ? yield opt.getApi().page.pv() : NaN;
...
function refreshStatCount(opt, args) {
  return __async(this, null, function* () {
    let data = args.data || {};
    let pageKeys = Array.from(document.querySelectorAll(args.numEl)).map((e) => e.getAttribute("data-page-key") || opt.pageKey).filter((e) => Number.isNaN(data[e]));
    pageKeys = [...new Set(pageKeys)];
    if (pageKeys.length > 0) {
      const res = yield opt.getApi().page.stat(args.query, pageKeys);
