...
function useAutoresize(chart, autoresize, root) {
	watch([
		...
		if (root$1 && chart$1 && autoresize$1) {
			...
			const autoresizeOptions = autoresize$1 === true ? {} : autoresize$1;
...
function useSlotOption(slots, onSlotsChange) {
	const detachedRoot = typeof window !== "undefined" ? document.createElement("div") : void 0;
	const containers = shallowReactive({});
	const initialized = shallowReactive({});
	const params = shallowReactive({});
	const isMounted = shallowRef(false);
	const teleportedSlots = () => {
		return isMounted.value ? h(Teleport, { to: detachedRoot }, Object.entries(slots).filter(([key]) => isValidSlotName(key)).map(([key, slot]) => {
			const slotName = key;
...
function register() {
	...
	try {
		...
		if (customElements.get(TAG_NAME) == null) customElements.define(TAG_NAME, ECElement);
	} catch {
...
var ECharts_default = defineComponent({
	...
	setup(props, { attrs, expose, slots }) {
		...
		const realInitOptions = computed(() => props.initOptions || toValue(defaultInitOptions) || {});
		...
		const { teleportedSlots, patchOption } = useSlotOption(slots, () => {
			if (!manualUpdate.value && props.option && chart.value) chart.value.setOption(patchOption(props.option), realUpdateOptions.value);
		});
		...
		function init$1(option) {
			...
			function commit() {
				...
				if (opt) instance.setOption(patchOption(opt), realUpdateOptions.value);
			}
		...
		const setOption = (option, notMerge, lazyUpdate) => {
			...
			if (!chart.value) init$1(option);
			else chart.value.setOption(patchOption(option), updateOptions);
		};
		function cleanup() {
			if (chart.value) {
				chart.value.dispose();
		...
		watch(manualUpdate, (manualUpdate$1) => {
			...
			if (!manualUpdate$1) unwatchOption = watch(() => props.option, (option, oldOption) => {
				if (!option) return;
				if (!chart.value) init$1();
				else chart.value.setOption(patchOption(option), {
