...
var ECharts_default = defineComponent({
	...
	setup(props, { attrs, expose, slots }) {
		...
		let lastSignature;
		function resolveUpdateOptions(plan, override) {
			const base = realUpdateOptions.value;
			const result = { ...override };
			const replacements = [...plan?.replaceMerge ?? [], ...override?.replaceMerge ?? []].filter((key) => key != null);
			if (replacements.length > 0) result.replaceMerge = [...new Set(replacements)];
			else delete result.replaceMerge;
			const notMerge = override?.notMerge ?? plan?.notMerge;
			if (notMerge !== void 0) result.notMerge = notMerge;
			else delete result.notMerge;
			return base ? {
				...base,
				...result
			} : result;
		...
		const setOption = (option, notMerge, lazyUpdate) => {
			...
			if (!chart.value) init$1(option, true, updateOptions ?? void 0);
			else applyOption(chart.value, option, updateOptions ?? void 0, true);
		};
		function cleanup() {
			if (chart.value) {
				chart.value.dispose();
		...
		watch(manualUpdate, (manualUpdate$1) => {
			...
			if (!manualUpdate$1) unwatchOption = watch(() => props.option, (option) => {
				...
				else applyOption(chart.value, option);
			}, { deep: true });
		}, { immediate: true });
