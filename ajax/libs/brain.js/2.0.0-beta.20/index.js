...
var index_js = require('../node_modules/gpu.js/src/index.js');
...
function makeKernel(fn, settings) {
    ...
    if (_gpuInstance === null) {
        _gpuInstance = new index_js.GPU({ mode: 'gpu' });
        setup(_gpuInstance);
    }
    return _gpuInstance
...
function kernelInput(value, size) {
    return new index_js.Input(value, size);
}
/**
 * Deletes a gpu.js texture and frees VRAM
 */
function release(possibleTexture) {
    if (possibleTexture instanceof index_js.Texture) {
...
function clear(value) {
    if (value instanceof index_js.Texture) {
        value.clear();
        return;
    }
    // array
...
class BaseLayer {
    ...
    static toJSON(layer) {
        ...
        return {
            ...
            height: layer.height,
            depth: layer.depth,
            weights: toUntypedArray((weights && weights instanceof index_js.Texture
                ? weights.toArray()
                : weights)),
...
class FeedForward {
    ...
    run(input) {
        ...
        if (output instanceof index_js.Texture) {
    ...
    _calculateTrainingError(preparedData) {
        ...
        release(sum);
        if (result instanceof index_js.Texture) {
...
class NeuralNetworkGPU extends NeuralNetwork {
    constructor(options = {}) {
        ...
        this.gpu = new index_js.GPU({ mode: options.mode });
    }
    initialize() {
        super.initialize();
        this.buildRunInput();
    ...
    calculateTrainingError(data) {
        ...
        release(sum);
        return (result instanceof index_js.Texture
            ? result.toArray()
            : result)[0];
    }
    adjustWeights() {
    ...
    buildCalculateDeltas() {
        ...
        calcDeltas = index_js.alias(index_js.utils.getMinifySafeName(() => calcDeltas), calcDeltas);
    ...
    run(input) {
        ...
        const output = outputTextures instanceof index_js.Texture
    ...
    toJSON() {
        ...
        // use Array.from, keeping json small
        const jsonLayerWeights = this.weights.map((layerWeights) => {
            return (layerWeights instanceof index_js.Texture
                ? layerWeights.toArray()
                : layerWeights).map((layerWeights) => Array.from(layerWeights));
        });
        const jsonLayerBiases = this.biases.map((layerBiases) => Array.from(layerBiases instanceof index_js.Texture
            ? layerBiases.toArray()
            : layerBiases));
        const jsonLayers = [];
        for (let i = 0; i <= this.outputLayer; i++) {
