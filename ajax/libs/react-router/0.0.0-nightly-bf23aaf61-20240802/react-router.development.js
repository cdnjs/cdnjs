...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('react'), require('turbo-stream'), require('cookie'), require('@web3-storage/multipart-parser'), require('set-cookie-parser')) :
  typeof define === 'function' && define.amd ? define(['exports', 'react', 'turbo-stream', 'cookie', '@web3-storage/multipart-parser', 'set-cookie-parser'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ReactRouter = {}, global.React, global.turboStream, global.cookie, global.multipartParser, global.setCookieParser));
})(this, (function (exports, React, turboStream, cookie, multipartParser, setCookieParser) { 'use strict';
  ...
  function createRouter(init) {
    ...
    function abortFetcher(key) {
      ...
      !controller ? invariant$2(false, "Expected fetch controller: " + key)  : void 0;
      controller.abort();
      fetchControllers.delete(key);
    }
  ...
  function createStaticHandler$1(routes, opts) {
    ...
    async function queryImpl(request, location, matches, requestContext, unstable_dataStrategy, skipLoaderErrorBubbling, routeMatch) {
      ...
      try {
        ...
        if (isHandlerResult(e) && isResponse$2(e.result)) {
  ...
  async function callDataStrategyImpl(dataStrategyImpl, type, request, matchesToLoad, matches, manifest, mapRouteProperties, requestContext) {
    ...
    matches.forEach(m => !loadedMatches.has(m.route.id) ? invariant$2(false, "`match.resolve()` was not called for route id \"" + m.route.id + "\". " + "You must call `match.resolve()` on every match passed to " + "`dataStrategy` to ensure all routes are properly loaded.")  : void 0);
  ...
  function processLoaderData(state, matches, matchesToLoad, results, pendingActionResult, revalidatingFetchers, fetcherResults) {
    ...
    for (let index = 0; index < revalidatingFetchers.length; index++) {
      ...
      !(fetcherResults !== undefined && fetcherResults[index] !== undefined) ? invariant$2(false, "Did not find corresponding fetcher result")  : void 0;
  ...
  function RouterProvider(_ref) {
    ...
    let setState = React__namespace.useCallback((newState, _ref2) => {
      ...
      warnOnce(flushSync === false || reactDomFlushSyncImpl != null, "You provided the `unstable_flushSync` option to a router update, " + "but you are not using the `<RouterProvider>` from `react-router/dom` " + "so `ReactDOM.flushSync()` is unavailable.  Please update your app " + 'to `import { RouterProvider } from "react-router/dom"` and ensure ' + "you have `react-dom` installed as a dependency to use the " + "`unstable_flushSync` option.");
      let isViewTransitionAvailable = router.window != null && router.window.document != null && typeof router.window.document.startViewTransition === "function";
      warnOnce(viewTransitionOpts == null || isViewTransitionAvailable, "You provided the `unstable_viewTransition` option to a router update, " + "but you do not appear to be running in a DOM environment as " + "`window.startViewTransition` is not available.");
  ...
  */
      ...
      if (manifest.routes[m.route.id].hasClientLoader) {
        ...
        result = await handler(async () => {
          ...
          if (!singleFetchPromise) {
            ...
            singleFetchPromise = fetchAndDecode(url, init).then(_ref3 => {
              let {
                data
              } = _ref3;
              return data;
  ...
  function getFogOfWarPaths(fogOfWar) {
    let {
      knownGoodPaths,
      known404Paths,
      nextPaths
  ...
  */
  ...
  function useViewTransitionState(to, opts) {
    ...
    !(vtContext != null) ? invariant$2(false, "`unstable_useViewTransitionState` must be used within `react-router-dom`'s `RouterProvider`.  " + "Did you accidentally import `RouterProvider` from `react-router`?")  : void 0;
    let {
      basename
  ...
  exports.UNSAFE_initFogOfWar = initFogOfWar;
