/**
 * React Router v0.0.0-experimental-f042b8c1e
 *
...
(function (global, factory) {
  ...
  function createRouter(init) {
    ...
    async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
      ...
      let {
        loaderData,
        errors
      } = processLoaderData(state, matches, matchesToLoad, loaderResults, undefined, revalidatingFetchers, fetcherResults);
  ...
  function createStaticHandler$1(routes, opts) {
    ...
    async function loadRouteData(request, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, pendingActionResult) {
      ...
      // Process and commit output from loaders
      let context = processRouteLoaderData(matches, results, pendingActionResult, skipLoaderErrorBubbling);
  ...
  function processRouteLoaderData(matches, results, pendingActionResult, skipLoaderErrorBubbling) {
  ...
  function processLoaderData(state, matches, matchesToLoad, results, pendingActionResult, revalidatingFetchers, fetcherResults) {
    let {
      loaderData,
      errors
    } = processRouteLoaderData(matches, results, pendingActionResult, false // This method is only called client side so we always want to bubble
    );
