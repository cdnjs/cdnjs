/**
 * React Router v0.0.0-experimental-ecd35cd60
 *
...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('react'), require('react-dom'), require('turbo-stream'), require('cookie'), require('@web3-storage/multipart-parser')) :
  typeof define === 'function' && define.amd ? define(['exports', 'react', 'react-dom', 'turbo-stream', 'cookie', '@web3-storage/multipart-parser'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ReactRouter = {}, global.React, global.ReactDOM, global.turboStream, global.cookie, global.multipartParser));
})(this, (function (exports, React, ReactDOM, turboStream, cookie, multipartParser) { 'use strict';
  ...
  function getUrlBasedHistory(getLocation, createHref, validateLocation, options) {
    ...
    function createURL(to) {
      ...
      !base ? invariant$1(false, "No window.location.(origin|href) available to create URL for href: " + href)  : void 0;
  ...
  function convertRoutesToDataRoutes(routes, mapRouteProperties, parentPath, manifest) {
    ...
    return routes.map((route, index) => {
      ...
      !(route.index !== true || !route.children) ? invariant$1(false, "Cannot specify children on an index route")  : void 0;
      !!manifest[id] ? invariant$1(false, "Found a route id collision on id \"" + id + "\".  Route " + "id's must be globally unique within Data Router usages")  : void 0;
  ...
  function flattenRoutes(routes, branches, parentsMeta, parentPath) {
    ...
    let flattenRoute = (route, index, relativePath) => {
      ...
      if (meta.relativePath.startsWith("/")) {
        !meta.relativePath.startsWith(parentPath) ? invariant$1(false, "Absolute route path \"" + meta.relativePath + "\" nested under path " + ("\"" + parentPath + "\" is not valid. An absolute child route path ") + "must start with the combined path of all its parent routes.")  : void 0;
      ...
      if (route.children && route.children.length > 0) {
        ...
        // @ts-expect-error
        route.index !== true) ? invariant$1(false, "Index routes must not have child routes. Please remove " + ("all child routes from route path \"" + path + "\"."))  : void 0;
  ...
  function generatePath(originalPath, params) {
    ...
    const segments = path.split(/\/+/).map((segment, index, array) => {
      ...
      if (keyMatch) {
        ...
        !(optional === "?" || param != null) ? invariant$1(false, "Missing \":" + key + "\" param")  : void 0;
  ...
  function resolveTo(toArg, routePathnames, locationPathname, isPathRelative) {
    ...
    if (typeof toArg === "string") {
      ...
      to = _extends({}, toArg);
      !(!to.pathname || !to.pathname.includes("?")) ? invariant$1(false, getInvalidPathError("?", "pathname", "search", to))  : void 0;
      !(!to.pathname || !to.pathname.includes("#")) ? invariant$1(false, getInvalidPathError("#", "pathname", "hash", to))  : void 0;
      !(!to.search || !to.search.includes("#")) ? invariant$1(false, getInvalidPathError("#", "search", "hash", to))  : void 0;
    }
  ...
  function createRouter(init) {
    ...
    !(init.routes.length > 0) ? invariant$1(false, "You must provide a non-empty routes array to createRouter")  : void 0;
    ...
    async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionResult) {
      ...
      let routesToUse = inFlightDataRoutes || dataRoutes;
      let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad(init.history, state, matches, activeSubmission, location, initialHydration === true, future.unstable_skipActionErrorRevalidation, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult);
      ...
      let {
        loaderData,
        errors
      } = processLoaderData(state, matches, matchesToLoad, loaderResults, pendingActionResult, revalidatingFetchers, fetcherResults);
    ...
    async function fetch(key, routeId, href, opts) {
      ...
      if (submission && isMutationMethod(submission.formMethod)) {
        await handleFetcherAction(key, routeId, path, match, matches, fogOfWar.active, flushSync, submission);
        return;
      }
      ...
      await handleFetcherLoader(key, routeId, path, match, matches, fogOfWar.active, flushSync, submission);
    }
    ...
    async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, submission) {
      ...
      !matches ? invariant$1(false, "Didn't find any matches after fetcher action")  : void 0;
      let loadId = ++incrementingLoadId;
      fetchReloadIds.set(key, loadId);
      let loadFetcher = getLoadingFetcher(submission, actionResult.data);
      state.fetchers.set(key, loadFetcher);
      let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad(init.history, state, matches, submission, nextLocation, false, future.unstable_skipActionErrorRevalidation, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, [match.route.id, actionResult]);
      ...
      let {
        loaderData,
        errors
      } = processLoaderData(state, state.matches, matchesToLoad, loaderResults, undefined, revalidatingFetchers, fetcherResults);
      ...
      if (state.navigation.state === "loading" && loadId > pendingNavigationLoadId) {
        !pendingAction ? invariant$1(false, "Expected pending action")  : void 0;
        pendingNavigationController && pendingNavigationController.abort();
    ...
    async function startRedirectNavigation(request, redirect, _temp2) {
      ...
      !location ? invariant$1(false, "Expected a Location header on the redirect Response")  : void 0;
        ...
        return await Promise.all(results.map((result, i) => {
          if (isRedirectHandlerResult(result)) {
            ...
            return {
              ...
              response: normalizeRelativeRoutingRedirectResponse(response, request, matchesToLoad[i].route.id, matches, basename)
            };
          }
    ...
    function abortFetcher(key) {
      ...
      !controller ? invariant$1(false, "Expected fetch controller: " + key)  : void 0;
      controller.abort();
      fetchControllers.delete(key);
    }
    ...
    function markFetchRedirectsDone() {
      ...
      for (let key of fetchRedirectIds) {
        let fetcher = state.fetchers.get(key);
        !fetcher ? invariant$1(false, "Expected fetcher: " + key)  : void 0;
    ...
    function updateBlocker(key, newBlocker) {
      ...
      !(blocker.state === "unblocked" && newBlocker.state === "blocked" || blocker.state === "blocked" && newBlocker.state === "blocked" || blocker.state === "blocked" && newBlocker.state === "proceeding" || blocker.state === "blocked" && newBlocker.state === "unblocked" || blocker.state === "proceeding" && newBlocker.state === "unblocked") ? invariant$1(false, "Invalid blocker state transition: " + blocker.state + " -> " + newBlocker.state)  : void 0;
      let blockers = new Map(state.blockers);
  ...
  function createStaticHandler$1(routes, opts) {
    !(routes.length > 0) ? invariant$1(false, "You must provide a non-empty routes array to createStaticHandler")  : void 0;
    let manifest = {};
    ...
    async function queryImpl(request, location, matches, requestContext, unstable_dataStrategy, skipLoaderErrorBubbling, routeMatch) {
      !request.signal ? invariant$1(false, "query()/queryRoute() requests must contain an AbortController signal")  : void 0;
      try {
    ...
    async function loadRouteData(request, matches, requestContext, unstable_dataStrategy, skipLoaderErrorBubbling, routeMatch, pendingActionResult) {
      ...
      // Process and commit output from loaders
      let context = processRouteLoaderData(matches, matchesToLoad, results, pendingActionResult, skipLoaderErrorBubbling);
    ...
    async function callDataStrategy(type, request, matchesToLoad, matches, isRouteRequest, requestContext, unstable_dataStrategy) {
      ...
      return await Promise.all(results.map((result, i) => {
        if (isRedirectHandlerResult(result)) {
          ...
          throw normalizeRelativeRoutingRedirectResponse(response, request, matchesToLoad[i].route.id, matches, basename);
        }
  ...
  function normalizeNavigateOptions(isFetcher, path, opts) {
    ...
    !(typeof FormData === "function") ? invariant$1(false, "FormData is not available in this environment")  : void 0;
    let searchParams;
    let formData;
    if (opts.formData) {
  ...
  function getMatchesToLoad(history, state, matches, submission, location, isInitialLoad, skipActionErrorRevalidation, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
  ...
  function patchRoutesImpl(routeId, children, routesToUse, manifest, mapRouteProperties) {
    if (routeId) {
      ...
      !route ? invariant$1(false, "No route found to patch children into: routeId = " + routeId)  : void 0;
  ...
  async function loadLazyRouteModule(route, mapRouteProperties, manifest) {
    ...
    !routeToUpdate ? invariant$1(false, "No route found in manifest")  : void 0;
  ...
  async function callDataStrategyImpl(dataStrategyImpl, type, request, matchesToLoad, matches, manifest, mapRouteProperties, requestContext) {
    ...
    matches.forEach(m => !loadedMatches.has(m.route.id) ? invariant$1(false, "`match.resolve()` was not called for route id \"" + m.route.id + "\". " + "You must call `match.resolve()` on every match passed to " + "`dataStrategy` to ensure all routes are properly loaded.")  : void 0);
  ...
  function normalizeRelativeRoutingRedirectResponse(response, request, routeId, matches, basename) {
    ...
    !location ? invariant$1(false, "Redirects returned/thrown from loaders/actions must have a Location header")  : void 0;
  ...
  function processRouteLoaderData(matches, matchesToLoad, results, pendingActionResult, skipLoaderErrorBubbling) {
    ...
    results.forEach((result, index) => {
      ...
      !!isRedirectResult(result) ? invariant$1(false, "Cannot handle redirect results in processLoaderData")  : void 0;
      if (isErrorResult(result)) {
  ...
  function processLoaderData(state, matches, matchesToLoad, results, pendingActionResult, revalidatingFetchers, fetcherResults) {
    let {
      loaderData,
      errors
    } = processRouteLoaderData(matches, matchesToLoad, results, pendingActionResult, false // This method is only called client side so we always want to bubble
    );
    ...
    for (let index = 0; index < revalidatingFetchers.length; index++) {
      ...
      !(fetcherResults !== undefined && fetcherResults[index] !== undefined) ? invariant$1(false, "Did not find corresponding fetcher result")  : void 0;
  ...
  function useHref(to, _temp) {
    ...
    !useInRouterContext() ? invariant$1(false, // TODO: This error is probably because they somehow have 2 versions of the
  ...
  function useRoutesImpl(routes, locationArg, dataRouterState, future) {
    ...
    if (locationArg) {
      ...
      !(parentPathnameBase === "/" || ((_parsedLocationArg$pa = parsedLocationArg.pathname) == null ? void 0 : _parsedLocationArg$pa.startsWith(parentPathnameBase))) ? invariant$1(false, "When overriding the location using `<Routes location>` or `useRoutes(routes, location)`, " + "the location pathname must begin with the portion of the URL pathname that was " + ("matched by all parent routes. The current pathname base is \"" + parentPathnameBase + "\" ") + ("but pathname \"" + parsedLocationArg.pathname + "\" was given in the `location` prop."))  : void 0;
  ...
  function _renderMatches(matches, parentMatches, dataRouterState, future) {
    ...
    if (errors != null) {
      ...
      !(errorIndex >= 0) ? invariant$1(false, "Could not find a matching route for errors on route IDs: " + Object.keys(errors).join(","))  : void 0;
  ...
  function useDataRouterContext$2(hookName) {
    ...
    !ctx ? invariant$1(false, getDataRouterConsoleError$1(hookName))  : void 0;
    return ctx;
  }
  function useDataRouterState$1(hookName) {
    let state = React__namespace.useContext(DataRouterStateContext);
    !state ? invariant$1(false, getDataRouterConsoleError$1(hookName))  : void 0;
    return state;
  }
  function useRouteContext(hookName) {
    let route = React__namespace.useContext(RouteContext);
    !route ? invariant$1(false, getDataRouterConsoleError$1(hookName))  : void 0;
    return route;
  }
  ...
  function useCurrentRouteId(hookName) {
    ...
    !thisRoute.route.id ? invariant$1(false, hookName + " can only be used on routes that contain a unique \"id\"")  : void 0;
  ...
  function Navigate(_ref2) {
    ...
    !useInRouterContext() ? invariant$1(false, // TODO: This error is probably because they somehow have 2 versions of
  ...
  function Router(_ref3) {
    ...
    !!useInRouterContext() ? invariant$1(false, "You cannot render a <Router> inside another <Router>." + " You should never have more than one in your app.")  : void 0;
  ...
  */
  ...
  function createRoutesFromChildren(children, parentPath) {
    ...
    React__namespace.Children.forEach(children, (element, index) => {
      ...
      !(element.type === Route) ? invariant$1(false, "[" + (typeof element.type === "string" ? element.type : element.type.name) + "] is not a <Route> component. All component children of <Routes> must be a <Route> or <React.Fragment>")  : void 0;
      !(!element.props.index || !element.props.children) ? invariant$1(false, "An index route cannot have child routes.")  : void 0;
      let route = {
  ...
  async function loadRouteModule(route, routeModulesCache) {
    ...
    try {
      ...
      if (window.__remixContext && window.__remixContext.isSpaMode &&
      // @ts-expect-error
  ...
  function StreamTransfer(_ref) {
    ...
    let scriptTag = value ? /*#__PURE__*/React__namespace.createElement("script", {
      ...
      dangerouslySetInnerHTML: {
        __html: "window.__remixContext.streamController.enqueue(" + escapeHtml$1(JSON.stringify(value)) + ");"
      }
    }) : null;
    if (done) {
  ...
  function getSingleFetchDataStrategy$1(manifest, routeModules) {
    return async _ref2 => {
      let {
        request,
        matches
      } = _ref2;
      return request.method !== "GET" ? singleFetchActionStrategy(request, matches) : singleFetchLoaderStrategy(manifest, routeModules, request, matches);
    };
  }
      ...
      if (manifest.routes[m.route.id].hasClientLoader) {
        ...
        result = await handler(async () => {
          ...
          if (!singleFetchPromise) {
            ...
            singleFetchPromise = fetchAndDecode(url).then(_ref3 => {
              let {
                data
              } = _ref3;
              return data;
  ...
  function addRevalidationParam(manifest, routeModules, matchedRoutes, loadRoutes, url) {
    ...
    let needsParam = matchedRoutes.some(r => {
      ...
      return ((_routeModules$r$id = routeModules[r.id]) == null ? void 0 : _routeModules$r$id.shouldRevalidate) || ((_manifest$routes$r$id = manifest.routes[r.id]) == null ? void 0 : _manifest$routes$r$id.hasClientLoader);
    });
    if (!needsParam) {
      return url;
    }
    ...
    let loadIds = genRouteIds(loadRoutes.filter(r => {
      ...
      return !((_manifest$routes$r$id2 = manifest.routes[r.id]) != null && _manifest$routes$r$id2.hasClientLoader);
    }).map(r => r.id));
  ...
  async function fetchAndDecode(url, init) {
    ...
    !res.body ? invariant(false, "No response body to decode")  : void 0;
    try {
  ...
  function createServerRoutes(manifest, routeModules, future, isSpaMode, parentId, routesByParentId, spaModeLazyPromise) {
    ...
    return (routesByParentId[parentId] || []).map(route => {
      ...
      !routeModule ? invariant(false, "No `routeModule` available to create server routes")  : void 0;
  ...
  function createClientRoutes(manifest, routeModulesCache, initialState, isSpaMode, parentId, routesByParentId, needsRevalidation) {
    ...
    return (routesByParentId[parentId] || []).map(route => {
      ...
      function fetchServerHandler(singleFetch) {
        !(typeof singleFetch === "function") ? invariant(false, "No single fetch function available for route handler")  : void 0;
        return singleFetch();
      }
      ...
      if (routeModule) {
        ...
        dataRoute.loader = async (_ref, singleFetch) => {
          ...
          try {
            let result = await prefetchStylesAndCallHandler(async () => {
              !routeModule ? "development" !== "production" ? invariant(false, "No `routeModule` available for critical-route loader") : invariant(false) : void 0;
        ...
        dataRoute.action = (_ref2, singleFetch) => {
          ...
          return prefetchStylesAndCallHandler(async () => {
            !routeModule ? "development" !== "production" ? invariant(false, "No `routeModule` available for critical-route action") : invariant(false) : void 0;
  ...
  function initFogOfWar(manifest, routeModules, isSpaMode, basename) {
    ...
    return {
      ...
      patchRoutesOnMiss: async _ref => {
        ...
        await fetchAndApplyManifestPatches([path], fogOfWar, manifest, routeModules, isSpaMode, basename, patch);
      }
    };
  }
  function useFogOFWarDiscovery(router, manifest, routeModules, isSpaMode) {
    React__namespace.useEffect(() => {
      ...
      async function fetchPatches() {
        ...
        try {
          await fetchAndApplyManifestPatches(lazyPaths, fogOfWar, manifest, routeModules, isSpaMode, router.basename, router.patchRoutes);
        } catch (e) {
  ...
  async function fetchAndApplyManifestPatches(paths, _fogOfWar, manifest, routeModules, isSpaMode, basename, patchRoutes) {
    let {
      nextPaths,
      knownGoodPaths,
      known404Paths
  ...
  function useDataRouterContext$1() {
    ...
    !context ? invariant(false, "You must render this element inside a <DataRouterContext.Provider> element")  : void 0;
    return context;
  }
  function useDataRouterStateContext() {
    let context = React__namespace.useContext(DataRouterStateContext);
    !context ? invariant(false, "You must render this element inside a <DataRouterStateContext.Provider> element")  : void 0;
    return context;
  }
  ...
  function useFrameworkContext() {
    ...
    !context ? invariant(false, "You must render this element inside a <HydratedRouter> element")  : void 0;
    return context;
  }
  ...
  function PrefetchPageLinksImpl(_ref3) {
    ...
    if (newMatchesForData.length > 0) {
      ...
      if (url.searchParams.get("_routes") !== "") {
        linksToRender = /*#__PURE__*/React__namespace.createElement("link", _extends({
    ...
    return /*#__PURE__*/React__namespace.createElement(React__namespace.Fragment, null, linksToRender, moduleHrefs.map(href => /*#__PURE__*/React__namespace.createElement("link", _extends({
      key: href,
      rel: "modulepreload",
      href: href
  ...
  const _excluded$1 = ["onClick", "discover", "prefetch", "relative", "reloadDocument", "replace", "state", "target", "to", "preventScrollReset", "unstable_viewTransition"],
    _excluded2 = ["aria-current", "caseSensitive", "className", "end", "style", "to", "unstable_viewTransition", "children"],
    _excluded3 = ["discover", "fetcherKey", "navigate", "reloadDocument", "replace", "state", "method", "action", "onSubmit", "relative", "preventScrollReset", "unstable_viewTransition"],
  ...
  function RouterProvider(_ref) {
    ...
    let setState = React__namespace.useCallback((newState, _ref2) => {
      ...
      if (!viewTransitionOpts || isViewTransitionUnavailable) {
        if (flushSync) {
          ReactDOM__namespace.flushSync(() => setStateImpl(newState));
        } else {
          React__namespace.startTransition(() => setStateImpl(newState));
        }
        return;
      ...
      if (flushSync) {
        ...
        ReactDOM__namespace.flushSync(() => setTransition(t));
        return;
      }
    ...
    return /*#__PURE__*/React__namespace.createElement(React__namespace.Fragment, null, /*#__PURE__*/React__namespace.createElement(DataRouterContext.Provider, {
      ...
      navigator: navigator
    }, /*#__PURE__*/React__namespace.createElement(DataRoutes$1, {
  ...
  /**
    ...
    @category Components
   */
  const Link = /*#__PURE__*/React__namespace.forwardRef(function LinkWithRef(_ref7, forwardedRef) {
    let {
        onClick,
        discover = "render",
        prefetch = "none",
  ...
  */
  ...
  function useDataRouterContext(hookName) {
    let ctx = React__namespace.useContext(DataRouterContext);
    !ctx ? invariant$1(false, getDataRouterConsoleError(hookName))  : void 0;
    return ctx;
  }
  function useDataRouterState(hookName) {
    let state = React__namespace.useContext(DataRouterStateContext);
    !state ? invariant$1(false, getDataRouterConsoleError(hookName))  : void 0;
    return state;
  }
  ...
  action, _temp2) {
    ...
    !routeContext ? invariant$1(false, "useFormAction must be used inside a RouteContext")  : void 0;
  ...
  function useFetcher(_temp3) {
    ...
    !fetcherData ? invariant$1(false, "useFetcher must be used inside a FetchersContext")  : void 0;
    !route ? invariant$1(false, "useFetcher must be used inside a RouteContext")  : void 0;
    !(routeId != null) ? invariant$1(false, "useFetcher can only be used on routes that contain a unique \"id\"")  : void 0;
    ...
    let load = React__namespace.useCallback(async (href, opts) => {
      !routeId ? invariant$1(false, "No routeId available for fetcher.load()")  : void 0;
  ...
  function useViewTransitionState(to, opts) {
    ...
    !(vtContext != null) ? invariant$1(false, "`unstable_useViewTransitionState` must be used within `react-router-dom`'s `RouterProvider`.  " + "Did you accidentally import `RouterProvider` from `react-router`?")  : void 0;
    let {
      basename
  ...
  function StaticRouterProvider(_ref2) {
    ...
    !(router && context) ? invariant$1(false, "You must provide `router` and `context` to <StaticRouterProvider>")  : void 0;
  ...
  function createStaticRouter(routes, context, opts) {
    ...
    return {
      ...
      get future() {
        ...
        return _extends({
          unstable_skipActionErrorRevalidation: false
        }, (_opts = opts) == null ? void 0 : _opts.future);
      },
      get state() {
        return {
  ...
  function createHydratedRouter() {
    ...
    if (!ssrInfo.stateDecodingPromise) {
      ...
      !stream ? invariant(false, "No stream found for single fetch decoding")  : void 0;
    ...
    if (!ssrInfo.context.isSpaMode) {
      ...
      if (initialMatches) {
        for (let match of initialMatches) {
          ...
          if (route && shouldHydrateRouteLoader(manifestRoute, route, ssrInfo.context.isSpaMode) && (route.HydrateFallback || !manifestRoute.hasLoader)) {
    ...
    let {
      ...
      patchRoutesOnMiss
    } = initFogOfWar(ssrInfo.manifest, ssrInfo.routeModules, ssrInfo.context.isSpaMode, ssrInfo.context.basename);

    // We don't use createBrowserRouter here because we need fine-grained control
    // over initialization to support synchronous `clientLoader` flows.
    let router = createRouter(_extends({
      routes,
      history: createBrowserHistory(),
      ...
      hydrationData,
      mapRouteProperties,
      unstable_dataStrategy: getSingleFetchDataStrategy$1(ssrInfo.manifest, ssrInfo.routeModules)
    }, isFogOfWarEnabled ? {
  ...
  function HydratedRouter() {
    ...
    !ssrInfo ? invariant(false, "ssrInfo unavailable for HydratedRouter")  : void 0;
    ...
    return (
      ...
      React__namespace.createElement(React__namespace.Fragment, null, /*#__PURE__*/React__namespace.createElement(FrameworkContext.Provider, {
        value: {
      ...
      if (routerRef.current == null) {
        remixContextRef.current = {
          future: {
            ...
            v3_throwAbortReason: (future == null ? void 0 : future.v3_throwAbortReason) === true
          },
          manifest: {
            routes: {},
            entry: {
   ...
   */
  const createCookieFactory = _ref => {
    let {
      sign,
      unsign
    } = _ref;
    return function (name, cookieOptions) {
      ...
      return {
        ...
        async serialize(value, serializeOptions) {
          return cookie.serialize(name, value === "" ? "" : await encodeCookieValue(sign, value, secrets), _extends({}, options, serializeOptions));
        }
      };
    };
  };
  ...
  async function callRouteAction(_ref) {
    let {
      loadContext,
      action,
      params,
  ...
  async function callRouteLoader(_ref2) {
    let {
      loadContext,
      loader,
      params,
  ...
  function getSingleFetchDataStrategy(responseStubs, _temp) {
    let {
      isActionDataRequest,
      loadRouteIds
    } = _temp === void 0 ? {} : _temp;
  ...
  function getSingleFetchResourceRouteDataStrategy(_ref2) {
    let {
      responseStubs
    } = _ref2;
    return async _ref3 => {
  ...
  async function singleFetchLoaders(serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
    try {
      ...
      loadedMatches.forEach(m => {
        ...
        let data = (_context$loaderData = context.loaderData) == null ? void 0 : _context$loaderData[m.route.id];
        let error = (_context$errors = context.errors) == null ? void 0 : _context$errors[m.route.id];
        if (error !== undefined) {
  ...
  function proxyResponseStubHeadersToHeaders(stub, headers) {
  ...
  function mergeResponseStubs(context, responseStubs, _temp2) {
    let {
      isActionDataRequest
    } = _temp2 === void 0 ? {} : _temp2;
    ...
    for (let stub of stubs) {
      ...
      statusCode !== undefined && statusCode < 300 && stub.status && statusCode !== (actionStub == null ? void 0 : actionStub.status)) {
  ...
  async function handleResourceRequest(serverMode, build, staticHandler, routeId, request, loadContext, handleError) {
    try {
      ...
      if (isResponseStub(response) || response == null) {
        ...
        throw new Error("Expected a Response to be returned from resource route handler");
      }
      return response;
    } catch (error) {
      if (isResponse(error)) {
   ...
   */
  const createSessionStorageFactory = createCookie => _ref => {
    let {
      cookie: cookieArg,
      createData,
      readData,
   ...
   */
  const createCookieSessionStorageFactory = createCookie => function (_temp) {
    let {
      cookie: cookieArg
    } = _temp === void 0 ? {} : _temp;
    let cookie = isCookie(cookieArg) ? cookieArg : createCookie((cookieArg == null ? void 0 : cookieArg.name) || "__session", cookieArg);
   ...
   */
  const createMemorySessionStorageFactory = createSessionStorage => function (_temp) {
    let {
      cookie
    } = _temp === void 0 ? {} : _temp;
    let map = new Map();
  ...
  exports.unstable_composeUploadHandlers = composeUploadHandlers;
  exports.unstable_createMemoryUploadHandler = createMemoryUploadHandler;
  exports.unstable_parseMultipartFormData = parseMultipartFormData;
