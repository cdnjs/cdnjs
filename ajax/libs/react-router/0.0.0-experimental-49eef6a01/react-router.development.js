/**
 * React Router v0.0.0-experimental-49eef6a01
 *
  ...
  */
  ...
  function createHydratedRouter() {
    ...
    if (!ssrInfo.context.isSpaMode) {
      ...
      let mismatchBetweenSsrMatchesAndHydratedMatches = (initialMatches || []).length !== ssrMatches.length || !(initialMatches || []).every((m, i) => ssrMatches[i] === m.route.id);
      if (mismatchBetweenSsrMatchesAndHydratedMatches) {
        let ssr = ssrMatches.join(",");
        let client = initialMatches ? initialMatches.map(m => m.route.id).join(",") : "[]";
        let errorMsg = "SSR Matches (" + ssr + ") do not match client matches at time of " + ("hydration (" + client + "), reloading page...");
