...
function updateProgress(translate) {
  ...
  if (params.loop) {
    const firstSlideIndex = swiper.slides.filter(el => el.getAttribute('data-swiper-slide-index') === '0').index();
    const lastSlideIndex = swiper.slides.filter(el => el.getAttribute('data-swiper-slide-index') * 1 === swiper.slides.length - 1).index();
...
function updateSlidesClasses() {
  ...
  slides.removeClass(`${params.slideActiveClass} ${params.slideNextClass} ${params.slidePrevClass}`);
  let activeSlide;

  if (isVirtual) {
    if (params.loop) {
      activeSlide = getFilteredSlides(`[data-swiper-slide-index="${activeIndex - swiper.virtual.slidesBefore}"]`);
    } else {
  ...
  activeSlide.addClass(params.slideActiveClass); // Next Slide

  let nextSlide = activeSlide.nextAll(`.${params.slideClass}, swiper-slide`).eq(0).addClass(params.slideNextClass);
  ...
  let prevSlide = activeSlide.prevAll(`.${params.slideClass}, swiper-slide`).eq(0).addClass(params.slidePrevClass);
...
function updateActiveIndex(newActiveIndex) {
  ...
  if (swiper.virtual && params.virtual.enabled && params.loop) {
    ...
    realIndex = parseInt(swiper.slides.eq(activeIndex).attr('data-swiper-slide-index') || activeIndex, 10);
  }
...
function updateClickedSlide(e) {
  const swiper = this;
  const params = swiper.params;
  const slide = $(e).closest(`.${params.slideClass}, swiper-slide`)[0];
...
function slideToLoop(index, speed, runCallbacks, internal) {
  ...
  if (swiper.params.loop) {
    if (swiper.virtual && swiper.params.virtual.enabled) {
      ...
      newIndex = swiper.slides.filter(slideEl => slideEl.getAttribute('data-swiper-slide-index') * 1 === newIndex).index();
    }
  }
  ...
  if (params.loop) {
    ...
    if (params.centeredSlides) {
      if (slideToIndex < swiper.loopedSlides - slidesPerView / 2 || slideToIndex > swiper.slides.length - swiper.loopedSlides + slidesPerView / 2) {
        swiper.loopFix();
        slideToIndex = $wrapperEl.children(`${slideSelector}[data-swiper-slide-index="${realIndex}"]`).eq(0).index();
        nextTick(() => {
...
function loopFix(slideRealIndex, slideTo) {
  ...
  const activeSlideIndex = swiper.slides.filter(el => el.classList.contains('swiper-slide-active')).index();
...
function Virtual(_ref) {
  ...
  function renderSlide(slide, index) {
    ...
    const $slideEl = params.renderSlide ? $(params.renderSlide.call(swiper, slide, index)) : swiper.isElement ? $(`<swiper-slide data-swiper-slide-index="${index}">${slide}</swiper-slide>`) : $(`<div class="${swiper.params.slideClass}" data-swiper-slide-index="${index}">${slide}</div>`);
    ...
    if (force) {
      ...
      for (let i = previousFrom; i <= previousTo; i += 1) {
        if (i < from || i > to) {
          ...
          swiper.$slidesEl.find(`.${swiper.params.slideClass}[data-swiper-slide-index="${slideIndex}"], swiper-slide[data-swiper-slide-index="${slideIndex}"]`).remove();
        }
      }
    }
...
function Keyboard(_ref) {
  ...
  function handle(event) {
    ...
    if (swiper.params.keyboard.onlyInViewport && (isPageUp || isPageDown || isArrowLeft || isArrowRight || isArrowUp || isArrowDown)) {
      ...
      if (swiper.$el.parents(`.${swiper.params.slideClass}, swiper-slide`).length > 0 && swiper.$el.parents(`.${swiper.params.slideActiveClass}`).length === 0) {
        return undefined;
      }
...
function Zoom(_ref) {
  ...
  function enable() {
    ...
    swiper.$wrapperEl.on(`pointerup pointercancel`, slideSelector, onGestureEnd, passiveListener); // Move image

    swiper.$wrapperEl.on('pointermove', `.${swiper.params.zoom.containerClass}`, onTouchMove, activeListenerWithCapture);
  }
  ...
  function disable() {
    ...
    swiper.$wrapperEl.off(`pointerup pointercancel`, slideSelector, onGestureEnd, passiveListener); // Move image

    swiper.$wrapperEl.off('pointermove', `.${swiper.params.zoom.containerClass}`, onTouchMove, activeListenerWithCapture);
  }
...
function A11y(_ref) {
  ...
  on('beforeInit', () => {
    liveRegion = $(`<span class="${swiper.params.a11y.notificationClass}" aria-live="assertive" aria-atomic="true" ${swiper.isElement ? 'slot="container-end"' : ''}></span>`);
  });
  on('afterInit', () => {
...
function HashNavigation(_ref) {
  ...
  const onHashChange = () => {
    ...
    if (newHash !== activeSlideHash) {
      const newIndex = swiper.$wrapperEl.children(`.${swiper.params.slideClass}[data-hash="${newHash}"], swiper-slide[data-hash="${newHash}"]`).index();
...
function prependSlide(slides) {
  ...
  if (typeof slides === 'object' && 'length' in slides) {
    for (let i = 0; i < slides.length; i += 1) {
      if (slides[i]) $slidesEl.prepend(slides[i]);
    }

    newActiveIndex = activeIndex + slides.length;
  } else {
    $slidesEl.prepend(slides);
  }
...
function EffectCube(_ref) {
  ...
  const setTranslate = () => {
    ...
    if (params.shadow) {
      if (isHorizontal) {
        $cubeShadowEl = swiper.isElement ? swiper.$el.find('.swiper-cube-shadow') : $wrapperEl.find('.swiper-cube-shadow');
