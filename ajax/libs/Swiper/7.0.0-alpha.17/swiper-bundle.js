...
(function (global, factory) {
  ...
  .replace(/ /g, '.')}`;
    ...
    function EffectFlip({
      ...
      const setTranslate = () => {
        ...
        for (let i = 0; i < slides.length; i += 1) {
          ...
          if (params.transformEl) {
            $slideEl.find(params.transformEl).css({
      ...
      const setTransition = duration => {
        ...
        if (swiper.params.virtualTranslate && duration !== 0) {
          let eventTriggered = false;
          const $transitionEndTarget = transformEl ? slides.eq(activeIndex).find(transformEl) : slides.eq(activeIndex); // eslint-disable-next-line
      ...
      const setTranslate = () => {
        ...
        for (let i = 0; i < slides.length; i += 1) {
          ...
          if (params.transformEl) {
            ...
            $slideEl.transform(transform).css({
              opacity: opacityString
            });
          }
        }
    ...
    // Swiper Class
    const modules = [Virtual, Keyboard, Mousewheel, Navigation, Pagination, Scrollbar, Parallax, Zoom, Lazy, Controller, A11y, History, HashNavigation, Autoplay, EffectFade, EffectCube, EffectFlip, EffectCoverflow, EffectCustom, Thumb, freeMode, Grid, Manipulation];
    Swiper.use(modules);
