...
__webpack_require__.d(__webpack_exports__, {
  "curvesPathName": function() { return /* binding */ curvesPathName; },
  "loadCurvesPath": function() { return /* binding */ loadCurvesPath; }
});
...
function CurvesPathGen(rndFunc, period, nbHarmonics, attenHarmonics, lowValue = 0, highValue = 1) {
  const arP0 = [],
        // 'preceding value' for each harmonic
  arP1 = [],
        // 'succeeding value'
  amplitudes = [],
        // amplitudes oh harmonics
  increments = [],
        // n / period, which will be added to phases for every point
  phases = [],
  ...
  return () => {
    ...
    for (let kh = nbHarmonics; kh >= 1; --kh) {
      ...
      pfl = Math.pow(pf, 2) * (3 - 2 * pf); // always 0..1, but smoother
...
class CurvesPathGenerator {
  ...
  generate(p) {
    ...
    if (p.curveVelocity === undefined) {
      p.curveVelocity = external_commonjs_tsparticles_commonjs2_tsparticles_amd_tsparticles_root_window_.Vector.origin;
