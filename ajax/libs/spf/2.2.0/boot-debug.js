...
function $spf$array$filter$$($arr$$10$$, $fn$$5$$) {
  var $res$$ = [];
  $spf$array$each$$($arr$$10$$, function($a$$, $i$$5$$, $arr$$11$$) {
    $fn$$5$$.call(void 0, $a$$, $i$$5$$, $arr$$11$$) && $res$$.push($a$$);
  });
  return $res$$;
}
...
function $spf$pubsub$publish_$$($topic$$4$$) {
  $topic$$4$$ in $spf$pubsub$subscriptions$$ && $spf$array$each$$($spf$pubsub$subscriptions$$[$topic$$4$$], function($subFn$$1$$, $i$$11$$, $arr$$16$$) {
    $arr$$16$$[$i$$11$$] = null;
    $subFn$$1$$ && $subFn$$1$$();
  });
}
...
function $spf$net$resource$canonicalize$$($type$$92$$, $url$$30$$) {
  var $key$$29_paths$$1$$ = "rsrc-p-" + $type$$92$$;
  if ($url$$30$$) {
    var $index$$45$$ = $url$$30$$.indexOf("//");
    if (0 > $index$$45$$) {
      if (0 == $url$$30$$.lastIndexOf("hash-", 0)) {
        return $url$$30$$;
      }
      $key$$29_paths$$1$$ = $spf$state$values_$$[$key$$29_paths$$1$$] || "";
      if ("string" == typeof $key$$29_paths$$1$$) {
        $url$$30$$ = $key$$29_paths$$1$$ + $url$$30$$;
      } else {
        for (var $p$$ in $key$$29_paths$$1$$) {
          $url$$30$$ = $url$$30$$.replace($p$$, $key$$29_paths$$1$$[$p$$]);
        }
      }
      $url$$30$$ = 0 > $url$$30$$.indexOf("." + $type$$92$$) ? $url$$30$$ + "." + $type$$92$$ : $url$$30$$;
      $url$$30$$ = $spf$url$absolute$$($url$$30$$);
    } else {
      0 == $index$$45$$ && ($url$$30$$ = $spf$url$absolute$$($url$$30$$));
    }
  }
  return $url$$30$$;
}
...
function $spf$net$resource$status$set$$($status$$, $key$$30_type$$94$$, $url$$32$$, $opt_group$$1$$) {
  $key$$30_type$$94$$ = $spf$net$resource$key$$($key$$30_type$$94$$, $url$$32$$, $opt_group$$1$$);
  $spf$net$resource$status_$$[$key$$30_type$$94$$] = $status$$;
}
function $spf$net$resource$status$get$$($key$$31_type$$95$$, $url$$33$$, $opt_group$$2$$) {
  $key$$31_type$$95$$ = $spf$net$resource$key$$($key$$31_type$$95$$, $url$$33$$, $opt_group$$2$$);
...
function $spf$net$resource$url$loaded$$($type$$104$$, $name$$62$$) {
  var $url$$41$$ = $spf$net$resource$url$get$$($type$$104$$, $name$$62$$), $JSCompiler_temp$$4_status$$inline_30$$;
  if ($JSCompiler_temp$$4_status$$inline_30$$ = void 0 != $url$$41$$) {
    $JSCompiler_temp$$4_status$$inline_30$$ = $spf$net$resource$status$get$$($type$$104$$, $url$$41$$), $JSCompiler_temp$$4_status$$inline_30$$ = "" == $url$$41$$ || $JSCompiler_temp$$4_status$$inline_30$$ == $spf$net$resource$State$LOADED$$;
  }
...
function $spf$net$script$require_$$($names$$4$$) {
  $spf$array$each$$($names$$4$$, function($name$$69$$) {
    function $next$$3$$() {
      $spf$net$resource$load$$($url$$46$$, $name$$69$$, void 0);
    }
    var $deps$$ = $spf$net$script$deps_$$[$name$$69$$], $url$$46$$ = $spf$net$script$url_$$[$name$$69$$] || $name$$69$$;
...
var $spf$bootloader$api_$$ = {script:{load:function($url$$42$$, $name$$63$$, $opt_fn$$1$$) {
  $spf$net$resource$load$$($url$$42$$, $name$$63$$, $opt_fn$$1$$);
}, get:function($url$$43$$, $opt_fn$$2$$) {
  $spf$net$resource$create$$($spf$net$resource$Type$JS$$, $url$$43$$, $opt_fn$$2$$);
