...
function nodeToObserverTree(node) {
    ...
    if (isObjectObservable(node) && hasObservers(node))
...
var BaseAtom = (function () {
    ...
    Object.defineProperty(BaseAtom.prototype, "observers", {
        get: function () {
...
var ComputedValue = (function () {
    ...
    Object.defineProperty(ComputedValue.prototype, "observers", {
        get: function () {
            return legacyObservers(this);
        },
        enumerable: true,
    ...
    ComputedValue.prototype.whyRun = function () {
        ...
        if (runReason === RunReason.GET) {
            var requiredBy = globalState.trackingDerivation;
            if (requiredBy)
...
var RunReason;
(function (RunReason) {
    RunReason[RunReason["GET"] = 0] = "GET";
    RunReason[RunReason["NOT_RUNNING"] = 1] = "NOT_RUNNING";
})(RunReason || (RunReason = {}));
var runReasonTexts = (_a = {},
    _a[RunReason.GET] = "[get] The value of this computed value was requested",
...
function legacyObservers(observable) {
    return {
        get length() {
...
function isObjectObservable(arg) {
...
function invariantObservers(observable) {
    var list = observable._observers;
    var map = observable._observersIndexes;
    var l = list.length;
    for (var i = 0; i < l; i++) {
...
function addObserver(observable, node) {
    var l = observable._observers.length;
    if (l) {
...
function removeObserver(observable, node) {
    if (observable._observers.length === 1) {
        ...
        var list = observable._observers;
        var map_1 = observable._observersIndexes;
        var filler = list.pop();
        if (filler !== node) {
...
function endBatch() {
    if (globalState.inBatch === 1) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (observable_1._observers.length === 0) {
...
function propagateChanged(observable) {
    ...
    var observers = observable._observers;
