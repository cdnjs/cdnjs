var niceErrors = {
  ...
  1: function _(annotationType, fieldName) {
  ...
  while (source && source !== objectPrototype) {
    ...
    if (descriptor) {
      ...
      if (source !== adm.target_) {
        ...
        var _actionDescriptor = createActionDescriptor(adm, this, key, descriptor);
...
function makeAutoObservable(target, overrides, options) {
  ...
  try {
    if (target[inferredAnnotationsSymbol]) {
      ...
      var ignoreKeys = (_ignoreKeys = {}, _ignoreKeys[$mobx] = 1, _ignoreKeys.constructor = 1, _ignoreKeys);
