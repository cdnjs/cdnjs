...
function nodeToObserverTree(node) {
    ...
    if (node.observers && !node.observers.isEmpty())
        result.observers = unique(node.observers.asArray()).map(nodeToObserverTree);
    return result;
}
...
function bindDependencies(derivation, prevObserving) {
    ...
    var newLength = observing.length = derivation.l;
...
function clearObserving(derivation) {
    derivation.observing.forEach(function (dep) { return removeObserver(dep, derivation); });
...
function addObserver(observable, node) {
    var wasEmpty = observable.observers.isEmpty();
...
var Reaction = (function () {
    function Reaction(name, onInvalidate) {
        ...
        this.observers = EMPTY_DERIVATION_SET || (EMPTY_DERIVATION_SET = new FastSet());
        this.observing = [];
        this.diffValue = 0;
        this.runId = 0;
        this.laRunId = 0;
...
var FastSet = (function () {
    ...
    Object.defineProperty(FastSet.prototype, "length", {
        get: function () {
            return this.size;
        },
        enumerable: true,
    ...
    FastSet.prototype.asArray = function () {
    ...
    FastSet.prototype.cloneAndClear = function () {
...
function quickDiff(current, base) {
    ...
    if (!current || !current.length)
        return [[], base];
    base.forEach(function (t) { return t['__diff'] = 0; });
    current.forEach(function (t) { return t['__diff'] = 0; });
    base.forEach(function (t) { return t['__diff'] -= 1; });
    current.forEach(function (t) { return t['__diff'] += 1; });
    return [
        current.filter(function (t) { return t['__diff'] === 1; }),
        base.filter(function (t) { return t['__diff'] === -1; })
    ];
}
function quickDiff2(current, base) {
    if (!base || !base.length)
        return [current, []];
    if (!current || !current.length)
        return [[], base];
