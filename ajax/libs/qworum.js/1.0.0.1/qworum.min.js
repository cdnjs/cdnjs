...
document.addEventListener("DOMContentLoaded",function(){var a=document.getElementById(buttonId("tocontentscript"));null==a&&(a=document.createElement("button"),a.setAttribute("id",buttonId("tocontentscript")),a.setAttribute("style","display:none"),document.body.appendChild(a));a.addEventListener("click",webCallback);a.setAttribute("data-event","page loaded");a.click()});
var qworum={config:{},_configDefaults:{singlePageService:!0,replaceState:!0},my:{stack:[{}],data:{}},eval:function(a){var c=document.getElementById(this.state._idToContentScript);try{a=JSON.stringify(a)}catch(b){a=JSON.stringify(["fault","message"])}c.setAttribute("data-event","eval");c.setAttribute("data-message",a);console.log("------------");console.log("> event:           "+c.getAttribute("data-event"));console.log("> message:         "+c.getAttribute("data-message"));c.click()},state:{_stack:[{}],
_idToContentScript:"qworum-tocontentscript-c046fc737747d09f02cddfc3d0b3a72ecbf4b3a8",_idFromContentScript:"qworum-fromcontentscript-c046fc737747d09f02cddfc3d0b3a72ecbf4b3a8",_idForm:"qworum-form-c046fc737747d09f02cddfc3d0b3a72ecbf4b3a8",_json_ns:"http://qworum.net/json",_serializer:new XMLSerializer,_parser:new DOMParser,_contentScriptCallback:function(){var a=null,c=document.getElementById(qworum.state._idFromContentScript),a=c.getAttribute("data-event");console.log("------------");console.log("< event:                  "+
a);if("eval"==a){console.log("< callReturns:            "+c.getAttribute("data-callReturns"));console.log("< currentCallVarsChanged: "+c.getAttribute("data-currentCallVarsChanged"));console.log("< newCallVars:            "+c.getAttribute("data-newCallVars"));console.log("< url:                    "+c.getAttribute("data-url"));console.log("< query:                  "+c.getAttribute("data-query"));a={callReturns:JSON.parse(c.getAttribute("data-callReturns")),currentCallVarsChanged:JSON.parse(c.getAttribute("data-currentCallVarsChanged")),
newCallVars:JSON.parse(c.getAttribute("data-newCallVars")),url:document.createElement("a"),query:JSON.parse(c.getAttribute("data-query"))};for(a.url.href=c.getAttribute("data-url");;){c=document.createElement("a");c.href=document.URL;if(c.protocol!=a.url.protocol)break;if(c.host!=a.url.host)break;if(!(null==qworum.config.singlePageService?qworum._configDefaults.singlePageService:qworum.config.singlePageService))break;if(a.callReturns>=qworum.state._stack.length)break;for(var b=a.callReturns;0<b;b--)qworum.state._stack.shift(),
qworum.my.stack.shift();for(var e in a.currentCallVarsChanged)qworum.state._stack[0][e]=qworum.state._parser.parseFromString(a.currentCallVarsChanged[e],"application/xml");if(null!=a.newCallVars){for(e in a.newCallVars)a.newCallVars[e]=qworum.state._parser.parseFromString(a.newCallVars[e],"application/xml");qworum.state._stack.unshift(a.newCallVars);qworum.my.stack.unshift({})}if(c.pathname==a.url.pathname&&c.search==a.url.search)window.location.replace(a.url.pathname+a.url.search+a.url.hash);else{if(!(null==
qworum.config.replaceState?qworum._configDefaults.replaceState:qworum.config.replaceState))break;window.history.replaceState(null,null,a.url.pathname+a.url.search+a.url.hash)}return}a.query?(e=document.createElement("form"),e.setAttribute("id",qworum.state._idForm),e.setAttribute("action",a.url.href),e.setAttribute("method","POST"),c=document.createElement("input"),c.setAttribute("name","qworum"),c.setAttribute("value",a.query),e.setAttribute("style","display:none"),e.appendChild(c),document.body.appendChild(e),
document.forms[qworum.state._idForm].submit()):window.location=a.url.href}},set:function(a,c){var b=document.getElementById(this._idToContentScript);if(null!=a&&"string"==typeof a){if(c instanceof Document||c instanceof Element)c=this._parser.parseFromString(this._serializer.serializeToString(c),"application/xml");else try{var e=this._parser.parseFromString("<data xmlns='"+this._json_ns+"' />","application/xml");e.documentElement.appendChild(e.createTextNode(JSON.stringify(c)));c=e}catch(d){return}this._stack[0][a]=
c;b.setAttribute("data-event","state.set");b.setAttribute("data-key",a);b.setAttribute("data-value",this._serializer.serializeToString(c));b.click()}},get:function(a){a=this._stack[0][a];return null==a?a:a.documentElement.namespaceURI==this._json_ns?JSON.parse(a.documentElement.textContent):this._parser.parseFromString(this._serializer.serializeToString(a),"application/xml")}}};
function webCallback(){var a=document.getElementById(buttonId("tocontentscript")),c=a.getAttribute("data-event");console.log("> "+c);"state.set"==c?(console.log("------------"),console.log(">> event:    set variable"),console.log(">> name:    "+a.getAttribute("data-key")),console.log(">> value:   "+a.getAttribute("data-value")),contentScriptCallback({event:"set variable",name:a.getAttribute("data-key"),value:a.getAttribute("data-value")},function(){})):"eval"==c?(console.log("------------"),console.log(">> event: message"),
console.log(">> xml:   "+messageToXml(a.getAttribute("data-message"))),console.log(">> url:   "+document.URL),contentScriptCallback({event:"message",xml:messageToXml(a.getAttribute("data-message")),url:""+document.URL,referrer:document.URL},function(b){console.log("------------");console.log("<< url:                    "+b.url);console.log("<< callReturns:            "+b.callReturns);console.log("<< currentCallVarsChanged: "+JSON.stringify(b.currentCallVarsChanged));console.log("<< newCallVars:            "+
JSON.stringify(b.newCallVars));a=document.getElementById(buttonId("fromcontentscript"));a.setAttribute("data-event","eval");a.setAttribute("data-callReturns",b.callReturns);a.setAttribute("data-currentCallVarsChanged",JSON.stringify(b.currentCallVarsChanged));a.setAttribute("data-newCallVars",JSON.stringify(b.newCallVars));a.setAttribute("data-url",b.url);a.setAttribute("data-query",JSON.stringify(b.query));a.click()})):"page loaded"==c&&(a=document.createElement("button"),a.setAttribute("id",buttonId("fromcontentscript")),
a.setAttribute("style","display:none"),document.body.appendChild(a),a.addEventListener("click",qworum.state._contentScriptCallback))}function messageToXml(a){var c=new DOMParser,b=c.parseFromString("<dummy />","application/xml");try{b.replaceChild(statementToElement(["sequence",JSON.parse(a)],b,c,"http://qworum.net/"),b.documentElement)}catch(e){b.replaceChild(statementToElement(["fault","message",e],b,c,"http://qworum.net/"),b.documentElement)}return(new XMLSerializer).serializeToString(b)}
function statementToElement(a,c,b,e){if(a instanceof Array){var d=a.shift();if("string"!=typeof d)throw"Statement name must be string.";if("fault"==d){b=a.shift();if(null!=b&&"string"!=typeof b)throw"Fault type must be string.";a=a.shift();if(null!=a&&"string"!=typeof a)throw"Fault title must be string.";var f=c.createElementNS(e,"qrm:"+d);null!=b&&f.setAttribute("type",b);null!=a&&(e=c.createElementNS(e,"title"),e.appendChild(c.createTextNode(a)),f.appendChild(e));return f}if("call"==d||"goto"==
d){f=c.createElementNS(e,"qrm:"+d);d=a.shift();if(null!=d){if("string"!=typeof d)throw"Phase URL must be string.";f.setAttribute("href",d)}d=a.shift();null!=d&&(d=statementToElement(d,c,b,e),f.appendChild(d));a=a.shift();if(null!=a){if("string"!=typeof a)throw"Phase title must be string.";e=c.createElementNS(e,"title");e.appendChild(c.createTextNode(a));f.appendChild(e)}return f}if("if"==d){f=c.createElementNS(e,"qrm:"+d);d=a.shift();if(null==d)throw"If must have a condition statement.";d=statementToElement(d,
c,b,e);f.appendChild(d);d=a.shift();if(null==d)throw"If must have a then statement.";d=statementToElement(d,c,b,e);f.appendChild(d);a=a.shift();null!=a&&(a=statementToElement(a,c,b,e),f.appendChild(a));return f}if("nil"==d)return c.createElementNS(e,"nil");if("return"==d)return f=c.createElementNS(e,"qrm:"+d),d=a.shift(),null!=d&&(d=statementToElement(d,c,b,e),f.appendChild(d)),f;if("transient"==d||"variable"==d){var f=c.createElementNS(e,"qrm:"+d),i=a.shift();if(null==i)throw"Name required for "+
d+".";if("string"!=typeof i)throw"Name must be string for "+d+".";f.setAttribute("name",i);d=a.shift();null!=d&&(d=statementToElement(d,c,b,e),f.appendChild(d));return f}if("select"==d){f=c.createElementNS(e,"qrm:"+d);d=a.shift();if(null==d)throw"XPath required for select.";if("string"!=typeof d)throw"Select XPath must be string.";f.setAttribute("xpath",d);d=a.shift();if(null!=d){if("string"!=typeof d)throw"Select namespaces must be string.";f.setAttribute("namespaces",d)}d=a.shift();null!=d&&(d=
statementToElement(d,c,b,e),f.appendChild(d));return f}if("sequence"==d){for(f=c.createElementNS(e,"qrm:"+d);;){d=a.shift();if(null==d)break;f.appendChild(statementToElement(d,c,b,e))}return f}if("transform"==d){f=c.createElementNS(e,"qrm:"+d);d=a.shift();if(null==d)throw"XSL required for transform.";if("string"!=typeof d)throw"Transform XSL must be string.";d=statementToElement(d,c,b,e);f.appendChild(d);d=a.shift();if(null==d)throw"Statement required for transform.";d=statementToElement(d,c,b,e);
f.appendChild(d);return f}if("try"==d){f=c.createElementNS(e,"qrm:"+d);d=a.shift();if(null==d)throw"Tried statement required for try.";d=statementToElement(d,c,b,e);f.appendChild(d);for(d=a.shift();;){if(null==d)throw"Try must have a catch clause.";if(!(d instanceof Array))throw"Catch clause must be array.";i=c.createElementNS(e,"catch");f.appendChild(i);var l=d.shift();if(null!=l){if("string"!=typeof l)throw"Caught types must be string.";i.setAttribute("types",l)}for(;;){l=d.shift();if(null==l)break;
l=statementToElement(l,c,b,e);i.appendChild(l)}d=a.shift();if(null==d)break}return f}if("json"==d){f=c.createElementNS(e+d,"data");try{f.appendChild(c.createTextNode(JSON.stringify(a.shift())))}catch(g){throw"Data must be serializable to JSON.";}return f}throw"Unknown statement.";}if("string"==typeof a)try{return f=b.parseFromString(a,"application/xml"),c.adoptNode(f.documentElement)}catch(k){throw"String is not XML.";}else throw"Statement must be array or string.";}
function buttonId(a){return"qworum-"+a+"-c046fc737747d09f02cddfc3d0b3a72ecbf4b3a8"}var state=new QworumInterpreter;
function contentScriptCallback(a,c){var b=state;if("page loaded"==a.event)c(b.inSession);else if("set variable"==a.event)null==a.value&&(a.value="<nil xmlns='"+QWORUM_NAMESPACE+"'/>"),b.currentFrame().setVariable(a.name,new QworumXML(a.value)),console.log(b.toString());else if("web page"==a.event)try{c(b.currentFrame().variablesToSerializable())}catch(e){}else if("message"==a.event){var d=a,f=new QworumURI(d.url),i=new QworumXML(d.xml);i.findDescendent(i.getRoot(),function(a){return"http://www.w3.org/1999/xhtml"==
a.namespaceURI&&"parsererror"==a.localName})&&(i=new QworumXML("<fault type='message' xmlns='"+QWORUM_NAMESPACE+"'><title>XML is not well-formed.</title></fault>"));console.log(b.toString());d=new QworumMessage(i,f);console.log("interpreting message: "+d.toString());i=null;try{i=b.eval(d)}catch(l){console.log("Error: "+l)}console.log(b.toString());if(i.result){if(i.result.data)f=["Session finished with:"],148<i.result.data.length?f.push(i.result.data.substring(0,148)+" ..."):f.push(i.result.data);
