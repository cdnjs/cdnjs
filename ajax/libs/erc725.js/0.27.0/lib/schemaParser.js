...
const fillDynamicKeyPart = (key, keySchema) => {
    ...
    if (isDynamicKeyName(keySchema.name)) {
        ...
        result.dynamicKeyPart = dynamicKeyPart;
        return result;
    }
...
const findArraySchemaForKey = (key, schemas) => {
    ...
    return Object.assign(Object.assign({}, arraySchema), { key, dynamicName: arraySchema.name.replace('[]', `[${elementIndex}]`), dynamicKeyPart: `0x${key.substring(34)}`, name: arraySchema.name, keyType: 'Singleton' });
};
