...
// PUBLIC FUNCTION
export async function supportsInterface(interfaceIdOrName, options) {
    ...
    return internalSupportsInterface(interfaceIdOrName, {
        ...
        provider: options.provider ||
            initializeProvider(options.rpcUrl, options?.gas ? options?.gas : DEFAULT_GAS_VALUE),
    });
}
/**
...
export class ERC725 {
    ...
    constructor(schemas, address, provider, config) {
        ...
        this.options = {
            schemas: this.validateSchemas(schemas.flatMap((schema) => duplicateMultiTypeERC725SchemaEntry(schema)), config?.throwSchemaErrors || defaultConfig.throwSchemaErrors),
            address,
    ...
    async fetchData(keyOrKeys) {
        ...
        const dataFromExternalSources = await getDataFromExternalSources(schemas, dataFromChain, this.options);
        if (keyOrKeys &&
