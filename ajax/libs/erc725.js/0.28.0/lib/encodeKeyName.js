 ...
 * @date 2022
 */
import { isAddress, isHex, slice, stringToBytes, toBytes } from 'viem';
...
export const encodeDynamicKeyPart = (type, value_, bytes) => {
    ...
    switch (baseType) {
        ...
        case 'bool': {
            ...
            value === 'true' || Number(value) === 1 ? '0x01' : '0x00', { size: bytes }).slice(2);
        }
        case 'address': {
            if (!value) {
                throw new Error(`Wrong value: ${value} for dynamic key with type: <address>. Value is empty.`);
            }
        ...
        case 'uint': {
            ...
            return pad(hex, { size: bytes }).slice(2);
        }
        case 'int':
            // TODO:
...
const encodeDynamicMappingWithGrouping = (name, dynamicKeyParts) => {
    ...
    if (keyNameSplit[1].startsWith('0x')) {
        secondPart = pad(keyNameSplit[1].slice(0, 2 + 4 * 2), {
            size: 4,
            dir: 'right',
        }).slice(2);
    }
    ...
    if (keyNameSplit[2].startsWith('0x')) {
        lastPart = pad(keyNameSplit[2].slice(0, 2 + 20 * 2), {
            size: 20,
            dir: 'right',
        }).slice(2);
    }
