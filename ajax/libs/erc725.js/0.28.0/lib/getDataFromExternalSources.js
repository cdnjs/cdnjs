...
export const getDataFromExternalSources = (schemas, dataFromChain, ipfsGateway) => {
    const promises = dataFromChain.map(async (dataEntry) => {
        ...
        try {
            ...
            const { url } = patchIPFSUrlsIfApplicable(urlDataWithHash, typeof ipfsGateway === 'string' ? ipfsGateway : ipfsGateway.ipfsGateway);
            let receivedData;
            ...
            if (receivedData.length >= 2) {
                ...
                try {
                    ...
                    if (/^(\[.*\]|\{.*\})\s*$/.test(key)) {
                        ...
                        throw new Error(`Hash mismatch, calculated hashes ("${mismatchedHashes.join('", "')}") are both different from expected hash "${urlDataWithHash.verification.data}"`);
                    }
                }
                catch {
                    // ignore
