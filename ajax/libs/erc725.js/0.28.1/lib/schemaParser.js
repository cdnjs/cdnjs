 ...
 * @date 2022
 */
import { concat, keccak256, slice, toBytes } from 'viem';
...
const fillDynamicKeyPart = (key, keySchema) => {
    const fillOneDynamicKeyPart = (key, keySchema, insertQuestionMarks = false) => {
        ...
        if (concat([slice(keccak256(toBytes(keyNameParts[1])), 0, 10), '0x0000']) ===
            `0x${secondWordHex}`) {
...
const findMappingWithGroupingSchemaForKey = (key, schemas) => {
    let keySchema = schemas.find((schema) => schema.key.substring(0, 26) === key.substring(0, 26)) || null;
    if (keySchema) {
        keySchema = { ...keySchema };
        const keyNameParts = keySchema.name.split(':');
