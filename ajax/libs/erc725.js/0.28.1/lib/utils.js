...
import { concat, hexToBytes, isAddress, isHex, pad, size, slice, toHex, } from 'viem';
/**
 *
...
export function encodeKeyValue(valueContent, valueType, decodedValue, name) {
    ...
    if (Array.isArray(decodedValue)) {
        ...
        for (let index = 0; index < decodedValue.length; index++) {
            ...
            results.push(encodeValueContent(valueContent.replace(/\[.*?\]$/, ''), element));
        }
        result = results;
    }
    if (
...
export function encodeArrayKey(key, index) {
    return concat([slice(key, 0, 16), pad(toHex(index), { size: 16 })]);
}
/**
 *
    ...
    switch (lowerCaseKeyType) {
        ...
        case 'mapping':
            if (isValidTuple(schema.valueType, schema.valueContent)) {
                ...
                const isCompactBytesArray = schema.valueType.endsWith(COMPACT_BYTES_ARRAY_STRING);
...
_value, name) {
    ...
    if (!valueContentEncodingMethods && valueContent.slice(0, 2) !== '0x') {
        throw new Error(`The valueContent "${valueContent}" for "${typeof name === 'object' ? name.name : name}" is not supported.`);
    }
    ...
    if (!valueTypeIsBytesNonArray && valueType !== 'string') {
        ...
        value = decodeValueType(valueType, value, typeof name === 'object' ? name : undefined);
    }
...
export function patchIPFSUrlsIfApplicable(receivedData, ipfsGateway) {
    ...
    if (receivedData?.url?.startsWith('ipfs://')) {
        ...
        if (typeof ipfsGateway !== 'object' || !ipfsGateway.ipfsGateway) {
            throw new Error('Invalid ipfsGateway provided. It should be a string or an object with an ipfsGateway property.');
        }
...
export const duplicateMultiTypeERC725SchemaEntry = (schema) => {
    ...
    const baseSchema = {
        ...schema,
        name: nameGroups.shift(), // The first element of the key name is never dynamic.
        key: baseKey,
    };
 ...
 */
export function negateSignedBigInt(value, bitLength) {
