import { hexToBigInt, isHex, pad, toHex } from 'viem';
...
export function decodePermissions(permissionHex) {
    ...
    const passedPermissionDecimal = permissionHex === '0x' ? 0n : hexToBigInt(permissionHex);
    // Do not use an for of loop here to not require the regenerator runtime
    ...
    for (let i = 0; i < permissionsToTest.length; i += 1) {
        ...
        const decimalTestPermission = hexToBigInt(LSP6_DEFAULT_PERMISSIONS[testPermission]);
 ...
 */
export const checkPermissions = (requiredPermissions, _grantedPermissions) => {
