 ...
 * @date 2020
 */
import { checkAddressChecksum, hexToBytes, isAddress, isHexStrict, leftPad, numberToHex, padLeft, stripHexPrefix, } from 'web3-utils';
 ...
 */
export function isDataAuthentic(data, options) {
    if (!options || !options.method) {
        return true;
    }
    const dataHash = hashData(data, options.method);
    if (dataHash !== options.data) {
        console.error(`Hash mismatch, returned JSON hash ("${dataHash}") is different from expected hash: "${options.method}"`);
        return false;
    }
    return true;
}
...
export function patchIPFSUrlsIfApplicable(receivedData, ipfsGateway) {
    var _a;
    if (((_a = receivedData === null || receivedData === void 0 ? void 0 : receivedData.url) === null || _a === void 0 ? void 0 : _a.indexOf('ipfs://')) !== -1) {
