...
(function (global, factory) {
  ...
  function useValidationState({ name, initValue, form, type, checkedValue, standalone, }) {
      ...
      const initialValueSourceRef = vue.ref(vue.unref(initValue));
      const initialValue = vue.computed(() => {
          return getFromPath(vue.unref(formInitialValues), vue.unref(name), vue.unref(initialValueSourceRef));
      });
      const value = useFieldValue$1(initialValueSourceRef.value === undefined ? initialValue : initialValueSourceRef, name, form);
      ...
      function resetValidationState(state) {
          ...
          if (form) {
              ...
              initialValueSourceRef.value = klona(newValue);
          }
  ...
  function useForm(opts) {
      ...
      function insertFieldAtPath(field, path) {
          ...
          const fieldGroup = fieldsByPath.value[fieldPath];
      ...
      function fieldGroupExists(path) {
      ...
      async function validate() {
          ...
          const validations = await Promise.all(Object.values(fieldsByPath.value).map(fieldGroup => {
              const field = normalizeField(fieldGroup);
              if (!field) {
                  return Promise.resolve({ key: '', valid: true, errors: [] });
              }
              return field.validate().then((result) => {
                  return {
