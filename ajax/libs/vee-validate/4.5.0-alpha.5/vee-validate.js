...
(function (global, factory) {
  ...
  function useCheckboxField(name, rules, opts) {
      ...
      const field = form.fieldsByPath.value[vue.unref(name)];
      if (!field) {
          return patchCheckboxApi(_useField(name, rules, opts));
      }
      field.instances++;
  ...
  function useFieldArray(opts) {
      ...
      if (!vue.unref(opts.name)) {
          warn('FieldArray requires a field path to be provided, did you forget to pass the `name` prop?');
          return noOpApi;
      }
      if (!vue.unref(opts.keyPath)) {
      ...
      function createEntry(value, keyFallback) {
          const key = getFromPath(value, vue.unref(opts.keyPath), keyFallback);
          return {
              key,
              value,
              isFirst: false,
      ...
      vue.watch(() => getFromPath(form === null || form === void 0 ? void 0 : form.values, vue.unref(opts.name), []), values => {
      ...
      function remove(idx) {
          const pathName = vue.unref(opts.name);
  ...
  const FieldArray = vue.defineComponent({
      ...
      setup(props, ctx) {
          const { push, remove, swap, insert, entries } = useFieldArray(vue.toRefs(props));
          function slotProps() {
              return {
