    ...
    data: {
      ...
      checks: {
        ...
        'angular-ng-click': {
          ...
          messages: {
            pass: function anonymous(it) {
              var out = 'ng-click is focusable and has an accessible role';
              return out;
            },
            fail: function anonymous(it) {
              var out = 'ng-click is not focusable or missing an accessible role';
              return out;
            }
          }
        },
    ...
    checks: [ {
      ...
      evaluate: function evaluate(node, options) {
        ...
        function testNodeForA11y(currentNode) {
          var tabIndex = currentNode.getAttribute('tabindex'), focusableWithTabindex = axe.commons.dom.isFocusable(currentNode) && tabIndex > -1;
          // if the element is natively focusable without tabindex, it passes
          if (axe.commons.dom.isFocusable(currentNode) && tabIndex === null) {
            return true;
          }
          var accRoles = [ 'button', 'checkbox', 'tab', 'menuitem', 'menuitemcheckbox' ], roleAttr = currentNode.getAttribute('role');
