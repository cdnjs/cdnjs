!function(global, factory) {
    ...
    function getNodeData(node) {
        if (void 0 !== node[ATTR_KEY]) return node[ATTR_KEY]; else ;
    }
    ...
    function setAccessor(node, name, value) {
        ...
        } else if ('key' !== name) if ('type' !== name && name in node) {
            node[name] = value;
            if (falsey(value)) node.removeAttribute(name);
        } else if ('on' === name.substring(0, 2)) {
    ...
    function getRawNodeAttributes(node) {
        ...
        for (;i--; ) attrs[list[i].name] = list[i].value;
        return attrs;
    }
    ...
    function createNode(nodeName) {
        var name = toLowerCase(nodeName), list = nodes[name], node = list && list.pop() || document.createElement(name);
        ensureNodeData(node);
    ...
    function diff(dom, vnode, context, mountAll) {
        ...
        var out = dom, nodeName = String(vnode.nodeName);
    ...
    function getKey(child) {
        ...
        if (data && !empty(data.key)) return data.key; else ;
    }
    function innerDiffNode(dom, vchildren, context, mountAll) {
        ...
        if (len) {
            children = [];
            for (var i = 0; len > i; i++) {
                var child = originalChildren[i], key = child._component ? child._component.__key : getKey(child);
                if (key || 0 === key) {
    ...
    function diffAttributes(dom, vnode) {
        var old = getNodeData(dom) || getRawNodeAttributes(dom), attrs = vnode.attributes;
        for (var _name in old) if (!attrs || !hasOwnProperty.call(attrs, _name)) setAccessor(dom, _name, null);
        if (attrs) for (var _name2 in attrs) {
            var value = void 0 === attrs[_name2] ? null : attrs[_name2];
            if (value != old[_name2]) setAccessor(dom, _name2, value);
        }
    }
    ...
    function setComponentProps(component, props, opts, context) {
        ...
        if (void 0 !== component.base && null !== component.base) hook(component, 'componentWillReceiveProps', props, context);
        ...
        if (opts !== NO_RENDER) if (opts === SYNC_RENDER || options.syncComponentUpdates !== !1) renderComponent(component); else triggerComponentRender(component);
