!function(global, factory) {
    ...
    function setAccessor(node, name, value) {
        ...
        if ('key' !== name && 'children' !== name) if ('class' === name) node.className = value || ''; else if ('style' === name) node.style.cssText = value || ''; else if ('dangerouslySetInnerHTML' === name) {
            ...
            var type = normalizeEventName(name), l = node._listeners || (node._listeners = {});
    ...
    function getRawNodeAttributes(node) {
        var list = node.attributes, attrs = {}, i = list.length;
        ...
        if (!component._disableRendering) {
            ...
            if (!skip) {
                ...
                if (initialBase && base !== initialBase) {
                    ...
                    if (!toUnmount && initialBase._component === component) recollectNodeTree(initialBase);
                }
