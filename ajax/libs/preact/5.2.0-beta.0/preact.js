!function(global, factory) {
    ...
    function innerDiffNode(dom, vchildren, context, mountAll) {
        ...
        if (len) for (var i = 0; i < len; i++) {
            var _child = originalChildren[i], key = getKey(_child);
            if ((key || 0 === key) && vlen) {
                keyedLen++;
                keyed[key] = _child;
    ...
    function renderComponent(component, opts, mountAll) {
        if (!component._disableRendering) {
            ...
            if (!skip) {
                ...
                if (isUpdate && !mountAll) hook(component, 'componentDidUpdate', previousProps, previousState, previousContext); else {
