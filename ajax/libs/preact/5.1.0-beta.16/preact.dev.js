(function (global, factory) {
  ...
  function setAccessor(node, name, value, isSvg) {
  	...
  	if (name === 'class') {
  		...
  		if (falsey(value)) {
  			if (ns) node.removeAttributeNS('http://www.w3.org/1999/xlink', ns[1]);else node.removeAttribute(name);
  		} else if (typeof value !== 'object' && !isFunction(value)) {
  			if (ns) node.setAttributeNS('http://www.w3.org/1999/xlink', ns[1], value);else node.setAttribute(name, value);
  		}
  	}
  }
