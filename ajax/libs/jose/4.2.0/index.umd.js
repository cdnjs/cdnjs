(function (global, factory) {
  ...
  var decrypt = async (enc, cek, ciphertext, iv, tag, aad) => {
    if (!isCryptoKey(cek) && !(cek instanceof Uint8Array)) {
      throw new TypeError(invalid_key_input_default(cek, "CryptoKey", "Uint8Array"));
    }
  ...
  function getCryptoKey(key, alg, usage) {
    ...
    throw new TypeError(invalid_key_input_default(key, "CryptoKey", "Uint8Array"));
  }
  ...
  var deriveKey = async (publicKey, privateKey, algorithm, keyLength, apu = new Uint8Array(0), apv = new Uint8Array(0)) => {
    if (!isCryptoKey(publicKey)) {
      throw new TypeError(invalid_key_input_default(publicKey, "CryptoKey"));
    }
    checkEncCryptoKey(publicKey, "ECDH-ES");
    if (!isCryptoKey(privateKey)) {
      throw new TypeError(invalid_key_input_default(privateKey, "CryptoKey"));
    }
  ...
  var generateEpk = async (key) => {
    if (!isCryptoKey(key)) {
      throw new TypeError(invalid_key_input_default(key, "CryptoKey"));
    }
  ...
  async function decryptKeyManagement(alg, key, encryptedKey, joseHeader) {
    ...
    switch (alg) {
      ...
      case "ECDH-ES+A256KW": {
        ...
        const sharedSecret = await deriveKey(epk, key, alg === "ECDH-ES" ? joseHeader.enc : alg, parseInt(alg.substr(-5, 3), 10) || bitLength2(joseHeader.enc), partyUInfo, partyVInfo);
        if (alg === "ECDH-ES")
          return sharedSecret;
        if (encryptedKey === void 0)
  ...
  async function encryptKeyManagement(alg, enc, key, providedCek, providedParameters = {}) {
    ...
    switch (alg) {
      ...
      case "ECDH-ES+A256KW": {
        ...
        const sharedSecret = await deriveKey(key, ephemeralKey, alg === "ECDH-ES" ? enc : alg, parseInt(alg.substr(-5, 3), 10) || bitLength2(enc), apu, apv);
  ...
  var GeneralEncrypt = class {
    ...
    async encrypt(options) {
      ...
      if (this._recipients.length === 1) {
        ...
        const flattened = await new FlattenedEncrypt(this._plaintext).setAdditionalAuthenticatedData(this._aad).setProtectedHeader(this._protectedHeader).setSharedUnprotectedHeader(this._unprotectedHeader).setUnprotectedHeader(unprotectedHeader).encrypt(key, { ...recipientOpts, ...options });
        let jwe2 = {
      ...
      for (let i = 0; i < this._recipients.length; i++) {
        ...
        if (!is_disjoint_default(this._protectedHeader, this._unprotectedHeader, unprotectedHeader)) {
          throw new JWEInvalid("JWE Protected, JWE Shared Unprotected and JWE Per-Recipient Header Parameter names must be disjoint");
        }
        const joseHeader = {
          ...this._protectedHeader,
        ...
        validate_crit_default(JWEInvalid, new Map(), recipientOpts === null || recipientOpts === void 0 ? void 0 : recipientOpts.crit, this._protectedHeader, joseHeader);
        if (joseHeader.zip !== void 0) {
          if (!this._protectedHeader || !this._protectedHeader.zip) {
            throw new JWEInvalid('JWE "zip" (Compression Algorithm) Header MUST be integrity protected');
          }
      ...
      for (let i = 0; i < this._recipients.length; i++) {
        ...
        const { encryptedKey, parameters } = await encrypt_key_management_default((unprotectedHeader === null || unprotectedHeader === void 0 ? void 0 : unprotectedHeader.alg) || ((_a = this._protectedHeader) === null || _a === void 0 ? void 0 : _a.alg) || ((_b = this._unprotectedHeader) === null || _b === void 0 ? void 0 : _b.alg), enc, key, cek);
