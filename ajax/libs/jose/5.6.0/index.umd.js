...
-----END ${descriptor}-----`;
  ...
  async function decryptKeyManagement(alg, key, encryptedKey, joseHeader, options) {
    if (normalizePrivateKey) {
      key = await normalizePrivateKey(key, alg);
    }
  ...
  async function encryptKeyManagement(alg, enc, key, providedCek, providedParameters = {}) {
    ...
    if (normalizePublicKey) {
      key = await normalizePublicKey(key, alg);
    }
  ...
  async function getCryptoKey3(alg, key, usage) {
    if (normalizePrivateKey && usage === "sign") {
      key = await normalizePrivateKey(key, alg);
    }
    if (normalizePublicKey && usage === "verify") {
      key = await normalizePublicKey(key, alg);
    }
    if (isCryptoKey(key)) {
