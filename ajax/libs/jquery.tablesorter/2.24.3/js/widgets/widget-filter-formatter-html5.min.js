...
k=i.skipTest||"range"===j.attr("type")&&"test"!==j.val(),l=[],m=e.closest("table")[0].config,n=function(b,d,g){/*jshint eqeqeq:false */
// hidden input changes may include compare symbols
b=("undefined"==typeof b?h.val():b).toString().replace(/[<>=]/g,"")||i.value;var j=(a.isArray(i.compare)?e.find(c).val()||i.compare[i.selected||0]:i.compare)||"",k=" ("+(j?j+b:b==i.min?i.allText:b)+")",n=m.$table[0].hasInitialized?(d?d:i.delayed)||"":!0;e.find("input[type=hidden]").val(j?j+b:b==i.min?"":(i.exactMatch?"=":"")+b).trigger(g?"":"search",n).end().find(".range").val(b),
// or add current value to the header cell, if desired
e.closest("thead").find("th[data-column="+f+"]").find(".curvalue").html(k),
// update sticky header cell
l.length&&(l.find(".range").val(b).end().find(c).val(j),l.closest("thead").find("th[data-column="+f+"]").find(".curvalue").html(k))};
// add HTML5 range
...
// has sticky headers?
// on reset
return j.remove(),k&&(e.html('<input type="hidden"><input class="range" type="range" min="'+i.min+'" max="'+i.max+'" value="'+i.value+'" />').closest("thead").find("th[data-column="+f+"]").addClass("filter-parsed").find(".tablesorter-header-inner").append('<span class="curvalue" />'),h=e.find("input[type=hidden]").bind("change"+m.namespace+"filter",function(){/*jshint eqeqeq:false */
var b=this.value,d=(a.isArray(i.compare)?e.find(c).val()||i.compare[i.selected||0]:i.compare)||"";b!==this.lastValue&&(this.lastValue=d?d+b:b==i.min?"":(i.exactMatch?"=":"")+b,this.value=this.lastValue,n(b))}),e.find(".range").bind("change",function(){n(this.value)}),m.$table.bind("filterFomatterUpdate",function(){var a=d.updateCompare(e,h,i)[0];e.find(".range").val(a),n(a,!1,!0),b.filter.formatterUpdated(e,f)}),i.compare&&(d.addCompare(e,f,i),e.find(c).bind("change",function(){n()})),m.$table.bind("stickyHeadersInit",function(){l=m.widgetOptions.$sticky.find(".tablesorter-filter-row").children().eq(f).empty(),l.html('<input class="range" type="range" min="'+i.min+'" max="'+i.max+'" value="'+i.value+'" />').find(".range").bind("change",function(){n(l.find(".range").val())}),n(),i.compare&&(d.addCompare(l,f,i),l.find(c).bind("change",function(){e.find(c).val(a(this).val()),n()}))}),e.closest("table").bind("filterReset",function(){a.isArray(i.compare)&&e.add(l).find(c).val(i.compare[i.selected||0]),setTimeout(function(){n(i.value,!1,!0)},0)}),n()),k?e.find('input[type="hidden"]'):a('<input type="search">')},/**********************\
		HTML5 Color picker
		\**********************/
html5Color:function(c,d,e){var f,g,h=a.extend({value:"#000000",disabled:!1,addToggle:!0,exactMatch:!0,valueToHeader:!1,skipTest:!1},e),
// Add a hidden input to hold the range values
i=a('<input type="color" style="visibility:hidden;" value="test">').appendTo(c),
// test if HTML5 color is supported - from Modernizr
j=h.skipTest||"color"===i.attr("type")&&"test"!==i.val(),k=[],l=c.closest("table")[0].config,m=function(a,b){a=("undefined"==typeof a?g.val():a).toString().replace("=","")||h.value;var e=!0,f=" ("+a+")";h.addToggle&&(e=c.find(".toggle").is(":checked")),c.find(".colorpicker").length&&(c.find(".colorpicker").val(a)[0].disabled=h.disabled||!e),g.val(e?a+(h.exactMatch?"=":""):"").trigger(!l.$table[0].hasInitialized||b?"":"search"),h.valueToHeader?
// add current color to the header cell
c.closest("thead").find("th[data-column="+d+"]").find(".curcolor").html(f):
// current color to span in cell
c.find(".currentColor").html(f),
// update sticky header cell
k.length&&(k.find(".colorpicker").val(a)[0].disabled=h.disabled||!e,h.addToggle&&(k.find(".toggle")[0].checked=e),h.valueToHeader?
// add current color to the header cell
k.closest("thead").find("th[data-column="+d+"]").find(".curcolor").html(f):
// current color to span in cell
k.find(".currentColor").html(f))};
