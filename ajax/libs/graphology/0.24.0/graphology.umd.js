(function (global, factory) {
  ...
  EdgeData.prototype.detachMulti = function () {
    ...
    var inKey = 'in';
    if (this.undirected) outKey = inKey = 'undirected';
    var adj = this.source[outKey][target];
    ...
    _proto.dropNode = function dropNode(node) {
      ...
      if (this.type !== 'undirected') {
        for (var neighbor in nodeData.out) {
          if (this.multi) {
            nodeData.out[neighbor].forEach(function (edgeData) {
              dropEdgeFromData(_this2, edgeData);
            });
          } else {
        ...
        for (var _neighbor in nodeData["in"]) {
          if (this.multi) {
            nodeData["in"][_neighbor].forEach(function (edgeData) {
              dropEdgeFromData(_this2, edgeData);
            });
          } else {
            dropEdgeFromData(this, nodeData["in"][_neighbor]);
          }
        }
      }
      ...
      if (this.type !== 'directed') {
        for (var _neighbor2 in nodeData.undirected) {
          if (this.multi) {
            nodeData.undirected[_neighbor2].forEach(function (edgeData) {
              dropEdgeFromData(_this2, edgeData);
            });
          } else {
            dropEdgeFromData(this, nodeData.undirected[_neighbor2]);
          }
        }
