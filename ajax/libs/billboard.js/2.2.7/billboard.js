...
var Cache = /*#__PURE__*/function () {
   ...
   */, _proto.reset = function reset(all) {
    var $$ = this;
    for (var x in $$.cache) // reset the prefixed '$' key(which is internal use data) only.
    (all || /^\$/.test(x)) && ($$.cache[x] = null);
  }
  ...
  getTotalDataSum: function getTotalDataSum(subtractHidden) {
    ...
    if (!isNumber(total)) {
      ...
      total = sum.length ? sum.reduce(function (p, c) {
        return p + c;
      }) : 0, $$.cache.add(cacheKey, total = sum);
    }
...
/* harmony default export */ var internals_color = ({
  ...
  generateColor: function generateColor() {
    var $$ = this,
      ...
      ids = [];
    var pattern = notEmpty(config.color_pattern) ? config.color_pattern : (0,external_commonjs_d3_scale_commonjs2_d3_scale_amd_d3_scale_root_d3_.scaleOrdinal)($$.getColorFromCss() || schemeCategory10).range();
...
/* harmony default export */ var domain = ({
  getYDomainMinMax: function getYDomainMinMax(targets, type) {
    ...
    if (dataGroups.length > 0) {
      ...
      for (var _loop = function (_idsInGroup) {
          // Determine baseId
          ...
          idsInGroup = _idsInGroup;
        }, idsInGroup, j = 0; idsInGroup = dataGroups[j]; j++) {
        var _ret = _loop(idsInGroup);
      }
    }
  ...
  getYDomain: function getYDomain(targets, axisId, xDomain) {
    ...
    if (yTargets.length === 0) // use domain of the other axis if target of axisId is none
      return $$.isHiddenTargetWithYDomain(axisId) ? scale[axisId].domain() : axisId === "y2" ? scale.y.domain() :
    ...
    var isZeroBased = [TYPE.BAR, TYPE.BUBBLE, TYPE.SCATTER].concat(TYPE_BY_CATEGORY.Line).some(function (v) {
      ...
      updatePositions = function (textElement, id, index) {
        var margin,
          ...
          updateValues = function (id2, withoutStep) {
            withoutStep || (margin = (areaLength - totalLength - itemLength) / 2, margin < 10 && (margin = (areaLength - itemLength) / 2, totalLength = 0, step++)), steps[id2] = step, margins[step] = state.isLegendInset ? 10 : margin, offsets[id2] = totalLength, totalLength += itemLength;
          };
...
/* harmony default export */ var redraw = ({
  ...
  generateRedrawList: function generateRedrawList(targets, flow, duration, withSubchart) {
    ...
    if (afterRedraw)
      ...
      } else state.transiting || afterRedraw();
...
/* harmony default export */ var shape = ({
  ...
  getShapeIndices: function getShapeIndices(typeFilter) {
    ...
    return hasXs && getUnique(Object.keys(xs).map(function (v) {
      ...
      for (var groups, xKey = (d.id in xs) ? xs[d.id] : "", ind = xKey ? indices[xKey] : indices, j = 0; groups = config.data_groups[j]; j++) if (!(groups.indexOf(d.id) < 0)) for (var row, k = 0; row = groups[k]; k++) if (row in ind) {
        ind[d.id] = ind[row];
        break;
      }
...
/* harmony default export */ var size = ({
  ...
  getCurrentPaddingRight: function getCurrentPaddingRight(withXAxisTickTextOverflow) {
    ...
    return padding = isValue(config.padding_right) ? config.padding_right + 1 : $$.axis && config.axis_rotated ? 10 + legendWidthOnRight : $$.axis && (!config.axis_y2_show || config.axis_y2_inner) ? Math.max(2 + legendWidthOnRight + ($$.axis.getAxisLabelPosition("y2").isOuter ? 20 : 0), xAxisTickTextOverflow) : Math.max(ceil10(axisWidth) + legendWidthOnRight, xAxisTickTextOverflow), padding + axisWidth * axesLen;
  },
  /**
  ...
  getParentRectValue: function getParentRectValue(key) {
    ...
    for (var v, parent = this.$el.chart.node(); !v && parent && parent.tagName !== "BODY";) {
      try {
...
/* harmony default export */ var internals_text = ({
  ...
  redrawText: function redrawText(x, y, forFlow, withTransition) {
    ...
    return $$.$el.text.each(function (d, index) {
      var text = (0,external_commonjs_d3_selection_commonjs2_d3_selection_amd_d3_selection_root_d3_.select)(this);
...
/* harmony default export */ var internals_tooltip = ({
  ...
  getTooltipContent: function getTooltipContent(d, defaultTitleFormat, defaultValueFormat, _color) {
    var $$ = this,
      ...
      config = $$.config,
      state = $$.state;
    var _map = ["title", "name", "value"].map(function (v) {
    ...
    var order = config.tooltip_order,
      getRowValue = function getRowValue(row) {
    ...
    var _loop = function _loop() {
      if (row = d[i], !row || !(getRowValue(row) || getRowValue(row) === 0)) return "continue";
      if (isUndefined(text)) {
  ...
  showTooltip: function showTooltip(selectedData, element) {
    ...
    if (tooltip && dataToShow.length !== 0 && config.tooltip_show) {
      ...
      if (!datum || datum.current !== dataStr) {
        ...
        callFn(config.tooltip_onshow, $$.api, selectedData), tooltip.html($$.getTooltipHTML(selectedData,
        // data
...
/* harmony default export */ var type = ({
  ...
  hasTypeOf: function hasTypeOf(type, targets, exclude) {
    var _this = this;
    return exclude === void 0 && (exclude = []), !TYPE_BY_CATEGORY[type]
    // @ts-ignore
    .filter(function (v) {
...
/* harmony default export */ var chart = ({
  ...
  destroy: function destroy() {
    ...
    if (notEmpty($$))
      ...
      for (var key in $$.callPluginHook("$willDestroy"), $$.charts.splice($$.charts.indexOf(this), 1), svg.select("*").interrupt(), $$.resizeFunction.clear(), win.removeEventListener("resize", $$.resizeFunction), chart.classed("bb", !1).html(""), Object.keys(this).forEach(function (key) {
        key === "internal" && Object.keys($$).forEach(function (k) {
          $$[k] = null;
        }), _this[key] = null, delete _this[key];
      }), this) this[key] = function () {};
    return null;
  },
  /**
...
var AxisRenderer = /*#__PURE__*/function () {
  ...
  , _proto.splitTickText = function splitTickText(d, scale, ticks, isLeftRight, charWidth) {
    ...
    function split(splitted, text) {
      for (var subtext, spaceIndex, textWidth, i = 1; i < text.length; i++)
...
var Axis_Axis = /*#__PURE__*/function () {
  ...
  , _proto.getAxis = function getAxis(id, scale, outerTick, noTransition, noTickTextRotate) {
    ...
    if (svg) {
      ...
      if (isDomainSame || isCurrentMaxTickDomainSame) return currentTickMax.size;
   ...
   */, _proto.getXAxisPadding = function getXAxisPadding(tickCount) {
    ...
    if (isEmpty(padding) ? padding = {
      ...
      if (tickCount && range) {
        ...
        left = padding.left / range / relativeTickWidth, right = padding.right / range / relativeTickWidth;
      }
      padding = {
        left: left,
        right: right
    ...
    !hasZoom && this.isCategorized() && targetsToShow.length === 0 && scale.x.domain([0, $el.axis.x.selectAll(".tick").size()]), scale.x && targetsToShow.length ? (!hasZoom && $$.updateXDomain(targetsToShow, wth.UpdateXDomain, wth.UpdateOrgXDomain, wth.TrimXDomain), !config.axis_x_tick_values && this.updateXAxisTickValues(targetsToShow)) : this.x && (this.x.tickValues([]), this.subX && this.subX.tickValues([])), config.zoom_rescale && !flow && (xDomainForZoom = scale.x.orgDomain()), ["y", "y2"].forEach(function (key) {
      ...
      if (axisScale) {
        ...
        if (axisScale.domain($$.getYDomain(targetsToShow, key, xDomainForZoom)), !tickValues && tickCount) {
          ...
          axis.tickValues(_this5.generateTickValues(domain, domain.every(function (v) {
            return v === 0;
...
/* harmony default export */ var eventrect = ({
  ...
  bindTouchOnEventRect: function bindTouchOnEventRect(isMultipleX) {
    var $$ = this,
      ...
      svg = _$$$$el.svg,
      selectRect = function selectRect(context) {
  ...
  updateEventRect: function updateEventRect(eventRect, force) {
    ...
    (!rendered || resizing || force) && (rectElement.attr("x", 0).attr("y", 0).attr("width", width).attr("height", height), !rendered && rectElement.attr("class", config_classes.eventRect)), updateClientRect();
  },
  /**
  ...
  updateEventRectData: function updateEventRectData() {
    ...
    x = 0, y = 0, w = state.width, h = state.height;else {
      var rectW, rectX;
      if ($$.axis.isCategorized()) rectW = $$.getEventRectWidth(), rectX = function rectX(d) {
        return xScale(d.x) - rectW / 2;
      };else {
        var getPrevNextX = function getPrevNextX(_ref) {
  ...
  generateEventRectsForSingleX: function generateEventRectsForSingleX(eventRectEnter) {
    var $$ = this,
      ...
      state = $$.state,
      _state2 = state,
      eventReceiver = _state2.eventReceiver,
...
/* harmony default export */ var grid = ({
  ...
  updateXGridLines: function updateXGridLines(duration) {
    ...
    var xLines = main.select("." + config_classes.xgridLines).selectAll("." + config_classes.xgridLine).data(config.grid_x_lines);
  ...
  updateYGridLines: function updateYGridLines(duration) {
    ...
    var ygridLines = $el.main.select("." + config_classes.ygridLines).selectAll("." + config_classes.ygridLine).data(config.grid_y_lines);
  ...
  showGridFocus: function showGridFocus(data) {
    ...
    if (!(!config.tooltip_show || dataToShow.length === 0 || $$.hasType("bubble") || $$.hasArcType())) {
      ...
      focusEl.style("visibility", "visible").data(dataToShow.concat(dataToShow)).each(function (d) {
        ...
        if (el.classed(config_classes.xgridFocus)) xy = isRotated ? [null,
        // x1
        pos.x,
        // y1
        isEdge ? pos.y : width,
...
/* harmony default export */ var arc = ({
  ...
  redrawArc: function redrawArc(duration, durationForExit, withTransform) {
    ...
    var mainArc = main.selectAll("." + config_classes.arcs).selectAll("." + config_classes.arc).data($$.arcData.bind($$));
    // bind arc events
  ...
  redrawBackgroundArcs: function redrawBackgroundArcs() {
    ...
    if (hasMultiArcGauge) {
      ...
      backgroundArc = backgroundArc.selectAll("path." + config_classes.chartArcsBackground).data($$.data.targets), backgroundArc.enter().append("path").attr("class", function (d, i) {
        ...
        var id = _ref2.id;
        if (showEmptyTextLabel || state.hiddenTargetIds.indexOf(id) >= 0) return "M 0 0";
        var d = {
          data: [{
    ...
    } else backgroundArc.attr("d", showEmptyTextLabel ? "M 0 0" : function () {
      var d = {
        data: [{
          value: config.gauge_max
        }],
...
/* harmony default export */ var shape_area = ({
  ...
  generateDrawArea: function generateDrawArea(areaIndices, isSub) {
    ...
    return function (d) {
      ...
      if ($$.isAreaType(d)) {
        ...
        area = isRotated ? area.y(xValue).x0(value0).x1(value1) : area.x(xValue)
        // @ts-ignore
  ...
  generateGetAreaPoints: function generateGetAreaPoints(areaIndices, isSub) {
    ...
    return function (d, i) {
      ...
      return config.axis_rotated && (d.value > 0 && posY < y0 || d.value < 0 && y0 < posY) && (posY = y0), [[posX, offset], [posX, posY - (y0 - offset)], [posX, posY - (y0 - offset)],
...
/* harmony default export */ var bar = ({
  initBar: function initBar() {
    var $el = this.$el;
    $el.bar = $el.main.select("." + config_classes.chart)
...
/* harmony default export */ var line = ({
  ...
  generateDrawLine: function generateDrawLine(lineIndices, isSub) {
    ...
    var line = (0,external_commonjs_d3_shape_commonjs2_d3_shape_amd_d3_shape_root_d3_.line)();
  ...
  lineWithRegions: function lineWithRegions(d, x, y, _regions) {
    var $$ = this,
      ...
      regions = [];
    // default value
    var xp, yp, diff, diffx2;
    // check weather data is within region
    var isWithinRegions = function isWithinRegions(withinX, withinRegions) {
    ...
    if (isDefined(_regions)) {
      var getValue = function getValue(v, def) {
    ...
    var xValue = isRotated ? function (dt) {
      ...
      generateM = function generateM(points) {
  ...
  initZoomBehaviour: function initZoomBehaviour() {
    ...
    $$.zoomBehaviour = (0,external_commonjs_d3_drag_commonjs2_d3_drag_amd_d3_drag_root_d3_.drag)().clickDistance(4).on("start", function () {
      ...
      if ($$.setDragStatus(!1), zoomRect.attr(prop.axis, 0).attr(prop.attr, 0), start > end && (_ref = [end, start], start = _ref[0], end = _ref[1], _ref), start < 0 && (end += Math.abs(start), start = 0), start !== end) $$.api.zoom([start, end].map(function (v) {
        ...
        var event = external_commonjs_d3_selection_commonjs2_d3_selection_amd_d3_selection_root_d3_.event.sourceEvent || external_commonjs_d3_selection_commonjs2_d3_selection_amd_d3_selection_root_d3_.event,
          _ref2 = "clientX" in event ? [event.clientX, event.clientY] : [event.x, event.y],
          x = _ref2[0],
          y = _ref2[1],
