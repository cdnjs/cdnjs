...
RedBlackTree.prototype = {
  ...
  remove: function remove(node) {
    ...
    if (next = left ? right ? RedBlackFirst(right) : left : right, parent ? parent.L === node ? parent.L = next : parent.R = next : this._ = next, left && right ? (red = next.C, next.C = node.C, next.L = left, left.U = next, next === right ? (next.U = parent, parent = next, node = next.R) : (parent = next.U, next.U = node.U, node = next.R, parent.L = node, next.R = right, right.U = next)) : (red = node.C, node = next), node && (node.U = parent), !red) {
    ...
    if (r = x1 - ax, dx || !(r < 0)) {
      ...
      if (r = y0 - ay, dy || !(r > 0)) {
        ...
        if (r = y1 - ay, dy || !(r < 0)) {
          ...
          return !(t0 > 0 || t1 < 1) || (t0 > 0 && (edge[0] = [ax + t0 * dx, ay + t0 * dy]), t1 < 1 && (edge[1] = [ax + t1 * dx, ay + t1 * dy]), !0); // TODO Better check?
        }
      }
    }
  }
...
function clipEdges(x0, y0, x1, y1) {
  for (var edge, i = edges.length; i--;) connectEdge(edge = edges[i], x0, y0, x1, y1) && clipEdge(edge, x0, y0, x1, y1) && (Math.abs(edge[0][0] - edge[1][0]) > epsilon || Math.abs(edge[0][1] - edge[1][1]) > epsilon) || delete edges[i];
}
...
function clipCells(x0, y0, x1, y1) {
  ...
  for (iCell = 0; iCell < nCells; ++iCell) if (cell = cells[iCell]) {
    ...
    for (site = cell.site, halfedges = cell.halfedges, iHalfedge = halfedges.length; iHalfedge--;) edges[halfedges[iHalfedge]] || halfedges.splice(iHalfedge, 1); // Insert any border edges as necessary.


    for (iHalfedge = 0, nHalfedges = halfedges.length; iHalfedge < nHalfedges;) end = cellHalfedgeEnd(cell, edges[halfedges[iHalfedge]]), endX = end[0], endY = end[1], start = cellHalfedgeStart(cell, edges[halfedges[++iHalfedge % nHalfedges]]), startX = start[0], startY = start[1], (Math.abs(endX - startX) > epsilon || Math.abs(endY - startY) > epsilon) && (halfedges.splice(iHalfedge, 0, edges.push(createBorderEdge(site, end, Math.abs(endX - x0) < epsilon && y1 - endY > epsilon ? [x0, Math.abs(startX - x0) < epsilon ? startY : y1] : Math.abs(endY - y1) < epsilon && x1 - endX > epsilon ? [Math.abs(startY - y1) < epsilon ? startX : x1, y1] : Math.abs(endX - x1) < epsilon && endY - y0 > epsilon ? [x1, Math.abs(startX - x1) < epsilon ? startY : y0] : Math.abs(endY - y0) < epsilon && endX - x0 > epsilon ? [Math.abs(startY - y0) < epsilon ? startX : x0, y0] : null)) - 1), ++nHalfedges);
  ...
  if (cover) {
    ...
    for (iCell = 0, cover = null; iCell < nCells; ++iCell) (cell = cells[iCell]) && (site = cell.site, dx = site[0] - x0, dy = site[1] - y0, d2 = dx * dx + dy * dy, d2 < dc && (dc = d2, cover = cell));
...
function removeBeach(beach) {
  ...
  for (iArc = 1; iArc < nArcs; ++iArc) rArc = disappearing[iArc], lArc = disappearing[iArc - 1], setEdgeEnd(rArc.edge, lArc.site, rArc.site, vertex);
...
function addBeach(site) {
  for (var lArc, rArc, dxl, dxr, x = site[0], directrix = site[1], node = beaches._; node;) if (dxl = leftBreakPoint(node, directrix) - x, dxl > epsilon) node = node.L;else if (dxr = x - rightBreakPoint(node, directrix), dxr > epsilon) {
    if (!node.R) {
      lArc = node;
      break;
    }
  ...
  if (beaches.insert(lArc, newArc), lArc || rArc) {
    ...
    if (!rArc) return void (newArc.edge = createEdge(lArc.site, newArc.site)); // else lArc !== rArc
...
function Diagram(sites, extent) {
  ...
  for (edges = [], cells = Array(sites.length), beaches = new src_RedBlackTree(), circles = new src_RedBlackTree();;) if (circle = firstCircle, site && (!circle || site[1] < circle.y || site[1] === circle.y && site[0] < circle.x)) (site[0] !== x || site[1] !== y) && (addBeach(site), x = site[0], y = site[1]), site = sites.pop();else if (circle) removeBeach(circle.arc);else break;
...
Diagram.prototype = {
  ...
  triangles: function () {
    ...
    return this.cells.forEach(function (cell, i) {
      if (m = (halfedges = cell.halfedges).length) for (var halfedges, m, s0, site = cell.site, j = -1, e1 = edges[halfedges[m - 1]], s1 = e1.left === site ? e1.right : e1.left; ++j < m;) s0 = s1, e1 = edges[halfedges[j]], s1 = e1.left === site ? e1.right : e1.left, s0 && s1 && i < s0.index && i < s1.index && triangleArea(site, s0, s1) < 0 && triangles.push([site.data, s0.data, s1.data]);
    }), triangles;
  },
  ...
  find: function find(x, y, radius) {
    ...
    return that._found = i0, radius == null || d2 <= radius * radius ? cell.site : null;
  }
};
...
/* harmony default export */ function value(a, b) {
  var c,
      t = typeof b;
  return b == null || t === "boolean" ? d3_interpolate_src_constant(b) : (t === "number" ? number : t === "string" ? (c = color(b)) ? (b = c, rgb) : string : b instanceof color ? rgb : b instanceof Date ? date : isNumberArray(b) ? numberArray : Array.isArray(b) ? genericArray : typeof b.valueOf !== "function" && typeof b.toString !== "function" || isNaN(b) ? object : number)(a, b);
}
...
var TextOverlap = /*#__PURE__*/function (_Plugin) {
  ...
  return _proto.$init = function $init() {
    ...
    var text = src_selectAll(this.config.selector);
  ...
  , _proto.generateVoronoi = function generateVoronoi(data) {
    ...
    return min[1] = _ref[0], max[0] = _ref[1], voronoi().extent([min, max]).polygons(data);
  }
  /**
  ...
  , _proto.preventLabelOverlap = function preventLabelOverlap(text) {
    ...
    text.each(function () {
      ...
      if (cell && this) {
        ...
        src_select(this) // @ts-ignore
        .attr("display", src_area(cell) < area ? "none" : null).attr("text-anchor", txtAnchor).attr("dy", "0." + (angle === 1 ? 71 : 35) + "em").attr("transform", "translate(" + xTranslate + ", " + yTranslate + ")");
      }
    });
  }, TextOverlap;
}(Plugin);
