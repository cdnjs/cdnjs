(function(root) {
	...
	Pizzicato.Effects.Quadrafuzz = function(options) {
		...
		for (var i = 0; i < 4; i++) {
			...
			this.overdrives[i].oversample = '4x';
	
			this.gains[i] = Pz.context.createGain();
		}
		...
		for (i = 0; i < filters.length; i++) {
			...
			this.overdrives[i].connect(this.gains[i]);
			this.gains[i].connect(this.outputNode);
		}
	...
	Pizzicato.Effects.Quadrafuzz.prototype = Object.create(baseEffect, {
		...
		gain: {
			...
			set: function(gain) {
				...
				this.options.gain = gain;
				var normalizedGain = Pz.Util.normalize(this.gain, 0, 150);
	
				for (var i = 0; i < this.overdrives.length; i++)
					this.overdrives[i].curve = getDistortionCurve(normalizedGain);
			}
		},
		...
		low: {
			...
			set: function(low) {
				if (!Pz.Util.isInRange(low, 0, 1))
					return;
	
				this.options.low = low;
				this.lowpassLeft.frequency.value = Pz.Util.normalize(this.low, 70, 2300);
			}
		},
		...
		mid1: {
			...
			get: function() {
				return this.options.mid1;
			},
	
			set: function(mid1) {
				if (!Pz.Util.isInRange(mid1, 0, 1))
					return;
	
				this.options.mid1 = mid1;
				this.bandpass1Left.frequency.value = Pz.Util.normalize(this.mid1, 300, 9000);
			}
		},
		...
		mid2: {
			...
			get: function() {
				return this.options.mid2;
			},
	
			set: function(mid2) {
				if (!Pz.Util.isInRange(mid2, 0, 1))
					return;
	
				this.options.mid2 = mid2;
				this.bandpass2Left.frequency.value = Pz.Util.normalize(this.mid2, 1200, 3500);
			}
		},
		...
		high: {
			...
			set: function(high) {
				if (!Pz.Util.isInRange(high, 0, 1))
					return;
	
				this.options.high = high;
				this.highpassLeft.frequency.value = Pz.Util.normalize(this.high, 3500, 7000);
			}
		}
	});
