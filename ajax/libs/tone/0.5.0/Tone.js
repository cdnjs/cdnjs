(function (root) {
	...
	Module(function (Tone) {
	    ...
	    Tone.Transport = function () {
	        ...
	        this._tickEvents = [];
	        /**
			 * 	Objects that have "start", "pause" and "stop"
			 * 	methods to be called whenever the Transport's
			 * 	state changes. 
			 *  @type {Array}
			 *  @private
			 */
	        ...
	        this._ticks = 0;
	        /**
			 *  Listen for events in each of these channels
			 *  @type  {Object}
			 *  @private
			 */
	        this._events = {
	    ...
	    Tone.Transport.prototype._processTick = function (tickTime) {
	        if (this.state === Tone.State.Started) {
	            ...
	            while (this._tickEvents.length > 0 && this._tickEvents[0].tickTime === this._ticks) {
	                var evnt = this._tickEvents.shift();
	                evnt.invoke(tickTime, this._ticks);
	            }
	            transportTicks += 1;
	            timelineTicks += 1;
	            this._ticks++;
	    ...
	    Tone.Transport.prototype.schedule = function (callback, time) {
	        ...
	        var tickEvent = new TickEvent(callback, ticks);
	        //put it in the right spot
	        for (var i = 0, len = this._tickEvents.length; i < len; i++) {
	            var testEvnt = this._tickEvents[i];
	            if (testEvnt.tickTime > tickEvent.tickTime) {
	                this._tickEvents.splice(i, 0, tickEvent);
	                return tickEvent.id;
	            }
	        }
	        //otherwise push it on the end
	        this._tickEvents.push(tickEvent);
	        return tickEvent.id;
	    };
	    /**
		 *  
	    ...
	    /**
		 *  Remove all scheduled events from the timeline. 
		 ...
		 */
	    var TickEvent = function (callback, tickTime) {
		 ...
		 */
	    TickEvent.prototype.invoke = function (playbackTime, currentTicks) {
	        this.callback(playbackTime, currentTicks);
	    };
	    ///////////////////////////////////////////////////////////////////////////////
	    //	INTERVAL
	    ///////////////////////////////////////////////////////////////////////////////
	    ...
	    Tone.Transport.prototype.start = function (time, offset) {
	        if (this.state === Tone.State.Stopped || this.state === Tone.State.Paused) {
	            ...
	            this._trigger('start', startTime, offset);
	    ...
	    Object.defineProperty(Tone.Transport.prototype, 'ticks', {
	        ...
	        set: function (t) {
	            ...
	            this._ticks = t;    //clear everything after that tick
		 ...
		 */
	    Tone.Transport.prototype.on = function (state, callback) {
	        if (this.isUndef(this._events[state])) {
	            throw new Error('invalid state: ' + state);
	        }
	        this._events[state].push(callback);
	        return this;
	    };
	    /**
		 ...
		 *  @private
		 */
	    Tone.Transport.prototype._trigger = function (state) {
	        var args = Array.prototype.slice.call(arguments, 1);
	        var callbacks = this._events[state];
	...
	Module(function (Tone) {
	    ...
	    Tone.Source = function (options) {
	        //unlike most Tone Nodes, Sources only have an output and no input
	        Tone.call(this, 0, 1);
	...
	Module(function (Tone) {
	    /**
		 *  @class A base-class for objects which have a schedulable
		 *         start and stop states. Implements 
		 *         <code>start</code> and <code>stop</code> methods
		 *         and calls startCallback and stopCallback
		 *         when the event should occur. 
		 *  @extends {Tone}
		 *  @param {Function} startCallback The method which will be invoked on start.
		 *  @param {Function} stopCallback The method which will be invoked on stop.
		 */
	    Tone.State = function (startCallback, stopCallback) {
	        /**
			 *  The method which should be invoked on start
			 *  @type {Function}
			 *  @private
			 */
	        this._start = this.defaultArg(startCallback, Tone.noOp);
	        /**
			 *  The method which should be invoked on stop
			 *  @type {Function}
			 *  @private
			 */
	        this._stop = this.defaultArg(stopCallback, Tone.noOp);
	        /**
		 ...
		 *  @name state
		 */
	    Object.defineProperty(Tone.State.prototype, 'state', {
	        get: function () {
	            return this._stateAtTime(this.now());
	        }
	    });
		 ...
		 *  @private
		 */
	    Tone.State.prototype._stateAtTime = function (time) {
	        time = this.toSeconds(time);
	        if (this._nextStart <= time && this._nextStop > time) {
	            return Tone.State.Started;
	        } else if (this._nextStop <= time) {
	    ...
	    /**
		 *  Start the state at the specified time. If no time is given, 
		 *  start the state now.
		 *  @param  {Time} [time=now] When the state should be started.
		 *  @returns {Tone.State} this
		 *  @example
		 * state.start("+0.5"); //starts the state 0.5 seconds from now
		 */
	    Tone.State.prototype.start = function (time) {
	        time = this.toSeconds(time);
	        if (this._stateAtTime(time) !== Tone.State.Started || this.retrigger) {
	            this._nextStart = time;
	            this._nextStop = Infinity;
	    ...
	    /**
		 *  Stop the state at the specified time. If no time is given, 
		 *  stop the state now.
		 *  @param  {Time} [time=now] When the state should be stopped. 
		 *  @returns {Tone.State} this
		 *  @example
		 * state.stop(); // stops the state immediately
		 */
	    Tone.State.prototype.stop = function (time) {
	        var now = this.now();
	        time = this.toSeconds(time, now);
	        if (this._stateAtTime(time) === Tone.State.Started) {
	            this._nextStop = this.toSeconds(time);
	    ...
	    /**
		 * Possible States that the object can be in. 
		 * @enum {string}
		 */
	    Tone.State = {
	        Started: 'started',
	...
	Module(function (Tone) {
		 ...
		 * chord.start();
		 * //loop it every measure
		 * chord.loop = 8;
		 * chord.loopEnd = "1m";
		 */
	    Tone.Note = function (callback, value) {
	        this._loop = false;
	        /**
	        ...
	        /**
			 *  This value will be passed in as the second argument
			 *  to the callback function. The first argument is the 
	        ...
	        this._startTime = -1;
	        /**
			 *  When the note is scheduled to stop.
			 *  @type  {Number}
			 *  @private
			 */
	        this._stopTime = -1;
	        //listen for changes in the transport to schedule the callback
	        Tone.Transport.on('start', this._start.bind(this));    // Tone.Transport.on("start scrub", this._start.bind(this));
	    };
	    ...
	    Tone.Note.prototype.start = function (time) {
	        if (this._startTime === -1) {
	            if (this.isUndef(time)) {
	                this._startTime = Tone.Transport.ticks;
	            } else {
	                this._startTime = this.toTicks(time);
	            }
	            this._stopTime = -1;
	            if (Tone.Transport.state === Tone.State.Started) {
	                this._start(time, Tone.Transport.ticks);
	            }
	        }
	        return this;
	    };
	    Tone.Note.prototype.stop = function (time) {
	        if (this.isUndef(time)) {
	            this._stopTime = Tone.Transport.ticks;
	        } else {
	            this._stopTime = this.toTicks(time);
	        }
	        return this;
	    };
	    /**
		 *  Get the next time it will tick. If there are no ticks
		 *  scheduled after the passed in number, return -1.
		 *  @param  {Number}  ticks
		 *  @returns {Number}
		 *  @private
		 */
	    Tone.Note.prototype._computeNextTick = function (ticks) {
	        var nextTick = -1;
	        if (this._startTime !== -1) {
	            //get the number of loops it's been
	            if (ticks >= this._startTime) {
	                if (this._loop) {
	                    var mod = (ticks - this._startTime) % this._loopEnd;
	                    if (mod === 0) {
	                        mod = this._loopEnd;
	                    }
	                    nextTick = ticks + mod;
	                }
	            } else if (ticks < this._startTime) {
	                nextTick = this._startTime;
	            }
	        }
	        return nextTick;
	    };
		 ...
		 *  @private
		 */
	    Tone.Note.prototype._start = function (time, ticks) {
	        if (this._startTime !== -1) {
	            var nextTick = this._computeNextTick(ticks);
	            if (ticks === this._startTime) {
	                Tone.Transport.schedule(this._tick.bind(this), ticks + 'i');
	            } else if (nextTick !== -1) {
	                Tone.Transport.schedule(this._tick.bind(this), nextTick + 'i');
	            }
	        }
	    };
	    Tone.Note.prototype._tick = function (time, ticks) {
	        if (this._startTime !== -1) {
	            if (this._stopTime !== -1 && ticks >= this._stopTime) {
	                this._startTime = -1;
	            } else {
	                this._invoke(time);
	                var nextTick = this._computeNextTick(ticks);
	                //see if there is one to schedule in the future
	                if (nextTick !== -1) {
	                    Tone.Transport.schedule(this._tick.bind(this), nextTick + 'i');
	                }
	            }
	    ...
	    Tone.Note.prototype._invoke = function (time) {
	        // this._args[0] = time;
	        this._callback(time, this._values);
	    };
	    return Tone.Note;
	});
	Module(function (Tone) {
	    //recreate some of the deprecated functions using Tone.Note
	    //these methods will be removed in r7. 
	    var EventID = 0;
	    var allEvents = {};
	    /**
	    ...
	    Tone.Transport.prototype.setInterval = function (callback, interval) {
	        console.warn('This method is deprecated. Use Tone.Note instead.');
	        var note = new Tone.Note({
	            'callback': callback,
	            'loop': true,
	            'loopEnd': interval
	        }).start();
	    ...
	    Tone.Transport.prototype.setTimeout = function (callback, timeout) {
	        console.warn('This method is deprecated. Use Tone.Note instead.');
	        var note = new Tone.Note({ 'callback': callback }).start('+' + timeout);
	        var id = EventID++;
	        allEvents[id] = note;
	        return id;
	    };
		 ...
		 *  }, "16m");
		 */
	    Tone.Transport.prototype.setTimeline = function (callback, position) {
	        console.warn('This method is deprecated. Use Tone.Note instead.');
	        var note = new Tone.Note({ 'callback': callback }).start(position);
	        var id = EventID++;
	        allEvents[id] = note;
	        return id;
	    };
	...
	Module(function (Tone) {
	    /**
		 *  @class Tone.PitchShift does real-time pitch shifting to the incoming signal. 
	    ...
	    Tone.PitchShift = function () {
	        Tone.call(this, 1, 1);
	        this.frequency = new Tone.Signal(2);
	        this.frequency._fromUnits = this._frequencyToInterval;
	        this.frequency._toUnits = this._intervalToFrequency;
	        /**
			 *  Uses two DelayNodes to cover up the jump in
			 *  the sawtooth wave. 
			 *  @type  {[type]}
			 */
	        this._delayA = this.context.createDelay();
	        /**
			 *  The first LFO.
			 *  @type  {Tone}
			 */
	        this._lfoA = new Tone.LFO(0, 0.1, 0);
	        this._lfoA.type = 'sawtooth';
	        this._lfoA.connect(this._delayA.delayTime);
	        this._delayB = this.context.createDelay();
	        this._lfoB = new Tone.LFO(0, 0.1, 0);
	        this._lfoB.phase = 180;
	        this._lfoB.type = 'sawtooth';
	        this._lfoB.connect(this._delayB.delayTime);
	        this.input.connect(this._delayA);
	        this.input.connect(this._delayB);
	        this._crossFade = new Tone.CrossFade().connect(this.output);
	        this._delayA.connect(this._crossFade.a);
	        this._delayB.connect(this._crossFade.b);
	        this._crossFadeLFO = new Tone.LFO(0, 0, 1);
	        this._crossFadeLFO.type = 'triangle';
	        this._crossFadeLFO.phase = 90;
	        this._crossFadeLFO.connect(this._crossFade.fade);
	        this.frequency.fan(this._lfoA.frequency, this._lfoB.frequency, this._crossFadeLFO.frequency);
	        //start the effect at the same time
	        var now = this.now() + 0.01;
	        this._lfoA.start(now);
	        this._lfoB.start(now);
	        this._crossFadeLFO.start(now);
	    };
	    Tone.extend(Tone.PitchShift);
	    Tone.PitchShift.prototype.setDepth = function (depth) {
	        this._lfoA.min = depth;
	        this._lfoB.min = depth;
	    };
	...
	Module(function (Tone) {
	    /**
		 *  @class  Base-class for musical structures. 
		 *  
		 */
	    Tone.Structure = function (callback) {
	        this._loop = false;
	        this._callback = callback;
	        this._startTime = -1;
	        this._stopTime = -1;
	        this.playbackSpeed = 1;
	        // Tone.Transport.syncStructure(this);
	        Tone.Transport.on('start', this._start.bind(this));
	    };
	    Tone.extend(Tone.Structure);
	    Tone.Structure.defaults = {
	        'callback': Tone.noOp,
	        'loop': false,
	        'loopEnd': '1m',
	        'loopStart': 0
	    };
	    Tone.Structure.prototype.start = function (time) {
	        if (this._startTime === -1) {
	            if (this.isUndef(time)) {
	                this._startTime = Tone.Transport.ticks;
	            } else {
	    ...
	    Tone.Structure.prototype.stop = function (time) {
	        if (this.isUndef(time)) {
	            this._stopTime = Tone.Transport.ticks;
	        } else {
	            this._stopTime = this.toTicks(time);
	    ...
	    /**
		 *  Get the next tick it will tick. If there are no ticks
		 *  scheduled after the passed in number, return -1.
		 *  @param  {Number}  ticks
		 *  @returns {Number}
		 *  @private
		 */
	    Tone.Structure.prototype._getNextTick = function (ticks) {
	        var nextTick = -1;
	        if (this._startTime !== -1) {
	            //get the number of loops it's been
	            if (ticks >= this._startTime) {
		 ...
		 *  @private
		 */
	    Tone.Structure.prototype._start = function (time, ticks) {
	        if (this._startTime !== -1) {
	            var nextTick = this._getNextTick(ticks);
	            if (ticks === this._startTime) {
	                Tone.Transport.schedule(this._tick.bind(this), ticks + 'i');
	            } else if (nextTick !== -1) {
	                Tone.Transport.schedule(this._tick.bind(this), nextTick + 'i');
	    ...
	    Tone.Structure.prototype._tick = function (time, ticks) {
	        if (this._startTime !== -1) {
	            if (this._stopTime !== -1 && ticks >= this._stopTime) {
	                this._startTime = -1;
	            } else {
	    ...
	    Tone.Structure.prototype._invoke = function (time) {
	        // this._args[0] = time;
	        this._callback(time, this._values);
	    };
	...
	Module(function (Tone) {
		 ...
		 * ]).start();
		 */
	    Tone.Part = function (callback, notes) {
	        /**
			 *  An array of Tone.Notes
			 *  @type  {Array}
			 *  @private
			 */
	        this._notes = [];
	        for (var i = 0; i < notes.length; i++) {
	            var n = new Tone.Note(callback, notes[i]);
	            this.notes.push(n);
	        }    //order the notes by startTime
	    };
	    Tone.extend(Tone.Part, Tone.Structure);
	    Tone.Note.prototype._start = function (time, ticks) {
	        for (var i = 0; i < this._notes.length; i++) {
	            this._notes._start(time, this._notes._startTime + ticks);
	        }
	    };
	    Tone.Note.prototype.stop = function (time) {
	        for (var i = 0; i < this._notes.length; i++) {
	            this._notes.stop(time);
	        }
	    };
	    /**
		 *  Add a note to the part. 
		 *  @param  {Tone.Note}  note  A Tone.Note to add to the part. 
		 */
	    Tone.Part.prototype.add = function (note) {
	    };
	    /**
		 *  Remove a note from the part. 
		 *  @param  {Tone.Note}  note  A Tone.Note to add to the part. 
		 */
	    Tone.Part.prototype.remove = function (note) {
	    };
	    return Tone.Part;
	});
	define('Tone/structure/Score', [], function () {
	    return;
	});
	define('Tone/structure/Sequence', [], function () {
	    return;
	});
