...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var DEXIE_VERSION = '4.0.1-alpha.15';
    ...
    function applyOptimisticOps(result, req, ops, table, cacheEntry, immutable) {
        ...
        finalResult.sort(function (a, b) {
            return cmp(extractIndex(a), extractIndex(b)) ||
    ...
    var cache = {};
    function invalidateCachedQueries(updateParts) {
        var queriesToSignal = new Set();
        for (var key in updateParts) {
            var parts = /^idb\:\/\/(.*)\/(.*)\//.exec(key);
            if (parts) {
                var dbName = parts[1], tableName = parts[2];
                var tblCache = cache["idb://".concat(dbName, "/").concat(tableName)];
                if (tblCache)
                    invalidateTableQueries(tblCache, updateParts, queriesToSignal);
            }
        }
        queriesToSignal.forEach(function (requery) { return requery(); });
    }
    function invalidateTableQueries(tblCache, mutatedParts, queriesToSignal) {
        ...
        for (var _i = 0, _a = Object.entries(tblCache.queries.query); _i < _a.length; _i++) {
            ...
            for (var _c = 0, entries_1 = entries; _c < entries_1.length; _c++) {
                ...
                if (entry.obsSet && obsSetsOverlap(mutatedParts, entry.obsSet)) {
                    entry.subscribers.forEach(function (requery) { return queriesToSignal.add(requery); });
                }
                else {
        ...
        for (var _d = 0, updatedEntryLists_1 = updatedEntryLists; _d < updatedEntryLists_1.length; _d++) {
            var _e = updatedEntryLists_1[_d], indexName = _e[0], newEntries = _e[1];
