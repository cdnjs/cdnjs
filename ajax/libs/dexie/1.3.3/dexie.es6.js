...
var dexieErrorNames = ['Modify', 'OpenFailed', 'VersionChange', 'Schema', 'Upgrade', 'InvalidTable', 'MissingAPI', 'NoSuchDatabase', 'InvalidArgument', 'SubTransaction', 'Unsupported', 'Internal', 'DatabaseClosed'];
...
var errnames = errorList.reduce(function (obj, name) {
    return obj[name + "Error"] = name + "Error", obj;
}, {});
                ...
                if (!self._locked()) {
                    p = self.active ? new Promise(function (resolve, reject) {
                        if (!self.idbtrans && mode) {
                            if (!idbdb) throw dbOpenError ? dbOpenError instanceof exceptions.DatabaseClosed ? dbOpenError : new exceptions.OpenFailed(dbOpenError) : new exceptions.DatabaseClosed();
                        ...
                        try {
                            ...
                            if (!(e instanceof DexieError) && e.name && exceptionMap[e.name]) {
                                ...
                                try {
                                    throw new exceptionMap[e.name](e.message, e);
                                } catch (e2) {
                                    e = e2;
                                }
                            }
    ...
    function Collection(whereClause, keyRangeGenerator) {
        ...
        if (keyRangeGenerator) try {
            ...
            if (!(ex instanceof DexieError) && ex.name && exceptionMap[ex.name]) {
                // Handle IDB exceptions in a nicer way - include call stack and correct type.
                try {
                    throw new exceptionMap[ex.name](ex.message, ex);
                } catch (e2) {
                    error = e2;
                }
            }
