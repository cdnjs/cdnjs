...
const EVENT_LOAD_DATA_API$4 = `load${EVENT_KEY$a}${DATA_API_KEY$6}`;
...
const EVENT_KEYDOWN = `keydown${EVENT_KEY$7}`;
const EVENT_LOAD_DATA_API$3 = `load${EVENT_KEY$7}`;
...
const SELECTOR_DROPDOWN_ITEM$1 = '.dropdown-item';
...
class Tab extends BaseComponent {
  ...
  show() {
    ...
    const hideEvent = active ? EventHandler.trigger(active, EVENT_HIDE$4, {
      relatedTarget: innerElem
    }) : null;
    const showEvent = EventHandler.trigger(innerElem, EVENT_SHOW$4, {
  ...
  _activate(element, relatedElem) {
    ...
    const complete = () => {
      ...
      EventHandler.trigger(element, EVENT_SHOWN$5, {
  ...
  _deactivate(element, relatedElem) {
    ...
    const complete = () => {
      ...
      EventHandler.trigger(element, EVENT_HIDDEN$5, {
        relatedTarget: relatedElem
      });
    };
  ...
  _toggleDropDown(element, open) {
    ...
    toggle(SELECTOR_DROPDOWN_TOGGLE$1, CLASS_NAME_ACTIVE$1);
    toggle(SELECTOR_DROPDOWN_MENU, CLASS_NAME_SHOW$5);
    toggle(SELECTOR_DROPDOWN_ITEM$1, CLASS_NAME_ACTIVE$1);
...
const EVENT_CLICK_DATA_API$1 = `click${EVENT_KEY$5}${DATA_API_KEY$3}`;
...
const EVENT_CLICK_DATA_API = `click${EVENT_KEY$4}${DATA_API_KEY$2}`;
...
class Tooltip extends BaseComponent {
  ...
  show() {
    ...
    tip.classList.add(CLASS_NAME_SHOW$1); // If this is a touch-enabled device we add extra
    // empty mouseover listeners to the body's immediate children;
    // only needed because of broken event delegation on iOS
    // https://www.quirksmode.org/blog/archives/2014/02/mouse_event_bub.html
  ...
  hide() {
    ...
    tip.classList.remove(CLASS_NAME_SHOW$1); // If this is a touch-enabled device we remove the extra
    // empty mouseover listeners we added for iOS support
  ...
  _createTipElement(content) {
    ...
    tip.classList.remove(CLASS_NAME_FADE$1, CLASS_NAME_SHOW$1); // todo: on v6 the following can be achieved with CSS only
...
const EVENT_CLICK$1 = `click${EVENT_KEY$2}`;
const EVENT_LOAD_DATA_API = `load${EVENT_KEY$2}${DATA_API_KEY}`;
...
const EVENT_FOCUSOUT$1 = `focusout${EVENT_KEY$1}`;
const EVENT_CLICK = `click${EVENT_KEY$1}`;
const EVENT_KEYUP = `keyup${EVENT_KEY$1}`;
const EVENT_HIDDEN$1 = 'hidden.bs.dropdown';
const EVENT_SHOWN$1 = 'shown.bs.dropdown';
const CLASS_NAME_FLOATING$1 = 'form-floating';
const CLASS_NAME_FLOATING_OUTLINED$1 = 'form-floating-outlined';
...
class SelectField extends BaseComponent {
  constructor(element) {
    ...
    this._formFloating = element.closest(`.${CLASS_NAME_FLOATING$1}`);
  ...
  initSelect() {
    ...
    if (this._inputGroup) {
      if (this._formFloating.className.includes(CLASS_NAME_FLOATING_OUTLINED$1)) {
  ...
  redraw() {
    if (this._label) {
      ...
      this._formFloating.style.setProperty('--label-floating-margin-right', `-${this._label.offsetWidth - this._label.offsetWidth * LABEL_SCALE$1}px`);
    }
  ...
  _setListeners() {
    ...
    EventHandler.on(this._dropdown, EVENT_HIDDEN$1, () => this.handleFocusOut());
...
export { Alert, Button, Carousel, Collapse, Dropdown, MaterialTab, Modal, Navbar, Offcanvas, Popover, Rainbow, ScrollSpy, SelectField, Shape, Snackbar, Tab, TextField, Toast, Tooltip };
//# sourceMappingURL=materialstyle.esm.js.map
