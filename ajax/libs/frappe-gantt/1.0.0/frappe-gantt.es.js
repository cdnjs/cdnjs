...
class F {
  ...
  draw_bar() {
    this.$bar = f("rect", {
      ...
      class: "bar" + (/^((?!chrome|android).)*safari/i.test(navigator.userAgent) ? " safari" : ""),
  ...
  draw_progress_bar() {
    ...
    this.progress_width = this.calculate_progress_width(), this.$bar_progress = f("rect", {
      x: this.x,
      y: this.y,
    ...
    let e = this.gantt.create_el({
      classes: `date-range-highlight hide highlight-${this.task.id}`,
      width: this.width,
      left: t
    });
    this.$date_highlight = e, this.gantt.$lower_header.prepend(this.$date_highlight), L(this.$bar_progress, "width", 0, this.progress_width);
  }
...
class B {
  ...
  change_view_mode(t = this.options.view_mode, e = !1) {
    ...
    let i, s;
    e && (i = this.current_date, s = this.options.scroll_to, this.options.scroll_to = null), this.options.view_mode = t.name, this.config.view_mode = t, this.update_view_scale(t), this.setup_dates(e), this.render(), e && (this.options.scroll_to = s, this.$container.scrollLeft = d.diff(i, this.gantt_start, this.config.unit) / this.config.step * this.config.column_width), this.trigger_event("view_change", [t]);
  }
  update_view_scale(t) {
