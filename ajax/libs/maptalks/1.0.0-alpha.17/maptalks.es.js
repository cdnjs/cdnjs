...
function loadGeoSymbol(symbol, geo) {
  return loadFunctionTypes(symbol, function () {
    var map = geo.getMap();
    return set(arr, map.getZoom(), extend({}, geo.getProperties(), setProp(prop, map.getBearing(), map.getPitch(), map.getZoom())));
  });
}
...
var ResourceCache = function () {
  ...
  _proto2.login = function login(url) {
  ...
  _proto2.logout = function logout(url) {
    var res = this.resources[url];
...
var SIZE = [];
function getVectorMarkerFixedExtent(out, symbol) {
  var size = calVectorMarkerSize(SIZE, symbol);
...
var SIZE_SYMBOL_PROPS = ['textName', 'markerType', 'markerFile', 'textHaloRadius'];
...
var Geometry = function (_JSONAble) {
    ...
    if (!this._textDesc) {
      ...
      if (Array.isArray(symbol)) {
        this._textDesc = symbol.map(function (s, i) {
          return describeText(textContent[i], s);
        });
      } else {
  ...
  _proto.getContainerExtent = function getContainerExtent(out) {
    ...
    var zoom = map.getGLZoom();
    var minAltitude = this.getMinAltitude();
    var altitude = map.distanceToPoint(minAltitude, 0, zoom, center).x * sign(minAltitude);
    var extent = extent2d.convertTo(function (c) {
      return map._pointToContainerPoint(c, zoom, altitude, TEMP_POINT0$3);
    }, out);
    ...
    if (maxAltitude !== minAltitude) {
      ...
      var extent2 = extent2d.convertTo(function (c) {
        return map._pointToContainerPoint(c, zoom, maxAltitude, TEMP_POINT0$3);
      }, TEMP_EXTENT$2);
    ...
    if (layer && this.type === 'LineString' && maxAltitude && layer.options['drawAltitude']) {
      var groundExtent = extent2d.convertTo(function (c) {
        return map._pointToContainerPoint(c, zoom, 0, TEMP_POINT0$3);
      }, TEMP_EXTENT$2);
...
var MapGeometryEventsHandler = function (_Handler) {
  ...
  _proto._identifyGeometryEvents = function _identifyGeometryEvents(domEvent, type) {
    ...
    var topOnlyEvent = (isMousedown || eventType === 'click') && domEvent.button !== 2;
...
Map$1.include({
  ...
  _pointsToContainerPoints: function _pointsToContainerPoints(points, zoom, altitudes) {
    if (altitudes === void 0) {
      altitudes = [];
    }
...
var TileLayer = function (_Layer) {
  ...
  _proto2._getRootNode = function _getRootNode() {
    if (this._rootNode) {
    ...
    var aspectRatio = this.map.width / this.map.height;
    ...
    this._rootNode = {
      ...
      idx: 0,
      idy: 0,
      extent2d: this._getTileConfig().getTilePrjExtent(0, 0, map._getResolution(0)).convertTo(function (c) {
        return map._prjToPoint(c, 0, TEMP_POINT);
      }),
  ...
  _proto2._getPyramidTiles = function _getPyramidTiles(z, layer) {
    ...
    root.offset[0] = offset0[0];
    root.offset[1] = offset0[1];
    var queue;
    ...
    if (this.options['repeatWorld']) {
      ...
      var scale = map.getResolution(0) / map.getResolution();
      ...
      if (!mapExtent2D.within(root.extent2d.copy()._scale(scale))) {
        ...
        var tileGrid = this._getTiles(0, visualContainerExtent, 2, layer && layer.getRenderer());
  ...
  _proto2._splitNode = function _splitNode(node, projectionView, queue, tiles, gridExtent, maxZoom, offset, parentRenderer) {
    var z = node.z + 1;
    var x = node.x,
        y = node.y,
...
var EditHandle = function (_Eventable) {
  ...
  function EditHandle(target, map, options) {
    ...
    _this.opacity = symbol['opacity'] || 1;
    _this.map = map;
...
var GeometryEditor = function (_Eventable) {
  ...
  _proto.createPolygonEditor = function createPolygonEditor() {
    ...
    function moveVertexHandle(handleConatainerPoint, index, ringIndex) {
      ...
      var nVertex = map._containerPointToPrj(handleConatainerPoint);
    ...
    function createVertexHandle(index, ringIndex) {
      ...
      var handle = me.createHandle(map.coordToContainerPoint(vertex), {
...
var TileLayerCanvasRenderer = function (_CanvasRenderer) {
  ...
  _proto._drawTile = function _drawTile(info, image) {
    if (image) {
...
var VectorLayerRenderer = function (_OverlayLayerCanvasRe) {
  ...
  _proto._batchConversionMarkers = function _batchConversionMarkers(glZoom) {
    ...
    var pts = map._pointsToContainerPoints(cPoints, glZoom, altitudes);
    ...
    for (var _i = 0, _len = markers.length; _i < _len; _i++) {
      ...
      if (!_geo._inCurrentView) {
        var symbolkey = _geo._symbolHash;
