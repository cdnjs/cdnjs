...
(function (global, factory) {
  ...
  function loadGeoSymbol(symbol, geo) {
    return loadFunctionTypes(symbol, function () {
      var map = geo.getMap();
      return set(arr, map ? map.getZoom() : 12, extend({}, geo.getProperties(), setProp(prop, map && map.getBearing() || 0, map && map.getPitch() || 0, map ? map.getZoom() : 10)));
    });
  }
  ...
  if (!IS_NODE) {
    var ua = navigator.userAgent.toLowerCase(),
        ...
        btoa$1 = typeof window !== 'undefined' && isFunction(window.btoa);
    var chromeVersion = 0;
  ...
  var CanvasSymbolizer = function (_Symbolizer) {
    ...
    _proto.prepareCanvas = function prepareCanvas(ctx, style, resources) {
      ...
      if (geometry._symbolHash && geometry._painter && geometry._painter.symbolizers.length === 1) {
        if (tempCtx === ctx && this._layerWidth === ctx.canvas.width && this._layerHeight === ctx.canvas.height && geometry._symbolHash === tempSymbolHash && geometry._layer === tempLayer) {
          return;
        }
      }
    ...
    _pointsToContainerPoints: function _pointsToContainerPoints(points, zoom, altitudes) {
      ...
      if (pitch === 0 && bearing === 0) {
        ...
        if (xmax > xmin && ymax > ymin) {
          var _res = this._getResolution(zoom) / this._getResolution();
      ...
      for (var _i = 0, _len = points.length; _i < _len; _i++) {
        ...
        var point = points[_i].copy()._multi(res);
  ...
  var TileLayer = function (_Layer) {
    ...
    _proto2._getRootNodes = function _getRootNodes(offset0) {
      ...
      var fullExtent = sr.getFullExtent();
      var origin = tileConfig.tileSystem.origin;
    ...
    _proto2._getPyramidTiles = function _getPyramidTiles(z, layer) {
      ...
      var glRes = map.getResolution(map.getGLZoom());
      var offsets = {
        0: offset0
      };
      var extent = new PointExtent();
  ...
  typeof console !== 'undefined' && console.log && console.log('maptalks v1.0.0-alpha.25');
