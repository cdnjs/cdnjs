...
var TileLayer = function (_Layer) {
  ...
  _proto2._splitNode = function _splitNode(node, projectionView, queue, tiles, gridExtent, renderer) {
    var map = this.getMap();
    var z = node.z + 1;
    var x = node.x,
        y = node.y,
        extent2d = node.extent2d;
    var width, height, minx, maxy;
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (z === 1) {
        extent = this._getTileConfig().getTilePrjExtent(childX, childY, map._getResolution(z)).convertTo(function (c) {
          return map._prjToPoint(c, z, TEMP_POINT);
        });
      } else {
      ...
      var tileId = this._getTileId(childX, childY, z);
      ...
      var visible = this._isTileVisible(childNode, projectionView, glScale);
  ...
  _proto2._isTileVisible = function _isTileVisible(node, projectionView, glScale) {
    if (node.z === 0) {
      return 1;
    }
    ...
    var error = this._getScreenSpaceError(node, glScale);

    return error >= maxError ? 1 : 0;
  };

  _proto2._isTileInFrustum = function _isTileInFrustum(node, projectionView, glScale) {
    ...
    var box = [[xmin * glScale, ymin * glScale, 0], [xmax * glScale, ymax * glScale, 0]];
    return intersectsBox(projectionView, box);
  };

  _proto2._getScreenSpaceError = function _getScreenSpaceError(node, glScale) {
    ...
    var distanceToCamera = distanceToRect([xmin * glScale, ymin * glScale, 0], [xmax * glScale, ymax * glScale, 0], map.cameraPosition);
    var distance$$1 = Math.max(Math.abs(distanceToCamera), 1E-7);
    var error = geometricError / distance$$1;
    return error;
  };
