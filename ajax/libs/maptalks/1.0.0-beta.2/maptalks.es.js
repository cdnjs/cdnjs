...
var DrawAltitudeSymbolizer = function (_PointSymbolizer) {
  ...
  _proto.symbolize = function symbolize(ctx) {
    ...
    if (this.geometry.type === 'LineString') {
      ...
      var groundPoints = this.getPainter().getPaintParams(style['lineDx'], style['lineDy'], true, '_groundpt')[0];
...
var Painter = function (_Class) {
  ...
  _proto.getPaintParams = function getPaintParams(dx, dy, ignoreAltitude, ptkey) {
    if (ptkey === void 0) {
      ptkey = '_pt';
    }
    ...
    var cPoints = this._pointContainerPoints(points, dx, dy, ignoreAltitude, this._hitPoint && !mapExtent.contains(this._hitPoint), null, ptkey);
...
var Map$1 = function (_Handlerable) {
  ...
  _proto._prjsToPointsAtRes = function _prjsToPointsAtRes(pCoords, res) {
  ...
  _proto._getPath2DPoints = function _getPath2DPoints(prjCoords, disableSimplify, res) {
    ...
    if (!Array.isArray(prjCoords)) {
      ...
      for (var i = 0, len = prjCoords.length; i < len; i++) {
        ...
        var pt = map._prjsToPointsAtRes(prjCoord, res);
