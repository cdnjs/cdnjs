...
var CanvasRenderer = function (_Class) {
  ...
  function CanvasRenderer(layer) {
    ...
    _this._drawTime = 0;
    if (Browser$1.decodeImageInWorker && (layer.options['renderer'] === 'gl' || !Browser$1.safari && !Browser$1.iosWeixin)) {
...
var ResourceCache = function () {
  ...
  _proto3.addResource = function addResource(url, img) {
    ...
    if (img && !img.close && Browser$1.imageBitMap && !Browser$1.safari && !Browser$1.iosWeixin) {
...
Map$1.include({
  ...
  updateCenterAltitude: function updateCenterAltitude() {
    var prjCenter = this._getPrjCenter();
    var altitude = this._queryTerrainByProjCoord(prjCenter);
  ...
  _recenterOnTerrain: function _recenterOnTerrain() {
    ...
    var pitch = this.getPitch() * RADIAN$1;
    var bearing = this.getBearing() * RADIAN$1;
    var centerAltitude = this._queryTerrainByProjCoord(this._prjCenter);
    ...
    this._resizeObserver = new ResizeObserver(function (entries) {
      if (entries.length) {
        ...
        if (borderBoxSize && borderBoxSize.length) {
          _this3._domContentRect = {
            width: borderBoxSize[0].inlineSize,
            height: borderBoxSize[0].blockSize
          };
        } else {
          _this3._domContentRect = entries[0].contentRect;
        }
      } else {
        delete _this3._domContentRect;
      }
      if (_this3.onDomSizeChange) {
        _this3.onDomSizeChange();
      }
    });
...
var EVENTS$2 = 'show hide remove setzindex';
...
var TileLayerCanvasRenderer = function (_CanvasRenderer) {
    ...
    if (layer._isPyramidMode()) {
      ...
      for (var j = 0; j < 2; j++) {
        for (var jj = 0; jj < 2; jj++) {
          var id = layer._getTileId(cx + j, cy + jj, cz + zoomOffset, info.layer);
  ...
  _proto2._findParentTile = function _findParentTile(info) {
    ...
    if (layer._isPyramidMode()) {
      for (var z = info.z - 1; z >= minZoom; z--) {
        ...
        var id = layer._getTileId(x, y, z + zoomOffset, info.layer);
        if (this.tileCache.has(id)) {
          var tile = this.tileCache.getAndRemove(id);
          if (this.isValidCachedTile(tile)) {
            this.tileCache.add(id, tile);
    ...
    for (var _diff = 1; _diff <= zoomDiff; _diff++) {
      ...
      var _res = sr.getResolution(_z + zoomOffset);
      if (!_res) continue;
      var tileIndex = layer._getTileConfig().getTileIndex(prj, _res);
      var _id = layer._getTileId(tileIndex.x, tileIndex.y, _z + zoomOffset, info.layer);
      if (this.tileCache.has(_id)) {
  ...
  _proto2.isValidCachedTile = function isValidCachedTile() {
    return true;
  };
...
var TileLayerGLRenderer = function (_ImageGLRenderable) {
  ...
  _proto.drawTile = function drawTile(tileInfo, tileImage, parentContext) {
    ...
    var polygonOffset = this.drawingCurrentTiles ? layerPolygonOffset - 1 : layerPolygonOffset;
