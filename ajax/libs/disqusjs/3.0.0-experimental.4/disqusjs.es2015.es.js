...
const useStore = create$1((set, get)=>{
    return {
        ...
        thread: null,
        fetchThread (shortname, identifier, apiKeys, api = 'https://disqus.com/api/') {
            return _asyncToGenerator(function*() {
                try {
                    const thread = yield disqusJsApiFetcher(apiKeys)(`${api}3.0/threads/list.json?forum=${encodeURIComponent(shortname)}&thread=${encodeURIComponent(`ident:${identifier}`)}`);
                    if (thread.code === 0) {
                        set({
                            thread
                        });
        ...
        fetchMorePosts (shortname, id, apiKeys, api = 'https://disqus.com/api/') {
            return _asyncToGenerator(function*() {
                set({
                    loadingPosts: true,
                    morePostsError: false
                ...
                try {
                    const newPosts = yield disqusJsApiFetcher(apiKeys)(url);
                    if (newPosts.code === 0) {
                        set((state)=>({
