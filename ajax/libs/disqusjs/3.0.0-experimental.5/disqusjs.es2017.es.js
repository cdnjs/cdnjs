                        ...
                        children: [
                            /*#__PURE__*/ e("div", {
                                ...
                                children: [
                                    ...
                                    props.admin === props.username && /*#__PURE__*/ e("span", {
...
const DisqusJSPosts = (props)=>{
    const apiKeys = _(()=>Array.isArray(props.apikey) ? props.apikey : [
            props.apikey
        ]
    , [
        props.apikey
    ...
    const fetchNextPageOfPosts = A$1(()=>fetchMorePosts(props.shortname, props.id, apiKeys, props.api)
    , [
        apiKeys,
        fetchMorePosts,
        props.api,
    ...
    y(()=>{
        ...
        if (posts.length === 0 && fetchFirstPageRef.current !== props.id) {
            resetPosts();
    ...
    const loadMoreCommentsButtonClickHandler = A$1(()=>{
        fetchNextPageOfPosts();
    }, [
        fetchNextPageOfPosts
    ]);
...
const DisqusJSThread = (props)=>{
    ...
    y(()=>{
        if (!thread && fetchThreadRef.current !== identifier) {
...
const getElementFromConfig = (el)=>el ? typeof el === 'string' ? document.querySelector(el) : el : document.getElementById('disqus_thread')
;
class DisqusJS {
    render(el) {
