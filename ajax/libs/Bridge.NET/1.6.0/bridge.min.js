...
var results=(/function (.{1,})\(/).exec(str);return(results&&results.length>1)?results[1]:"Object";},is:function(obj,type,ignoreFn){if(typeof type=="string"){type=Bridge.unroll(type);}
...
return false;},as:function(obj,type){return Bridge.is(obj,type)?obj:null;},cast:function(obj,type){if(obj===null){return null;}
var result=Bridge.as(obj,type);if(result==null){throw new Bridge.InvalidCastException('Unable to cast type '+Bridge.getTypeName(obj)+' to type '+Bridge.getTypeName(type));}
...
try{return instance.constructor;}catch(ex){return Object;}},isLower:function isLower(c){var s=String.fromCharCode(c);return s===s.toLowerCase()&&s!==s.toUpperCase();},isUpper:function isUpper(c){var s=String.fromCharCode(c);return s!==s.toLowerCase()&&s===s.toUpperCase();},fn:{call:function(obj,fnName){var args=Array.prototype.slice.call(arguments,2);obj=obj||Bridge.global;return obj[fnName].apply(obj,args);},bind:function(obj,method,args,appendArgs){if(method&&method.$method==method&&method.$scope==obj){return method;}
...
result.reverse();return Bridge.fn.$build(result);}}};Bridge=core;})();
...
return arguments[0].apply(null,Array.prototype.slice.call(arguments,1));}};Bridge.Nullable=nullable;Bridge.hasValue=Bridge.Nullable.hasValue;})();

(function(){var char={isWhiteSpace:function(value){return/\s/.test(value);},isDigit:function(value){if(value<256){return(value>=48&&value<=57);}
...
return str.match(suffix+"$")!==null;},contains:function(str,value){if(value==null){throw new Bridge.ArgumentNullException();}
...
return arr;}};Bridge.String=string;})();
...
if(config.init){config.init.apply(this,arguments);}};},define:function(className,gscope,prop){if(!prop){prop=gscope;gscope=Bridge.global;}
...
prop=prop||{};var extend=prop.$inherits||prop.inherits,statics=prop.$statics||prop.statics,base,cacheName=prop.$cacheName,prototype,nameParts,scope=prop.$scope||Bridge.global,i,v,ctorCounter,isCtor,name,fn;if(prop.$inherits){delete prop.$inherits;}
...
prop.$$initCtor=Bridge.Class.initCtor;ctorCounter=0;for(name in prop){v=prop[name];isCtor=name==="constructor";if(Bridge.isFunction(v)&&(isCtor||Bridge.String.startsWith(name,"constructor\\$"))){ctorCounter++;}
...
return result;}},constructor:function(name){this.name=name;Bridge.CultureInfo.cultures[name]=this;},getFormat:function(type){switch(type){case Bridge.NumberFormatInfo:return this.numberFormat;case Bridge.DateTimeFormatInfo:return this.dateTimeFormat;default:return null;}},clone:function(){return Bridge.copy(new Bridge.CultureInfo(this.name),this,["englishName","nativeName","numberFormat","dateTimeFormat"]);}});
...
return this.trunc(x/y);}}});Bridge.Class.addExtend(Bridge.Int,[Bridge.IComparable$1(Bridge.Int),Bridge.IEquatable$1(Bridge.Int)]);
...
return part;});},parse:function(value,provider){var dt=Date.parse(value);if(!isNaN(dt)){return new Date(dt);}
return this.parseExact(value,null,provider);},parseExact:function(str,format,provider,silent){if(!format){format=["G","g","F","f","D","d","R","r","s","U","u","O","o","Y","y","M","m","T","t"];}
if(Bridge.isArray(format)){var i,d;for(i=0;i<format.length;i++){d=Bridge.Date.parseExact(str,format[i],provider,true);if(d!=null){return d;}}
...
if(zzh==0&&zzm==0){return new Date(year,month-1,date,hh,mm,ss,ff);}
...
return null;},tryParse:function(value,provider,result){result.v=this.parseExact(value,null,provider,true);if(result.v==null){result.v=new Date(-864e13);return false;}
return true;},tryParseExact:function(value,format,provider,result){result.v=this.parseExact(value,format,provider,true);if(result.v==null){result.v=new Date(-864e13);return false;}
...
Bridge.Class.generic('Bridge.List$1',function(T){var $$name=Bridge.Class.genericName('Bridge.List$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.define($$name,{inherits:[Bridge.ICollection$1(T),Bridge.ICollection],constructor:function(obj){if(Object.prototype.toString.call(obj)==='[object Array]'){this.items=obj;}
else if(Bridge.is(obj,Bridge.IEnumerable)){this.items=Bridge.toArray(obj);}
else{this.items=[];}},checkIndex:function(index){if(index<0||index>(this.items.length-1)){throw new Bridge.ArgumentOutOfRangeException('Index out of range');}},getCount:function(){return this.items.length;},get:function(index){this.checkIndex(index);return this.items[index];},set:function(index,value){this.checkReadOnly();this.checkIndex(index);this.items[index]=value;},add:function(value){this.checkReadOnly();this.items.push(value);},addRange:function(items){this.checkReadOnly();var array=Bridge.toArray(items),i,len;for(i=0,len=array.length;i<len;++i){this.items.push(array[i]);}},clear:function(){this.checkReadOnly();this.items=[];},indexOf:function(item,startIndex){var i;if(!Bridge.isDefined(startIndex)){startIndex=0;}
...
promise.then(function(){task.setResult(handler?handler.apply(null,arguments):arguments);},function(){task.setError(new Error(Array.prototype.slice.call(arguments,0)));});return task;}},continueWith:function(continuationAction,raise){var task=new Bridge.Task(),me=this,fn=raise?function(){task.setResult(continuationAction(me));}:function(){try{task.setResult(continuationAction(me));}
...
return idx;},$get:function(indices){var r=this[Bridge.Array.toIndex(this,indices)];return typeof r!=="undefined"?r:this.$v;},get:function(arr,indices){var r=arr[Bridge.Array.toIndex(arr,indices)];return typeof r!=="undefined"?r:arr.$v;},$set:function(indices,value){this[Bridge.Array.toIndex(this,indices)]=value;},set:function(arr,indices,value){arr[Bridge.Array.toIndex(arr,indices)]=value;},getLength:function(arr,dimension){if(dimension>=(arr.$s?arr.$s.length:1)){throw new Bridge.ArgumentException("Invalid dimension");}
...
arr[i]=v;}}
return arr;},toEnumerable:function(array){return new Bridge.ArrayEnumerable(array);},toEnumerator:function(array){return new Bridge.ArrayEnumerator(array);}};Bridge.Array=array;})();
