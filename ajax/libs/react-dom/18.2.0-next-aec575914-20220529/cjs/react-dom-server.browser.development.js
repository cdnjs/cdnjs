...
var ReactVersion = '18.2.0-next-aec575914-20220529';
...
function writeStartClientRenderedSuspenseBoundary(destination, responseState, errorHash, errorMesssage, errorComponentStack) {
  var result;
  result = writeChunkAndReturn(destination, startClientRenderedSuspenseBoundary);

  if (errorHash) {
    writeChunk(destination, clientRenderedSuspenseBoundaryError1);
    writeChunk(destination, stringToChunk(escapeTextForBrowser(errorHash))); // In prod errorMessage will usually be nullish but there is one case where
...
function writeClientRenderBoundaryInstruction(destination, responseState, boundaryID, errorHash, errorMessage, errorComponentStack) {
  writeChunk(destination, responseState.startInlineScript);
  ...
  if (errorHash || errorMessage || errorComponentStack) {
    ...
    writeChunk(destination, stringToChunk(escapeJSStringsForInstructionScripts(errorHash || '')));
  }
...
function logRecoverableError(request, error) {
  ...
  var errorHash = request.onError(error);

  if (errorHash != null && typeof errorHash !== 'string') {
    // eslint-disable-next-line react-internal/prod-error-codes
    throw new Error("onError returned something with a type other than \"string\". onError should return a string and may return null or undefined but must not return anything else. It received something of type \"" + typeof errorHash + "\" instead");
  }
...
function erroredTask(request, boundary, segment, error) {
  ...
  var errorHash = logRecoverableError(request, error);
...
function abortTask(task) {
  ...
  if (boundary === null) {
    ...
    if (!boundary.forceClientRender) {
      ...
      var _error = new Error('This Suspense boundary was aborted by the server');

      boundary.errorHash = request.onError(_error);
...
function flushSegment(request, destination, segment) {
  ...
  if (boundary.forceClientRender) {
    ...
    writeStartClientRenderedSuspenseBoundary(destination, request.responseState, boundary.errorHash, boundary.errorMessage, boundary.errorComponentStack); // Flush the fallback.
...
function flushClientRenderedBoundary(request, destination, boundary) {
  return writeClientRenderBoundaryInstruction(destination, request.responseState, boundary.id, boundary.errorHash, boundary.errorMessage, boundary.errorComponentStack);
}
