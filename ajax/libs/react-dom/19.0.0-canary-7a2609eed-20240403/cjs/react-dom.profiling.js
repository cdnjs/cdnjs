...
function resumeMountClassInstance(workInProgress, ctor, newProps, renderLanes) {
  ...
  const oldProps = resolveClassComponentProps(ctor, unresolvedOldProps, workInProgress.type === workInProgress.elementType);
...
function mountLazyComponent(_current, workInProgress, elementType, renderLanes) {
  ...
  if (typeof Component === 'function') {
    ...
    if ($$typeof === REACT_FORWARD_REF_TYPE) {
      ...
      return updateMemoComponent(null, workInProgress, Component, resolveDefaultProps(Component.type, resolvedProps), // The inner type can have defaults too
      renderLanes);
    }
  }
  ...
  if (!didSuspend) {
    ...
    if (isSuspenseInstanceFallback(suspenseInstance)) {
...

      {
        ...
        error.stack = '';
        error.digest = digest;
        capturedValue = createCapturedValueFromError(error, componentStack);
      }
    ...
    if (workInProgress.flags & ForceClientRender) {
      ...
      pushPrimaryTreeSuspenseHandler(workInProgress);
      workInProgress.flags &= ~ForceClientRender;
      const capturedValue = createCapturedValueFromError(Error(formatProdErrorMessage(422)), null);
      return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, capturedValue);
    ...
    case ClassComponent:
      {
        const Component = workInProgress.type;
        const unresolvedProps = workInProgress.pendingProps;
        const resolvedProps = resolveClassComponentProps(Component, unresolvedProps, workInProgress.elementType === Component);
    ...
    case ClassComponent:
      {
        if ((flags & Snapshot) !== NoFlags$1) {
          if (current !== null) {
            ...
            const snapshot = instance.getSnapshotBeforeUpdate(resolveClassComponentProps(finishedWork.type, prevProps, finishedWork.elementType === finishedWork.type), prevState);
...
function commitClassLayoutLifecycles(finishedWork, current) {
  ...
  if (current === null) {
    ...
    const prevProps = resolveClassComponentProps(finishedWork.type, current.memoizedProps, finishedWork.elementType === finishedWork.type);
