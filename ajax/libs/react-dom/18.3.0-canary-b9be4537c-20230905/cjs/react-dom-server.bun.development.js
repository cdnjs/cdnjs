...
function createResumableState(identifierPrefix, nonce, bootstrapScriptContent, bootstrapScripts, bootstrapModules, externalRuntimeConfig) {
  ...
  if (bootstrapScripts !== undefined) {
    for (var i = 0; i < bootstrapScripts.length; i++) {
      ...
      preloadBootstrapScript(resumableState, src, nonce, integrity, crossOrigin);
  ...
  if (bootstrapModules !== undefined) {
    for (var _i = 0; _i < bootstrapModules.length; _i++) {
      ...
      preloadBootstrapModule(resumableState, _src, nonce, _integrity, _crossOrigin);
...
function assignSuspenseBoundaryID(renderState, resumableState) {
  ...
  return stringToPrecomputedChunk(renderState.boundaryPrefix + generatedID.toString(16));
}
...
function pushImg(target, props, resumableState) {
  ...
  !(typeof src === 'string' && src[4] === ':' && (src[0] === 'd' || src[0] === 'D') && (src[1] === 'a' || src[1] === 'A') && (src[2] === 't' || src[2] === 'T') && (src[3] === 'a' || src[3] === 'A')) && !(typeof srcSet === 'string' && srcSet[4] === ':' && (srcSet[0] === 'd' || srcSet[0] === 'D') && (srcSet[1] === 'a' || srcSet[1] === 'A') && (srcSet[2] === 't' || srcSet[2] === 'T') && (srcSet[3] === 'a' || srcSet[3] === 'A'))) {
    ...
    if (props.fetchPriority === 'high' || resumableState.highImagePreloads.size < 10) {
...
function pushScript(target, props, resumableState, textEmbedded, insertionMode, noscriptTagInScope) {
  {
...
function pushStartInstance(target, type, props, resumableState, renderState, formatContext, textEmbedded) {
  ...
  switch (type) {
    ...
    case 'title':
      return pushTitle(target, props, renderState, formatContext.insertionMode, formatContext.noscriptTagInScope) ;

    case 'link':
      return pushLink(target, props, resumableState, renderState, textEmbedded, formatContext.insertionMode, formatContext.noscriptTagInScope);

    case 'script':
      return pushScript(target, props, resumableState, textEmbedded, formatContext.insertionMode, formatContext.noscriptTagInScope) ;

    case 'style':
      return pushStyle(target, props, resumableState, renderState, textEmbedded, formatContext.insertionMode, formatContext.noscriptTagInScope);

    case 'meta':
      return pushMeta(target, props, renderState, textEmbedded, formatContext.insertionMode, formatContext.noscriptTagInScope);
    // Newline eating tags
...
function writeCompletedBoundaryInstruction(destination, resumableState, renderState, boundaryID, contentSegmentID, boundaryResources) {
...
function writeClientRenderBoundaryInstruction(destination, resumableState, renderState, boundaryID, errorDigest, errorMessage, errorComponentStack) {
  var scriptFormat = resumableState.streamingFormat === ScriptStreamingFormat;
...
function writePreamble(destination, resumableState, renderState, willFlushAllSegments) {
  ...
  resumableState.highImagePreloads.forEach(flushResourceInPreamble, destination);
...
function createRequest(children, resumableState, renderState, rootFormatContext, progressiveChunkSize, onError, onAllReady, onShellReady, onShellError, onFatalError, onPostpone) {
  ...
  var rootTask = createTask(request, null, children, null, rootSegment, abortSet, null, rootFormatContext, emptyContextObject, rootContextSnapshot, emptyTreeContext);
...
function createTask(request, thenableState, node, blockedBoundary, blockedSegment, abortSet, keyPath, formatContext, legacyContext, context, treeContext) {
...
function renderSuspenseBoundary(request, task, props) {
  ...
  var newBoundary = createSuspenseBoundary(request, fallbackAbortSet, task.keyPath);
  ...
  var suspendedFallbackTask = createTask(request, null, fallback, parentBoundary, boundarySegment, fallbackAbortSet, task.keyPath, task.formatContext, task.legacyContext, task.context, task.treeContext);
...
function spawnNewSuspendedTask(request, task, thenableState, x) {
  ...
  segment.lastPushedText = false;
  var newTask = createTask(request, thenableState, task.node, task.blockedBoundary, newSegment, task.abortSet, task.keyPath, task.formatContext, task.legacyContext, task.context, task.treeContext);
...
function renderToReadableStream(children, options) {
  return new Promise(function (resolve, reject) {
    ...
    var resumableState = createResumableState(options ? options.identifierPrefix : undefined, options ? options.nonce : undefined, options ? options.bootstrapScriptContent : undefined, options ? options.bootstrapScripts : undefined, options ? options.bootstrapModules : undefined, options ? options.unstable_externalRuntimeSrc : undefined);
    var request = createRequest(children, resumableState, createRenderState(resumableState, options ? options.nonce : undefined, options ? options.importMap : undefined), createRootFormatContext(options ? options.namespaceURI : undefined), options ? options.progressiveChunkSize : undefined, options ? options.onError : undefined, onAllReady, onShellReady, onShellError, onFatalError, options ? options.onPostpone : undefined);
