...
else if(2!==m.status&&null===m.destination){m.destination=q;try{Ie(m,q)}catch(v){xe(m,v),ye(m,v)}}},cancel:function(){Oe(m)}},{highWaterMark:0});t.allReady=g;c(t)},function(t){g.catch(function(){});d(t)},e);if(b&&b.signal){var k=b.signal;if(k.aborted)Oe(m,k.reason);else{var p=function(){Oe(m,k.reason);k.removeEventListener("abort",p)};k.addEventListener("abort",p)}}we(m)})};exports.version="18.3.0-next-1528c5ccd-20230306";
