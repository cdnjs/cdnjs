...
else if(2!==m.status&&null===m.destination){m.destination=q;try{He(m,q)}catch(u){we(m,u),xe(m,u)}}},cancel:function(){Ne(m)}},{highWaterMark:0});r.allReady=g;c(r)},function(r){g.catch(function(){});d(r)},e);if(b&&b.signal){var k=b.signal;if(k.aborted)Ne(m,k.reason);else{var p=function(){Ne(m,k.reason);k.removeEventListener("abort",p)};k.addEventListener("abort",p)}}ve(m)})};exports.version="18.3.0-next-1528c5ccd-20230306";
