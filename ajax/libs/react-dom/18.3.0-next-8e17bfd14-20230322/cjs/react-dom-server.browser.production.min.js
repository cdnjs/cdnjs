...
else if(2!==m.status&&null===m.destination){m.destination=q;try{Je(m,q)}catch(v){ye(m,v),ze(m,v)}}},cancel:function(){Pe(m)}},{highWaterMark:0});t.allReady=g;c(t)},function(t){g.catch(function(){});d(t)},e);if(b&&b.signal){var k=b.signal;if(k.aborted)Pe(m,k.reason);else{var p=function(){Pe(m,k.reason);k.removeEventListener("abort",p)};k.addEventListener("abort",p)}}xe(m)})};exports.version="18.3.0-next-8e17bfd14-20230322";
