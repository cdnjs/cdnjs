...
var Iconify = (function () {
	...
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.listAPIProviders = exports.getAPIConfig = exports.setAPIConfig = exports.createAPIConfig = void 0;
	/**
	...
	exports.listAPIProviders = listAPIProviders;
	});
	...
	function sendAPIQuery(target, query, callback) {
	    ...
	    if (typeof target === 'string') {
	        ...
	        if (config$1) {
	            ...
	            var moduleKey = target.resources
	                ? target.resources[0]
	                : '';
	            var api$1 = modules.getAPIModule(moduleKey);
	            if (api$1) {
	                send = api$1.send;
	            }
	        }
	...
	exports.APIInternalFunctions = {
	    ...
	    sendAPIQuery: query.sendAPIQuery,
	    listAPIProviders: config.listAPIProviders,
	...
	function calculateMaxLength(provider, prefix) {
	    ...
	    if (!config$1.maxURL) {
	        ...
	        var maxHostLength = 0;
	        config$1.resources.forEach(function (item) {
	            var host = item;
	        ...
	        var url = params.mergeParams(prefix + '.js', {
	            icons: '',
	            callback: rootVarName,
	        });
	        result =
	            config$1.maxURL - maxHostLength - config$1.path.length - url.length;
	    }
	    // Cache stuff and return result
	    var cacheKey = provider + ':' + prefix;
	    pathCache[cacheKey] = config$1.path;
	 ...
	 * Load icons
	 */
	var send = function (host, params$1, status) {
	    if (params$1.type !== 'icons') {
	        // JSONP supports only icons
	        status.done(void 0, 400);
	        return;
	    }
	    var provider = params$1.provider;
	    var prefix = params$1.prefix;
	    var icons = params$1.icons;
	    ...
	    var path = pathCache[cacheKey] + url;
	 ...
	 * Return API module
	 */
	exports.jsonpAPIModule = { prepare: prepare, send: send };
	});

	var fetch_1 = createCommonjsModule(function (module, exports) {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.fetchAPIModule = exports.getFetch = exports.setFetch = void 0;
	...
	var detectFetch = function () {
	    ...
	    try {
	        ...
	        var req = commonjsGlobal[chunk + 'qui' + chunk];
	...
	function calculateMaxLength(provider, prefix) {
	    ...
	    if (!config$1.maxURL) {
	        ...
	        // Get available length
	        var url = params.mergeParams(prefix + '.json', {
	            icons: '',
	        });
	        result =
	            config$1.maxURL - maxHostLength - config$1.path.length - url.length;
	    ...
	    // Cache stuff and return result
	    var cacheKey = provider + ':' + prefix;
	    pathCache[provider] = config$1.path;
	    maxLengthCache[cacheKey] = result;
	    return result;
	}
	/**
	...
	var send = function (host, params$1, status) {
	    ...
	    // Get path
	    var path = getPath(params$1.provider);
	    switch (params$1.type) {
	        case 'icons': {
	            var prefix = params$1.prefix;
	            var icons = params$1.icons;
	            var iconsList = icons.join(',');
	            path += params.mergeParams(prefix + '.json', {
	                icons: iconsList,
	            });
	            break;
	        }
	        case 'custom': {
	            var uri = params$1.uri;
	...
	var commonFunctions = {
	    // Version
	    getVersion: function () { return '2.1.0-beta.2'; },
	    // Render SVG
	 ...
	 */
	// Set API module
	modules.setAPIModule('', fetch_1.getFetch() ? fetch_1.fetchAPIModule : jsonp.jsonpAPIModule);
	/**
	 * Function to enable node-fetch for getting icons on server side
	 */
	Iconify._api.setFetch = function (nodeFetch) {
	    fetch_1.setFetch(nodeFetch);
	    modules.setAPIModule('', fetch_1.fetchAPIModule);
	};
	/**
	 * Browser stuff
	 */
	if (typeof document !== 'undefined' && typeof window !== 'undefined') {
	    ...
	    cache.cache.store = browserStorage.storeCache;
