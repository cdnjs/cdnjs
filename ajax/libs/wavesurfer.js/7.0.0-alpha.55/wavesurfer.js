...
class WaveSurfer extends Player {
    ...
    async load(url, channelData, duration) {
        ...
        // Render the audio's first two channels
        const data = [this.decodedData.getChannelData(0)];
        if (this.decodedData.numberOfChannels > 1) {
            data.push(this.decodedData.getChannelData(1));
        }
        this.renderer.render(data, this.decodedData.duration);
    }
    /** Zoom in or out */
    zoom(minPxPerSec) {
        if (!this.decodedData) {
