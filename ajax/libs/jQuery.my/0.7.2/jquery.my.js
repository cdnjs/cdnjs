 ...
 * 
 * Changes since 0.7.0:
 * – Redactor weird behavior in Opera and FF issue fixed
 * – Select2 plugin compat issues resolved
 * – Nested forms error messages disappear fixed
 * – watch param of ui control implemeted – it forces
 * 	 control to subscribe to changes of other control,
 * 	 it's just an alternative way for recalc
 * – $(form).my("valid") returns true if form has no errors
 * – $(form).my("manifest","json") returns manifest converted to json
 * 	 with functions and regexps folded to strings
 * 
...
;(function($) {var _version = "jQuery.my 0.7.2";
	...
	var f = {
		...
		tojson:(function(){
			...
			function s2 (w, ctab0, tab){
				var tl=0,a,i,k,l,v,ctab=ctab0||0,xt = '\t\t\t\t\t\t\t\t\t';
				...
				switch(typeof w){
					case'string': return JSON.stringify(w);
					case'number':return isFinite(w)?'"'+String(w)+'"':'null';
					case'boolean':case'null':return String(w);
					case'function':return s2(w.toString().replace(/^(function)([^\(]*)(\(.*)/,"$1 $3").replace(/(})([^}]*$)/,'$1'));
					case'object':if(!w) return'null';
					if(typeof w.toJSON==='function') return s2(w.toJSON(),ctab+(tab?1:0),tab);
					a=[];
					if(typeof w.length==='number'&&!(w.propertyIsEnumerable('length'))){
						l=w.length; for(i=0;i<l;i+=1){a.push(s2(w[i],ctab+(tab?1:0),tab)||'null')}
						return'['+
							a.join(','+(tab?"\n"+xt.to(ctab*tl+tl):""))+
						']';
					}
					for(k in w) if (typeof k=='string') {
			...
			if (xdata) {
				...
				if (depth && oui.recalc) {
					for (var i=0;i<list.length;i++) {
						if (list[i] && $.type(list[i])=="string" && (item = list[i].compact()) && ui[item]) {
							if (ui[item].recalc) {
								if (dest.indexOf(item)==-1) dest.push(item);
							} else once[item]=true;
						}
					};
					for (i=0; i<dest.length; i++) once = $e(true, once, f.update($root.find(dest[i]),null,depth-1));
		...
		changed: function ($this,obj,uiNode,p) {
			var d = $this.data("my"); if (d && !d.disabled) {
				if (!d.errors || d.errors.values().join("").length==0 && obj ) 
					obj.data("my").lastCorrect = $e(true, {}, d.ddata||d.data);
	...
	var methods = {
		...
		init : function( data, defaults ) { 
			...
			if (!data.params || (data.params && !data.params.strict)) unwindXJSON(data);
		...
		ui: function (u) {
			...
			d.ui = unwindUI(f.overlap(d.ui,u));
			for (var i=0;i<a.length;i++) f.initControl($x.find(a[i]), $x, d.ui[a[i]], a[i]);
		...
		manifest: function (format) {return format=="json"?f.tojson(this.data("my").manifest):this.data("my").manifest},
	...
	function unwindXJSON (node){ //recursively unwinds string def of funcs and regexps, modifies  source obj!
		for (var i in node) {
			if (node.hasOwnProperty(i)) {
				var n = node[i], t = $.type(n);
				if (/^(ob|ar)/.test(t)) unwindXJSON(n);
				else if (t=="string" && /(^function\s\(|^new\sRegExp)/.test(n)) node[i]=eval('('+n+')');
			}
		}
	}
	
	function unwindUI (ui) { //unwinds ui recalcs, short defs and watch hooks, modifies source obj!
		$.each(ui, function (i,v){
			...
			if (v.recalc) {
				if (typeof v.recalc == 'string') list = v.recalc.split(re);
				else if ($.type(v.recalc)=="array") list = v.recalc;
				list = list.compact().unique();
			}
			if (list.length) ui[i].recalc=list;
			if (v.watch) {
				if (typeof v.watch == 'string') watch = v.watch.split(re);
				else if ($.type(v.watch)=="array") watch = v.watch.slice(0);
				watch = watch.compact().unique();
				for (var j=0;j<watch.length;j++) if (row = ui[watch[j]]) {
					if (!row["recalc"]) row["recalc"]=[i];
					else if (typeof row["recalc"] == "string") row["recalc"]+=","+i;
					else if ($.type(row["recalc"]) == "array" &&
							row["recalc"].indexOf(i)==-1) row["recalc"].push(i);
				}
			}
		});
		return ui;
	...
	function $e() {
		return $e.prototype.apply(this,arguments);
	}
