 ...
 *
 * — Optimized try/catch for v8 hydrogen
 * — Console now prints arguments as list, not array
 * — Minor bug fixes for select.uiSelectmenu plugin
 *
...
;(function ($) {var _version = "jQuery.my 1.1.4";
	...
	function _ERR () { console.error.apply(console, arguments); }
				...
				"": function ($o,v) {
					if (n(v)) {
						...
						//now ditinguish between jQ selectmenu plugin and jQ Mobile
							...
							else if ($o.data("selectmenu")) {
								$o.find("option").each(function (i){
									if (f.extval($(this)) == v) $o.selectmenu("value",i);
								});
							}
						}*/
					}
		...
		if (xdata) {
			...
			if (oui.bind != N) {
				...
				if (N != oui.check) {
					...
					catch (e) { _ERR ("Error "+ e.message+" in .check validatir for "+selector, $root, e.stack); }
				}
...
//=======================================
	...
	function _unjson (node, exclude){
		var i, incl = !exclude;
		for (i in node) if (node.hasOwnProperty(i)) _unjsonNode(i, node, incl);
	}
...
//######### PUBLIC METHODS ##############
	...
	return {
		"con": function () {try { console.log.apply (console, arguments); } catch (e) {}},
