 ...
 * Маin сhanges:
 * 
 * — Internal call structrure refactored to provide async
 * 
 * — Entire form is promise which is resolved when form come initialized,
 *   form data object is provided as resolution data
 *   
 * — Form's promise is rejected if form or any control init failed
 * 	 unless params.silent key is set to true in the manifest
 *   
 * — If check field is present controls are first validated using 
 * 	 rules defined in attrs of DOM control (if any) and then check is
 * 	 called if there were no error during DOM validation
 * 
 * — Autotext, generated by $.my during validation over DOM,
 * 	 now respects locale (ru and en are predefined)
 * 
 * — formgen now supports multiple inline row width and other 
 * 	 row properties change 
 * 
 * — data("my") is mounted on form DOM node before init call
 * 
 * — If field has no DOM elt for error msg, it's mounted as title attr on control
 * 
 * — Any init exept those in lists can return promise – and become async
 * 
 * — .error field on manifest allows replacement of the form's container
 * 	 with provided html string or with the result of error:function (err, stack)
 * 	 in case of error during init
 * 
 * — Added $.my.modal plugin to show modals, see end of this file
 * 
 * — Added style functionality to manifest, allows custom style rules  
 *   to be applied on form
 *   
 * — Supports form augmentation with external manifest cache
 *   params:{cache: function (stringRef) {... return manifest;//or null}}
 * 
...
;(function($) {var _version = "jQuery.my 0.9.4";
	...
	function _putmanifest (obj0, root0) {
		...
		if (prev=f.mask(root, id)) {
			if (prev._protected) return "Can't mount on protected";
	...
	//initializes control
		...
		function _countdown () {
			...
			//right before this moment all controls are "dead"
			$o.each(function() {
				...
				//bind events to the control
				$this.bind("recalc.my", function (evt) {
					evt.stopPropagation();
...
(function(jQuery){
	...
	root.modal = function modal (obj, done0, css0) {
		...
		if (!$m || !$(parent).find(">.my-modal").size()) {
			$m= $('<div class="my-modal" style="display:none;"><div class="my-modal-form"></div></div>').prependTo($(parent));	
		...
		// Remove commit/cancel trackers if any
		$f.unbind(".my");
		...
		if (typeof obj == "object" && obj.jquery) {
			if (obj.is("img")) {
				...
				$f.html('<img src="'+obj.attr("src")+'" style="width:'+width+'px"/>'
						+(text?'<h4>'+text+'</h4>':""));
