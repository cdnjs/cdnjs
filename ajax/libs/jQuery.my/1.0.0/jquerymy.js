 ...
 *
 * New features:
 * — require property of manifest, allows resources preload
 * — files property of manifest, container for binary resources, that obtain session URL when form starts
 * — improvements of lists of nested forms
 * 	 — new properties hash and id
 * 	 — overall speed-up
 * Optimizations:
 * — ie8 compatibility improvements
 * — no more eval()
 * 
...
;(function ($) {var _version = "jQuery.my 1.0.0";
...
(function (jQuery){
	...
	root.modal = function modal (obj, done0, css0) {
		...
		// check if opened
		if ($(parent).find(">.my-modal").is(":visible")) {
			isOpen = true;
			// No obj – close
		...
		// $.my form
			...
			width=1*($.my.f.getref(
				...
				||obj.width||width);
			$m.css({top:"3000px",display:"block",height:"none"});
			$f.data("my",null);
			show=1;
		...
		//plain html
			...
			$("body").css({overflow:"auto"});
			isOpen = false;
			return promise.reject("Cancelled").promise();
		}
		else {
			$("body").css({overflow:"auto"});
			isOpen = false;
			return promise.reject("Invalid data").promise();
		}
