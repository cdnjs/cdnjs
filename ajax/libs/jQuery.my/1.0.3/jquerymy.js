 ...
 *
 * — jQuery UI 1.11 selectmenu compatible
 * — bug fixes in modal
 * — aded 'but' shortcut for button into formgen
 *
...
;(function ($) {var _version = "jQuery.my 1.0.3";
			...
			//object is list of forms
				...
				if (n(list) && isA(list)) {
					...
					if ((now-gen.stamp>1.4*delay)
						...
						$c.each(function (idx) {
							var $x = $(this), xd, xf, chash, dirty = false;
	...
	/**/	"input":{
				"[type='text'],[type='number'],[type='search'],[type='hidden'],\
[type='password'],[type='button'],[type='range'],:not([type])":{
		...
		function _row (line, j) {
			...
			if (isS(line)) {
				...
				row = $E(true,{}, Row, {ref:j, ajax:$.extend(
					...
					Object.select(
						line,
						["accepts","async","cache","data","dataType",
							"password","timeout","type","url","username"]
					)
				)});
...
//######### PUBLIC METHODS ##############
	...
	return {
		...
		"base642blob": function(base64, done,  mime) {
			...
			done(new Blob([view], {type:mime||"application/octet-stream"}));
		},
...
(function ($){
	...
	$.fn.modal = function (obj0, done0, width0) {
		...
		m=$E({
			...
			focus:true,
			manifest:{}, data:{},
			nose: "", close:true, esc: false, enter: false,
			screen: false, width:width0||300, height:null,
			x:"0", y:"0", z:"1901", css:"",
			animate:200,
			silent:true
		}, obj, {
		...
		function _adjust (skipMeasure){
			...
			if (!m.global) $m.css({ left: m.pos.vx+"px"});
			else $m.css({
				left:"50%",
				"margin-left":"-"+((m.width+padx)/2).round(0)+"px"
			});
