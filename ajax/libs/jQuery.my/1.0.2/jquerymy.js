...
;(function ($) {var _version = "jQuery.my 1.0.2";
...
(function ($){
	...
	root.modal = function modal (obj, done0, w0) {
		...
		if (isOpen) { return $o.modal(obj, done0, w0); }
		else {
			if (!$o.size()) {
			...
			if (_convert(o, obj, ovl, w0)) return $o.modal(o);
			else {
				var pi = $.Deferred();
	...
	$.fn.modal = function (obj0, done0, width0) {
		...
		$o.data("modal", m);
		$m.css({
			position: m.global?"fixed":"absolute",
			left: m.pos.vx+"px",
			top: m.pos.vy+"px",
			display:"block",
			height:"none",
