...
/***/ function(module, exports, __webpack_require__) {
	...
	var _SmartTable = __webpack_require__(16);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ModalEdit = __webpack_require__(15);
	...
	exports.default = {
	  ...
	  computed: {
	    ...
	    processedSmartBody: function processedSmartBody() {
	      ...
	      retVal = R.filter(function (row) {
	        return R.all(function (col) {
	          return R.all(function (filter) {
	            return filter !== col || JSON.stringify(row[col]).toLowerCase().indexOf(_this3.filters[filter].model.toLowerCase()) !== -1;
	          }, R.keys(_this3.filters));
	        }, R.keys(row));
	      }, retVal);
	      ...
	      if (this.sum.length > 0) {
	        ...
	        this.totals = R.mapObjIndexed(function (v, k) {
	          if (R.contains(k, _this3.sum)) {
	            return R.reduce(addAsNumbers, 0, R.values(R.map(function (r) {
	              return r[k];
	            }, retVal))).toFixed(2);
	          } else {
	            return '';
	  ...
	  beforeCompile: function beforeCompile() {
	    ...
	    if (Array.isArray(this.canFilterBy)) {
	      this.filters = R.zipObj(this.canFilterBy, R.map(function (x) {
	  ...
	  methods: {
	    ...
	    refreshTableHeader: function refreshTableHeader() {
	      var persistArea = $('.persist-area', this.$el);
