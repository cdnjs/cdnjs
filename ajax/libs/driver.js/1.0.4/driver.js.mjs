...
function ae(e, t, o, n) {
  ...
  const s = p || o.getBoundingClientRect(), f = n.getBoundingClientRect(), w = W(e, s.x, f.x - s.x, t), i = W(e, s.y, f.y - s.y, t), h = W(e, s.width, f.width - s.width, t), r = W(e, s.height, f.height - s.height, t);
  p = {
    x: w,
    y: i,
    width: h,
    height: r
  }, G(p), b("__activeStagePosition", p);
}
function Z(e) {
  if (!e)
    return;
...
function ce(e) {
  const t = de(e);
  document.body.appendChild(t), ee(t, (o) => {
...
function de(e) {
  ...
  return p.setAttribute("d", J(e)), p.style.fill = a("overlayColor") || "rgb(0,0,0)", p.style.opacity = `${a("overlayOpacity")}`, p.style.pointerEvents = "auto", p.style.cursor = "auto", n.appendChild(p), n;
}
function J(e) {
  const t = window.innerWidth, o = window.innerHeight, n = a("stagePadding") || 0, p = a("stageRadius") || 0, s = e.width + n * 2, f = e.height + n * 2, w = Math.min(p, s / 2, f / 2), i = Math.floor(Math.max(w, 0)), h = e.x - n + i, r = e.y - n, l = s - i * 2, c = f - i * 2;
...
function he(e, t) {
  const n = Date.now(), p = d("activeStep"), s = d("activeElement") || e, f = !s || s === e, w = e.id === "driver-dummy-element", i = s.id === "driver-dummy-element", h = a("animate"), r = t.onHighlightStarted || a("onHighlightStarted"), l = (t == null ? void 0 : t.onHighlighted) || a("onHighlighted"), c = (p == null ? void 0 : p.onDeselected) || a("onDeselected"), m = a(), g = d();
  !f && c && c(i ? void 0 : s, p, {
    config: m,
    state: g
  }), r && r(w ? void 0 : e, t, {
  ...
  const P = () => {
    ...
    t.popover && y && !v && u && (Y(e, t), v = !0), a("animate") && k < 400 ? ae(k, 400, s, e) : (Z(e), l && l(w ? void 0 : e, t, {
      config: a(),
      state: d()
    }), b("__transitionCallback", void 0), b("previousStep", p), b("previousElement", s), b("activeStep", t), b("activeElement", e)), window.requestAnimationFrame(P);
  };
  b("__transitionCallback", P), window.requestAnimationFrame(P), Q(e), !u && t.popover && Y(e, t), s.classList.remove("driver-active-element"), e.classList.add("driver-active-element");
}
function fe() {
  var e;
...
function ee(e, t, o) {
  const n = (s, f) => {
    const w = s.target;
    e.contains(w) && ((!o || o(w)) && (s.preventDefault(), s.stopPropagation(), s.stopImmediatePropagation()), f == null || f(s));
  };
...
function ge() {
  window.addEventListener("keyup", U, !1), window.addEventListener("resize", M), window.addEventListener("scroll", M);
}
function we() {
  window.removeEventListener("keyup", U), window.removeEventListener("resize", M), window.removeEventListener("scroll", M);
}
function me() {
  const e = d("popover");
...
function Y(e, t) {
  ...
  const {
    ...
    progressText: r = a("progressText") || "{current} of {total}"
  } = t.popover || {};
  o.nextButton.innerHTML = i, o.previousButton.innerHTML = h, o.progress.innerHTML = r, n ? (o.title.innerText = n, o.title.style.display = "block") : o.title.style.display = "none", p ? (o.description.innerHTML = p, o.description.style.display = "block") : o.description.style.display = "none";
  const l = s || a("showButtons"), c = w || a("showProgress") || !1, m = (l == null ? void 0 : l.includes("next")) || (l == null ? void 0 : l.includes("previous")) || c;
  ...
  u.className = `driver-popover ${P}`.trim(), ee(
    o.wrapper,
    (y) => {
      var E, B, A;
...
function K(e, t) {
  const { elementDimensions: o, popoverDimensions: n, popoverPadding: p, popoverArrowDimensions: s } = t;
  return e === "start" ? Math.max(
    Math.min(
      o.top - p,
      window.innerHeight - n.realHeight - s.width
    ),
    s.width
  ) : e === "end" ? Math.max(
    Math.min(
      o.top - (n == null ? void 0 : n.realHeight) + o.height + p,
      window.innerHeight - (n == null ? void 0 : n.realHeight) - s.width
    ),
    s.width
  ) : e === "center" ? Math.max(
    Math.min(
...
function j(e, t) {
  ...
  return e === "start" ? Math.max(
    Math.min(
      o.left - p,
      window.innerWidth - n.realWidth - s.width
    ),
    s.width
  ) : e === "end" ? Math.max(
    Math.min(
      o.left - (n == null ? void 0 : n.realWidth) + o.width + p,
      window.innerWidth - (n == null ? void 0 : n.realWidth) - s.width
    ),
    s.width
  ) : e === "center" ? Math.max(
    Math.min(
...
function oe(e, t) {
  ...
  if (!o)
    return;
  const { align: n = "start", side: p = "left" } = (t == null ? void 0 : t.popover) || {}, s = n, f = e.id === "driver-dummy-element" ? "over" : p, w = a("stagePadding") || 0, i = te(), h = o.arrow.getBoundingClientRect(), r = e.getBoundingClientRect(), l = r.top - i.height;
  let c = l >= 0;
  ...
  k ? o.arrow.classList.add("driver-popover-arrow-none") : ye(s, C, e);
}
function ye(e, t, o) {
  const n = d("popover");
  if (!n)
    return;
  const p = o.getBoundingClientRect(), s = te(), f = n.arrow, w = s.width, i = window.innerWidth, h = p.width, r = p.left, l = s.height, c = window.innerHeight, m = p.top, g = p.height;
  f.className = "driver-popover-arrow";
  let u = t, v = e;
  t === "top" ? (r + h <= 0 ? (u = "right", v = "end") : r + h - w <= 0 && (u = "top", v = "start"), r >= i ? (u = "left", v = "end") : r + w >= i && (u = "top", v = "end")) : t === "bottom" ? (r + h <= 0 ? (u = "right", v = "start") : r + h - w <= 0 && (u = "bottom", v = "start"), r >= i ? (u = "left", v = "start") : r + w >= i && (u = "bottom", v = "end")) : t === "left" ? (m + g <= 0 ? (u = "bottom", v = "end") : m + g - l <= 0 && (u = "left", v = "start"), m >= c ? (u = "top", v = "end") : m + l >= c && (u = "left", v = "end")) : t === "right" && (m + g <= 0 ? (u = "bottom", v = "start") : m + g - l <= 0 && (u = "right", v = "start"), m >= c ? (u = "top", v = "start") : m + l >= c && (u = "right", v = "end")), u ? (f.classList.add(`driver-popover-arrow-side-${u}`), f.classList.add(`driver-popover-arrow-align-${v}`)) : f.classList.add("driver-popover-arrow-none");
}
function xe() {
  ...
  return h.classList.add("driver-popover-next-btn"), h.innerHTML = "Next &rarr;", w.appendChild(i), w.appendChild(h), s.appendChild(f), s.appendChild(w), e.appendChild(p), e.appendChild(t), e.appendChild(o), e.appendChild(n), e.appendChild(s), {
    wrapper: e,
    arrow: t,
    title: o,
    description: n,
...
function be(e = {}) {
  ...
  function o() {
    const r = d("activeIndex"), l = a("steps") || [];
    if (typeof r > "u")
      return;
    const c = r + 1;
    l[c] ? i(c) : h();
  ...
  function w() {
    d("isInitialized") || (b("isInitialized", !0), document.body.classList.add("driver-active", a("animate") ? "driver-fade" : "driver-simple"), ge(), _("overlayClick", t), _("escapePress", t), _("arrowLeftPress", s), _("arrowRightPress", f));
  }
  function i(r = 0) {
    var H, E, B, A, z, D, F, q;
    const l = a("steps");
    ...
    b("activeIndex", r);
    const c = l[r], m = l[r + 1], g = l[r - 1], u = ((H = c.popover) == null ? void 0 : H.doneBtnText) || a("doneBtnText") || "Done", v = a("allowClose"), P = typeof ((E = c.popover) == null ? void 0 : E.showProgress) < "u" ? (B = c.popover) == null ? void 0 : B.showProgress : a("showProgress"), k = (((A = c.popover) == null ? void 0 : A.progressText) || a("progressText") || "{{current}} of {{total}}").replace("{{current}}", `${r + 1}`).replace("{{total}}", `${l.length}`), C = ((z = c.popover) == null ? void 0 : z.showButtons) || a("showButtons"), y = [
      "next",
      "previous",
      ...v ? ["close"] : []
    ].filter((ne) => !(C != null && C.length) || C.includes(ne)), x = ((D = c.popover) == null ? void 0 : D.onNextClick) || a("onNextClick"), T = ((F = c.popover) == null ? void 0 : F.onPrevClick) || a("onPrevClick"), $ = ((q = c.popover) == null ? void 0 : q.onCloseClick) || a("onCloseClick");
    X({
      ...c,
      popover: {
        showButtons: y,
  ...
  function h(r = !0) {
    ...
    if (document.body.classList.remove("driver-active", "driver-fade", "driver-simple"), we(), Ce(), fe(), pe(), se(), V(), l && c) {
  ...
  return {
    ...
    isLastStep: () => {
      const r = a("steps") || [], l = d("activeIndex");
