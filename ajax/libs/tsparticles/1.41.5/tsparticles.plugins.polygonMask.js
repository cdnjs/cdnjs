(function webpackUniversalModuleDefinition(root, factory) {
  ...
  return function() {
    ...
    class PolygonMaskDraw {
      ...
      set lineColor(value) {
        this.stroke.color = OptionsColor_OptionsColor.create(this.stroke.color, value);
      }
      load(data) {
        var _a;
        if (!data) {
    ...
    class PolygonMask {
      ...
      load(data) {
        ...
        if (data.position !== undefined) {
          this.position = Utils_deepExtend({}, data.position);
        }
      }
    }
    function drawPolygonMask(context, rawData, stroke) {
      const color = ColorUtils_colorToRgb(stroke.color);
      if (!color) {
        return;
      }
      context.beginPath();
      ...
      context.closePath();
      context.strokeStyle = ColorUtils_getStyleFromRgb(color);
    ...
    function drawPolygonMaskPath(context, path, stroke, position) {
      ...
      context.strokeStyle = ColorUtils_getStyleFromRgb(color, stroke.opacity);
      context.lineWidth = stroke.width;
      context.stroke(path);
    }
    ...
    function calcClosestPtOnSegment(s1, s2, pos) {
      const {dx: dx, dy: dy} = NumberUtils_getDistances(pos, s1);
      const {dx: dxx, dy: dyy} = NumberUtils_getDistances(s2, s1);
    ...
    function segmentBounce(start, stop, velocity) {
      const {dx: dx, dy: dy} = NumberUtils_getDistances(start, stop);
      const wallAngle = Math.atan2(dy, dx);
      const wallNormal = Vector_Vector.create(Math.sin(wallAngle), -Math.cos(wallAngle));
    ...
    class PolygonMaskInstance {
      ...
      particlePosition(position) {
        ...
        return Utils_deepExtend({}, position ? position : this.randomPoint());
      }
      ...
      polygonBounce(particle, _delta, direction) {
        ...
        if (options.type === "inside" || options.type === "outside") {
          ...
          const dist = NumberUtils_getDistance(particle.initialPosition, particle.getPosition());
      ...
      getRandomPoint() {
        ...
        const coords = Utils_itemFromArray(this.raw);
        return {
          x: coords.x,
          y: coords.y
        };
      ...
      getRandomPointByLength() {
        ...
        const path = Utils_itemFromArray(this.paths);
    ...
    async function loadPolygonMaskPlugin(engine) {
      if (!Utils_isSsr() && !("SVGPathSeg" in window)) {
