  ...
  return function() {
    ...
    function calculateBubbleValue(particleValue, modeValue, optionsValue, ratio) {
      if (modeValue >= optionsValue) {
        ...
        return NumberUtils_clamp(value, particleValue, modeValue);
      } else if (modeValue < optionsValue) {
        const value = particleValue - (optionsValue - modeValue) * ratio;
        return NumberUtils_clamp(value, modeValue, particleValue);
      }
    }
    class Bubbler extends ExternalInteractorBase {
      ...
      isEnabled() {
        ...
        return Utils_isInArray("bubble", hoverMode) || Utils_isInArray("bubble", clickMode) || divBubble;
      }
      ...
      interact() {
        ...
        if (hoverEnabled && Utils_isInArray("bubble", hoverMode)) {
          this.hoverBubble();
        } else if (clickEnabled && Utils_isInArray("bubble", clickMode)) {
          this.clickBubble();
        } else {
      ...
      singleSelectorHover(selector, div) {
        ...
        selectors.forEach((item => {
          ...
          }, repulseRadius = elem.offsetWidth / 2 * pxRatio, area = div.type === "circle" ? new Circle_Circle(pos.x, pos.y, repulseRadius) : new Rectangle_Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), query = container.particles.quadTree.query(area);
      ...
      clickBubble() {
        ...
        for (const particle of query) {
          ...
          const pos = particle.getPosition(), distMouse = NumberUtils_getDistance(pos, mouseClickPos), timeSpent = ((new Date).getTime() - (container.interactivity.mouse.clickTime || 0)) / 1e3;
          ...
          const sizeData = {
            ...
            particlesObj: {
              optValue: NumberUtils_getRangeMax(particle.options.size.value) * container.retina.pixelRatio,
          ...
          const opacityData = {
            ...
            particlesObj: {
              optValue: NumberUtils_getRangeMax(particle.options.opacity.value),
      ...
      hoverBubble() {
        ...
        for (const particle of query) {
          particle.bubble.inRange = true;
          const pos = particle.getPosition(), pointDistance = NumberUtils_getDistance(pos, mousePos), ratio = 1 - pointDistance / distance;
      ...
      hoverBubbleSize(particle, ratio, divBubble) {
        ...
        const optSize = NumberUtils_getRangeMax(particle.options.size.value) * container.retina.pixelRatio;
      ...
      hoverBubbleOpacity(particle, ratio, divBubble) {
        ...
        const opacity = calculateBubbleValue(pOpacity, modeOpacity, NumberUtils_getRangeMax(optOpacity), ratio);
      ...
      hoverBubbleColor(particle, ratio, divBubble) {
        ...
        if (!particle.bubble.finalColor) {
          ...
          const bubbleColor = modeColor instanceof Array ? Utils_itemFromArray(modeColor) : modeColor;
          particle.bubble.finalColor = ColorUtils_colorToHsl(bubbleColor);
        }
        if (!particle.bubble.finalColor) {
          return;
        }
        if (bubbleOptions.mix) {
          ...
          particle.bubble.color = pColor ? rgbToHsl(ColorUtils_colorMix(pColor, particle.bubble.finalColor, 1 - ratio, ratio)) : particle.bubble.finalColor;
        } else {
