    ...
    class Attractor extends ExternalInteractorBase {
      ...
      interact() {
        ...
        if (mouseMoveStatus && hoverEnabled && Utils_isInArray("attract", hoverMode)) {
          this.hoverAttract();
        } else if (clickEnabled && Utils_isInArray("attract", clickMode)) {
          this.clickAttract();
        }
      }
      hoverAttract() {
      ...
      processAttract(position, attractRadius, area) {
        ...
        for (const particle of query) {
          ...
          const attractFactor = NumberUtils_clamp(calcEasing(1 - distance / attractRadius, attractOptions.easing) * velocity, 0, attractOptions.maxSpeed);
