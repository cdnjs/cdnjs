    ...
    ***************************************************************************** */
    ...
    function () {
      ...
      __proto._updateItem = function (item) {
        ...
        if (element) {
          attributes = element ? getDataAttributes(element, this.options.attributePrefix) : {};
        }
    ...
    function (_super) {
      ...
      __proto._updateItems = function (items) {
        ...
        itemRenderer.updateItems(items);

        if (this.options.isEqualSize) {
          var updatedItem = items.filter(function (item) {
            return item.updateState === UPDATE_STATE.UPDATED;
          })[0];

          if (updatedItem) {
            itemRenderer.updateItems(this.getItems().filter(function (item) {
              return item.updateState !== UPDATE_STATE.UPDATED;
            }));
          }
        }
