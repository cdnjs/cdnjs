...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function (_super) {
      ...
      __proto._getStretchItemCost = function (items, rowSize) {
        var gap = this.options.gap;
        var itemsLength = items.length;
        ...
        var distSize = containerInlineSize - sum(itemInfos.map(function (info) {
          return info.size;
        }));

        if (distSize < 0) {
          return Math.abs(distSize) * (itemsLength + 1) / (itemInfos.filter(function (info) {
            return !info.isMin;
          }).length + 1);
        } else if (distSize > 0) {
          return Math.abs(distSize) * (itemsLength + 1) / (itemInfos.filter(function (info) {
      ...
      __proto._getCost = function (items, i, j, isEndDirection) {
        ...
        if (this.stretch) {
          ...
          if (!this.passUnstretchRow || (isEndDirection ? j !== items.length : i !== 0) || expectedInlineSize >= containerInlineSize) {
            extraCost = this._getStretchItemCost(lineItems, rowSize);
          }
