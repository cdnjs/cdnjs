        ...
        groups.forEach(function (groupItems, rowIndex) {
          ...
          if (stretch && expectedInlineSize && noGapContainerInlineSize !== noGapExpectedContainerInlineSize) {
            ...
            if (passUnstretchRow && noGapExpectedContainerInlineSize < noGapContainerInlineSize && (isEndDirection ? rowIndex === groupsLength - 1 : rowIndex === 0)) {
              ...
              var itemInfoslength = itemInfos.length;

              var subInfos = __spreadArray$1([], itemInfos);

              var _loop_1 = function (i) {
                var maxRatio = Math.max.apply(Math, __spreadArray$1([1], subInfos.map(function (info) {
                  return info.minInlineSize / info.orgInlineSize;
                })));
                var minRatio = Math.min.apply(Math, __spreadArray$1([1], subInfos.map(function (info) {
                  return info.maxInlineSize / info.orgInlineSize;
                })));
                var stretchScale = 1;
                ...
                subInfos.forEach(function (info) {
                  ...
                  var nextInlineSize = between(info.orgInlineSize, info.minInlineSize, info.maxInlineSize);
                ...
                var firstInfo = subInfos[0];
                var expectedScale = noGapContainerInlineSize / noGapExpectedContainerInlineSize;
                var nextInlineSize = between(firstInfo.orgInlineSize * expectedScale, firstInfo.minInlineSize, firstInfo.maxInlineSize);
              ...
              for (var i = 0; i < itemInfoslength; ++i) {
                _loop_1(i);
              }
              ...
              if (noGapContainerInlineSize) {
                ...
                var extraInlineSize = itemInfos.reduce(function (prev, cur) {
                  return prev + cur.inlineSize;
                }, 0);
                var extraScale_1 = (containerInlineSize - allGap) / extraInlineSize;
                itemInfos.forEach(function (info) {
                  info.inlineSize *= extraScale_1;
                });
              }
