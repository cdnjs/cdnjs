...
var DEFAULT_INFINITEGRID_OPTIONS = {
...
function (_super) {
  ...
  __proto.getStatus = function () {
    return __assign(__assign({}, _super.prototype.getStatus.call(this)), {
      type: ITEM_TYPE.NORMAL,
      groupKey: this.groupKey,
      html: this.html
    });
  ...
  var _loop_1 = function (name) {
    var attributes = {
      ...
      set: function (value) {
        ...
        this.groupManager.setGridOptions((_a = {}, _a[name] = value, _a));
      }
    };
...
function (_super) {
  ...
  __proto.setGridOptions = function (options) {
  ...
  __proto.applyGrid = function (items, direction, outline) {
    ...
    var originalGroups = this.groups;
    var length = originalGroups.length;
    ...
    var groups = originalGroups.slice();
  ...
  __proto.syncItems = function (nextItemInfos) {
    ...
    var startVirtualGroups = this._getVirtualGroups("start", nextManagerGroups);

    var endVirtualGroups = this._getVirtualGroups("end", nextManagerGroups);

    nextManagerGroups = __spreadArrays(startVirtualGroups, nextManagerGroups, endVirtualGroups);
  ...
  __proto.getGroupStatus = function (type) {
    ...
    var groupStatus = groups.map(function (_a, i) {
      ...
      var items = isVirtualGroup ? [] : gridItems.map(function (item) {
        return isVirtualItems ? item.getVirtualStatus() : item.getStatus();
      });
      return {
  ...
  __proto._getGroupVisibleItems = function () {
  ...
  __proto._applyVirtualGrid = function (grid, direction, outline) {
    ...
    if (prevOutline.length !== outline.length || prevOutline.some(function (value, i) {
      return value !== outline[i];
    })) {
      return {
        start: __spreadArrays(outline),
        end: __spreadArrays(outline)
      };
    }
  ...
  __proto._getVirtualGroups = function (direction, nextGroups) {
    var groups = this.groups;
    var virtualGroups = [];
    ...
    virtualGroups = listDiffer.diff(virtualGroups, nextGroups, function (_a) {
      var groupKey = _a.groupKey;
      return groupKey;
...
function (_super) {
  ...
  __proto.sync = function (nextItems) {
    ...
    if (!isChange) {
      var prevVisibleItems = prevItems.slice(prevStartCursor, prevEndCursor);
      var nextVisibleItems = nextItems.slice(nextStartCursor, nextEndCursor);
  ...
  __proto.getRenderedVisibleItems = function () {
    var groups = this.getVisibleItems();
    var rendered = groups.map(function (group) {
      return group.startOutline.length > 0;
    });
...
function (_super) {
  ...
  function InfiniteGrid(wrapper, options) {
    ...
    _this._onRendererUpdated = function (e) {
      ...
      removed.forEach(function (index) {
        ...
        orgItem.mountState = orgItem.mountState === Grid.MOUNT_STATE.UNCHECKED ? Grid.MOUNT_STATE.UNCHECKED : Grid.MOUNT_STATE.UNMOUNTED;
      });
      var items = renderedItems.map(function (item) {
        // set grid element
      ...
      _this.itemRenderer.renderItems(items);
    ...
    _this._onRequestAppend = function (e) {
      // TODO LOADING
      _this.trigger(EVENTS.REQUEST_APPEND, {
        groupKey: e.groupKey,
    ...
    _this._onRequestPrepend = function (e) {
      // TODO LOADING
      _this.trigger(EVENTS.REQUEST_PREPEND, {
        groupKey: e.groupKey,
        nextGroupKey: e.nextGroupKey
      });
    };
    ...
    _this._onRenderComplete = function (_a) {
      ...
      _this.trigger(EVENTS.RENDER_COMPLETE, {
        isResize: isResize,
        direction: direction,
        mounted: mounted,
        updated: updated,
    ...
    _this.options = __assign(__assign(__assign({}, _this.constructor.defaultOptions), {
      renderer: new VanillaGridRenderer().on("update", function () {
    ...
    var _a = _this.options,
        ...
        threshold = _a.threshold,
        gridOptions = __rest(_a, ["gridConstructor", "container", "renderer", "threshold"]); // options.container === false, wrapper = container, scrollContainer = document.body
  ...
  __proto.setStatus = function (status) {
    ...
    setTimeout(function () {
      _this._getRenderer().updateKey();

      _this._render({
        isReisze: _this.containerManager.getInlineSize() !== prevInlineSize,
        isRestore: true
      });
    });
    return this;
  ...
  __proto._render = function (state) {
    var hasPlaceholder = this.groupManager.hasPlaceholder();
    var items = this.getVisibleItems(hasPlaceholder); // has placeHolder

    if (!hasPlaceholder) {
      items = items.filter(function (item) {
        return item.type === ITEM_TYPE.NORMAL;
      });
    }

    this._getRenderer().render(items.map(function (item) {
      return {
  ...
  __proto._syncInfinite = function () {
    this.infinite.sync(this.getGroups(true).map(function (_a) {
      var groupKey = _a.groupKey,
          grid = _a.grid;
  ...
  __proto._requestFillVirtualGroups = function () {
    var groups = this.groupManager.getGroups(true);
    var visibleGroups = this.getVisibleGroups();
    var length = visibleGroups.length;
    var isEndDirection = this.defaultDirection === "end";
    var startCursor = this.getStartCursor();
    var endCursor = this.getEndCursor();

    if (length) {
      var startGroupKey_1 = visibleGroups[0].groupKey;
      var endGroupKey_1 = visibleGroups[length - 1].groupKey;
      var startGroupIndex = findIndex(groups, function (item) {
        return item.groupKey === startGroupKey_1;
      }) - 1;
      var endGroupIndex = findIndex(groups, function (item) {
        return item.groupKey === endGroupKey_1;
      }) + 1;
      var isEnd = endGroupIndex <= endCursor;
      var isStart = startGroupIndex >= startCursor; // Fill the placeholder with the original item.

      if ((isEndDirection || !isStart) && isEnd) {
        this._onRequestAppend({
          groupKey: endGroupKey_1,
      ...
      var visiblePlaceholderGroups = this.getVisibleGroups(true);
      var placeholderLength = visiblePlaceholderGroups.length;
  ...
  InfiniteGrid.propertyTypes = Grid.GRID_PROPERTY_TYPES;
...
var modules = {
    ...
    DEFAULT_INFINITEGRID_OPTIONS: DEFAULT_INFINITEGRID_OPTIONS,
    EVENTS: EVENTS,
