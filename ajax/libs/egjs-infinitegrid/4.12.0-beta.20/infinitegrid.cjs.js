...
var ScrollManager = /*#__PURE__*/function (_super) {
  ...
  __proto.getEndScrollOffset = function () {
    return this.endScrollOffset;
  };
  ...
  __proto.setStatus = function (status) {
    ...
    this.endScrollOffset = status.endScrollOffset;
  ...
  __proto.getStatus = function () {
    return {
      ...
      endScrollOffset: this.endScrollOffset,
  ...
  __proto.resize = function () {
    ...
    var orgPos = this.getOrgScrollPos() || 0;
    var scrollOffset = orgPos + (horizontal ? containerRect.left - scrollContainerRect.left : containerRect.top - scrollContainerRect.top);
    var scrollSize = 0;
    var contentSize = 0;
    if (isBody) {
      scrollSize = horizontal ? document.documentElement.scrollWidth || document.body.scrollWidth : document.documentElement.scrollHeight || document.body.scrollHeight;
      contentSize = horizontal ? window.innerWidth : window.innerHeight;
    } else {
      scrollSize = horizontal ? scrollContainer.scrollWidth : scrollContainer.scrollHeight;
      contentSize = horizontal ? scrollContainer.offsetWidth : scrollContainer.offsetHeight;
    }
    this.scrollOffset = scrollOffset;
    this.contentSize = contentSize;
    this.endScrollOffset = Math.max(contentSize, scrollSize) - contentSize - scrollOffset;
  };
  __proto.destroy = function () {
    var container = this.container;
