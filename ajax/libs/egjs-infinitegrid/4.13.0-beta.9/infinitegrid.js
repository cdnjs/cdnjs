    ...
    ***************************************************************************** */
    ...
    function (_super) {
      ...
      __proto.applyGrid = function (items, direction, outline) {
        ...
        var _a = this.options,
            ...
            stretchOutline = _a.stretchOutline;
        ...
        var itemIndexes = range(startOutline.length).map(function () {
          return [];
        });
        ...
        var _loop_1 = function (i) {
          var itemIndex = isEndDirection ? i : itemsLength - 1 - i;
          ...
          itemIndexes[columnIndex].push(itemIndex); // stretch mode or data-grid-column > "1"
          ...
          if (columnSizeRatio === true) {
            var ratio = item.orgContentSize / item.orgInlineSize;
        ...
        if (stretchOutline && items.length) {
          ...
          if (stretchOutline === "scale-up") {
            scalePoint_1 = Math.max.apply(Math, endOutline);
          } else if (stretchOutline === "scale-center") {
            scalePoint_1 = (Math.max.apply(Math, endOutline) + Math.min.apply(Math, endOutline)) / 2;
          } else {
            // scale-down
            scalePoint_1 = Math.min.apply(Math, endOutline);
          }

          if (isEndDirection) {
            scalePoint_1 -= contentGap;
          }

          endOutline.forEach(function (point, i) {
            var totalGap = (itemIndexes[i].length - 1) * contentGap;
            var startPoint = startOutline[i];
            var scale = (Math.abs(scalePoint_1 - startPoint) - totalGap) / (Math.abs(point - startPoint) - (isEndDirection ? contentGap : 0) - totalGap);
            ...
            if (isEndDirection) {
              endOutline[i] = scalePoint_1 + contentGap;
            } else {
              endOutline[i] = scalePoint_1;
            }

            var length = itemIndexes[i].length;
            var prevPoint = isEndDirection ? startOutline[i] : startOutline[i] - contentGap;
            itemIndexes[i].forEach(function (itemIndex, j) {
              var item = items[itemIndex];
              var prevSize = item.computedContentSize;
              var nextSize = prevSize * scale;
              item.addCSSGridRect({
              ...
              if (j === length - 1) {
                ...
                if (isEndDirection) {
                  sizeOffset = (item.cssContentPos % 1 >= 0.5 ? 1 : 0) + (item.cssContentSize % 1 >= 0.5 ? 1 : 0) + (scalePoint_1 % 1 >= 0.5 ? -1 : 0);
                } else {
                  posOffset = (item.cssContentPos % 1 < 0.5 ? -1 : 0) + (scalePoint_1 % 1 < 0.5 ? 1 : 0);
      ...
      __proto.getComputedOutlineLength = function (items) {
        ...
        if (columnOption) {
          ...
          var columnSize = this.getComputedOutlineSize(items);
          column = Math.max(1, Math.floor((this.getContainerInlineSize() + inlineGap) / (columnSize - columnCalculationThreshold + inlineGap)));

          if (!this.maxStretchColumnSize) {
            column = Math.min(items.length, column);
          }
        }
    ...
    function (_super) {
      ...
      __proto.applyGrid = function (items, direction, outline) {
        ...
        items.forEach(function (item) {
          ...
          var contentOffset = parseFloat(attributes.contentOffset); // let contentMargin = parseFloat(attributes.contentMargin);
          ...
          gridData.contentOffset = contentOffset; // gridData.contentMargin = contentMargin;
        });
      ...
      __proto._getExpectedRowSize = function (items, forceStretch) {
        ...
        items.forEach(function (item) {
          ...
          var inlineOffset = item.gridData.inlineOffset || 0;
          var contentOffset = item.gridData.contentOffset || 0; // const contentMargin = item.gridData.contentMargin || 0;

          var maintainedRatio = contentSize <= contentOffset ? 1 : (inlineSize - inlineOffset) / (contentSize - contentOffset);
          ratioSum += maintainedRatio; // inlineSum += (contentOffset + contentMargin) * maintainedRatio;
      ...
      __proto._getStretchItemInfos = function (items, rowSize) {
        ...
        var _loop_1 = function (i) {
          ...
          var distScale = (containerInlineSize - passedItemsSize) / restItemsSize; // minimize or maximize
          ...
          costInfos.forEach(function (info) {
            if (!info.passed) {
              ...
              if (isIncrease && !throttle(info.size - info.maxSize, 0.001) || !isIncrease && !throttle(info.size - info.minSize, 0.001)) {
                info.passed = true;
              }
            }
          });
      ...
      __proto._setStyle = function (items, path, outline, isEndDirection) {
        ...
        var _a = this.options,
            ...
            stretch = _a.stretch,
            passUnstretchRow = _a.passUnstretchRow;
        var itemsLength = items.length;
        ...
        groups.forEach(function (groupItems, rowIndex) {
          ...
          var allGap = inlineGap * (groupItemslength - 1);
          ...
          if (stretch && expectedInlineSize) {
            ...
            var passed = passUnstretchRow && noGapExpectedContainerInlineSize < noGapContainerInlineSize && (isEndDirection ? rowIndex === groupsLength - 1 : rowIndex === 0);
    ...
    function (_super) {
      ...
      __proto.applyGrid = function (items, direction, outline) {
        ...
        var _a = this.getRectSize(frameInlineSize),
            rectInlineSize = _a.inlineSize,
            rectContentSize = _a.contentSize;
        ...
        for (var startIndex = 0; startIndex < itemsLength; startIndex += rectsLength) {
          ...
          for (var rectIndex = 0; rectIndex < rectsLength && startIndex + rectIndex < itemsLength; ++rectIndex) {
            ...
            var _b = frameRects[rectIndex],
                frameRectContentPos = _b.contentPos,
                frameRectInlinePos = _b.inlinePos,
                frameRectContentSize = _b.contentSize,
                frameRectInlineSize = _b.inlineSize;
      ...
      var _loop_1 = function (name) {
        var attributes = {
          ...
          set: function set(value) {
            var setterName = "_".concat(camelize$1("set ".concat(name)));
            if (this[setterName]) {
