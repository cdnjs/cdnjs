...
function splitVirtualGroups(groups, direction, nextGroups) {
  ...
  var nextVirtualGroups = listDiffer.diff(virtualGroups, nextGroups, function (_a) {
    var groupKey = _a.groupKey;
    return groupKey;
...
var withInfiniteGridMethods = Grid.withMethods(INFINITEGRID_METHODS);
...
function (_super) {
  ...
  __proto.applyGrid = function (items, direction, outline) {
    ...
    items.forEach(function (item) {
      item.mountState = Grid.MOUNT_STATE.MOUNTED;
    });
    ...
    groups.forEach(function (group) {
      ...
      var appliedItems = gridItems.filter(function (item) {
        return item.mountState !== Grid.MOUNT_STATE.UNCHECKED && item.rect.width;
      });
      var gridOutlines;
...
function (_super) {
  ...
  __proto.syncItems = function (nextItems) {
    var prevItems = this.items;
    this.setItems(nextItems);
    var result = listDiffer.diff(prevItems, this.items, function (item) {
...
function (_super) {
  ...
  function InfiniteGrid(wrapper, options) {
    ...
    _this._onScroll = function (_a) {
      ...
      _this.trigger(new Component.ComponentEvent(INFINITEGRID_EVENTS.SCROLL, {
        direction: direction,
        scrollPos: scrollPos,
