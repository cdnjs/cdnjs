...
(function(){function d(a,b){this.hooks=a;var d=Math.floor(Math.random()*1E3)+"/",e;e=[];for(var f=0;f<8;f++)e.push(Math.floor(Math.random()*32).toString(32));e=e.join("");this.session=d+e;d=/([^\?]*)\/*(\??.*)/.exec(b);this.location={base:d[1],queryString:d[2]};this.readyState=c;this.openStream()}function a(a){var b=a.indexOf("?")===-1?"?":"&";return a+b+"t="+ +new Date+"&n="+g++}function b(a,b){if(Pusher.Util.isXHRSupported())return Pusher.HTTP.getXHR(a,b);else if(Pusher.Util.isXDRSupported(b.indexOf("https:")===
0))return Pusher.HTTP.getXDR(a,b);else throw"Cross-origin HTTP requests are not supported";}var c=0,g=1,e=d.prototype;e.send=function(a){return this.sendRaw(JSON.stringify([a]))};e.ping=function(){this.hooks.sendHeartbeat(this)};e.close=function(a,b){this.onClose(a,b,!0)};e.sendRaw=function(h){if(this.readyState===1)try{return b("POST",a(this.location.base+"/"+this.session+"/xhr_send")).start(h),!0}catch(c){return!1}else return!1};e.reconnect=function(){this.closeStream();this.openStream()};e.onClose=
