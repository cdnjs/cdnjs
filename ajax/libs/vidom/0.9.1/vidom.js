...
function getAttrName(attrName) {
    return attrName in ATTR_NAMES ? ATTR_NAMES[attrName] : ATTR_NAMES[attrName] = attrName.toLowerCase();
}
...
function patchComponent(nextAttrs, nextChildren, nextContext, callReceivers) {
    ...
    var shouldRerender = this.shouldRerender(prevAttrs, prevChildren, this.__prevState, prevContext);

    if (IS_DEBUG) {
        var shouldRerenderResType = typeof shouldRerender;

        if (shouldRerenderResType !== 'boolean') {
            consoleWrapper.warn('Component#shouldRerender() should return boolean instead of ' + shouldRerenderResType);
        }
    }
...
ComponentNode.prototype = {
    ...
    _getInstance: function () {
        return this._instance === null ? this._instance = new this._component(this.attrs, this.children, this._ctx) : this._instance;
    }
};
...
var SHORT_TAGS = new SimpleMap$1([['area', true], ['base', true], ['br', true], ['col', true], ['command', true], ['embed', true], ['hr', true], ['img', true], ['input', true], ['keygen', true], ['link', true], ['menuitem', true], ['meta', true], ['param', true], ['source', true], ['track', true], ['wbr', true]]);
...
function processChildren(children) {
    ...
    return typeOfChildren === 'string' ? children : '' + children;
}
