...
function mountToDom(domNode, tree, ctx, cb) {
    if (IS_DEBUG) {
...
function unmountFromDom(domNode, cb) {
    if (IS_DEBUG) {
...
function getComponentDomRef(ref) {
    return this.__domRefs[ref] ? this.__domRefs[ref].getDomNode() : null;
}
...
var Input = createComponent({
    onInit: function () {
        ...
        this.onInput = function (e) {
            var _getAttrs = _this.getAttrs();

            var onInput = _getAttrs.onInput;
            var onChange = _getAttrs.onChange;
            ...
            if (_this.isMounted()) {
                ...
                var _getAttrs2 = _this.getAttrs();

                var value = _getAttrs2.value; // attrs could be changed during applyBatch()
        ...
        this.onChange = function (e) {
            var _getAttrs3 = _this.getAttrs();

            var onChange = _getAttrs3.onChange;
            var control = _this.getDomRef('control');
            ...
            if (_this.isMounted()) {
                var _getAttrs4 = _this.getAttrs();

                var name = _getAttrs4.name;
                var type = _getAttrs4.type;
                var checked = _getAttrs4.checked; // attrs could be changed during applyBatch()
                    ...
                    if (type === 'radio' && name) {
                        ...
                        while (i < len) {
                            ...
                            if (typeof _checked !== 'undefined') {
                                var radioControl = radioInput.getDomRef('control');
    ...
    onUnmount: function () {
        var _getAttrs5 = this.getAttrs();

        var type = _getAttrs5.type;
        var name = _getAttrs5.name;
    ...
    onInit: function () {
        ...
        this.onInput = function (e) {
            ...
            if (_this.isMounted()) {
                ...
                var value = _getAttrs.value; // attrs could be changed during applyBatch()
