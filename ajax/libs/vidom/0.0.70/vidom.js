...
function setObjProp(node, name, val) {
    if ("production" !== 'production') {
        ...
        if (typeOfVal !== 'object') {
            _console2.default.error('Error! "' + name + '" attribute expects an object as a value, not a ' + typeOfVal);
            return;
        }
    }
...
function patchComponent(attrs, children, parentNode) {
    ...
    if ("production" !== 'production') {
        ...
        if (shouldUpdateResType !== 'boolean') {
            _console2.default.warn('Warning! Component#shouldUpdate() should return boolean instead of ' + shouldUpdateResType);
        }
    }
...
function renderComponent() {
    ...
    if ("production" !== 'production') {
        if ((typeof renderRes === 'undefined' ? 'undefined' : _typeof(renderRes)) !== 'object' || Array.isArray(renderRes)) {
            _console2.default.error('Error! Component#onRender must return a single node object on the top level');
        }
    }
...
function createNode(type) {
    switch (typeof type === 'undefined' ? 'undefined' : _typeof(type)) {
        ...
        case 'function':
            return new _ComponentNode2.default(type);

        default:
            throw Error('unsupported node type: ' + (typeof type === 'undefined' ? 'undefined' : _typeof(type)));
    }
}
                    ...
                    if (value != null) {
                        if (name === 'value') {
                            switch (tag) {
                                ...
                                case 'option':
                                    if (ctx.multiple ? (0, _isInArray2.default)(ctx.value, value) : ctx.value === value) {
...
function checkChildren(children) {
    ...
    while (i < len) {
        ...
        if ((typeof child === 'undefined' ? 'undefined' : _typeof(child)) !== 'object') {
            _console2.default.error('Error! You mustn\'t use simple child in case of multiple children.');
        }
        ...
        //}
                ...
                if (child._key in keys) {
                    _console2.default.error('Error! Childrens\' keys must be unique across the children');
                } else {
...
function checkAttrs(attrs) {
    for (var name in attrs) {
        if (name.substr(0, 2) === 'on' && !_attrsToEvents2.default[name]) {
            _console2.default.error('Error! You\'re trying to add unsupported event listener "' + name + '"');
        }
    }
}
...
function normalizeChildren(children) {
    ...
    while (i < len) {
        ...
        if (Array.isArray(child)) {
            ...
            var typeOfChild = typeof child === 'undefined' ? 'undefined' : _typeof(child);
