...
import { applyTransform, getBottomLeft, getBottomRight, getTopLeft, getTopRight, getWidth, } from './extent.js';
 ...
 */
export function createSafeCoordinateTransform(sourceProj, destProj, forward, inverse) {
    return function (coord) {
        ...
        var transformed = forward(x === undefined ? coord : [x, coord[1]]);
        var destExtent = destProj.getExtent();
        if (!isFinite(transformed[0]) || !isFinite(transformed[1])) {
            // Try to recover from out-of-bounds transform
            if (destExtent) {
                var corner1 = inverse(getBottomLeft(destExtent));
                var corner2 = inverse(getBottomRight(destExtent));
                var corner3 = inverse(getTopLeft(destExtent));
                var corner4 = inverse(getTopRight(destExtent));
