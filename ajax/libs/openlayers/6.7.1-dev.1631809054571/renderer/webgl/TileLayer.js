...
var WebGLTileLayerRenderer = /** @class */ (function (_super) {
    ...
    WebGLTileLayerRenderer.prototype.renderFrame = function (frameState) {
        ...
        var layerState = frameState.layerStatesArray[frameState.layerIndex];
        var extent = getRenderExtent(frameState);
        var tileLayer = this.getLayer();
        ...
        for (var j = 0, jj = zs.length; j < jj; ++j) {
            var tileZ = zs[j];
            var tileResolution_1 = tileGrid.getResolution(tileZ);
            var tileSize = toSize(tileGrid.getTileSize(tileZ), this.tempSize_);
            var tileOrigin = tileGrid.getOrigin(tileZ);
            var centerI = (centerX - tileOrigin[0]) / (tileSize[0] * tileResolution_1);
            var centerJ = (tileOrigin[1] - centerY) / (tileSize[1] * tileResolution_1);
            var tileScale = viewState.resolution / tileResolution_1;
