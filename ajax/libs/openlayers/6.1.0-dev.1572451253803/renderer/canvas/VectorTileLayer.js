...
var CanvasVectorTileLayerRenderer = /** @class */ (function (_super) {
    ...
    CanvasVectorTileLayerRenderer.prototype.getFeatures = function (pixel) {
        return new Promise(function (resolve, reject) {
            ...
            for (var i = 0, ii = this.renderedTiles.length; i < ii; ++i) {
                if (tileCoord.toString() === this.renderedTiles[i].tileCoord.toString()) {
                    ...
                    if (tile.getState() === TileState.LOADED && tile.hifi) {
                        ...
                        var sourceTiles = source.getSourceTiles(pixelRatio, projection, tile);
                        var sourceTileCoord_1 = sourceTileGrid.getTileCoordForCoordAndResolution(coordinate, resolution);
                        for (var j = 0, jj = sourceTiles.length; j < jj; ++j) {
                            if (sourceTileCoord_1.toString() === sourceTiles[j].tileCoord.toString()) {
            ...
            var sourceTileCoord = sourceTile.tileCoord;
            var corner = getTopLeft(tileGrid.getTileCoordExtent(sourceTileCoord));
            var renderScale = sourceTileGrid.getResolution(sourceTileCoord[0]) / resolution;
            var tilePixel = [
            ...
            if (!sourceTile.hitDetectionImageData) {
                ...
                var transforms_1 = [
                    this.getRenderTransform(tileGrid.getTileCoordCenter(sourceTile.tileCoord), resolution * renderScale, 0, 0.5, size[0], size[1], 0)
                ];
