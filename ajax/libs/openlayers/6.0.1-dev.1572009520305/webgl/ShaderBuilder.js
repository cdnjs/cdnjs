...
export function formatColor(color) {
    ...
    return formatArray(array.map(function (c, i) {
        return i < 3 ? c / 255 : c;
    }));
}
/**
...
function getValueType(value) {
    ...
    if (value.length === 3 || value.length === 4) {
        var onlyNumbers = value.every(function (v) {
...
var ShaderBuilder = /** @class */ (function () {
     ...
     */
    ShaderBuilder.prototype.getSymbolVertexShader = function () {
     ...
     * @returns {string} The full shader as a string.
     */
    ShaderBuilder.prototype.getSymbolFragmentShader = function () {
        return "precision mediump float;\nuniform float u_time;\n" + this.uniforms.map(function (uniform) {
            return 'uniform ' + uniform + ';';
        }).join('\n') + "\nvarying vec2 v_texCoord;\nvarying vec2 v_quadCoord;\n" + this.varyings.map(function (varying) {
            return 'varying ' + varying.type + ' ' + varying.name + ';';
...
export function parseLiteralStyle(style) {
    ...
    var size = Array.isArray(symbStyle.size) && typeof symbStyle.size[0] == 'number' ?
    ...
    var parsedColor = pFrag(color, ValueTypes.COLOR);
    var builder = new ShaderBuilder()
        .setSizeExpression("vec2(" + pVert(size[0]) + ", " + pVert(size[1]) + ")")
        .setSymbolOffsetExpression("vec2(" + pVert(offset[0]) + ", " + pVert(offset[1]) + ")")
        .setTextureCoordinateExpression("vec4(" + pVert(texCoord[0]) + ", " + pVert(texCoord[1]) + ", " + pVert(texCoord[2]) + ", " + pVert(texCoord[3]) + ")")
        .setSymbolRotateWithView(!!symbStyle.rotateWithView)
        .setColorExpression("vec4(" + parsedColor + ".rgb, " + parsedColor + ".a * " + pFrag(opacity) + " * " + opacityFilter + ")");
    if (style.filter) {
