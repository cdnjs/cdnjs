    ...
    WebGLTileLayerRenderer.prototype.renderFrame = function (frameState) {
        ...
        for (var j = 0, jj = zs.length; j < jj; ++j) {
            ...
            for (var i = 0, ii = tileTextures_1.length; i < ii; ++i) {
                ...
                composeTransform(this.tileTransform_, 0, 0, 2 / ((frameState.size[0] * tileScale) / (tileSize[0] + 2 * gutter)), -2 / ((frameState.size[1] * tileScale) / (tileSize[1] + 2 * gutter)), viewState.rotation, ((tileCenterI - centerI - gutter / tileSize[0]) * tileSize[0]) /
                    (tileSize[0] + 2 * gutter), ((tileCenterJ - centerJ - gutter / tileSize[1]) * tileSize[1]) /
                    (tileSize[1] + 2 * gutter));
                ...
                this.helper.setUniformFloatValue(Uniforms.TEXTURE_PIXEL_WIDTH, tileSize[0] + 2 * gutter);
                this.helper.setUniformFloatValue(Uniforms.TEXTURE_PIXEL_HEIGHT, tileSize[1] + 2 * gutter);
