            ...
            do {
                ...
                sourceTileGrid.forEachTileCoord(extent, loadedZ, function (sourceTileCoord) {
                    ...
                    if (sourceTile.getState() !== TileState.EMPTY && tile.getState() === TileState.IDLE) {
                        ...
                        var onSourceTileChange_1 = function () {
                            var state = sourceTile.getState();
                            var sourceTileKey = sourceTile.getKey();
                            if (state === TileState.LOADED || state === TileState.ERROR) {
                                if (state === TileState.LOADED) {
                                    sourceTile.removeEventListener(EventType.CHANGE, onSourceTileChange_1);
                                    tile.loadingSourceTiles--;
                                    // eslint-disable-next-line no-use-before-define
                                    tile.removeEventListener(EventType.CHANGE, onTileChange_1);
                        ...
                        var onTileChange_1 = function () {
                            if (tile.getState() === TileState.ABORT) {
                                sourceTile.removeEventListener(EventType.CHANGE, onSourceTileChange_1);
                                tile.removeEventListener(EventType.CHANGE, onTileChange_1);
                            }
                        };
                        sourceTile.addEventListener(EventType.CHANGE, onSourceTileChange_1);
                        tile.addEventListener(EventType.CHANGE, onTileChange_1);
                    }
                }.bind(this));
                if (!covered) {
