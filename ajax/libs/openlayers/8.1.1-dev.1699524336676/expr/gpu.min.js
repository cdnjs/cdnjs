...
${n.join("\n")}
  return false;
}`,o+`(${e})`}),[Ops.Array]:createCompiler(e=>`vec${e.length}(${e.join(", ")})`),[Ops.Color]:createCompiler(e=>{const r=e.slice(0,3).map(e=>e+" / 255.0");return 3===e.length?`vec4(${r.join(", ")}, 1.0)`:`(${e[3]} * vec4(${r.join(", ")}, 1.0))`}),[Ops.Band]:createCompiler(([e,r,t],o)=>{if(!(GET_BAND_VALUE_FUNC in o.functions)){let t="";var n=o.bandCount||1;for(let r=0;r<n;r++){var a=Math.floor(r/4);let e=r%4;r===n-1&&1===e&&(e=3);a=Uniforms.TILE_TEXTURE_ARRAY+`[${a}]`;t+=`  if (band == ${r+1}.0) {
