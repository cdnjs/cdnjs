...
var MapboxVectorLayer = /** @class */ (function (_super) {
    ...
    function MapboxVectorLayer(options) {
        ...
        if (options.accessToken) {
            ...
            var url = new URL(options.styleUrl, location.href);
    ...
    MapboxVectorLayer.prototype.onStyleLoad = function (style) {
        ...
        if (styleSource.url.indexOf('mapbox://') === 0 ||
            styleSource.url.indexOf('{z}') !== -1) {
            // Tile source url, handle it directly
            source.setUrl(normalizeSourceUrl(styleSource.url, this.accessToken, this.accessTokenParam_));
            ...
            if (styleSource.tiles) {
                styleSource.tiles = styleSource.tiles.map(function (url) {
                    return normalizeSourceUrl(url, _this.accessToken, _this.accessTokenParam_);
                });
            }
            setupVectorSource(styleSource, normalizeSourceUrl(styleSource.url, this.accessToken, this.accessTokenParam_)).then(function (source) {
