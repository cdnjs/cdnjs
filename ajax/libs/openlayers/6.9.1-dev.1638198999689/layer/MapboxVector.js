...
var MapboxVectorLayer = /** @class */ (function (_super) {
    ...
    MapboxVectorLayer.prototype.onStyleLoad = function (style) {
        ...
        if (styleSource.url && styleSource.url.indexOf('mapbox://') === 0) {
            ...
            setupVectorSource(styleSource, styleSource.url
                ? normalizeSourceUrl(styleSource.url, this.accessToken, this.accessTokenParam_)
                : undefined).then(function (source) {
    ...
    MapboxVectorLayer.prototype.configureSource = function (source, style) {
        ...
        var background = style.layers.find(function (layer) { return layer.type === 'background'; });
        if (background &&
            (!background.layout || background.layout.visibility !== 'none')) {
            var style_1 = new Style({
                fill: new Fill(),
            });
            targetSource.addEventListener(TileEventType.TILELOADEND, function (event) {
                var tile = /** @type {import("../VectorTile.js").default} */ (
                /** @type {import("../source/Tile.js").TileSourceEvent} */ (event)
                    .tile);
                var styleFuntion = function () {
                    var opacity = 
                    /** @type {number} */ (getValue(background, 'paint', 'background-opacity', tile.tileCoord[0])) || 1;
                    var color = /** @type {*} */ (getValue(background, 'paint', 'background-color', tile.tileCoord[0]));
                    style_1
                        .getFill()
                        .setColor([
                        color.r * 255,
                ...
                var extentGeometry = fromExtent(targetSource.tileGrid.getTileCoordExtent(tile.tileCoord));
                var renderFeature = new RenderFeature(GeometryType.POLYGON, extentGeometry.getFlatCoordinates(), extentGeometry.getEnds(), { layer: background.id }, undefined);
