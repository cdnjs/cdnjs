...
export function parseSymbolStyle(style) {
    var size = Array.isArray(style.size) && typeof style.size[0] == 'number' ?
        style.size : [style.size, style.size];
    var color = (typeof style.color === 'string' ?
        asArray(style.color).map(function (c, i) {
            return i < 3 ? c / 255 : c;
        }) :
        style.color || [255, 255, 255, 1]);
    var texCoord = style.textureCoord || [0, 0, 1, 1];
