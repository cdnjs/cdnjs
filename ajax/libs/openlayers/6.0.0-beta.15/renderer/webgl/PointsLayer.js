...
import WebGLLayerRenderer, { colorDecodeId, colorEncodeId, getBlankImageData, POINT_INSTRUCTIONS_COUNT, POINT_VERTEX_STRIDE, WebGLWorkerMessageType, writePointFeatureInstructions } from './Layer.js';
...
var WebGLPointsLayerRenderer = /** @class */ (function (_super) {
    ...
    function WebGLPointsLayerRenderer(vectorLayer, opt_options) {
         ...
         * @private
         */
        _this.hitRenderTarget_ = new WebGLRenderTarget(_this.helper);
    ...
    WebGLPointsLayerRenderer.prototype.rebuildBuffers_ = function (frameState) {
        ...
        for (var i = 0; i < features.length; i++) {
            ...
            writePointFeatureInstructions(this.renderInstructions_, elementIndex, tmpCoords[0], tmpCoords[1], u0, v0, u1, v1, size, opacity, rotateWithView, color);
            // for hit detection, the feature uid is saved in the opacity value
            // and the index of the opacity value is encoded in the color values
            elementIndex = writePointFeatureInstructions(this.hitRenderInstructions_, elementIndex, tmpCoords[0], tmpCoords[1], u0, v0, u1, v1, size, opacity > 0 ? Number(getUid(feature)) : 0, rotateWithView, colorEncodeId(elementIndex + 7, tmpColor));
        }
