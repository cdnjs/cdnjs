...
case "max-aspect-ratio":var el_width=EQCSS.data[i].conditions[k].value.split("/")[0],el_height=EQCSS.data[i].conditions[k].value.split("/")[1];if(!(elements[j].offsetWidth/elements[j].offsetHeight<=el_width/el_height)){test=false;break test_conditions}break}}if(test===true){css_code=EQCSS.data[i].style;css_code=css_code.replace(/eval\( *((".*?")|('.*?')) *\)/g,function(string,match){var $it=elements[j];var ret="";try{with($it)ret=eval(match.slice(1,-1))}catch(e){ret=""}return ret});css_code=css_code.replace(/(\$|eq_)this/g,
"["+element_guid+"]");css_code=css_code.replace(/(\$|eq_)parent/g,"["+element_guid_parent+"]");css_code=css_code.replace(/(\$|eq_)prev/g,"["+element_guid_prev+"]");css_code=css_code.replace(/(\$|eq_)next/g,"["+element_guid_next+"]");css_code=css_code.replace(/(\$|eq_)root/g,"html");try{css_block.innerHTML=css_code}catch(e){css_block.styleSheet.cssText=css_code}}else try{css_block.innerHTML=""}catch(e$0){css_block.styleSheet.cssText=""}}}};
