...
var render = /* @__PURE__ */ __name(async (data4Layout, svg, {
  ...
  const addVertex = /* @__PURE__ */ __name(async (nodeEl2, graph, nodeArr, node) => {
    ...
    if (!node.isGroup) {
      const childNodeEl = await insertNode(nodeEl2, node, node.dir);
    ...
    (edge) => {
      ...
      if (edge.sections) {
        ...
        if (endNode.shape === "diamond") {
          ...
          if (Math.abs(edge.points[edge.points.length - 1].y - endNode.y - offset.y) > 1e-3 || Math.abs(edge.points[edge.points.length - 1].x - x) > 1e-3) {
            edge.points.push({
