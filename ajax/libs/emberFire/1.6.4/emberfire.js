...
exports['default'] = _emberData2['default'].Adapter.extend(_mixinsWaitable2['default'], {
  ...
  findRecord: function findRecord(store, typeClass, id) {
    ...
    return this._fetch(ref, log).then(function (snapshot) {
      ...
      _this._updateRecordCacheForType(typeClass, payload);
      if (payload === null) {
  ...
  _fetch: function _fetch(ref, log) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      ref.once('value', function (snapshot) {
        ...
        _ember2['default'].run(null, resolve, snapshot);
      }, function (err) {
        this._decrementWaiters();
        _ember2['default'].run(null, reject, err);
      });
    }, log);
  },
    ...
    record.eachRelationship(function (key, relationship) {
      if (relationship.kind === 'belongsTo') {
        ...
        if (inverseKey && parentRecord.get('id')) {
          ...
          _this3._removeHasManyRecord(store, parentRef, inverseKey.name, record.id);
        }
      }
    });
  },
  ...
  query: function query(store, typeClass, _query, recordArray) {
    ...
    return this._fetch(ref, log).then(function (snapshot) {
      ...
      snapshot.forEach(function (childSnapshot) {
        ...
        _this6._updateRecordCacheForType(typeClass, payload);
        results.push(payload);
      });
      return results;
    });
  ...
  updateRecord: function updateRecord(store, typeClass, snapshot) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      snapshot.record.eachRelationship(function (key, relationship) {
        ...
        if (relationship.kind === 'hasMany') {
          if (serializedRecord[key]) {
            save = _this8._saveHasManyRelationship(store, typeClass, relationship, serializedRecord[key], recordRef, recordCache);
          ...
          if (_this8.isRelationshipEmbedded(store, typeClass.modelName, relationship) && serializedRecord[key]) {
            save = _this8._saveEmbeddedBelongsToRecord(store, typeClass, relationship, serializedRecord[key], recordRef);
            savedRelationships.push(save);
            delete serializedRecord[key];
          }
        }
      ...
      var recordPromise = _this8._updateRecord(recordRef, serializedRecord);
  ...
  _saveHasManyRelationship: function _saveHasManyRelationship(store, typeClass, relationship, ids, recordRef, recordCache) {
    ...
    removedRecords = (0, _lodashCollectionMap2['default'])(removedRecords, function (id) {
      return _this9._removeHasManyRecord(store, recordRef, relationship.key, id);
    });
...
},{"../mixins/waitable":3,"../utils/to-promise":5,"lodash/array/indexOf":6,"lodash/collection/filter":8,"lodash/collection/find":9,"lodash/collection/forEach":10,"lodash/collection/includes":11,"lodash/collection/map":12}],2:[function(require,module,exports){
(function (global){
'use strict';
