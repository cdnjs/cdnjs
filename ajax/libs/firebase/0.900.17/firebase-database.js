(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function viewGetCompleteServerCache(view, path) {
        var cache = view.viewCache_.getCompleteServerSnap();
        if (cache) {
    ...
    function viewApplyOperation(view, operation, writesCache, completeServerCache) {
        if (operation.type === OperationType.MERGE &&
            ...
            assert(view.viewCache_.getCompleteServerSnap(), 'We should always have a full cache before handling merges');
            assert(view.viewCache_.getCompleteEventSnap(), 'Missing event cache, even though we have a server cache');
        }
        var oldViewCache = view.viewCache_;
        var result = view.processor_.applyOperation(oldViewCache, operation, writesCache, completeServerCache);
        ...
        return viewGenerateEventsForChanges_(view, result.changes, result.viewCache.getEventCache().getNode(), null);
    }
    function viewGetInitialEvents(view, registration) {
        var eventSnap = view.viewCache_.getEventCache();
        var initialChanges = [];
