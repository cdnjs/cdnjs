import { p as St, f as $ } from "./flowDb-116e16bb.js";
import { h as S, f as tt, G as Lt } from "./graph-940f1512.js";
import { j as x, n as P, o as z, p as et, c as J, f as rt, l as R, q as V, r as Et } from "./mermaid-a09fe7cd.js";
import { u as _t, r as Tt, p as Nt, l as At, d as M } from "./layout-5e46170e.js";
import { a as N, b as at, i as nt, c as _, e as st, d as it, s as Ct, f as It, g as Bt } from "./styles-7e0c8b80.js";
import { l as Mt } from "./line-0f31738a.js";
...
function Zt(r, e) {
  ...
  return t.append("path").attr("class", "path").attr("d", function(n) {
    var a = e.edge(n), s = e.node(n.v).elem, i = Tt(a.points.length).map(function() {
      return Kt(s);
    });
    return ot(a, i);
  }), t.append("defs"), t;
...
var q = function(r, e, t) {
  ...
  a.exit().remove(), a.enter().append("g").attr("class", "node").style("opacity", 0), a = r.selectAll("g.node"), a.each(function(i) {
    ...
    var d = c.append("g").attr("class", "label"), l = K(d, o), v = t[o.shape], h = Nt(l.node().getBBox(), "width", "height");
...
`), i.style === void 0 && (l.style = l.style || "stroke: #333; stroke-width: 1.5px;fill:none"), l.labelStyle = l.labelStyle.replace("color:", "fill:"))), l.id = o, l.class = c + " " + d, l.minlen = i.length || 1, e.setEdge(t.db.lookUpDomId(i.start), t.db.lookUpDomId(i.end), l, n);
  ...
  d === void 0 && (d = "TD");
  const l = s.nodeSpacing || 50, v = s.rankSpacing || 50, h = new Lt({
    multigraph: !0,
    compound: !0
  }).setGraph({
    rankdir: d,
  ...
  for (let m = p.length - 1; m >= 0; m--)
    u = p[m], n.db.addVertex(u.id, u.title, "group", void 0, u.classes);
  const g = n.db.getVertices();
  R.warn("Get vertices", g);
  const f = n.db.getEdges();
  let y = 0;
  for (y = p.length - 1; y >= 0; y--) {
    u = p[y], Ct("cluster").append("text");
    for (let m = 0; m < u.nodes.length; m++)
      R.warn(
        "Setting subgraph",
        u.nodes[m],
        n.db.lookUpDomId(u.nodes[m]),
        n.db.lookUpDomId(u.id)
      ), h.setParent(n.db.lookUpDomId(u.nodes[m]), n.db.lookUpDomId(u.id));
  }
  bt(g, h, e, o, c, n), kt(f, h, n);
  const k = new de();
  ge.addToRender(k), k.arrows().none = function(b, L, w, B) {
  ...
  for (k(E, h), E.selectAll("g.node").attr("title", function() {
    ...
    if (u = p[y], u.title !== "undefined") {
      ...
      ), L = m[0].x.baseVal.value, w = m[0].y.baseVal.value, B = m[0].width.baseVal.value, C = x(b[0]).select(".label");
      C.attr("transform", `translate(${L + B / 2}, ${w + 14})`), C.attr("id", e + "Text");
      for (let W = 0; W < u.classes.length; W++)
        b[0].classList.add(u.classes[W]);
    }
  if (!s.htmlLabels) {
    const m = c.querySelectorAll('[id="' + e + '"] .edgeLabel .label');
    for (const b of m) {
  ...
  Et(h, I, s.diagramPadding, s.useMaxWidth), Object.keys(g).forEach(function(m) {
    const b = g[m];
    if (b.link) {
      const L = o.select("#" + e + ' [id="' + n.db.lookUpDomId(m) + '"]');
      if (L) {
