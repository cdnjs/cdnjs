...
function date$1(a, b) {
  ...
  return a = +a, b = +b, function(t5) {
    return d.setTime(a * (1 - t5) + b * t5), d;
  };
}
function object(a, b) {
  var i = {}, c = {}, k;
...
function interpolate(a, b) {
  var t5 = typeof b, c;
  return b == null || t5 === "boolean" ? constant(b) : (t5 === "number" ? interpolateNumber : t5 === "string" ? (c = color(b)) ? (b = c, interpolateRgb) : interpolateString : b instanceof color ? interpolateRgb : b instanceof Date ? date$1 : isNumberArray(b) ? numberArray : Array.isArray(b) ? genericArray : typeof b.valueOf !== "function" && typeof b.toString !== "function" || isNaN(b) ? object : interpolateNumber)(a, b);
}
function interpolateRound(a, b) {
  return a = +a, b = +b, function(t5) {
    return Math.round(a * (1 - t5) + b * t5);
  };
}
function hcl(hue2) {
...
function formatGroup(grouping, thousands) {
  return function(value, width) {
    var i = value.length, t5 = [], j = 0, g = grouping[0], length = 0;
    while (i > 0 && g > 0) {
      if (length + g + 1 > width)
        g = Math.max(1, width - length);
      t5.push(value.substring(i -= g, i + g));
      if ((length += g + 1) > width)
        break;
...
var t = function t4(format2) {
  return format2.replace(/(\[[^\]]+])|(MMMM|MM|DD|dddd)/g, function(_, a, b) {
    return a || b.slice(1);
  });
};
var englishFormats = {
...
var parseFormattedInput = function parseFormattedInput2(input, format2, utc) {
  try {
    ...
    var d = day2 || (!year2 && !month2 ? now.getDate() : 1);
    var y = year2 || now.getFullYear();
    var M = 0;
    if (!(year2 && !month2)) {
      M = month2 > 0 ? month2 - 1 : now.getMonth();
    }
    var h = hours || 0;
    var m = minutes || 0;
    var s = seconds || 0;
