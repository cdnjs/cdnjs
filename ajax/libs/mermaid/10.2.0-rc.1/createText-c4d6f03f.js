import { l as log, as as decodeEntities } from "./mermaid-5545c3f9.js";
var CR_NEWLINE_R = /\r\n?/g;
var TAB_R = /\t/g;
var FORMFEED_R = /\f/g;
...
var inlineRegex = function inlineRegex2(regex) {
  var match = function match2(source, state, prevCapture) {
    if (state.inline) {
...
var blockRegex = function blockRegex2(regex) {
  var match = function match2(source, state) {
    if (state.inline) {
      return null;
    } else {
      return regex.exec(source);
...
var parseInline = function parseInline2(parse, content, state) {
...
var parseBlock = function parseBlock2(parse, content, state) {
  var isCurrentlyInline = state.inline || false;
  state.inline = false;
...
var parseCaptureInline = function parseCaptureInline2(capture, parse, state) {
  return {
...
var TABLES = function() {
  ...
  var parseTableAlign = function parseTableAlign2(source, parse, state, trimEndSeparators) {
  ...
  var parseTableRow = function parseTableRow2(source, parse, state, trimEndSeparators) {
  ...
  var parseTableCells = function parseTableCells2(source, parse, state, trimEndSeparators) {
...
var outputFor = function outputFor2(rules, property, defaultState = {}) {
  if (!property) {
