import { c as A, s as Ct, g as vt, a as At, b as Ot, B as St, D as It, l as G, f as D, E as Gt, H as Pt, j as Ht, z as Nt, I as Bt } from "./mermaid-c2fb2af7.js";
var mt = function() {
  var r = function(q, h, b, k) {
    ...
    parse: function(h) {
      var b = this, k = [0], s = [], T = [null], t = [], X = this.table, e = "", rt = 0, ft = 0, Tt = 2, pt = 1, Lt = t.slice.call(arguments, 1), M = Object.create(this.lexer), Y = { yy: {} };
      ...
      var ot = M.yylloc;
      t.push(ot);
      var Rt = M.options && M.options.ranges;
      typeof Y.yy.parseError == "function" ? this.parseError = Y.yy.parseError : this.parseError = Object.getPrototypeOf(this).parseError;
      function Mt() {
        var j;
        return j = s.pop() || M.lex() || pt, typeof j != "number" && (j instanceof Array && (s = j, j = s.pop()), j = b.symbols_[j] || j), j;
      }
      for (var I, K, V, lt, W = {}, it, z, bt, st; ; ) {
        if (K = k[k.length - 1], this.defaultActions[K] ? V = this.defaultActions[K] : ((I === null || typeof I > "u") && (I = Mt()), V = X[K] && X[K][I]), typeof V > "u" || !V.length || !V[0]) {
          var ht = "";
          st = [];
          for (it in X[K])
...
Expecting ` + st.join(", ") + ", got '" + (this.terminals_[I] || I) + "'" : ht = "Parse error on line " + (rt + 1) + ": Unexpected " + (I == pt ? "end of input" : "'" + (this.terminals_[I] || I) + "'"), this.parseError(ht, {
        ...
        switch (V[0]) {
          case 1:
            k.push(I), T.push(M.yytext), t.push(M.yylloc), k.push(V[1]), I = null, ft = M.yyleng, e = M.yytext, rt = M.yylineno, ot = M.yylloc;
            break;
          case 2:
...
const Xt = function() {
  ...
  L = {}, L[r] = null, Z = {}, Z[r] = { name: r, order: a }, v = r, U = 0, Gt();
}, Qt = function() {
...
const se = () => {
  ...
  O === "TB" && (m = 30), Object.keys(a).sort((_, i) => a[_].seq - a[i].seq).forEach((_) => {
    const i = a[_], g = O === "TB" ? m + 10 : C[i.branch].pos, f = O === "TB" ? C[i.branch].pos : m + 10;
    if (o) {
    ...
    if (O === "TB" ? tt[i.id] = { x: f, y: m + 10 } : tt[i.id] = { x: m + 10, y: g }, o) {
      if (i.type !== H.CHERRY_PICK && (i.customId && i.type === H.MERGE || i.type !== H.MERGE) && u.showCommitLabel) {
        ...
        if (y.attr("x", m + 10 - w.width / 2 - 2).attr("y", g + 13.5).attr("width", w.width + 2 * 2).attr("height", w.height + 2 * 2), O === "TB" && (y.attr("x", f - (w.width + 4 * 4 + 5)).attr("y", g - 12), N.attr("x", f - (w.width + 4 * 4)).attr("y", g + w.height - 12)), O !== "TB" && N.attr("x", m + 10 - w.width / 2), u.rotateCommitLabel)
          if (O === "TB")
  ...
  if (m) {
    ...
    O === "TB" ? n.x < c.x ? f = `M ${n.x} ${n.y} L ${p - _} ${n.y} ${E} ${p} ${n.y + i} L ${p} ${c.y - _} ${l} ${p + i} ${c.y} L ${c.x} ${c.y}` : f = `M ${n.x} ${n.y} L ${p + _} ${n.y} ${l} ${p} ${n.y + i} L ${p} ${c.y - _} ${E} ${p - i} ${c.y} L ${c.x} ${c.y}` : n.y < c.y ? f = `M ${n.x} ${n.y} L ${n.x} ${x - _} ${l} ${n.x + i} ${x} L ${c.x - _} ${x} ${E} ${c.x} ${x + i} L ${c.x} ${c.y}` : f = `M ${n.x} ${n.y} L ${n.x} ${x + _} ${E} ${n.x + i} ${x} L ${c.x - _} ${x} ${l} ${c.x} ${x - i} L ${c.x} ${c.y}`;
  } else
  ...
  a.forEach((n, c) => {
    ...
    ), O === "TB" && (g.attr("x", l - p.width / 2 - 10).attr("y", 0), x.attr("transform", "translate(" + (l - p.width / 2 - 5) + ", 0)")), O !== "TB" && g.attr("transform", "translate(-19, " + (l - p.height / 2) + ")");
  });
}, le = function(r, a, o, u) {
  se();
...
`, "id:", a, o), J = u.db.getCommits();
  ...
  let E = 0;
  m.forEach((_, i) => {
    const g = wt(_.name), f = l.append("g"), x = f.insert("g").attr("class", "branchLabel"), p = x.insert("g").attr("class", "label branch-label");
    p.node().appendChild(g);
    let d = g.getBBox();
    C[_.name] = { pos: E, index: i }, E += 50 + (c.rotateCommitLabel ? 40 : 0) + (O === "TB" ? d.width / 2 : 0), p.remove(), x.remove(), f.remove();
