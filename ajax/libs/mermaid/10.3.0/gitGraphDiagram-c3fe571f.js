import { c as I, s as Oe, g as Ie, a as Ge, b as Pe, B as He, D as Ne, m as Be, l as H, f as V, E as De, H as Ve, I as fe, j as ze, z as je, J as qe } from "./mermaid-42f7bf2b.js";
var pe = function() {
  var r = function(q, m, b, k) {
    for (b = b || {}, k = q.length; k--; b[q[k]] = m)
      ;
    return b;
  }, a = [1, 4], o = [1, 7], p = [1, 5], n = [1, 9], c = [1, 6], u = [2, 6], h = [1, 16], w = [6, 8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40, 50, 55], x = [8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], l = [8, 13, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], g = [1, 26], f = [6, 8, 14, 50, 55], s = [8, 14, 55], _ = [1, 53], T = [1, 52], O = [8, 14, 30, 33, 35, 38, 55], E = [1, 67], d = [1, 68], y = [1, 69], N = [8, 14, 33, 35, 42, 55], ce = {
    trace: function() {
    },
    yy: {},
    ...
    parse: function(m) {
      ...
      for (var P, K, D, me, J = {}, ie, z, ke, se; ; ) {
        if (K = k[k.length - 1], this.defaultActions[K] ? D = this.defaultActions[K] : ((P === null || typeof P > "u") && (P = Se()), D = W[K] && W[K][P]), typeof D > "u" || !D.length || !D[0]) {
          var ue = "";
          se = [];
          for (ie in W[K])
...
const We = function(r) {
  ...
  try {
    ...
    H.error("error while parsing gitGraph options", a.message);
  }
}, Xe = function() {
  return Ee;
}, Qe = function(r, a, o, p) {
  H.debug("Entering commit:", r, a, o, p), a = V.sanitizeText(a, I()), r = V.sanitizeText(r, I()), p = V.sanitizeText(p, I());
  const n = {
    id: a || U + "-" + be(),
    message: r,
    seq: U++,
  ...
  G = n, R[n.id] = n, L[A] = n.id, H.debug("in pushCommit " + n.id);
}, Ze = function(r, a) {
  if (r = V.sanitizeText(r, I()), L[r] === void 0)
    L[r] = G != null ? G.id : null, Z[r] = { name: r, order: a ? parseInt(a, 10) : null }, we(r), H.debug("in createBranch");
  else {
    let o = new Error(
  ...
  G = u, R[u.id] = u, L[A] = u.id, H.debug(L), H.debug("in mergeBranch");
}, et = function(r, a, o) {
  if (H.debug("Entering cherryPick:", r, a, o), r = V.sanitizeText(r, I()), a = V.sanitizeText(a, I()), o = V.sanitizeText(o, I()), !r || R[r] === void 0) {
    let c = new Error(
  ...
  if (!a || R[a] === void 0) {
    ...
    G = u, R[u.id] = u, L[A] = u.id, H.debug(L), H.debug("in cherryPick");
  }
}, we = function(r) {
  if (r = V.sanitizeText(r, I()), L[r] === void 0) {
    let a = new Error(
...
function Te(r) {
  ...
  for (let n in L)
    L[n] === a.id && p.push(n);
  if (H.debug(p.join(" ")), a.parents && a.parents.length == 2) {
...
const tt = function() {
  ...
  L = {}, L[r] = null, Z = {}, Z[r] = { name: r, order: a }, A = r, U = 0, De();
}, it = function() {
  ...
  S === "TB" && (u = 30), Object.keys(a).sort((x, l) => a[x].seq - a[l].seq).forEach((x) => {
    ...
    if (o) {
      ...
      if (_ === B.HIGHLIGHT) {
        ...
        if (T.attr("cx", f), T.attr("cy", g), T.attr("r", l.type === B.MERGE ? 9 : 10), T.attr(
          "class",
        ...
        _ === B.REVERSE && n.append("path").attr("d", `M ${f - 5},${g - 5}L${f + 5},${g + 5}M${f - 5},${g + 5}L${f + 5},${g - 5}`).attr(
          "class",
          `commit ${s} ${l.id} commit${C[l.branch].index % F}`
        );
      }
      ...
      if (l.type !== B.CHERRY_PICK && (l.customId && l.type === B.MERGE || l.type !== B.MERGE) && p.showCommitLabel) {
        ...
        if (O.attr("x", u + 10 - d.width / 2 - 2).attr("y", g + 13.5).attr("width", d.width + 2 * 2).attr("height", d.height + 2 * 2), S === "TB" && (O.attr("x", f - (d.width + 4 * 4 + 5)).attr("y", g - 12), E.attr("x", f - (d.width + 4 * 4)).attr("y", g + d.height - 12)), S !== "TB" && E.attr("x", u + 10 - d.width / 2), p.rotateCommitLabel)
          if (S === "TB")
            ...
            let y = -7.5 - (d.width + 10) / 25 * 9.5, N = 10 + d.width / 25 * 8.5;
            T.attr(
              "transform",
              "translate(" + y + ", " + N + ") rotate(-45, " + u + ", " + g + ")"
            );
          }
      }
      if (l.tag) {
        ...
        const y = d.height / 2, N = g - 19.2;
        T.attr("class", "tag-label-bkg").attr(
          "points",
          `
          ${u - d.width / 2 - 4 / 2},${N + 2}
          ${u - d.width / 2 - 4 / 2},${N - 2}
          ${u + 10 - d.width / 2 - 4},${N - y - 2}
          ${u + 10 + d.width / 2 + 4},${N - y - 2}
          ${u + 10 + d.width / 2 + 4},${N + y + 2}
          ${u + 10 - d.width / 2 - 4},${N + y + 2}`
        ), O.attr("cx", u - d.width / 2 + 4 / 2).attr("cy", N).attr("r", 1.5).attr("class", "tag-hole"), S === "TB" && (T.attr("class", "tag-label-bkg").attr(
          "points",
          `
            ${f},${u + 2}
            ${f},${u - 2}
