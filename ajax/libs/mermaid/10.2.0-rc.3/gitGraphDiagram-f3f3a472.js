import { c as A, s as Ae, g as Oe, a as Se, b as Ie, y as Ge, z as Pe, m as Ne, l as G, f as V, A as He, D as De, E as pe, j as Ve, x as ze, F as je } from "./mermaid-17672fee.js";
var me = function() {
  var r = function(B, l, u, b) {
    for (u = u || {}, b = B.length; b--; u[B[b]] = l)
      ;
    return u;
  }, n = [1, 4], o = [1, 7], h = [1, 5], a = [1, 9], c = [1, 6], f = [2, 6], p = [1, 16], w = [6, 8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40, 50, 55], y = [8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], m = [8, 13, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], d = [1, 26], _ = [6, 8, 14, 50, 55], s = [8, 14, 55], C = [1, 53], v = [1, 52], H = [8, 14, 30, 33, 35, 38, 55], x = [1, 67], g = [1, 68], k = [1, 69], P = [8, 14, 33, 35, 42, 55], ne = {
    trace: function() {
    },
    yy: {},
    ...
    parse: function(l) {
      var u = this, b = [0], i = [], E = [null], e = [], W = this.table, t = "", Z = 0, be = 0, Re = 2, ge = 1, Ce = e.slice.call(arguments, 1), L = Object.create(this.lexer), q = { yy: {} };
...
me.parser = me;
const Be = me;
let te = A().gitGraph.mainBranchName, qe = A().gitGraph.mainBranchOrder, R = {}, O = null, Q = {};
...
const $e = function() {
  ...
  T = {}, T[r] = null, Q = {}, Q[r] = { name: r, order: n }, M = r, K = 0, He();
}, tt = function() {
...
const ot = () => {
  ...
  Object.keys(n).sort((y, m) => n[y].seq - n[m].seq).forEach((y) => {
    ...
    if (o) {
      ...
      if (C === N.HIGHLIGHT) {
        ...
        v.attr("x", _ - 10), v.attr("y", d - 10), v.attr("height", 20), v.attr("width", 20), v.attr(
          "class",
          `commit ${m.id} commit-highlight${I[m.branch].index % F} ${s}-outer`
        ), a.append("rect").attr("x", _ - 6).attr("y", d - 6).attr("height", 12).attr("width", 12).attr(
          "class",
          `commit ${m.id} commit${I[m.branch].index % F} ${s}-inner`
        );
        ...
        if (v.attr("cx", _), v.attr("cy", d), v.attr("r", m.type === N.MERGE ? 9 : 10), v.attr(
          "class",
          `commit ${m.id} commit${I[m.branch].index % F}`
        ), C === N.MERGE) {
          const H = a.append("circle");
          H.attr("cx", _), H.attr("cy", d), H.attr("r", 6), H.attr(
            "class",
            `commit ${s} ${m.id} commit${I[m.branch].index % F}`
          );
        }
        C === N.REVERSE && a.append("path").attr("d", `M ${_ - 5},${d - 5}L${_ + 5},${d + 5}M${_ - 5},${d + 5}L${_ + 5},${d - 5}`).attr(
          "class",
          `commit ${s} ${m.id} commit${I[m.branch].index % F}`
        );
      }
    ...
    if (re[m.id] = { x: f + 10, y: d }, o) {
      if (m.type !== N.CHERRY_PICK && (m.customId && m.type === N.MERGE || m.type !== N.MERGE) && h.showCommitLabel) {
        const v = c.append("g"), H = v.insert("rect").attr("class", "commit-label-bkg"), x = v.append("text").attr("x", f).attr("y", d + 25).attr("class", "commit-label").text(m.id);
      ...
      if (m.tag) {
        const v = c.insert("polygon"), H = c.append("circle"), x = c.append("text").attr("y", d - 16).attr("class", "tag-label").text(m.tag);
        let g = x.node().getBBox();
  ...
  n.forEach((a, c) => {
    const f = c % F, p = I[a.name].pos, w = h.append("line");
...
`, "id:", n, o), J = h.db.getCommits();
  ...
  const w = Ve(`[id="${n}"]`);
  _e(w, J, !1), c.showBranches && pt(w, f), ut(w, J), _e(w, J, !0), ze.insertTitle(
    w,
    "gitTitleText",
    c.titleTopMargin,
