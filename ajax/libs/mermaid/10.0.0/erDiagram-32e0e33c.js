...
import { m as kt } from "./mermaidAPI-f2dd6058.js";
import { s as Ot, g as bt, b as Rt, a as Nt, d as Tt, e as vt, f as At } from "./commonDb-9eb4b6e7.js";
import { G as Mt, l as It } from "./index-7f531408.js";
import { u as wt, E as St } from "./utils-c190d844.js";
import { c as Bt } from "./setupGraphViewbox-a7344a0b.js";
import { l as Dt } from "./isPlainObject-0b348591.js";
...
var ot = function() {
  var t = function(v, n, s, d) {
    for (s = s || {}, d = v.length; d--; s[v[d]] = n)
      ;
    return s;
  }, r = [1, 2], e = [1, 5], a = [6, 9, 11, 23, 25, 27, 29, 30, 31, 51], c = [1, 17], p = [1, 18], f = [1, 19], l = [1, 20], h = [1, 21], _ = [1, 22], m = [1, 25], E = [1, 30], O = [1, 31], y = [1, 32], M = [1, 33], D = [6, 9, 11, 15, 20, 23, 25, 27, 29, 30, 31, 44, 45, 46, 47, 51], V = [1, 45], z = [30, 31, 48, 49], L = [4, 6, 9, 11, 23, 25, 27, 29, 30, 31, 51], R = [44, 45, 46, 47], N = [22, 37], T = [1, 65], x = [1, 64], S = [22, 37, 39, 41], C = {
    trace: function() {
    },
    yy: {},
...
const ae = function(t) {
  ...
  if (e.length > 0) {
    ...
    l.forEach((x) => {
      ...
      const C = t.insert("rect", "#" + x.tn.node().id).classed(`er ${T}`, !0).attr("x", 0).attr("y", N).attr("width", m + c * 2 + R).attr("height", x.height + a * 2), B = parseFloat(C.attr("x")) + parseFloat(C.attr("width"));
      x.nn.attr(
        "transform",
        "translate(" + (B + c) + "," + S + ")"
      );
  ...
  return a.forEach(function(p) {
    ...
    c = c === void 0 ? f : c;
    const h = "text-" + f, _ = l.append("text").classed("er entityLabel", !0).attr("id", h).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "middle").style("font-family", U().fontFamily).style("font-size", k.fontSize + "px").text(p), { width: m, height: E } = se(
      l,
      _,
      r[p].attributes
...
const he = function(t, r, e, a, c) {
  ...
  const p = e.edge(
    ...
    return M.y;
  }).curve(St), l = t.insert("path", "#" + a).classed("er relationshipLine", !0).attr("d", f(p.points)).style("stroke", k.stroke).style("fill", "none");
  ...
  const _ = l.node().getTotalLength(), m = l.node().getPointAtLength(_ * 0.5), E = "rel" + ut, y = t.append("text").classed("er relationshipLabel", !0).attr("id", E).attr("x", m.x).attr("y", m.y).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-family", U().fontFamily).style("font-size", k.fontSize + "px").text(r.roleA).node().getBBox();
  ...
  Bt(l, M, y, k.useMaxWidth), l.attr("viewBox", `${O.x - E} ${O.y - E} ${y} ${M}`);
...
//# sourceMappingURL=erDiagram-32e0e33c.js.map
