import { c as O, s as Ce, g as ve, a as Oe, b as Pe, C as Ae, D as Ie, l as H, f as D, E as Se, H as Ge, j as Ne, A as He, I as Be } from "./mermaid-48a62b8a.js";
var me = function() {
...
const Je = function() {
  ...
  M = {}, M[r] = null, ee = {}, ee[r] = { name: r, order: n }, A = r, W = 0, Se();
}, Xe = function() {
  ...
  y.forEach((E) => {
    ...
    if (L === "TB" ? Y[b.id] = { x: g, y: f } : Y[b.id] = { x: f, y: k }, l) {
      if (b.type !== B.CHERRY_PICK && (b.customId && b.type === B.MERGE || b.type !== B.MERGE) && h.showCommitLabel) {
        const C = c.append("g"), S = C.insert("rect").attr("class", "commit-label-bkg"), I = C.append("text").attr("x", p).attr("y", k + 25).attr("class", "commit-label").text(b.id);
        let o = I.node().getBBox();
        if (S.attr("x", f - o.width / 2 - 2).attr("y", k + 13.5).attr("width", o.width + 2 * 2).attr("height", o.height + 2 * 2), L === "TB" && (S.attr("x", g - (o.width + 4 * 4 + 5)).attr("y", k - 12), I.attr("x", g - (o.width + 4 * 4)).attr("y", k + o.height - 12)), L !== "TB" && I.attr("x", f - o.width / 2), h.rotateCommitLabel)
          if (L === "TB")
      ...
      if (b.tag) {
        const C = c.insert("polygon"), S = c.append("circle"), I = c.append("text").attr("y", k - 16).attr("class", "tag-label").text(b.tag);
        let o = I.node().getBBox();
        ...
        C.attr("class", "tag-label-bkg").attr(
          ...
          ${f + o.width / 2 + 4},${d + u + 2}
          ${f - o.width / 2 - 4},${d + u + 2}`
        ), S.attr("cx", p - o.width / 2 + 4 / 2).attr("cy", d).attr("r", 1.5).attr("class", "tag-hole"), L === "TB" && (C.attr("class", "tag-label-bkg").attr(
          "points",
          `
            ${g},${p + 2}
            ${g},${p - 2}
