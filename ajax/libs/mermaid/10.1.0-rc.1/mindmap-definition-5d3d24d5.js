...
import { c as xp } from "./createText-898ab18c.js";
...
const Vp = function(he, fe) {
  ...
  const S = T.append("g"), G = T.append("g");
  xp(G, fe.descr, {
    useHtmlLabels: H,
    width: fe.width,
    classes: "mindmap-node-label"
  }), H || G.attr("dy", "1em").attr("alignment-baseline", "middle").attr("dominant-baseline", "middle").attr("text-anchor", "middle");
  const U = G.node().getBBox(), P = ve.fontSize.replace ? ve.fontSize.replace("px", "") : ve.fontSize;
  if (fe.height = U.height + P * 1.1 * 0.5 + fe.padding, fe.width = U.width + 2 * fe.padding, fe.icon)
    if (fe.type === yt.CIRCLE)
      ...
      T.append("foreignObject").attr("width", "60px").attr("height", fe.height).attr("style", "text-align: center;margin-top:" + D / 2 + "px;").append("div").attr("class", "icon-container").append("i").attr("class", "node-icon-" + O + " " + fe.icon), G.attr(
        "transform",
        "translate(" + (25 + fe.width / 2) + ", " + (D / 2 + fe.padding / 2) + ")"
      );
    }
  else if (H) {
    const K = (fe.width - U.width) / 2, D = (fe.height - U.height) / 2;
    G.attr("transform", "translate(" + K + ", " + D + ")");
  } else {
    const K = fe.width / 2, D = fe.padding / 2;
    G.attr("transform", "translate(" + K + ", " + D + ")");
  }
  switch (fe.type) {
    case yt.DEFAULT:
...
const ry = async (he, fe, ee, ve) => {
  const H = nn();
  H.htmlLabels = !1, ve.db.clear(), ve.parser.parse(he), qr.debug(`Renering mindmap diagram
` + he, ve);
...
//# sourceMappingURL=mindmap-definition-5d3d24d5.js.map
