import { c as V, s as xt, g as Rt, b as Ot, a as bt, D as Nt, E as Tt, m as At, l as et, F as Mt, j as st, A as vt, k as Dt, G as wt, H as It } from "./mermaid-d733041c.js";
import { G as St, l as Lt } from "./layout-df07420e.js";
import { l as Bt } from "./line-b0ce4b23.js";
...
var ct = function() {
  var t = function(M, a, l, d) {
    ...
    parse: function(a) {
      ...
      for (var D, z, L, at, K = {}, $, F, ut, tt; ; ) {
        if (z = d[d.length - 1], this.defaultActions[z] ? L = this.defaultActions[z] : ((D === null || typeof D > "u") && (D = kt()), L = Q[z] && Q[z][D]), typeof L > "u" || !L.length || !L[0]) {
          var nt = "";
          tt = [];
          for ($ in Q[z])
...
const se = function(t) {
  ...
  e.forEach((N) => {
    ...
    w.tn = P, w.nn = Z;
    const H = P.node().getBBox(), S = Z.node().getBBox();
    if (E = Math.max(E, H.width), g = Math.max(g, S.width), k = Math.max(H.height, S.height), c) {
  ...
  if (e.length > 0) {
    ...
    o.forEach((x) => {
      ...
      const H = t.insert("rect", "#" + x.nn.node().id).classed(`er ${k}`, !0).attr("x", w).attr("y", v).attr("width", g + s * 2 + N).attr("height", x.height + n * 2);
      let S = parseFloat(H.attr("x")) + parseFloat(H.attr("width"));
      if (c) {
        x.kn.attr(
          "transform",
          "translate(" + (S + s) + "," + P + ")"
