...
var isDescendant = /* @__PURE__ */ __name((id, ancenstorId) => {
...
var note = /* @__PURE__ */ __name(async (parent, node) => {
  ...
  rect3.attr("rx", node.rx).attr("ry", node.ry).attr("x", -bbox.width / 2 - halfPadding).attr("y", -bbox.height / 2 - halfPadding).attr("width", bbox.width + node.padding).attr("height", bbox.height + node.padding);
  updateNodeBounds(node, rect3);
...
var rectWithTitle = /* @__PURE__ */ __name((parent, node) => {
  ...
  rect3.attr("class", "outer title-state").attr("x", -bbox.width / 2 - halfPadding).attr("y", -bbox.height / 2 - halfPadding).attr("width", bbox.width + node.padding).attr("height", bbox.height + node.padding);
...
var stadium = /* @__PURE__ */ __name(async (parent, node) => {
  ...
  const rect3 = shapeSvg.insert("rect", ":first-child").attr("style", node.style).attr("rx", h / 2).attr("ry", h / 2).attr("x", -w / 2).attr("y", -h / 2).attr("width", w).attr("height", h);
  updateNodeBounds(node, rect3);
  node.intersect = function(point2) {
    return intersect_default.rect(node, point2);
  };
...
var class_box = /* @__PURE__ */ __name((parent, node) => {
  ...
  rect3.attr("class", "outer title-state").attr("x", -maxWidth / 2 - halfPadding).attr("y", -(maxHeight / 2) - halfPadding).attr("width", maxWidth + node.padding).attr("height", maxHeight + node.padding);
  updateNodeBounds(node, rect3);
  node.intersect = function(point2) {
    return intersect_default.rect(node, point2);
  };
...
var rect2 = /* @__PURE__ */ __name((parent, node) => {
  log.info("Creating subgraph rect for ", node.id, node);
  const shapeSvg = parent.insert("g").attr("class", "cluster" + (node.class ? " " + node.class : "")).attr("id", node.id);
  const rect3 = shapeSvg.insert("rect", ":first-child");
  const useHtmlLabels = evaluate(getConfig().flowchart.htmlLabels);
  const label = shapeSvg.insert("g").attr("class", "cluster-label");
  const text = node.labelType === "markdown" ? createText(label, node.labelText, { style: node.labelStyle, useHtmlLabels }) : label.node().appendChild(createLabel_default(node.labelText, node.labelStyle, void 0, true));
  let bbox = text.getBBox();
  if (evaluate(getConfig().flowchart.htmlLabels)) {
    const div = text.children[0];
...
var noteGroup = /* @__PURE__ */ __name((parent, node) => {
  ...
  const padding = 0 * node.padding;
  const halfPadding = padding / 2;
  rect3.attr("rx", node.rx).attr("ry", node.ry).attr("x", node.x - node.width / 2 - halfPadding).attr("y", node.y - node.height / 2 - halfPadding).attr("width", node.width + padding).attr("height", node.height + padding).attr("fill", "none");
  const rectBox = rect3.node().getBBox();
  node.width = rectBox.width;
  node.height = rectBox.height;
  node.intersect = function(point2) {
...
var roundedWithTitle = /* @__PURE__ */ __name((parent, node) => {
  ...
  rect3.attr("class", "outer").attr("x", node.x - width / 2 - halfPadding).attr("y", node.y - node.height / 2 - halfPadding).attr("width", width + padding).attr("height", node.height + padding);
...
var divider = /* @__PURE__ */ __name((parent, node) => {
  ...
  const padding = 0 * node.padding;
  const halfPadding = padding / 2;
  rect3.attr("class", "divider").attr("x", node.x - node.width / 2 - halfPadding).attr("y", node.y - node.height / 2).attr("width", node.width + padding).attr("height", node.height + padding);
  const rectBox = rect3.node().getBBox();
  node.width = rectBox.width;
  node.height = rectBox.height;
  node.diff = -node.padding / 2;
...
var shapes2 = { rect: rect2, roundedWithTitle, noteGroup, divider };
var clusterElems = {};
...
import { line, curveBasis, select as select5 } from "d3";
...
var cutPathAtIntersect = /* @__PURE__ */ __name((_points, boundryNode) => {
...
var insertEdge = /* @__PURE__ */ __name(function(elem, e, edge, clusterDb2, diagramType, graph, id) {
  ...
  if (edge.toCluster) {
    log.info("to cluster abc88", clusterDb2[edge.toCluster]);
    points = cutPathAtIntersect(edge.points, clusterDb2[edge.toCluster].node);
    pointsHasChanged = true;
  }
  if (edge.fromCluster) {
    log.info("from cluster abc88", clusterDb2[edge.fromCluster]);
...
// src/dagre-wrapper/index.js
var recursiveRender = /* @__PURE__ */ __name(async (_elem, graph, diagramtype, id, parentCluster) => {
  ...
  log.info(graph);
  dagreLayout(graph);
  log.info("Graph after layout:", graphlibJson2.write(graph));
  let diff = 0;
...
var render = /* @__PURE__ */ __name(async (elem, graph, markers2, diagramtype, id) => {
  markers_default(elem, markers2, diagramtype, id);
  clear2();
  clear4();
  clear3();
  clear();
