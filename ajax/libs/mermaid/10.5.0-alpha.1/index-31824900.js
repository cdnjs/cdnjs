import { b as j, i as m, m as D, G, l as A } from "./layout-5e3bc0fc.js";
import { c as O, i as X, a as M, b as _, d as $, u as H, s as V, e as U, f as W, p as B, g as Y, h as q } from "./edges-4eab8f28.js";
import { l as i, p as E, c as N, j as L } from "./mermaid-2aa9f9d3.js";
import { c as z } from "./createText-4209acc9.js";
var K = 4;
function Q(e) {
  return j(e, K);
}
...
const tt = () => {
  ...
  e.nodes().forEach(function(n) {
    ...
    if (i.warn(
      ...
      const l = structuredClone(r), d = structuredClone(r);
  ...
  const n = e.insert("g").attr("class", "cluster" + (t.class ? " " + t.class : "")).attr("id", t.id), r = n.insert("rect", ":first-child"), s = E(N().flowchart.htmlLabels), a = n.insert("g").attr("class", "cluster-label"), u = t.labelType === "markdown" ? z(a, t.labelText, { style: t.labelStyle, useHtmlLabels: s }) : a.node().appendChild(O(t.labelText, t.labelStyle, void 0, !0));
  let l = u.getBBox();
  if (E(N().flowchart.htmlLabels)) {
  ...
  t.width <= l.width + t.padding ? t.diff = (l.width + t.padding * 0 - t.width) / 2 : t.diff = -t.padding / 2, r.attr("class", "outer").attr("x", t.x - g / 2 - w).attr("y", t.y - t.height / 2 - w).attr("width", g + d).attr("height", t.height + d), a.attr("class", "inner").attr("x", t.x - g / 2 - w).attr("y", t.y - t.height / 2 - w + l.height - 1).attr("width", g + d).attr("height", t.height + d - l.height - 3), s.attr(
    "transform",
    "translate(" + (t.x - l.width / 2) + ", " + (t.y - t.height / 2 - t.padding / 3 + (E(N().flowchart.htmlLabels) ? 5 : 3)) + ")"
  );
  const c = r.node().getBBox();
...
const lt = (e, t) => {
  ...
  await Promise.all(
    t.nodes().map(async function(c) {
      ...
      if (i.info("(Insert) Node XXX" + c + ": " + JSON.stringify(t.node(c))), o && o.clusterNode) {
        ...
        t.children(c).length > 0 ? (i.info("Cluster - the non recursive path XXX", c, o.id, o, t), i.info(b(o.id, t)), f[o.id] = { id: b(o.id, t), node: o }) : (i.info("Node - the non recursive path", c, o.id, o), await U(w, t.node(c), s));
    })
  ...
  M(e, n, r, s), _(), $(), ft(), tt(), i.warn("Graph at first:", JSON.stringify(y(t))), nt(t), i.warn("Graph after:", JSON.stringify(y(t))), await k(e, t, r);
};
export {
  gt as r
};
