import { l as u, p as H, c as w, aR as Q, j as _, d as J, z as D, F as nt } from "./mermaid-cc0215b1.js";
import { a as at } from "./createText-0abd1a9d.js";
import { l as lt } from "./line-d42cab03.js";
const ct = (a, t, r, n) => {
  t.forEach((e) => {
    bt[e](a, r, n);
  });
          ...
          (y) => new Promise((x) => {
            function M() {
              if (y.style.display = "flex", y.style.flexDirection = "column", g) {
                const C = w().fontSize ? w().fontSize : window.getComputedStyle(document.body).fontSize, O = 5;
                y.style.width = parseInt(C, 10) * O + "px";
              } else
                y.style.width = "100%";
              x(y);
            }
...
function Et(a, t, r) {
  ...
  return i.length ? (i.length > 1 && i.sort(function(b, k) {
    var g = b.x - r.x, y = b.y - r.y, x = Math.sqrt(g * g + y * y), M = k.x - r.x, C = k.y - r.y, O = Math.sqrt(M * M + C * C);
    return x < O ? -1 : x === O ? 0 : 1;
  }), i[0]) : a;
}
const Mt = (a, t) => {
...
const Dt = (a, t) => {
  ...
  t.classData.members.forEach((L) => {
    ...
    T.width > h && (h = T.width), f += T.height + n, O.push(N);
  }), f += e;
  const V = [];
  ...
  ), p += C.height + n, c.attr("class", "divider").attr("x1", -h / 2 - r).attr("x2", h / 2 + r).attr("y1", -f / 2 - r + e + p).attr("y2", -f / 2 - r + e + p), p += e, O.forEach((L) => {
    _(L).attr(
      "transform",
...
const z = (a) => Array.isArray(a) ? { x: a[0], y: a[1] } : a, Jt = (a) => ({
  x: function(t, r, n) {
    ...
    if (r === 0 && Object.hasOwn(X, a.arrowTypeStart)) {
      ...
      e = X[a.arrowTypeStart] * Math.cos(i) * (s >= 0 ? 1 : -1);
    } else if (r === n.length - 1 && Object.hasOwn(X, a.arrowTypeEnd)) {
      const { angle: i, deltaX: s } = A(
        n[n.length - 1],
        n[n.length - 2]
      );
      e = X[a.arrowTypeEnd] * Math.cos(i) * (s >= 0 ? 1 : -1);
    }
    return z(t).x + e;
  },
  y: function(t, r, n) {
    ...
    if (r === 0 && Object.hasOwn(X, a.arrowTypeStart)) {
      ...
      e = X[a.arrowTypeStart] * Math.abs(Math.sin(i)) * (s >= 0 ? 1 : -1);
    } else if (r === n.length - 1 && Object.hasOwn(X, a.arrowTypeEnd)) {
      const { angle: i, deltaY: s } = A(
        n[n.length - 1],
        n[n.length - 2]
      );
      e = X[a.arrowTypeEnd] * Math.abs(Math.sin(i)) * (s >= 0 ? 1 : -1);
    }
    return z(t).y + e;
  }
});
