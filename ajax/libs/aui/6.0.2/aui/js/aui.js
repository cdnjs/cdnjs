...
(typeof window === 'undefined' ? global : window).__e3407e85cd5fc30bed4faefa24c7e49c = (function () {
  ...
  function whenIType(keys) {
      ...
      function appendKeyComboInstructions(keyCombo, title, isFirst) {
          if (isFirst) {
              title += ' (' + _i18n2.default.getText('aui.keyboard.shortcut.type.x', keyCombo.shift());
          } else {
              title = title.replace(/\)$/, '');
              title += _i18n2.default.getText('aui.keyboard.shortcut.or.x', keyCombo.shift());
          }
  
          keyCombo.forEach(function (key) {
              title += ' ' + _i18n2.default.getText('aui.keyboard.shortcut.then.x', key);
          });
          title += ')';
