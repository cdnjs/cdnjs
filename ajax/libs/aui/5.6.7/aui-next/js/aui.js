...
(function($, skate, Tether) {
    ...
    function createTether(layer) {
        ...
        var staticAlignment = layer.hasAttribute('data-aui-alignment-static');
        ...
        layer._tether = new Tether({
            ...
            enabled: false,
            classPrefix: 'aui-alignment',
            constraints: [
                {
                    // Try and keep the element on page
                    to: 'window',
                    attachment: (staticAlignment === true) ? 'none' : 'together'
                }
            ]
        });
    }
    ...
    skate('aui-layer', {
        ...
        insert: function(layer) {
            if (layer.getAttribute('data-aui-alignment')) {
                createTether(layer);
            }
        }
    });
    ...
    function triggerEvent ($el, deprecatedName, newNativeName) {
        var e1 = AJS.$.Event(EVENT_PREFIX + deprecatedName);
        var e2 = AJS.$.Event(GLOBAL_EVENT_PREFIX + deprecatedName);
    ...
    Layer.prototype = {
        ...
        show: function () {
            ...
            if (triggerEvent(this.$el, 'beforeShow', 'show')) {
                ...
                if (this.$el.attr('data-aui-alignment')) {
                    this.$el[0]._tether.enable();
                }
            }
        ...
        remove: function () {
            ...
            if (this.$el[0]._tether) {
                this.$el[0]._tether.destroy();
            }
        ...
        _hideLayer: function (triggerBeforeEvents) {
            ...
            if (this.isModal() || this.isBlanketed()) {
    ...
    AJS.layer.on = function (eventName, selector, fn) {
        $doc.on(GLOBAL_EVENT_PREFIX + eventName, selector, fn);
        return this;
    };

    AJS.layer.off = function (eventName, selector, fn) {
...
(function ($) {
    ...
    LayerManager.prototype = {
         ...
         *
         * @param {jQuery} $el  Element to push onto the stack.
         *
         ...
         *
         * @param {DOMElement | String | jQuery} The element to find in the stack.
         *
         * @returns {Integer}
         */
         ...
         *
         * @param {Integer} The index of the element to get.
         *
        ...
        hideAll: function() {
            this._stack.reverse().forEach(function(element) {
                ...
                if (layer.isBlanketed() || layer.isModal()) {
                    return;
                }
                layer.hide();
            });
        ...
        /**
         * Gets the previous layer below the given layer, which is non modal. If it find a blanketed layer on the way
         * it returns it regardless if it is modal or not
         *
         * @param {jQuery} $el layer to start the search from.
         *
         * @returns {jQuery | null} the next matching layer or null if none found.
         */
        getNextLowerNonModalOrBlanketedLayer: function($el) {
            ...
            while (index >= 0) {
                ...
                if (!AJS.layer($nextEl).isModal() || AJS.layer($nextEl).isBlanketed()) {
                    return $nextEl;
                }
                index--;
            }
         ...
         */
        getNextHigherNonModalAndNonBlanketedLayer: function($el) {
            var index = layerIndex(this._stack, $el);
         ...
         *
         * @returns The last layer that was popped, or null if no layer matching the given $el was found.
         */
        popUntil: function ($el) {
            var index = layerIndex(this._stack, $el);
            ...
            if (oldTopBlanketedIndex !== newTopBlanketedIndex) {
                AJS.undim();
        ...
        popUntilTopBlanketed: function () {
            ...
            if (AJS.layer($topBlanketedLayer).isModal()) {
                ...
                var $next = this.getNextHigherNonModalAndNonBlanketedLayer($topBlanketedLayer);
                if ($next) {
                    return this.popUntil($next);
                }
                return null;
            }
    ...
    function initCloseLayerOnEscPress() {
        $doc.on('keydown', function(e) {
            if (e.keyCode === AJS.keyCode.ESCAPE) {
                ...
                if (AJS.layer($toPop).isModal()) {
                    if (AJS.layer($toPop).isBlanketed()) {
                        return;
                    } else {
                        // Get the closest non modal layer below, stop at the first blanketed layer though, we don't want to pop below that
                        $toPop = AJS.LayerManager.global.getNextLowerNonModalOrBlanketedLayer($toPop);

                        if ($toPop && !AJS.layer($toPop).isModal()) {
                            AJS.layer($toPop).hide();
                        } else {
                            // Here we have a blanketed modal layer
                            return;
                        }
                    }
                } else {
    ...
    function initCloseLayerOnOuterClick () {
        $doc.on('click', function (e) {
            ...
            if ($layer.length) {
                ...
                var $next = AJS.LayerManager.global.getNextHigherNonModalAndNonBlanketedLayer($layer);
...
(function($) {
    ...
    AJS.dialog2.on = function(eventName, fn) {
        AJS.layer.on(eventName, '.aui-dialog2', fn);
        return this;
    };

    AJS.dialog2.off = function(eventName, fn) {
        AJS.layer.off(eventName, '.aui-dialog2', fn);
        return this;
    };
...
(function ($, skate) {
    ...
    skate(ATTRIBUTE_FIELD_COMPONENT, {
        ready: function(field) {
            var $field = $(field);
            AJS.validator._initValidation($field);
        },
...
(function(skate) {
    ...
    function getTrigger (inlineDialog) {
        return document.querySelector('[aria-controls="' + inlineDialog.id + '"]');
    }

    function doIfTrigger(inlineDialog, callback) {
        var trigger = getTrigger(inlineDialog);
    ...
    skate('aui-inline-dialog2', {
        ...
        ready: function(inlineDialog) {
            doIfTrigger(inlineDialog, function (trigger) {
                trigger.setAttribute('aria-expanded', inlineDialog.isVisible());
    ...
    var messageHandler = {
        click: function (inlineDialog) {
            if (inlineDialog.isVisible()) {
                if (!AJS.layer(inlineDialog).isModal()) {
                    inlineDialog.hide();
                }
            } else {
                inlineDialog.show();
        ...
        mouseleave: function (inlineDialog) {
            if (!AJS.layer(inlineDialog).isModal() && inlineDialog.isVisible()) {
                inlineDialog.hide();
            }
        },
    ...
    AJS.messages = {
        ...
        makeCloseable: function (message) {
            $(message || "div.aui-message.closeable").each(function () {
                var $this = $(this),
                    $icon = $('<span class="aui-icon icon-close" role="button" tabindex="0"></span>').click(function () {
                        $this.closeMessage();
        ...
        createMessage: function (type) {
            AJS.messages[type] = function (context, obj) {
                ...
                $message = $(AJS.template(template).fill({
