...
const createFetchFunction = (fetchFn = defaultFetchFunction) => {
    return (input, init) => fetchFn(input, Object.assign(Object.assign({}, init), { headers: Object.assign({ 'X-Tolgee-SDK-Type': 'JS', 'X-Tolgee-SDK-Version': "5.19.3-prerelease.a08fc0b3.0"  }, init === null || init === void 0 ? void 0 : init.headers) }));
};
...
function parseCombinedOptions(_a) {
    var { ns, noWrap, orEmpty, params, language } = _a, rest = __rest$1(_a, ["ns", "noWrap", "orEmpty", "params", "language"]);
    const options = {
        ns: ns,
        noWrap: noWrap,
        orEmpty: orEmpty,
...
function MouseEventHandler({ highlightKeys, elementStore, onClick, options, }) {
    var _a, _b;
    ...
    function subscribe(type, listener, options) {
        targetDocument.addEventListener(type, listener, options);
        subscribedEvents.push([type, listener, options]);
    }
    ...
    function getClosestTolgeeElement(elements) {
        for (const element of elements) {
            const result = findAncestor(element, (el) => elementStore.get(el));
            if (result !== undefined) {
...
function NodeHandler(options, wrapper) {
    const self = Object.freeze({
        handleAttributes(node, includeChild = true) {
            var _a, _b;
            const result = [];
            const tagAttributes = Object.fromEntries(Object.entries(options.tagAttributes).map(([tag, attributes]) => [
                tag.toUpperCase(),
                attributes,
            ]));
            if (node.nodeType === Node.ELEMENT_NODE) {
                const element = node;
                let attributes = (_a = tagAttributes[element.tagName.toUpperCase()]) !== null && _a !== void 0 ? _a : [];
                if ('*' in tagAttributes) {
                ...
                result.push(...attributes
                    ...
                    .filter((attrNode) => wrapper.testAttribute(attrNode)));
            }
            if (includeChild) {
                const walker = document.createTreeWalker(node, NodeFilter.SHOW_ELEMENT, (f) => {
                    var _a, _b;
                    return ((_a = tagAttributes[f.tagName.toUpperCase()]) === null || _a === void 0 ? void 0 : _a.some((t) => f.hasAttribute(t))) || ((_b = tagAttributes['*']) === null || _b === void 0 ? void 0 : _b.some((t) => f.hasAttribute(t)))
                        ? NodeFilter.FILTER_ACCEPT
                        : NodeFilter.FILTER_SKIP;
                });
                while (walker.nextNode()) {
                    const element = walker.currentNode;
                    let attributes = (_b = tagAttributes[element.tagName.toUpperCase()]) !== null && _b !== void 0 ? _b : [];
                    if ('*' in tagAttributes) {
                        attributes = attributes.concat(tagAttributes['*']);
                    }
                    result.push(...attributes
        ...
        handleText(node) {
            ...
            const nodes = [];
            const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, (f) => wrapper.testTextNode(f)
                ? NodeFilter.FILTER_ACCEPT
                : NodeFilter.FILTER_SKIP);
            while (walker.nextNode()) {
...
function GeneralObserver() {
    ...
    function createRunningInstance({ mouseHighlight, options, wrapper, onClick, }) {
        if (isSSR()) {
            return {
                stop() {
                    isObserving = false;
        ...
        function handleKeyAttribute(node, includeChild) {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
                const element = node;
                const attr = element.getAttributeNode(TOLGEE_WRAPPED_ONLY_DATA_ATTRIBUTE);
                if (attr) {
                    handleKeyAttributeAttr(attr);
                }
            }
            ...
            const walker = document.createTreeWalker(node, NodeFilter.SHOW_ELEMENT, (e) => e.hasAttribute(TOLGEE_WRAPPED_ONLY_DATA_ATTRIBUTE)
                ? NodeFilter.FILTER_ACCEPT
                : NodeFilter.FILTER_SKIP);
            while (walker.nextNode()) {
                const attr = walker.currentNode.getAttributeNode(TOLGEE_WRAPPED_ONLY_DATA_ATTRIBUTE);
                handleKeyAttributeAttr(attr);
            }
        }
        const observer = new MutationObserver((mutationsList) => {
            ...
            if (mutationsList.some((m) => m.type === 'attributes' &&
        ...
        const monitorAttributeList = new Set();
        monitorAttributeList.add(TOLGEE_WRAPPED_ONLY_DATA_ATTRIBUTE);
        Object.values(options.tagAttributes).forEach((attrs) => attrs.forEach((attr) => monitorAttributeList.add(attr.toLowerCase())));
    ...
    const self = Object.freeze({
        ...
        forEachElement(callback) {
            var _a, _b;
            (_b = (_a = instance === null || instance === void 0 ? void 0 : instance.elementRegistry) === null || _a === void 0 ? void 0 : _a.forEachElement) === null || _b === void 0 ? void 0 : _b.call(_a, callback);
        },
        highlight(key, ns) {
            var _a;
            const elements = ((_a = instance === null || instance === void 0 ? void 0 : instance.elementRegistry) === null || _a === void 0 ? void 0 : _a.findAll(key, ns)) || [];
        ...
        findPositions(key, ns) {
            ...
            elements.sort((a, b) => {
                if (a.element.compareDocumentPosition(b.element) &
            ...
            elements.forEach((meta) => {
                ...
                meta.nodes.forEach((node) => {
                    node.keys.forEach((val) => {
                        if (compareDescriptors({ key, ns: getFallback(ns) }, { key: val.key, ns: getFallback(val.ns) }))
                            result.push({
                                position: {
                                    x: shape.x,
                                    y: shape.y,
...
// eslint-disable-next-line no-console
(_a = console.assert) === null || _a === void 0 ? void 0 : _a.call(console, FastTextEncoding);
...
function InvisibleWrapper({ fullKeyEncode }) {
    ...
    return Object.freeze({
        ...
        testTextNode(textNode) {
            var _a, _b, _c;
            return ((_c = (((_a = textNode.textContent) === null || _a === void 0 ? void 0 : _a.includes(`${INVISIBLE_CHARACTERS[0]}${INVISIBLE_CHARACTERS[0]}`)) ||
                ((_b = textNode.textContent) === null || _b === void 0 ? void 0 : _b.includes(`${INVISIBLE_CHARACTERS[1]}${INVISIBLE_CHARACTERS[0]}`)))) !== null && _c !== void 0 ? _c : false);
        },
        testAttribute(attribute) {
            return (attribute.value.includes(`${INVISIBLE_CHARACTERS[0]}${INVISIBLE_CHARACTERS[0]}`) ||
                attribute.value.includes(`${INVISIBLE_CHARACTERS[1]}${INVISIBLE_CHARACTERS[0]}`));
        },
    });
}
...
function TextWrapper({ inputPrefix, inputSuffix, translate, }) {
    ...
    return Object.freeze({
        ...
        testTextNode(textNode) {
            var _a, _b, _c;
            return ((_c = (((_a = textNode.textContent) === null || _a === void 0 ? void 0 : _a.includes(inputPrefix)) &&
                ((_b = textNode.textContent) === null || _b === void 0 ? void 0 : _b.includes(inputSuffix)))) !== null && _c !== void 0 ? _c : false);
        },
        testAttribute(attribute) {
            return (attribute.value.includes(inputPrefix) &&
...
if (sessionStorageAvailable()) {
    BrowserExtensionPlugin = () => (tolgee) => {
        ...
        const getTolgeePlugin = async () => {
            const InContextTools = await loadInContextLib("5.19.3-prerelease.a08fc0b3.0" );
            return (tolgee) => {
...
export { BackendFetch, BrowserExtensionPlugin, DEVTOOLS_ID, DevBackend, DevTools, FormatSimple, LanguageDetector, LanguageStorage, ObserverPlugin, PREFERRED_LANGUAGES_LOCAL_STORAGE_KEY, TOLGEE_ATTRIBUTE_NAME, TOLGEE_RESTRICT_ATTRIBUTE, TOLGEE_WRAPPED_ONLY_DATA_ATTRIBUTE, Tolgee, TolgeeCore, createFetchFunction, getFallback, getFallbackArray, getProjectIdFromApiKey, getTranslateProps };
//# sourceMappingURL=tolgee-web.production.esm.mjs.map
