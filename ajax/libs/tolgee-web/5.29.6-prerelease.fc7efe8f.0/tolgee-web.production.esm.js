...
const createFetchFunction = (fetchFn = defaultFetchFunction) => {
  return (input, init) => {
    ...
    if (headers["x-api-key"]) {
      headers = Object.assign({ "x-tolgee-sdk-type": "JS", "x-tolgee-sdk-version": "5.29.6-prerelease.fc7efe8f.0" }, headers);
    }
...
function createBackendFetch(options) {
  ...
  }), { prefix, getPath, getData, headers } = _a2, fetchOptions = __objRest(_a2, ["prefix", "getPath", "getData", "headers"]);
  return {
    getRecord({ namespace, language, fetch: fetch2 }) {
      ...
      return fetch2(path, __spreadValues({ headers }, fetchOptions)).then((r) => {
        if (!r.ok) {
