...
const createFetchFunction = (fetchFn = defaultFetchFunction) => {
  return (input, init) => {
    ...
    if (headers["x-api-key"]) {
      headers = Object.assign({ "x-tolgee-sdk-type": "JS", "x-tolgee-sdk-version": "5.29.4-prerelease.e850f912.0" }, headers);
    }
...
function decodeFromText(text) {
  ...
  const result = [];
  invisibleMessages == null ? void 0 : invisibleMessages.map(decodeMessage).forEach((message) => {
    result.push(message);
  });
  return result;
}
