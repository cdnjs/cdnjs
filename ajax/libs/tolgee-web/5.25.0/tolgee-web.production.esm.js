...
const createFetchFunction = (fetchFn = defaultFetchFunction) => {
  return (input, init) => fetchFn(input, Object.assign(Object.assign({}, init), { headers: Object.assign({ "X-Tolgee-SDK-Type": "JS", "X-Tolgee-SDK-Version": "5.25.0" }, init === null || init === void 0 ? void 0 : init.headers) }));
};
...
function createBackendFetch(options) {
  ...
  return {
    getRecord({ namespace, language, fetch: fetch2 }) {
      const path = getPath({
        namespace,
        language,
        prefix
      });
      return fetch2(path, { headers, ...fetchOptions }).then((r) => {
        if (!r.ok) {
