...
const createFetchFunction = (fetchFn = defaultFetchFunction) => {
    return (input, init) => fetchFn(input, Object.assign(Object.assign({}, init), { headers: Object.assign({ 'X-Tolgee-SDK-Type': 'JS', 'X-Tolgee-SDK-Version': "5.20.4-prerelease.afe383ff.0"  }, init === null || init === void 0 ? void 0 : init.headers) }));
};
...
if (sessionStorageAvailable()) {
    BrowserExtensionPlugin = () => (tolgee) => {
        ...
        const getTolgeePlugin = async () => {
            const InContextTools = await loadInContextLib("5.20.4-prerelease.afe383ff.0" );
            return (tolgee) => {
...
export { BackendFetch, BrowserExtensionPlugin, DEVTOOLS_ID, DevBackend, DevTools, FormatSimple, LanguageDetector, LanguageDetectorError, LanguageStorage, LanguageStorageError, ObserverPlugin, PREFERRED_LANGUAGES_LOCAL_STORAGE_KEY, RecordFetchError, TOLGEE_ATTRIBUTE_NAME, TOLGEE_RESTRICT_ATTRIBUTE, TOLGEE_WRAPPED_ONLY_DATA_ATTRIBUTE, Tolgee, TolgeeCore, createFetchFunction, getFallback, getFallbackArray, getProjectIdFromApiKey, getTranslateProps };
