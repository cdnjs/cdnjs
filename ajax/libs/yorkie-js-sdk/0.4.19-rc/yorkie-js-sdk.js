...
var __publicField = (obj, key, value) => {
  ...
  class CRDTText extends CRDTGCElement {
    ...
    setStyle(range, attributes, editedAt, latestCreatedAtMapByActor) {
      ...
      for (const node of nodes) {
        ...
        const latestCreatedAt = (latestCreatedAtMapByActor == null ? void 0 : latestCreatedAtMapByActor.size) ? latestCreatedAtMapByActor.has(actorID) ? latestCreatedAtMapByActor.get(actorID) : InitialTimeTicket : MaxTimeTicket;
        if (node.canStyle(editedAt, latestCreatedAt)) {
          const latestCreatedAt2 = createdAtMapByActor.get(actorID);
          const createdAt = node.getCreatedAt();
          if (!latestCreatedAt2 || createdAt.after(latestCreatedAt2)) {
  ...
  class CRDTTreeNode extends IndexTreeNode {
    ...
    canDelete(editedAt, latestCreatedAt) {
      return !this.getCreatedAt().after(latestCreatedAt) && (!this.removedAt || editedAt.after(this.removedAt));
    }
  }
  ...
  class CRDTTree extends CRDTGCElement {
    ...
    style(range, attributes, editedAt) {
      ...
      const changes = [];
      const value = attributes ? parseObjectValues(attributes) : void 0;
        ...
        ([node]) => {
          if (!node.isRemoved && !node.isText && attributes) {
            ...
            for (const [key, value2] of Object.entries(attributes)) {
              node.attrs.set(key, value2, editedAt);
            }
            changes.push({
              type: "style",
    ...
    edit(range, contents, splitLevel, editedAt, issueTimeTicket, latestCreatedAtMapByActor) {
      ...
      const latestCreatedAtMap = /* @__PURE__ */ new Map();
      this.traverseInPosRange(
        fromParent,
        fromLeft,
        toParent,
        ...
        ([node, tokenType], ended) => {
          ...
          const actorID = node.getCreatedAt().getActorID();
          const latestCreatedAt = latestCreatedAtMapByActor ? latestCreatedAtMapByActor.has(actorID) ? latestCreatedAtMapByActor.get(actorID) : InitialTimeTicket : MaxTimeTicket;
          if (node.canDelete(editedAt, latestCreatedAt) || nodesToBeRemoved.includes(node.parent)) {
            const latestCreatedAt2 = latestCreatedAtMap.get(actorID);
            const createdAt = node.getCreatedAt();
            if (!latestCreatedAt2 || createdAt.after(latestCreatedAt2)) {
    ...
    *[Symbol.iterator]() {
    ...
    filterNodes(candidates, editedAt, latestCreatedAtMapByActor) {
      ...
      for (const node of candidates) {
        const actorID = node.getCreatedAt().getActorID();
        const latestCreatedAt = isRemote ? latestCreatedAtMapByActor.has(actorID) ? latestCreatedAtMapByActor.get(actorID) : InitialTimeTicket : MaxTimeTicket;
