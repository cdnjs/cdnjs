...
function elementFromPoint(baseNode, clientX, clientY) {
  var doc = utils.getDocument(baseNode);
...
function createElement(jsx, prevTarget, container) {
  var tag = jsx.tag,
      ...
      style = jsx.style;
  var el = prevTarget || utils.getDocument(container).createElement(tag);
  ...
  if (className) {
    className.split(/\s+/g).forEach(function (name) {
      if (name && !utils.hasClass(el, name)) {
...
function getLineSize(points) {
  ...
  for (var i = 1; i < length; ++i) {
    size = Math.max(utils.getDist(points[i], points[i - 1]), size);
  }
...
function (_super) {
  ...
  function Selecto(options) {
    ...
    _this._onDragStart = function (e, clickedTarget) {
      ...
      data.data = {};
      var win = utils.getWindow(_this.container);
      ...
      if (boundContainer) {
        ...
        if (boundElement) {
          if (utils.isString(boundElement)) {
            rectElement = utils.getDocument(container).querySelector(boundElement);
      ...
      if (isPreventSelect && selectByClick && !clickBySelectEnd) {
        ...
        if (innerScrollOptions) {
          ...
          while (parentElement && parentElement !== utils.getDocument(container).body) {
    ...
    _this._onDocumentSelectStart = function (e) {
      var doc = utils.getDocument(_this.container);
      ...
      if (dragContainer === utils.getWindow(_this.container)) {
        dragContainer = doc.documentElement;
      }

      var containers = utils.isNode(dragContainer) ? [dragContainer] : [].slice.call(dragContainer);
      var target = e.target;
  ...
  __proto.setSelectedTargets = function (selectedTargets) {
    ...
    var _a = childrenDiffer.diff(beforeSelected, selectedTargets),
        added = _a.added,
        removed = _a.removed,
        prevList = _a.prevList,
        list = _a.list;
  ...
  __proto.getSelectableElements = function () {
    ...
    this.options.selectableTargets.forEach(function (target) {
      ...
      } else if (utils.isNode(target)) {
        selectableElements.push(target);
      } else if (utils.isObject(target)) {
        selectableElements.push(target.value || target.current);
      } else {
        var elements = [].slice.call(utils.getDocument(container).querySelectorAll(target));
  ...
  __proto.findSelectableTargets = function (data) {
    ...
    var doc = utils.getDocument(this.container);
  ...
  __proto.setKeyController = function () {
    ...
    if (toggleContinueSelect || toggleContinueSelectWithoutDeselect) {
      this.keycon = new KeyController(keyContainer || utils.getWindow(this.container));
  ...
  __proto.initElement = function () {
    ...
    this.dragContainer = typeof dragContainer === "string" ? [].slice.call(utils.getDocument(container).querySelectorAll(dragContainer)) : dragContainer || this.target.parentNode;
    this.gesto = new Gesto(this.dragContainer, {
      checkWindowBlur: true,
      container: utils.getWindow(container),
  ...
  __proto.hitTest = function (selectRect, data, isDrag, gestoEvent) {
    ...
    var isHit = function (points, el) {
      var hitRateValue = typeof hitRate === "function" ? utils.splitUnit("".concat(hitRate(el))) : utils.splitUnit("".concat(hitRate));
      ...
      var overlapSize = overlapArea.getAreaSize(overlapPoints); // Line

      var targetSize = 0;

      if (overlapSize === 0 && overlapArea.getAreaSize(points) === 0) {
        targetSize = getLineSize(points);
        overlapSize = getLineSize(overlapPoints);
      } else {
        targetSize = overlapArea.getAreaSize(points);
      }
  ...
  __proto._select = function (selectedTargets, rect, e, isStart, isDragStartEnd) {
    ...
    var _a = childrenDiffer.diff(data.startSelectedTargets, selectedTargets),
        added = _a.added,
        removed = _a.removed,
        prevList = _a.prevList,
        list = _a.list;
