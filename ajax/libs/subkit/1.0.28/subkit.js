...
module.exports = function(subkit){
	...
	return {
		...
		setAlias: function(id, alias, callback){
			var deferred = subkit.$q.defer();
			var url = subkit.baseUrl + '/baas/alias/' + id;
			var msg = JSON.parse(JSON.stringify(subkit.options));
			msg['data'] = {
				alias: alias
			};			
		...
		removeAlias: function(id, callback){
			var deferred = subkit.$q.defer();
			var url = Subkit.prototype.self.baseUrl + '/baas/alias/' + id;
			subkit.httpRequest.del(url, subkit.options, function(status, result){
				if (status === 0) deferred.reject(new Error('No network connection.'));
				else if (status !== 200 && status!==202 && status!==204) deferred.reject(new Error(result.json().message));
				else deferred.resolve(result.json());
...
module.exports = function(subkit){
	...
	return {
		...
		open: function(templateName, metadata, callback){
			var deferred = subkit.$q.defer();			
			var url = subkit.baseUrl + '/template/'+templateName+'/action/render';
			if(metadata){
...
module.exports = function(subkit){
	...
	return {
		login: function(callback){
			var deferred = subkit.$q.defer();
			var url = subkit.baseUrl + '/users/action/login';
		...
		register: function(username, password, callback){
			var deferred = subkit.$q.defer();
			var url = subkit.baseUrl + '/users/action/register';
			var msg = JSON.parse(JSON.stringify(subkit.options));
			msg.data = { 
				username: username,
				password: password
	...
	self.manage = {
		...
		acl:{
			...
			grantInsert: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/grantinsert/' + identity;
			...
			grantUpdate: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/grantupdate/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
			...
			grantDelete: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/grantdelete/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
			...
			grantRead: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/grantread/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
			...
			revokeInsert: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/revokeinsert/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
			...
			revokeUpdate: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/revokeupdate/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
			...
			revokeDelete: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/revokedelete/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
			...
			revokeRead: function(key, identity, callback){
				var url = self.baseUrl + '/shares/' + encodeURIComponent(key) + '/action/revokeread/' + identity;
				httpRequest.put(url, self.options, function(status, result){
					if(!callback) return;
					if(status === 0) return callback({message: 'No network connection.'});
					if(status !== 202) return callback(result.json());
		...
		password: {
			set: function(oldPassword, newPassword, verifyPassword, callback){
				var deferred = self.$q.defer();
				var url = self.baseUrl + '/manage/password/action/reset';
				var msg = JSON.parse(JSON.stringify(self.options));
				msg.data = {
		...
		apikey: {
			reset: function(callback){
				var deferred = self.$q.defer();
				var url = self.baseUrl + '/manage/apikey/action/reset';
				httpRequest.put(url, self.options, function(status, result){
					if (status === 0) deferred.reject(new Error('No network connection.'));
					else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
					else deferred.resolve(result.json());
		...
		certificate:{
			...
			set: function(certificate, key, callback){
				var deferred = self.$q.defer();
				var url = self.baseUrl + '/manage/certificate/action/change';
				var msg = JSON.parse(JSON.stringify(self.options));
				msg.data = {
		...
		plugin: {
			...
			add: function(name,callback){
				var deferred = self.$q.defer();
				var url = self.baseUrl + '/plugins/' + name;
				httpRequest.post(url, self.options, function(status, result){
					if (status === 0) deferred.reject(new Error('No network connection.'));
					else if (status !== 200 && status !== 201 && status !== 202) deferred.reject(new Error(result.json().message));
					else deferred.resolve(result.json());
...
},{"./plugins/baas":3,"./plugins/email":4,"./plugins/file":5,"./plugins/task":6,"./plugins/template":7,"./plugins/user":8,"q":2}]},{},[9])
(9)
});
;