		...
		return {
			...
			logger:{
				on: function(query, callback){
					...
					var url = _prepareParams(subkit.baseUrl + '/logs/stream', query);
					subkit.httpRequest.get(url, subkit.options, function(status, result){
	...
	return function(){
		...
		return {
			...
			on: function(task, callback){
				if(subscriptions[task]) subscriptions[task]().abort();
				subscriptions[task] = poll(task, { size: 1 }, callback);
                return true;	
			},
			off: function(task){
				if(subscriptions[task]) subscriptions[task]().abort();
				delete subscriptions[task];
                return false;	
			}
		};
	};
