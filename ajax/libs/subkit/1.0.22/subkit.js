...
/**
* File
* @param {Subkit} subkit - A Subkit instance
*/
module.exports = function(subkit){
	'use strict';
	...
	return {
		...
		* @method subkit.file.upload
		* @memberOf :file#
		* @param {Blob} file - A file as Blob
		* @param {callback|undefined} callback - Optional handler function
		* @return {Promise} Q based Promise
		*/
	...
	return {
		list: function(callback){
			...
			subkit.httpRequest.get(url, msg,function (status, result){
				...
				else deferred.resolve(result.json().map(function(itm){
					return itm.value;
				}));
			});
			return deferred.promise.nodeify(callback);
...
module.exports = function(subkit){
	...
	return {
		...
		run: function(taskName, value, callback, logCallback){
			var deferred = subkit.$q.defer();
			...
			if(metadata){
				...
				for(var idx in metadata){
				   if (metadata.hasOwnProperty(idx)) {
				       str.push(encodeURIComponent(idx) + '=' + encodeURIComponent(metadata[idx]));
				   }
				}
				url += '?' + str.join('&');
			}
...
module.exports = function(subkit){
	...
	return {
		...
		listAll: function(callback){
			...
			subkit.httpRequest.get(url, msg,function (status, result){
				if (status === 0) deferred.reject(new Error('No network connection.'));
				else if (status !== 200) deferred.reject(result.json().message);
				else deferred.resolve(result.json().map(function(itm){
					return itm.value;
				}));
			});
			...
			subkit.httpRequest.get(url,subkit.options,function (status, result){
				if (status === 0) deferred.reject(new Error('No network connection.'));
					...
					for(var idx in result){
						if (result.hasOwnProperty(idx)) results.push(idx);
					}
...
module.exports = function(config){
	...
	self.manage = {
		/**
		 * Login into Subkit backend.
		 * @method subkit.manage.login
		 * @memberOf module:Subkit#manage
		 * @param {callback|undefined} callback - Optional handler function
		 * @return {Promise} Q based Promise
		 */
		login: function(callback){
		...
		/**
		 * Import data.
		 * @method subkit.manage.import
		 * @memberOf module:Subkit#
		 * @param {file} file - A file as Blob
		 * @param {callback|undefined} callback - Optional handler function
		 * @return {Promise} Q based Promise
		 */		
	...
	self.store = function(store){
		...
		var ref = {
			...
			history: function(callback){
                var deferred = self.$q.defer();
				httpRequest.get('', self.options, function(status, result){
                    if (status === 0) deferred.reject(new Error('No network connection.'));
                    else if (status !== 200) deferred.reject(new Error(result.json().message));
                    else deferred.resolve(result.json());
				});
