			...
			login: function(callback){
				...
				subkit.httpRequest.post(url, subkit.options, function(status, result){
					if (status === 0) deferred.reject(new Error('No network connection.'));
						...
						subkit.options.apiKey = jsonResult.apiKey;
						subkit.options.domain = jsonResult.domain;
						subkit.options.id = jsonResult.id;
						subkit.options.port = jsonResult.port;
						deferred.resolve({
							id: subkit.options.id,
							domain: subkit.options.domain,
							port: subkit.options.port,
