...
module.exports = function(subkit, subscriptions, poll){
	...
	return function(){
		...
		return {
			...
			set: function(webhook, callback){
				var deferred = subkit.$q.defer();
				var url = subkit.baseUrl + '/events/stream/' + webhook.stream;
				var msg = JSON.parse(JSON.stringify(subkit.options));
				if(webhook.url) msg.headers['x-subkit-event-webhook'] = webhook.url;
				if(webhook.apiKey) msg.headers['x-subkit-event-apikey'] = webhook.apiKey;
				if(webhook.filter) msg.headers['x-subkit-event-filter'] = JSON.stringify(webhook.filter);
			...
			remove: function(webhook, callback){
                var deferred = subkit.$q.defer();
                var url = subkit.baseUrl + '/events/stream/' + webhook.stream;
                var msg = JSON.parse(JSON.stringify(subkit.options));
                if(webhook.url) msg.headers['x-subkit-event-webhook'] = webhook.url;
				subkit.httpRequest.del(url, msg, function(status, result){
                    if (status === 0) deferred.reject(new Error('No network connection.'));
			...
			history: function(stream, callback){
				var deferred = subkit.$q.defer();
				var url = subkit.baseUrl + '/events/history/' + stream;
			...
			clear: function(stream, callback){
                var deferred = subkit.$q.defer();
                var url = subkit.baseUrl + '/events/history/' + stream;
			...
			on: function(stream, query, callback) {
				stream = stream.replace('/', '_');
				subscriptions[stream] = poll(stream, query, callback);
				return {
					off: function(){
		...
		return {
			...
			logger:{
				on: function(query, callback){
					...
					var url = _prepareParams(subkit.baseUrl + '/events/log', query);
					var subscription = subkit.httpRequest.get(url, subkit.options, function(status, result){
				...
				export: function(name, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/log/' + name;
					subkit.httpRequest.get(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200) deferred.reject(new Error(result.json().message));
						else deferred.resolve('data:application/octet-stream,' + result.text());
			...
			backups:{
				list:function(callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/savepoints';
					subkit.httpRequest.get(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				backup:function(callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/backup';
					subkit.httpRequest.post(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 201) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				restore:function(name, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/restore/' + name;
				...
				uploadRestore: function(file, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/restore/' + file.name;
					var msg = JSON.parse(JSON.stringify(subkit.options));
			...
			password: {
				set: function(oldPassword, newPassword, verifyPassword, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/password/reset';
					var msg = JSON.parse(JSON.stringify(subkit.options));
					msg.data = {
			...
			apikey: {
				reset: function(callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/apikey/reset';
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
			...
			status: {
				...
				update: function(callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/update';
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status!==202 && status!==204) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
			...
			plugins: {
				...
				uploadInstall: function(file, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/plugins/' + file.name;
					var msg = JSON.parse(JSON.stringify(subkit.options));
					msg.headers['Content-Type'] = 'application/octed-stream';
					/*jshint -W069 */
					msg['data'] = file;
			...
			permissions:{
				...
				grantInsert: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/grantinsert/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				grantUpdate: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/grantupdate/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				grantDelete: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/grantdelete/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				grantRead: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/grantread/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				revokeInsert: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/revokeinsert/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				revokeUpdate: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/revokeupdate/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				revokeDelete: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/revokedelete/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
				...
				revokeRead: function(key, identity, callback){
					var deferred = subkit.$q.defer();
					var url = subkit.baseUrl + '/manage/permissions/' + encodeURIComponent(key) + '/revokeread/' + identity;
					subkit.httpRequest.put(url, subkit.options, function(status, result){
						if (status === 0) deferred.reject(new Error('No network connection.'));
						else if (status !== 200 && status !== 202) deferred.reject(new Error(result.json().message));
						else deferred.resolve(result.json());
	...
	return function(store){
		...
		var ref = {
			...
			on: function(callback){
				...
				subscriptions[store] = poll(store, { size: 1 }, callback);
                return true;	
			},
			off: function(){
				if(subscriptions[store]) subscriptions[store]().abort();
...
module.exports = function(subkit, subscriptions, poll){
	...
	return function(){
      ...
      on: function(taskName, query, callback){
        if(subscriptions[taskName]) subscriptions[taskName].abort();
        subscriptions[taskName] = poll(taskName, query, callback);
        return {
            off: function(){
                if(subscriptions[taskName]) subscriptions[taskName].abort();
                delete subscriptions[taskName];                        
            }
        };
      }
		};
...
function Subkit(config){
	...
	self.tasks = require('./lib/tasks')(self, subscriptions, stream);
	self.stores = require('./lib/stores')(self, subscriptions, stream);
	self.events = require('./lib/events')(self, subscriptions, stream);
	self.UUID = Subkit.UUID;
		  ...
		  xhr.onreadystatechange = function () {
			if(!callback) return;
		  	...
		  	if(xhr.readyState === 3 && xhr.status === 200) {		  		
				var rawChunk = this.response.split('\n\n');
				var chunk = [];
				for (var i = 0; i < rawChunk.length; i++) {
					try{ chunk.push(JSON.parse(rawChunk[i])); }
					catch(e){}
				}
		  		callback(xhr.status, {
		  			json: function() {
	...
	function stream(name, query, callback){		
		var _prepareParams = function(url, params){
			var queryString = '';
			for(var key in params){
				if(key === 'where' && params[key]) {
		...
		var url = _prepareParams(self.baseUrl + '/events/stream/' + name, query);
		return self.httpRequest.get(url, self.options, function(status, result){
			callback(null, result.json());
		});		
	}
}
...
},{"./lib/events":3,"./lib/manage":4,"./lib/stores":5,"./lib/tasks":6,"q":2}]},{},[7])
(7)
});
;