    ...
    calculatePointStyle: function calculatePointStyle(index) {
      ...
      if (type !== 'linear') {
        ...
        if (type === 'circle') {
          ...
          return {
            left: "calc(".concat(radius * Math.cos(step * index), "px + ").concat($dt('item.diff.x', '0px'), ")"),
            top: "calc(".concat(radius * Math.sin(step * index), "px + ").concat($dt('item.diff.y', '0px'), ")")
          };
        } else if (type === 'semi-circle') {
          var direction = this.direction;
          var _step = Math.PI / (length - 1);
          var x = "calc(".concat(radius * Math.cos(_step * index), "px + ").concat($dt('item.diff.x', '0px'), ")");
          var y = "calc(".concat(radius * Math.sin(_step * index), "px + ").concat($dt('item.diff.y', '0px'), ")");
          if (direction === 'up') {
            return {
              left: x,
              bottom: y
          ...
          var _x = "calc(".concat(radius * Math.cos(_step2 * index), "px + ").concat($dt('item.diff.x', '0px'), ")");
          var _y = "calc(".concat(radius * Math.sin(_step2 * index), "px + ").concat($dt('item.diff.y', '0px'), ")");
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return openBlock(), createElementBlock(Fragment, null, [createElementVNode("div", mergeProps({
    ...
    return [createVNode(_component_Button, mergeProps({
      "class": [_ctx.cx('button'), _ctx.buttonClass],
      ...
      icon: withCtx(function (slotProps) {
        return [renderSlot(_ctx.$slots, "icon", {
          ...
          return [$data.d_visible && !!_ctx.hideIcon ? (openBlock(), createBlock(resolveDynamicComponent(_ctx.hideIcon ? 'span' : 'PlusIcon'), mergeProps({
            key: 0,
            "class": [_ctx.hideIcon, _ctx.cx('buttonIcon'), slotProps["class"]]
    ...
    return openBlock(), createElementBlock(Fragment, {
    ...
    }, $options.getPTOptions("".concat($data.id, "_").concat(index), 'menuitem')), [!_ctx.$slots.item ? withDirectives((openBlock(), createBlock(_component_Button, mergeProps({
      key: 0,
      tabindex: -1,
      role: "menuitem",
      ...
      pt: $options.getPTOptions("".concat($data.id, "_").concat(index), 'action')
    }), createSlots({
      _: 2
    }, [item.icon ? {
      name: "icon",
      fn: withCtx(function (slotProps) {
        return [createElementVNode("span", mergeProps({
          "class": [_ctx.cx('actionIcon'), item.icon, slotProps["class"]]
