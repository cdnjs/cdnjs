...
this.primevue.speeddial = (function (Button, PlusIcon, Ripple, themes, Tooltip, utils, BaseComponent, SpeedDialStyle, vue) {
    'use strict';
    ...
    var script = {
      ...
      mounted: function mounted() {
        ...
        if (this.type !== 'linear') {
          var button = utils.DomHandler.findSingle(this.container, '[data-pc-name="pcbutton"]');
          var firstItem = utils.DomHandler.findSingle(this.list, '[data-pc-section="item"]');
          if (button && firstItem) {
            var wDiff = Math.abs(button.offsetWidth - firstItem.offsetWidth);
            var hDiff = Math.abs(button.offsetHeight - firstItem.offsetHeight);
            this.list.style.setProperty(themes.$dt('item.diff.x').name, "".concat(wDiff / 2, "px"));
            this.list.style.setProperty(themes.$dt('item.diff.y').name, "".concat(hDiff / 2, "px"));
          }
        }
      ...
      methods: {
        ...
        onEnterKey: function onEnterKey(event) {
          var _this = this;
          var items = utils.DomHandler.find(this.container, '[data-pc-section="item"]');
        ...
        changeFocusedOptionIndex: function changeFocusedOptionIndex(index) {
          ...
          if (filteredItems[index]) {
            ...
            var buttonEl = utils.DomHandler.findSingle(filteredItems[index], '[type="button"]');
            buttonEl && utils.DomHandler.focus(buttonEl);
          }
        },
        ...
        calculatePointStyle: function calculatePointStyle(index) {
          ...
          if (type !== 'linear') {
            ...
            if (type === 'circle') {
              ...
              return {
                left: "calc(".concat(radius * Math.cos(step * index), "px + ").concat(themes.$dt('item.diff.x', '0px').variable, ")"),
                top: "calc(".concat(radius * Math.sin(step * index), "px + ").concat(themes.$dt('item.diff.y', '0px').variable, ")")
              };
            } else if (type === 'semi-circle') {
              var direction = this.direction;
              var _step = Math_PI / (length - 1);
              var x = "calc(".concat(radius * Math.cos(_step * index), "px + ").concat(themes.$dt('item.diff.x', '0px').variable, ")");
              var y = "calc(".concat(radius * Math.sin(_step * index), "px + ").concat(themes.$dt('item.diff.y', '0px').variable, ")");
              if (direction === 'up') {
                return {
                  left: x,
                  bottom: y
              ...
              var _x = "calc(".concat(radius * Math.cos(_step2 * index), "px + ").concat(themes.$dt('item.diff.x', '0px').variable, ")");
              var _y = "calc(".concat(radius * Math.sin(_step2 * index), "px + ").concat(themes.$dt('item.diff.y', '0px').variable, ")");
    ...
    var _hoisted_1 = ["id"];
    var _hoisted_2 = ["id", "aria-controls", "data-p-active"];
    function render(_ctx, _cache, $props, $setup, $data, $options) {
      ...
      return vue.openBlock(), vue.createElementBlock(vue.Fragment, null, [vue.createElementVNode("div", vue.mergeProps({
        ...
        return vue.openBlock(), vue.createElementBlock(vue.Fragment, {
          ...
          ref_for: true
        }, $options.getPTOptions("".concat($data.id, "_").concat(index), 'item')), [!_ctx.$slots.item ? vue.withDirectives((vue.openBlock(), vue.createBlock(_component_Button, vue.mergeProps({
          key: 0,
          tabindex: -1,
          role: "menuitem",
        ...
        }, null, 8, ["item", "onClick", "toggleCallback"]))], 16, _hoisted_2)) : vue.createCommentVNode("", true)], 64);
...
})(primevue.button, primevue.icons.plus, primevue.ripple, primevue.themes, primevue.tooltip, primevue.utils, primevue.basecomponent, primevue.speeddial.style, Vue);
