...
this.primevue.badgedirective = (function (utils, BadgeDirectiveStyle, BaseDirective) {
    ...
    var BadgeDirective = BaseBadgeDirective.extend('badge', {
      mounted: function mounted(el, binding) {
        ...
        var badge = utils.DomHandler.createElement('span', _defineProperty(_defineProperty({
          id: id,
        ...
        if (binding.value != null) {
          ...
          if (String(el.$_badgeValue).length === 1 && !this.isUnstyled()) {
            !this.isUnstyled() && utils.DomHandler.addClass(badge, 'p-badge-circle');
          }
        } else {
        ...
        if (binding.oldValue !== binding.value) {
          ...
          if (!this.isUnstyled()) {
            if (el.$_badgeValue) {
              ...
              if (el.$_badgeValue.length === 1) utils.DomHandler.addClass(badge, 'p-badge-circle');else utils.DomHandler.removeClass(badge, 'p-badge-circle');
