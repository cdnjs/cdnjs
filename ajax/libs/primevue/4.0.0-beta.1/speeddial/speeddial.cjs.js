  ...
  mounted: function mounted() {
    ...
    if (this.type !== 'linear') {
      ...
      if (button && firstItem) {
        ...
        this.list.style.setProperty(themes.$dtp('item.diff.x').name, "".concat(wDiff / 2, "px"));
        this.list.style.setProperty(themes.$dtp('item.diff.y').name, "".concat(hDiff / 2, "px"));
      }
    }
    ...
    calculatePointStyle: function calculatePointStyle(index) {
      ...
      if (type !== 'linear') {
        ...
        if (type === 'circle') {
          ...
          return {
            left: "calc(".concat(radius * Math.cos(step * index), "px + ").concat(themes.$dt('item.diff.x', '0px'), ")"),
            top: "calc(".concat(radius * Math.sin(step * index), "px + ").concat(themes.$dt('item.diff.y', '0px'), ")")
          };
        } else if (type === 'semi-circle') {
          var direction = this.direction;
          var _step = Math.PI / (length - 1);
          var x = "calc(".concat(radius * Math.cos(_step * index), "px + ").concat(themes.$dt('item.diff.x', '0px'), ")");
          var y = "calc(".concat(radius * Math.sin(_step * index), "px + ").concat(themes.$dt('item.diff.y', '0px'), ")");
          if (direction === 'up') {
            return {
              left: x,
              bottom: y
          ...
          var _x = "calc(".concat(radius * Math.cos(_step2 * index), "px + ").concat(themes.$dt('item.diff.x', '0px'), ")");
          var _y = "calc(".concat(radius * Math.sin(_step2 * index), "px + ").concat(themes.$dt('item.diff.y', '0px'), ")");
