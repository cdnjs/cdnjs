...
var render = function render() {
  ...
  return _c('span', {
  ...
  }, 'input', _vm.$attrs, false), _vm.listeners)) : _vm._e(), _vm.multiple ? _c('ul', {
    ref: "multiContainer",
    ...
    }, [_vm._v(_vm._s(_vm.getItemContent(item)))]), _c('span', {
  ...
  }, 'input', _vm.$attrs, false), _vm.listeners))])], 2) : _vm._e(), _vm.searching ? _c('i', {
    staticClass: "p-autocomplete-loader pi pi-spinner pi-spin"
  }) : _vm._e(), _vm.dropdown ? _c('Button', {
    ref: "dropdownButton",
  ...
  methods: {
    onOverlayEnter: function onOverlayEnter() {
      ...
      if (this.autoHighlight && this.suggestions && this.suggestions.length) {
        DomHandler_DomHandler.addClass(this.$refs.overlay.firstElementChild.firstElementChild, 'p-highlight');
      }
    },
    ...
    alignOverlay: function alignOverlay() {
      ...
      if (this.appendTo) DomHandler_DomHandler.absolutePosition(this.$refs.overlay, target);else DomHandler_DomHandler.relativePosition(this.$refs.overlay, target);
    },
    ...
    getItemContent: function getItemContent(item) {
      return this.field ? ObjectUtils_ObjectUtils.resolveFieldData(item, this.field) : item;
    },
    ...
    onKeyDown: function onKeyDown(event) {
      if (this.overlayVisible) {
        var highlightItem = DomHandler_DomHandler.findSingle(this.$refs.overlay, 'li.p-highlight');
        switch (event.which) {
          //down
          case 40:
            if (highlightItem) {
              ...
              if (nextElement) {
                DomHandler_DomHandler.addClass(nextElement, 'p-highlight');
                DomHandler_DomHandler.removeClass(highlightItem, 'p-highlight');
                DomHandler_DomHandler.scrollInView(this.$refs.overlay, nextElement);
              }
            } else {
              DomHandler_DomHandler.addClass(this.$refs.overlay.firstChild.firstElementChild, 'p-highlight');
            }
          ...
          case 38:
            if (highlightItem) {
              ...
              if (previousElement) {
                DomHandler_DomHandler.addClass(previousElement, 'p-highlight');
                DomHandler_DomHandler.removeClass(highlightItem, 'p-highlight');
                DomHandler_DomHandler.scrollInView(this.$refs.overlay, previousElement);
              }
            }
            event.preventDefault();
            break;
          ...
          case 13:
            if (highlightItem) {
              this.selectItem(event, this.suggestions[DomHandler_DomHandler.index(highlightItem)]);
              this.hideOverlay();
            }
            event.preventDefault();
            break;
    ...
    onChange: function onChange(event) {
      if (this.forceSelection) {
        ...
        if (this.suggestions) {
          var _iterator = AutoCompletevue_type_script_lang_js_createForOfIteratorHelper(this.suggestions),
            _step;
          try {
            for (_iterator.s(); !(_step = _iterator.n()).done;) {
              var item = _step.value;
              var itemValue = this.field ? ObjectUtils_ObjectUtils.resolveFieldData(item, this.field) : item;
    ...
    isSelected: function isSelected(val) {
      ...
      if (this.value && this.value.length) {
        for (var i = 0; i < this.value.length; i++) {
          if (ObjectUtils_ObjectUtils.equals(this.value[i], val)) {
            selected = true;
            break;
          }
        }
    ...
    inputValue: function inputValue() {
      if (this.value) {
        if (this.field && _typeof(this.value) === 'object') {
          var resolvedFieldData = ObjectUtils_ObjectUtils.resolveFieldData(this.value, this.field);
