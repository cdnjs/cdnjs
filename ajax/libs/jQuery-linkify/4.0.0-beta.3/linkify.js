var linkify = (function (exports) {
	...
	var assign = Object.assign || function (target, properties) {
	...
	******************************************************************************/
	...
	var scheme = [SCHEME, SLASH_SCHEME, COMPOUND_SCHEME, COMPOUND_SLASH_SCHEME];
	var collections = {
	  ascii: ascii,
...
한국'.split(' ');
	...
	*/
	...
	function Options(opts, defaultRender) {
	  ...
	  if (opts) {
	    assign(o, opts instanceof Options ? opts.o : opts);
	...
	*/
	...
	MultiToken.prototype = {
	  ...
	  */
	   ...
	   * @returns {string}
	   */
	  toFormattedString: function toFormattedString(opts) {
	    var val = this.toString();
	    var truncate = opts.get('truncate', val, this);
	    var formatted = opts.get('format', val, this);
	   ...
	   * @returns {string}
	   */
	  toFormattedHref: function toFormattedHref(opts) {
	  ...
	  */
	   ...
	   */
	  toFormattedObject: function toFormattedObject(opts) {
	    return {
	      type: this.t,
	  ...
	  render: function render(opts) {
	    var token = this;
	    var tagName = opts.get('tagName', href, token);
	    var href = this.toFormattedHref(opts);
	    var content = this.toFormattedString(opts);
	    var attributes = {};
	    var className = opts.get('className', href, token);
	    var target = opts.get('target', href, token);
	    var rel = opts.get('rel', href, token);
	    var attrs = opts.getObj('attributes', href, token);
	    var eventListeners = opts.getObj('events', href, token);
	...
	*/
	...
	function find(str, type, options) {
	  if (type === void 0) {
	    type = null;
	  }
