...
function addToCollections(t, collections, flags) {
  if (flags[numeric]) {
  ...
  for (var k in flags) {
    var collection = registerCollection(k, collections);

    if (collection.indexOf(t) < 0) {
      collection.push(t);
    }
  }
}
...
State.prototype = {
   ...
   */
  ta: function ta(inputs, next, collections, flags) {
   ...
   */
  tr: function tr(regexp, next, collections, flags) {
    var nextState;
   ...
   * @returns {State<T>} taken after the given input
   */
  ts: function ts(input, next, collections, flags) {
    var state = this;
   ...
   * @returns {State<T>} taken after the given input
   */
  tt: function tt(input, next, collections, flags) {
    ...
    var t = next; // // Check for existing easy (non-regex) transition
    ...
    if (t) {
      ...
      if (collections) {
        if (nextState.t) {
          var allFlags = assign(collectionFlagsForToken(nextState.t, collections), flags);
...
var ta = function ta(state, input, next, collections, flags) {
  return state.ta(input, next, collections, flags);
};
/**
 * @template T
 * @param {State<T>} state
...
var tr = function tr(state, regexp, next, collections, flags) {
  return state.tr(regexp, next, collections, flags);
};
/**
 * @template T
 * @param {State<T>} state
...
var ts = function ts(state, input, next, collections, flags) {
  return state.ts(input, next, collections, flags);
};
/**
 * @template T
 * @param {State<T>} state
...
var tt = function tt(state, input, next, collections, flags) {
  return state.tt(input, next, collections, flags);
};
...
function init$2(customSchemes) {
  ...
  tt(Start, '\\', BACKSLASH);
  var Num = tr(Start, DIGIT, NUM, groups, (_tr = {}, _tr[numeric] = true, _tr));
  tr(Num, DIGIT, Num); // State which emits a word token

  var Word = tr(Start, ASCII_LETTER, WORD, groups, (_tr2 = {}, _tr2[ascii] = true, _tr2));
  tr(Word, ASCII_LETTER, Word); // Same as previous, but specific to non-fsm.ascii alphabet words

  var UWord = tr(Start, LETTER, UWORD, groups, (_tr3 = {}, _tr3[alpha] = true, _tr3));
  ...
  var Ws = tr(Start, SPACE, WS, groups, (_tr4 = {}, _tr4[whitespace] = true, _tr4));
  tt(Start, NL, NL$1, groups, (_tt = {}, _tt[whitespace] = true, _tt));
  tt(Ws, NL); // non-accepting state to avoid mixing whitespaces

  tr(Ws, SPACE, Ws); // Emoji tokens. They are not grouped by the scanner except in cases where a
  // zero-width joiner is present

  var Emoji = tr(Start, EMOJI, EMOJI$1, groups, (_tr5 = {}, _tr5[emoji] = true, _tr5));
  ...
  addToCollections(TLD, groups, {
    tld: true,
    ascii: true
  });
  addToCollections(UTLD, groups, {
    utld: true,
    alpha: true
  ...
  addToCollections(SCHEME, groups, {
    scheme: true,
    ascii: true
  });
  addToCollections(SLASH_SCHEME, groups, {
    slashscheme: true,
    ascii: true
  ...
  ts(Start, 'localhost', LOCALHOST, groups, {
    ascii: true
