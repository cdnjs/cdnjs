...
(function () {
  ...
  function objectValuesSubset(obj, model) {
    ...
    for (var key in model) {
      if (hasOwn$4.call(model, key) && hasOwn$4.call(obj, key)) {
  ...
  function extend(a, b, undefOnly, allProperties) {
    for (var prop in b) {
      if (hasOwn$4.call(b, prop) || allProperties) {
        if (b[prop] === undefined) {
          delete a[prop];
  ...
  var config = {
    ...
    _globalHooks: {},
    _pq: null,
    // ProcessingQueue singleton, assigned in /src/core.js
    _runStarted: false,
  ...
  Test.prototype = {
    ...
    pushResult: function pushResult(resultInfo) {
      ...
      if (hasOwn$4.call(resultInfo, 'expected')) {
  ...
  function addTest(settings) {
    ...
    !(settings.callback && settings.callback.validTest) && (focused || config.currentModule.ignored)) {
      return;
    }
  ...
  var TapReporter = /*#__PURE__*/function () {
    ...
    return _createClass(TapReporter, [{
      ...
      value: function logAssertion(error, severity) {
        ...
        if (hasOwn$3.call(error, 'actual')) {
          out += "\n  actual  : ".concat(prettyYamlValue(error.actual));
        }
        if (hasOwn$3.call(error, 'expected')) {
  ...
  var HtmlReporter = /*#__PURE__*/function () {
    ...
    return _createClass(HtmlReporter, [{
      ...
      value: function onToolbarChanged(ev) {
        ...
        if (field.name === 'hidepassed' && 'replaceState' in window$1.history) {
          ...
          if (field.checked) {
            ...
            var _iterator = _createForOfIteratorHelper(this.hiddenTests),
              _step;
            try {
      ...
      key: "appendInterface",
      value: function appendInterface(beginDetails) {
        // Since QUnit 1.3, these are created automatically.
        this.element.setAttribute('role', 'main');
        this.element.innerHTML = '<div id="qunit-header">' + '<h1><a href="' + escapeText(this.unfilteredUrl) + '">' + escapeText(document.title) + '</a></h1> ' + '<div id="qunit-userAgent">' + escapeText('QUnit ' + version + '; ' + navigator.userAgent) + '</div>' + '</div>' + '<div id="qunit-toolbar" role="navigation">' + '<div id="qunit-banner"></div>' + '<div id="qunit-testrunner-toolbar"></div>' + '<div id="qunit-testresult" class="result">' + '<div id="qunit-testresult-controls"></div>' + '<div id="qunit-testresult-display">Running...<br />&#160;</div>' + '</div>' + this.appendFilteredTest() + '</div>' + '<ol id="qunit-tests"></ol>';
      ...
      key: "onError",
      value: function onError(error) {
        var testItem = this.elementTests && this.appendTest('global failure');
        if (!testItem) {
