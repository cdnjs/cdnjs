...
const createStructureSetupElements = t => {
  ...
  const {_t: Y, vt: W, ln: J, L: Q, ht: nt} = X;
  const ot = [ () => {
    removeAttrs(W, [ j, P ]);
    removeAttrs(Y, P);
    if (g) {
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const {vt: c, ln: r, L: i, V: a, Kt: u, gt: f, bt: d, yt: _, un: p} = t;
  const {U: v} = e;
  const g = d && a;
  const h = bind(o, 0);
  const b = {
  ...
  const B = getStaticPluginModuleInstance(xt);
  ...
  return ({zt: n, Qt: o, fn: l, Tt: a}, {_n: u}) => {
    ...
    const x = B && B.R(t, s, l, e, n);
    const {X: H, Y: E, W: D} = x || {};
    const [F, X] = getShowNativeOverlaidScrollbars(n, e);
    ...
    const vt = u || w || S || ft || at || ct || it || W || X || Q || d && g;
    const [gt, ht] = U(a);
    const bt = w || y || ht || _t || a;
    const [wt, yt] = bt ? P(getMeasuredScrollCoordinates(gt), a) : q();
