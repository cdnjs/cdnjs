...
var OverlayScrollbarsGlobal = function(t) {
  ...
  const overflowCssValueToOverflowStyle = t => t ? [ E, z, H ].includes(t) ? t : E : E;
    ...
    [Ht]: {
      static: () => ({
        R: (t, n, o, s, e) => {
          ...
          const _hideNativeScrollbars = (t, {j: o}, s) => {
            if (!c) {
              const e = assignDeep({}, {
                [S]: 0,
                [m]: 0,
  ...
  const createDOMObserver = (t, n, o, s) => {
    ...
    const [_, p] = createEventContentChange(t, f, i);
    const v = c || [];
    const g = r || [];
    const h = concat(v, g);
  ...
  const createObserversSetup = (t, n, o, s) => {
    ...
    const {_t: _, vt: v, L: g, gt: h, ht: b, V: w, bt: y, wt: S, yt: m, St: O} = t;
  ...
  const createScrollbarsSetupElements = (t, n, o, s) => {
    ...
    const {slot: a} = l;
    const {_t: u, vt: f, L: d, At: _, gt: p, bt: g, V: h} = n;
  ...
  const createScrollbarsSetupEvents = (t, n, o, s) => (e, i, l) => {
    const {vt: a, L: f, V: d, gt: _, Jt: p, St: v} = n;
    ...
    const createInteractiveScrollEvents = () => {
      ...
      return addEventListener(h, "pointerdown", (o => {
        ...
        if (x) {
          ...
          const O = c(getBoundingClientRect(_)[e]) / A(_)[a] || 1;
          ...
          const k = [ addEventListener(p, n, releasePointerCapture), addEventListener(p, "selectstart", (t => preventDefault(t)), {
            I: false
          }), addEventListener(h, n, releasePointerCapture), D && addEventListener(h, "pointermove", (t => $(T + (t[s] - C)))), D && (() => {
  ...
  const createScrollbarsSetup = (t, n, o, s, e, c) => {
    ...
    const {vt: $, Kt: C, bt: x} = e;
    const {Bt: E, Pt: I, Nt: T, qt: A, jt: D} = m;
  ...
  const createStructureSetupElements = t => {
    ...
    const k = !H && y(S, r, d);
    const R = !I && z;
    const V = [ R, T, k, D ].map((t => isHTMLElement(t) && !parent(t) && t));
    ...
    const q = {
      ...
      wt: t => hasAttrClass(T, X, t),
      yt: (t, n) => addRemoveAttrClass(T, X, t, n),
      St: () => addRemoveAttrClass(U, X, K, true)
    };
    const {_t: Y, vt: W, ln: G, L: J, ht: Z} = q;
    const ot = [ () => {
      removeAttrs(W, [ B, N ]);
      removeAttrs(Y, N);
      if (g) {
  ...
  const createOverflowUpdateSegment = (t, n) => {
    ...
    const {vt: c, ln: r, L: i, V: l, Kt: u, gt: f, bt: d, yt: _, un: p} = t;
    const {U: v} = e;
    const g = d && l;
    const h = bind(s, 0);
    const b = {
    ...
    const [m, C] = createCache(y, bind(getFractionalSize, i));
    const [x, I] = createCache(y, bind(M, i));
    const [T, A] = createCache(y);
    ...
    const [P, N] = createCache({
      i: (t, n) => equalXY(t.D, n.D) && equalXY(t.M, n.M),
      o: getZeroScrollCoordinates()
    });
    const j = getStaticPluginModuleInstance(Ht);
    const createViewportOverflowStyleClassName = (t, n) => {
      const o = n ? Y : W;
      return `${o}${capitalizeFirstLetter(t)}`;
    };
    return ({zt: o, Qt: s, fn: l, Tt: a}, {_n: u}) => {
      const {dt: f, Ht: d, xt: b, $t: w, ft: y, Et: S} = s || {};
      const H = j && j.R(t, n, l, e, o);
      const {X: E, Y: z, W: M} = H || {};
      const [F, X] = getShowNativeOverlaidScrollbars(o, e);
      const [Y, W] = o("overflow");
      const G = overflowIsVisible(Y.x);
      const J = overflowIsVisible(Y.y);
      const K = f || u || b || w || S || X;
      let Z = C(a);
      let nt = I(a);
      let ot = A(a);
      ...
      const pt = G && J && (dt.x || dt.y) || G && dt.x && !dt.y || J && dt.y && !dt.x;
      const vt = u || w || S || ft || at || ct || it || W || X || K || d && g;
      const [gt, ht] = U(a);
      const bt = w || y || ht || _t || a;
      const [wt, yt] = bt ? P(getMeasuredScrollCoordinates(gt), a) : N();
      let St = getViewportOverflowStyle(dt, Y);
      setMeasuringMode(false);
      if (vt) {
        setViewportOverflowStyle(St);
        const {overflowX: t, overflowY: n} = getStyles(i, [ O, $ ]);
        St = {
          x: overflowCssValueToOverflowStyle(t),
          y: overflowCssValueToOverflowStyle(n)
        };
        if (M && E) {
          setStyles(i, M(St, l, E(St, lt, ut)));
        }
      }
      const [mt, Ot] = L(St);
      addRemoveAttrClass(c, B, q, pt);
      addRemoveAttrClass(r, tt, q, pt);
      assignDeep(n, {
        cn: mt,
        Vt: {
          x: et.w,
  ...
  const OverlayScrollbars = (t, n, o) => {
    ...
    if (n && !r) {
      ...
      const [_, p, v] = createEventListenerHub(o);
      const triggerEvent = (t, n) => {
        v(t, n);
        d(t, n);
      };
      const [g, h, b, w, y] = createSetups(t, a, (() => r), (({vn: t, Tt: n}, {Qt: o, Zt: s}) => {
      ...
      const S = {
        ...
        elements() {
          const {_t: t, vt: n, ln: o, L: s, ht: e, gt: c, Kt: r} = w.wn;
          const {Ft: i, Gt: l} = w.yn;
