...
const createObserversSetup = (t, n, o, s) => {
  ...
  const {vt: p, ht: v, ot: h, gt: g, bt: b, nt: y, yt: w, wt: S, St: m, Ot: O} = t;
...
const createScrollbarsSetupEvents = (t, n, o, s) => (r, l, i) => {
  const {ht: u, ot: _, nt: d, gt: f, Kt: p, Ot: v} = n;
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  const {Ft: H, Nt: z, qt: I, jt: A, Bt: D} = S;
  ...
  const M = [ w, h, b, p, () => _(), addEventListener(O, "pointerover", onHostMouseEnter, {
    A: true
...
const createStructureSetupElements = t => {
  ...
  const j = !elementIsGenerated(M) && elementHasOverflow(M) ? M : v;
  const B = E ? b : M;
  const F = E ? g : M;
  const X = {
    vt: v,
    ...
    wt: t => hasAttrClass(M, q, t),
    St: (t, n) => addRemoveAttrClass(M, q, t, n),
    Ot: () => addRemoveAttrClass(B, q, Y, true)
  };
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const {ht: c, ln: r, ot: l, nt: a, Qt: u, gt: _, yt: d, St: f, un: p} = t;
  const {R: v} = e;
  const h = d && a;
  const g = bind(o, 0);
  const b = {
  ...
  return ({It: n, Zt: o, _n: i, Dt: a}, {fn: u}) => {
    ...
    const dt = u || b || w || it || rt || ot || et || F || j || J;
    const ft = createViewportOverflowState(at, B);
    const [pt, vt] = R(ft.K);
    const [ht, gt] = V(a);
    const bt = b || y || gt || ut || a;
    const [yt, wt] = bt ? U(getMeasuredScrollCoordinates(ht), a) : N();
    if (dt) {
