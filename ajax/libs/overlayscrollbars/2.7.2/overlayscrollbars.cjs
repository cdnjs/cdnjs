...
const createEnvironment = () => {
  ...
  const p = bind(assignDeep, {}, v);
  const h = bind(assignDeep, {}, f);
  const g = {
    P: a,
    T: d,
    L: u,
    ...
    Z: bind(c, "r"),
    G: h,
    tt: t => assignDeep(f, t) && h(),
    nt: p,
    ot: t => assignDeep(v, t) && p(),
    st: assignDeep({}, f),
    et: assignDeep({}, v)
  };
  removeAttrs(s, "style");
...
const createStructureSetupElements = t => {
  ...
  const d = u ? {} : t;
  const {elements: f} = d;
  const {host: _, padding: v, viewport: p, content: h} = f || {};
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  return ({Et: n, Gt: a, an: p, Tt: h}, {dn: I}) => {
    const {ht: E, Ht: A, xt: T, It: D} = a || {};
