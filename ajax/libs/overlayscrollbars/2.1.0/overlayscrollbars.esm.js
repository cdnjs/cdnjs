...
const vt = "never scroll leavemove";
...
const createOverflowUpdateSegment = (t, n) => {
  ...
  const [H, P] = v ? v.H(w, _, r, i, o, getViewportOverflowState, hideNativeScrollbars) : [ () => w, () => [ noop ] ];
  return (t, n, i) => {
    const {vt: u, Ot: f, wt: v, xt: w, gt: M, yt: D} = t;
    const {ht: B, bt: V} = o();
    const [Y, j] = n("showNativeOverlaidScrollbars");
    const [N, q] = n("overflow");
    const F = Y && g.x && g.y;
    const G = !l && !_ && (u || v || f || j || M);
...
const createStructureSetup = (t, n) => {
  ...
  const [_, h, g, v] = createStructureSetupObservers(r, d, (t => triggerUpdateEvent(f(o, t), {}, false)));
  const w = u.bind(0);
  w.jt = t => s("u", t);
  w.Nt = () => {
    const {W: t, J: n} = r;
...
const createInteractiveScrollEvents = (t, n, o, s, e, c) => {
  ...
  return on(l, "pointerdown", (o => {
    ...
    if (continuePointerDown(o, t, e)) {
      ...
      const r = createRelativeHandleMove(s[u] || 0, 1 / getScale(s)[g]);
      const a = o[d];
      const h = getHandleRect();
      const v = getTrackRect();
      const w = h[f];
      const p = getHandleOffset(h, v) + w / 2;
      const b = a - v[_];
      const m = e ? 0 : b - p;
...
const createScrollbarsSetupElements = (t, n, o) => {
  ...
  const {slot: r} = c;
  const {ct: i, W: l, Z: a, J: u, lt: d, ot: f, it: h, ut: g} = n;
  const {scrollbars: v} = d ? {} : t;
  const {slot: w} = v || {};
  const p = dynamicInitializationElement([ l, a, u ], (() => g && h ? l : a), r, w);
