...
const domTokenListAttr = (t, n) => {
  ...
  return {
    S: t => s(domTokenListOperation(t, "delete")),
...
const cancelInitialization = (t, n) => {
  ...
  const {k: e, R: c, L: r} = getEnvironment();
...
const createObserversSetup = (t, n, o, s) => {
  ...
  let i;
  let a;
  const {R: u} = getEnvironment();
  const d = `[${B}]`;
  const _ = `[${Y}]`;
  const p = [];
  const v = [ "wrap", "cols", "rows" ];
  const h = [ "id", "class", "style", "open" ];
  const {ft: g, vt: b, nt: w, ht: y, gt: m, bt: S, tt: O, wt: $, yt: C} = t;
  ...
  const {V: T} = H;
  const [D, k] = m ? createTrinsicObserver(b, onTrinsicChanged) : [];
  const R = !O && createSizeObserver(b, onSizeChanged, {
    dt: true
  });
  const [M, L] = createDOMObserver(b, false, onHostMutation, {
    B: h,
    F: concat(h, p)
  });
  const P = O && f && new f((t => {
  ...
  return [ () => {
    ...
    if (g) {
      r && r();
      l && l();
      const [t, n] = createDOMObserver(m || w, true, onContentMutation, {
        F: concat(A, a || []),
        X: f,
        Y: d,
        J: (t, n) => {
  ...
  const createInteractiveScrollEvents = () => {
    ...
    return addEventListener(b, "pointerdown", (o => {
      ...
      if (m) {
        ...
        const f = e(getBoundingClientRect(p)[H]) / R(p)[l] || 1;
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  return [ () => bind(runEachAndClear, push(I, S())), ({Et: t, At: n, Kt: e, Gt: c}) => {
    ...
    const [R, M] = t("scrollbars.visibility");
    ...
    const setScrollbarVisibility = (t, n, o) => {
      const s = t.includes(T) && (R === A || R === "auto" && n === T);
      x(gt, s, o);
      return s;
    };
    _ = N;
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const {vt: c, en: r, nt: l, tt: a, Jt: u, ht: d, Tt: _, yt: f, rn: p} = t;
  const {R: v} = e;
  const h = _ && a;
  const g = bind(o, 0);
  ...
  const getFlowDirectionStyles = () => assignDeep({}, hasDimensions(l) ? getStyles(l, b) : {});
  ...
  const setViewportOverflow = t => {
    ...
    each(keys(n), (t => {
      ...
      f(s.join(" "));
      f(createViewportOverflowStyleClassName(n[t], o), true);
    }));
  };
  return ({Et: n, Kt: i, ln: a, At: u}, {un: d}) => {
    const {_t: _, Ct: b, Ot: w, dt: y, Ht: A} = i || {};
    const I = P && P.Z(t, s, a, e, n);
    const {lt: T, it: V, ut: U} = I || {};
    ...
    if (J) {
      ...
      const r = e;
      t && t();
      const i = getWindowSize(p());
      const a = {
        w: g(o(s.w, c.w) + n.w),
        h: g(o(s.h, c.h) + n.h)
      };
      const d = {
        w: g((h ? i.w : r.w + g(e.w - s.w)) + n.w),
        h: g((h ? i.h : r.h + g(e.h - s.h)) + n.h)
      };
      tt = E(d);
      Z = C(getOverflowAmount(a, d), u);
    }
    const [ot, st] = tt;
    const [et, ct] = Z;
    const [rt, lt] = G;
    ...
    const ft = d || w || A || at || lt || st || ct || X || j || J;
    const pt = createViewportOverflowState(ut, F);
    const [vt, ht] = D(pt.K);
    const [, gt] = k(getFlowDirectionStyles(), u);
    const bt = w || y || gt || dt || u;
    const [wt, yt] = bt ? R(measureScrollCoordinates(), u) : L();
    if (ft) {
      setViewportOverflow(pt);
      if (U && T) {
        setStyles(l, U(pt, a, T(pt, rt, it)));
      }
    }
    ...
    assignDeep(s, {
      ...
      sn: ut,
      Lt: sanitizeScrollCoordinates(wt, et)
    });
    return {
      tn: ht,
      Qt: st,
...
const OverlayScrollbars = (t, n, o) => {
  ...
  if (n && !r) {
    ...
    const [h, g, b, w, y] = createSetups(t, a, (({_n: t, At: n}, {Kt: o, Gt: s}) => {
    ...
    const m = {
      ...
      state() {
        const {pn: t, vn: n} = b();
        const {et: o} = t;
        const {Mt: s, Rt: e, K: c, sn: l, en: i, an: a, Lt: u} = n;
        return assignDeep({}, {
          overflowEdge: s,
          overflowAmount: e,
          overflowStyle: c,
      ...
      elements() {
        const {ft: t, vt: n, en: o, nt: s, gt: e, ht: c, Jt: r} = w.hn;
        const {qt: l, Yt: i} = w.gn;
    ...
    if (cancelInitialization(w.hn.Tt, !e && t.cancel)) {
      destroy(true);
      return m;
    }
    push(l, h());
...
OverlayScrollbars.env = () => {
  const {D: t, k: n, R: o, M: s, j: e, q: c, L: r, P: l, U: i, N: a} = getEnvironment();
  return assignDeep({}, {
    scrollbarsSize: t,
...
export { Mt as ClickScrollPlugin, OverlayScrollbars, kt as ScrollbarsHidingPlugin, Tt as SizeObserverPlugin };
