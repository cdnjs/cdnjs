...
const createObserversSetup = (t, n, o, s) => {
  ...
  const {vt: p, gt: v, ot: g, ht: h, bt: b, nt: y, yt: w, wt: S, St: m, Ot: O} = t;
  ...
  const [T, M] = createDOMObserver(v, false, onHostMutation, {
    X: d,
    F: d
  });
  const k = y && f && new f((t => {
  ...
  const createInteractiveScrollEvents = () => {
    ...
    return addEventListener(h, "pointerdown", (o => {
      ...
      if (C) {
        ...
        const w = e(getBoundingClientRect(f)[r]) / I(f)[a] || 1;
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  const T = [ w, g, b, p, () => _(), addEventListener(O, "pointerover", onHostMouseEnter, {
    A: true
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const {gt: c, ln: r, ot: l, nt: a, Qt: u, ht: _, yt: d, St: f, un: p} = t;
  const {R: v} = e;
  const g = d && a;
  const h = bind(o, 0);
  const b = {
  ...
  return ({It: n, Zt: o, _n: i, Dt: a}, {fn: u}) => {
    ...
    const [yt, wt] = bt ? U(getMeasuredScrollCoordinates(gt), a) : N();
    if (dt) {
...
const OverlayScrollbars = (t, n, o) => {
  ...
  if (n && !r) {
    ...
    const [g, h, b, y, w] = createSetups(t, a, (() => r), (({vn: t, Dt: n}, {Zt: o, tn: s}) => {
