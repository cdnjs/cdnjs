...
var OverlayScrollbarsGlobal = function(t) {
  ...
  const createOverflowUpdateSegment = (t, n) => {
    ...
    const [w, S] = createCache(h, bind(V, l));
    const [y, m] = createCache(h);
    const [O, $] = createCache(h);
    const [C] = createCache(p);
    const I = getStaticPluginModuleInstance(Tt);
    return ({Et: o, Zt: u, un: h, Tt: p}, {_n: z}) => {
      const {ht: E, Ht: A, Ct: T, zt: D} = u || {};
      const k = I && I.M(t, n, h, e, o);
      const {q: R, W: V, X: L} = k || {};
      ...
      if (j) {
        ...
        const S = {
          w: v((d ? h.w : _.w + v(i.w - c.w)) + o.w),
          h: v((d ? h.h : _.h + v(i.h - c.h)) + o.h)
        };
        ct = O(S);
      ...
      const gt = z || T || D || vt || _t || it || ut || N || U || j;
      if (gt) {
        const n = {};
        const o = setViewportOverflowState(t, ht, B, n);
        L && L(o, h, !!R && R(o, ft, dt), n);
        if (i) {
          setAttrs(c, q, n[x]);
          setAttrs(c, W, n[H]);
        } else {
      ...
      addRemoveAttrClass(c, F, X, pt);
      addRemoveAttrClass(r, nt, ot, pt);
      if (!i) {
        addRemoveAttrClass(l, K, tt, J);
      }
