...
const cancelInitialization = (t, n) => {
  ...
  const {M: e, R: c, U: r} = getEnvironment();
...
const createObserversSetup = (t, n, o, s) => {
  ...
  const {vt: p, gt: v, ot: g, ht: h, bt: b, nt: w, wt: y, yt: S, St: m, Ot: O} = t;
  ...
  const [T, k] = createDOMObserver(v, false, onHostMutation, {
    X: d,
    j: d
  });
  const M = w && f && new f((t => {
...
const createScrollbarsSetupElements = (t, n, o, s) => {
  ...
  const {slot: a} = i;
  const {vt: u, gt: _, ot: d, Tt: f, ht: v, wt: g, nt: h} = n;
  const {scrollbars: b} = f ? {} : t;
  const {slot: w} = b || {};
  const y = [];
  const S = [];
  const m = [];
  const O = dynamicInitializationElement([ u, _, d ], (() => h && g ? u : _), a, w);
  ...
  const refreshScrollbarsHandleLength = () => {
    ...
    const createScrollbarStyleFn = t => n => [ n.kt, {
      [e]: roundCssNumber(t) + ""
    } ];
  ...
  const appendElements = () => {
    appendChildren(O, S[0].kt);
    appendChildren(O, m[0].kt);
...
const createScrollbarsSetupEvents = (t, n, o, s) => (r, l, i) => {
  const {gt: u, ot: _, nt: d, ht: f, Kt: p, Ot: v} = n;
  const {kt: g, Ut: h, Pt: b} = r;
  ...
  const createInteractiveScrollEvents = () => {
    ...
    return addEventListener(h, "pointerdown", (o => {
      ...
      const {pointers: $} = g;
      const C = w === 0 && y && g[i ? "dragScroll" : "clickScroll"] && ($ || []).includes(O);
      if (C) {
        runEachAndClear(_);
        m();
        const t = !i && o.shiftKey;
        const g = bind(getBoundingClientRect, b);
        const w = bind(getBoundingClientRect, h);
        const getHandleOffset = (t, n) => (t || g())[l] - (n || w())[l];
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  const [v, g] = selfClearTimeout(100);
  ...
  const T = [ y, g, b, p, () => _(), addEventListener(O, "pointerover", onHostMouseEnter, {
    A: true
...
const createStructureSetupElements = t => {
  ...
  const M = !H && y(S, r, d);
  const R = !A && z;
  const L = [ R, T, M, k ].map((t => isHTMLElement(t) && !parent(t) && t));
  const elementIsGenerated = t => t && inArray(L, t);
  const B = !elementIsGenerated(T) && elementHasOverflow(T) ? T : v;
  const F = E ? b : T;
  const j = E ? h : T;
  const X = {
    vt: v,
  ...
  const {vt: J, gt: Q, ln: Z, ot: tt, bt: nt} = X;
  const ot = [ () => {
    removeAttrs(Q, [ P, V ]);
    removeAttrs(J, V);
    if (g) {
  ...
  const appendElements = () => {
    ...
    focusElement(!H && g && n === J && t.top === t ? tt : n);
    c();
    st = 0;
    return ct;
  };
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const {gt: c, ln: r, ot: l, nt: a, Qt: u, ht: _, wt: d, St: f, un: p} = t;
  const {R: v} = e;
  const g = d && a;
  const h = bind(o, 0);
  ...
  const [O, $] = createCache(w, bind(D, l));
  const [C, z] = createCache(w);
  const [I] = createCache(y);
  const [T, k] = createCache(w);
  const [M] = createCache(y);
  const [R] = createCache({
    i: (t, n) => equal(t, n, b),
    o: {}
  }, (() => hasDimensions(l) ? getStyles(l, b) : {}));
  const [V, U] = createCache({
  ...
  return ({It: n, Zt: o, _n: i, Dt: a}, {fn: u}) => {
    ...
    const [pt, vt] = M(ft.K);
    const [, gt] = R(a);
    const ht = b || w || gt || ut || a;
    const [bt, wt] = ht ? V(a) : U();
    if (dt) {
    ...
    assignDeep(s, {
      ...
      rn: at,
      Lt: sanitizeScrollCoordinates(bt, st)
    });
    return {
      en: vt,
      nn: ot,
...
const OverlayScrollbars = (t, n, o) => {
  ...
  if (n && !r) {
    ...
    const [g, h, b, w, y] = createSetups(t, a, (() => r), (({vn: t, Dt: n}, {Zt: o, tn: s}) => {
    ...
    const S = {
      ...
      elements() {
        const {vt: t, gt: n, ln: o, ot: s, bt: e, ht: c, Qt: r} = w.wn;
        const {Xt: l, Gt: i} = w.yn;
        const translateScrollbarStructure = t => {
          const {Pt: n, Ut: o, kt: s} = t;
          return {
            scrollbar: s,
            track: o,
            handle: n
...
OverlayScrollbars.env = () => {
  const {k: t, M: n, R: o, V: s, B: e, F: c, U: r, P: l, N: i, q: a} = getEnvironment();
  return assignDeep({}, {
    scrollbarsSize: t,
