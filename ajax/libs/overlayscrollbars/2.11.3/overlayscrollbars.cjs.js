...
const createObserversSetup = (t, n, o, s) => {
  ...
  const {_t: p, vt: v, L: g, gt: h, ht: b, V: w, bt: y, wt: S, yt: m, St: O} = t;
...
const createScrollbarsSetupEvents = (t, n, o, s) => (r, i, l) => {
  const {vt: u, L: f, V: d, gt: _, Gt: p, St: v} = n;
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  const {Bt: z, Pt: I, Nt: T, qt: A, jt: D} = m;
...
const createStructureSetupElements = t => {
  ...
  const {_t: Y, vt: W, rn: J, L: Q, ht: nt} = X;
  const ot = [ () => {
    removeAttrs(W, [ j, P ]);
    removeAttrs(Y, P);
    if (g) {
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const {vt: c, rn: r, L: i, V: a, Kt: u, gt: f, bt: d, yt: _, an: p} = t;
  const {U: v} = e;
  const g = d && a;
  const h = bind(o, 0);
  const b = {
  ...
  return ({zt: n, Qt: o, un: l, Tt: a}, {dn: u}) => {
    ...
    const _t = u || w || S || at || it || st || ct || X || B || J || d && g;
    const [pt, vt] = V(a);
    ...
    const [yt, St] = R(wt);
    addRemoveAttrClass(c, j, N, dt);
    addRemoveAttrClass(r, Z, N, dt);
    assignDeep(s, {
      cn: yt,
      Vt: {
        x: ot.w,
