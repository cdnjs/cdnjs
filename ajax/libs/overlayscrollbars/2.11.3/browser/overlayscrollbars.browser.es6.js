...
var OverlayScrollbarsGlobal = function(t) {
  ...
  const createScrollbarsSetup = (t, n, o, s, e, c) => {
    ...
    const M = [ S, h, w, p, () => f(), addEventListener($, "pointerover", onHostMouseEnter, {
      A: true
  ...
  const createStructureSetupElements = t => {
    ...
    const u = a ? {} : t;
    const {elements: f} = u;
    const {padding: d, viewport: _, content: v} = f || {};
    ...
    const L = !elementIsGenerated(T) && elementHasOverflow(T) ? T : p;
    const U = E ? b : T;
    const P = E ? h : T;
    const q = {
      _t: p,
    ...
    const {_t: Y, vt: W, rn: G, L: J, ht: Z} = q;
    const ot = [ () => {
      removeAttrs(W, [ B, N ]);
      removeAttrs(Y, N);
      if (g) {
  ...
  const createOverflowUpdateSegment = (t, n) => {
    ...
    const {vt: c, rn: r, L: i, V: l, Kt: u, gt: f, bt: d, yt: _, an: v} = t;
    const {U: p} = e;
    const g = d && l;
    const h = bind(s, 0);
    const b = {
    ...
    return ({zt: o, Qt: s, un: l, Tt: a}, {dn: u}) => {
      ...
      const dt = Y && W && (ut.x || ut.y) || Y && ut.x && !ut.y || W && ut.y && !ut.x;
      const _t = u || w || S || at || it || st || ct || X || j || G || d && g;
      const [vt, pt] = V(a);
      const gt = w || y || pt || ft || a;
      const [ht, bt] = gt ? L(getMeasuredScrollCoordinates(vt), a) : U();
  ...
  const createSetups = (t, n, o, s, e) => {
    ...
    const [d, _, v] = createObserversSetup(u, a, r, (t => {
      update({}, t);
    }));
  ...
  const OverlayScrollbars = (t, n, o) => {
    ...
    if (n && !r) {
      ...
      const S = {
        ...
        elements() {
          const {_t: t, vt: n, rn: o, L: s, ht: e, gt: c, Kt: r} = w.bn;
