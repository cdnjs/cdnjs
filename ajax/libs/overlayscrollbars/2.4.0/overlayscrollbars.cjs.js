...
const createDOMObserver = (t, n, o, s) => {
  let e = false;
  const {Z: c, J: r, K: l, tt: i, nt: a, ot: u} = s || {};
...
const createSizeObserver = (t, n, o) => {
  let s = false;
  const e = 3333333;
  const {st: c, et: l, ct: i} = o || {};
  const a = getStaticPluginModuleInstance(Vt);
  const {U: u} = getEnvironment();
...
const createObserversSetup = (t, n) => {
  ...
  const d = {
    it: false,
    ut: getDirectionIsRTL(t.ft)
  };
  const {ft: v, _t: h, dt: p, vt: g, ht: b, gt: w, bt: y} = t;
  const {B: S} = getEnvironment();
  const [$] = createCache({
    _: equalWH,
    u: {
      w: 0,
  ...
  const onSizeChanged = ({lt: t, rt: o, et: s}) => {
    const e = t && !s && !o;
    const c = !e && l ? O : n;
    const [r, i] = o || [];
    o && assignDeep(d, {
...
const createScrollbarsSetupElements = (t, n, o) => {
  const {G: s, T: e} = getEnvironment();
  const {scrollbars: c} = s();
  const {slot: r} = c;
  const {It: i, ft: a, _t: u, At: f, Et: _, Tt: v, ht: h} = n;
  ...
  const refreshScrollbarsScrollbarOffsetTimeline = ({zt: t}) => {
    concat(O, x).forEach((({Dt: n}) => {
  ...
  const appendElements = () => {
    appendChildren(m, x[0].Dt);
    appendChildren(m, O[0].Dt);
    d((() => {
  ...
  return [ {
    ...
    Lt: refreshScrollbarsHandleOffset,
    Pt: refreshScrollbarsHandleOffsetTimeline,
    Vt: refreshScrollbarsScrollbarOffsetTimeline,
    Ut: refreshScrollbarsScrollbarOffset,
    Bt: scrollbarsAddRemoveClass,
    jt: {
      V: w,
      Nt: x,
      Gt: C,
      qt: bind(scrollbarStyle, x)
    },
    Wt: {
      V: y,
      Nt: O,
      Gt: z,
      qt: bind(scrollbarStyle, O)
    }
  }, appendElements ];
};

const createScrollbarsSetupEvents = (t, n, o) => {
  const {ft: s, Et: e, Ft: c} = n;
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  const {ft: z, Xt: H, Tt: I} = e;
  const {Bt: A, Mt: E, Lt: T, Pt: D, Vt: R, Ut: k} = O;
  ...
  return [ () => bind(runEachAndClear, push(M, C())), ({xt: t, Ct: n, Yt: e, Zt: c}) => {
    const {Jt: a, Kt: d, Qt: v} = c || {};
    const {yt: h, et: p} = e || {};
    const {ut: g} = o;
    const {A: b} = getEnvironment();
    const {zt: w, tn: y, nn: S} = s;
...
const createStructureSetupElements = t => {
  ...
  const q = {
    ...
    ht: E,
    rn: R,
    gt: (t, n) => hasAttrClass(V, E ? J : et, E ? n : t),
    bt: (t, n, o) => attrClass(V, E ? J : et, E ? n : t, o)
  };
  ...
  const {It: F, ft: X, sn: tt, _t: nt, dt: ot, en: st} = q;
  const ct = [ () => {
    removeAttr(X, J);
    removeAttr(X, Z);
    removeAttr(F, Z);
...
const createTrinsicUpdateSegment = ({dt: t}) => ({Yt: n, ln: o, Ct: s}) => {
...
const createPaddingUpdateSegment = ({ft: t, sn: n, _t: o, ht: s}, e) => {
  const [c, r] = createCache({
    _: equalTRBL,
    u: topRightBottomLeft()
  }, bind(topRightBottomLeft, t, "padding", ""));
  return ({xt: t, Yt: l, ln: i, Ct: a}) => {
    let [u, f] = r(a);
    const {I: _, B: d} = getEnvironment();
    const {lt: v, St: h, yt: p} = l || {};
    const {ut: g} = i;
...
const createOverflowUpdateSegment = ({ft: t, sn: n, _t: o, en: s, ht: e, bt: c, Tt: i, cn: a}, u) => {
  const f = bind(l, 0);
  const _ = "visible";
  const d = 42;
  const v = {
  ...
  const [q, W] = y ? y.D(m, g, o, s, u, getViewportOverflowState, hideNativeScrollbars) : [ () => m, () => [ noop ] ];
  return ({xt: s, Yt: r, ln: i, Ct: _}, {un: d}) => {
    const {lt: v, $t: h, St: p, wt: y, yt: m} = r || {};
    const {it: C, ut: z} = i;
    ...
    const [Ct, zt] = G(getViewportOverflowState(X).tn);
    assignDeep(u, {
      tn: Ct,
      Ht: {
        x: ht.w,
...
const createSetups = (t, n, o, s) => {
  ...
  const update = (t, s) => {
    const {_n: e, Ct: r, Ot: l, dn: i, vn: a} = t;
    const _ = e || {};
    const v = !!r;
    const h = {
      xt: createOptionCheck(n, _, v),
      _n: _,
      Ct: v
    };
    if (i) {
    ...
    y && o(t, {
      Yt: booleanUpdateHints(p),
      Zt: booleanUpdateHints(g)
    });
    return y;
  };
  return [ () => {
    const {It: t, _t: n, Ft: o, Tt: s} = l;
...
const OverlayScrollbars = (t, n, o) => {
  const {W: s, j: e, N: c} = getEnvironment();
  ...
  if (n && !i) {
    ...
    const [h, p, g, b, w] = createSetups(t, f, (({_n: t, Ct: n}, {Yt: o, Zt: s}) => {
      const {lt: e, yt: c, wt: r, St: l, $t: i} = o;
      const {Jt: a, Kt: u, Qt: f} = s;
      v("updated", [ y, {
        updateHints: {
          sizeChanged: e,
          directionChanged: c,
    ...
    const y = {
      ...
      state() {
        const {hn: t, pn: n} = g();
        const {ut: o} = t;
        const {Ht: s, zt: e, tn: c, nn: r, sn: l, an: a} = n;
        return assignDeep({}, {
          overflowEdge: s,
          overflowAmount: e,
          overflowStyle: c,
      ...
      elements() {
        const {It: t, ft: n, sn: o, _t: s, dt: e, Et: c, Xt: r} = b.gn;
        const {jt: l, Wt: i} = b.bn;
        const translateScrollbarStructure = t => {
          const {Rt: n, kt: o, Dt: s} = t;
          return {
            scrollbar: s,
            track: o,
            handle: n
    ...
    if (cancelInitialization(b.gn.Tt, !r && t.cancel)) {
      destroy(true);
      return y;
    }
    push(a, h());
...
OverlayScrollbars.env = () => {
  const {P: t, A: n, I: o, U: s, B: e, T: c, V: r, X: l, Y: i, G: a, q: u, W: f, F: _} = getEnvironment();
  return assignDeep({}, {
    scrollbarsSize: t,
