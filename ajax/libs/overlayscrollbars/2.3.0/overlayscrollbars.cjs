...
const createScrollTimelineEvents = ({Xt: t}, n, o) => {
  if (!n) {
    return noop;
  }
...
const createScrollbarsSetup = (t, n, o, s) => {
  ...
  const {K: T, ct: z, ut: E} = o.Gt;
  const {tn: A, Zt: I, Jt: H, Kt: L, Qt: M} = x;
  ...
  D.Ft = C;
  return [ (t, s, l) => {
    const {Lt: u, Mt: f, Ht: d, $t: _, Pt: h} = l;
    const {H: g} = getEnvironment();
    const v = createOptionCheck(n, t, s);
    const p = o();
    const {At: w, Tt: b, St: y, It: S} = p;
