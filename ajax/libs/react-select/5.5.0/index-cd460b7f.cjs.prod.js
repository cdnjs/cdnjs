...
var _templateObject, menuPortalCSS = function(_ref6) {
...
}, MenuPortal = function(_ref7) {
  var appendTo = _ref7.appendTo, children = _ref7.children, className = _ref7.className, controlElement = _ref7.controlElement, cx = _ref7.cx, innerProps = _ref7.innerProps, menuPlacement = _ref7.menuPlacement, menuPosition = _ref7.menuPosition, getStyles = _ref7.getStyles, menuPortalRef = React.useRef(null), cleanupRef = React.useRef(null), _useState = React.useState(coercePlacement(menuPlacement)), _useState2 = _slicedToArray__default.default(_useState, 2), placement = _useState2[0], setPlacement = _useState2[1], _useState3 = React.useState(null), _useState4 = _slicedToArray__default.default(_useState3, 2), computedPosition = _useState4[0], setComputedPosition = _useState4[1], getPortalPlacement = React.useCallback((function(_ref8) {
    var updatedPlacement = _ref8.placement;
    updatedPlacement !== placement && setPlacement(updatedPlacement);
  }), [ placement ]), updateComputedPosition = React.useCallback((function() {
    if (controlElement) {
      var rect = getBoundingClientObj(controlElement), scrollDistance = "fixed" === menuPosition ? 0 : window.pageYOffset, offset = rect[placement] + scrollDistance;
      offset === (null == computedPosition ? void 0 : computedPosition.offset) && rect.left === (null == computedPosition ? void 0 : computedPosition.rect.left) && rect.width === (null == computedPosition ? void 0 : computedPosition.rect.width) || setComputedPosition({
        offset: offset,
        rect: rect
      });
    }
  }), [ controlElement, menuPosition, placement, null == computedPosition ? void 0 : computedPosition.offset, null == computedPosition ? void 0 : computedPosition.rect.left, null == computedPosition ? void 0 : computedPosition.rect.width ]);
  useLayoutEffect__default.default((function() {
    updateComputedPosition();
  }), [ updateComputedPosition ]);
  var runAutoUpdate = React.useCallback((function() {
    "function" == typeof cleanupRef.current && (cleanupRef.current(), cleanupRef.current = null), 
    controlElement && menuPortalRef.current && (cleanupRef.current = dom.autoUpdate(controlElement, menuPortalRef.current, updateComputedPosition));
  ...
  if (!appendTo && "fixed" !== menuPosition || !computedPosition) return null;
