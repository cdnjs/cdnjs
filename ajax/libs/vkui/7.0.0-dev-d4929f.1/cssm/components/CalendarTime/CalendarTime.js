...
export const CalendarTime = ({ value, doneButtonText = 'Готово', onChange, onClose, changeHoursLabel, changeMinutesLabel, isDayDisabled })=>{
    ...
    const onHoursChange = React.useCallback((newValue)=>onChange?.(setHours(value, Number(newValue))), [
        onChange,
        value
    ]);
    const onMinutesChange = React.useCallback((newValue)=>onChange?.(setMinutes(value, Number(newValue))), [
        onChange,
        value
    ]);
