...
const useRestoreFocus = ({ restoreFocus, timeout, mount, ref })=>{
    const restoreFocusRef = (0, _react.useRef)(restoreFocus);
    restoreFocusRef.current = restoreFocus;
    const [restoreFocusTo, setRestoreFocusTo] = (0, _react.useState)(null);
    const restoreFocusImpl = (0, _useStableCallback.useStableCallback)(()=>{
        const shouldRestoreFocus = typeof restoreFocusRef.current === 'function' ? restoreFocusRef.current() : restoreFocusRef.current;
        if (!shouldRestoreFocus) {
            return;
        }
        setTimeout(()=>{
            const restoreFocusElement = (0, _dom.isHTMLElement)(shouldRestoreFocus) && shouldRestoreFocus || (0, _dom.isHTMLElement)(restoreFocusTo) && restoreFocusTo || null;
