...
shown: shownProp, onShownChange: onShownChangeProp, onShownChanged: onShownChangedProp })=>{
    ...
    const handleMouseEnterOnBoth = (0, _useStableCallback.useStableCallback)((event)=>{
    ...
    const handleMouseLeaveOnBothForHoverAndFocusStates = (0, _useStableCallback.useStableCallback)((event)=>{
        if (willBeHide && event.currentTarget === refs.floating.current) {
            return;
        }
        blockFocusRef.current = false;
