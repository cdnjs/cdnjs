...
class ViewInfiniteComponent extends React.Component {
    ...
    shouldDisableTransitionMotion() {
        return this.props.configProvider?.transitionMotionEnabled === false || !this.props.splitCol?.animate || this.props.platform === Platform.VKCOM;
    }
    ...
    onMoveX = (event)=>{
        if (!this.window || this.props.platform !== Platform.IOS || this.state.swipeBackPrevented || swipeBackExcluded(event) || this.shouldDisableTransitionMotion()) {
            return;
        }
        if (!this.props.configProvider?.isWebView) {
            if ((event.startX <= ViewInfiniteComponent.SWIPE_BACK_AREA || event.startX >= this.window.innerWidth - ViewInfiniteComponent.SWIPE_BACK_AREA) && !this.state.browserSwipe) {
                this.setState({
                    browserSwipe: true
                });
            }
        ...
        if (!this.props.onSwipeBack || this.state.animated && event.startX <= ViewInfiniteComponent.SWIPE_BACK_AREA) {
            return;
        }
        if (!this.state.swipingBack && event.startX <= ViewInfiniteComponent.SWIPE_BACK_AREA && this.props.history && this.props.history.length > 1) {
    ...
    render() {
        ...
        return /*#__PURE__*/ React.createElement(Touch, {
            Component: "section",
            ...restProps,
            className: classNames(styles['View'], platform === Platform.IOS && classNames(styles['View--ios'], 'vkuiInternalView--ios'), !disableAnimation && this.state.animated && styles['View--animated'], !disableAnimation && this.state.swipingBack && styles['View--swiping-back'], disableAnimation && styles['View--no-motion'], className),
