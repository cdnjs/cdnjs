...
const ChipsInputBase = (_param)=>{
    ...
    const [chipFocusedIndex, setChipFocusedIndex] = _react.useState(0);
    ...
    const handleListboxKeyDown = (event)=>{
        ...
        if (event.defaultPrevented || !inputRef.current || !listboxRef.current || !(0, _dom.isHTMLElement)(targetEl)) {
            return;
        }
        switch(event.key){
            case _accessibility.Keys.ENTER:
                {
                    if ((0, _dom1.contains)(targetEl, inputRef.current) && !(0, _helpers.isInputValueEmpty)(inputRef.current.value)) {
            ...
            case _accessibility.Keys.BACKSPACE:
                {
                    if (valueLength > 0) {
                        ...
                        } else if (event.key === _accessibility.Keys.BACKSPACE && (0, _helpers.isInputValueEmpty)(inputRef.current.value)) {
                            event.preventDefault();
