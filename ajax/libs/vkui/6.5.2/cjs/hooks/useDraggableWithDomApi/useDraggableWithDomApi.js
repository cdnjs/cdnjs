...
const useDraggableWithDomApi = ({ elRef: draggingElRef, onDragFinish })=>{
    ...
    const itemsGapRef = _react.useRef(0);
    const initializeItems = (draggingEl)=>{
        ...
        siblingItemsRef.current.forEach((sibling)=>(0, _utils.setInitialSiblingItemStyles)(sibling, itemsGapRef.current)); // 2. repaint
    };
    const cleanupItems = ()=>{
        if (placeholderItemRef.current) {
    ...
    const setShiftAndUnshiftItemStyles = (shiftItemEls, unshiftItemEls)=>{
        shiftItemEls.forEach((item)=>(0, _utils.setSiblingItemsShiftStyles)(item, itemsGapRef.current));
        unshiftItemEls.forEach((item)=>(0, _utils.setSiblingItemsShiftStyles)(item, itemsGapRef.current));
    };
