 ...
 */ export const UsersStack = (_param)=>{
    var { photos = [], visibleCount = 3, count = Math.max(0, photos.length - visibleCount), size = 'm', children, avatarsPosition = 'inline-start', getRootRef } = _param, restProps = _object_without_properties(_param, [
        "photos",
        "visibleCount",
        "count",
        "size",
    ...
    const directionClip = textDirection === 'ltr' ? canShowOthers ? 'right' : 'left' : canShowOthers ? 'left' : 'right';
