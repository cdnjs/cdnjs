...
export const CarouselBase = ({ bullets = false, getRootRef, children, slideWidth = '100%', slideIndex = 0, dragDisabled = false, resizeSource = 'window', onDragStart, onDragEnd, onChange, onPrevClick, onNextClick, align = 'left', showArrows, getRef, arrowSize, arrowAreaHeight, arrowNextLabel, arrowPrevLabel, slideTestId, bulletTestId, nextArrowTestId, prevArrowTestId, looped = false, ...restProps })=>{
    ...
    const calculateCanSlideRight = ()=>{
        ...
        (align === 'left' && slidesManager.current.containerWidth - revertRtlValue(shiftXCurrentRef.current, isRtl) < (slidesManager.current.layerWidth ?? 0) || // otherwise we need to check current slide index (align = right or align = center)
