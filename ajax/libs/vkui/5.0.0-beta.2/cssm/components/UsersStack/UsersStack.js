import _extends from "@babel/runtime/helpers/extends";
import _objectWithoutProperties from "@babel/runtime/helpers/objectWithoutProperties";
var _excluded = ["photoSize", "direction"],
    _excluded2 = ["photos", "visibleCount", "count", "size", "layout", "children", "className"];
import * as React from "react";
import { hasReactNode } from "../../lib/utils";
import { classNamesString } from "../../lib/classNames";
import { Footnote } from "../Typography/Footnote/Footnote";
import { Caption } from "../Typography/Caption/Caption";
import "./UsersStack.module.css";

function PathElement(_ref) {
  var photoSize = _ref.photoSize,
      direction = _ref.direction,
      props = _objectWithoutProperties(_ref, _excluded);

  switch (direction) {
    case "circle":
      var radius = photoSize / 2;
      return /*#__PURE__*/React.createElement("circle", _extends({
        cx: radius,
        cy: radius,
        r: radius
      }, props));

    case "right":
      switch (photoSize) {
        case 24:
          return /*#__PURE__*/React.createElement("path", _extends({
            d: "M22,18.625A12 12 0 0 1 12 24A12 12 0 0 1 12 0A12 12 0 0 1 22 5.375A12 12 0 0 0 22,18.625"
          }, props));

        default:
          return /*#__PURE__*/React.createElement("path", _extends({
            d: "M30,23.75A16 16 0 0 1 16 32A16 16 0 0 1 16 0A16 16 0 0 1 30 8.25A16 16 0 0 0 30,23.75"
          }, props));
      }

    default:
      switch (photoSize) {
        case 16:
          return /*#__PURE__*/React.createElement("path", _extends({
            d: "M2,13.285A8 8 0 0 0 8 16A8 8 0 0 0 8 0A8 8 0 0 0 2 2.715A8 8 0 0 1 2,13.285"
          }, props));

        case 24:
          return /*#__PURE__*/React.createElement("path", _extends({
            d: "M2,18.625A12 12 0 0 0 12 24A12 12 0 0 0 12 0A12 12 0 0 0 2 5.375A12 12 0 0 1 2,18.625"
          }, props));

        default:
          return /*#__PURE__*/React.createElement("path", _extends({
            d: "M2,23.75A16 16 0 0 0 16 32A16 16 0 0 0 16 0A16 16 0 0 0 2 8.25A16 16 0 0 1 2,23.75"
          }, props));
      }

  }
}

var photoSizes = {
  s: 16,
  m: 24,
  l: 32
};
/**
 * @see https://vkcom.github.io/VKUI/#/UsersStack
 */

export var UsersStack = function UsersStack(_ref2) {
  var _ref2$photos = _ref2.photos,
      photos = _ref2$photos === void 0 ? [] : _ref2$photos,
      _ref2$visibleCount = _ref2.visibleCount,
      visibleCount = _ref2$visibleCount === void 0 ? 3 : _ref2$visibleCount,
      _ref2$count = _ref2.count,
      count = _ref2$count === void 0 ? Math.max(0, photos.length - visibleCount) : _ref2$count,
      _ref2$size = _ref2.size,
      size = _ref2$size === void 0 ? "m" : _ref2$size,
      _ref2$layout = _ref2.layout,
      layout = _ref2$layout === void 0 ? "horizontal" : _ref2$layout,
      children = _ref2.children,
      className = _ref2.className,
      restProps = _objectWithoutProperties(_ref2, _excluded2);

  var canShowOthers = count > 0 && size !== "s";
  var CounterTypography = size === "m" ? Footnote : Caption;
  var photoSize = photoSizes[size];
  var directionClip = canShowOthers ? "right" : "left";
  var photosShown = photos.slice(0, visibleCount);
  return /*#__PURE__*/React.createElement("div", _extends({}, restProps, {
    className: classNamesString("vkuiUsersStack", styles["UsersStack--size-".concat(size)], styles["UsersStack--layout-".concat(layout)], canShowOthers && "vkuiUsersStack--others", className)
  }), /*#__PURE__*/React.createElement("div", {
    className: "vkuiUsersStack__photos",
    role: "presentation"
  }, photosShown.map(function (photo, i) {
    var direction = i === 0 && !canShowOthers ? "circle" : directionClip;
    return /*#__PURE__*/React.createElement("svg", {
      xmlns: "http://www.w3.org/2000/svg",
      className: "vkuiUsersStack__photo",
      key: i,
      "aria-hidden": true
    }, /*#__PURE__*/React.createElement("g", {
      className: styles["UsersStack__mask--".concat(photoSize, "-").concat(direction)]
    }, /*#__PURE__*/React.createElement(PathElement, {
      direction: direction,
      photoSize: photoSize,
      className: "vkuiUsersStack__fill"
    }), /*#__PURE__*/React.createElement("image", {
      href: photo,
      width: photoSize,
      height: photoSize
    }), /*#__PURE__*/React.createElement(PathElement, {
      direction: direction,
      photoSize: photoSize,
      fill: "none",
      stroke: "rgba(0, 0, 0, 0.08)"
    })));
  }), canShowOthers && /*#__PURE__*/React.createElement(CounterTypography, {
    caps: true,
    weight: "1",
    className: classNamesString("vkuiUsersStack__photo", "vkuiUsersStack__photo--others"),
    "aria-hidden": true
  }, /*#__PURE__*/React.createElement("span", null, "+", count))), hasReactNode(children) && /*#__PURE__*/React.createElement(Footnote, {
    className: "vkuiUsersStack__text"
  }, children));
};
var styles = {
  "UsersStack--size-s": "vkuiUsersStack--size-s",
  "UsersStack--size-m": "vkuiUsersStack--size-m",
  "UsersStack--size-l": "vkuiUsersStack--size-l",
  "UsersStack--layout-vertical": "vkuiUsersStack--layout-vertical",
  "UsersStack--layout-horizontal": "vkuiUsersStack--layout-horizontal",
  "UsersStack__mask--16-left": "vkuiUsersStack__mask--16-left",
  "UsersStack__mask--16-circle": "vkuiUsersStack__mask--16-circle",
  "UsersStack__mask--24-left": "vkuiUsersStack__mask--24-left",
  "UsersStack__mask--24-circle": "vkuiUsersStack__mask--24-circle",
  "UsersStack__mask--24-right": "vkuiUsersStack__mask--24-right",
  "UsersStack__mask--32-left": "vkuiUsersStack__mask--32-left",
  "UsersStack__mask--32-circle": "vkuiUsersStack__mask--32-circle",
  "UsersStack__mask--32-right": "vkuiUsersStack__mask--32-right",
  "UsersStack__mask--16-right": "vkuiUsersStack__mask--16-right"
};
//# sourceMappingURL=UsersStack.js.map