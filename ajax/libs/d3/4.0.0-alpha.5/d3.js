	...
	function clipEdges(x0, y0, x1, y1) {
	  ...
	  while (i--) {
	    ...
	    if (!connectEdge(e, x0, y0, x1, y1)
	        ...
	        || (Math.abs(e.a.x - e.b.x) < epsilon$1 && Math.abs(e.a.y - e.b.y) < epsilon$1)) {
	      e.a = e.b = null;
	      edges.splice(i, 1);
	    }
	  }
	    ...
	    while (iHalfEdge < nHalfEdges) {
	      ...
	      if (Math.abs(x3 - x2) > epsilon$1 || Math.abs(y3 - y2) > epsilon$1) {
	        halfEdges.splice(iHalfEdge, 0, createHalfEdge(createBorderEdge(cell.site, end,
	            Math.abs(x3 - x0) < epsilon$1 && y1 - y3 > epsilon$1 ? {x: x0, y: Math.abs(x2 - x0) < epsilon$1 ? y2 : y1}
	            : Math.abs(y3 - y1) < epsilon$1 && x1 - x3 > epsilon$1 ? {x: Math.abs(y2 - y1) < epsilon$1 ? x2 : x1, y: y1}
	            : Math.abs(x3 - x1) < epsilon$1 && y3 - y0 > epsilon$1 ? {x: x1, y: Math.abs(x2 - x1) < epsilon$1 ? y2 : y0}
	            : Math.abs(y3 - y0) < epsilon$1 && x3 - x0 > epsilon$1 ? {x: Math.abs(y2 - y0) < epsilon$1 ? x2 : x0, y: y0}
	            : null), cell.site, null));
	        ++nHalfEdges;
	      }
	    }
	...
	function voronoi() {
	  ...
	  function points(data) {
	    return data.map(function(d, i) {
	      return {
	        x: Math.round(fx(d, i) / epsilon$1) * epsilon$1,
	        y: Math.round(fy(d, i) / epsilon$1) * epsilon$1,
	        i: i
	      };
	    });
	  }
