(function (global, factory) {
	...
	var identity$2 = {a: 1, b: 0, c: 0, d: 1, e: 0, f: 0};
	var g;
	...
	function tweenFunction(key, id, name, value) {
	  var tween0, tween1;
	  return function() {
	    var schedule = set$2(this, key, id),
	...
	function transition_tween(name, value) {
	  ...
	  if (arguments.length < 2) {
	    var tween = get$1(this.node(), key, id).tween;
	...
	function tweenValue(transition, name, value) {
	  ...
	  transition.each(function() {
	    var schedule = set$2(this, key, id), v = value.apply(this, arguments);
	    (schedule.value || (schedule.value = {}))[name] = v == null ? null : v + "";
	  });

	  return function(node) {
	    return get$1(node, key, id).value[name];
	  };
	}
	...
	function transition_attr(name, value) {
	  ...
	  return this.attrTween(name, typeof value === "function"
	      ? (fullname.local ? attrFunctionNS$1 : attrFunction$1)(fullname, tweenValue(this, "attr." + name, value))
	...
	function transition_delay(value) {
	  ...
	  return arguments.length
	      ...
	      : get$1(this.node(), key, id).delay;
	}

	function durationFunction(key, id, value) {
	  return function() {
	    set$2(this, key, id).duration = +value.apply(this, arguments);
	  };
	}

	function durationConstant(key, id, value) {
	  return value = +value, function() {
	    set$2(this, key, id).duration = value;
	  };
	}
	...
	function transition_duration(value) {
	  ...
	  return arguments.length
	      ...
	      : get$1(this.node(), key, id).duration;
	}

	function easeConstant(key, id, value) {
	  if (typeof value !== "function") throw new Error;
	  return function() {
	    set$2(this, key, id).ease = value;
	  };
	}
	...
	function transition_on(name, listener) {
	  ...
	  return arguments.length < 2
	      ? get$1(this.node(), key, id).on.on(name)
	  ...
	  for (var groups = this._groups, m = groups.length, subgroups = new Array(m), j = 0; j < m; ++j) {
	    for (var group = groups[j], n = group.length, subgroup = subgroups[j] = new Array(n), node, subnode, i = 0; i < n; ++i) {
	      if ((node = group[i]) && (subnode = select.call(node, node.__data__, i, group))) {
	        if ("__data__" in node) subnode.__data__ = node.__data__;
	        subgroup[i] = subnode;
	        schedule(subgroup[i], key, id, i, subgroup, get$1(node, key, id));
	      }
	    }
	  }
	...
	function transition_selectAll(select) {
	  ...
	  for (var groups = this._groups, m = groups.length, subgroups = [], parents = [], j = 0; j < m; ++j) {
	    for (var group = groups[j], n = group.length, node, i = 0; i < n; ++i) {
	      if (node = group[i]) {
	        for (var children = select.call(node, node.__data__, i, group), child, inherit = get$1(node, key, id), k = 0, l = children.length; k < l; ++k) {
	          if (child = children[k]) {
	...
	function transition_style(name, value, priority) {
	  return value == null ? this
	          .styleTween(name, styleRemove$1(name))
	          .on("end.style." + name, styleRemoveEnd(name))
	      : this.styleTween(name, typeof value === "function"
	          ? styleFunction$1(name, tweenValue(this, "style." + name, value))
	          : styleConstant$1(name, value + ""), priority);
	}
	...
	function transition_transition() {
	  ...
	  for (var groups = this._groups, m = groups.length, j = 0; j < m; ++j) {
	    for (var group = groups[j], n = group.length, node, i = 0; i < n; ++i) {
	      if (node = group[i]) {
	        var inherit = get$1(node, key, id0);
	...
	function selection_transition(name) {
	  ...
	  if (name instanceof Transition) {
	    key = name._key, id = name._id, timing = get$1(name.node(), key, id);
	  } else {
	...
	function active(node, name) {
	  ...
	  return new Transition([[node]], root$1, key, active.id);
	}
