...
;// CONCATENATED MODULE: ./dist/PolygonPathGenerator.js
...
class PolygonPathGenerator {
  ...
  generate(p) {
    ...
    p.velocity.y = 0;
    p.hexStep++;
    return external_commonjs_tsparticles_commonjs2_tsparticles_amd_tsparticles_root_window_.Vector.create(this.dirsList[p.hexDirection].x * p.hexSpeed, this.dirsList[p.hexDirection].y * p.hexSpeed);
  }
  ...
  createDirs() {
    ...
    for (let i = 0; i < 360; i += 360 / this.options.sides) {
      ...
      this.dirsList.push(external_commonjs_tsparticles_commonjs2_tsparticles_amd_tsparticles_root_window_.Vector.create(Math.cos(angle * Math.PI / 180), Math.sin(angle * Math.PI / 180)));
    }
  }
