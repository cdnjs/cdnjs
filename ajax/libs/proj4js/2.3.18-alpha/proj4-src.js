...
var PrimeMeridian = Object.freeze({
		greenwich: greenwich,
		lisbon: lisbon,
		paris: paris,
		bogota: bogota,
...
var Ellipsoid = Object.freeze({
	...
	var wgs84$1 = match(Ellipsoid, 'WGS84');
	function sphere$$1(a, b, rf, ellps, sphere$$1) {
...
var Datum = Object.freeze({
	...
	function compareDatums(source, dest) {
	  if (source.datum_type !== dest.datum_type) {
	    ...
	    return false;
	  } else if (source.datum_type === PJD_3PARAM$3) {
	    return (source.datum_params[0] === dest.datum_params[0] && source.datum_params[1] === dest.datum_params[1] && source.datum_params[2] === dest.datum_params[2]);
	  } else if (source.datum_type === PJD_7PARAM$3) {
	...
	*/
	function geocentricToWgs84(p, datum_type, datum_params) {
	  ...
	  } else if (datum_type === PJD_7PARAM$3) {
	...
	function checkParams(type) {
	  return (type === PJD_3PARAM$2 || type === PJD_7PARAM$2);
	}
	...
	function forward$2(p) {
	  ...
	  if (!this.es) {
	    ...
	    if ((Math.abs(Math.abs(b) - 1)) < EPSLN$2) {
	      return (93);
	    }
	    else {
	    ...
	    var cs = Math.pow(c, 2);
	    var tq = Math.abs(cos_phi) > EPSLN$2 ? Math.tan(lat) : 0;
	    var t = Math.pow(tq, 2);
	    var ts = Math.pow(t, 2);
	...
	function inverse$2(p) {
	  ...
	  if (!this.es) {
	    ...
	    if (Math.abs(phi) < HALF_PI$4) {
	      ...
	      var tan_phi = Math.abs(cos_phi) > EPSLN$2 ? Math.tan(phi) : 0;
	      var c = this.ep2 * Math.pow(cos_phi, 2);
	      var cs = Math.pow(c, 2);
	...
	function init$7() {
	  ...
	  if (this.sphere) {
	    if (this.k0 === 1 && !isNaN(this.lat_ts) && Math.abs(this.coslat0) <= EPSLN$4) {
	      this.k0 = 0.5 * (1 + sign(this.lat0) * Math.sin(this.lat_ts));
	    }
	  }
	  else {
	    if (Math.abs(this.coslat0) <= EPSLN$4) {
	      if (this.lat0 > 0) {
	        //North pole
	...
	function forward$6(p) {
	  ...
	  if (Math.abs(Math.abs(lon - this.long0) - Math.PI) <= EPSLN$4 && Math.abs(lat + this.lat0) <= EPSLN$4) {
	  ...
	  if (this.sphere) {
	    ...
	    else if (Math.abs(this.sinlat0) < EPSLN$4) {
	      //Eq
	...
	function forward$8(p) {
	  ...
	  var us, vs;
	  var con;
	  if (Math.abs(Math.abs(lat) - HALF_PI$7) <= EPSLN$5) {
	    if (lat > 0) {
	      con = -1;
	    }
	    else {
	    ...
	    if (Math.abs(Math.abs(ul) - 1) <= EPSLN$5) {
	      vs = Number.POSITIVE_INFINITY;
	    }
	    else {
	      vs = 0.5 * this.al * Math.log((1 - ul) / (1 + ul)) / this.bl;
	    }
	    if (Math.abs(Math.cos(this.bl * (dlon))) <= EPSLN$5) {
	...
	function inverse$8(p) {
	  ...
	  if (Math.abs(up - 1) < EPSLN$5) {
	    p.x = this.long0;
	    p.y = HALF_PI$7;
	  }
	  else if (Math.abs(up + 1) < EPSLN$5) {
	    p.x = this.long0;
	    p.y = -1 * HALF_PI$7;
	  }
	  else {
	...
	function init$10() {
	  ...
	  if (Math.abs(this.lat1 + this.lat2) < EPSLN$6) {
	    return;
	  }
	...
	function forward$9(p) {
	  ...
	  // singular cases :
	  if (Math.abs(2 * Math.abs(lat) - Math.PI) <= EPSLN$6) {
	    lat = sign(lat) * (HALF_PI$8 - 2 * EPSLN$6);
	  }
	...
	function inverse$11(p) {
	  ...
	  if (this.sphere) {
	    ...
	    if (Math.abs(Math.abs(phi1) - HALF_PI$9) <= EPSLN$7) {
	      p.x = this.long0;
	      p.y = HALF_PI$9;
	      if (y < 0) {
	        p.y *= -1;
	...
	function init$13() {
	  ...
	  if (Math.abs(t - HALF_PI$11) < EPSLN$8) {
	    this.mode = this.lat0 < 0 ? this.S_POLE : this.N_POLE;
	  }
	  else if (Math.abs(t) < EPSLN$8) {
	...
	function forward$12(p) {
	  ...
	  if (this.sphere) {
	    ...
	    if (this.mode === this.OBLIQ || this.mode === this.EQUIT) {
	      ...
	      if (Math.abs(phi + this.phi0) < EPSLN$8) {
	        return null;
	      }
	      y = FORTPI$3 - phi * 0.5;
	...
	function inverse$12(p) {
	  ...
	  if (this.sphere) {
	    ...
	    switch (this.mode) {
	    case this.EQUIT:
	      phi = (Math.abs(rh) <= EPSLN$8) ? 0 : Math.asin(y * sinz / rh);
	      x *= sinz;
	      y = cosz * rh;
	      break;
	    case this.OBLIQ:
	      phi = (Math.abs(rh) <= EPSLN$8) ? this.phi0 : Math.asin(cosz * this.sinph0 + y * sinz * this.cosph0 / rh);
	      x *= sinz * this.cosph0;
	...
	function init$14() {
	  ...
	  if (Math.abs(this.lat1 + this.lat2) < EPSLN$9) {
	    return;
	  }
	...
	*/
	...
	function inverse$17(p) {
	  ...
	  if (this.sphere) {
	    if (Math.abs(y + this.a * this.lat0) <= EPSLN$11) {
	    ...
	    if (Math.abs(y + this.ml0) <= EPSLN$11) {
	      lat = 0;
	...
	var EPSLN$17 = 1.0e-10;
	var HALF_PI$16 = Math.PI/2;
	...
	function inverse$25(p) {
	  ...
	  con = Math.abs(this.lat0) - HALF_PI$16;
	  if (Math.abs(con) <= EPSLN$17) {
	    if (this.lat0 >= 0) {
	      lon = adjust_lon(this.long0 + Math.atan2(p.x, - p.y));
	    }
	    else {
