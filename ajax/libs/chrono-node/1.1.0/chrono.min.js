...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(today|tonight|tomorrow|tmr|yesterday|last\s*night|this\s*(morning|afternoon|evening))(?=\W|$)/i;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(t,r,e,a){var s=e.index,t=e[0],i=new ParsedResult({index:s,text:t,ref:r}),n=moment(r),o=n.clone(),m=t.toLowerCase();if("tonight"==m)i.start.imply("hour",22),i.start.imply("meridiem",1);else if("tomorrow"==m||"tmr"==m)n.hour()>4&&o.add(1,"day");else if("yesterday"==m)o.add(-1,"day");else if(m.match(/last\s*night/))i.start.imply("hour",0),n.hour()>6&&o.add(-1,"day");else if(m.match("this")){var u=e[2].toLowerCase();"afternoon"==u?i.start.imply("hour",15):"evening"==u?i.start.imply("hour",18):"morning"==u&&i.start.imply("hour",6)}return i.start.assign("day",o.date()),i.start.assign("month",o.month()+1),i.start.assign("year",o.year()),i.tags.ENCasualDateParser=!0,i}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(\W|^)(?:within\s*)?([0-9]+|an?)\s*(minutes?|hours?|weeks?|days?|months?|years?)\s*(?:ago|before|earlier)(?=(?:\W|$))/i,STRICT_PATTERN=/(\W|^)(?:within\s*)?([0-9]+|an?)\s*(minutes?|hours?|days?)\s*ago(?=(?:\W|$))/i;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return this.isStrictMode()?STRICT_PATTERN:PATTERN},this.extract=function(t,a,e,r){if(e.index>0&&t[e.index-1].match(/\w/))return null;var t=e[0];t=e[0].substr(e[1].length,e[0].length-e[1].length),index=e.index+e[1].length;var s=new ParsedResult({index:index,text:t,ref:a}),n=e[2];n="a"===n||"an"===n?1:parseInt(n);var i=moment(a);return e[3].match(/hour/)||e[3].match(/minute/)?(e[3].match(/hour/)?i.add(-n,"hour"):e[3].match(/minute/)&&i.add(-n,"minute"),s.start.imply("day",i.date()),s.start.imply("month",i.month()+1),s.start.imply("year",i.year()),s.start.assign("hour",i.hour()),s.start.assign("minute",i.minute()),s.tags.ENTimeAgoFormatParser=!0,s):e[3].match(/weeks/)?(i.add(-n,"week"),s.start.imply("day",i.date()),s.start.imply("month",i.month()+1),s.start.imply("year",i.year()),s.start.imply("weekday",i.day()),s):(e[3].match(/day/)&&i.add(-n,"d"),e[3].match(/month/)&&i.add(-n,"month"),e[3].match(/year/)&&i.add(-n,"year"),s.start.assign("day",i.date()),s.start.assign("month",i.month()+1),s.start.assign("year",i.year()),s)}};
},{"../../result":25,"../parser":16,"moment":1}],12:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,FIRST_REG_PATTERN=new RegExp("(^|\\s|T)(?:(?:at|from)\\s*)?(\\d{1,4}|noon|midnight)(?:(?:\\.|\\:|\\：)(\\d{1,2})(?:(?:\\:|\\：)(\\d{2}))?)?(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?))?(?=\\W|$)","i"),SECOND_REG_PATTERN=new RegExp("^\\s*(\\-|\\–|\\~|\\〜|to|\\?)\\s*(\\d{1,4})(?:(?:\\.|\\:|\\：)(\\d{1,2})(?:(?:\\.|\\:|\\：)(\\d{1,2}))?)?(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?))?(?=\\W|$)","i"),HOUR_GROUP=2,MINUTE_GROUP=3,SECOND_GROUP=4,AM_PM_HOUR_GROUP=5;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return FIRST_REG_PATTERN},this.extract=function(e,t,r,n){if(r.index>0&&e[r.index-1].match(/\w/))return null;var s=moment(t),a=new ParsedResult;a.ref=t,a.index=r.index+r[1].length,a.text=r[0].substring(r[1].length),a.tags.ENTimeExpressionParser=!0,a.start.imply("day",s.date()),a.start.imply("month",s.month()+1),a.start.imply("year",s.year());var i=0,u=0,l=-1;if(null!=r[SECOND_GROUP]){var R=parseInt(r[SECOND_GROUP]);if(R>=60)return null;a.start.assign("second",R)}if("noon"==r[HOUR_GROUP].toLowerCase()?(l=1,i=12):"midnight"==r[HOUR_GROUP].toLowerCase()?(l=0,i=0):i=parseInt(r[HOUR_GROUP]),null!=r[MINUTE_GROUP]?u=parseInt(r[MINUTE_GROUP]):i>100&&(u=i%100,i=parseInt(i/100)),u>=60)return null;if(i>24)return null;if(i>=12&&(l=1),null!=r[AM_PM_HOUR_GROUP]){if(i>12)return null;var P=r[AM_PM_HOUR_GROUP][0].toLowerCase();"a"==P&&(l=0,12==i&&(i=0)),"p"==P&&(l=1,12!=i&&(i+=12))}if(a.start.assign("hour",i),a.start.assign("minute",u),l>=0&&a.start.assign("meridiem",l),r=SECOND_REG_PATTERN.exec(e.substring(a.index+a.text.length)),!r)return a.text.match(/^\d+$/)?null:a;if(r[0].match(/^\s*(\+|\-)\s*\d{3,4}$/))return a;null==a.end&&(a.end=a.start.clone());var i=0,u=0,l=-1;if(null!=r[SECOND_GROUP]){var R=parseInt(r[SECOND_GROUP]);if(R>=60)return null;a.end.assign("second",R)}if(i=parseInt(r[2]),null!=r[MINUTE_GROUP]){if(u=parseInt(r[MINUTE_GROUP]),u>=60)return a}else i>100&&(u=i%100,i=parseInt(i/100));if(u>=60)return null;if(i>24)return null;if(i>=12&&(l=1),null!=r[AM_PM_HOUR_GROUP]){if(i>12)return null;"a"==r[AM_PM_HOUR_GROUP][0].toLowerCase()&&(l=0,12==i&&(i=0,a.end.isCertain("day")||a.end.imply("day",a.end.get("day")+1))),"p"==r[AM_PM_HOUR_GROUP][0].toLowerCase()&&(l=1,12!=i&&(i+=12)),a.start.isCertain("meridiem")||(0==l?(a.start.imply("meridiem",0),12==a.start.get("hour")&&a.start.assign("hour",0)):(a.start.imply("meridiem",1),12!=a.start.get("hour")&&a.start.assign("hour",a.start.get("hour")+12)))}return i>=12&&(l=1),a.text=a.text+r[0],a.end.assign("hour",i),a.end.assign("minute",u),l>=0&&a.end.assign("meridiem",l),a}};
},{"../../result":25,"../parser":16,"moment":1}],13:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)(?:(?:\\,|\\(|\\（)\\s*)?(?:(this|last|next)\\s*)?(Sunday|Sun|Monday|Mon|Tuesday|Tues|Tue|Wednesday|Wed|Thursday|Thurs|Thur|Friday|Fri|Saturday|Sat)(?:\\s*(?:\\,|\\)|\\）))?(?:\\s*(this|last|next)\\s*week)?(?=\\W|$)","i"),DAYS_OFFSET={sunday:0,sun:0,monday:1,mon:1,tuesday:2,tues:2,tue:2,wednesday:3,wed:3,thursday:4,thurs:4,thur:4,thu:4,friday:5,fri:5,saturday:6,sat:6},PREFIX_GROUP=2,WEEKDAY_GROUP=3,POSTFIX_GROUP=4;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,t,a,r){var s=a.index+a[1].length,e=a[0].substr(a[1].length,a[0].length-a[1].length),n=new ParsedResult({index:s,text:e,ref:t}),u=a[WEEKDAY_GROUP].toLowerCase(),d=DAYS_OFFSET[u];if(void 0===d)return null;var y=moment(t),i=a[PREFIX_GROUP],h=a[POSTFIX_GROUP];if(i||h){var P=i||h;P=P.toLowerCase(),"last"==P?y.day(d-7):"next"==P?y.day(d+7):"this"==P&&y.day(d)}else{var l=y.day();Math.abs(d-7-l)<Math.abs(d-l)?y.day(d-7):Math.abs(d+7-l)<Math.abs(d-l)?y.day(d+7):y.day(d)}return n.start.assign("weekday",d),n.start.imply("day",y.date()),n.start.imply("month",y.month()+1),n.start.imply("year",y.year()),n}};
