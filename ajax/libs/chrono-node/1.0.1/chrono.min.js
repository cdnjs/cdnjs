...
},{"./options":4,"./parsers/parser":14,"./refiners/refiner":21,"./result":22}],3:[function(require,module,exports){
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)([0-9]{4})\\-([0-9]{1,2})\\-([0-9]{1,2})(?:T([0-9]{1,2}):([0-9]{1,2})(?::([0-9]{1,2})(?:\\.\\d{1,4})?)?(?:Z|([+-]\\d{2}):?(\\d{2})?))?(?=\\W|$)","i"),YEAR_NUMBER_GROUP=2,MONTH_NUMBER_GROUP=3,DATE_NUMBER_GROUP=4,HOUR_NUMBER_GROUP=5,MINUTE_NUMBER_GROUP=6,SECOND_NUMBER_GROUP=7,TZD_HOUR_OFFSET_GROUP=8,TZD_MINUTE_OFFSET_GROUP=9;exports.Parser=function(){Parser.call(this),this.pattern=function(){return PATTERN},this.extract=function(t,e,r){var t=r[0].substr(r[1]),s=r.index+r[1].length,a=new ParsedResult({text:t,index:s,ref:e});if(a.start.assign("year",parseInt(r[YEAR_NUMBER_GROUP])),a.start.assign("month",parseInt(r[MONTH_NUMBER_GROUP])),a.start.assign("day",parseInt(r[DATE_NUMBER_GROUP])),moment(a.start.get("month"))>12||moment(a.start.get("month"))<1||moment(a.start.get("day"))>31||moment(a.start.get("day"))<1)return null;if(null!=r[HOUR_NUMBER_GROUP])if(a.start.assign("hour",parseInt(r[HOUR_NUMBER_GROUP])),a.start.assign("minute",parseInt(r[MINUTE_NUMBER_GROUP])),null!=r[SECOND_NUMBER_GROUP]&&a.start.assign("second",parseInt(r[SECOND_NUMBER_GROUP])),null==r[TZD_HOUR_OFFSET_GROUP])a.start.assign("timezoneOffset",0);else{var n=0,R=parseInt(r[TZD_HOUR_OFFSET_GROUP]);null!=r[TZD_MINUTE_OFFSET_GROUP]&&(n=parseInt(r[TZD_MINUTE_OFFSET_GROUP]));var _=60*R;0>_?_-=n:_+=n,a.start.assign("timezoneOffset",_)}return a.tags.ENISOFormatParser=!0,a}};
},{"../../result":22,"../parser":14,"moment":1}],8:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,CONST=require("../../constants/EN"),DAYS_OFFSET={sunday:0,sun:0,monday:1,mon:1,tuesday:2,tue:2,wednesday:3,wed:3,thursday:4,thur:4,thu:4,friday:5,fri:5,saturday:6,sat:6},PATTERN=new RegExp("(\\W|^)(?:(Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sun|Mon|Tue|Wed|Thu|Fri|Sat)\\s*,?\\s*)?([0-9]{1,2})(?:st|nd|rd|th)?(?:\\s*(?:to|\\-|\\s)\\s*([0-9]{1,2})(?:st|nd|rd|th)?)?\\s*(?:of)?\\s*(Jan(?:uary|\\.)?|Feb(?:ruary|\\.)?|Mar(?:ch|\\.)?|Apr(?:il|\\.)?|May|Jun(?:e|\\.)?|Jul(?:y|\\.)?|Aug(?:ust|\\.)?|Sep(?:tember|\\.)?|Oct(?:ober|\\.)?|Nov(?:ember|\\.)?|Dec(?:ember|\\.)?)(?:(\\s*[0-9]{2,4}(?![^\\s]\\d))(\\s*BE)?)?(?=\\W|$)","i"),WEEKDAY_GROUP=2,DATE_GROUP=3,DATE_TO_GROUP=4,MONTH_NAME_GROUP=5,YEAR_GROUP=6,YEAR_BE_GROUP=7;exports.Parser=function(){Parser.call(this),this.pattern=function(){return PATTERN},this.extract=function(e,a,t){var r=new ParsedResult({text:t[0].substr(t[1].length,t[0].length-t[1].length),index:t.index+t[1].length,ref:a}),s=moment(a),n=t[MONTH_NAME_GROUP];n=CONST.MONTH_OFFSET[n.toLowerCase()];var d=t[DATE_GROUP];d=parseInt(d);var u=null;if(t[YEAR_GROUP]&&(u=t[YEAR_GROUP],u=parseInt(u),t[YEAR_BE_GROUP]?u-=543:100>u&&(u+=2e3)),s.month(n-1),s.date(d),u)s.year(u),r.start.assign("day",s.date()),r.start.assign("month",s.month()+1),r.start.assign("year",s.year());else{s.year(moment(a).year());var i=s.clone().add(1,"y"),o=s.clone().add(-1,"y");Math.abs(i.diff(moment(a)))<Math.abs(s.diff(moment(a)))?s=i:Math.abs(o.diff(moment(a)))<Math.abs(s.diff(moment(a)))&&(s=o),r.start.assign("day",s.date()),r.start.assign("month",s.month()+1),r.start.imply("year",s.year())}if(t[WEEKDAY_GROUP]){var y=t[WEEKDAY_GROUP];y=CONST.WEEKDAY_OFFSET[y.toLowerCase()],r.start.assign("weekday",y)}return t[DATE_TO_GROUP]&&(r.end=r.start.clone(),r.end.assign("day",parseInt(t[DATE_TO_GROUP]))),r.tags.ENMonthNameLittleEndianParser=!0,r}};
},{"../../constants/EN":3,"../../result":22,"../parser":14,"moment":1}],9:[function(require,module,exports){
...
function isAbleToMerge(e,t,n){var r=t.index+t.text.length,i=n.index,a=e.substring(r,i);return a.match(OVERLAP_PATTERN)}function mergeResult(e,t,n){for(var r in n.start.knownValues)t.start.isCertain(r)||t.start.assign(r,n.start.get(r));for(var r in t.start.knownValues)n.start.isCertain(r)||n.start.assign(r,t.start.get(r));if(t.start.date().getTime()>n.start.date()){var i=n;n=t,t=i}t.end=n.start;for(var a in n.tags)t.tags[a]=!0;var s=Math.min(t.index,n.index),g=Math.max(t.index+t.text.length,n.index+n.text.length);return t.index=s,t.text=e.substring(s,g),t.tags.ENMergeDateRangeRefiner=!0,t}var Refiner=require("../refiner").Refiner,OVERLAP_PATTERN=/^\s*(to|\-)\s*$/i;exports.Refiner=function(){Refiner.call(this),this.refine=function(e,t){if(t.length<2)return t;for(var n=[],r=null,i=null,a=1;a<t.length;a++)r=t[a],i=t[a-1],i.end||r.end||!isAbleToMerge(e,i,r)||(i=mergeResult(e,i,r),r=null,a+=1),n.push(i);return null!=r&&n.push(r),n}};
