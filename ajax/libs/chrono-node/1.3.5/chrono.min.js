...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/EN"),TIME_UNIT="("+util.INTEGER_WORDS_PATTERN+"|[0-9]+|an?(?:\\s*few)?|half(?:\\s*an?)?)\\s*(sec(?:onds?)?|min(?:ute)?s?|hours?|weeks?|days?|months?|years?)\\s*",TIME_UNIT_STRICT="([0-9]+|an?)\\s*(seconds?|minutes?|hours?|days?)\\s*",PATTERN_TIME_UNIT=new RegExp(TIME_UNIT,"i"),PATTERN=new RegExp("(\\W|^)(?:within\\s*)?((?:"+TIME_UNIT+")+)(?:ago|before|earlier)(?=(?:\\W|$))","i"),STRICT_PATTERN=new RegExp("(\\W|^)(?:within\\s*)?((?:"+TIME_UNIT_STRICT+")+)ago(?=(?:\\W|$))","i");exports.Parser=function(){function e(e){for(var r={},a=e,s=PATTERN_TIME_UNIT.exec(a);s;)t(s,r),a=a.substring(s[0].length),s=PATTERN_TIME_UNIT.exec(a);return r}function t(e,t){var r=e[1].toLowerCase();return r=void 0!==util.INTEGER_WORDS[r]?util.INTEGER_WORDS[r]:"a"===r||"an"===r?1:r.match(/few/)?3:r.match(/half/)?.5:parseInt(r),e[2].match(/hour/i)?t.hour=r:e[2].match(/min/i)?t.minute=r:e[2].match(/sec/i)?t.second=r:e[2].match(/week/i)?t.week=r:e[2].match(/day/i)?t.d=r:e[2].match(/month/i)?t.month=r:e[2].match(/year/i)&&(t.year=r),t}Parser.apply(this,arguments),this.pattern=function(){return this.isStrictMode()?STRICT_PATTERN:PATTERN},this.extract=function(t,r,a,s){if(a.index>0&&t[a.index-1].match(/\w/))return null;var t=a[0];t=a[0].substr(a[1].length,a[0].length-a[1].length),index=a.index+a[1].length;var n=new ParsedResult({index:index,text:t,ref:r}),i=e(a[2]),T=moment(r);for(var o in i)T.add(-i[o],o);return(i.hour>0||i.minute>0||i.second>0)&&(n.start.assign("hour",T.hour()),n.start.assign("minute",T.minute()),n.start.assign("second",T.second()),n.tags.ENTimeAgoFormatParser=!0),i.d>0||i.month>0||i.year>0?(n.start.assign("day",T.date()),n.start.assign("month",T.month()+1),n.start.assign("year",T.year())):(i.week>0&&n.start.imply("weekday",T.day()),n.start.imply("day",T.date()),n.start.imply("month",T.month()+1),n.start.imply("year",T.year())),n}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/EN"),PATTERN=new RegExp("(\\W|^)(?:within\\s*)?("+util.INTEGER_WORDS_PATTERN+"|[0-9]+|an?(?:\\s*few)?|half(?:\\s*an?)?)\\s*(seconds?|min(?:ute)?s?|hours?|weeks?|days?|months?|years?)\\s*(?:from now|henceforth|forward|out)(?=(?:\\W|$))","i"),STRICT_PATTERN=new RegExp("(\\W|^)(?:within\\s*)?([0-9]+|an?)\\s*(seconds?|minutes?|hours?|days?)\\s*from now(?=(?:\\W|$))","i");exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return this.isStrictMode()?STRICT_PATTERN:PATTERN},this.extract=function(t,e,a,r){if(a.index>0&&t[a.index-1].match(/\w/))return null;var t=a[0];t=a[0].substr(a[1].length,a[0].length-a[1].length),index=a.index+a[1].length;var s=new ParsedResult({index:index,text:t,ref:e}),n=a[2].toLowerCase();n=void 0!==util.INTEGER_WORDS[n]?util.INTEGER_WORDS[n]:"a"===n||"an"===n?1:n.match(/few/)?3:n.match(/half/)?.5:parseInt(n);var i=moment(e);return a[3].match(/hour|min|second/i)?(a[3].match(/hour/i)?i.add(+n,"hour"):a[3].match(/min/i)?i.add(+n,"minute"):a[3].match(/second/i)&&i.add(+n,"second"),s.start.imply("day",i.date()),s.start.imply("month",i.month()+1),s.start.imply("year",i.year()),s.start.assign("hour",i.hour()),s.start.assign("minute",i.minute()),s.start.assign("second",i.second()),s.tags.ENTimeFromNowFormatParser=!0,s):a[3].match(/week/i)?(i.add(+n,"week"),s.start.imply("day",i.date()),s.start.imply("month",i.month()+1),s.start.imply("year",i.year()),s.start.imply("weekday",i.day()),s):(a[3].match(/day/i)&&i.add(+n,"d"),a[3].match(/month/i)&&i.add(+n,"month"),a[3].match(/year/i)&&i.add(+n,"year"),s.start.assign("day",i.date()),s.start.assign("month",i.month()+1),s.start.assign("year",i.year()),s)}};
