...
const typeName = v => toString$1.call(v).slice(8, -1);
...
var SubscriptionObserver = function (subscription) {
  setInternalState$2(this, { subscription: subscription });
  if (!descriptors) this.closed = false;
};
...
var $Observable = function Observable(subscriber) {
  ...
  setInternalState$2(this, { subscriber: aFunction(subscriber) });
};
...
const map = fn => source => new Observable(sink =>
  source.subscribe({
    next(value) {
      try {
        value = fn(value);
...
const spreadAssign = (...operators) => source =>
  pipe(
...
const makeBackIn = (overshoot = 1.70158) => t =>
  t * t * ((overshoot + 1) * t - overshoot);

const makeBackOut = (overshoot = 1.70158) => t =>
