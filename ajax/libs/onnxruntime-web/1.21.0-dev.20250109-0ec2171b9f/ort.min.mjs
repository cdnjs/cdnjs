/*!
 * ONNX Runtime Web v1.21.0-dev.20250109-0ec2171b9f
...
`)}get variablesInfo(){if(this.uniforms.length===0)return;let t=n=>[12,10,1,6][["u32","f16","f32","i32"].indexOf(n)];return this.uniforms.map(n=>[t(n.type),n.length??1])}},io=(e,t)=>new Bn(e,t)});var ju,so,Zu,Qu,Xu,Yu,pe,ao,uo,De=A(()=>{"use strict";U();W();re();H();ju=(e,t)=>{if(!e||e.length!==1)throw new Error("Transpose requires 1 input.");if(t.length!==0&&t.length!==e[0].dims.length)throw new Error(`perm size ${t.length} does not match input rank ${e[0].dims.length}`)},so=(e,t)=>t.length!==0?t:[...new Array(e).keys()].reverse(),Zu=(e,t)=>$.sortBasedOnPerm(e,so(e.length,t)),Qu=(e,t,n,r)=>{let o=`fn perm(i: ${r.type.indices}) -> ${n.type.indices} {
