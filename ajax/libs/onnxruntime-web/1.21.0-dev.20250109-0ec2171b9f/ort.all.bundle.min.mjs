...
`)}get variablesInfo(){if(this.uniforms.length===0)return;let e=n=>[12,10,1,6][["u32","f16","f32","i32"].indexOf(n)];return this.uniforms.map(n=>[e(n.type),n.length??1])}},Xh=(r,e)=>new Ys(r,e)});var $2,Zh,A2,O2,P2,E2,it,Yh,Jh,pr=k(()=>{"use strict";le();me();Ke();ge();$2=(r,e)=>{if(!r||r.length!==1)throw new Error("Transpose requires 1 input.");if(e.length!==0&&e.length!==r[0].dims.length)throw new Error(`perm size ${e.length} does not match input rank ${r[0].dims.length}`)},Zh=(r,e)=>e.length!==0?e:[...new Array(r).keys()].reverse(),A2=(r,e)=>C.sortBasedOnPerm(r,Zh(r.length,e)),O2=(r,e,n,t)=>{let o=`fn perm(i: ${t.type.indices}) -> ${n.type.indices} {
