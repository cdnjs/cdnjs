...
}`,ti=e=>{let t=ce(e.inputs[0].dataType);e.compute(J(e.inputs[0],"Erf",n=>`erf_vf32(${n})`,qt(t)))},ni=e=>{e.compute(J(e.inputs[0],"Exp","exp"))},ri=e=>{e.compute(J(e.inputs[0],"Floor","floor"))},oi=e=>{let t=ce(e.inputs[0].dataType);e.compute(J(e.inputs[0],"Gelu",n=>`0.5 * ${n} * (1.0 + erf_vf32(${n} * 0.7071067811865475))`,qt(t)))},ii=(e,t)=>{let n=ce(e.inputs[0].dataType);e.compute(J(e.inputs[0],"LeakyRelu",r=>`select(leaky_relu_alpha_ * ${r}, ${r}, ${r} >= vec4<${n}>(0.0))`,`const leaky_relu_alpha_ = ${n}(${t.alpha});`,t.cacheKey))},si=e=>{e.compute(J(e.inputs[0],"Not",t=>`!${t}`))},ai=e=>{e.compute(J(e.inputs[0],"Neg",t=>`-${t}`))},ui=e=>{e.compute(J(e.inputs[0],"Reciprocal",t=>`1.0/${t}`))},di=e=>{let t=ce(e.inputs[0].dataType);e.compute(J(e.inputs[0],"Relu",n=>`select(vec4<${t}>(0.0), ${n}, ${n} > vec4<${t}>(0.0))`))},li=e=>{e.compute(J(e.inputs[0],"Sigmoid",t=>`(1.0 / (1.0 + exp(-${t})))`))},ci=e=>V(e),pi=(e,t)=>{let n=ce(e.inputs[0].dataType);e.compute(J(e.inputs[0],"HardSigmoid",r=>`max(vec4<${n}>(0.0), min(vec4<${n}>(1.0), ${t.alpha} * ${r} + vec4<${n}>(${t.beta})))`,void 0,t.cacheKey))},mi=e=>{e.compute(J(e.inputs[0],"Sin","sin"))},fi=e=>{e.compute(J(e.inputs[0],"Sinh","sinh"))},hi=e=>{e.compute(J(e.inputs[0],"Sqrt","sqrt"))},gi=e=>{e.compute(J(e.inputs[0],"Tan","tan"))},yi=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,bi=e=>{e.compute(J(e.inputs[0],"Tanh",yi))},Vn=(e="f32")=>`
...
`,Nn=e=>`(fast_gelu_a + fast_gelu_a * tanh_v(${e} * (fast_gelu_c * ${e} * ${e} + fast_gelu_b))) * ${e}`,_i=e=>{let t=ce(e.inputs[0].dataType);e.compute(J(e.inputs[0],"FastGelu",Nn,Vn(t),void 0,e.inputs[0].dataType))},wi=(e,t)=>{let n=ce(e.inputs[0].dataType);return e.compute(J(e.inputs[0],"ThresholdedRelu",r=>`select(vec4<${n}>(0.0), ${r}, ${r} > thresholded_relu_alpha_)`,`const thresholded_relu_alpha_ = vec4<${n}>(${t.alpha});`,t.cacheKey)),0},$i=e=>{e.compute(J(e.inputs[0],"Log","log"))},Pd=(e,t)=>`
