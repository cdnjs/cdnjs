...
`)}get variablesInfo(){if(this.uniforms.length===0)return;let t=n=>[12,10,1,6][["u32","f16","f32","i32"].indexOf(n)];return this.uniforms.map(n=>[t(n.type),n.length??1])}},uo=(e,t)=>new Dn(e,t)});var Xu,lo,Yu,Ju,ed,td,pe,co,po,Oe=A(()=>{"use strict";L();q();re();K();Xu=(e,t)=>{if(!e||e.length!==1)throw new Error("Transpose requires 1 input.");if(t.length!==0&&t.length!==e[0].dims.length)throw new Error(`perm size ${t.length} does not match input rank ${e[0].dims.length}`)},lo=(e,t)=>t.length!==0?t:[...new Array(e).keys()].reverse(),Yu=(e,t)=>$.sortBasedOnPerm(e,lo(e.length,t)),Ju=(e,t,n,r)=>{let o=`fn perm(i: ${r.type.indices}) -> ${n.type.indices} {
  ...
  }`},{name:"TransposeCopy",shaderCache:{inputDependencies:["type"]},getRunData:()=>{let h=$.size(i);return{outputs:[{dims:i,dataType:e.dataType}],dispatchGroup:{x:Math.ceil(h/64/4)},programUniforms:[{type:12,data:Math.ceil(h/4)}]}},getShaderSource:d};let{newShape:l,newPerm:c}=ed(e.dims,o),p=$.areEqual(c,[2,3,1]),f=$.areEqual(c,[3,1,2]);if(l.length===2||p||f){s=p?[l[0],l[1]*l[2]]:f?[l[0]*l[1],l[2]]:l,a=[s[1],s[0]];let h=16;return d=b=>{let y=S("a",n,s.length),g=C("output",n,a.length);return`
  ...
  ${h.mainStart()}
    ...
    ${y.setByOffset("global_idx",b.getByIndices("aIndices"))}
       ...
       `,m=h=>`
        ...
        }`;return{name:e,shaderCache:{hint:`${t};${p}`,inputDependencies:["type"]},getShaderSource:m,getRunData:()=>({outputs:[{dims:i,dataType:o}],dispatchGroup:{x:u},programUniforms:[{type:12,data:d}]})}},ke=(e,t,n,r)=>{let o=e.inputs.length===1?n:Un(e.inputs,n),i=o.axes;i.length===0&&!o.noopWithEmptyAxes&&(i=e.inputs[0].dims.map((f,m)=>m));let s=$.normalizeAxes(i,e.inputs[0].dims.length),a=s,u=e.inputs[0],d=ld(a,e.inputs[0].dims.length);d.length>0&&(u=e.compute(pe(e.inputs[0],d),{inputs:[0],outputs:[-1]})[0],a=sd(a.length,u.dims.length));let[l,c]=ad(u.dims,a),p=l;o.keepDims&&(p=ud(l,s)),e.compute(cd(t,o.cacheKey,[u],r,e.inputs[0].dataType,p,c),{inputs:[u]})},mo=(e,t)=>{ke(e,"ReduceMeanShared",t,"mean")},fo=(e,t)=>{ke(e,"ReduceL1Shared",t,"l1")},ho=(e,t)=>{ke(e,"ReduceL2Shared",t,"l2")},go=(e,t)=>{ke(e,"ReduceLogSumExpShared",t,"logSumExp")},yo=(e,t)=>{ke(e,"ReduceMaxShared",t,"max")},bo=(e,t)=>{ke(e,"ReduceMinShared",t,"min")},_o=(e,t)=>{ke(e,"ReduceProdShared",t,"prod")},wo=(e,t)=>{ke(e,"ReduceSumShared",t,"sum")},$o=(e,t)=>{ke(e,"ReduceSumSquareShared",t,"sumSquare")},vo=(e,t)=>{ke(e,"ReduceLogSumShared",t,"logSum")}});var Ee,pd,Ht,Un,Pe,md,fd,hd,gd,yd,bd,_d,wd,$d,vd,ze,So,To,Io,Co,Ao,ko,Eo,Po,zo,Bo,Gt=A(()=>{"use strict";L();q();re();K();xo();Ee=e=>{if(!e||e.length===0||e.length>2)throw new Error("Reduce op requires 1 or 2 inputs.");if(e.length===2&&e[1].dims.length!==1)throw new Error("Invalid axes input dims.")},pd=e=>["","",`var value = ${e.getByIndices("input_indices")};`,""],Ht=(e,t,n,r,o,i,s=!1,a=!1)=>{let u=[],d=n[0].dims,l=d.length,c=$.normalizeAxes(o,l),p=!a&&c.length===0;d.forEach((b,y)=>{p||c.indexOf(y)>=0?s&&u.push(1):u.push(b)});let f=u.length,m=$.size(u);return{name:e,shaderCache:t,getShaderSource:b=>{let y=[],g=S("_A",n[0].dataType,l),_=C("output",i,f),w=r(g,_,c),v=w[2];for(let x=0,T=0;x<l;x++)p||c.indexOf(x)>=0?(s&&T++,v=`for(var j${x}: u32 = 0; j${x} < ${d[x]}; j${x}++) {
...
`,Md=e=>`quick_gelu_impl(${e})`,Si=(e,t)=>{let n=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"QuickGelu",Md,Dd(n,t.alpha),t.cacheKey,e.inputs[0].dataType))}});var Rd,Ud,Ii,Ci=A(()=>{"use strict";q();K();Kt();Rd=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},Ud=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let n=S("input",e[0].dataType,e[0].dims,4),r=S("bias",e[0].dataType,[e[0].dims[2]],4),o=C("output",e[0].dataType,t,4),i=$.size(t)/4,s=ee(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(i/64)}}),getShaderSource:u=>`
  ...
  ${u.mainStart()}
    ...
    ${o.setByOffset("global_idx","valueLeft * geluRight")}
        ...
        ${e.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.vec_size")}
        ${y}
      }`},Nd=(e,t,n,r,o,i,s=n.dataType)=>{let a=n.dims.map(h=>Number(h)??1),u=r.dims.map(h=>Number(h)??1),d=!$.areEqual(a,u),l=a,c=$.size(a),p=!1,f=!1,m=[d];if(d){let h=Ae.calcShape(a,u,!1);if(!h)throw new Error("Can't perform binary op on the given tensors");l=h.slice(),c=$.size(l);let b=$.size(a)===1,y=$.size(u)===1,g=a.length>0&&a[a.length-1]%4===0,_=u.length>0&&u[u.length-1]%4===0;m.push(b),m.push(y),m.push(g),m.push(_);let w=1;for(let v=1;v<l.length;v++){let x=a[a.length-v],T=u[u.length-v];if(x===T)w*=x;else break}w%4===0?(f=!0,p=!0):(b||y||g||_)&&(p=!0)}else p=!0;return m.push(p),{name:e,shaderCache:{hint:t+m.map(h=>h.toString()).join("_"),inputDependencies:["rank","rank"]},getShaderSource:h=>Vd(h,a,u,l,p,d,f,o,n.dataType,r.dataType,s,i),getRunData:()=>({outputs:[{dims:l,dataType:s}],dispatchGroup:{x:Math.ceil(c/64/4)},programUniforms:[{type:12,data:Math.ceil($.size(l)/4)},...E(a,u,l)]})}},Be=(e,t,n,r,o,i)=>{e.compute(Nd(t,o??"",e.inputs[0],e.inputs[1],n,r,i))},Ai=e=>{Be(e,"Add",(t,n)=>`${t}+${n}`)},ki=e=>{Be(e,"Div",(t,n)=>`${t}/${n}`)},Ei=e=>{Be(e,"Equal",{scalar:(t,n)=>`u32(${t}==${n})`,vector:(t,n)=>`vec4<u32>(${t}==${n})`},void 0,void 0,9)},Pi=e=>{Be(e,"Mul",(t,n)=>`${t}*${n}`)},zi=e=>{let t=S("input",e.inputs[0].dataType,e.inputs[0].dims).type.value;Be(e,"Pow",{scalar:(r,o)=>`pow_custom(${r},${o})`,vector:(r,o)=>`pow_vector_custom(${r},${o})`},`
...
`});var pt,Qt,Xt=A(()=>{"use strict";L();q();K();Le();pt=(e,t,n,r,o)=>{let i=r-n;return`
...
`},Qt=(e,t,n,r,o=!1,i)=>{let s=e[0].dims,a=e[1].dims,u=s[s.length-2],d=a[a.length-1],l=s[s.length-1],c=X(d),p=X(l),f=X(u),m=$.size(n)/c/f,h=e.length>2,b=r?r.slice(0,-2):n.slice(0,-2),g=[$.size(b),u,d],_=[{type:12,data:m},{type:12,data:u},{type:12,data:d},{type:12,data:l}];Te(t,_),_.push(...E(b,s,a)),h&&_.push(...E(e[2].dims)),_.push(...E(g));let w=v=>{let x=Wt("batch_dims",e[0].dataType,b.length),T=S("a",e[0].dataType,s.length,p),I=S("b",e[1].dataType,a.length,c),P=C("output",e[0].dataType,g.length,c),B=ee(P.type.tensor),M=Se(t,P.type.value,B),V=[T,I],z="";if(h){let D=o?c:1;V.push(S("bias",e[2].dataType,e[2].dims.length,D)),z=`${o?`value += bias[col / ${D}];`:`value += ${P.type.value}(bias[row + i]);`}`}let G=[{name:"output_size",type:"u32"},{name:"M",type:"u32"},{name:"N",type:"u32"},{name:"K",type:"u32"}];Ie(t,G);let ne=()=>{let D=`var a_data: ${T.type.value};`;for(let R=0;R<p;R++)D+=`
...
`}return D};return`
  ...
  `};return{name:"MatMulNaive",shaderCache:{hint:`${t.activation};${c};${p};${f};${o}`,inputDependencies:h?["rank","rank","rank"]:["rank","rank"]},getRunData:()=>({outputs:[{dims:i?i(n):n,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(m/64)},programUniforms:_}),getShaderSource:w}}});var Fd,Kd,Gn,qi,jd,Hn,Zd,mt,Yt=A(()=>{"use strict";L();q();K();Le();Xt();Zt();Fd=(e,t)=>e?`
...
`},Zd=(e,t,n,r,o=!1)=>{let[i,s,a,u]=r,d=ee(r[0].type.tensor);return`
    ...
    fn mm_write(batch: i32, row : i32, colIn : i32, valueIn : ${v}) {
      ...
      fn setOutputAtCoords(d0 : i32, d1 : i32, d2 : i32, d3 : i32, value : ${h?`vec4<${N}>`:N}) {
        ...
        fn getBiasByOutputCoords(coords : array<u32, 5>) -> ${u?`vec4<${v}>`:v} {
          ...
          }`};return{name:"Conv3DNaive",shaderCache:{hint:`${t.cacheKey};${s};${p};${b}`,inputDependencies:h},getRunData:()=>({outputs:[{dims:n,dataType:e[0].dataType}],dispatchGroup:{x:c[0],y:c[1],z:c[2]},programUniforms:m}),getShaderSource:y}}});var Ji,es,ts=A(()=>{"use strict";L();q();K();Le();Ji=(e,t,n,r)=>{let o=e.length>2,i=o?"value += b[output_channel];":"",s=e[0].dims,a=e[1].dims,u=t.format==="NHWC",d=u?n[3]:n[1],l=d/t.group,c=u&&l>=4?X(d):1,p=$.size(n)/c,f=[{type:12,data:p},{type:12,data:t.dilations},{type:12,data:[t.strides[0],t.strides[1]]},{type:12,data:[t.pads[0],t.pads[1]]},{type:12,data:l}];Te(t,f),f.push(...E(s,[a[0],a[1],a[2],a[3]/c]));let m=o?["rank","rank","rank"]:["rank","rank"];f.push(...E([n[0],n[1],n[2],n[3]/c]));let h=b=>{let y=C("output",e[0].dataType,n.length,c),g=ee(y.type.tensor),_=Se(t,y.type.value,g),w=S("x",e[0].dataType,s.length),v=S("w",e[1].dataType,a.length,c),x=[w,v];o&&x.push(S("b",e[2].dataType,e[2].dims,c));let T=[{name:"output_size",type:"u32"},{name:"dilations",type:"u32",length:t.dilations.length},{name:"strides",type:"u32",length:2},{name:"pads",type:"u32",length:2},{name:"output_channels_per_group",type:"u32"}];Ie(t,T);let I=u?`
  ...
  }`};return{name:"GroupedConv",shaderCache:{hint:`${t.cacheKey}_${c}`,inputDependencies:m},getRunData:()=>({outputs:[{dims:r?r(n):n,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(p/64)},programUniforms:f}),getShaderSource:h}},es=(e,t,n,r)=>{let o=e.length>2,i=X(n[3]),s=X(n[2]),a=$.size(n)/i/s,u=[e[0].dims[0],e[0].dims[1],e[0].dims[2],e[0].dims[3]/i],d=[e[1].dims[0],e[1].dims[1],e[1].dims[2],e[1].dims[3]/i],l=[n[0],n[1],n[2],n[3]/i],c=[{type:12,data:a},{type:6,data:[t.strides[0],t.strides[1]]},{type:6,data:[t.pads[0],t.pads[1]]}];Te(t,c),c.push(...E(u,d,l));let p=(s-1)*t.strides[1]+d[1],f=m=>{let h=C("output",e[0].dataType,l.length,i),b=ee(h.type.tensor),y=Se(t,h.type.value,b),g=S("x",e[0].dataType,u.length,i),_=S("w",e[1].dataType,d.length,i),w=[g,_];o&&w.push(S("b",e[2].dataType,e[2].dims,i));let v=o?"value += b[output_channel];":"",x=[{name:"output_size",type:"u32"},{name:"strides",type:"i32",length:2},{name:"pads",type:"i32",length:2}];return Ie(t,x),`
...
`)};
    ...
    ${m}`},l=[{type:12,data:u},...E(t,r)];return{name:"Expand",shaderCache:{hint:`${r.length};${s}${a}`,inputDependencies:["rank"]},getShaderSource:d,getRunData:()=>({outputs:[{dims:r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(u/64)},programUniforms:l})}},xs=e=>{gl(e.inputs),e.compute(bl(e.inputs),{inputs:[0]})}});var _l,Ts,Is=A(()=>{"use strict";L();q();K();Kt();_l=e=>{let t=e[0].dataType,n=$.size(e[0].dims),r=$.size(e[1].dims),o=r%4===0,i=s=>{let a=S("x",t,[1],4),u=S("bias",t,[1],4),d=C("y",t,[1],4),l=[{name:"output_vec_size",type:"u32"},{name:"bias_size",type:"u32"}],c=f=>`
    ...
    }`};return{name:"FastGeluWithBias",shaderCache:{hint:`${o}`,inputDependencies:["type","type"]},getShaderSource:i,getRunData:s=>({outputs:[{dims:s[0].dims,dataType:s[0].dataType}],programUniforms:[{type:12,data:Math.ceil(n/4)},{type:12,data:r}],dispatchGroup:{x:Math.ceil(n/je/4)}})}},Ts=e=>{e.inputs.length<2||$.size(e.inputs[1].dims)===0?$i(e):e.compute(_l(e.inputs))}});var wl,$l,Cs,As,ks=A(()=>{"use strict";L();q();re();K();wl=e=>{if(!e||e.length!==2)throw new Error("Gather requires 2 inputs.")},$l=(e,t)=>{let n=e[0].dims,r=e[1].dims,o=n.length,i=$.normalizeAxis(t.axis,o),s=n.slice(0);s.splice(i,1,...r);let a=n[i],u=e[0].dataType===9?4:1,d=Math.ceil($.size(s)/u),l=[{type:12,data:d},{type:6,data:a},{type:12,data:i},...E(e[0].dims,e[1].dims,s)],c=p=>{let f=S("data",e[0].dataType,e[0].dims.length,u),m=S("inputIndices",e[1].dataType,e[1].dims.length),h=C("output",e[0].dataType,s.length,u),b=g=>{let _=r.length,w=`var indicesIndices${g}  = ${m.type.indices}(0);`;for(let v=0;v<_;v++)w+=`${_>1?`indicesIndices${g}[${v}]`:`indicesIndices${g}`} = ${s.length>1?`outputIndices${g}[uniforms.axis + ${v}]`:`outputIndices${g}`};`;w+=`
  ...
  }`};return e.compute({name:"computeSliceOffsets",shaderCache:{hint:`${o.length}_${n.length}`,inputDependencies:["rank"]},getRunData:()=>({outputs:[{dims:l,dataType:e.inputs[1].dataType}],dispatchGroup:{x:Math.ceil(i/64)},programUniforms:d}),getShaderSource:c},{inputs:[t],outputs:[-1]})[0]},Es=(e,t)=>{let n=e.inputs,r=n[0].dims,o=n[0].dataType,i=n[1].dims,s=i[i.length-1],a=$.sizeToDimension(i,i.length-1),u=$.sizeFromDimension(r,t.batchDims+s),d=$.sizeToDimension(r,t.batchDims),l=$.sizeFromDimension(r,t.batchDims),c=a/d,p=new Array(s),f=u;for(let w=0;w<s;++w)p[s-1-w]=f,f*=r[t.batchDims+s-1-w];let m=vl(e,n[1],p,t.batchDims,r,a,c,l,s),h=t.batchDims+s;if(h>r.length)throw new Error("last dimension of indices must not be larger than rank of input tensor");let b=i.slice(0,-1).concat(r.slice(h)),y=$.size(b),g=[{type:12,data:y},{type:12,data:u},...E(n[0].dims,m.dims,b)],_=w=>{let v=S("data",n[0].dataType,n[0].dims.length),x=S("slice_offsets",12,m.dims.length),T=C("output",n[0].dataType,b.length);return`
...
`,Dl=(e,t,n)=>(()=>{switch(n.mode){case"nearest":return`
          let result = pixel_at_grid(i32(round(y)), i32(round(x)), H_in, W_in, indices[${De}], indices[${We}], border);
        `;case"bilinear":return`
          ...
          let p11 = pixel_at_grid(y1, x1, H_in, W_in, indices[${De}], indices[${We}], border);
          let p12 = pixel_at_grid(y1, x2, H_in, W_in, indices[${De}], indices[${We}], border);
          let p21 = pixel_at_grid(y2, x1, H_in, W_in, indices[${De}], indices[${We}], border);
          let p22 = pixel_at_grid(y2, x2, H_in, W_in, indices[${De}], indices[${We}], border);
        ...
        `;case"bicubic":return`
          ...
          for (var h = 0; h < 4; h++) {
            for (var w = 0; w < 4; w++) {
              p[h][w] = pixel_at_grid(h + y0, w + x0, H_in, W_in, indices[${De}], indices[${We}], border);
            }
          }
...
`)}
  ...
  }`};return e.compute({name:"InstanceNormComputeChannelScaleShift",shaderCache:{hint:`${u};${a};${p}`,inputDependencies:h},getRunData:()=>({outputs:[{dims:m,dataType:1}],dispatchGroup:{x:c},programUniforms:b}),getShaderSource:y},{inputs:[t,n,r],outputs:[-1]})[0]},Kl=(e,t,n)=>{let r=t[0].dims,o=r,i=2,s=r[0],a=r[1],u=$.sizeFromDimension(r,i),d=X(u),l=$.size(o)/d,c=Js(e,t[0],t[1],t[2],s,u,a,n.epsilon),p=[s,a,u/d],f=[s,a],m=["type","none"],h=b=>{let y=S("x",t[0].dataType,p.length,d),g=S("scale_shift",1,f.length,2),_=C("output",t[0].dataType,p.length,d),w=[y,g,_];return`
  ...
  ${y.mainStart()}
    ...
    output[global_idx] = fma(input[global_idx], ${w(0)}, ${w(1)});
       ...
       Got scale size of ${c} and bias size of ${p}`);let f=[];for(let v=0;v<o.length;++v)v<u?f.push(o[v]):f.push(1);let m=X(l),h=["type","type"],b=[{type:12,data:d},{type:1,data:l},{type:12,data:Math.floor(l/m)},{type:1,data:t.epsilon}];s&&h.push("type");let y=n>1,g=n>2,_=v=>{let x=ee(e[0].dataType),T=[S("x",e[0].dataType,e[0].dims,m),S("scale",i.dataType,i.dims,m)];s&&T.push(S("bias",s.dataType,s.dims,m)),T.push(C("output",e[0].dataType,a,m)),y&&T.push(C("mean_data_output",1,f)),g&&T.push(C("inv_std_output",1,f));let I=[{name:"norm_count",type:"u32"},{name:"norm_size",type:"f32"},{name:"norm_size_vectorized",type:"u32"},{name:"epsilon",type:"f32"}];return`
  ...
  for (var i = 0u; i < uniforms.num_updates_elements; i++) {
    ...
    ${gc(t.reduction,"output[data_offset + i]","value",m.type.value)}
      ...
      }`};return{name:"ScatterND",shaderCache:{hint:`${t.cacheKey}_${t.reduction}`,inputDependencies:["rank","rank"]},getRunData:()=>({outputs:[{dims:o,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)},programUniforms:d}),getShaderSource:l}},Ba=e=>W({reduction:e.reduction}),Oa=(e,t)=>{e.compute(yc(e.inputs,t),{inputs:[e.inputs[1],e.inputs[2]],outputs:[]})}});var bc,_c,wc,Ma,$c,vc,xc,Sc,Tc,Ic,Cc,Ac,Ra,kc,Ec,Pc,zc,Bc,Ua,Va,Na=A(()=>{"use strict";L();q();re();K();bc=(e,t)=>{if(e.every(n=>n>0||(()=>{throw new Error("Resize requires scales input values to be positive")})),e.length>0){if(t.mode==="linear"){if(!(e.length===2||e.length===3||e.length===4&&e[0]===1&&e[1]===1||e.length===4&&e[0]===1&&e[3]===1||e.length===5&&e[0]===1&&e[1]===1))throw new Error(`For linear mode, Resize requires scales to be 2D, 3D, 4D with either two outermost or one innermost and
...
`:"",kc=(e,t,n,r,o)=>{let[s,a,u,d]=n.length===2?[-1,0,1,-1]:[0,2,3,1],l=e.type.value;return`
    ...
    }`},Ec=(e,t,n,r,o,i,s,a,u,d)=>{let l=n.length===2,c=!0,[p,f]=l?[0,1]:c?[2,3]:[1,2],m=e.type.value,h=b=>{let y=b===p?"row":"col";return`
  ...
  fn bicubicInterpolation(output_indices: ${t.type.indices}) -> ${m} {
    ...
    depth = max(0, min(depth, ${n[a]} - 1));
    ...
    }`},zc=(e,t,n,r,o,i)=>{let s=e.dims,a=xc(i,t.axes,s.length),u=Sc(s,r,o,t.axes),d=r.slice();r.length===0&&(d=s.map((g,_)=>g===0?1:u[_]/g),t.keepAspectRatioPolicy!=="stretch"&&(u=Tc(s,d,t)));let l=C("output",e.dataType,u.length),c=S("input",e.dataType,s.length),p=$.size(u),f=s.length===u.length&&s.every((g,_)=>g===u[_]),m=t.coordinateTransformMode==="tf_crop_and_resize",h=t.extrapolationValue,b=c.type.value,y=g=>`
      ${f?"":`
      ...
      ${(()=>{switch(t.mode){case"nearest":return`
              ...
              ${(()=>{if(s.length===2||s.length===4)return`${kc(c,l,s,m,h)}`;if(s.length===3||s.length===5)return`${Pc(c,l,s,m,h)}`;throw Error("Linear mode only supports input dims 2, 3, 4 and 5 are supported in linear mode.")})()};
            `;case"cubic":return`
...
`}
        ...
        }`};return{name:"RotaryEmbedding",shaderCache:{hint:W({interleaved:n}).cacheKey,inputDependencies:["rank","rank","rank","rank"]},getShaderSource:h,getRunData:()=>({outputs:[{dims:e[0].dims,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil($.size(p)/je)},programUniforms:m})}},La=(e,t)=>{Oc(e.inputs,t),e.compute(Dc(e.inputs,t))}});var Mc,Rc,Ga,Ha=A(()=>{"use strict";L();q();K();Mc=e=>{if(!e||e.length<3)throw new Error("layerNorm requires at least 3 inputs.");let t=e[0],n=e[1],r=e[2];if(t.dataType!==n.dataType||t.dataType!==r.dataType)throw new Error("All inputs must have the same data type");if(t.dims.length!==3&&t.dims.length!==2)throw new Error("Input must be 2D or 3D");if(n.dims.length!==3&&n.dims.length!==2)throw new Error("Skip must be 2D or 3D");let o=t.dims[t.dims.length-1],i=t.dims[t.dims.length-2];if(n.dims[n.dims.length-1]!==o)throw new Error("Skip must have the same hidden size as input");if(n.dims[n.dims.length-2]!==i)throw new Error("Skip must have the same sequence length as input");if(r.dims.length!==1)throw new Error("Gamma must be 1D");if(r.dims[r.dims.length-1]!==o)throw new Error("Gamma must have the same hidden size as input");if(e.length>3){let s=e[3];if(s.dims.length!==1)throw new Error("Beta must be 1D");if(s.dims[s.dims.length-1]!==o)throw new Error("Beta must have the same hidden size as input")}if(e.length>4){let s=e[4];if(s.dims.length!==1)throw new Error("Bias must be 1D");if(s.dims[s.dims.length-1]!==o)throw new Error("Bias must have the same hidden size as input")}},Rc=(e,t,n,r)=>{let o=t.simplified,i=e[0].dims,s=$.size(i),a=i,u=s,d=i.slice(-1)[0],l=r?i.slice(0,-1).concat(1):[],c=!o&&e.length>3,p=e.length>4,f=r&&n>1,m=r&&n>2,h=n>3,b=64,y=X(d),g=[{type:12,data:u},{type:12,data:y},{type:12,data:d},{type:1,data:t.epsilon}],_=v=>{let x=[{name:"output_size",type:"u32"},{name:"components",type:"u32"},{name:"hidden_size",type:"u32"},{name:"epsilon",type:"f32"}],T=[S("x",e[0].dataType,e[0].dims,y),S("skip",e[1].dataType,e[1].dims,y),S("gamma",e[2].dataType,e[2].dims,y)];c&&T.push(S("beta",e[3].dataType,e[3].dims,y)),p&&T.push(S("bias",e[4].dataType,e[4].dims,y)),T.push(C("output",e[0].dataType,a,y)),f&&T.push(C("mean_output",1,l)),m&&T.push(C("inv_std_output",1,l)),h&&T.push(C("input_skip_bias_sum",e[0].dataType,a,y));let I=ee(e[0].dataType),P=ee(1,y);return`
      ...
      }`},w=[{dims:a,dataType:e[0].dataType}];return n>1&&w.push({dims:l,dataType:1}),n>2&&w.push({dims:l,dataType:1}),n>3&&w.push({dims:i,dataType:e[0].dataType}),{name:"SkipLayerNormalization",shaderCache:{hint:`${y};${f};${m};${h}`,inputDependencies:e.map((v,x)=>"type")},getShaderSource:_,getRunData:()=>({outputs:w,dispatchGroup:{x:Math.ceil(u/d)},programUniforms:g})}},Ga=(e,t)=>{Mc(e.inputs);let r=[0];e.outputCount>1&&r.push(-3),e.outputCount>2&&r.push(-3),e.outputCount>3&&r.push(3),e.compute(Rc(e.inputs,t,e.outputCount,!1),{outputs:r})}});var Uc,nn,Vc,qa,Nc,Lc,Fa,Ka,ja=A(()=>{"use strict";L();q();re();K();Uc=(e,t)=>{if(!e||e.length<1)throw new Error("too few inputs");if(t.axes.length!==0){if(t.axes.length!==t.starts.length||t.axes.length!==t.ends.length)throw new Error("axes, starts and ends must have the same length")}else if(t.starts.length!==t.ends.length)throw new Error("starts and ends must have the same length");e.slice(1).forEach((n,r)=>{if(e[r+1].dataType!==6&&e[r+1].dataType!==7)throw new Error(`Input ${r} must be an array of int32 or int64`)})},nn=(e,t)=>{let n=[];if(e.length>t)if(e[t].dataType===7)e[t].getBigInt64Array().forEach(r=>n.push(Number(r)));else if(e[t].dataType===6)e[t].getInt32Array().forEach(r=>n.push(Number(r)));else throw new Error(`Input ${t} must be an array of int32 or int64`);return n},Vc=(e,t)=>{if(e.length>1){let n=nn(e,1),r=nn(e,2),o=nn(e,3);return o.length===0&&(o=[...Array(e[0].dims.length).keys()]),W({starts:n,ends:r,axes:o})}else return t},qa=(e,t,n,r,o)=>{let i=e;return e<0&&(i+=n[r[t]]),o[t]<0?Math.max(0,Math.min(i,n[r[t]]-1)):Math.max(0,Math.min(i,n[r[t]]))},Nc=(e,t,n)=>`fn calculateInputIndices(output_indices: ${t.type.indices}) -> ${e.type.indices} {
          var input_indices: ${e.type.indices};
          var carry = 0u;
      ...
      }`;return{name:"Slice",shaderCache:{hint:`${u.length}_${s.length}_${i.length}`,inputDependencies:["rank"]},getShaderSource:b,getRunData:()=>({outputs:[l],dispatchGroup:{x:Math.ceil(r/64)},programUniforms:h})}},Fa=(e,t)=>{Uc(e.inputs,t);let n=Vc(e.inputs,t);e.compute(Lc(e.inputs,n),{inputs:[0]})},Ka=e=>{let t=e.starts,n=e.ends,r=e.axes;return W({starts:t,ends:n,axes:r})}});var Wc,Gc,Za,Qa,Xa=A(()=>{"use strict";L();q();re();Oe();K();Wc=e=>{if(!e||e.length!==1)throw new Error("Softmax op requires 1 input.")},Gc=(e,t)=>{let n=e.inputs[0],r=n.dims,o=$.size(r),i=r.length,s=$.normalizeAxis(t.axis,i),a=s<r.length-1,u,d=[];a?(d=Array.from({length:i},(T,I)=>I),d[s]=i-1,d[i-1]=s,u=e.compute(pe(n,d),{inputs:[n],outputs:[-1]})[0]):u=n;let l=u.dims,c=l[i-1],p=o/c,f=X(c),m=c/f,h=64;p===1&&(h=256);let b=(T,I)=>I===4?`max(max(${T}.x, ${T}.y), max(${T}.z, ${T}.w))`:I===2?`max(${T}.x, ${T}.y)`:I===3?`max(max(${T}.x, ${T}.y), ${T}.z)`:T,y=S("x",u.dataType,u.dims,f),g=C("result",u.dataType,u.dims,f),_=y.type.value,w=ee(u.dataType)==="f32"?`var threadMax = ${_}(-3.402823e+38f);`:`var threadMax = ${_}(-65504.0h);`,v=T=>`
    ...
    }`;return{name:"Tile",shaderCache:{hint:`${r}`,inputDependencies:["rank"]},getRunData:()=>({outputs:[{dims:o,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(i/64)},programUniforms:[{type:12,data:i},...E(e[0].dims,o)]}),getShaderSource:d}},Ja=e=>{Hc(e.inputs),e.compute(Fc(e.inputs),{inputs:[0]})}});var Kc,jc,tu,nu=A(()=>{"use strict";L();q();K();Kc=(e,t,n,r,o)=>{let i=C("output_data",o,n.length,4),s=S("a_data",t[1].dataType,t[1].dims.length,4),a=S("b_data",t[2].dataType,t[2].dims.length,4),u=S("c_data",t[0].dataType,t[0].dims.length,4),d,l=(c,p,f)=>`select(${p}, ${c}, ${f})`;if(!r)d=i.setByOffset("global_idx",l(s.getByOffset("global_idx"),a.getByOffset("global_idx"),u.getByOffset("global_idx")));else{let c=(p,f,m="")=>{let h=`a_data[index_a${f}][component_a${f}]`,b=`b_data[index_b${f}][component_b${f}]`,y=`bool(c_data[index_c${f}] & (0xffu << (component_c${f} * 8)))`;return`
