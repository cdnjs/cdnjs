...
}`,ei=e=>{let t=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"Erf",n=>`erf_vf32(${n})`,Ht(t)))},ti=e=>{e.compute(Y(e.inputs[0],"Exp","exp"))},ni=e=>{e.compute(Y(e.inputs[0],"Floor","floor"))},ri=e=>{let t=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"Gelu",n=>`0.5 * ${n} * (1.0 + erf_vf32(${n} * 0.7071067811865475))`,Ht(t)))},oi=(e,t)=>{let n=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"LeakyRelu",r=>`select(leaky_relu_alpha_ * ${r}, ${r}, ${r} >= vec4<${n}>(0.0))`,`const leaky_relu_alpha_ = ${n}(${t.alpha});`,t.cacheKey))},ii=e=>{e.compute(Y(e.inputs[0],"Not",t=>`!${t}`))},si=e=>{e.compute(Y(e.inputs[0],"Neg",t=>`-${t}`))},ai=e=>{e.compute(Y(e.inputs[0],"Reciprocal",t=>`1.0/${t}`))},ui=e=>{let t=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"Relu",n=>`select(vec4<${t}>(0.0), ${n}, ${n} > vec4<${t}>(0.0))`))},di=e=>{e.compute(Y(e.inputs[0],"Sigmoid",t=>`(1.0 / (1.0 + exp(-${t})))`))},li=e=>V(e),ci=(e,t)=>{let n=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"HardSigmoid",r=>`max(vec4<${n}>(0.0), min(vec4<${n}>(1.0), ${t.alpha} * ${r} + vec4<${n}>(${t.beta})))`,void 0,t.cacheKey))},pi=e=>{e.compute(Y(e.inputs[0],"Sin","sin"))},mi=e=>{e.compute(Y(e.inputs[0],"Sinh","sinh"))},fi=e=>{e.compute(Y(e.inputs[0],"Sqrt","sqrt"))},hi=e=>{e.compute(Y(e.inputs[0],"Tan","tan"))},gi=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,yi=e=>{e.compute(Y(e.inputs[0],"Tanh",gi))},Vn=(e="f32")=>`
...
`,Nn=e=>`(fast_gelu_a + fast_gelu_a * tanh_v(${e} * (fast_gelu_c * ${e} * ${e} + fast_gelu_b))) * ${e}`,bi=e=>{let t=ce(e.inputs[0].dataType);e.compute(Y(e.inputs[0],"FastGelu",Nn,Vn(t),void 0,e.inputs[0].dataType))},_i=(e,t)=>{let n=ce(e.inputs[0].dataType);return e.compute(Y(e.inputs[0],"ThresholdedRelu",r=>`select(vec4<${n}>(0.0), ${r}, ${r} > thresholded_relu_alpha_)`,`const thresholded_relu_alpha_ = vec4<${n}>(${t.alpha});`,t.cacheKey)),0},wi=e=>{e.compute(Y(e.inputs[0],"Log","log"))},Pd=(e,t)=>`
  ...
  ${f?Ln(b,y,N,ne):Wn(b,y,N,ne)}
    ...
    fn mm_write(batch: i32, row : i32, colIn : i32, valueIn : ${v}) {
      ...
      fn setOutputAtCoords(d0 : i32, d1 : i32, d2 : i32, d3 : i32, value : ${f?`vec4<${F}>`:F}) {
        ...
        }`}let O=ue(p,v),L=Se(t,O,v);return`
            ${B}
