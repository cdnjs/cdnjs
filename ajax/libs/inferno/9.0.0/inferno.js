(function (global, factory) {
    ...
    function createVNode(flags, type, className, children, childFlags, props, key, ref) {
      ...
      var childFlag = childFlags === undefined ? 1 /* ChildFlags.HasInvalidChildren */ : childFlags;
    ...
    function normalizeProps(vNode) {
      ...
      if (props) {
        var flags = vNode.flags;
        if (flags & 481 /* VNodeFlags.Element */) {
          if (props.children !== undefined && isNullOrUndef(vNode.children)) {
    ...
    function mountRef(ref, value, lifecycle) {
      if (!isNullOrUndef(ref) && (isFunction(ref) || ref.current !== undefined)) {
    ...
    function patchDangerInnerHTML(lastValue, nextValue, lastVNode, dom, animations) {
      var lastHtml = (lastValue == null ? undefined : lastValue.__html) || '';
      var nextHtml = (nextValue == null ? undefined : nextValue.__html) || '';
      ...
      switch (prop) {
        ...
        default:
          if (syntheticEvents[prop]) {
            ...
            {
              if (prop === 'href' && isString(nextValue) && nextValue.startsWith('javascript:')) {
    ...
    function renderNewInput(instance, props, context) {
      ...
      if (isFunction(instance.getChildContext)) {
        childContext = _extends$2({}, context, instance.getChildContext());
      }
      instance.$CX = childContext;
      return nextInput;
    }
    function createClassComponentInstance(vNode, ComponentCtr, props, context, isSVG, lifecycle) {
      var instance = new ComponentCtr(props, context);
      var usesNewAPI = instance.$N = Boolean(ComponentCtr.getDerivedStateFromProps || instance.getSnapshotBeforeUpdate);
      instance.$SVG = isSVG;
    ...
    function mountClassComponentCallbacks(ref, instance, lifecycle, animations) {
      mountRef(ref, instance, lifecycle);
      {
        if (isStringOrNumber(ref)) {
          throwError('string "refs" are not supported in Inferno 1.0. Use callback ref or Inferno.createRef() API instead.');
        } else if (!isNullOrUndef(ref) && typeof ref === 'object' && ref.current === undefined) {
    ...
    function updateClassComponent(instance, nextState, nextProps, parentDOM, context, isSVG, force, nextNode, lifecycle, animations) {
      ...
      if (usesNewAPI) {
        nextState = createDerivedState(instance, nextProps, nextState !== lastState ? _extends$1({}, lastState, nextState) : nextState);
      }
    ...
    function patchClassComponent(lastVNode, nextVNode, parentDOM, context, isSVG, nextNode, lifecycle, animations) {
      ...
      if (!instance.$N) {
        ...
        if (!isNull(instance.$PS)) {
          nextState = _extends$1({}, nextState, instance.$PS);
          instance.$PS = null;
        }
      }
    ...
    var nextTick = Promise.resolve().then.bind(Promise.resolve());
    var microTaskPending = false;
    function queueStateChanges(component, newState, callback, force) {
      var pending = component.$PS;
      if (isFunction(newState)) {
        newState = newState(pending ? _extends({}, component.state, pending) : component.state, component.props, component.context);
      }
    ...
    function applyState(component, force) {
      if (force || !component.$BR) {
        ...
        var animations = new AnimationQueues();
        renderCheck.v = true;
        updateClassComponent(component, _extends({}, component.state, pendingState), component.props, findDOMFromVNode(component.$LI, true).parentNode, component.context, component.$SVG, force, null, lifecycle, animations);
        callAll(lifecycle);
        callAllAnimationHooks(animations.componentDidAppear);
        renderCheck.v = false;
      } else {
...

    {
      ...
      var skipWarnings = typeof SKIP_INFERNO_WARNINGS !== 'undefined' || typeof process === 'object' && (((_process$env = process.env) == null ? undefined : _process$env.SKIP_INFERNO_WARNINGS) !== undefined || ((_process$env2 = process.env) == null ? undefined : _process$env2.JEST_WORKER_ID) !== undefined);
      if (!skipWarnings) {
        ...
        if (!(testFunc.name || testFunc.toString()).includes('testFn')) {
          warning("It looks like you're using a minified copy of the development build " + 'of Inferno. When deploying Inferno apps to production, make sure to use ' + 'the production build which skips development warnings and is faster. ' + 'See https://infernojs.org for more details.');
        }
      }
    }
    var version = "9.0.0";
