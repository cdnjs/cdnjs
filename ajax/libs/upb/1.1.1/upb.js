...
function generateChecksum(commandCheck) {
...
function generate(command, callbackFinal) {// generate(commandInputAsJSON, function(commandJSON, err){});
	...
	if (!validCommands.contains(String(command.cmd)) || command.cmd == '1') {
		callbackFinal(null, new Error ("Invalid command specified! Please us the correct command name or number."));
	}
	if (command.powerlineRepeater && !(command.powerlineRepeater == 1 || command.powerlineRepeater == 2 || command.powerlineRepeater == 4 || command.powerlineRepeater == 'false')) { callbackFinal(null, new Error ("Power line repeater argument only accepts 1, 2, 4, or false!")); }
	...
	if (command.cmd == 20 || command.cmd == "activate") {command.cmd = "activate"; command.msg = 20}
	else if (command.cmd == 21 || command.cmd == "deactivate") {command.msg = 21; command.cmd = "deactivate"}
	else if (command.cmd == 22 || command.cmd == "goto") {command.msg = 22; command.cmd = "goto"}
	else if (command.cmd == 23 || command.cmd == "fadeStart") {command.msg = 23; command.cmd = "fadeStart"}
	else if (command.cmd == 24 || command.cmd == "fadeStop") {command.msg = 24; command.cmd = "fadeStop"}
	else if (command.cmd == 25 || command.cmd == "blink") {command.msg = 25; command.cmd = "blink"}
	else if (command.cmd == 26 || command.cmd == "indicate") {command.msg = 26; command.cmd = "indicate"}
	else if (command.cmd == 27 || command.cmd == "toggle") {command.msg = 27; command.cmd = "toggle"}
	else if (command.cmd == 30 || command.cmd == "reportState") {command.msg = 30; command.cmd = "reportState"}
	else if (command.cmd == 31 || command.cmd == "storeState") {command.msg = 31; command.cmd = "storeState"}
	else if (command.cmd == 80 || command.cmd == "ackResponse") {command.msg = 80; command.cmd = "ackResponse"}
	else if (command.cmd == 85 || command.cmd == "setupTimeReport") {command.msg = 85; command.cmd = "setupTimeReport"}
	else if (command.cmd == 86 || command.cmd == "deviceStateReport") {command.msg = 86; command.cmd = "deviceStateReport"}
	else if (command.cmd == 87 || command.cmd == "deviceStatusReport") {command.msg = 87; command.cmd = "deviceStatusReport"}
	else if (command.cmd == '8F' || command.cmd == "deviceSignatureReport") {command.msg == '8F'; command.cmd = "deviceSignatureReport"}
	else if (command.cmd == 90 || command.cmd == "registerValuesReport") {command.msg = 90; command.cmd = "registerValuesReport"}
	else if (command.cmd == 91 || command.cmd == "RAMvaluesReport") {command.msg = 91; command.cmd = "RAMvaluesReport"}
	else if (command.cmd == 92 || command.cmd == "rawDataReport") {command.msg = 92; command.cmd = "rawDataReport"}
	else if (command.cmd == 93 || command.cmd == "heartbeatReport") {command.msg = 93; command.cmd = "heartbeatReport"}
	...
	command.checksum = generateChecksum(command);
