 ...
 */
function TransportAPI(matchID, socket, clientInfo, roomInfo, filterPlayerView, provisionalClient) {
    /**
        ...
        for (const game of games) {
            ...
            nsp.on('connection', (socket) => {
                socket.on('update', async (...args) => {
                    ...
                    const master = new Master(game, app.context.db, TransportAPI(matchID, socket, this.clientInfo, this.roomInfo, filterPlayerView), app.context.auth);
                ...
                socket.on('sync', async (...args) => {
                    ...
                    const transport = TransportAPI(matchID, socket, this.clientInfo, this.roomInfo, filterPlayerView, client);
