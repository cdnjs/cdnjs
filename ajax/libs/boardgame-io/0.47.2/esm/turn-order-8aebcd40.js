    ...
    update(state) {
        ...
        EventQueue: for (let i = 0; i < this.dispatch.length; i++) {
            ...
            if (endedTurns >= this.maxEndedTurnsPerAction) {
                return stateWithError(event.error, "Maximum number of turn endings exceeded for this update.\nThis likely means game code is triggering an infinite loop." /* MaxTurnEndings */);
            }
            if (event.calledFrom === undefined) {
                return stateWithError(event.error, "Events must be called from moves or the `onBegin`, `onEnd`, and `onMove` hooks.\nThis error probably means you called an event from other game code, like an `endIf` trigger or one of the `turn.order` methods." /* CalledOutsideHook */);
            }
            switch (event.type) {
                case 'endStage':
                case 'setStage':
                case 'setActivePlayers': {
                    switch (event.calledFrom) {
                        ...
                        case GameMethod.PHASE_ON_END:
                            return stateWithError(event.error, "`setStage`, `endStage` & `setActivePlayers` are disallowed in `onEnd` hooks." /* StageEventInOnEnd */);
                        case GameMethod.PHASE_ON_BEGIN:
                            return stateWithError(event.error, "`setStage`, `endStage` & `setActivePlayers` are disallowed in a phase\u2019s `onBegin` hook.\nUse `setActivePlayers` in a `turn.onBegin` hook or declare stages with `turn.activePlayers` instead." /* StageEventInPhaseBegin */);
                        // Disallow setStage & endStage in turn.onBegin hooks.
                        case GameMethod.TURN_ON_BEGIN:
                            if (event.type === 'setActivePlayers')
                                break;
                            return stateWithError(event.error, "`setStage` & `endStage` are disallowed in `turn.onBegin`.\nUse `setActivePlayers` or declare stages with `turn.activePlayers` instead." /* StageEventInTurnBegin */);
                    }
                ...
                case 'endTurn': {
                    if (event.calledFrom === GameMethod.TURN_ON_END ||
                        ...
                        return stateWithError(event.error, "`endTurn` is disallowed in `onEnd` hooks \u2014 the turn is already ending." /* EndTurnInOnEnd */);
                    }
                ...
                case 'setPhase': {
                    if (event.calledFrom === GameMethod.PHASE_ON_END) {
                        return stateWithError(event.error, "`setPhase` & `endPhase` are disallowed in a phase\u2019s `onEnd` hook \u2014 the phase is already ending.\nIf you\u2019re trying to dynamically choose the next phase when a phase ends, use the phase\u2019s `next` trigger." /* PhaseEventInOnEnd */);
                    }
...
export { ActionCreators as A, update as B, patch as C, EnhanceCtx as E, FnWrap as F, GameMethod as G, InitTurnOrderState as I, MAKE_MOVE as M, NoClient as N, PLUGIN as P, RESET as R, SetActivePlayers as S, TurnOrder as T, UpdateTurnOrderState as U, Stage as a, UpdateActivePlayersOnceEmpty as b, STRIP_TRANSIENTS as c, GAME_EVENT as d, error as e, Enhance as f, gameEvent as g, INVALID_MOVE as h, info as i, UPDATE as j, SYNC as k, UNDO as l, REDO as m, ProcessAction as n, PATCH as o, FlushAndValidate as p, stripTransients as q, Setup as r, sync as s, reset as t, undo as u, redo as v, PlayerView as w, makeMove as x, alea as y, ActivePlayers as z };
