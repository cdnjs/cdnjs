...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["flatpickr"] = factory();
	else
		root["flatpickr"] = factory();
})(this, function() {
...
var formatting_do_nothing = function do_nothing() {
  return undefined;
};

var formatting_monthToStr = function monthToStr(monthNumber, shorthand, locale) {
  return locale.months[shorthand ? "shorthand" : "longhand"][monthNumber];
};
var formatting_revFormat = {
  D: formatting_do_nothing,
  ...
  w: formatting_do_nothing,
  y: function y(dateObj, year) {
    dateObj.setFullYear(2000 + parseFloat(year));
  }
};
var formatting_tokenRegex = {
  D: "(\\w+)",
  F: "(\\w+)",
  G: "(\\d\\d|\\d)",
  H: "(\\d\\d|\\d)",
...
var formatting_formats = {
  ...
  F: function F(date, locale, options) {
    return formatting_monthToStr(formatting_formats.n(date, locale, options) - 1, false, locale);
  },
  // padded hour 1-12
  ...
  M: function M(date, locale) {
    return formatting_monthToStr(date.getMonth(), true, locale);
  },
  // seconds 00-59
  S: function S(date) {
...
var dates_createDateFormatter = function createDateFormatter(_ref) {
  var _ref$config = _ref.config,
      config = _ref$config === void 0 ? options_defaults : _ref$config,
      _ref$l10n = _ref.l10n,
  ...
  return function (dateObj, frmt, overrideLocale) {
    ...
    return frmt.split("").map(function (c, i, arr) {
      return formatting_formats[c] && arr[i - 1] !== "\\" ? formatting_formats[c](dateObj, locale, config) : c !== "\\" ? c : "";
    }).join("");
  };
};
var dates_createDateParser = function createDateParser(_ref2) {
  var _ref2$config = _ref2.config,
      config = _ref2$config === void 0 ? options_defaults : _ref2$config,
  ...
  return function (date, givenFormat, timeless) {
    ...
    if (date instanceof Date) parsedDate = new Date(date.getTime());else if (typeof date !== "string" && date.toFixed !== undefined // timestamp
      ...
      if (datestr === "today") {
        ...
        for (var i = 0, matchIndex = 0, regexStr = ""; i < format.length; i++) {
          ...
          if (formatting_tokenRegex[token] && !escaped) {
            regexStr += formatting_tokenRegex[token];
            var match = new RegExp(regexStr).exec(date);

            if (match && (matched = true)) {
              ops[token !== "Y" ? "push" : "unshift"]({
                fn: formatting_revFormat[token],
    ...
    if (!(parsedDate instanceof Date)) {
      config.errorHandler(new Error("Invalid date provided: ".concat(date_orig)));
      return undefined;
    }
...
function dates_compareDates(date1, date2) {
...
function dates_compareTimes(date1, date2) {
  return 3600 * (date1.getHours() - date2.getHours()) + 60 * (date1.getMinutes() - date2.getMinutes()) + date1.getSeconds() - date2.getSeconds();
}
var dates_getWeek = function getWeek(givenDate) {
...
var dates_isBetween = function isBetween(ts, ts1, ts2) {
  return ts > Math.min(ts1, ts2) && ts < Math.max(ts1, ts2);
};
var dates_duration = {
  DAY: 86400000
};
// CONCATENATED MODULE: ./src/types/options.ts
function options_typeof(obj) { if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { options_typeof = function _typeof(obj) { return typeof obj; }; } else { options_typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return options_typeof(obj); }
...
var options_defaults = {
  ...
  animate: (typeof window === "undefined" ? "undefined" : options_typeof(window)) === "object" && window.navigator.userAgent.indexOf("MSIE") === -1,
...
// EXTERNAL MODULE: ./src/utils/polyfills.ts
var polyfills = __webpack_require__("xkpU");
...
var src_DEBOUNCED_CHANGE_MS = 300;

function src_FlatpickrInstance(element, instanceConfig) {
  var self = {
    config: Object.assign({}, src_flatpickr_0.defaultConfig),
    l10n: l10n_default
  };
  self.parseDate = dates_createDateParser({
    config: self.config,
    l10n: self.l10n
  });
  self._handlers = [];
  ...
  function setHoursFromInputs() {
    ...
    var limitMinHours = self.config.minTime !== undefined || self.config.minDate && self.minDateHasTime && self.latestSelectedDateObj && dates_compareDates(self.latestSelectedDateObj, self.config.minDate, true) === 0;
    var limitMaxHours = self.config.maxTime !== undefined || self.config.maxDate && self.maxDateHasTime && self.latestSelectedDateObj && dates_compareDates(self.latestSelectedDateObj, self.config.maxDate, true) === 0;
  ...
  function bindEvents() {
    ...
    self._debouncedChange = debounce(triggerChange, src_DEBOUNCED_CHANGE_MS);
  ...
  function createDay(className, date, dayNumber, i) {
    ...
    if (dates_compareDates(date, self.now) === 0) {
    ...
    if (dateIsEnabled) {
      ...
      if (isDateSelected(date)) {
        ...
        if (self.config.mode === "range") {
          toggleClass(dayElement, "startRange", self.selectedDates[0] && dates_compareDates(date, self.selectedDates[0], true) === 0);
          toggleClass(dayElement, "endRange", self.selectedDates[1] && dates_compareDates(date, self.selectedDates[1], true) === 0);
  ...
  function updateWeekdays() {
    ...
    var weekdays = src_toConsumableArray(self.l10n.weekdays.shorthand);

    if (firstDayOfWeek > 0 && firstDayOfWeek < weekdays.length) {
      weekdays = src_toConsumableArray(weekdays.splice(firstDayOfWeek, weekdays.length)).concat(src_toConsumableArray(weekdays.splice(0, firstDayOfWeek)));
    }
  ...
  function changeMonth(value) {
    var is_offset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    var from_keyboard = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  function changeYear(newYear) {
    if (!newYear || self.config.minDate && newYear < self.config.minDate.getFullYear() || self.config.minDate && newYear > self.config.minDate.getFullYear()) return;
  ...
  function isEnabled(date) {
    ...
    if (self.config.minDate && dateToCheck && dates_compareDates(dateToCheck, self.config.minDate, timeless !== undefined ? timeless : !self.minDateHasTime) < 0 || self.config.maxDate && dateToCheck && dates_compareDates(dateToCheck, self.config.maxDate, timeless !== undefined ? timeless : !self.maxDateHasTime) > 0) return false;
    ...
    for (var i = 0, d; i < array.length; i++) {
      ...
      src_typeof(d) === "object" && dateToCheck !== undefined && d.from && d.to && dateToCheck.getTime() >= d.from.getTime() && dateToCheck.getTime() <= d.to.getTime()) return bool;
    }
      ...
      switch (e.keyCode) {
        ...
        case 9:
          if (e.target === self.hourElement) {
            e.preventDefault(); // can't have hour elem without minute element
  ...
  function onMouseOver(elem) {
    ...
    for (var t = firstDay; t < lastDay; t += dates_duration.DAY) {
    ...
    for (var m = 0; m < self.config.showMonths; m++) {
      ...
      var _loop = function _loop(i, l) {
        ...
        if (month.contains(elem) || !(m > 0 && prevMonth && prevMonth.lastChild.dateObj.getTime() >= timestamp)) {
          ...
          if (timestamp >= minRange && (maxRange === 0 || timestamp <= maxRange) && dates_isBetween(timestamp, initialDate, hoverDate)) dayElem.classList.add("inRange");
        }
      };
  ...
  function open(e) {
    var positionElement = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : self._input;
  ...
  function parseConfig() {
    ...
    if (!userConfig.dateFormat && userConfig.enableTime) {
      formats.dateFormat = userConfig.noCalendar ? "H:i" + (userConfig.enableSeconds ? ":S" : "") : src_flatpickr_0.defaultConfig.dateFormat + " H:i" + (userConfig.enableSeconds ? ":S" : "");
    }

    if (userConfig.altInput && userConfig.enableTime && !userConfig.altFormat) {
      formats.altFormat = userConfig.noCalendar ? "h:i" + (userConfig.enableSeconds ? ":S K" : " K") : src_flatpickr_0.defaultConfig.altFormat + " h:i".concat(userConfig.enableSeconds ? ":S" : "", " K");
    }
  ...
  function setupLocale() {
    if (src_typeof(self.config.locale) !== "object" && typeof src_flatpickr_0.l10ns[self.config.locale] === "undefined") self.config.errorHandler(new Error("flatpickr: invalid locale ".concat(self.config.locale)));
    self.l10n = Object.assign({}, src_flatpickr_0.l10ns.default, src_typeof(self.config.locale) === "object" ? self.config.locale : self.config.locale !== "default" ? src_flatpickr_0.l10ns[self.config.locale] : undefined);
    formatting_tokenRegex.K = "(".concat(self.l10n.amPM[0], "|").concat(self.l10n.amPM[1], "|").concat(self.l10n.amPM[0].toLowerCase(), "|").concat(self.l10n.amPM[1].toLowerCase(), ")");
    self.formatDate = dates_createDateFormatter(self);
  }
  ...
  function selectDate(e) {
    ...
    if (self.config.mode === "single") self.selectedDates = [selectedDate];else if (self.config.mode === "multiple") {
      ...
      if (dates_compareDates(selectedDate, self.selectedDates[0], true) !== 0) self.selectedDates.sort(function (a, b) {
    ...
    buildDays();
    if (self.config.minDate && self.minDateHasTime && self.config.enableTime && dates_compareDates(selectedDate, self.config.minDate) === 0) setHoursFromDate(self.config.minDate);
    updateValue();
  ...
  function set(option, value) {
    if (option !== null && src_typeof(option) === "object") Object.assign(self.config, option);else {
  ...
  function parseDateRules(arr) {
    return arr.map(function (rule) {
      ...
      } else if (rule && src_typeof(rule) === "object" && rule.from && rule.to) return {
  ...
  function isDateSelected(date) {
    for (var i = 0; i < self.selectedDates.length; i++) {
      if (dates_compareDates(self.selectedDates[i], date) === 0) return "" + i;
    }
  ...
  function isDateInRange(date) {
    ...
    return dates_compareDates(date, self.selectedDates[0]) >= 0 && dates_compareDates(date, self.selectedDates[1]) <= 0;
  }
  ...
  function updateNavigationCurrentMonth() {
    ...
    self.yearElements.forEach(function (yearElement, i) {
      var d = new Date(self.currentYear, self.currentMonth, 1);
      d.setMonth(self.currentMonth + i);
      self.monthElements[i].textContent = formatting_monthToStr(d.getMonth(), self.config.shorthandCurrentMonth, self.l10n) + " ";
  ...
  function onMonthNavClick(e) {
    ...
    } else if (self.yearElements.indexOf(e.target) > -1) {
      e.preventDefault();
      e.target.select();
...
function src_flatpickr(nodeList, config) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    try {
      ...
      node._flatpickr = src_FlatpickrInstance(node, config || {});
...
if (typeof HTMLElement !== "undefined") {
  ...
  HTMLCollection.prototype.flatpickr = NodeList.prototype.flatpickr = function (config) {
    return src_flatpickr(this, config);
  };
...
src_flatpickr_0 = function flatpickr(selector, config) {
  if (selector instanceof NodeList) return src_flatpickr(selector, config);else if (typeof selector === "string") return src_flatpickr(window.document.querySelectorAll(selector), config);
  return src_flatpickr([selector], config);
};
/* istanbul ignore next */


src_flatpickr_0.defaultConfig = options_defaults;
src_flatpickr_0.l10ns = {
  en: Object.assign({}, l10n_default),
  default: Object.assign({}, l10n_default)
};

src_flatpickr_0.localize = function (l10n) {
  src_flatpickr_0.l10ns.default = Object.assign({}, src_flatpickr_0.l10ns.default, l10n);
};

src_flatpickr_0.setDefaults = function (config) {
  src_flatpickr_0.defaultConfig = Object.assign({}, src_flatpickr_0.defaultConfig, config);
};

src_flatpickr_0.parseDate = dates_createDateParser({});
src_flatpickr_0.formatDate = dates_createDateFormatter({});
src_flatpickr_0.compareDates = dates_compareDates;
/* istanbul ignore next */
...
/* harmony default export */ var src = __webpack_exports__["default"] = (src_flatpickr_0);
