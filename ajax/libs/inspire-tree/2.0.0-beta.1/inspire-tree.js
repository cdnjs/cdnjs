        ...
        value: function insertAt(index, object) {
            ...
            if (object.id) {
                var _ret = function () {
                    ...
                    var existingNode = _this4.node(object.id);
                    if (existingNode) {
                        ...
                        if (_.isArrayLike(object.children)) {
                            ...
                            if (!_.isArrayLike(existingNode.children)) {
                                existingNode.children = new TreeNodes(_this4._tree);
...
var TreeNode = function () {
    ...
    createClass(TreeNode, [{
            ...
            return new es6Promise_1(function (resolve, reject) {
                ...
                var complete = function complete(nodes, totalNodes) {
                    if (_.parseInt(totalNodes) > nodes.length) {
                        _this3.itree.pagination.total = _.parseInt(totalNodes);
                    }
                ...
                var loader = _this3._tree.config.data(_this3, complete, error, _this3.itree.pagination);
        ...
        value: function refreshIndeterminateState() {
            ...
            if (this.hasChildren()) {
                (function () {
                    var childrenCount = _this4.children.length;
                    var indeterminate = 0;
                    var checked = 0;

                    _this4.children.each(function (n) {
                        if (n.checked()) {
                            checked++;
                        }
                    ...
                    if (checked === childrenCount) {
                        baseStateChange('checked', true, 'checked', _this4);
                    } else {
                        baseStateChange('checked', false, 'unchecked', _this4);
                    }

                    // Set indeterminate if any children are, or some children are selected
                    if (!_this4.checked()) {
                        _this4.state('indeterminate', indeterminate > 0 || childrenCount > 0 && checked > 0 && checked < childrenCount);
                    }
                })();
            }
    ...
    createClass(InspireTree, [{
        ...
        value: function load(loader) {
            ...
            var promise = new es6Promise_1(function (resolve, reject) {
                var complete = function complete(nodes, totalNodes) {
                    if (_.get(_this3, 'dom.pagination')) {
                        // @fix
                        _this3.dom.pagination.total = nodes.length;

                        if (_.parseInt(totalNodes) > nodes.length) {
                            _this3.dom.pagination.total = _.parseInt(totalNodes);
                        }
                    }
                    ...
                    // @fix
                    // if (_.isFunction(this.dom.scrollSelectedIntoView)) {
                    //     this.dom.scrollSelectedIntoView();
                    // }
                };
                ...
                // Data loader requires a caller/callback
                        ...
                        var resp = loader(null, complete, reject, _.get(_this3, 'dom.pagination'));
