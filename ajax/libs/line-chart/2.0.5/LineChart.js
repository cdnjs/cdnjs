...
(function (n3Charts) {
    ...
    (function (Factory) {
        ...
        var Zoom = (function (_super) {
            ...
            Zoom.prototype.updateFromOuterWorld = function (translate) {
    ...
    (function (Factory) {
        ...
        var SyncLayer = (function (_super) {
            ...
            SyncLayer.prototype.sanitizeAttributes = function () {
                var _a = this.attributes, tooltipSyncKey = _a.tooltipSyncKey, dragSyncKey = _a.dragSyncKey;
            ...
            SyncLayer.prototype.syncDrag = function () {
                var _this = this;
                var eventMgr = this.eventMgr;
                var callbacks = [];
                if (!!this.attributes.onDrag) {
                    var fn = this.$parse(this.attributes.onDrag);
                    var xAxis = this.factoryMgr.get('x-axis');
                    var yAxis = this.factoryMgr.get('y-axis');
                    callbacks.push(function (translate) {
                        fn(_this.scope.$parent, {
                            $domains: {
                ...
                if (!!this.attributes.dragSyncKey) {
                    callbacks.push(function (translate) {
                        _this.scope.$emit(_this.attributes.dragSyncKey, translate);
                    });
                }
                if (callbacks.length > 0) {
                    this.unregisteringFunctions.push(this.scope.$root.$on(this.attributes.dragSyncKey, function (event, translate) {
                        eventMgr.triggerDataAndOptions('outer-world-zoom', translate);
                    }));
                    eventMgr.on('zoom.directive', function (event, bubble) {
                        if (bubble) {
                            callbacks.forEach(function (fn) { return fn(event.translate); });
                        }
                    });
                }
            };
