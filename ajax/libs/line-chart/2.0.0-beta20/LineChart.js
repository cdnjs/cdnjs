...
(function (n3Charts) {
    ...
    var LineChart = (function () {
        function LineChart($window, $parse, $timeout) {
            var _this = this;
            this.$window = $window;
            this.$parse = $parse;
            ...
            this.link = function (scope, element, attributes) {
                ...
                scope.$watch('hoveredCoordinates', function (value) {
                    if (value === undefined) {
                        return;
                    }
                    factoryMgr.get('tooltip').showFromCoordinates(value, new n3Charts.Utils.Data(angular.copy(scope.data)), new n3Charts.Utils.Options(angular.copy(scope.options)));
                });
                var setHoveredCoordinates = function (value) {
                    // Jesus. Fruitcake. Christ.
                    var attrs = attributes;
                    var getter = _this.$parse(attrs.hoveredCoordinates);
...
(function (n3Charts) {
    ...
    angular.module('n3-line-chart', [])
        ...
        function ($window, $parse, $timeout) { return new n3Charts.LineChart($window, $parse, $timeout); }
    ]);
})(n3Charts || (n3Charts = {}));
