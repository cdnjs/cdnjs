...
(function (n3Charts) {
    ...
    (function (Factory) {
        ...
        var Axis = (function (_super) {
            ...
            Axis.prototype.getExtentForDatasets = function (data, filter, accessor, includeZero) {
        ...
        var Pan = (function (_super) {
            ...
            Pan.prototype.move = function (deltaX, deltaY) {
                if (deltaX !== 0) {
                    ...
                    x1.setDomain(x1.range().map(function (x) { return x + deltaX; }).map(x1.invert, x1));
                    x2.setDomain(x1.getDomain());
                }
                if (deltaY !== 0) {
                    var y1 = this.factoryMgr.get('y-axis');
                    var y2 = this.factoryMgr.get('y2-axis');
                    y1.setDomain(y1.range().map(function (x) { return x + deltaY; }).map(y1.invert, y1));
                    y2.setDomain(y2.range().map(function (x) { return x + deltaY; }).map(y2.invert, y2));
                }
            };
