...
50;if(v&&G||1500<=Q){if(p.debug){var U="BoomerangJS loaded:["+v+"], countly_boomerang loaded:["+G+"].";v&&G?console.log("[DEBUG] "+U):console.warn("[WARNING] "+U+" Initializing without APM.")}p.init(l);clearInterval(O)}},50)}var M={NPS:"[CLY]_nps",SURVEY:"[CLY]_survey",STAR_RATING:"[CLY]_star_rating",VIEW:"[CLY]_view",ORIENTATION:"[CLY]_orientation",ACTION:"[CLY]_action"},Vb=Object.values(M),c={ERROR:"[ERROR] ",WARNING:"[WARNING] ",INFO:"[INFO] ",DEBUG:"[DEBUG] ",VERBOSE:"[VERBOSE] "},Hb={BOOMERANG_SRC:"https://cdn.jsdelivr.net/npm/countly-sdk-web@latest/plugin/boomerang/boomerang.min.js",
CLY_BOOMERANG_SRC:"https://cdn.jsdelivr.net/npm/countly-sdk-web@latest/plugin/boomerang/countly_boomerang.js"},S=Object.freeze({errorCount:"cly_hc_error_count",warningCount:"cly_hc_warning_count",statusCode:"cly_hc_status_code",errorMessage:"cly_hc_error_message"}),Ib=/^(((([^:\/#\?]+:)?(?:(\/\/)((?:(([^:@\/#\?]+)(?::([^:@\/#\?]+))?)@)?(([^:\/#\?\]\[]+|\[[^\/\]@#?]+\])(?::([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/,x="undefined"!==typeof window,p=globalThis.Countly||{},
...
n(r,R))}},!0)):(b(c.ERROR,"fetch_remote_config_explicit, Remote config requires explicit consent"),n&&n(Error("Remote config requires explicit consent"),R))}function Q(){f.ignore_prefetch&&x&&"undefined"!==typeof document.visibilityState&&"prerender"===document.visibilityState&&(f.ignore_visitor=!0);f.ignore_bots&&Bb()&&(f.ignore_visitor=!0)}function O(){0<J.length&&(b(c.DEBUG,"Flushing events"),N({events:JSON.stringify(J)}),J=[],w("cly_event",J))}function U(a,d){if(x)if(document.getElementById("countly-feedback-sticker-"+
a._id))b(c.ERROR,"Widget with same ID exists");else try{var e=document.createElement("div");e.className="countly-iframe-wrapper";e.id="countly-iframe-wrapper-"+a._id;var h=document.createElement("span");h.className="countly-feedback-close-icon";h.id="countly-feedback-close-icon-"+a._id;h.innerText="x";var m=document.createElement("iframe");m.name="countly-feedback-iframe";m.id="countly-feedback-iframe";m.src=f.url+"/feedback?widget_id="+a._id+"&app_key="+f.app_key+"&device_id="+f.device_id+"&sdk_version="+
oa;document.body.appendChild(e);e.appendChild(h);e.appendChild(m);A(document.getElementById("countly-feedback-close-icon-"+a._id),"click",function(){document.getElementById("countly-iframe-wrapper-"+a._id).style.display="none";document.getElementById("cfbg").style.display="none"});if(d){var g=document.createElementNS("http://www.w3.org/2000/svg","svg");g.id="feedback-sticker-svg";g.setAttribute("aria-hidden","true");g.setAttribute("data-prefix","far");g.setAttribute("data-icon","grin");g.setAttribute("class",
...
C)}else b(c.WARNING,"processWidget, window object is not available. Not processing widget.")}function Aa(){var a;if("undefined"!==typeof f.onload&&0<f.onload.length){for(a=0;a<f.onload.length;a++)if("function"===typeof f.onload[a])f.onload[a](f);f.onload=[]}}function ma(){if(W){var a={name:W};f.check_consent("views")&&(v({key:M.VIEW,dur:pa?F()-Ga:Ha,segmentation:a},ha),W=null)}}function Ba(){if(ia){var a=y("cly_session");if(!a||parseInt(a)<=F())V=!1,f.begin_session(!Ia);w("cly_session",F()+60*Ja)}}
...
0<e.length){var h=f,m=0;p.i[e[m]]&&(h=p.i[e[m]],m++);if("function"===typeof h[e[m]])h[e[m]].apply(h,e.slice(m+1));else if(0===e[m].indexOf("userData.")){var g=e[m].replace("userData.","");"function"===typeof h.userData[g]&&h.userData[g].apply(h,e.slice(m+1))}else"function"===typeof p[e[m]]&&p[e[m]].apply(p,e.slice(m+1))}}}V&&Ia&&pa&&(a=F(),a-ja>Za&&(f.session_duration(a-ja),ja=a,0<f.hcErrorCount&&w(S.errorCount,f.hcErrorCount),0<f.hcWarningCount&&w(S.warningCount,f.hcWarningCount)));0<J.length&&!f.test_mode_eq&&
...
t("enable_orientation_tracking",k,!0):void 0;this.maxKeyLength=t("max_key_length",k,128);this.maxValueSize=t("max_value_size",k,256);this.maxSegmentationValues=t("max_segmentation_values",k,100);this.maxBreadcrumbCount=t("max_breadcrumb_count",k,null);this.maxStackTraceLinesPerThread=t("max_stack_trace_lines_per_thread",k,30);this.maxStackTraceLineLength=t("max_stack_trace_line_length",k,200);this.heatmapWhitelist=t("heatmap_whitelist",k,[]);f.hcErrorCount=y(S.errorCount)||0;f.hcWarningCount=y(S.warningCount)||
0;f.hcStatusCode=y(S.statusCode)||-1;f.hcErrorMessage=y(S.errorMessage)||"";wa&&!this.maxBreadcrumbCount?(this.maxBreadcrumbCount=wa,b(c.WARNING,"initialize, 'maxCrashLogs' is deprecated. Use 'maxBreadcrumbCount' instead!")):wa||this.maxBreadcrumbCount||(this.maxBreadcrumbCount=100);"cookie"===this.storage&&(ra=!1);this.rcAutoOptinAb||this.useExplicitRcApi||(b(c.WARNING,"initialize, Auto opting is disabled, switching to explicit RC API"),this.useExplicitRcApi=!0);Array.isArray(ca)||(ca=[]);""===this.url&&
...
a=this.heatmapWhitelist.map(function(r){return ta(r)})):a=[this.url];a.includes(this.passed_data.url)&&"heatmap"===this.passed_data.purpose&&(this.ignore_visitor=!0,Fb(),Eb(this.passed_data.url+"/views/heatmap.js",Gb))}if(this.ignore_visitor)b(c.WARNING,"initialize, ignore_visitor:["+this.ignore_visitor+"], this user will not be tracked");else{"javascript_native_web"===va&&"23.12.1"===oa?b(c.DEBUG,"initialize, SDK name:["+va+"], version:["+oa+"]"):b(c.DEBUG,"initialize, SDK name:["+va+"], version:["+
oa+"], default name:[javascript_native_web] and default version:[23.12.1]");b(c.DEBUG,"initialize, app_key:["+this.app_key+"], url:["+this.url+"]");b(c.DEBUG,"initialize, device_id:["+t("device_id",k,void 0)+"]");b(c.DEBUG,"initialize, require_consent is enabled:["+this.require_consent+"]");try{b(c.DEBUG,"initialize, metric override:["+JSON.stringify(this.metrics)+"]"),b(c.DEBUG,"initialize, header override:["+JSON.stringify(this.headers)+"]"),b(c.DEBUG,"initialize, number of onload callbacks provided:["+
...
"]");"boolean"===typeof this.rcAutoOptinAb&&b(c.DEBUG,"initialize, automatic RC optin is enabled:["+this.rcAutoOptinAb+"]");this.useExplicitRcApi||b(c.WARNING,"initialize, will use legacy RC API. Consider enabling new API during init with use_explicit_rc_api flag");this.track_domains&&b(c.DEBUG,"initialize, tracking domain info:["+this.track_domains+"]");this.enableOrientationTracking&&b(c.DEBUG,"initialize, enableOrientationTracking:["+this.enableOrientationTracking+"]");ia||b(c.WARNING,"initialize, use_session_cookie is enabled:["+
ia+"]");K&&b(c.DEBUG,"initialize, offline_mode:["+K+"], user info won't be send to the servers");K&&b(c.DEBUG,"initialize, stored remote configs:["+JSON.stringify(R)+"]");b(c.DEBUG,"initialize, 'getViewName' callback override provided:["+(this.getViewName!==p.getViewName)+"]");b(c.DEBUG,"initialize, 'getSearchQuery' callback override provided:["+(this.getSearchQuery!==p.getSearchQuery)+"]");128!==this.maxKeyLength&&b(c.DEBUG,"initialize, maxKeyLength set to:["+this.maxKeyLength+"] characters");256!==
this.maxValueSize&&b(c.DEBUG,"initialize, maxValueSize set to:["+this.maxValueSize+"] characters");100!==this.maxSegmentationValues&&b(c.DEBUG,"initialize, maxSegmentationValues set to:["+this.maxSegmentationValues+"] key/value pairs");100!==this.maxBreadcrumbCount&&b(c.DEBUG,"initialize, maxBreadcrumbCount for custom logs set to:["+this.maxBreadcrumbCount+"] entries");30!==this.maxStackTraceLinesPerThread&&b(c.DEBUG,"initialize, maxStackTraceLinesPerThread set to:["+this.maxStackTraceLinesPerThread+
"] lines");200!==this.maxStackTraceLineLength&&b(c.DEBUG,"initialize, maxStackTraceLineLength set to:["+this.maxStackTraceLineLength+"] characters");500!==bb&&b(c.DEBUG,"initialize, interval for heartbeats set to:["+bb+"] milliseconds");1E3!==Va&&b(c.DEBUG,"initialize, queue_size set to:["+Va+"] items max");60!==ab&&b(c.DEBUG,"initialize, fail_timeout set to:["+ab+"] seconds of wait time after a failed connection to server");20!==Na&&b(c.DEBUG,"initialize, inactivity_time set to:["+Na+"] minutes to consider a user as inactive after no observable action");
60!==Za&&b(c.DEBUG,"initialize, session_update set to:["+Za+"] seconds to check if extending a session is needed while the user is active");100!==Ka&&b(c.DEBUG,"initialize, max_events set to:["+Ka+"] events to send in one batch");wa&&b(c.WARNING,"initialize, max_logs set to:["+wa+"] breadcrumbs to store for crash logs max, deprecated ");30!==Ja&&b(c.DEBUG,"initialize, session_cookie_timeout set to:["+Ja+"] minutes to expire a cookies session");b(c.INFO,"initialize, Countly initialized");a=null;g=
f.getSearchQuery();var d=!1,e={};if(g){g=g.substring(1).split("&");for(var h=0;h<g.length;h++){var m=g[h].split("=");"cly_id"===m[0]?w("cly_cmp_id",m[1]):"cly_uid"===m[0]?w("cly_cmp_uid",m[1]):"cly_device_id"===m[0]?a=m[1]:0===(m[0]+"").indexOf("utm_")&&this.utm[m[0].replace("utm_","")]&&(e[m[0].replace("utm_","")]=m[1],d=!0)}}var g="[CLY]_temp_id"===y("cly_id");h=t("device_id",k,void 0);"number"===typeof h&&(h=h.toString());y("cly_id")&&!g?(this.device_id=y("cly_id"),b(c.INFO,"initialize, Set the stored device ID"),
D=y("cly_id_type"),D||(b(c.INFO,"initialize, No device ID type info from the previous session, falling back to DEVELOPER_SUPPLIED"),D=0)):null!==a?(b(c.INFO,"initialize, Device ID set by URL"),this.device_id=a,D=3):h?(b(c.INFO,"initialize, Device ID set by developer"),this.device_id=h,k&&Object.keys(k).length?void 0!==k.device_id&&(D=0):void 0!==p.device_id&&(D=0)):K||g?(this.device_id="[CLY]_temp_id",D=2,K&&g?b(c.INFO,"initialize, Temp ID set, continuing offline mode from previous app session"):
K&&!g?b(c.INFO,"initialize, Temp ID set, entering offline mode"):(K=!0,b(c.INFO,"initialize, Temp ID set, enabling offline mode"))):(b(c.INFO,"initialize, Generating the device ID"),this.device_id=t("device_id",k,nb()),k&&Object.keys(k).length?void 0!==k.device_id&&(D=0):void 0!==p.device_id&&(D=0));w("cly_id",this.device_id);w("cly_id_type",D);if(d){ka={};for(var n in this.utm)e[n]?(this.userData.set("utm_"+n,e[n]),ka[n]=e[n]):this.userData.unset("utm_"+n);this.userData.save()}Aa();setTimeout(function(){Wa();
f.remote_config&&f.fetch_remote_config(f.remote_config)},1);x&&document.documentElement.setAttribute("data-countly-useragent",ua());qa.sendInstantHCRequest()}};this.halt=function(){b(c.WARNING,"halt, Resetting Countly");p.i=void 0;Ya=!p.i;V=!1;mb="/i";Ua="/o/sdk";bb=500;Va=1E3;H=[];J=[];R={};sa=[];da={};ca=[];sb=null;Ia=!0;tb=0;W=null;lb=Ha=Ga=0;ab=60;Na=20;Oa=0;Za=60;Ka=100;wa=null;ia=!0;Ja=30;$a=!0;K=Xa=!1;ea={};pa=!0;Pb=F();ra=!0;xa=null;D=1;Ma=!1;La=0;ub=!1;ka=Da=ha=null;try{localStorage.setItem("cly_testLocal",
!0),localStorage.removeItem("cly_testLocal")}catch(d){b(c.ERROR,"halt, Local storage test failed, will fallback to cookies"),ra=!1}p.features="sessions events views scrolls clicks forms crashes attribution users star-rating location apm feedback remote-config".split(" ");E={};for(var a=0;a<p.features.length;a++)E[p.features[a]]={};f.app_key=void 0;f.device_id=void 0;f.onload=void 0;f.utm=void 0;f.ignore_prefetch=void 0;f.debug=void 0;f.test_mode=void 0;f.test_mode_eq=void 0;f.metrics=void 0;f.headers=
void 0;f.url=void 0;f.app_version=void 0;f.country_code=void 0;f.city=void 0;f.ip_address=void 0;f.ignore_bots=void 0;f.force_post=void 0;f.rcAutoOptinAb=void 0;f.useExplicitRcApi=void 0;f.remote_config=void 0;f.ignore_visitor=void 0;f.require_consent=void 0;f.track_domains=void 0;f.storage=void 0;f.enableOrientationTracking=void 0;f.maxKeyLength=void 0;f.maxValueSize=void 0;f.maxSegmentationValues=void 0;f.maxBreadcrumbCount=void 0;f.maxStackTraceLinesPerThread=void 0;f.maxStackTraceLineLength=void 0};
this.group_features=function(a){b(c.INFO,"group_features, Grouping features");if(a)for(var d in a)E[d]?b(c.WARNING,"group_features, Feature name ["+d+"] is already reserved"):"string"===typeof a[d]?E[d]={features:[a[d]]}:a[d]&&Array.isArray(a[d])&&a[d].length?E[d]={features:a[d]}:b(c.ERROR,"group_features, Incorrect feature list for ["+d+"] value: ["+a[d]+"]");else b(c.ERROR,"group_features, Incorrect features:["+a+"]")};this.check_consent=function(a){b(c.INFO,"check_consent, Checking if consent is given for specific feature:["+
a+"]");if(!this.require_consent)return b(c.INFO,"check_consent, require_consent is off, no consent is necessary"),!0;if(E[a])return!(!E[a]||!E[a].optin);b(c.ERROR,"check_consent, No feature available for ["+a+"]");return!1};this.get_device_id_type=function(){b(c.INFO,"check_device_id_type, Retrieving the current device id type.["+D+"]");switch(D){case 1:var a=f.DeviceIdType.SDK_GENERATED;break;case 3:case 0:a=f.DeviceIdType.DEVELOPER_SUPPLIED;break;case 2:a=f.DeviceIdType.TEMPORARY_ID;break;default:a=
...
a+"]");if(Array.isArray(a))for(var d=0;d<a.length;d++)this.add_consent(a[d]);else E[a]?E[a].features?(E[a].optin=!0,this.add_consent(E[a].features)):!0!==E[a].optin&&(E[a].optin=!0,Qb(),setTimeout(function(){"sessions"===a&&ea.begin_session?(f.begin_session.apply(f,ea.begin_session),ea.begin_session=null):"views"===a&&ea.track_pageview&&(W=null,f.track_pageview.apply(f,ea.track_pageview),ea.track_pageview=null)},1)):b(c.ERROR,"add_consent, No feature available for ["+a+"]")};this.remove_consent=function(a){b(c.INFO,
"remove_consent, Removing consent for ["+a+"]");this.remove_consent_internal(a,!0)};this.remove_consent_internal=function(a,d){d=d||!1;if(Array.isArray(a))for(var e=0;e<a.length;e++)this.remove_consent_internal(a[e],d);else E[a]?E[a].features?this.remove_consent_internal(E[a].features,d):(E[a].optin=!1,d&&!1!==E[a].optin&&Qb()):b(c.WARNING,"remove_consent, No feature available for ["+a+"]")};var cb,Qb=function(){cb&&(clearTimeout(cb),cb=null);cb=setTimeout(function(){for(var a={},d=0;d<p.features.length;d++)a[p.features[d]]=
!0===E[p.features[d]].optin?!0:!1;N({consent:JSON.stringify(a)});b(c.DEBUG,"Consent update request has been sent to the queue.")},1E3)};this.enable_offline_mode=function(){b(c.INFO,"enable_offline_mode, Enabling offline mode");this.remove_consent_internal(p.features,!1);K=!0;this.device_id="[CLY]_temp_id";f.device_id=this.device_id;D=2};this.disable_offline_mode=function(a){if(K){b(c.INFO,"disable_offline_mode, Disabling offline mode");K=!1;a&&this.device_id!==a?(this.device_id=a,f.device_id=this.device_id,
D=0,w("cly_id",this.device_id),w("cly_id_type",0),b(c.INFO,"disable_offline_mode, Changing id to: "+this.device_id)):(this.device_id=nb(),"[CLY]_temp_id"===this.device_id&&(this.device_id=gb()),f.device_id=this.device_id,this.device_id!==y("cly_id")&&(w("cly_id",this.device_id),w("cly_id_type",1)));a=!1;if(0<H.length)for(var d=0;d<H.length;d++)"[CLY]_temp_id"===H[d].device_id&&(H[d].device_id=this.device_id,a=!0);a&&w("cly_queue",H,!0)}else b(c.WARNING,"disable_offline_mode, Countly was not in offline mode.")};
this.begin_session=function(a,d){b(c.INFO,"begin_session, Starting the session. There was an ongoing session: ["+V+"]");a&&b(c.INFO,"begin_session, Heartbeats are disabled");d&&b(c.INFO,"begin_session, Session starts irrespective of session cookie");if(this.check_consent("sessions")){if(!V){this.enableOrientationTracking&&(this.report_orientation(),A(window,"resize",function(){f.report_orientation()}));ja=F();V=!0;Ia=!a;var e=y("cly_session");b(c.VERBOSE,"begin_session, Session state, forced: ["+
d+"], useSessionCookie: ["+ia+"], seconds to expire: ["+(e-ja)+"], expired: ["+(parseInt(e)<=F())+"] ");if(d||!ia||!e||parseInt(e)<=F())b(c.INFO,"begin_session, Session started"),null===xa&&(xa=!0),e={begin_session:1},e.metrics=JSON.stringify(Ta()),N(e);w("cly_session",F()+60*Ja)}}else ea.begin_session=arguments};this.session_duration=function(a){b(c.INFO,"session_duration, Reporting session duration");this.check_consent("sessions")&&V&&(b(c.INFO,"session_duration, Session extended: ",a),N({session_duration:a}),
Ba())};this.end_session=function(a,d){b(c.INFO,"end_session, Ending the current session. There was an on going session:["+V+"]");this.check_consent("sessions")&&V&&(a=a||F()-ja,ma(),!ia||d?(b(c.INFO,"end_session, Session ended"),N({end_session:1,session_duration:a})):this.session_duration(a),V=!1)};this.change_id=function(a,d){b(c.INFO,"change_id, Changing the ID");d&&b(c.INFO,"change_id, Will merge the IDs");if(!a||"string"!==typeof a||0===a.length)b(c.ERROR,"change_id, The provided ID: ["+a+"] is not a valid ID");
else if(K)b(c.WARNING,"change_id, Offline mode was on, initiating disabling sequence instead."),this.disable_offline_mode(a);else if(this.device_id!=a){d||(O(),this.end_session(null,!0),da={},this.remove_consent_internal(p.features,!1));var e=this.device_id;this.device_id=a;f.device_id=this.device_id;D=0;w("cly_id",this.device_id);w("cly_id_type",0);b(c.INFO,"change_id, Changing ID from:["+e+"] to ["+a+"]");d?N({old_device_id:e}):this.begin_session(!Ia,!0);this.remote_config&&(R={},w("cly_remote_configs",
R),this.fetch_remote_config(this.remote_config))}};this.add_event=function(a){b(c.INFO,"add_event, Adding event: ",a);switch(a.key){case M.NPS:var d=this.check_consent("feedback");break;case M.SURVEY:d=this.check_consent("feedback");break;case M.STAR_RATING:d=this.check_consent("star-rating");break;case M.VIEW:d=this.check_consent("views");break;case M.ORIENTATION:d=this.check_consent("users");break;case M.ACTION:d=this.check_consent("clicks")||this.check_consent("scrolls");break;default:d=this.check_consent("events")}d&&
v(a)};this.start_event=function(a){a&&"string"===typeof a?(b(c.INFO,"start_event, Starting timed event with key: ["+a+"]"),a=z(a,f.maxKeyLength,"start_event",b),da[a]?b(c.WARNING,"start_event, Timed event with key: ["+a+"] already started"):da[a]=F()):b(c.WARNING,"start_event, you have to provide a valid string key instead of: ["+a+"]")};this.cancel_event=function(a){if(!a||"string"!==typeof a)return b(c.WARNING,"cancel_event, you have to provide a valid string key instead of: ["+a+"]"),!1;b(c.INFO,
"cancel_event, Canceling timed event with key: ["+a+"]");a=z(a,f.maxKeyLength,"cancel_event",b);if(da[a])return delete da[a],b(c.INFO,"cancel_event, Timed event with key: ["+a+"] is canceled"),!0;b(c.WARNING,"cancel_event, Timed event with key: ["+a+"] was not found");return!1};this.end_event=function(a){a?(b(c.INFO,"end_event, Ending timed event"),"string"===typeof a&&(a=z(a,f.maxKeyLength,"end_event",b),a={key:a}),a.key?da[a.key]?(a.dur=F()-da[a.key],this.add_event(a),delete da[a.key]):b(c.ERROR,
...
a.byear=z(a.byear,f.maxValueSize,"user_details",b),a.custom=aa(a.custom,f.maxKeyLength,f.maxValueSize,f.maxSegmentationValues,"user_details",b),N({user_details:JSON.stringify(za(a,"name username email organization phone picture gender byear custom".split(" ")))}))};var Y={},fa=function(a,d,e){f.check_consent("users")&&(Y[a]||(Y[a]={}),"$push"===e||"$pull"===e||"$addToSet"===e?(Y[a][e]||(Y[a][e]=[]),Y[a][e].push(d)):Y[a][e]=d)};this.userData={set:function(a,d){b(c.INFO,"[userData] set, Setting user's custom property value: ["+
...
a+"] by one");a=z(a,f.maxKeyLength,"userData increment",b);fa(a,1,"$inc")},increment_by:function(a,d){b(c.INFO,"[userData] increment_by, Increasing user's custom property value under the key: ["+a+"] by: ["+d+"]");a=z(a,f.maxKeyLength,"userData increment_by",b);d=z(d,f.maxValueSize,"userData increment_by",b);fa(a,d,"$inc")},multiply:function(a,d){b(c.INFO,"[userData] multiply, Multiplying user's custom property value under the key: ["+a+"] by: ["+d+"]");a=z(a,f.maxKeyLength,"userData multiply",b);
d=z(d,f.maxValueSize,"userData multiply",b);fa(a,d,"$mul")},max:function(a,d){b(c.INFO,"[userData] max, Saving user's maximum custom property value compared to the value: ["+d+"] under the key: ["+a+"]");a=z(a,f.maxKeyLength,"userData max",b);d=z(d,f.maxValueSize,"userData max",b);fa(a,d,"$max")},min:function(a,d){b(c.INFO,"[userData] min, Saving user's minimum custom property value compared to the value: ["+d+"] under the key: ["+a+"]");a=z(a,f.maxKeyLength,"userData min",b);d=z(d,f.maxValueSize,
"userData min",b);fa(a,d,"$min")},push:function(a,d){b(c.INFO,"[userData] push, Pushing a value: ["+d+"] under the key: ["+a+"] to user's custom property array");a=z(a,f.maxKeyLength,"userData push",b);d=z(d,f.maxValueSize,"userData push",b);fa(a,d,"$push")},push_unique:function(a,d){b(c.INFO,"[userData] push_unique, Pushing a unique value: ["+d+"] under the key: ["+a+"] to user's custom property array");a=z(a,f.maxKeyLength,"userData push_unique",b);d=z(d,f.maxValueSize,"userData push_unique",b);
fa(a,d,"$addToSet")},pull:function(a,d){b(c.INFO,"[userData] pull, Removing the value: ["+d+"] under the key: ["+a+"] from user's custom property array");fa(a,d,"$pull")},save:function(){b(c.INFO,"[userData] save, Saving changes to user's custom property");f.check_consent("users")&&(O(),b(c.INFO,"user_details, flushed the event queue"),N({user_details:JSON.stringify({custom:Y})}));Y={}}};this.report_trace=function(a){b(c.INFO,"report_trace, Reporting performance trace");if(this.check_consent("apm")){for(var d=
"type name stz etz apm_metrics apm_attr".split(" "),e=0;e<d.length;e++)if("apm_attr"!==d[e]&&"undefined"===typeof a[d[e]]){b(c.WARNING,"report_trace, APM trace don't have the property: "+d[e]);return}a.name=z(a.name,f.maxKeyLength,"report_trace",b);a.app_metrics=aa(a.app_metrics,f.maxKeyLength,f.maxValueSize,f.maxSegmentationValues,"report_trace",b);d=za(a,d);d.timestamp=a.stz;a=new Date;d.hour=a.getHours();d.dow=a.getDay();N({apm:JSON.stringify(d)});b(c.INFO,"report_trace, Successfully adding APM trace: ",
d)}};this.track_errors=function(a){x?(b(c.INFO,"track_errors, Started tracking errors"),p.i[this.app_key].tracking_crashes=!0,window.cly_crashes||(window.cly_crashes=!0,sb=a,window.onerror=function r(e,h,m,g,n){if(void 0!==n&&null!==n)ib(n,!1);else{g=g||window.event&&window.event.errorCharacter;n="";"undefined"!==typeof e&&(n+=e+"\n");"undefined"!==typeof h&&(n+="at "+h);"undefined"!==typeof m&&(n+=":"+m);"undefined"!==typeof g&&(n+=":"+g);n+="\n";try{e=[];for(var u=r.caller;u;)e.push(u.name),u=u.caller;
n+=e.join("\n")}catch(I){b(c.ERROR,"track_errors, Call stack generation experienced a problem: "+I)}ib(n,!1)}},window.addEventListener("unhandledrejection",function(e){ib(Error("Unhandled rejection (reason: "+(e.reason&&e.reason.stack?e.reason.stack:e.reason)+")."),!0)}))):b(c.WARNING,"track_errors, window object is not available. Not tracking errors.")};this.log_error=function(a,d){b(c.INFO,"log_error, Logging errors");this.recordError(a,!0,d)};this.add_log=function(a){b(c.INFO,"add_log, Adding a new log of breadcrumbs: [ "+
...
G(h,m,this.rcAutoOptinAb?1:0,null,g)):(b(c.WARNING,"fetch_remote_config, Fetching remote config, with legacy API"),G(h,m,null,"legacy",g))};this.enrollUserToAb=function(a){b(c.INFO,"enrollUserToAb, Providing AB test keys to opt in for");a&&Array.isArray(a)&&0!==a.length?(a={method:"ab",keys:JSON.stringify(a),av:f.app_version},Ea(a),ba("enrollUserToAb",this.url+Ua,a,function(d,e,h){if(!d)try{var m=JSON.parse(h);b(c.DEBUG,"enrollUserToAb, Parsed the response's result: ["+m.result+"]")}catch(g){b(c.ERROR,
"enrollUserToAb, Had an issue while parsing the response: "+g)}},!0)):b(c.ERROR,"enrollUserToAb, No keys provided")};this.get_remote_config=function(a){b(c.INFO,"get_remote_config, Getting remote config from storage");return"undefined"!==typeof a?R[a]:R};this.stop_time=function(){b(c.INFO,"stop_time, Stopping tracking duration");pa&&(pa=!1,tb=F()-ja,Ha=F()-Ga)};this.start_time=function(){b(c.INFO,"start_time, Starting tracking duration");pa||(pa=!0,ja=F()-tb,Ga=F()-Ha,Ha=0,Ba())};this.track_sessions=
function(){function a(){document[e]||!document.hasFocus()?f.stop_time():f.start_time()}function d(){Oa>=Na&&f.start_time();Oa=0}if(x){b(c.INFO,"track_session, Starting tracking user session");this.begin_session();this.start_time();A(window,"beforeunload",function(){O();f.end_session()});var e="hidden";A(window,"focus",a);A(window,"blur",a);A(window,"pageshow",a);A(window,"pagehide",a);"onfocusin"in document&&(A(window,"focusin",a),A(window,"focusout",a));e in document?document.addEventListener("visibilitychange",
...
this.track_pageview=function(a,d,e){if(x||a)if(b(c.INFO,"track_pageview, Tracking page views"),b(c.VERBOSE,"track_pageview, last view is:["+W+"], current view ID is:["+ha+"], previous view ID is:["+Da+"]"),W&&ub&&(b(c.DEBUG,"track_pageview, Scroll registry triggered"),rb(),Ma=!0,La=0),ma(),Da=ha,ha=fb(),(a=z(a,f.maxKeyLength,"track_pageview",b))&&Array.isArray(a)&&(d=a,a=null),a||(a=this.getViewName()),void 0===a||""===a)b(c.ERROR,"track_pageview, No page name to track (it is either undefined or empty string). No page view can be tracked.");
else if(null===a)b(c.ERROR,"track_pageview, View name returned as null. Page view will be ignored.");else{if(d&&d.length)for(var h=0;h<d.length;h++)try{if((new RegExp(d[h])).test(a)){b(c.INFO,"track_pageview, Ignoring the page: "+a);return}}catch(r){b(c.ERROR,"track_pageview, Problem with finding ignore list item: "+d[h]+", error: "+r)}W=a;Ga=F();b(c.VERBOSE,"track_pageview, last view is assigned:["+W+"], current view ID is:["+ha+"], previous view ID is:["+Da+"]");h={name:a,visit:1,view:f.getViewUrl()};
h=aa(h,f.maxKeyLength,f.maxValueSize,f.maxSegmentationValues,"track_pageview",b);this.track_domains&&(h.domain=window.location.hostname);if(ia)if(V)xa&&(xa=!1,h.start=1);else{var m=y("cly_session");if(!m||parseInt(m)<=F())xa=!1,h.start=1}else x&&"undefined"!==typeof document.referrer&&document.referrer.length&&(m=Ib.exec(document.referrer))&&m[11]&&m[11]!==window.location.hostname&&(h.start=1);if(ka&&Object.keys(ka).length){b(c.INFO,"track_pageview, Adding fresh utm tags to segmentation:["+JSON.stringify(ka)+
"]");for(var g in ka)"undefined"===typeof h["utm_"+g]&&(h["utm_"+g]=ka[g])}x&&ob()&&(b(c.INFO,"track_pageview, Adding referrer to segmentation:["+document.referrer+"]"),h.referrer=document.referrer);if(e){e=aa(e,f.maxKeyLength,f.maxValueSize,f.maxSegmentationValues,"track_pageview",b);for(var n in e)"undefined"===typeof h[n]&&(h[n]=e[n])}this.check_consent("views")?v({key:M.VIEW,segmentation:h},ha):ea.track_pageview=arguments}else b(c.WARNING,"track_pageview, window object is not available. Not tracking page views is page is not provided.")};
this.track_view=function(a,d,e){b(c.INFO,"track_view, Initiating tracking page views");this.track_pageview(a,d,e)};this.track_clicks=function(a){if(x){b(c.INFO,"track_clicks, Starting to track clicks");a&&b(c.INFO,"track_clicks, Tracking the specified children:["+a+"]");a=a||document;var d=!0;A(a,"click",function(e){if(d){d=!1;jb(e);if("undefined"!==typeof e.pageX&&"undefined"!==typeof e.pageY){var h=Ra(),m=kb();f.check_consent("clicks")&&(e={type:"click",x:e.pageX,y:e.pageY,width:m,height:h,view:f.getViewUrl()},
e=aa(e,f.maxKeyLength,f.maxValueSize,f.maxSegmentationValues,"processClick",b),f.track_domains&&(e.domain=window.location.hostname),v({key:M.ACTION,segmentation:e}))}setTimeout(function(){d=!0},1E3)}})}else b(c.WARNING,"track_clicks, window object is not available. Not tracking clicks.")};this.track_scrolls=function(a){x?(b(c.INFO,"track_scrolls, Starting to track scrolls"),a&&b(c.INFO,"track_scrolls, Tracking the specified children"),a=a||window,ub=Ma=!0,A(a,"scroll",Mb),A(a,"beforeunload",rb)):
b(c.WARNING,"track_scrolls, window object is not available. Not tracking scrolls.")};this.track_links=function(a){x?(b(c.INFO,"track_links, Starting to track clicks to links"),a&&b(c.INFO,"track_links, Tracking the specified children"),a=a||document,A(a,"click",function(d){a:{var e=Qa(d);var h;for(h="A";e;){if(e.nodeName.toUpperCase()===h)break a;e=e.parentElement}e=void 0}e&&(jb(d),f.check_consent("clicks")&&v({key:"linkClick",segmentation:{href:e.href,text:e.innerText,id:e.id,view:f.getViewUrl()}}))})):
...
(b(c.WARNING,"recordRatingWidgetWithID, You have entered a rating higher than 5. Changing it back to 5 now."),d.segmentation.rating=5):1>d.segmentation.rating&&(b(c.WARNING,"recordRatingWidgetWithID, You have entered a rating lower than 1. Changing it back to 1 now."),d.segmentation.rating=1);b(c.INFO,"recordRatingWidgetWithID, Reporting Rating Widget: ",d);v(d)}else b(c.ERROR,"recordRatingWidgetWithID, Rating Widget must contain rating property");else b(c.ERROR,"recordRatingWidgetWithID, Rating Widget must contain widget_id property")};
...
for(var d=document.getElementsByClassName("countly-feedback-sticker");0<d.length;)d[0].remove();ba("initializeRatingWidgets,",this.url+"/o/feedback/multiple-widgets-by-id",{widgets:JSON.stringify(a),av:f.app_version},function(e,h,m){if(!e)try{var g=JSON.parse(m);for(e=0;e<g.length;e++)if("true"===g[e].is_active){var n=g[e].target_devices,r=Ab();if(n[r])if("string"===typeof g[e].hide_sticker&&(g[e].hide_sticker="true"===g[e].hide_sticker),"all"!==g[e].target_page||g[e].hide_sticker){var u=g[e].target_pages;
for(h=0;h<u.length;h++){var I=u[h].substr(0,u[h].length-1)===window.location.pathname.substr(0,u[h].length-1),C=u[h]===window.location.pathname;(u[h].includes("*")&&I||C)&&!g[e].hide_sticker&&U(g[e],!0)}}else U(g[e],!0)}}catch(na){b(c.ERROR,"initializeRatingWidgets, JSON parse error: "+na)}},!0)}}else b(c.WARNING,"initializeRatingWidgets, window object is not available. Not initializing rating widgets.")};this.enable_feedback=function(a){x?(b(c.WARNING,"enable_feedback, Deprecated function call! Use 'enableRatingWidgets' in place of this call. Call will be redirected now!"),
...
K?b(c.ERROR,"get_available_feedback_widgets, Cannot enable feedback widgets in offline mode."):ba("get_available_feedback_widgets,",this.url+Ua,{method:"feedback",device_id:this.device_id,app_key:this.app_key,av:f.app_version},function(d,e,h){if(d)a&&a(null,d);else try{var m=JSON.parse(h).result||[];a&&a(m,null)}catch(g){b(c.ERROR,"get_available_feedback_widgets, Error while parsing feedback widgets list: "+g),a&&a(null,g)}},!1):a&&a(null,Error("Consent for feedback not provided."))};this.getFeedbackWidgetData=
...
a.type)e+="/o/surveys/rating/widget";else{b(c.ERROR,"getFeedbackWidgetData, Unknown type info: ["+a.type+"]");return}ba("getFeedbackWidgetData,",e,h,function(m,g,n){if(m)d&&d(null,m);else try{var r=JSON.parse(n);d&&d(r,null)}catch(u){b(c.ERROR,"getFeedbackWidgetData, Error while parsing feedback widgets list: "+u),d&&d(null,u)}},!0)}else d&&d(null,Error("Consent for feedback not provided."));else b(c.ERROR,"getFeedbackWidgetData, Expected the provided widget object to have a type but got: ["+JSON.stringify(a)+
"], aborting.")};this.present_feedback_widget=function(a,d,e,h){function m(B){document.getElementById("countly-surveys-wrapper-"+B._id).style.display="block";document.getElementById("csbg").style.display="block"}function g(B){if(!B.appearance.hideS){b(c.DEBUG,"present_feedback_widget, handling the sticker as it was not set to hidden");var T=document.createElement("div");T.innerText=B.appearance.text;T.style.color=7>B.appearance.text_color.length?"#"+B.appearance.text_color:B.appearance.text_color;
T.style.backgroundColor=7>B.appearance.bg_color.length?"#"+B.appearance.bg_color:B.appearance.bg_color;T.className="countly-feedback-sticker  "+B.appearance.position+"-"+B.appearance.size;T.id="countly-feedback-sticker-"+B._id;document.body.appendChild(T);A(document.getElementById("countly-feedback-sticker-"+B._id),"click",function(){document.getElementById("countly-ratings-wrapper-"+B._id).style.display="flex";document.getElementById("csbg").style.display="block"})}A(document.getElementById("countly-feedback-close-icon-"+
...
return}var r=window.origin||window.location.origin;if("rating"===a.type){b(c.DEBUG,"present_feedback_widget, Loading css for rating widget.");var u="ratings";Sa(this.url+"/star-rating/stylesheets/countly-feedback-web.css")}else b(c.DEBUG,"present_feedback_widget, Loading css for survey or nps."),Sa(this.url+"/surveys/stylesheets/countly-surveys.css"),u="surveys";n+="?widget_id="+a._id;n+="&app_key="+this.app_key;n+="&device_id="+this.device_id;n+="&sdk_name="+va;n+="&platform="+this.platform;n+="&app_version="+
this.app_version;n+="&sdk_version="+oa;if(h){var I={};I.sg=h;n+="&custom="+JSON.stringify(I)}n+="&origin="+r;n+="&widget_v=web";var C=document.createElement("iframe");C.src=n;C.name="countly-"+u+"-iframe";C.id="countly-"+u+"-iframe";var na=!1;C.onload=function(){na&&(document.getElementById("countly-"+u+"-wrapper-"+a._id).style.display="none",document.getElementById("csbg").style.display="none");na=!0;b(c.DEBUG,"present_feedback_widget, Loaded iframe.")};for(var Fa=document.getElementById("csbg");Fa;)Fa.remove(),
Fa=document.getElementById("csbg"),b(c.DEBUG,"present_feedback_widget, Removing past overlay.");var Z=document.getElementsByClassName("countly-"+u+"-wrapper");for(h=0;h<Z.length;h++)Z[h].remove(),b(c.DEBUG,"present_feedback_widget, Removed a wrapper.");Z=document.createElement("div");Z.className="countly-"+u+"-wrapper";Z.id="countly-"+u+"-wrapper-"+a._id;"survey"===a.type&&(Z.className=Z.className+" "+a.appearance.position);var db=document.body;h=!1;d&&(document.getElementById(d)?(db=document.getElementById(d),
h=!0):b(c.ERROR,"present_feedback_widget, Provided ID not found."));h||e&&(document.getElementsByClassName(e)[0]?db=document.getElementsByClassName(e)[0]:b(c.ERROR,"present_feedback_widget, Provided class not found."));db.insertAdjacentHTML("beforeend",'<div id="csbg"></div>');db.appendChild(Z);if("rating"===a.type){var wb=document.createElement("div");wb.className="countly-ratings-overlay";wb.id="countly-ratings-overlay-"+a._id;Z.appendChild(wb);b(c.DEBUG,"present_feedback_widget, appended the rating overlay to wrapper");
A(document.getElementById("countly-ratings-overlay-"+a._id),"click",function(){document.getElementById("countly-ratings-wrapper-"+a._id).style.display="none"})}Z.appendChild(C);b(c.DEBUG,"present_feedback_widget, Appended the iframe");A(window,"message",function(B){var T={};try{T=JSON.parse(B.data),b(c.DEBUG,"present_feedback_widget, Parsed response message "+T)}catch(Xb){b(c.ERROR,"present_feedback_widget, Error while parsing message body "+Xb)}T.close?(document.getElementById("countly-"+u+"-wrapper-"+
...
"mouseleave",function(){P||(P=!0,m(a))}):A(document,"readystatechange",function(B){"complete"===B.target.readyState&&A(document,"mouseleave",function(){P||(P=!0,m(a))})});break;case "onScrollHalfwayDown":A(window,"scroll",function(){if(!P){var B=Math.max(window.scrollY,document.body.scrollTop,document.documentElement.scrollTop),T=Ra();B>=T/2&&(P=!0,m(a))}});break;default:P||(P=!0,m(a))}}else if("nps"===a.type)document.getElementById("countly-"+u+"-wrapper-"+a._id).style.display="block",document.getElementById("csbg").style.display=
...
N(d)}};this.onStorageChange=function(a,d){b(c.DEBUG,"onStorageChange, Applying storage changes for key:",a);b(c.DEBUG,"onStorageChange, Applying storage changes for value:",d);switch(a){case "cly_queue":H=f.deserialize(d||"[]");break;case "cly_event":J=f.deserialize(d||"[]");break;case "cly_remote_configs":R=f.deserialize(d||"{}");break;case "cly_ignore":f.ignore_visitor=f.deserialize(d);break;case "cly_id":f.device_id=d;break;case "cly_id_type":D=f.deserialize(d)}};this._internals={store:w,getDocWidth:kb,
getDocHeight:Ra,getViewportHeight:Cb,get_page_coord:jb,get_event_target:Qa,add_event_listener:A,createNewObjectFromProperties:za,truncateObject:aa,truncateSingleValue:z,stripTrailingSlash:ta,prepareParams:ya,sendXmlHttpRequest:Kb,isResponseValid:qb,getInternalDeviceIdType:function(){return D},getMsTimestamp:hb,getTimestamp:F,isResponseValidBroad:pb,secureRandom:fb,log:b,checkIfLoggingIsOn:Ca,getMetrics:Ta,getUA:Jb,prepareRequest:Ea,generateUUID:gb,sendEventsForced:O,isUUID:function(a){return/[0-9a-fA-F]{8}\-[0-9a-fA-F]{4}\-4[0-9a-fA-F]{3}\-[0-9a-fA-F]{4}\-[0-9a-fA-F]{12}/.test(a)},
isReferrerUsable:ob,getId:nb,heartBeat:Wa,toRequestQueue:N,reportViewDuration:ma,loadJS:Eb,loadCSS:Sa,getLastView:function(){return W},setToken:Nb,getToken:function(){var a=y("cly_token");X("cly_token");return a},showLoader:Fb,hideLoader:Gb,setValueInStorage:w,getValueFromStorage:y,removeValueFromStorage:X,add_cly_events:v,processScrollView:rb,processScroll:Mb,currentUserAgentString:ua,userAgentDeviceDetection:Ab,userAgentSearchBotDetection:Bb,getRequestQueue:function(){return H},getEventQueue:function(){return J},
sendFetchRequest:Lb,makeNetworkRequest:ba,clearQueue:function(){H=[];w("cly_queue",[]);J=[];w("cly_event",[])},getLocalQueues:function(){return{eventQ:J,requestQ:H}}};var qa={sendInstantHCRequest:function(){var a=z(f.hcErrorMessage,1E3,"healthCheck",b);a={el:f.hcErrorCount,wl:f.hcWarningCount,sc:f.hcStatusCode,em:JSON.stringify(a)};a={hc:JSON.stringify(a),metrics:JSON.stringify({_app_version:f.app_version})};Ea(a);ba("[healthCheck]",f.url+mb,a,function(d){d||qa.resetAndSaveCounters()},!0)},resetAndSaveCounters:function(){qa.resetCounters();
w(S.errorCount,f.hcErrorCount);w(S.warningCount,f.hcWarningCount);w(S.statusCode,f.hcStatusCode);w(S.errorMessage,f.hcErrorMessage)},incrementErrorCount:function(){f.hcErrorCount++},incrementWarningCount:function(){f.hcWarningCount++},resetCounters:function(){f.hcErrorCount=0;f.hcWarningCount=0;f.hcStatusCode=-1;f.hcErrorMessage=""},saveRequestCounters:function(a,d){f.hcStatusCode=a;f.hcErrorMessage=d;w(S.statusCode,f.hcStatusCode);w(S.errorMessage,f.hcErrorMessage)}};this.initialize()}),Sb=!0;p.features=
"apm attribution clicks crashes events feedback forms location remote-config scrolls sessions star-rating users views".split(" ");p.q=p.q||[];p.onload=p.onload||[];p.CountlyClass=Rb;p.init=function(k){k=k||{};if(p.loadAPMScriptsAsync&&Sb)Sb=!1,Ub(k);else{var q=k.app_key||p.app_key;if(!p.i||!p.i[q]){k=new Rb(k);if(!p.i){p.i={};for(var v in k)p[v]=k[v]}p.i[q]=k}return p.i[q]}};p.serialize=function(k){"object"===L(k)&&(k=JSON.stringify(k));return k};p.deserialize=function(k){if(""===k)return k;try{k=
JSON.parse(k)}catch(q){Ca()&&console.warn("[WARNING] [Countly] deserialize, Could not parse the file:["+k+"], error: "+q)}return k};p.getViewName=function(){return x?window.location.pathname:"web_worker"};p.getViewUrl=function(){return x?window.location.pathname:"web_worker"};p.getSearchQuery=function(){if(x)return window.location.search};p.DeviceIdType={DEVELOPER_SUPPLIED:0,SDK_GENERATED:1,TEMPORARY_ID:2};x&&window.addEventListener("storage",function(k){var q=(k.key+"").split("/"),v=q.pop();q=q.pop();
if(p.i&&p.i[q])p.i[q].onStorageChange(v,k.newValue)});la.default=p;Object.defineProperty(la,"__esModule",{value:!0})});
