...
var H={NPS:"[CLY]_nps",SURVEY:"[CLY]_survey",STAR_RATING:"[CLY]_star_rating",VIEW:"[CLY]_view",ORIENTATION:"[CLY]_orientation",PUSH_ACTION:"[CLY]_push_action",ACTION:"[CLY]_action"},zb=Object.values(H),d={ERROR:"[ERROR] ",WARNING:"[WARNING] ",INFO:"[INFO] ",DEBUG:"[DEBUG] ",VERBOSE:"[VERBOSE] "};m.q=m.q||[];m.onload=m.onload||[];var pb=/^(((([^:\/#\?]+:)?(?:(\/\/)((?:(([^:@\/#\?]+)(?::([^:@\/#\?]+))?)@)?(([^:\/#\?\]\[]+|\[[^\/\]@#?]+\])(?::([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/;
m.CountlyClass=function(h){function p(a,c){if(g.ignore_visitor)b(d.ERROR,"Adding event failed. Possible bot or user opt out");else if(a.key){a.count||(a.count=1);zb.includes(a.key)||(a.key=w(a.key,g.maxKeyLength,"add_cly_event",b));a.segmentation=W(a.segmentation,g.maxKeyLength,g.maxValueSize,g.maxSegmentationValues,"add_cly_event",b);a=ra(a,["key","count","sum","dur","segmentation"]);a.timestamp=Va();var e=new Date;a.hour=e.getHours();a.dow=e.getDay();a.id=c||Ta();a.key===H.VIEW?a.pvid=ta||"":a.cvid=
X||"";I.push(a);v("cly_event",I);b(d.INFO," With event ID: ["+a.id+"], successfully adding the last event:",a)}else b(d.ERROR,"Adding event failed. Event must have a key property")}function u(a,c,e,k,l){b(d.INFO,"fetch_remote_config_explicit, Fetching sequence initiated");var f={method:"rc"};a&&(f.keys=JSON.stringify(a));c&&(f.omit_keys=JSON.stringify(c));var n;"legacy"===k&&(f.method="fetch_remote_config");0===e&&(f.oi=0);1===e&&(f.oi=1);"function"===typeof l&&(n=l);g.check_consent("sessions")&&
(f.metrics=JSON.stringify(Ga()));g.check_consent("remote-config")?(da(f),ea("fetch_remote_config_explicit",g.url+Ha,f,function(q,t,J){if(q)b(d.ERROR,"fetch_remote_config_explicit, An error occurred: "+q);else{try{var Q=JSON.parse(J);if(f.keys||f.omit_keys)for(var L in Q)N[L]=Q[L];else N=Q;v("cly_remote_configs",N)}catch(oa){b(d.ERROR,"fetch_remote_config_explicit, Had an issue while parsing the response: "+oa)}n&&(b(d.INFO,"fetch_remote_config_explicit, Callback function is provided"),n(q,N))}},!0)):
(b(d.ERROR,"fetch_remote_config_explicit, Remote config requires explicit consent"),n&&n(Error("Remote config requires explicit consent"),N))}function D(){g.ignore_prefetch&&"undefined"!==typeof document.visibilityState&&"prerender"===document.visibilityState&&(g.ignore_visitor=!0);g.ignore_bots&&jb()&&(g.ignore_visitor=!0)}function E(){0<I.length&&(b(d.DEBUG,"Flushing events"),G({events:JSON.stringify(I)}),I=[],v("cly_event",I))}function P(a,c){if(document.getElementById("countly-feedback-sticker-"+
a._id))b(d.ERROR,"Widget with same ID exists");else try{var e=document.createElement("div");e.className="countly-iframe-wrapper";e.id="countly-iframe-wrapper-"+a._id;var k=document.createElement("span");k.className="countly-feedback-close-icon";k.id="countly-feedback-close-icon-"+a._id;k.innerText="x";var l=document.createElement("iframe");l.name="countly-feedback-iframe";l.id="countly-feedback-iframe";l.src=g.url+"/feedback?widget_id="+a._id+"&app_key="+g.app_key+"&device_id="+g.device_id+"&sdk_version=23.02.0";
document.body.appendChild(e);e.appendChild(k);e.appendChild(l);y(document.getElementById("countly-feedback-close-icon-"+a._id),"click",function(){document.getElementById("countly-iframe-wrapper-"+a._id).style.display="none";document.getElementById("cfbg").style.display="none"});if(c){var f=document.createElementNS("http://www.w3.org/2000/svg","svg");f.id="feedback-sticker-svg";f.setAttribute("aria-hidden","true");f.setAttribute("data-prefix","far");f.setAttribute("data-icon","grin");f.setAttribute("class",
"svg-inline--fa fa-grin fa-w-16");f.setAttribute("role","img");f.setAttribute("xmlns","http://www.w3.org/2000/svg");f.setAttribute("viewBox","0 0 496 512");var n=document.createElementNS("http://www.w3.org/2000/svg","path");n.id="smileyPathInStickerSvg";n.setAttribute("fill","white");n.setAttribute("d","M248 8C111 8 0 119 0 256s111 248 248 248 248-111 248-248S385 8 248 8zm0 448c-110.3 0-200-89.7-200-200S137.7 56 248 56s200 89.7 200 200-89.7 200-200 200zm105.6-151.4c-25.9 8.3-64.4 13.1-105.6 13.1s-79.6-4.8-105.6-13.1c-9.9-3.1-19.4 5.4-17.7 15.3 7.9 47.1 71.3 80 123.3 80s115.3-32.9 123.3-80c1.6-9.8-7.7-18.4-17.7-15.3zM168 240c17.7 0 32-14.3 32-32s-14.3-32-32-32-32 14.3-32 32 14.3 32 32 32zm160 0c17.7 0 32-14.3 32-32s-14.3-32-32-32-32 14.3-32 32 14.3 32 32 32z");
var q=document.createElement("span");q.innerText=a.trigger_button_text;var t=document.createElement("div");t.style.color=7>a.trigger_font_color.length?"#"+a.trigger_font_color:a.trigger_font_color;t.style.backgroundColor=7>a.trigger_bg_color.length?"#"+a.trigger_bg_color:a.trigger_bg_color;t.className="countly-feedback-sticker  "+a.trigger_position+"-"+a.trigger_size;t.id="countly-feedback-sticker-"+a._id;f.appendChild(n);t.appendChild(f);t.appendChild(q);document.body.appendChild(t);var J=document.getElementById("smileyPathInStickerSvg");
...
Q)}}function ba(){var a;if("undefined"!==typeof g.onload&&0<g.onload.length){for(a=0;a<g.onload.length;a++)if("function"===typeof g.onload[a])g.onload[a](g);g.onload=[]}}function ca(){if(S){var a={name:S};g.check_consent("views")&&p({key:H.VIEW,dur:ia?C()-ua:va,segmentation:a},X);ta=X;S=X=null}}function sa(){if(fa){var a=x("cly_session");if(!a||parseInt(a)<=C())R=!1,g.begin_session(!wa);v("cly_session",C()+60*xa)}}function da(a){a.app_key=g.app_key;a.device_id=g.device_id;a.sdk_name="javascript_native_web";
a.sdk_version="23.02.0";a.t=A;var c=qb();if(a.metrics){var e=JSON.parse(a.metrics);e._ua||(e._ua=c,a.metrics=JSON.stringify(e))}else a.metrics=JSON.stringify({_ua:c});g.check_consent("location")?(g.country_code&&(a.country_code=g.country_code),g.city&&(a.city=g.city),null!==g.ip_address&&(a.ip_address=g.ip_address)):a.location="";a.timestamp=Va();c=new Date;a.hour=c.getHours();a.dow=c.getDay()}function G(a){g.ignore_visitor?b(d.WARNING,"User is opt_out will ignore the request: "+a):g.app_key&&g.device_id?
(da(a),F.length>Ia&&F.shift(),F.push(a),v("cly_queue",F,!0)):b(d.ERROR,"app_key or device_id is missing ",g.app_key,g.device_id)}function Ja(){ba();if(g.ignore_visitor)Ka=!1,b(d.WARNING,"User opt_out, no heartbeat");else{Ka=!0;var a=0;if(La&&"undefined"!==typeof m.q&&0<m.q.length){var c=m.q;m.q=[];for(a=0;a<c.length;a++){var e=c[a];b(d.DEBUG,"Processing queued call",e);if("function"===typeof e)e();else if(Array.isArray(e)&&0<e.length){var k=g,l=0;m.i[e[l]]&&(k=m.i[e[l]],l++);if("function"===typeof k[e[l]])k[e[l]].apply(k,
e.slice(l+1));else if(0===e[l].indexOf("userData.")){var f=e[l].replace("userData.","");"function"===typeof k.userData[f]&&k.userData[f].apply(k,e.slice(l+1))}else"function"===typeof m[e[l]]&&m[e[l]].apply(m,e.slice(l+1))}}}R&&wa&&ia&&(a=C(),a-ha>Ma&&(g.session_duration(a-ha),ha=a));0<I.length&&!g.test_mode_eq&&(I.length<=ya?(G({events:JSON.stringify(I)}),I=[]):(a=I.splice(0,ya),G({events:JSON.stringify(a)})),v("cly_event",I));!K&&0<F.length&&Na&&C()>Za&&(Na=!1,a=F[0],b(d.DEBUG,"Processing request",
...
"function"===typeof k&&k(!1,e,this.responseText):(b(d.ERROR,a+" Failed Server XML HTTP request, ",this.status),"function"===typeof k&&k(!0,e)))};"GET"===q?f.send():f.send(n)}catch(J){b(d.ERROR,a+" Failed XML HTTP request: "+J),"function"===typeof k&&k(!0,e)}}function tb(a,c){if(!(200<=a&&300>a))return b(d.ERROR,"Http response status code is not within the expected range:["+a+"]"),!1;try{var e=JSON.parse(c);return"[object Object]"!==Object.prototype.toString.call(e)?(b(d.ERROR,"Http response is not JSON Object"),
...
A=0)):null!==a?(b(d.INFO,"initialize, Device ID set by URL"),this.device_id=a,A=3):k?(b(d.INFO,"initialize, Device ID set by developer"),this.device_id=k,h&&Object.keys(h).length?void 0!==h.device_id&&(A=0):void 0!==m.device_id&&(A=0)):K||f?(this.device_id="[CLY]_temp_id",A=2,K&&f?b(d.INFO,"initialize, Temp ID set, continuing offline mode from previous app session"):K&&!f?b(d.INFO,"initialize, Temp ID set, entering offline mode"):(K=!0,b(d.INFO,"initialize, Temp ID set, enabling offline mode"))):
(b(d.INFO,"initialize, Generating the device ID"),this.device_id=r("device_id",h,$a()),h&&Object.keys(h).length?void 0!==h.device_id&&(A=0):void 0!==m.device_id&&(A=0));v("cly_id",this.device_id);v("cly_id_type",A);if(c){for(var n in this.utm)e[n]?this.userData.set("utm_"+n,e[n]):this.userData.unset("utm_"+n);this.userData.save()}ba();setTimeout(function(){Ja();g.remote_config&&g.fetch_remote_config(g.remote_config)},1);document.documentElement.setAttribute("data-countly-useragent",na())}};this.halt=
function(){b(d.WARNING,"halt, Resetting Countly");m.i=void 0;La=!m.i;R=!1;rb="/i";Ha="/o/sdk";Pa=500;Ia=1E3;F=[];I=[];N={};ka=[];Y={};T=[];bb=null;wa=!0;cb=0;S=null;Za=va=ua=0;Oa=60;Ba=20;Ca=0;Ma=60;ya=100;pa=null;fa=!0;xa=30;Na=!0;K=Ka=!1;Z={};ia=!0;xb=C();ja=!0;qa=null;A=1;Aa=!1;za=0;db=!1;ta=X=null;try{localStorage.setItem("cly_testLocal",!0),localStorage.removeItem("cly_testLocal")}catch(c){b(d.ERROR,"halt, Local storage test failed, will fallback to cookies"),ja=!1}m.features="sessions events views scrolls clicks forms crashes attribution users star-rating location apm feedback remote-config".split(" ");
...
"check_any_consent, require_consent is off, no consent is necessary"),!0;for(var a in B)if(B[a]&&B[a].optin)return!0;b(d.INFO,"check_any_consent, No consents given");return!1};this.add_consent=function(a){b(d.INFO,"add_consent, Adding consent for ["+a+"]");if(Array.isArray(a))for(var c=0;c<a.length;c++)this.add_consent(a[c]);else B[a]?B[a].features?(B[a].optin=!0,this.add_consent(B[a].features)):!0!==B[a].optin&&(B[a].optin=!0,yb(),setTimeout(function(){"sessions"===a&&Z.begin_session?(g.begin_session.apply(g,
Z.begin_session),Z.begin_session=null):"views"===a&&Z.track_pageview&&(S=null,g.track_pageview.apply(g,Z.track_pageview),Z.track_pageview=null)},1)):b(d.ERROR,"add_consent, No feature available for ["+a+"]")};this.remove_consent=function(a){b(d.INFO,"remove_consent, Removing consent for ["+a+"]");this.remove_consent_internal(a,!0)};this.remove_consent_internal=function(a,c){c=c||!1;if(Array.isArray(a))for(var e=0;e<a.length;e++)this.remove_consent_internal(a[e],c);else B[a]?B[a].features?this.remove_consent_internal(B[a].features,
c):(B[a].optin=!1,c&&!1!==B[a].optin&&yb()):b(d.ERROR,"remove_consent, No feature available for ["+a+"]")};var Qa,yb=function(){Qa&&(clearTimeout(Qa),Qa=null);Qa=setTimeout(function(){for(var a={},c=0;c<m.features.length;c++)a[m.features[c]]=!0===B[m.features[c]].optin?!0:!1;G({consent:JSON.stringify(a)});b(d.DEBUG,"Consent update request has been sent to the queue.")},1E3)};this.enable_offline_mode=function(){b(d.INFO,"enable_offline_mode, Enabling offline mode");this.remove_consent_internal(m.features,
!1);K=!0;this.device_id="[CLY]_temp_id";g.device_id=this.device_id;A=2};this.disable_offline_mode=function(a){b(d.INFO,"disable_offline_mode, Disabling offline mode");K=!1;a&&this.device_id!==a?(this.device_id=a,g.device_id=this.device_id,A=0,v("cly_id",this.device_id),v("cly_id_type",0),b(d.INFO,"disable_offline_mode, Changing id to: "+this.device_id)):(this.device_id=$a(),"[CLY]_temp_id"===this.device_id&&(this.device_id=Ua()),g.device_id=this.device_id,this.device_id!==x("cly_id")&&(v("cly_id",
this.device_id),v("cly_id_type",1)));a=!1;if(0<F.length)for(var c=0;c<F.length;c++)"[CLY]_temp_id"===F[c].device_id&&(F[c].device_id=this.device_id,a=!0);a&&v("cly_queue",F,!0)};this.begin_session=function(a,c){b(d.INFO,"begin_session, Starting the session. There was an ongoing session: ["+R+"]");a&&b(d.INFO,"begin_session, Heartbeats are disabled");c&&b(d.INFO,"begin_session, Session starts irrespective of session cookie");if(this.check_consent("sessions")){if(!R){this.enableOrientationTracking&&
(this.report_orientation(),y(window,"resize",function(){g.report_orientation()}));ha=C();R=!0;wa=!a;var e=x("cly_session");b(d.VERBOSE,"begin_session, Session state, forced: ["+c+"], useSessionCookie: ["+fa+"], seconds to expire: ["+(e-ha)+"], expired: ["+(parseInt(e)<=C())+"] ");if(c||!fa||!e||parseInt(e)<=C())b(d.INFO,"begin_session, Session started"),null===qa&&(qa=!0),e={begin_session:1},e.metrics=JSON.stringify(Ga()),G(e);v("cly_session",C()+60*xa)}}else Z.begin_session=arguments};this.session_duration=
function(a){b(d.INFO,"session_duration, Reporting session duration");this.check_consent("sessions")&&R&&(b(d.INFO,"session_duration, Session extended: ",a),G({session_duration:a}),sa())};this.end_session=function(a,c){b(d.INFO,"end_session, Ending the current session. There was an on going session:["+R+"]");this.check_consent("sessions")&&R&&(a=a||C()-ha,ca(),!fa||c?(b(d.INFO,"end_session, Session ended"),G({end_session:1,session_duration:a})):this.session_duration(a),R=!1)};this.change_id=function(a,
c){b(d.INFO,"change_id, Changing the ID");c&&b(d.INFO,"change_id, Will merge the IDs");if(!a||"string"!==typeof a||0===a.length)b(d.ERROR,"change_id, The provided ID: ["+a+"] is not a valid ID");else if(K)b(d.WARNING,"change_id, Offline mode was on, initiating disabling sequence instead."),this.disable_offline_mode(a);else if(this.device_id!==a){c||(E(),this.end_session(null,!0),Y={},this.remove_consent_internal(m.features,!1));var e=this.device_id;this.device_id=a;g.device_id=this.device_id;A=0;
v("cly_id",this.device_id);v("cly_id_type",0);b(d.INFO,"change_id, Changing ID from:["+e+"] to ["+a+"]");c?G({old_device_id:e}):this.begin_session(!wa,!0);this.remote_config&&(N={},v("cly_remote_configs",N),this.fetch_remote_config(this.remote_config))}};this.add_event=function(a){b(d.INFO,"add_event, Adding event: ",a);switch(a.key){case H.NPS:var c=this.check_consent("feedback");break;case H.SURVEY:c=this.check_consent("feedback");break;case H.STAR_RATING:c=this.check_consent("star-rating");break;
case H.VIEW:c=this.check_consent("views");break;case H.ORIENTATION:c=this.check_consent("users");break;case H.PUSH_ACTION:c=this.check_consent("push");break;case H.ACTION:c=this.check_consent("clicks")||this.check_consent("scroll");break;default:c=this.check_consent("events")}c&&p(a)};this.start_event=function(a){a&&"string"===typeof a?(b(d.INFO,"start_event, Starting timed event with key: ["+a+"]"),a=w(a,g.maxKeyLength,"start_event",b),Y[a]?b(d.WARNING,"start_event, Timed event with key: ["+a+"] already started"):
Y[a]=C()):b(d.WARNING,"start_event, you have to provide a valid string key instead of: ["+a+"]")};this.cancel_event=function(a){if(!a||"string"!==typeof a)return b(d.WARNING,"cancel_event, you have to provide a valid string key instead of: ["+a+"]"),!1;b(d.INFO,"cancel_event, Canceling timed event with key: ["+a+"]");a=w(a,g.maxKeyLength,"cancel_event",b);if(Y[a])return delete Y[a],b(d.INFO,"cancel_event, Timed event with key: ["+a+"] is canceled"),!0;b(d.WARNING,"cancel_event, Timed event with key: ["+
a+"] was not found");return!1};this.end_event=function(a){a?(b(d.INFO,"end_event, Ending timed event"),"string"===typeof a&&(a=w(a,g.maxKeyLength,"end_event",b),a={key:a}),a.key?Y[a.key]?(a.dur=C()-Y[a.key],this.add_event(a),delete Y[a.key]):b(d.ERROR,"end_event, Timed event with key: ["+a.key+"] was not started"):b(d.ERROR,"end_event, Timed event must have a key property")):b(d.WARNING,"end_event, you have to provide a valid string key or event object instead of: ["+a+"]")};this.report_orientation=
...
c+"] under the key: ["+a+"] ");a=w(a,g.maxKeyLength,"userData set_once",b);c=w(c,g.maxValueSize,"userData set_once",b);aa(a,c,"$setOnce")},increment:function(a){b(d.INFO,"increment, Increasing user's custom property value under the key: ["+a+"] by one");a=w(a,g.maxKeyLength,"userData increment",b);aa(a,1,"$inc")},increment_by:function(a,c){b(d.INFO,"increment_by, Increasing user's custom property value under the key: ["+a+"] by: ["+c+"]");a=w(a,g.maxKeyLength,"userData increment_by",b);c=w(c,g.maxValueSize,
"userData increment_by",b);aa(a,c,"$inc")},multiply:function(a,c){b(d.INFO,"multiply, Multiplying user's custom property value under the key: ["+a+"] by: ["+c+"]");a=w(a,g.maxKeyLength,"userData multiply",b);c=w(c,g.maxValueSize,"userData multiply",b);aa(a,c,"$mul")},max:function(a,c){b(d.INFO,"max, Saving user's maximum custom property value compared to the value: ["+c+"] under the key: ["+a+"]");a=w(a,g.maxKeyLength,"userData max",b);c=w(c,g.maxValueSize,"userData max",b);aa(a,c,"$max")},min:function(a,
c){b(d.INFO,"min, Saving user's minimum custom property value compared to the value: ["+c+"] under the key: ["+a+"]");a=w(a,g.maxKeyLength,"userData min",b);c=w(c,g.maxValueSize,"userData min",b);aa(a,c,"$min")},push:function(a,c){b(d.INFO,"push, Pushing a value: ["+c+"] under the key: ["+a+"] to user's custom property array");a=w(a,g.maxKeyLength,"userData push",b);c=w(c,g.maxValueSize,"userData push",b);aa(a,c,"$push")},push_unique:function(a,c){b(d.INFO,"push_unique, Pushing a unique value: ["+
c+"] under the key: ["+a+"] to user's custom property array");a=w(a,g.maxKeyLength,"userData push_unique",b);c=w(c,g.maxValueSize,"userData push_unique",b);aa(a,c,"$addToSet")},pull:function(a,c){b(d.INFO,"pull, Removing the value: ["+c+"] under the key: ["+a+"] from user's custom property array");aa(a,c,"$pull")},save:function(){b(d.INFO,"save, Saving changes to user's custom property");g.check_consent("users")&&(E(),b(d.INFO,"user_details, flushed the event queue"),G({user_details:JSON.stringify({custom:V})}));
...
typeof e||(f=e);this.useExplicitRcApi?(b(d.INFO,"fetch_remote_config, Fetching remote config"),u(k,l,this.rcAutoOptinAb?1:0,null,f)):(b(d.WARNING,"fetch_remote_config, Fetching remote config, with legacy API"),u(k,l,null,"legacy",f))};this.enrollUserToAb=function(a){b(d.INFO,"enrollUserToAb, Providing AB test keys to opt in for");a&&Array.isArray(a)&&0!==a.length?(a={method:"ab",keys:JSON.stringify(a)},da(a),ea("enrollUserToAb",this.url+Ha,a,function(c,e,k){if(c)b(d.ERROR,"enrollUserToAb, An error occurred: "+
...
Ca>=Ba&&g.stop_time()},6E4)};this.track_pageview=function(a,c,e){b(d.INFO,"track_pageview, Tracking page views");b(d.VERBOSE,"track_pageview, last view is:["+S+"], current view ID is:["+X+"], previous view ID is:["+ta+"]");S&&db&&(b(d.DEBUG,"track_pageview, Scroll registry triggered"),ab(),Aa=!0,za=0);ca();X=Ta();(a=w(a,g.maxKeyLength,"track_pageview",b))&&Array.isArray(a)&&(c=a,a=null);a||(a=this.getViewName());if(void 0===a||""===a)b(d.ERROR,"track_pageview, No page name to track (it is either undefined or empty string). No page view can be tracked.");
else if(null===a)b(d.ERROR,"track_pageview, View name returned as null. Page view will be ignored.");else{if(c&&c.length)for(var k=0;k<c.length;k++)try{if((new RegExp(c[k])).test(a)){b(d.INFO,"track_pageview, Ignoring the page: "+a);return}}catch(n){b(d.ERROR,"track_pageview, Problem with finding ignore list item: "+c[k]+", error: "+n)}S=a;ua=C();b(d.VERBOSE,"track_pageview, last view is assigned:["+S+"], current view ID is:["+X+"], previous view ID is:["+ta+"]");k={name:a,visit:1,view:g.getViewUrl()};
...
this.collect_from_facebook=function(a){b(d.INFO,"collect_from_facebook, Starting to collect possible user data");FB&&FB.api&&FB.api("/me",function(c){var e={};c.name&&(e.name=c.name);c.email&&(e.email=c.email);"male"===c.gender?e.gender="M":"female"===c.gender&&(e.gender="F");if(c.birthday){var k=c.birthday.split("/").pop();k&&4===k.length&&(e.byear=k)}c.work&&c.work[0]&&c.work[0].employer&&c.work[0].employer.name&&(e.organization=c.work[0].employer.name);if(a){e.custom={};for(var l in a){k=a[l].split(".");
for(var f=c,n=0;n<k.length&&(f=f[k[n]],"undefined"!==typeof f);n++);"undefined"!==typeof f&&(e.custom[l]=f)}}g.user_details(e)})};this.opt_out=function(){b(d.INFO,"opt_out, Opting out the user");this.ignore_visitor=!0;v("cly_ignore",!0)};this.opt_in=function(){b(d.INFO,"opt_in, Opting in the user");v("cly_ignore",!1);this.ignore_visitor=!1;D();this.ignore_visitor||Ka||Ja()};this.report_feedback=function(a){b(d.WARNING,"report_feedback, Deprecated function call! Use 'recordRatingWidgetWithID' or 'reportFeedbackWidgetManually' in place of this call. Call will be redirected to 'recordRatingWidgetWithID' now!");
this.recordRatingWidgetWithID(a)};this.recordRatingWidgetWithID=function(a){b(d.INFO,"recordRatingWidgetWithID, Providing information about user with ID: [ "+a.widget_id+" ]");if(this.check_consent("star-rating"))if(a.widget_id)if(a.rating){var c={key:H.STAR_RATING,count:1,segmentation:{}};c.segmentation=ra(a,"widget_id contactMe platform app_version rating email comment".split(" "));c.segmentation.app_version||(c.segmentation.app_version=this.metrics._app_version||this.app_version);5<c.segmentation.rating?
...
Math.round(e.rating);10<e.rating?(b(d.WARNING,"reportFeedbackWidgetManually, You have entered a rating higher than 10. Changing it back to 10 now."),e.rating=10):0>e.rating&&(b(d.WARNING,"reportFeedbackWidgetManually, You have entered a rating lower than 0. Changing it back to 0 now."),e.rating=0);k=["rating","comment"]}var l=H.NPS}else if("survey"===c){if(e){if(1>Object.keys(e).length){b(d.ERROR,"reportFeedbackWidgetManually, Widget should have answers to be reported");return}k=Object.keys(e)}l=
H.SURVEY}else if("rating"===c){if(e){if(!e.rating){b(d.ERROR,"reportFeedbackWidgetManually, Widget must contain rating property");return}e.rating=Math.round(e.rating);5<e.rating?(b(d.WARNING,"reportFeedbackWidgetManually, You have entered a rating higher than 5. Changing it back to 5 now."),e.rating=5):1>e.rating&&(b(d.WARNING,"reportFeedbackWidgetManually, You have entered a rating lower than 1. Changing it back to 1 now."),e.rating=1);k=["rating","comment","email","contactMe"]}l=H.STAR_RATING}else{b(d.ERROR,
"reportFeedbackWidgetManually, Widget has an unacceptable type");return}a={key:l,count:1,segmentation:{widget_id:a._id,platform:this.platform,app_version:this.metrics._app_version||this.app_version}};if(null===e)a.segmentation.closed=1;else{l=a.segmentation;if(k){for(var f,n=0,q=k.length;n<q;n++)f=k[n],"undefined"!==typeof e[f]&&(l[f]=e[f]);e=l}else e=void 0;a.segmentation=e}b(d.INFO,"reportFeedbackWidgetManually, Reporting "+c+": ",a);p(a)}else b(d.ERROR,"reportFeedbackWidgetManually, Feedback Widgets must contain _id property");
...
(K?b(d.ERROR,"presentRatingWidgetWithID, Cannot show ratingWidget popup in offline mode"):ea("presentRatingWidgetWithID",this.url+"/o/feedback/widget",{widget_id:a},function(c,e,k){if(c)b(d.ERROR,"presentRatingWidgetWithID, An error occurred: "+c);else try{var l=JSON.parse(k);P(l,!1)}catch(f){b(d.ERROR,"presentRatingWidgetWithID, JSON parse failed: "+f)}},!0))};this.initialize_feedback_popups=function(a){b(d.WARNING,"initialize_feedback_popups, Deprecated function call! Use 'initializeRatingWidgets' in place of this call. Call will be redirected now!");
...
var O=document.createElement("div");O.innerText=z.appearance.text;O.style.color=7>z.appearance.text_color.length?"#"+z.appearance.text_color:z.appearance.text_color;O.style.backgroundColor=7>z.appearance.bg_color.length?"#"+z.appearance.bg_color:z.appearance.bg_color;O.className="countly-feedback-sticker  "+z.appearance.position+"-"+z.appearance.size;O.id="countly-feedback-sticker-"+z._id;document.body.appendChild(O);y(document.getElementById("countly-feedback-sticker-"+z._id),"click",function(){document.getElementById("countly-ratings-wrapper-"+
...
Array.isArray(a))b(d.ERROR,"present_feedback_widget, Please provide at least one feedback widget object.");else try{var f=this.url;if("nps"===a.type)b(d.DEBUG,"present_feedback_widget, Widget type: nps."),f+="/feedback/nps";else if("survey"===a.type)b(d.DEBUG,"present_feedback_widget, Widget type: survey."),f+="/feedback/survey";else if("rating"===a.type)b(d.DEBUG,"present_feedback_widget, Widget type: rating."),f+="/feedback/rating";else{b(d.ERROR,"present_feedback_widget, Feedback widget only accepts nps, rating and survey types.");
return}var n=window.origin||window.location.origin;if("rating"===a.type){b(d.DEBUG,"present_feedback_widget, Loading css for rating widget.");var q="ratings";Fa(this.url+"/star-rating/stylesheets/countly-feedback-web.css")}else b(d.DEBUG,"present_feedback_widget, Loading css for survey or nps."),Fa(this.url+"/surveys/stylesheets/countly-surveys.css"),q="surveys";f+="?widget_id="+a._id;f+="&app_key="+this.app_key;f+="&device_id="+this.device_id;f+="&sdk_name=javascript_native_web";f+="&platform="+
this.platform;f+="&app_version="+this.app_version;f+="&sdk_version=23.02.0";f+="&origin="+n;f+="&widget_v=web";var t=document.createElement("iframe");t.src=f;t.name="countly-"+q+"-iframe";t.id="countly-"+q+"-iframe";var J=!1;t.onload=function(){J&&(document.getElementById("countly-"+q+"-wrapper-"+a._id).style.display="none",document.getElementById("csbg").style.display="none");J=!0;b(d.DEBUG,"present_feedback_widget, Loaded iframe.")};for(var Q=document.getElementById("csbg");Q;)Q.remove(),Q=document.getElementById("csbg"),
b(d.DEBUG,"present_feedback_widget, Removing past overlay.");var L=document.getElementsByClassName("countly-"+q+"-wrapper");for(f=0;f<L.length;f++)L[f].remove(),b(d.DEBUG,"present_feedback_widget, Removed a wrapper.");L=document.createElement("div");L.className="countly-"+q+"-wrapper";L.id="countly-"+q+"-wrapper-"+a._id;"survey"===a.type&&(L.className=L.className+" "+a.appearance.position);var oa=document.body;f=!1;c&&(document.getElementById(c)?(oa=document.getElementById(c),f=!0):b(d.ERROR,"present_feedback_widget, Provided ID not found."));
f||e&&(document.getElementsByClassName(e)[0]?oa=document.getElementsByClassName(e)[0]:b(d.ERROR,"present_feedback_widget, Provided class not found."));oa.insertAdjacentHTML("beforeend",'<div id="csbg"></div>');oa.appendChild(L);if("rating"===a.type){var fb=document.createElement("div");fb.className="countly-ratings-overlay";fb.id="countly-ratings-overlay-"+a._id;L.appendChild(fb);b(d.DEBUG,"present_feedback_widget, appended the rating overlay to wrapper");y(document.getElementById("countly-ratings-overlay-"+
a._id),"click",function(){document.getElementById("countly-ratings-wrapper-"+a._id).style.display="none"})}L.appendChild(t);b(d.DEBUG,"present_feedback_widget, Appended the iframe");y(window,"message",function(z){var O={};try{O=JSON.parse(z.data),b(d.DEBUG,"present_feedback_widget, Parsed response message "+O)}catch(Bb){b(d.ERROR,"present_feedback_widget, Error while parsing message body "+Bb)}O.close?(document.getElementById("countly-"+q+"-wrapper-"+a._id).style.display="none",document.getElementById("csbg").style.display=
"none"):b(d.DEBUG,"present_feedback_widget, Closing signal not sent yet")});if("survey"===a.type){var M=!1;switch(a.showPolicy){case "afterPageLoad":"complete"===document.readyState?M||(M=!0,k(a)):y(document,"readystatechange",function(z){"complete"!==z.target.readyState||M||(M=!0,k(a))});break;case "afterConstantDelay":setTimeout(function(){M||(M=!0,k(a))},1E4);break;case "onAbandon":"complete"===document.readyState?y(document,"mouseleave",function(){M||(M=!0,k(a))}):y(document,"readystatechange",
function(z){"complete"===z.target.readyState&&y(document,"mouseleave",function(){M||(M=!0,k(a))})});break;case "onScrollHalfwayDown":y(window,"scroll",function(){if(!M){var z=Math.max(window.scrollY,document.body.scrollTop,document.documentElement.scrollTop),O=Ea();z>=O/2&&(M=!0,k(a))}});break;default:M||(M=!0,k(a))}}else if("nps"===a.type)document.getElementById("countly-"+q+"-wrapper-"+a._id).style.display="block",document.getElementById("csbg").style.display="block";else if("rating"===a.type){var Sa=
!1;"complete"===document.readyState?Sa||(Sa=!0,l(a)):y(document,"readystatechange",function(z){"complete"!==z.target.readyState||Sa||(Sa=!0,l(a))})}}catch(z){b(d.ERROR,"present_feedback_widget, Something went wrong while presenting the widget: "+z)}};this.recordError=function(a,c,e){b(d.INFO,"recordError, Recording error");if(this.check_consent("crashes")&&a){e=e||bb;var k="";"object"===typeof a?"undefined"!==typeof a.stack?k=a.stack:("undefined"!==typeof a.name&&(k+=a.name+":"),"undefined"!==typeof a.message&&
...
(a[k]=a[k].substring(0,g.maxStackTraceLineLength));k=a.join("\n")}c=!!c;a=Ga();k={_resolution:a._resolution,_error:k,_app_version:a._app_version,_run:C()-xb,_not_os_specific:!0,_javascript:!0};if(l=navigator.battery||navigator.webkitBattery||navigator.mozBattery||navigator.msBattery)k._bat=Math.floor(100*l.level);"undefined"!==typeof navigator.onLine&&(k._online=!!navigator.onLine);k._background=!document.hasFocus();0<ka.length&&(k._logs=ka.join("\n"));ka=[];k._nonfatal=c;k._view=this.getViewName();
"undefined"!==typeof e&&(e=W(e,g.maxKeyLength,g.maxValueSize,g.maxSegmentationValues,"record_error",b),k._custom=e);try{var f=document.createElement("canvas").getContext("experimental-webgl");k._opengl=f.getParameter(f.VERSION)}catch(n){b(d.ERROR,"Could not get the experimental-webgl context: "+n)}c={};c.crash=JSON.stringify(k);c.metrics=JSON.stringify({_ua:a._ua});G(c)}};this.onStorageChange=function(a,c){b(d.INFO,"onStorageChange, Applying storage changes");switch(a){case "cly_queue":F=g.deserialize(c||
"[]");break;case "cly_event":I=g.deserialize(c||"[]");break;case "cly_remote_configs":N=g.deserialize(c||"{}");break;case "cly_ignore":g.ignore_visitor=g.deserialize(c);break;case "cly_id":g.device_id=g.deserialize(c);break;case "cly_id_type":A=g.deserialize(c)}};this._internals={store:v,getDocWidth:Ya,getDocHeight:Ea,getViewportHeight:kb,get_page_coord:Xa,get_event_target:Ra,add_event:y,createNewObjectFromProperties:ra,truncateObject:W,truncateSingleValue:w,stripTrailingSlash:ma,prepareParams:hb,
sendXmlHttpRequest:ea,isResponseValid:tb,getInternalDeviceIdType:function(){return A},getMsTimestamp:Va,getTimestamp:C,isResponseValidBroad:sb,secureRandom:Ta,log:b,getMetrics:Ga,getUA:qb,prepareRequest:da,generateUUID:Ua,sendEventsForced:E,isUUID:function(a){return/[0-9a-fA-F]{8}\-[0-9a-fA-F]{4}\-4[0-9a-fA-F]{3}\-[0-9a-fA-F]{4}\-[0-9a-fA-F]{12}/.test(a)},getId:$a,heartBeat:Ja,toRequestQueue:G,reportViewDuration:ca,loadJS:mb,loadCSS:Fa,getLastView:function(){return S},setToken:vb,getToken:function(){var a=
x("cly_token");U("cly_token");return a},showLoader:nb,hideLoader:ob,setValueInStorage:v,getValueFromStorage:x,removeValueFromStorage:U,add_cly_events:p,processScrollView:ab,processScroll:ub,currentUserAgentString:na,userAgentDeviceDetection:ib,userAgentSearchBotDetection:jb,getRequestQueue:function(){return F},getEventQueue:function(){return I},clearQueue:function(){F=[];v("cly_queue",[]);I=[];v("cly_event",[])}};this.initialize()};m.init=function(h){h=h||{};var p=h.app_key||m.app_key;if(!m.i||!m.i[p]){h=
new m.CountlyClass(h);if(!m.i){m.i={};for(var u in h)m[u]=h[u]}m.i[p]=h}return m.i[p]};var Da=0,y=function(h,p,u){"undefined"!==typeof h.addEventListener?h.addEventListener(p,u,!1):h.attachEvent("on"+p,u)},Ra=function(h){return h?"undefined"!==typeof h.target?h.target:h.srcElement:window.event.srcElement};window.addEventListener("storage",function(h){var p=(h.key+"").split("/"),u=p.pop();p=p.pop();if(m.i&&m.i[p])m.i[p].onStorageChange(u,h.newValue)});m.serialize=function(h){"object"===typeof h&&(h=
JSON.stringify(h));return h};m.deserialize=function(h){try{h=JSON.parse(h)}catch(p){}return h};m.getViewName=function(){return window.location.pathname};m.getViewUrl=function(){return window.location.pathname};m.getSearchQuery=function(){return window.location.search};m.DeviceIdType={DEVELOPER_SUPPLIED:0,SDK_GENERATED:1,TEMPORARY_ID:2};return m}});
