    ...
    function create_sandbox() {
        ...
        function create_chain() {
            ...
            function chain_script_ready(script_obj, exec_trigger) {
                if (chain_opts[_debug]) log_msg("script preload finished: " + script_obj[_real_src]);
                script_obj[_ready] = true;
                script_obj[_exec_trigger] = exec_trigger;
                advance_exec_cursor();
            }
            function chain_script_executed(script_obj, chain_group) {
                if (chain_opts[_debug]) log_msg("script execution finished: " + script_obj[_real_src]);
            ...
            function advance_exec_cursor() {
                while (exec_cursor < chain[_length]) {
                    if (is_func(chain[exec_cursor])) {
                        if (chain_opts[_debug]) log_msg("$O.wait() executing: " + chain[exec_cursor]);
                        try {
                            chain[exec_cursor++]();
                        } catch (err) {
                            if (chain_opts[_debug]) log_error("$O.wait() error caught: ", err);
                        }
                        continue;
