...
function enableLogs(debugConfig, id) {
  ...
  if (typeof console === 'object' && debugConfig === true || typeof debugConfig === 'object') {
    ...
    try {
      exportedLogger.log(`Debug logs enabled for "${id}" in hls.js version ${"1.5.0-alpha.1.0.canary.9751"}`);
    } catch (e) {
...
function parseSEIMessageFromNALu(unescapedData, headerSize, pts, samples) {
  ...
  while (seiPtr < data.length) {
    ...
    if (payloadSize < leftOver) {
      ...
      logger.error(`SEI payload of ${payloadSize} is too small, only ${leftOver} bytes left.`);
      break;
    }
    if (payloadType === 4) {
...
class Hls {
   ...
   */
  static get version() {
    return "1.5.0-alpha.1.0.canary.9751";
  }
