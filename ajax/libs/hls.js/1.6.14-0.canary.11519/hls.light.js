...
(function (global, factory) {
  ...
  function enableLogs(debugConfig, context, id) {
    ...
    if (typeof console === 'object' && debugConfig === true || typeof debugConfig === 'object') {
      ...
      try {
        newLogger.log("Debug logs enabled for \"" + context + "\" in hls.js version " + "1.6.14-0.canary.11519");
      } catch (e) {
  ...
  var AbrController = /*#__PURE__*/function (_Logger) {
    ...
    _proto.findBestLevel = function findBestLevel(currentBw, minAutoLevel, maxAutoLevel, bufferStarvationDelay, maxStarvationDelay, bwFactor, bwUpFactor) {
      ...
      var _loop = function _loop() {
          ...
          var fetchDuration = _this3.getTimeToLoadFrag(ttfbEstimateSec, adjustedbw, bitrate * avgDuration, !levelDetails || levelDetails.live);
  ...
  var CapLevelController = /*#__PURE__*/function () {
    ...
    _proto.observe = function observe() {
      var _this2 = this;
      var ResizeObserver = self.ResizeObserver;
      if (ResizeObserver) {
        this.observer = new ResizeObserver(function (entries) {
          var _entries$;
          var bounds = (_entries$ = entries[0]) == null ? void 0 : _entries$.contentRect;
          if (bounds) {
            _this2.clientRect = bounds;
            _this2.detectPlayerSize();
          }
        });
      }
    ...
    return _createClass(CapLevelController, [{
      ...
      get: function get() {
        ...
        var _this$hls$config = this.hls.config,
          ignoreDevicePixelRatio = _this$hls$config.ignoreDevicePixelRatio,
          maxDevicePixelRatio = _this$hls$config.maxDevicePixelRatio;
  ...
  var version = "1.6.14-0.canary.11519";
    ...
    _proto.nextLevelSwitch = function nextLevelSwitch() {
      ...
      if (media != null && media.readyState && levels && hls && config) {
        ...
        if (!media.paused) {
          ...
          var ttfbSec = 1 + hls.ttfbEstimate / 1000;
          var bandwidth = hls.bandwidthEstimate * config.abrBandWidthUpFactor;
          var nextLevelId = hls.nextLoadLevel;
          var nextLevel = levels[nextLevelId];
          var fragDuration = levelDetails && (this.loadingParts ? levelDetails.partTarget : levelDetails.averagetargetduration) || ((_this$fragCurrent = this.fragCurrent) == null ? void 0 : _this$fragCurrent.duration) || 6;
        ...
        var okToFlushForwardBuffer = !(levelDetails != null && levelDetails.live) || (bufferInfo.len || 0) > levelDetails.targetduration * 2;

        // find buffer range that will be reached once new fragment will be fetched
        var bufferedFrag = okToFlushForwardBuffer ? this.getBufferedFrag(media.currentTime + fetchdelay) : null;
        if (bufferedFrag) {
          ...
          if (nextBufferedFrag) {
            ...
            var _fragDuration = nextBufferedFrag.duration;
            var startPts = Math.max(bufferedFrag.end, maxStart + Math.min(Math.max(_fragDuration - this.config.maxFragLookUpTolerance, _fragDuration * (this.couldBacktrack ? 0.5 : 0.125)), _fragDuration * (this.couldBacktrack ? 0.75 : 0.25)));
