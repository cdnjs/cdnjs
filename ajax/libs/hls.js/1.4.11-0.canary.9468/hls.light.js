...
(function (global, factory) {
  ...
  function enableLogs(debugConfig, id) {
    ...
    if (self.console && debugConfig === true || typeof debugConfig === 'object') {
      ...
      try {
        exportedLogger.log("Debug logs enabled for \"" + id + "\" in hls.js version " + "1.4.11-0.canary.9468");
      } catch (e) {
  ...
  function isCodecMediaSourceSupported(codec, type) {
    ...
    return (_MediaSource$isTypeSu = MediaSource$2 == null ? void 0 : MediaSource$2.isTypeSupported(mimeTypeForCodec(codec, type))) != null ? _MediaSource$isTypeSu : false;
  }
      ...
      Object.keys(data).forEach(function (trackName) {
        if (sourceBufferCount) {
          ...
          if (track && typeof track.buffer.changeType === 'function') {
            ...
            var currentCodedFull = pickMostCompleteCodecName(track.codec, track.levelCodec);
            var currentCodec = currentCodedFull.replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
            var trackCodec = pickMostCompleteCodecName(codec, levelCodec);
            var nextCodec = trackCodec.replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
            if (currentCodec !== nextCodec) {
              ...
              _this3.log("switching codec " + currentCodedFull + " to " + trackCodec);
        ...
        Promise.resolve().then(function () {
          if (_this2.hls) {
            if (unfilteredLevels.length) {
              _this2.warn("One or more CODECS in variant not supported: " + JSON.stringify(unfilteredLevels[0].attrs));
            }
