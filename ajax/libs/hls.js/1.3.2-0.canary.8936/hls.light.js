...
/* harmony import */ var _controller_audio_stream_controller__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./utils/mediakeys-helper */ "./src/empty.js");
...
/* harmony import */ var _controller_content_steering_controller__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./controller/content-steering-controller */ "./src/controller/content-steering-controller.ts");
/* harmony import */ var _controller_error_controller__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./controller/error-controller */ "./src/controller/error-controller.ts");
...
var hlsDefaultConfig = _objectSpread(_objectSpread({
  ...
  errorController: _controller_error_controller__WEBPACK_IMPORTED_MODULE_6__["default"],
  ...
  drmSystemOptions: {},
  // used by eme-controller
  requestMediaKeySystemAccessFunc:  false ? 0 : null,
  // used by eme-controller
  testBandwidth: true,
  progressive: false,
  lowLatencyMode: true,
  ...
  steeringManifestLoadPolicy: {
    default:  false ? 0 : defaultLoadPolicy
  },
  ...
  subtitleStreamController:  false ? 0 : undefined,
  subtitleTrackController:  false ? 0 : undefined,
  timelineController:  false ? 0 : undefined,
  audioStreamController:  false ? 0 : undefined,
  audioTrackController:  false ? 0 : undefined,
  emeController:  false ? 0 : undefined,
  cmcdController:  false ? 0 : undefined,
  contentSteeringController:  false ? 0 : undefined
});
function timelineConfig() {
  return {
    cueHandler: (_controller_audio_stream_controller__WEBPACK_IMPORTED_MODULE_1___default()),
...
function mergeConfig(defaultConfig, userConfig) {
  ...
  deprecatedSettingTypes.forEach(function (type) {
    ...
    if (report.length) {
      _utils_logger__WEBPACK_IMPORTED_MODULE_9__.logger.warn("hls.js config: \"" + report.join('", "') + "\" setting(s) are deprecated, use \"" + policyName + "\": " + JSON.stringify(userConfig[policyName]));
    }
  });
  return _objectSpread(_objectSpread({}, defaultsCopy), userConfig);
}
...
function enableStreamingMode(config) {
  ...
  if (currentLoader !== _utils_fetch_loader__WEBPACK_IMPORTED_MODULE_8__["default"] && currentLoader !== _utils_xhr_loader__WEBPACK_IMPORTED_MODULE_7__["default"]) {
    ...
    _utils_logger__WEBPACK_IMPORTED_MODULE_9__.logger.log('[config]: Custom loader detected, cannot enable progressive streaming');
    config.progressive = false;
  } else {
    var canStreamProgressively = (0,_utils_fetch_loader__WEBPACK_IMPORTED_MODULE_8__.fetchSupported)();
    if (canStreamProgressively) {
      config.loader = _utils_fetch_loader__WEBPACK_IMPORTED_MODULE_8__["default"];
      config.progressive = true;
      config.enableSoftwareAES = true;
      _utils_logger__WEBPACK_IMPORTED_MODULE_9__.logger.log('[config]: Progressive streaming enabled, using FetchLoader');
    }
  }
}
...
var AbrController = /*#__PURE__*/function () {
  ...
  _proto.registerListeners = function registerListeners() {
    ...
    hls.on(_events__WEBPACK_IMPORTED_MODULE_2__.Events.LEVEL_SWITCHING, this.onLevelSwitching, this);
  ...
  _proto.unregisterListeners = function unregisterListeners() {
    ...
    hls.off(_events__WEBPACK_IMPORTED_MODULE_2__.Events.LEVEL_SWITCHING, this.onLevelSwitching, this);
  ...
  _proto._abandonRulesCheck = function _abandonRulesCheck() {
    ...
    for (nextLoadLevel = frag.level - 1; nextLoadLevel > minAutoLevel; nextLoadLevel--) {
      ...
      fragLevelNextLoadedDelay = duration * levelNextBitrate / bwe + ttfbEstimate / 1000;
  ...
  _proto.onFragBuffered = function onFragBuffered(event, data) {
    var frag = data.frag,
      part = data.part;
    var stats = part !== null && part !== void 0 && part.stats.loaded ? part.stats : frag.stats;
    if (stats.aborted) {
      return;
    }
    if (this.ignoreFragment(frag)) {
  ...
  _proto.ignoreFragment = function ignoreFragment(frag) {
    ...
    return frag.type !== _types_loader__WEBPACK_IMPORTED_MODULE_3__.PlaylistLevelType.MAIN || frag.sn === 'initSegment';
  };
  ...
  _proto.getNextABRAutoLevel = function getNextABRAutoLevel() {
    ...
    _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.trace("[abr] " + (bufferStarvationDelay ? 'rebuffering expected' : 'buffer is empty') + ", finding optimal quality level");
    ...
    if (!bufferStarvationDelay) {
      ...
      if (bitrateTestDelay) {
        ...
        _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.trace("[abr] bitrate test took " + Math.round(1000 * bitrateTestDelay) + "ms, set first fragment max fetchDuration to " + Math.round(1000 * maxStarvationDelay) + " ms");
  ...
  _proto.findBestLevel = function findBestLevel(currentBw, minAutoLevel, maxAutoLevel, maxFetchDuration, bwFactor, bwUpFactor) {
    ...
    for (var i = maxAutoLevel; i >= minAutoLevel; i--) {
      ...
      if (levelSkippedMax !== -1) {
        _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.trace("[abr] Skipped level(s) " + levelSkippedMin + "-" + levelSkippedMax + " with CODECS:\"" + levels[levelSkippedMax].attrs.CODECS + "\"; not compatible with \"" + level.attrs.CODECS + "\"");
      }
      ...
      var fetchDuration = bitrate * avgDuration / adjustedbw + ttfbEstimate / 1000;
      _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.trace("[abr] level:" + i + " adjustedbw-bitrate:" + Math.round(adjustedbw - bitrate) + " avgDuration:" + avgDuration.toFixed(1) + " maxFetchDuration:" + maxFetchDuration.toFixed(1) + " fetchDuration:" + fetchDuration.toFixed(1));
...
/* harmony import */ var _utils_error_helper__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../utils/error-helper */ "./src/utils/error-helper.ts");
...
var BasePlaylistController = /*#__PURE__*/function () {
  ...
  _proto.switchParams = function switchParams(playlistUri, previous) {
    ...
    if (renditionReports) {
      ...
      if (foundIndex !== -1) {
        ...
        var msn = parseInt(_attr['LAST-MSN']) || (previous === null || previous === void 0 ? void 0 : previous.lastPartSn);
        var part = parseInt(_attr['LAST-PART']) || (previous === null || previous === void 0 ? void 0 : previous.lastPartIndex);
  ...
  _proto.checkRetry = function checkRetry(errorEvent) {
    ...
    var isTimeout = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_4__.isTimeoutError)(errorEvent);
    ...
    var retry = action === _errors__WEBPACK_IMPORTED_MODULE_3__.NetworkErrorAction.RetryRequest && !!errorAction && !!retryConfig;
    if (retry) {
      var _errorEvent$context;
      this.requestScheduled = -1;
      if (isTimeout && (_errorEvent$context = errorEvent.context) !== null && _errorEvent$context !== void 0 && _errorEvent$context.deliveryDirectives) {
        // The LL-HLS request already timed out so retry immediately
        this.warn("Retrying playlist loading " + (retryCount + 1) + "/" + retryConfig.maxNumRetry + " after \"" + errorDetails + "\" without delivery-directives");
        this.loadPlaylist();
      } else {
        var delay = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_4__.getRetryDelay)(retryConfig, retryCount);
...
/* harmony import */ var _utils_error_helper__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(/*! ../utils/error-helper */ "./src/utils/error-helper.ts");
...
var BaseStreamController = /*#__PURE__*/function (_TaskLoop) {
  ...
  function BaseStreamController(hls, fragmentTracker, keyLoader, logPrefix) {
    ...
    hls.on(_events__WEBPACK_IMPORTED_MODULE_5__.Events.MANIFEST_LOADED, _this.onManifestLoaded, _assertThisInitialized(_this));
    return _this;
  }
  ...
  _proto.stopLoad = function stopLoad() {
    ...
    var frag = this.fragCurrent;
    if (frag !== null && frag !== void 0 && frag.loader) {
      frag.abortRequests();
    ...
    this._doFragLoad(frag, level).then(function (data) {
      ...
      if (payload && payload.byteLength > 0 && decryptData && decryptData.key && decryptData.iv && decryptData.method === 'AES-128') {
        ...
        return _this3.decrypter.decrypt(new Uint8Array(payload), decryptData.key.buffer, decryptData.iv.buffer).catch(function (err) {
          hls.trigger(_events__WEBPACK_IMPORTED_MODULE_5__.Events.ERROR, {
            type: _errors__WEBPACK_IMPORTED_MODULE_6__.ErrorTypes.MEDIA_ERROR,
            details: _errors__WEBPACK_IMPORTED_MODULE_6__.ErrorDetails.FRAG_DECRYPT_ERROR,
            fatal: false,
            error: err,
            reason: err.message,
            frag: frag
  ...
  _proto.doFragPartsLoad = function doFragPartsLoad(frag, fromPart, level, progressCallback) {
    ...
    return new Promise(function (resolve, reject) {
      var _level$details;
      var partsLoaded = [];
      var initialPartList = (_level$details = level.details) === null || _level$details === void 0 ? void 0 : _level$details.partList;
      var loadPart = function loadPart(part) {
        _this5.fragmentLoader.loadPart(frag, part, progressCallback).then(function (partLoadedData) {
          ...
          var nextPart = (0,_level_helper__WEBPACK_IMPORTED_MODULE_11__.getPartWith)(level, frag.sn, part.index + 1) || (0,_level_helper__WEBPACK_IMPORTED_MODULE_11__.findPart)(initialPartList, frag.sn, part.index + 1);
          if (nextPart) {
            loadPart(nextPart);
          } else {
            return resolve({
  ...
  _proto.getCurrentContext = function getCurrentContext(chunkMeta) {
    ...
    if (!(levels !== null && levels !== void 0 && levels[levelIndex])) {
    ...
    var frag = part ? part.fragment : (0,_level_helper__WEBPACK_IMPORTED_MODULE_11__.getFragmentWithSN)(level, sn, fragCurrent);
    if (!frag) {
      return null;
    }
  ...
  _proto.bufferFragmentData = function bufferFragmentData(data, frag, part, chunkMeta) {
    ...
    if (!((_buffer = buffer) !== null && _buffer !== void 0 && _buffer.length)) {
      return;
    }
    var segment = {
      type: data.type,
  ...
  */;
  ...
  _proto.setStartPosition = function setStartPosition(details, sliding) {
    ...
    if (startPosition === -1 || this.lastCurrentTime === -1) {
      ...
      if (startTimeOffset !== null && (0,_home_runner_work_hls_js_hls_js_src_polyfills_number__WEBPACK_IMPORTED_MODULE_0__.isFiniteNumber)(startTimeOffset)) {
  ...
  _proto.onFragmentOrKeyLoadError = function onFragmentOrKeyLoadError(filterType, data) {
    ...
    if (this.fragContextChanged(frag)) {
      ...
      this.warn("Frag load error must match current frag to retry " + frag.url + " > " + ((_this$fragCurrent2 = this.fragCurrent) === null || _this$fragCurrent2 === void 0 ? void 0 : _this$fragCurrent2.url));
      return;
    }
    ...
    if (errorAction && action === _errors__WEBPACK_IMPORTED_MODULE_6__.NetworkErrorAction.RetryRequest && retryConfig) {
      if (!this.loadedmetadata) {
        this.startFragRequested = false;
        this.nextLoadPosition = this.startPosition;
      }
      var delay = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_16__.getRetryDelay)(retryConfig, retryCount);
      ...
      if (retryCount < retryConfig.maxNumRetry) {
        ...
        _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.warn(data.details + " reached or exceeded max retry (" + retryCount + ")");
      }
    } else {
  ...
  _proto.updateLevelTiming = function updateLevelTiming(frag, part, level, partial) {
    ...
    if (parsed) {
      level.fragmentError = 0;
    } else if (((_this$transmuxer = this.transmuxer) === null || _this$transmuxer === void 0 ? void 0 : _this$transmuxer.error) === null) {
...
var BufferController = /*#__PURE__*/function () {
  ...
  _proto._onSBUpdateError = function _onSBUpdateError(type, event) {
    ...
    _utils_logger__WEBPACK_IMPORTED_MODULE_2__.logger.error("[buffer-controller]: " + error, event);
  ...
  _proto.blockBuffers = function blockBuffers(onUnblocked, buffers) {
    ...
    Promise.all(blockingOperations).then(function () {
      ...
      buffers.forEach(function (type) {
        ...
        if (!(sb !== null && sb !== void 0 && sb.updating)) {
          operationQueue.shiftAndExecuteNext(type);
        }
      });
    });
...
var CapLevelController = /*#__PURE__*/function () {
  ...
  CapLevelController.getMaxLevelByMediaSize = function getMaxLevelByMediaSize(levels, width, height) {
    if (!(levels !== null && levels !== void 0 && levels.length)) {
      return -1;
    }
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (/* binding */ ContentSteeringController)
/* harmony export */ });
/* harmony import */ var _events__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../events */ "./src/events.ts");
/* harmony import */ var _types_level__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../types/level */ "./src/types/level.ts");
/* harmony import */ var _utils_attr_list__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../utils/attr-list */ "./src/utils/attr-list.ts");
/* harmony import */ var _level_controller__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./level-controller */ "./src/controller/level-controller.ts");
...
var ContentSteeringController = /*#__PURE__*/function () {
  function ContentSteeringController(hls) {
    ...
    this.hls = hls;
    this.log = _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.log.bind(_utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger, "[content-steering]:");
    this.registerListeners();
  }
  ...
  _proto.onError = function onError(event, data) {
    var errorAction = data.errorAction;
    if ((errorAction === null || errorAction === void 0 ? void 0 : errorAction.action) === _errors__WEBPACK_IMPORTED_MODULE_4__.NetworkErrorAction.SendAlternateToPenaltyBox && errorAction.flags === _errors__WEBPACK_IMPORTED_MODULE_4__.ErrorActionFlags.MoveAllAlternatesMatchingHost) {
  ...
  _proto.updatePathwayPriority = function updatePathwayPriority(pathwayPriority) {
    ...
    for (var i = 0; i < pathwayPriority.length; i++) {
      ...
      if (levels.length > 0) {
        ...
        this.hls.trigger(_events__WEBPACK_IMPORTED_MODULE_0__.Events.LEVELS_UPDATED, {
          levels: levels
        });
  ...
  _proto.clonePathways = function clonePathways(pathwayClones) {
    ...
    pathwayClones.forEach(function (pathwayClone) {
      ...
      var clonedVariants = _this.getLevelsForPathway(baseId).map(function (baseLevel) {
        ...
        var attributes = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_2__.AttrList(baseLevel.attrs);
        ...
        var clonedLevel = new _types_level__WEBPACK_IMPORTED_MODULE_1__.Level(levelParsed);
        (0,_level_controller__WEBPACK_IMPORTED_MODULE_3__.addGroupId)(clonedLevel, 'audio', clonedAudioGroupId);
        (0,_level_controller__WEBPACK_IMPORTED_MODULE_3__.addGroupId)(clonedLevel, 'text', clonedSubtitleGroupId);
        return clonedLevel;
      });
  ...
  _proto.loadSteeringManifest = function loadSteeringManifest(uri) {
    ...
    var callbacks = {
      onSuccess: function onSuccess(response, stats, context, networkDetails) {
        ...
        if (reloadUri) {
          try {
            _this2.uri = new URL(reloadUri, url).href;
          } catch (error) {
...
function cloneRenditionGroups(tracks, groupCloneMap, uriReplacement, cloneId) {
  ...
  Object.keys(groupCloneMap).forEach(function (audioGroupId) {
    var clonedTracks = tracks.filter(function (track) {
      ...
      clonedTrack.attrs = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_2__.AttrList(clonedTrack.attrs);
...
/* harmony import */ var _types_loader__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../types/loader */ "./src/types/loader.ts");
/* harmony import */ var _utils_error_helper__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../utils/error-helper */ "./src/utils/error-helper.ts");
...
var ErrorController = /*#__PURE__*/function () {
  function ErrorController(hls) {
    ...
    this.error = void 0;
    this.hls = hls;
    this.log = _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.log.bind(_utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger, "[info]:");
    this.warn = _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.warn.bind(_utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger, "[warning]:");
    this.error = _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.error.bind(_utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger, "[error]:");
    this.registerListeners();
  }
  var _proto = ErrorController.prototype;
  _proto.registerListeners = function registerListeners() {
    this.hls.on(_events__WEBPACK_IMPORTED_MODULE_0__.Events.ERROR, this.onError, this);
  };
  _proto.unregisterListeners = function unregisterListeners() {
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_0__.Events.ERROR, this.onError, this);
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_0__.Events.ERROR, this.onErrorOut, this);
  };
  _proto.destroy = function destroy() {
    this.unregisterListeners();
    // @ts-ignore
  ...
  _proto.getVariantLevelIndex = function getVariantLevelIndex(frag) {
    return (frag === null || frag === void 0 ? void 0 : frag.type) === _types_loader__WEBPACK_IMPORTED_MODULE_2__.PlaylistLevelType.MAIN ? frag.level : this.hls.loadLevel;
  };
  _proto.onError = function onError(event, data) {
    var _data$level;
    if (data.fatal) {
    ...
    switch (data.details) {
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.FRAG_LOAD_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.FRAG_LOAD_TIMEOUT:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.KEY_LOAD_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.KEY_LOAD_TIMEOUT:
        data.errorAction = this.getFragRetryOrSwitchAction(data);
        return;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.FRAG_PARSING_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.FRAG_DECRYPT_ERROR:
        {
          // Switch level if possible, otherwise allow retry count to reach max error retries
          data.errorAction = this.getFragRetryOrSwitchAction(data);
          data.errorAction.action = _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.SendAlternateToPenaltyBox;
          return;
        }
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.LEVEL_EMPTY_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.LEVEL_PARSING_ERROR:
        {
          var _data$context, _data$context$levelDe;
          // Only retry when empty and live
          var levelIndex = data.parent === _types_loader__WEBPACK_IMPORTED_MODULE_2__.PlaylistLevelType.MAIN ? data.level : hls.loadLevel;
          if (data.details === _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.LEVEL_EMPTY_ERROR && !!((_data$context = data.context) !== null && _data$context !== void 0 && (_data$context$levelDe = _data$context.levelDetails) !== null && _data$context$levelDe !== void 0 && _data$context$levelDe.live)) {
        ...
        return;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.LEVEL_LOAD_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.LEVEL_LOAD_TIMEOUT:
        if (typeof (context === null || context === void 0 ? void 0 : context.level) === 'number') {
          data.errorAction = this.getPlaylistRetryOrSwitchAction(data, context.level);
        }
        return;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.AUDIO_TRACK_LOAD_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.AUDIO_TRACK_LOAD_TIMEOUT:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.SUBTITLE_LOAD_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.SUBTITLE_TRACK_LOAD_TIMEOUT:
        if (context) {
          var level = hls.levels[hls.loadLevel];
          if (level && (context.type === _types_loader__WEBPACK_IMPORTED_MODULE_2__.PlaylistContextType.AUDIO_TRACK && level.audioGroupIds && context.groupId === level.audioGroupIds[level.urlId] || context.type === _types_loader__WEBPACK_IMPORTED_MODULE_2__.PlaylistContextType.SUBTITLE_TRACK && level.textGroupIds && context.groupId === level.textGroupIds[level.urlId])) {
            // Perform Pathway switch or Redundant failover if possible for fastest recovery
            // otherwise allow playlist retry count to reach max error retries
            data.errorAction = this.getPlaylistRetryOrSwitchAction(data, hls.loadLevel);
            data.errorAction.action = _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.SendAlternateToPenaltyBox;
            data.errorAction.flags = _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.MoveAllAlternatesMatchingHost;
            return;
          }
        }
        return;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.KEY_SYSTEM_STATUS_OUTPUT_RESTRICTED:
        {
          var _level = hls.levels[hls.loadLevel];
          var restrictedHdcpLevel = _level === null || _level === void 0 ? void 0 : _level.attrs['HDCP-LEVEL'];
          if (restrictedHdcpLevel) {
            data.errorAction = {
              action: _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.SendAlternateToPenaltyBox,
              flags: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.MoveAllAlternatesMatchingHDCP,
              hdcpLevel: restrictedHdcpLevel
            };
          }
        }
        return;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_ADD_CODEC_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.REMUX_ALLOC_ERROR:
        data.errorAction = this.getLevelSwitchAction(data, (_data$level = data.level) != null ? _data$level : hls.loadLevel);
        return;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.INTERNAL_EXCEPTION:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_APPENDING_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_APPEND_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_FULL_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.LEVEL_SWITCH_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_STALLED_ERROR:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_SEEK_OVER_HOLE:
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_NUDGE_ON_STALL:
        data.errorAction = {
          action: _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.DoNothing,
          flags: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.None
        };
        return;
    }
    if (data.type === _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorTypes.KEY_SYSTEM_ERROR) {
  ...
  _proto.getPlaylistRetryOrSwitchAction = function getPlaylistRetryOrSwitchAction(data, levelIndex) {
    ...
    var hls = this.hls;
    var retryConfig = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_3__.getRetryConfig)(hls.config.playlistLoadPolicy, data);
    var retryCount = this.playlistError++;
    var httpStatus = (_data$response = data.response) === null || _data$response === void 0 ? void 0 : _data$response.code;
    var retry = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_3__.shouldRetry)(retryConfig, retryCount, (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_3__.isTimeoutError)(data), httpStatus);
    if (retry) {
      return {
        action: _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.RetryRequest,
        flags: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.None,
    ...
    if ((_data$context2 = data.context) !== null && _data$context2 !== void 0 && _data$context2.deliveryDirectives) {
      return {
        action: _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.DoNothing,
        flags: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.None,
        retryConfig: retryConfig || {
  ...
  _proto.getFragRetryOrSwitchAction = function getFragRetryOrSwitchAction(data) {
    ...
    var retryConfig = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_3__.getRetryConfig)(data.details.startsWith('key') ? keyLoadPolicy : fragLoadPolicy, data);
    ...
    if (level) {
      var _data$response2;
      level.fragmentError++;
      var httpStatus = (_data$response2 = data.response) === null || _data$response2 === void 0 ? void 0 : _data$response2.code;
      var retry = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_3__.shouldRetry)(retryConfig, fragmentErrors, (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_3__.isTimeoutError)(data), httpStatus);
      if (retry) {
        return {
          action: _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.RetryRequest,
          flags: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.None,
  ...
  _proto.getLevelSwitchAction = function getLevelSwitchAction(data, levelIndex) {
    ...
    return {
      action: _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.SendAlternateToPenaltyBox,
      flags: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.MoveAllAlternatesMatchingHost
    };
  };
  _proto.onErrorOut = function onErrorOut(event, data) {
    var _data$errorAction;
    switch ((_data$errorAction = data.errorAction) === null || _data$errorAction === void 0 ? void 0 : _data$errorAction.action) {
      case _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.DoNothing:
        break;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.SendAlternateToPenaltyBox:
        this.sendAlternateToPenaltyBox(data);
        if (!data.errorAction.resolved) {
          data.fatal = true;
        }
        break;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.NetworkErrorAction.RetryRequest:
  ...
  _proto.sendAlternateToPenaltyBox = function sendAlternateToPenaltyBox(data) {
    ...
    switch (flags) {
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.None:
        this.switchLevel(data, nextAutoLevel);
        break;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.MoveAllAlternatesMatchingHost:
        {
          var levelIndex = data.parent === _types_loader__WEBPACK_IMPORTED_MODULE_2__.PlaylistLevelType.MAIN ? data.level : hls.loadLevel;
        ...
        break;
      case _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorActionFlags.MoveAllAlternatesMatchingHDCP:
        if (hdcpLevel) {
          hls.maxHdcpLevel = _types_level__WEBPACK_IMPORTED_MODULE_4__.HdcpLevels[_types_level__WEBPACK_IMPORTED_MODULE_4__.HdcpLevels.indexOf(hdcpLevel) - 1];
          errorAction.resolved = true;
        }
  ...
  _proto.redundantFailover = function redundantFailover(levelIndex) {
    ...
    if (redundantLevels > 1) {
      ...
      this.log("Switching to Redundant Stream " + (newUrlId + 1) + "/" + redundantLevels + ": \"" + level.url[newUrlId] + "\"");
      this.playlistError = 0;
...
var FPSController = /*#__PURE__*/function () {
  ...
  _proto.unregisterListeners = function unregisterListeners() {
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_0__.Events.MEDIA_ATTACHING, this.onMediaAttaching, this);
  };
  _proto.destroy = function destroy() {
    if (this.timer) {
...
var FragmentTracker = /*#__PURE__*/function () {
  ...
  _proto.destroy = function destroy() {
    ...
    this.fragments =
    // @ts-ignore
    this.endListFragments = this.timeRanges = this.mainFragEntity = this.activeParts = null;
  }
  ...
  _proto.getAppendedFrag = function getAppendedFrag(position, levelType) {
    if (levelType === _types_loader__WEBPACK_IMPORTED_MODULE_1__.PlaylistLevelType.MAIN) {
      var mainFragEntity = this.mainFragEntity,
        activeParts = this.activeParts;
      if (mainFragEntity) {
        if (mainFragEntity && activeParts) {
          for (var i = activeParts.length; i--;) {
            var activePart = activeParts[i];
            var appendedPTS = activePart ? activePart.end : mainFragEntity.appendedPTS;
        ...
        } else if (mainFragEntity.body.start <= position && mainFragEntity.appendedPTS !== null && position <= mainFragEntity.appendedPTS) {
  ...
  _proto.onBufferAppended = function onBufferAppended(event, data) {
    ...
    if (frag.type === _types_loader__WEBPACK_IMPORTED_MODULE_1__.PlaylistLevelType.MAIN) {
      var lastMainFrag = mainFragEntity ? mainFragEntity.body : null;
      ...
      if (!part && mainFragEntity) {
        ...
        for (var i = 0; i < timeRange.length; i++) {
          ...
          if (rangeEnd <= streamInfo.endPTS && rangeEnd > streamInfo.startPTS) {
            mainFragEntity.appendedPTS = Math.max(rangeEnd, mainFragEntity.appendedPTS || 0);
          } else {
...
var GapController = /*#__PURE__*/function () {
  ...
  _proto._reportStall = function _reportStall(bufferInfo) {
    ...
    if (!stallReported && media) {
      ...
      _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.warn(error.message);
  ...
  _proto._tryNudgeBuffer = function _tryNudgeBuffer() {
    ...
    if (nudgeRetry < config.nudgeMaxRetry) {
      ...
      _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.error(_error.message);
      hls.trigger(_events__WEBPACK_IMPORTED_MODULE_2__.Events.ERROR, {
        type: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorTypes.MEDIA_ERROR,
        details: _errors__WEBPACK_IMPORTED_MODULE_1__.ErrorDetails.BUFFER_STALLED_ERROR,
        error: _error,
...
var LatencyController = /*#__PURE__*/function () {
  ...
  _proto.unregisterListeners = function unregisterListeners() {
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_1__.Events.MEDIA_ATTACHED, this.onMediaAttached, this);
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_1__.Events.MEDIA_DETACHING, this.onMediaDetaching, this);
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_1__.Events.MANIFEST_LOADING, this.onManifestLoading, this);
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_1__.Events.LEVEL_UPDATED, this.onLevelUpdated, this);
    this.hls.off(_events__WEBPACK_IMPORTED_MODULE_1__.Events.ERROR, this.onError, this);
  };
  _proto.onMediaAttached = function onMediaAttached(event, data) {
    this.media = data.media;
...
/* harmony import */ var _base_playlist_controller__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./base-playlist-controller */ "./src/controller/base-playlist-controller.ts");
...
var LevelController = /*#__PURE__*/function (_BasePlaylistControll) {
  ...
  _proto._registerListeners = function _registerListeners() {
    ...
    hls.on(_events__WEBPACK_IMPORTED_MODULE_1__.Events.LEVELS_UPDATED, this.onLevelsUpdated, this);
  ...
  _proto._unregisterListeners = function _unregisterListeners() {
    ...
    hls.off(_events__WEBPACK_IMPORTED_MODULE_1__.Events.LEVELS_UPDATED, this.onLevelsUpdated, this);
  ...
  _proto.onManifestLoaded = function onManifestLoaded(event, data) {
    ...
    data.levels.forEach(function (levelParsed) {
      ...
      if (((_levelParsed$audioCod = levelParsed.audioCodec) === null || _levelParsed$audioCod === void 0 ? void 0 : _levelParsed$audioCod.indexOf('mp4a.40.34')) !== -1) {
      ...
      var contentSteeringPrefix =  false ? 0 : '';
  ...
  _proto.filterAndSortMediaOptions = function filterAndSortMediaOptions(unfilteredLevels, data) {
    ...
    var levels = unfilteredLevels.filter(function (_ref) {
      ...
      return !(unknownCodecs !== null && unknownCodecs !== void 0 && unknownCodecs.length) && (!audioCodec || (0,_utils_codecs__WEBPACK_IMPORTED_MODULE_3__.isCodecSupportedInMp4)(audioCodec, 'audio')) && (!videoCodec || (0,_utils_codecs__WEBPACK_IMPORTED_MODULE_3__.isCodecSupportedInMp4)(videoCodec, 'video'));
    });
    ...
    if (levels.length === 0) {
      ...
      Promise.resolve().then(function () {
        if (_this2.hls) {
          ...
          _this2.hls.trigger(_events__WEBPACK_IMPORTED_MODULE_1__.Events.ERROR, {
  ...
  _proto.onError = function onError(event, data) {
    ...
    if (data.context.type === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.LEVEL && data.context.level === this.level) {
  ...
  _proto.onFragLoaded = function onFragLoaded(event, _ref3) {
    var frag = _ref3.frag;
    if (frag !== undefined && frag.type === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistLevelType.MAIN) {
  ...
  _proto.onAudioTrackSwitched = function onAudioTrackSwitched(event, data) {
    ...
    if (currentLevel.audioGroupIds && currentLevel.audioGroupIds[currentLevel.urlId] !== audioGroupId) {
      var urlId = -1;
  ...
  _proto.loadPlaylist = function loadPlaylist(hlsUrlParameters) {
    ...
    if (currentLevel && this.shouldLoadPlaylist(currentLevel)) {
      ...
      this.log("Loading level index " + currentLevelIndex + ((hlsUrlParameters === null || hlsUrlParameters === void 0 ? void 0 : hlsUrlParameters.msn) !== undefined ? ' at sn ' + hlsUrlParameters.msn + ' part ' + hlsUrlParameters.part : '') + " with" + (pathwayId ? ' Pathway ' + pathwayId : '') + " URI " + (id + 1) + "/" + currentLevel.url.length + " " + url);
  ...
  _createClass(LevelController, [{
    ...
    set: function set(newLevel) {
      ...
      if (this.currentLevelIndex === newLevel && level.details && lastLevel && lastPathwayId === pathwayId) {
        return;
      }
...
}(_base_playlist_controller__WEBPACK_IMPORTED_MODULE_4__["default"]);
...
/* harmony export */   "findPart": () => (/* binding */ findPart),
...
function getFragmentWithSN(level, sn, fragCurrent) {
  if (!(level !== null && level !== void 0 && level.details)) {
    return null;
  }
  var levelDetails = level.details;
...
function getPartWith(level, sn, partIndex) {
  ...
  return findPart((_level$details = level.details) === null || _level$details === void 0 ? void 0 : _level$details.partList, sn, partIndex);
}
...
var StreamController = /*#__PURE__*/function (_BaseStreamController) {
  ...
  _proto.doTick = function doTick() {
    ...
    if (this.state === _base_stream_controller__WEBPACK_IMPORTED_MODULE_1__.State.IDLE) {
      this.doTickIdle();
    }
    this.onTickEnd();
  };
  ...
  _proto.doTickIdle = function doTickIdle() {
    ...
    if (!(levels !== null && levels !== void 0 && levels[level])) {
      return;
    }
  ...
  */;
  ...
  _proto.onAudioTrackSwitching = function onAudioTrackSwitching(event, data) {
    ...
    if (!altAudio) {
      ...
      hls.trigger(_events__WEBPACK_IMPORTED_MODULE_3__.Events.AUDIO_TRACK_SWITCHED, data);
    }
  };
  _proto.onAudioTrackSwitched = function onAudioTrackSwitched(event, data) {
    var trackId = data.id;
  ...
  _proto.onError = function onError(event, data) {
    ...
    switch (data.details) {
      ...
      case _errors__WEBPACK_IMPORTED_MODULE_11__.ErrorDetails.FRAG_DECRYPT_ERROR:
      ...
      case _errors__WEBPACK_IMPORTED_MODULE_11__.ErrorDetails.LEVEL_PARSING_ERROR:
        // in case of non fatal error while loading level, if level controller is not retrying to load level, switch back to IDLE
        if (!data.levelRetry && this.state === _base_stream_controller__WEBPACK_IMPORTED_MODULE_1__.State.WAITING_LEVEL && ((_data$context = data.context) === null || _data$context === void 0 ? void 0 : _data$context.type) === _types_loader__WEBPACK_IMPORTED_MODULE_6__.PlaylistContextType.LEVEL) {
          this.state = _base_stream_controller__WEBPACK_IMPORTED_MODULE_1__.State.IDLE;
        }
        break;
...
var TransmuxerInterface = /*#__PURE__*/function () {
  function TransmuxerInterface(hls, id, onTransmuxComplete, onFlush) {
    ...
    var forwardMessage = function forwardMessage(ev, data) {
      ...
      data.id = _this.id;
      if (ev === _events__WEBPACK_IMPORTED_MODULE_1__.Events.ERROR) {
...
var Transmuxer = /*#__PURE__*/function () {
  ...
  _proto.push = function push(data, decryptdata, chunkMeta, state) {
    ...
    if (resetMuxers) {
      ...
      if (error) {
        _utils_logger__WEBPACK_IMPORTED_MODULE_9__.logger.warn("[transmuxer] " + error.message);
    ...
    units.forEach(function (unit) {
      switch (unit.type) {
        ...
        case 7:
          ...
          if (!track.sps) {
            var sps = unit.data;
            var expGolombDecoder = new _exp_golomb__WEBPACK_IMPORTED_MODULE_2__["default"](sps);
...
var ErrorActionFlags; // Reserved for future use
(function (ErrorActionFlags) {
  ErrorActionFlags[ErrorActionFlags["None"] = 0] = "None";
...
/* harmony import */ var eventemitter3__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! eventemitter3 */ "./node_modules/eventemitter3/index.mjs");
...
var Hls = /*#__PURE__*/function () {
  ...
  function Hls(userConfig) {
    ...
    this._media = null;
    this.url = null;
    (0,_utils_logger__WEBPACK_IMPORTED_MODULE_9__.enableLogs)(userConfig.debug || false, 'Hls instance');
    ...
    var levelController = this.levelController = new _controller_level_controller__WEBPACK_IMPORTED_MODULE_4__["default"](this, contentSteering);
    ...
    if (typeof onErrorOut === 'function') {
      this.on(_events__WEBPACK_IMPORTED_MODULE_12__.Events.ERROR, onErrorOut, errorController);
    }
  }
...
/* harmony import */ var _home_runner_work_hls_js_hls_js_src_polyfills_number__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./src/polyfills/number */ "./src/polyfills/number.ts");
/* harmony import */ var _errors__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../errors */ "./src/errors.ts");
/* harmony import */ var _utils_error_helper__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../utils/error-helper */ "./src/utils/error-helper.ts");
...
var FragmentLoader = /*#__PURE__*/function () {
  ...
  _proto.load = function load(frag, _onProgress) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var loadPolicy = (0,_utils_error_helper__WEBPACK_IMPORTED_MODULE_2__.getLoaderConfigWithoutReties)(config.fragLoadPolicy.default);
      var loaderConfig = {
        loadPolicy: loadPolicy,
        timeout: loadPolicy.maxLoadTimeMs,
        maxRetry: 0,
...
var KeyLoader = /*#__PURE__*/function () {
  ...
  _proto.loadInternal = function loadInternal(frag, keySystemFormat) {
    ...
    if (!decryptdata) {
      ...
      return Promise.reject(this.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, error));
    }
    var uri = decryptdata.uri;
    if (!uri) {
      return Promise.reject(this.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, new Error("Invalid key URI: \"" + uri + "\"")));
    }
    var keyInfo = this.keyUriToKeyInfo[uri];
    ...
    switch (decryptdata.method) {
      ...
      case 'AES-128':
        return this.loadKeyHTTP(keyInfo, frag);
      default:
        return Promise.reject(this.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, new Error("Key supplied with unsupported METHOD: \"" + decryptdata.method + "\"")));
    }
  };
      ...
      var loaderCallbacks = {
        onSuccess: function onSuccess(response, stats, context, networkDetails) {
          ...
          if (!frag.decryptdata || keyInfo !== _this3.keyUriToKeyInfo[uri]) {
            return reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, new Error('after key load, decryptdata unset or changed'), networkDetails));
          }
        ...
        onError: function onError(response, context, networkDetails, stats) {
          ...
          reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, new Error("HTTP Error " + response.code + " loading key " + response.text), networkDetails, _objectSpread({
            url: loaderContext.url,
            data: undefined
          }, response)));
        },
        onTimeout: function onTimeout(stats, context, networkDetails) {
          _this3.resetLoader(context);
          reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_TIMEOUT, new Error('key loading timed out'), networkDetails));
        },
        onAbort: function onAbort(stats, context, networkDetails) {
          _this3.resetLoader(context);
          reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.INTERNAL_ABORTED, new Error('key loading aborted'), networkDetails));
        }
      };
...
/* harmony import */ var _utils_variable_substitution__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ../utils/variable-substitution */ "./src/utils/variable-substitution.ts");
/* harmony import */ var _utils_codecs__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../utils/codecs */ "./src/utils/codecs.ts");
...
var M3U8Parser = /*#__PURE__*/function () {
  ...
  M3U8Parser.parseMasterPlaylist = function parseMasterPlaylist(string, baseurl) {
    var hasVariableRefs =  false ? 0 : false;
    var parsed = {
      ...
      if (result[1]) {
        ...
        switch (tag) {
          case 'SESSION-DATA':
            {
              // #EXT-X-SESSION-DATA
              var sessionAttrs = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_6__.AttrList(attributes);
              if (false) {}
          ...
          case 'SESSION-KEY':
            {
              ...
              if (sessionKey.encrypted && sessionKey.isSupported()) {
                ...
                _utils_logger__WEBPACK_IMPORTED_MODULE_7__.logger.warn("[Keys] Ignoring invalid EXT-X-SESSION-KEY tag: \"" + attributes + "\"");
              }
              break;
            }
          case 'DEFINE':
            {
              // #EXT-X-DEFINE
              if (false) { var variableAttributes; }
              break;
            }
          case 'CONTENT-STEERING':
            {
              // #EXT-X-CONTENT-STEERING
              var contentSteeringAttributes = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_6__.AttrList(attributes);
              if (false) {}
  ...
  M3U8Parser.parseLevelPlaylist = function parseLevelPlaylist(string, baseurl, id, type, levelUrlId, multivariantVariableList) {
    ...
    level.m3u8 = string;
    level.hasVariableRefs =  false ? 0 : false;
          ...
          case 'PART':
            {
              ...
              var partAttrs = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_6__.AttrList(value1);
              if (false) {}
              var part = new _fragment__WEBPACK_IMPORTED_MODULE_3__.Part(partAttrs, frag, baseurl, index, previousFragmentPart);
              partList.push(part);
...
function parseKey(keyTagAttributes, baseurl, parsed) {
  ...
  var keyAttrs = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_6__.AttrList(keyTagAttributes);
  if (false) {}
...
function parseStartTimeOffset(startAttributes) {
  var startAttrs = new _utils_attr_list__WEBPACK_IMPORTED_MODULE_6__.AttrList(startAttributes);
...
function setCodecs(codecs, level) {
  ['video', 'audio', 'text'].forEach(function (type) {
    var filtered = codecs.filter(function (codec) {
      return (0,_utils_codecs__WEBPACK_IMPORTED_MODULE_9__.isCodecType)(codec, type);
    });
    if (filtered.length) {
...
var PlaylistLoader = /*#__PURE__*/function () {
  ...
  _proto.load = function load(context) {
    ...
    var loadPolicy;
    if (context.type === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.MANIFEST) {
    ...
    var loaderCallbacks = {
      onSuccess: function onSuccess(response, stats, context, networkDetails) {
        ...
        if (_m3u8_parser__WEBPACK_IMPORTED_MODULE_4__["default"].isMediaPlaylist(string)) {
  ...
  _proto.handleMasterPlaylist = function handleMasterPlaylist(response, stats, context, networkDetails) {
    ...
    var parsedResult = _m3u8_parser__WEBPACK_IMPORTED_MODULE_4__["default"].parseMasterPlaylist(string, url);
    ...
    var _M3U8Parser$parseMast = _m3u8_parser__WEBPACK_IMPORTED_MODULE_4__["default"].parseMasterPlaylistMedia(string, url, parsedResult),
  ...
  _proto.handleTrackOrLevelPlaylist = function handleTrackOrLevelPlaylist(response, stats, context, networkDetails, loader) {
    ...
    var levelDetails = _m3u8_parser__WEBPACK_IMPORTED_MODULE_4__["default"].parseLevelPlaylist(response.data, url, levelId, levelType, levelUrlId, this.variableList);
  ...
  _proto.handleNetworkError = function handleNetworkError(context, networkDetails, timeout, response, stats) {
    ...
    if (context.type === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.LEVEL) {
      message += ": " + context.level + " id: " + context.id;
    } else if (context.type === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.AUDIO_TRACK || context.type === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.SUBTITLE_TRACK) {
      message += " id: " + context.id + " group-id: \"" + context.groupId + "\"";
    }
    var error = new Error(message);
    _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.warn("[playlist-loader]: " + message);
    ...
    if (response) {
      ...
      errorData.response = _objectSpread({
        url: url,
        data: undefined
      }, response);
    }
  ...
  _proto.handlePlaylistLoaded = function handlePlaylistLoaded(levelDetails, response, stats, context, networkDetails, loader) {
    ...
    var levelIndex = typeof context.level === 'number' && parent === _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistLevelType.MAIN ? level : undefined;
    ...
    if (error) {
      hls.trigger(_events__WEBPACK_IMPORTED_MODULE_1__.Events.ERROR, {
        type: _errors__WEBPACK_IMPORTED_MODULE_2__.ErrorTypes.NETWORK_ERROR,
        details: _errors__WEBPACK_IMPORTED_MODULE_2__.ErrorDetails.LEVEL_PARSING_ERROR,
        fatal: false,
        url: url,
        error: error,
        reason: error.message,
    ...
    switch (type) {
      case _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.MANIFEST:
      case _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.LEVEL:
        hls.trigger(_events__WEBPACK_IMPORTED_MODULE_1__.Events.LEVEL_LOADED, {
          details: levelDetails,
          level: levelIndex || 0,
          id: id || 0,
          stats: stats,
        ...
        break;
      case _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.AUDIO_TRACK:
        hls.trigger(_events__WEBPACK_IMPORTED_MODULE_1__.Events.AUDIO_TRACK_LOADED, {
          details: levelDetails,
          id: id || 0,
          groupId: groupId || '',
          stats: stats,
        ...
        break;
      case _types_loader__WEBPACK_IMPORTED_MODULE_5__.PlaylistContextType.SUBTITLE_TRACK:
        hls.trigger(_events__WEBPACK_IMPORTED_MODULE_1__.Events.SUBTITLE_TRACK_LOADED, {
          details: levelDetails,
          id: id || 0,
          groupId: groupId || '',
          stats: stats,
...
var MP4Remuxer = /*#__PURE__*/function () {
  ...
  _proto.remuxVideo = function remuxVideo(track, timeOffset, contiguous, audioTrackLength) {
    ...
    var moof = _mp4_generator__WEBPACK_IMPORTED_MODULE_1__["default"].moof(track.sequenceNumber++, firstDTS, _extends({}, track, {
  ...
  _proto.remuxAudio = function remuxAudio(track, timeOffset, contiguous, accurateTimeOffset, videoTimeOffset) {
      ...
      for (var i = 0, nextPts = nextAudioPts; i < inputSamples.length; i++) {
        ...
        // 4: remuxing with video (videoTimeOffset !== undefined)
          ...
          for (var j = 0; j < missing; j++) {
            ...
            var fillFrame = _aac_helper__WEBPACK_IMPORTED_MODULE_0__["default"].getSilentFrame(track.manifestCodec || track.codec, track.channelCount);
            if (!fillFrame) {
    ...
    var moof = rawMPEG ? new Uint8Array(0) : _mp4_generator__WEBPACK_IMPORTED_MODULE_1__["default"].moof(track.sequenceNumber++, firstPTS / scaleFactor, _extends({}, track, {
      samples: outputSamples
    }));
  ...
  _proto.remuxEmptyAudio = function remuxEmptyAudio(track, timeOffset, contiguous, videoData) {
    ...
    // silent frame
    var silentFrame = _aac_helper__WEBPACK_IMPORTED_MODULE_0__["default"].getSilentFrame(track.manifestCodec || track.codec, track.channelCount);
...
var PassThroughRemuxer = /*#__PURE__*/function () {
  ...
  _proto.remux = function remux(audioTrack, videoTrack, id3Track, textTrack, timeOffset, accurateTimeOffset) {
    ...
    if (!((_initData = initData) !== null && _initData !== void 0 && _initData.length)) {
      this.generateInitSegment(data);
      initData = this.initData;
    }
    if (!((_initData2 = initData) !== null && _initData2 !== void 0 && _initData2.length)) {
    ...
    if (isInvalidInitPts(initPTS, startDTS, timeOffset) || initSegment.timescale !== initPTS.timescale && accurateTimeOffset) {
    ...
    var startTime = audioTrack ? startDTS - initPTS.baseTime / initPTS.timescale : lastEndTime;
    var endTime = startTime + duration;
    (0,_utils_mp4_tools__WEBPACK_IMPORTED_MODULE_2__.offsetStartDTS)(initData, data, initPTS.baseTime / initPTS.timescale);
    if (duration > 0) {
    ...
    result.id3 = (0,_mp4_remuxer__WEBPACK_IMPORTED_MODULE_1__.flushTextTrackMetadataCueSamples)(id3Track, timeOffset, initPTS, initPTS);
    if (textTrack.samples.length) {
      result.text = (0,_mp4_remuxer__WEBPACK_IMPORTED_MODULE_1__.flushTextTrackUserdataCueSamples)(textTrack, timeOffset, initPTS);
    }
    return result;
  };
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "getLoaderConfigWithoutReties": () => (/* binding */ getLoaderConfigWithoutReties),
/* harmony export */   "getRetryConfig": () => (/* binding */ getRetryConfig),
/* harmony export */   "getRetryDelay": () => (/* binding */ getRetryDelay),
/* harmony export */   "isTimeoutError": () => (/* binding */ isTimeoutError),
/* harmony export */   "retryForHttpStatus": () => (/* binding */ retryForHttpStatus),
...
function isTimeoutError(error) {
  switch (error.details) {
    case _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.FRAG_LOAD_TIMEOUT:
    case _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_TIMEOUT:
    case _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.LEVEL_LOAD_TIMEOUT:
    case _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.MANIFEST_LOAD_TIMEOUT:
      return true;
  }
  return false;
}
...
function getLoaderConfigWithoutReties(loderConfig) {
  return _objectSpread(_objectSpread({}, loderConfig), {
    errorRetry: null,
    timeoutRetry: null
  });
}
...
var EwmaBandWidthEstimator = /*#__PURE__*/function () {
  function EwmaBandWidthEstimator(slow, fast, defaultEstimate, defaultTTFB) {
    ...
    this.ttfb_ = new _utils_ewma__WEBPACK_IMPORTED_MODULE_0__["default"](slow);
  }
  ...
  _proto.update = function update(slow, fast) {
    ...
    if (ttfb_.halfLife !== slow) {
      this.ttfb_ = new _utils_ewma__WEBPACK_IMPORTED_MODULE_0__["default"](slow, ttfb_.getEstimate(), ttfb_.getTotalWeight());
    }
  };
...
function getContentLength(headers) {
  ...
  if (contentRange) {
    ...
    if ((0,_home_runner_work_hls_js_hls_js_src_polyfills_number__WEBPACK_IMPORTED_MODULE_0__.isFiniteNumber)(byteRangeLength)) {
...
function parseSegmentIndex(sidx) {
  ...
  for (var i = 0; i < referencesCount; i++) {
    ...
    if (referenceType === 1) {
      _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.warn('SIDX has hierarchical references (not supported)');
      return null;
    }
...
function parseEmsg(data) {
  ...
  if (version === 0) {
    ...
    if (!Number.isSafeInteger(presentationTime)) {
      ...
      _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.warn('Presentation time exceeds safe integer limit and wrapped to max safe integer in parsing emsg box');
    }
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "addVariableDefinition": () => (/* binding */ addVariableDefinition),
/* harmony export */   "hasVariableReferences": () => (/* binding */ hasVariableReferences),
/* harmony export */   "importVariableDefinition": () => (/* binding */ importVariableDefinition),
/* harmony export */   "substituteVariables": () => (/* binding */ substituteVariables),
/* harmony export */   "substituteVariablesInAttributes": () => (/* binding */ substituteVariablesInAttributes)
/* harmony export */ });
...
function substituteVariables(parsed, value) {
  if (parsed.variableList !== null || parsed.hasVariableRefs) {
    ...
    return value.replace(VARIABLE_REPLACEMENT_REGEX, function (variableReference) {
      ...
      var variableValue = variableList === null || variableList === void 0 ? void 0 : variableList[variableName];
...
function addVariableDefinition(parsed, attr) {
  var variableList = parsed.variableList;
  if (!variableList) {
...
/* harmony import */ var _utils_logger__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../utils/logger */ "./src/utils/logger.ts");
/* harmony import */ var _loader_load_stats__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../loader/load-stats */ "./src/loader/load-stats.ts");
/* harmony import */ var _error_helper__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./error-helper */ "./src/utils/error-helper.ts");
...
var XhrLoader = /*#__PURE__*/function () {
  ...
  _proto.loadInternal = function loadInternal() {
    ...
    self.clearTimeout(this.requestTimeout);
    config.timeout = config.loadPolicy.maxTimeToFirstByteMs;
    this.requestTimeout = self.setTimeout(this.loadtimeout.bind(this), config.loadPolicy.maxTimeToFirstByteMs);
    xhr.send();
  };
    ...
    if (readyState >= 2) {
      ...
      if (readyState === 4) {
        ...
        if (_status >= 200 && _status < 300 && (useResponse && xhr.response || xhr.responseText !== null)) {
          ...
          if ((0,_error_helper__WEBPACK_IMPORTED_MODULE_2__.shouldRetry)(retryConfig, retryCount, false, _status)) {
            this.retry(retryConfig);
          } else {
            _utils_logger__WEBPACK_IMPORTED_MODULE_0__.logger.error(_status + " while loading " + context.url);
            this.callbacks.onError({
              code: _status,
              text: xhr.statusText
            }, context, xhr, stats);
  ...
  _proto.loadtimeout = function loadtimeout() {
    var _this$config;
    var retryConfig = (_this$config = this.config) === null || _this$config === void 0 ? void 0 : _this$config.loadPolicy.timeoutRetry;
    var retryCount = this.stats.retry;
    if ((0,_error_helper__WEBPACK_IMPORTED_MODULE_2__.shouldRetry)(retryConfig, retryCount, true)) {
      this.retry(retryConfig);
    } else {
  ...
  _proto.retry = function retry(retryConfig) {
    var context = this.context,
      stats = this.stats;
    this.retryDelay = (0,_error_helper__WEBPACK_IMPORTED_MODULE_2__.getRetryDelay)(retryConfig, stats.retry);
    stats.retry++;
    _utils_logger__WEBPACK_IMPORTED_MODULE_0__.logger.warn((status ? 'HTTP Status ' + status : 'Timeout') + " while loading " + context.url + ", retrying " + stats.retry + "/" + retryConfig.maxNumRetry + " in " + this.retryDelay + "ms");
