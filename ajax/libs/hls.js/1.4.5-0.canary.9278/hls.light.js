...
(function (global, factory) {
  ...
  function enableLogs(debugConfig, id) {
    ...
    if (self.console && debugConfig === true || typeof debugConfig === 'object') {
      ...
      try {
        exportedLogger.log("Debug logs enabled for \"" + id + "\" in hls.js version " + "1.4.5-0.canary.9278");
      } catch (e) {
      ...
      case 'av01':
        {
          ...
          codec += '.' + _profile2 + '.' + addLeadingZero(_level2) + _tierFlag + '.' + addLeadingZero(_bitDepth) + '.' + monochrome + '.' + chromaSubsamplingX + chromaSubsamplingY + chromaSamplePosition;
          break;
        }
    }
    return {
  ...
  function areCodecsMediaSourceSupported(codecs, type) {
    return !codecs.split(',').some(function (codec) {
      return !isCodecMediaSourceSupported(codec, type);
    });
  }
  function isCodecMediaSourceSupported(codec, type) {
  ...
  function getCodecCompatibleNameLower(lowerCaseCodec) {
  ...
  function getCodecCompatibleName(codec) {
    return codec.replace(AUDIO_CODEC_REGEXP, function (m) {
      return getCodecCompatibleNameLower(m.toLowerCase());
    });
  }
  ...
  var LevelController = /*#__PURE__*/function (_BasePlaylistControll) {
    ...
    _proto.filterAndSortMediaOptions = function filterAndSortMediaOptions(unfilteredLevels, data) {
      ...
      var levels = unfilteredLevels.filter(function (_ref) {
        ...
        return !(unknownCodecs != null && unknownCodecs.length) && (!audioCodec || areCodecsMediaSourceSupported(audioCodec, 'audio')) && (!videoCodec || areCodecsMediaSourceSupported(videoCodec, 'video'));
      });
        ...
        Promise.resolve().then(function () {
          if (_this2.hls) {
            if (unfilteredLevels.length) {
              _this2.warn("One or more CODECS in variant not supported: \"" + unfilteredLevels[0].attrs.CODECS + "\"");
            }
      ...
      if (data.audioTracks) {
        audioTracks = data.audioTracks.filter(function (track) {
          return !track.audioCodec || areCodecsMediaSourceSupported(track.audioCodec, 'audio');
        });
  ...
  var TSDemuxer = /*#__PURE__*/function () {
    ...
    _proto.parseAC3PES = function parseAC3PES(pes) {
      var data = pes.data;
      var pts = pes.pts;
      var length = data.length;
      var frameIndex = 0;
      var offset = 0;
      var parsed;
      while (offset < length && (parsed = this.parseAC3(data, offset, length, frameIndex++, pts)) > 0) {
        offset += parsed;
      }
    };
    _proto.onAC3Frame = function onAC3Frame(data, sampleRate, channelCount, config, frameIndex, pts) {
    ...
    _proto.parseAC3 = function parseAC3(data, start, end, frameIndex, pts) {
      if (start + 8 > end) {
      ...
      this.onAC3Frame(data.subarray(start, start + frameLength), sampleRate, channelCount, config, frameIndex, pts);
      return frameLength;
    };
  ...
  var TransmuxerInterface = /*#__PURE__*/function () {
    function TransmuxerInterface(hls, id, onTransmuxComplete, onFlush) {
      ...
      var typeSupported = {
        ...
        ac3: MediaSource$1.isTypeSupported('audio/mp4; codecs="ac-3"')
      };
    ...
    function BufferController(hls) {
      ...
      this._onMediaEmptied = function () {
        ...
        if (media && media.src !== _objectUrl) {
          _this.error("Media element src was set while attaching MediaSource (" + _objectUrl + " > " + media.src + ")");
        }
      };
      this.hls = hls;
    ...
    _proto.onBufferReset = function onBufferReset() {
      ...
      this.getSourceBufferTypes().forEach(function (type) {
        ...
        try {
          ...
          _this2.warn("Failed to reset the " + type + " buffer", err);
        }
      });
      this._initSourceBuffer();
    };
      ...
      Object.keys(data).forEach(function (trackName) {
        if (sourceBufferCount) {
          ...
          if (track && typeof track.buffer.changeType === 'function') {
            ...
            var currentCodec = (track.levelCodec || track.codec).replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
            var nextCodec = (levelCodec || codec).replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
            if (currentCodec !== nextCodec) {
              var trackCodec = levelCodec || codec;
              if (trackName.slice(0, 5) === 'audio') {
                trackCodec = getCodecCompatibleName(trackCodec);
              }
              var mimeType = container + ";codecs=" + trackCodec;
              _this3.appendChangeType(trackName, mimeType);
              _this3.log("switching codec " + currentCodec + " to " + nextCodec);
          ...
          if (err.code === DOMException.QUOTA_EXCEEDED_ERR) {
            ...
            */
            if (_this5.appendError > hls.config.appendErrorMaxRetry) {
              _this5.error("Failed " + hls.config.appendErrorMaxRetry + " times to append segment in sourceBuffer");
              event.fatal = true;
            }
          }
