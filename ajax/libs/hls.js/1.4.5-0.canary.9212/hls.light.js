...
(function (global, factory) {
  ...
  function enableLogs(debugConfig, id) {
    ...
    if (self.console && debugConfig === true || typeof debugConfig === 'object') {
      ...
      try {
        exportedLogger.log("Debug logs enabled for \"" + id + "\" in hls.js version " + "1.4.5-0.canary.9212");
      } catch (e) {
            ...
            if (type) {
              ...
              if (stsd) {
                ...
                // Handle H265
                  ...
                  var hvcC = void 0;
                  var codecBox = findBox(stsd.subarray(8), [codec])[0];
                  if (codecBox) {
                    var _parseVisualSampleEnt = parseVisualSampleEntry(codecBox),
                      end = _parseVisualSampleEnt.end;
  ...
  function getDuration(data, initData) {
    ...
    if (videoDuration === 0 && audioDuration === 0) {
      ...
      for (var _i4 = 0; _i4 < sidxs.length; _i4++) {
        var sidx = parseSegmentIndex(sidxs[_i4]);
  ...
  function mimeTypeBuilderHEVC(codecName, codecDetails) {
    var generalProfileSpaceMap = ['', 'A', 'B', 'C'];
    var codecMimeType = codecName;
    if (codecDetails) {
      codecMimeType += '.';
      ...
      var constraintString = '';
      var lastByteIndex = codecDetails.generalConstraintIndicator.findLastIndex(function (x) {
        return x !== 0;
      });
      if (lastByteIndex !== -1) {
        constraintString = '.' + codecDetails.generalConstraintIndicator.slice(0, lastByteIndex + 1).map(function (x) {
