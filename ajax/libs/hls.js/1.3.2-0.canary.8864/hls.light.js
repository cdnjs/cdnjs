...
function enableStreamingMode(config) {
  ...
  if (currentLoader !== _utils_fetch_loader__WEBPACK_IMPORTED_MODULE_6__["default"] && currentLoader !== _utils_xhr_loader__WEBPACK_IMPORTED_MODULE_5__["default"]) {
    ...
    _utils_logger__WEBPACK_IMPORTED_MODULE_7__.logger.log('[config]: Custom loader detected, cannot enable progressive streaming');
    config.progressive = false;
  } else {
    var canStreamProgressively = (0,_utils_fetch_loader__WEBPACK_IMPORTED_MODULE_6__.fetchSupported)();
    if (canStreamProgressively) {
      config.loader = _utils_fetch_loader__WEBPACK_IMPORTED_MODULE_6__["default"];
      config.progressive = true;
      config.enableSoftwareAES = true;
      _utils_logger__WEBPACK_IMPORTED_MODULE_7__.logger.log('[config]: Progressive streaming enabled, using FetchLoader');
    }
  }
}
...
var AbrController = /*#__PURE__*/function () {
  ...
  _proto.ignoreFragment = function ignoreFragment(frag) {
    ...
    return frag.type !== _types_loader__WEBPACK_IMPORTED_MODULE_4__.PlaylistLevelType.MAIN || frag.sn === 'initSegment';
  };
  ...
  _proto.getNextABRAutoLevel = function getNextABRAutoLevel() {
    ...
    _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.trace("[abr] " + (bufferStarvationDelay ? 'rebuffering expected' : 'buffer is empty') + ", finding optimal quality level");
    ...
    if (!bufferStarvationDelay) {
      ...
      if (bitrateTestDelay) {
        ...
        _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.trace("[abr] bitrate test took " + Math.round(1000 * bitrateTestDelay) + "ms, set first fragment max fetchDuration to " + Math.round(1000 * maxStarvationDelay) + " ms");
  ...
  _proto.findBestLevel = function findBestLevel(currentBw, minAutoLevel, maxAutoLevel, maxFetchDuration, bwFactor, bwUpFactor) {
    ...
    for (var i = maxAutoLevel; i >= minAutoLevel; i--) {
      ...
      _utils_logger__WEBPACK_IMPORTED_MODULE_5__.logger.trace("[abr] level:" + i + " adjustedbw-bitrate:" + Math.round(adjustedbw - bitrate) + " avgDuration:" + avgDuration.toFixed(1) + " maxFetchDuration:" + maxFetchDuration.toFixed(1) + " fetchDuration:" + fetchDuration.toFixed(1));
    ...
    if (this.config.lowLatencyMode) {
      ...
      if (partList && progressCallback) {
        ...
        if (partIndex > -1) {
          ...
          return this.doFragPartsLoad(frag, part, level, progressCallback).catch(function (error) {
            return _this4.handleFragLoadError(error);
          });
...
var LevelController = /*#__PURE__*/function (_BasePlaylistControll) {
  ...
  _proto.onError = function onError(event, data) {
    ...
    switch (data.details) {
      ...
      case _errors__WEBPACK_IMPORTED_MODULE_2__.ErrorDetails.LEVEL_PARSING_ERROR:
...
var LoadError = /*#__PURE__*/function (_Error) {
  ...
  function LoadError(data, message) {
    var _this3;
...
var KeyLoader = /*#__PURE__*/function () {
  ...
  _proto.createKeyLoadError = function createKeyLoadError(frag, details, message, networkDetails) {
    if (details === void 0) {
  ...
  _proto.loadInternal = function loadInternal(frag, keySystemFormat) {
    ...
    if (!decryptdata) {
      ...
      return Promise.reject(this.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, errorMessage));
    }
    var uri = decryptdata.uri;
    if (!uri) {
      return Promise.reject(this.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, "Invalid key URI: \"" + uri + "\""));
    }
    var keyInfo = this.keyUriToKeyInfo[uri];
    ...
    switch (decryptdata.method) {
      ...
      case 'AES-128':
        return this.loadKeyHTTP(keyInfo, frag);
      default:
        return Promise.reject(this.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, "Key supplied with unsupported METHOD: \"" + decryptdata.method + "\""));
    }
  };
      ...
      var loaderCallbacks = {
        onSuccess: function onSuccess(response, stats, context, networkDetails) {
          ...
          if (!frag.decryptdata || keyInfo !== _this3.keyUriToKeyInfo[uri]) {
            return reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, 'after key load, decryptdata unset or changed', networkDetails));
          }
        ...
        onError: function onError(error, context, networkDetails) {
          ...
          reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_ERROR, 'error loading key', networkDetails));
        },
        onTimeout: function onTimeout(stats, context, networkDetails) {
          _this3.resetLoader(context);
          reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.KEY_LOAD_TIMEOUT, 'key loading timed out', networkDetails));
        },
        onAbort: function onAbort(stats, context, networkDetails) {
          _this3.resetLoader(context);
          reject(_this3.createKeyLoadError(frag, _errors__WEBPACK_IMPORTED_MODULE_0__.ErrorDetails.INTERNAL_ABORTED, 'key loading aborted', networkDetails));
        }
      };
...
var M3U8Parser = /*#__PURE__*/function () {
  ...
  M3U8Parser.parseMasterPlaylist = function parseMasterPlaylist(string, baseurl) {
    ...
    if (parsed.levels.length === 0) {
      parsed.playlistParsingError = new Error("no level found in manifest: " + (stripUnknownCodecLevels ? 'unknown codecs' : 'empty'));
    }
    return parsed;
  };
      ...
      if (duration) {
        ...
        if ((0,_home_runner_work_hls_js_hls_js_src_polyfills_number__WEBPACK_IMPORTED_MODULE_0__.isFiniteNumber)(frag.duration)) {
          ...
          if (levelkeys) {
            ...
            if (frag.levelkeys && Object.keys(frag.levelkeys).some(function (format) {
              return frag.levelkeys[format].isCommonEncryption;
            }) && (!encryptedFragments.length || encryptedFragments[encryptedFragments.length - 1].levelkeys !== levelkeys)) {
...
var PlaylistLoader = /*#__PURE__*/function () {
  ...
  _proto.loadsuccess = function loadsuccess(response, stats, context, networkDetails) {
    ...
    if (string.indexOf('#EXTM3U') !== 0) {
      this.handleManifestParsingError(response, context, new Error('no EXTM3U delimiter'), networkDetails);
      return;
    }
  ...
  _proto.handleMasterPlaylist = function handleMasterPlaylist(response, stats, context, networkDetails) {
    ...
    if (parsedResult.playlistParsingError) {
      this.handleManifestParsingError(response, context, parsedResult.playlistParsingError, networkDetails);
      return;
    }
  ...
  _proto.handleManifestParsingError = function handleManifestParsingError(response, context, error, networkDetails) {
    this.hls.trigger(_events__WEBPACK_IMPORTED_MODULE_1__.Events.ERROR, {
      type: _errors__WEBPACK_IMPORTED_MODULE_2__.ErrorTypes.NETWORK_ERROR,
  ...
  _proto.handlePlaylistLoaded = function handlePlaylistLoaded(response, stats, context, networkDetails) {
    ...
    if (!(levelDetails !== null && levelDetails !== void 0 && levelDetails.targetduration)) {
      this.handleManifestParsingError(response, context, new Error('invalid target duration'), networkDetails);
      return;
    }
    if (!loader) {
      return;
