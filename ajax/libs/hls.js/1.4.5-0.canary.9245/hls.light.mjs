...
function areCodecsMediaSourceSupported(codecs, type) {
  return !codecs.split(',').some(codec => !isCodecMediaSourceSupported(codec, type));
}
...
function getCodecCompatibleName(codec) {
  return codec.replace(AUDIO_CODEC_REGEXP, m => getCodecCompatibleNameLower(m.toLowerCase()));
}
...
class LevelController extends BasePlaylistController {
  ...
  filterAndSortMediaOptions(unfilteredLevels, data) {
      ...
      Promise.resolve().then(() => {
        if (this.hls) {
          if (unfilteredLevels.length) {
            this.warn(`One or more CODECS in variant not supported: "${unfilteredLevels[0].attrs.CODECS}"`);
          }
    ...
    if (data.audioTracks) {
      audioTracks = data.audioTracks.filter(track => !track.audioCodec || areCodecsMediaSourceSupported(track.audioCodec, 'audio'));
...
class TSDemuxer {
  ...
  onAC3Frame(data, sampleRate, channelCount, config, frameIndex, pts) {
  ...
  parseAC3(data, start, end, frameIndex, pts) {
    if (start + 8 > end) {
  ...
  constructor(hls) {
    ...
    this._onMediaEmptied = () => {
      ...
      if (media && media.src !== _objectUrl) {
        this.error(`Media element src was set while attaching MediaSource (${_objectUrl} > ${media.src})`);
      }
    };
    this.hls = hls;
    ...
    Object.keys(data).forEach(trackName => {
      if (sourceBufferCount) {
        ...
        if (track && typeof track.buffer.changeType === 'function') {
          ...
          const currentCodec = (track.levelCodec || track.codec).replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
          const nextCodec = (levelCodec || codec).replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
        ...
        if (err.code === DOMException.QUOTA_EXCEEDED_ERR) {
          ...
          */
          if (this.appendError > hls.config.appendErrorMaxRetry) {
            this.error(`Failed ${hls.config.appendErrorMaxRetry} times to append segment in sourceBuffer`);
            event.fatal = true;
          }
        }
