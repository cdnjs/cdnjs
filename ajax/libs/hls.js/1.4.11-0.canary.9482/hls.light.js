...
(function (global, factory) {
  ...
  function enableLogs(debugConfig, id) {
    ...
    if (self.console && debugConfig === true || typeof debugConfig === 'object') {
      ...
      try {
        exportedLogger.log("Debug logs enabled for \"" + id + "\" in hls.js version " + "1.4.11-0.canary.9482");
      } catch (e) {
  ...
  var BufferController = /*#__PURE__*/function () {
    ...
    _proto.createSourceBuffers = function createSourceBuffers(tracks) {
      ...
      var _loop = function _loop(trackName) {
        if (!sourceBuffer[trackName]) {
          ...
          if (codec) {
            if (trackName.slice(0, 5) === 'audio') {
              codec = getCodecCompatibleName(codec, _this10.hls.config.preferManagedMediaSource);
            }
          }
          var mimeType = track.container + ";codecs=" + codec;
          _this10.log("creating sourceBuffer(" + mimeType + ")");
          try {
            var sb = sourceBuffer[trackName] = mediaSource.addSourceBuffer(mimeType);
            var sbName = trackName;
            _this10.addBufferListener(sbName, 'updatestart', _this10._onSBUpdateStart);
            _this10.addBufferListener(sbName, 'updateend', _this10._onSBUpdateEnd);
            _this10.addBufferListener(sbName, 'error', _this10._onSBUpdateError);
            // ManagedSourceBuffer bufferedchange event
            _this10.addBufferListener(sbName, 'bufferedchange', function (event) {
              _this10.hls.trigger(Events.BUFFER_FLUSHED, {
                type: trackName
              });
            });
            _this10.tracks[trackName] = {
              buffer: sb,
              codec: codec,
            ...
            _this10.error("error while trying to add sourceBuffer: " + err.message);
            _this10.hls.trigger(Events.ERROR, {
              type: ErrorTypes.MEDIA_ERROR,
    ...
    _proto.blockBuffers = function blockBuffers(onUnblocked, buffers) {
      ...
      Promise.all(blockingOperations).then(function () {
        ...
        onUnblocked();
        buffers.forEach(function (type) {
          var sb = _this11.sourceBuffer[type];
