function createAuth$4(appId, apiKey, authMode = 'WithinHeaders') {
  const credentials = {
...
function createBrowserLocalStorageCache$5(options) {
  let storage;
...
function createFallbackableCache$5(options) {
  const caches = [...options.caches];
  const current = caches.shift();
  if (current === undefined) {
    return createNullCache$5();
  }
  return {
    get(key, defaultValue, events = {
      miss: () => Promise.resolve()
      ...
      return current.get(key, defaultValue, events).catch(() => {
        return createFallbackableCache$5({
          caches
        }).get(key, defaultValue, events);
      });
    },
...
function createMemoryCache$5(options = {
  serializable: true
}) {
  let cache = {};
  return {
...
const EXPIRATION_DELAY$4 = 2 * 60 * 1000;
function createStatefulHost$4(host, status = 'up') {
  const lastUpdate = Date.now();
  function isUp() {
    return status === 'up' || Date.now() - lastUpdate > EXPIRATION_DELAY$4;
  }
  function isTimedOut() {
    return status === 'timed out' && Date.now() - lastUpdate <= EXPIRATION_DELAY$4;
  }
  return {
    ...host,
    status,
...
let AlgoliaError$4 = class AlgoliaError extends Error {
  constructor(message, name) {
    super(message);
    _defineProperty$4(this, "name", 'AlgoliaError');
    if (name) {
      this.name = name;
    }
  }
};
let ErrorWithStackTrace$4 = class ErrorWithStackTrace extends AlgoliaError$4 {
  constructor(message, stackTrace, name) {
    super(message, name);
    // the array and object should be frozen to reflect the stackTrace at the time of the error
    _defineProperty$4(this, "stackTrace", void 0);
    this.stackTrace = stackTrace;
  }
};
let RetryError$4 = class RetryError extends ErrorWithStackTrace$4 {
...
let ApiError$4 = class ApiError extends ErrorWithStackTrace$4 {
  constructor(message, status, stackTrace, name = 'ApiError') {
    super(message, stackTrace, name);
    _defineProperty$4(this, "status", void 0);
    this.status = status;
  }
};
let DeserializationError$4 = class DeserializationError extends AlgoliaError$4 {
  constructor(message, response) {
    super(message, 'DeserializationError');
    _defineProperty$4(this, "response", void 0);
    this.response = response;
  }
};
// DetailedApiError is only used by the ingestion client to return more informative error, other clients will use ApiClient.
let DetailedApiError$4 = class DetailedApiError extends ApiError$4 {
  constructor(message, status, error, stackTrace) {
    super(message, status, stackTrace, 'DetailedApiError');
    _defineProperty$4(this, "error", void 0);
    this.error = error;
  }
};
function serializeUrl$4(host, path, queryParameters) {
  const queryParametersAsString = serializeQueryParameters$4(queryParameters);
...
function serializeQueryParameters$4(parameters) {
...
function serializeData$4(request, requestOptions) {
...
function serializeHeaders$4(baseHeaders, requestHeaders, requestOptionsHeaders) {
  const headers = {
...
function deserializeSuccess$4(response) {
  try {
    return JSON.parse(response.content);
  } catch (e) {
    throw new DeserializationError$4(e.message, response);
  }
}
function deserializeFailure$4({
  content,
  status
}, stackFrame) {
  try {
    ...
    if ('error' in parsed) {
      return new DetailedApiError$4(parsed.message, status, parsed.error, stackFrame);
    }
    return new ApiError$4(parsed.message, status, stackFrame);
  } catch {
    // ..
  }
  return new ApiError$4(content, status, stackFrame);
}

function isNetworkError$4({
  isTimedOut,
  status
}) {
  return !isTimedOut && ~~status === 0;
}
function isRetryable$4({
  isTimedOut,
  status
}) {
  return isTimedOut || isNetworkError$4({
    isTimedOut,
    status
  }) || ~~(status / 100) !== 2 && ~~(status / 100) !== 4;
}
function isSuccess$4({
  status
}) {
  return ~~(status / 100) === 2;
}

function stackTraceWithoutCredentials$4(stackTrace) {
  return stackTrace.map(stackFrame => stackFrameWithoutCredentials$4(stackFrame));
}
function stackFrameWithoutCredentials$4(stackFrame) {
...
function createTransporter$4({
  ...
  async function createRetryableOptions(compatibleHosts) {
    const statefulHosts = await Promise.all(compatibleHosts.map(compatibleHost => {
      return hostsCache.get(compatibleHost, () => {
        return Promise.resolve(createStatefulHost$4(compatibleHost));
      });
    }));
  ...
  async function retryableRequest(request, requestOptions, isRead = true) {
     ...
     */
    const data = serializeData$4(request, requestOptions);
    const headers = serializeHeaders$4(baseHeaders, request.headers, requestOptions.headers);
    ...
    const retry = async (retryableHosts, getTimeout) => {
      ...
      if (host === undefined) {
        throw new RetryError$4(stackTraceWithoutCredentials$4(stackTrace));
      }
      const timeout = {
        ...timeouts,
      ...
      const payload = {
        ...
        url: serializeUrl$4(host, request.path, queryParameters),
      ...
      if (isRetryable$4(response)) {
        ...
        console.log('Retryable failure', stackFrameWithoutCredentials$4(stackFrame));
        /**
         * We also store the state of the host in failure cases. If the host, is
         * down it will remain down for the next 2 minutes. In a timeout situation,
         * this host will be added end of the list of hosts on the next request.
         */
        await hostsCache.set(host, createStatefulHost$4(host, response.isTimedOut ? 'timed out' : 'down'));
        return retry(retryableHosts, getTimeout);
      }
      if (isSuccess$4(response)) {
        return deserializeSuccess$4(response);
      }
      pushToStackTrace(response);
      throw deserializeFailure$4(response, stackTrace);
    };
    /**
...
function createAlgoliaAgent$4(version) {
  const algoliaAgent = {
...
function getAlgoliaAgent$4({
  algoliaAgents,
  client,
  version
}) {
  const defaultAlgoliaAgent = createAlgoliaAgent$4(version).add({
    segment: client,
    version
  });
...
const DEFAULT_CONNECT_TIMEOUT_BROWSER$5 = 1000;
const DEFAULT_READ_TIMEOUT_BROWSER$5 = 2000;
const DEFAULT_WRITE_TIMEOUT_BROWSER$5 = 30000;

function createXhrRequester$5() {
...
const apiClientVersion$5 = '5.2.2';
...
function createAbtestingClient({ appId: appIdOption, apiKey: apiKeyOption, authMode, algoliaAgents, region: regionOption, ...options }) {
    const auth = createAuth$4(appIdOption, apiKeyOption, authMode);
    const transporter = createTransporter$4({
        hosts: getDefaultHosts$4(regionOption),
        ...options,
        algoliaAgent: getAlgoliaAgent$4({
            algoliaAgents,
            client: 'Abtesting',
...
function abtestingClient(appId, apiKey, region, options) {
    ...
    if (region && (typeof region !== 'string' || !REGIONS$2.includes(region))) {
    ...
    return createAbtestingClient({
        ...
        region,
        timeouts: {
            connect: DEFAULT_CONNECT_TIMEOUT_BROWSER$5,
            read: DEFAULT_READ_TIMEOUT_BROWSER$5,
            write: DEFAULT_WRITE_TIMEOUT_BROWSER$5,
        },
        requester: createXhrRequester$5(),
        algoliaAgents: [{ segment: 'Browser' }],
        authMode: 'WithinQueryParameters',
        responsesCache: createMemoryCache$5(),
        requestsCache: createMemoryCache$5({ serializable: false }),
        hostsCache: createFallbackableCache$5({
            caches: [createBrowserLocalStorageCache$5({ key: `${apiClientVersion$5}-${appId}` }), createMemoryCache$5()],
        }),
        ...options,
    });
}

function createAuth$3(appId, apiKey, authMode = 'WithinHeaders') {
  const credentials = {
    'x-algolia-api-key': apiKey,
    'x-algolia-application-id': appId
  };
...
function createBrowserLocalStorageCache$4(options) {
  let storage;
  // We've changed the namespace to avoid conflicts with v4, as this version is a huge breaking change
  const namespaceKey = `algolia-client-js-${options.key}`;
  function getStorage() {
...
function createFallbackableCache$4(options) {
  const caches = [...options.caches];
  const current = caches.shift();
  if (current === undefined) {
    return createNullCache$4();
  }
  return {
    get(key, defaultValue, events = {
      miss: () => Promise.resolve()
      ...
      return current.get(key, defaultValue, events).catch(() => {
        return createFallbackableCache$4({
          caches
        }).get(key, defaultValue, events);
      });
    },
...
function createMemoryCache$4(options = {
  serializable: true
}) {
  let cache = {};
  return {
...
// By default, API Clients at Algolia have expiration delay of 5 mins.
// In the JavaScript client, we have 2 mins.
const EXPIRATION_DELAY$3 = 2 * 60 * 1000;
function createStatefulHost$3(host, status = 'up') {
  const lastUpdate = Date.now();
  function isUp() {
    return status === 'up' || Date.now() - lastUpdate > EXPIRATION_DELAY$3;
  }
  function isTimedOut() {
    return status === 'timed out' && Date.now() - lastUpdate <= EXPIRATION_DELAY$3;
  }
  return {
    ...host,
    status,
...
let AlgoliaError$3 = class AlgoliaError extends Error {
  constructor(message, name) {
    super(message);
    _defineProperty$3(this, "name", 'AlgoliaError');
    if (name) {
      this.name = name;
    }
  }
};
let ErrorWithStackTrace$3 = class ErrorWithStackTrace extends AlgoliaError$3 {
  constructor(message, stackTrace, name) {
    super(message, name);
    // the array and object should be frozen to reflect the stackTrace at the time of the error
    _defineProperty$3(this, "stackTrace", void 0);
    this.stackTrace = stackTrace;
  }
};
let RetryError$3 = class RetryError extends ErrorWithStackTrace$3 {
  constructor(stackTrace) {
    super('Unreachable hosts - your application id may be incorrect. If the error persists, please reach out to the Algolia Support team: https://alg.li/support.', stackTrace, 'RetryError');
  }
};
let ApiError$3 = class ApiError extends ErrorWithStackTrace$3 {
  constructor(message, status, stackTrace, name = 'ApiError') {
    super(message, stackTrace, name);
    _defineProperty$3(this, "status", void 0);
    this.status = status;
  }
};
let DeserializationError$3 = class DeserializationError extends AlgoliaError$3 {
  constructor(message, response) {
    super(message, 'DeserializationError');
    _defineProperty$3(this, "response", void 0);
    this.response = response;
  }
};
// DetailedApiError is only used by the ingestion client to return more informative error, other clients will use ApiClient.
let DetailedApiError$3 = class DetailedApiError extends ApiError$3 {
  constructor(message, status, error, stackTrace) {
    super(message, status, stackTrace, 'DetailedApiError');
    _defineProperty$3(this, "error", void 0);
    this.error = error;
  }
};
function serializeUrl$3(host, path, queryParameters) {
  const queryParametersAsString = serializeQueryParameters$3(queryParameters);
  let url = `${host.protocol}://${host.url}${host.port ? `:${host.port}` : ''}/${path.charAt(0) === '/' ? path.substring(1) : path}`;
  if (queryParametersAsString.length) {
    url += `?${queryParametersAsString}`;
  }
...
function serializeQueryParameters$3(parameters) {
  return Object.keys(parameters).filter(key => parameters[key] !== undefined).sort().map(key => `${key}=${encodeURIComponent(Object.prototype.toString.call(parameters[key]) === '[object Array]' ? parameters[key].join(',') : parameters[key]).replaceAll('+', '%20')}`).join('&');
}
function serializeData$3(request, requestOptions) {
  if (request.method === 'GET' || request.data === undefined && requestOptions.data === undefined) {
    return undefined;
  }
  const data = Array.isArray(request.data) ? request.data : {
...
function serializeHeaders$3(baseHeaders, requestHeaders, requestOptionsHeaders) {
  const headers = {
    Accept: 'application/json',
    ...baseHeaders,
    ...requestHeaders,
...
function deserializeSuccess$3(response) {
  try {
    return JSON.parse(response.content);
  } catch (e) {
    throw new DeserializationError$3(e.message, response);
  }
}
function deserializeFailure$3({
  content,
  status
}, stackFrame) {
  try {
    const parsed = JSON.parse(content);
    if ('error' in parsed) {
      return new DetailedApiError$3(parsed.message, status, parsed.error, stackFrame);
    }
    return new ApiError$3(parsed.message, status, stackFrame);
  } catch {
    // ..
  }
  return new ApiError$3(content, status, stackFrame);
}

function isNetworkError$3({
  isTimedOut,
  status
}) {
  return !isTimedOut && ~~status === 0;
}
function isRetryable$3({
  isTimedOut,
  status
}) {
  return isTimedOut || isNetworkError$3({
    isTimedOut,
    status
  }) || ~~(status / 100) !== 2 && ~~(status / 100) !== 4;
}
function isSuccess$3({
  status
}) {
  return ~~(status / 100) === 2;
}

function stackTraceWithoutCredentials$3(stackTrace) {
  return stackTrace.map(stackFrame => stackFrameWithoutCredentials$3(stackFrame));
}
function stackFrameWithoutCredentials$3(stackFrame) {
  const modifiedHeaders = stackFrame.request.headers['x-algolia-api-key'] ? {
    'x-algolia-api-key': '*****'
  } : {};
  return {
...
function createTransporter$3({
  ...
  async function createRetryableOptions(compatibleHosts) {
    const statefulHosts = await Promise.all(compatibleHosts.map(compatibleHost => {
      return hostsCache.get(compatibleHost, () => {
        return Promise.resolve(createStatefulHost$3(compatibleHost));
      });
    }));
    const hostsUp = statefulHosts.filter(host => host.isUp());
    const hostsTimedOut = statefulHosts.filter(host => host.isTimedOut());
  ...
  async function retryableRequest(request, requestOptions, isRead = true) {
     ...
     * First we prepare the payload that do not depend from hosts.
     */
    const data = serializeData$3(request, requestOptions);
    const headers = serializeHeaders$3(baseHeaders, request.headers, requestOptions.headers);
    // On `GET`, the data is proxied to query parameters.
    const dataQueryParameters = request.method === 'GET' ? {
      ...request.data,
      ...requestOptions.data
    ...
    const retry = async (retryableHosts, getTimeout) => {
      ...
      const host = retryableHosts.pop();
      if (host === undefined) {
        throw new RetryError$3(stackTraceWithoutCredentials$3(stackTrace));
      }
      const timeout = {
        ...timeouts,
        ...requestOptions.timeouts
      ...
      const payload = {
        ...
        headers,
        method: request.method,
        url: serializeUrl$3(host, request.path, queryParameters),
        connectTimeout: getTimeout(timeoutsCount, timeout.connect),
        responseTimeout: getTimeout(timeoutsCount, isRead ? timeout.read : timeout.write)
      };
      /**
      ...
      if (isRetryable$3(response)) {
         ...
         */
        // eslint-disable-next-line no-console -- this will be fixed by exposing a `logger` to the transporter
        console.log('Retryable failure', stackFrameWithoutCredentials$3(stackFrame));
        /**
         * We also store the state of the host in failure cases. If the host, is
         * down it will remain down for the next 2 minutes. In a timeout situation,
         * this host will be added end of the list of hosts on the next request.
         */
        await hostsCache.set(host, createStatefulHost$3(host, response.isTimedOut ? 'timed out' : 'down'));
        return retry(retryableHosts, getTimeout);
      }
      if (isSuccess$3(response)) {
        return deserializeSuccess$3(response);
      }
      pushToStackTrace(response);
      throw deserializeFailure$3(response, stackTrace);
    };
    /**
     * Finally, for each retryable host perform request until we got a non
     * retryable response. Some notes here:
...
function createAlgoliaAgent$3(version) {
  const algoliaAgent = {
    value: `Algolia for JavaScript (${version})`,
    add(options) {
      const addedAlgoliaAgent = `; ${options.segment}${options.version !== undefined ? ` (${options.version})` : ''}`;
...
function getAlgoliaAgent$3({
  algoliaAgents,
  client,
  version
}) {
  const defaultAlgoliaAgent = createAlgoliaAgent$3(version).add({
    segment: client,
    version
  });
  algoliaAgents.forEach(algoliaAgent => defaultAlgoliaAgent.add(algoliaAgent));
...
const DEFAULT_CONNECT_TIMEOUT_BROWSER$4 = 1000;
const DEFAULT_READ_TIMEOUT_BROWSER$4 = 2000;
const DEFAULT_WRITE_TIMEOUT_BROWSER$4 = 30000;

function createXhrRequester$4() {
    function send(request) {
        return new Promise((resolve) => {
            const baseRequester = new XMLHttpRequest();
            baseRequester.open(request.method, request.url, true);
...
// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.
const apiClientVersion$4 = '5.2.2';
...
function createAnalyticsClient({ appId: appIdOption, apiKey: apiKeyOption, authMode, algoliaAgents, region: regionOption, ...options }) {
    const auth = createAuth$3(appIdOption, apiKeyOption, authMode);
    const transporter = createTransporter$3({
        hosts: getDefaultHosts$3(regionOption),
        ...options,
        algoliaAgent: getAlgoliaAgent$3({
            algoliaAgents,
            client: 'Analytics',
...
function analyticsClient(appId, apiKey, region, options) {
    ...
    if (region && (typeof region !== 'string' || !REGIONS$1.includes(region))) {
    ...
    return createAnalyticsClient({
        ...
        region,
        timeouts: {
            connect: DEFAULT_CONNECT_TIMEOUT_BROWSER$4,
            read: DEFAULT_READ_TIMEOUT_BROWSER$4,
            write: DEFAULT_WRITE_TIMEOUT_BROWSER$4,
        },
        requester: createXhrRequester$4(),
        algoliaAgents: [{ segment: 'Browser' }],
        authMode: 'WithinQueryParameters',
        responsesCache: createMemoryCache$4(),
        requestsCache: createMemoryCache$4({ serializable: false }),
        hostsCache: createFallbackableCache$4({
            caches: [createBrowserLocalStorageCache$4({ key: `${apiClientVersion$4}-${appId}` }), createMemoryCache$4()],
        }),
        ...options,
    });
}

function createBrowserLocalStorageCache$3(options) {
  let storage;
  // We've changed the namespace to avoid conflicts with v4, as this version is a huge breaking change
  const namespaceKey = `algolia-client-js-${options.key}`;
  function getStorage() {
...
function createFallbackableCache$3(options) {
  const caches = [...options.caches];
  const current = caches.shift();
  if (current === undefined) {
    return createNullCache$3();
  }
  return {
    get(key, defaultValue, events = {
      miss: () => Promise.resolve()
      ...
      return current.get(key, defaultValue, events).catch(() => {
        return createFallbackableCache$3({
          caches
        }).get(key, defaultValue, events);
      });
    },
...
function createMemoryCache$3(options = {
  serializable: true
}) {
  let cache = {};
  return {
...
const DEFAULT_CONNECT_TIMEOUT_BROWSER$3 = 1000;
const DEFAULT_READ_TIMEOUT_BROWSER$3 = 2000;
const DEFAULT_WRITE_TIMEOUT_BROWSER$3 = 30000;

function createAuth$2(appId, apiKey, authMode = 'WithinHeaders') {
  const credentials = {
    'x-algolia-api-key': apiKey,
    'x-algolia-application-id': appId
  };
...
function createBrowserLocalStorageCache$2(options) {
  let storage;
  // We've changed the namespace to avoid conflicts with v4, as this version is a huge breaking change
  const namespaceKey = `algolia-client-js-${options.key}`;
  function getStorage() {
...
function createFallbackableCache$2(options) {
  const caches = [...options.caches];
  const current = caches.shift();
  if (current === undefined) {
    return createNullCache$2();
  }
  return {
    get(key, defaultValue, events = {
      miss: () => Promise.resolve()
      ...
      return current.get(key, defaultValue, events).catch(() => {
        return createFallbackableCache$2({
          caches
        }).get(key, defaultValue, events);
      });
    },
...
function createMemoryCache$2(options = {
  serializable: true
}) {
  let cache = {};
  return {
...
// By default, API Clients at Algolia have expiration delay of 5 mins.
// In the JavaScript client, we have 2 mins.
const EXPIRATION_DELAY$2 = 2 * 60 * 1000;
function createStatefulHost$2(host, status = 'up') {
  const lastUpdate = Date.now();
  function isUp() {
    return status === 'up' || Date.now() - lastUpdate > EXPIRATION_DELAY$2;
  }
  function isTimedOut() {
    return status === 'timed out' && Date.now() - lastUpdate <= EXPIRATION_DELAY$2;
  }
  return {
    ...host,
    status,
...
let AlgoliaError$2 = class AlgoliaError extends Error {
  constructor(message, name) {
    super(message);
    _defineProperty$2(this, "name", 'AlgoliaError');
    if (name) {
      this.name = name;
    }
  }
};
let ErrorWithStackTrace$2 = class ErrorWithStackTrace extends AlgoliaError$2 {
  constructor(message, stackTrace, name) {
    super(message, name);
    // the array and object should be frozen to reflect the stackTrace at the time of the error
    _defineProperty$2(this, "stackTrace", void 0);
    this.stackTrace = stackTrace;
  }
};
let RetryError$2 = class RetryError extends ErrorWithStackTrace$2 {
  constructor(stackTrace) {
    super('Unreachable hosts - your application id may be incorrect. If the error persists, please reach out to the Algolia Support team: https://alg.li/support.', stackTrace, 'RetryError');
  }
};
let ApiError$2 = class ApiError extends ErrorWithStackTrace$2 {
  constructor(message, status, stackTrace, name = 'ApiError') {
    super(message, stackTrace, name);
...
let DeserializationError$2 = class DeserializationError extends AlgoliaError$2 {
  constructor(message, response) {
    super(message, 'DeserializationError');
    _defineProperty$2(this, "response", void 0);
    this.response = response;
  }
};
// DetailedApiError is only used by the ingestion client to return more informative error, other clients will use ApiClient.
let DetailedApiError$2 = class DetailedApiError extends ApiError$2 {
  constructor(message, status, error, stackTrace) {
    super(message, status, stackTrace, 'DetailedApiError');
    _defineProperty$2(this, "error", void 0);
    this.error = error;
  }
};
function serializeUrl$2(host, path, queryParameters) {
  const queryParametersAsString = serializeQueryParameters$2(queryParameters);
  let url = `${host.protocol}://${host.url}${host.port ? `:${host.port}` : ''}/${path.charAt(0) === '/' ? path.substring(1) : path}`;
  if (queryParametersAsString.length) {
    url += `?${queryParametersAsString}`;
  }
...
function serializeQueryParameters$2(parameters) {
  return Object.keys(parameters).filter(key => parameters[key] !== undefined).sort().map(key => `${key}=${encodeURIComponent(Object.prototype.toString.call(parameters[key]) === '[object Array]' ? parameters[key].join(',') : parameters[key]).replaceAll('+', '%20')}`).join('&');
}
function serializeData$2(request, requestOptions) {
  if (request.method === 'GET' || request.data === undefined && requestOptions.data === undefined) {
    return undefined;
  }
  const data = Array.isArray(request.data) ? request.data : {
...
function serializeHeaders$2(baseHeaders, requestHeaders, requestOptionsHeaders) {
  const headers = {
    Accept: 'application/json',
    ...baseHeaders,
    ...requestHeaders,
...
function deserializeSuccess$2(response) {
  try {
    return JSON.parse(response.content);
  } catch (e) {
    throw new DeserializationError$2(e.message, response);
  }
}
function deserializeFailure$2({
  content,
  status
}, stackFrame) {
  try {
    const parsed = JSON.parse(content);
    if ('error' in parsed) {
      return new DetailedApiError$2(parsed.message, status, parsed.error, stackFrame);
    }
    return new ApiError$2(parsed.message, status, stackFrame);
  } catch {
    // ..
  }
  return new ApiError$2(content, status, stackFrame);
}

function isNetworkError$2({
  isTimedOut,
  status
}) {
  return !isTimedOut && ~~status === 0;
}
function isRetryable$2({
  isTimedOut,
  status
}) {
  return isTimedOut || isNetworkError$2({
    isTimedOut,
    status
  }) || ~~(status / 100) !== 2 && ~~(status / 100) !== 4;
}
function isSuccess$2({
  status
}) {
  return ~~(status / 100) === 2;
}

function stackTraceWithoutCredentials$2(stackTrace) {
  return stackTrace.map(stackFrame => stackFrameWithoutCredentials$2(stackFrame));
}
function stackFrameWithoutCredentials$2(stackFrame) {
  const modifiedHeaders = stackFrame.request.headers['x-algolia-api-key'] ? {
    'x-algolia-api-key': '*****'
  } : {};
  return {
...
function createTransporter$2({
  ...
  async function createRetryableOptions(compatibleHosts) {
    const statefulHosts = await Promise.all(compatibleHosts.map(compatibleHost => {
      return hostsCache.get(compatibleHost, () => {
        return Promise.resolve(createStatefulHost$2(compatibleHost));
      });
    }));
    const hostsUp = statefulHosts.filter(host => host.isUp());
    const hostsTimedOut = statefulHosts.filter(host => host.isTimedOut());
  ...
  async function retryableRequest(request, requestOptions, isRead = true) {
     ...
     * First we prepare the payload that do not depend from hosts.
     */
    const data = serializeData$2(request, requestOptions);
    const headers = serializeHeaders$2(baseHeaders, request.headers, requestOptions.headers);
    // On `GET`, the data is proxied to query parameters.
    const dataQueryParameters = request.method === 'GET' ? {
      ...request.data,
      ...requestOptions.data
    ...
    const retry = async (retryableHosts, getTimeout) => {
      ...
      const host = retryableHosts.pop();
      if (host === undefined) {
        throw new RetryError$2(stackTraceWithoutCredentials$2(stackTrace));
      }
      const timeout = {
        ...timeouts,
        ...requestOptions.timeouts
      ...
      const payload = {
        ...
        headers,
        method: request.method,
        url: serializeUrl$2(host, request.path, queryParameters),
        connectTimeout: getTimeout(timeoutsCount, timeout.connect),
        responseTimeout: getTimeout(timeoutsCount, isRead ? timeout.read : timeout.write)
      };
      /**
      ...
      if (isRetryable$2(response)) {
         ...
         */
        // eslint-disable-next-line no-console -- this will be fixed by exposing a `logger` to the transporter
        console.log('Retryable failure', stackFrameWithoutCredentials$2(stackFrame));
        /**
         * We also store the state of the host in failure cases. If the host, is
         * down it will remain down for the next 2 minutes. In a timeout situation,
         * this host will be added end of the list of hosts on the next request.
         */
        await hostsCache.set(host, createStatefulHost$2(host, response.isTimedOut ? 'timed out' : 'down'));
        return retry(retryableHosts, getTimeout);
      }
      if (isSuccess$2(response)) {
        return deserializeSuccess$2(response);
      }
      pushToStackTrace(response);
      throw deserializeFailure$2(response, stackTrace);
    };
    /**
     * Finally, for each retryable host perform request until we got a non
     * retryable response. Some notes here:
...
function createAlgoliaAgent$2(version) {
  const algoliaAgent = {
    value: `Algolia for JavaScript (${version})`,
    add(options) {
      const addedAlgoliaAgent = `; ${options.segment}${options.version !== undefined ? ` (${options.version})` : ''}`;
...
function getAlgoliaAgent$2({
  algoliaAgents,
  client,
  version
}) {
  const defaultAlgoliaAgent = createAlgoliaAgent$2(version).add({
    segment: client,
    version
  });
  algoliaAgents.forEach(algoliaAgent => defaultAlgoliaAgent.add(algoliaAgent));
...
const DEFAULT_CONNECT_TIMEOUT_BROWSER$2 = 1000;
const DEFAULT_READ_TIMEOUT_BROWSER$2 = 2000;
const DEFAULT_WRITE_TIMEOUT_BROWSER$2 = 30000;

function createXhrRequester$3() {
    function send(request) {
        return new Promise((resolve) => {
            const baseRequester = new XMLHttpRequest();
            baseRequester.open(request.method, request.url, true);
...
// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.
const apiClientVersion$3 = '5.2.2';
const REGIONS = ['eu', 'us'];
...
function createPersonalizationClient({ appId: appIdOption, apiKey: apiKeyOption, authMode, algoliaAgents, region: regionOption, ...options }) {
    const auth = createAuth$2(appIdOption, apiKeyOption, authMode);
    const transporter = createTransporter$2({
        hosts: getDefaultHosts$2(regionOption),
        ...options,
        algoliaAgent: getAlgoliaAgent$2({
            algoliaAgents,
...
function personalizationClient(appId, apiKey, region, options) {
    ...
    return createPersonalizationClient({
        ...
        region,
        timeouts: {
            connect: DEFAULT_CONNECT_TIMEOUT_BROWSER$2,
            read: DEFAULT_READ_TIMEOUT_BROWSER$2,
            write: DEFAULT_WRITE_TIMEOUT_BROWSER$2,
        },
        requester: createXhrRequester$3(),
        algoliaAgents: [{ segment: 'Browser' }],
        authMode: 'WithinQueryParameters',
        responsesCache: createMemoryCache$2(),
        requestsCache: createMemoryCache$2({ serializable: false }),
        hostsCache: createFallbackableCache$2({
            caches: [createBrowserLocalStorageCache$2({ key: `${apiClientVersion$3}-${appId}` }), createMemoryCache$2()],
        }),
        ...options,
    });
}

function createAuth$1(appId, apiKey, authMode = 'WithinHeaders') {
  const credentials = {
    'x-algolia-api-key': apiKey,
    'x-algolia-application-id': appId
  };
...
function createBrowserLocalStorageCache$1(options) {
  let storage;
  // We've changed the namespace to avoid conflicts with v4, as this version is a huge breaking change
  const namespaceKey = `algolia-client-js-${options.key}`;
  function getStorage() {
...
function createFallbackableCache$1(options) {
  const caches = [...options.caches];
  const current = caches.shift();
  if (current === undefined) {
    return createNullCache$1();
  }
  return {
    get(key, defaultValue, events = {
      miss: () => Promise.resolve()
      ...
      return current.get(key, defaultValue, events).catch(() => {
        return createFallbackableCache$1({
          caches
        }).get(key, defaultValue, events);
      });
    },
...
function createMemoryCache$1(options = {
  serializable: true
}) {
  let cache = {};
  return {
...
// By default, API Clients at Algolia have expiration delay of 5 mins.
// In the JavaScript client, we have 2 mins.
const EXPIRATION_DELAY$1 = 2 * 60 * 1000;
function createStatefulHost$1(host, status = 'up') {
  const lastUpdate = Date.now();
  function isUp() {
    return status === 'up' || Date.now() - lastUpdate > EXPIRATION_DELAY$1;
  }
  function isTimedOut() {
    return status === 'timed out' && Date.now() - lastUpdate <= EXPIRATION_DELAY$1;
  }
  return {
    ...host,
    status,
...
let AlgoliaError$1 = class AlgoliaError extends Error {
  constructor(message, name) {
    super(message);
    _defineProperty$1(this, "name", 'AlgoliaError');
    if (name) {
      this.name = name;
    }
  }
};
let ErrorWithStackTrace$1 = class ErrorWithStackTrace extends AlgoliaError$1 {
  constructor(message, stackTrace, name) {
    super(message, name);
    // the array and object should be frozen to reflect the stackTrace at the time of the error
    _defineProperty$1(this, "stackTrace", void 0);
    this.stackTrace = stackTrace;
  }
};
let RetryError$1 = class RetryError extends ErrorWithStackTrace$1 {
  constructor(stackTrace) {
    super('Unreachable hosts - your application id may be incorrect. If the error persists, please reach out to the Algolia Support team: https://alg.li/support.', stackTrace, 'RetryError');
  }
};
let ApiError$1 = class ApiError extends ErrorWithStackTrace$1 {
  constructor(message, status, stackTrace, name = 'ApiError') {
    super(message, stackTrace, name);
    _defineProperty$1(this, "status", void 0);
    this.status = status;
  }
};
let DeserializationError$1 = class DeserializationError extends AlgoliaError$1 {
  constructor(message, response) {
    super(message, 'DeserializationError');
...
// DetailedApiError is only used by the ingestion client to return more informative error, other clients will use ApiClient.
let DetailedApiError$1 = class DetailedApiError extends ApiError$1 {
  constructor(message, status, error, stackTrace) {
    super(message, status, stackTrace, 'DetailedApiError');
    _defineProperty$1(this, "error", void 0);
    this.error = error;
  }
};
...
function serializeUrl$1(host, path, queryParameters) {
...
function serializeData$1(request, requestOptions) {
  if (request.method === 'GET' || request.data === undefined && requestOptions.data === undefined) {
    return undefined;
  }
  const data = Array.isArray(request.data) ? request.data : {
...
function serializeHeaders$1(baseHeaders, requestHeaders, requestOptionsHeaders) {
  const headers = {
    Accept: 'application/json',
    ...baseHeaders,
    ...requestHeaders,
...
function deserializeSuccess$1(response) {
  try {
    return JSON.parse(response.content);
  } catch (e) {
    throw new DeserializationError$1(e.message, response);
  }
}
function deserializeFailure$1({
  content,
  status
}, stackFrame) {
  try {
    const parsed = JSON.parse(content);
    if ('error' in parsed) {
      return new DetailedApiError$1(parsed.message, status, parsed.error, stackFrame);
    }
    return new ApiError$1(parsed.message, status, stackFrame);
  } catch {
    // ..
  }
  return new ApiError$1(content, status, stackFrame);
}

function isNetworkError$1({
  isTimedOut,
  status
}) {
  return !isTimedOut && ~~status === 0;
}
function isRetryable$1({
  isTimedOut,
  status
}) {
  return isTimedOut || isNetworkError$1({
    isTimedOut,
    status
  }) || ~~(status / 100) !== 2 && ~~(status / 100) !== 4;
}
function isSuccess$1({
  status
}) {
  return ~~(status / 100) === 2;
}

function stackTraceWithoutCredentials$1(stackTrace) {
  return stackTrace.map(stackFrame => stackFrameWithoutCredentials$1(stackFrame));
}
function stackFrameWithoutCredentials$1(stackFrame) {
  const modifiedHeaders = stackFrame.request.headers['x-algolia-api-key'] ? {
    'x-algolia-api-key': '*****'
  } : {};
  return {
...
function createTransporter$1({
  ...
  async function createRetryableOptions(compatibleHosts) {
    const statefulHosts = await Promise.all(compatibleHosts.map(compatibleHost => {
      return hostsCache.get(compatibleHost, () => {
        return Promise.resolve(createStatefulHost$1(compatibleHost));
      });
    }));
    const hostsUp = statefulHosts.filter(host => host.isUp());
    const hostsTimedOut = statefulHosts.filter(host => host.isTimedOut());
  ...
  async function retryableRequest(request, requestOptions, isRead = true) {
     ...
     * First we prepare the payload that do not depend from hosts.
     */
    const data = serializeData$1(request, requestOptions);
    const headers = serializeHeaders$1(baseHeaders, request.headers, requestOptions.headers);
    // On `GET`, the data is proxied to query parameters.
    const dataQueryParameters = request.method === 'GET' ? {
      ...request.data,
      ...requestOptions.data
    ...
    const retry = async (retryableHosts, getTimeout) => {
      ...
      const host = retryableHosts.pop();
      if (host === undefined) {
        throw new RetryError$1(stackTraceWithoutCredentials$1(stackTrace));
      }
      const timeout = {
        ...timeouts,
        ...requestOptions.timeouts
      ...
      const payload = {
        ...
        headers,
        method: request.method,
        url: serializeUrl$1(host, request.path, queryParameters),
        connectTimeout: getTimeout(timeoutsCount, timeout.connect),
        responseTimeout: getTimeout(timeoutsCount, isRead ? timeout.read : timeout.write)
      };
      /**
      ...
      if (isRetryable$1(response)) {
         ...
         */
        // eslint-disable-next-line no-console -- this will be fixed by exposing a `logger` to the transporter
        console.log('Retryable failure', stackFrameWithoutCredentials$1(stackFrame));
        /**
         * We also store the state of the host in failure cases. If the host, is
         * down it will remain down for the next 2 minutes. In a timeout situation,
         * this host will be added end of the list of hosts on the next request.
         */
        await hostsCache.set(host, createStatefulHost$1(host, response.isTimedOut ? 'timed out' : 'down'));
        return retry(retryableHosts, getTimeout);
      }
      if (isSuccess$1(response)) {
        return deserializeSuccess$1(response);
      }
      pushToStackTrace(response);
      throw deserializeFailure$1(response, stackTrace);
    };
    /**
     * Finally, for each retryable host perform request until we got a non
     * retryable response. Some notes here:
...
function createAlgoliaAgent$1(version) {
  const algoliaAgent = {
    value: `Algolia for JavaScript (${version})`,
    add(options) {
      const addedAlgoliaAgent = `; ${options.segment}${options.version !== undefined ? ` (${options.version})` : ''}`;
...
function getAlgoliaAgent$1({
  algoliaAgents,
  client,
  version
}) {
  const defaultAlgoliaAgent = createAlgoliaAgent$1(version).add({
    segment: client,
    version
  });
  algoliaAgents.forEach(algoliaAgent => defaultAlgoliaAgent.add(algoliaAgent));
...
const DEFAULT_CONNECT_TIMEOUT_BROWSER$1 = 1000;
const DEFAULT_READ_TIMEOUT_BROWSER$1 = 2000;
const DEFAULT_WRITE_TIMEOUT_BROWSER$1 = 30000;

function createXhrRequester$2() {
    function send(request) {
        return new Promise((resolve) => {
            const baseRequester = new XMLHttpRequest();
            baseRequester.open(request.method, request.url, true);
...
// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.
const apiClientVersion$2 = '5.2.2';
...
function createSearchClient({ appId: appIdOption, apiKey: apiKeyOption, authMode, algoliaAgents, ...options }) {
    const auth = createAuth$1(appIdOption, apiKeyOption, authMode);
    const transporter = createTransporter$1({
        hosts: getDefaultHosts$1(appIdOption),
        ...options,
        algoliaAgent: getAlgoliaAgent$1({
            algoliaAgents,
            client: 'Search',
...
function searchClient(appId, apiKey, options) {
    ...
    return createSearchClient({
        ...
        apiKey,
        timeouts: {
            connect: DEFAULT_CONNECT_TIMEOUT_BROWSER$1,
            read: DEFAULT_READ_TIMEOUT_BROWSER$1,
            write: DEFAULT_WRITE_TIMEOUT_BROWSER$1,
        },
        requester: createXhrRequester$2(),
        algoliaAgents: [{ segment: 'Browser' }],
        authMode: 'WithinQueryParameters',
        responsesCache: createMemoryCache$1(),
        requestsCache: createMemoryCache$1({ serializable: false }),
        hostsCache: createFallbackableCache$1({
            caches: [createBrowserLocalStorageCache$1({ key: `${apiClientVersion$2}-${appId}` }), createMemoryCache$1()],
        }),
        ...options,
    });
}
...
// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.
const apiClientVersion$1 = '5.2.2';
...
function recommendClient(appId, apiKey, options) {
    ...
    return createRecommendClient({
        ...
        requester: createXhrRequester$1(),
        algoliaAgents: [{ segment: 'Browser' }],
        authMode: 'WithinQueryParameters',
        responsesCache: createMemoryCache(),
        requestsCache: createMemoryCache({ serializable: false }),
        hostsCache: createFallbackableCache({
            caches: [createBrowserLocalStorageCache({ key: `${apiClientVersion$1}-${appId}` }), createMemoryCache()],
        }),
        ...options,
    });
}
