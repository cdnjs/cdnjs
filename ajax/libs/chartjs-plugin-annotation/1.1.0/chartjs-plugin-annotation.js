...
function dispatchEvent(chart, handler, element, event) {
  helpers.callback(handler, [{chart, element}, event]);
}
...
function drawLabel(ctx, line, chartArea) {
  ...
  const font = helpers.toFont(label.font);
  ...
  helpers.addRoundedRectPath(ctx, {
    ...
    // TODO: v2 remove support for cornerRadius
    radius: clampAll(helpers.toTRBLCorners(helpers.valueOrDefault(label.cornerRadius, label.borderRadius)), 0, Math.min(width, height) / 2)
  });
  ctx.closePath();
  ctx.fill();
  if (stroke) {
