...
import { distanceBetweenPoints, callback, isFinite as isFinite$1, toFontString, isArray, toRadians, clone, isObject, merge, resolve, clipArea, unclipArea, valueOrDefault } from 'chart.js/helpers';

const clickHooks = ['click', 'dblclick'];
const moveHooks = ['enter', 'leave'];
const hooks = clickHooks.concat(moveHooks);

function updateListeners(chart, state) {
...
function handleEvent(chart, state, event) {
  if (state.listened) {
    switch (event.type) {
    case 'mousemove':
    case 'mouseout':
