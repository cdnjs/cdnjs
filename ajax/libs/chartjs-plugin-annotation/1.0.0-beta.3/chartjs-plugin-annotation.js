...
var Annotation = {
  ...
  beforeUpdate(chart, args, options) {
    ...
    state.options = helpers.clone(options);
    const annotations = state.options.annotations;

    if (helpers.isObject(annotations)) {
    ...
    } else if (helpers.isArray(annotations)) {
...
function resolveAnnotationOptions(chart, options) {
  ...
  const elOptions = helpers.merge(Object.create(null), [chart.options.elements[elType.id], options]);
  elOptions.display = !!helpers.resolve([elOptions.display, true], {chart, options: elOptions});
  return elOptions;
}
