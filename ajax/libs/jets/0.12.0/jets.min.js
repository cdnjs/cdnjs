...
this.options,a=this.replaceDiacritics(this.search_tag.value.trim().toLowerCase().replace(/\s\s+/g," ")).replace(/\\/g,"\\\\"),c=b.searchSelectorMode?a.split(" ").filter(function(a,b,c){return c.indexOf(a)==b}):[a],d="AND"==b.searchSelectorMode,e=Array(c.length),h=0,f=c.length;h<f;h++)e[h]=(d?this.content_param+">":"")+(b.invert?"":":not(")+"[data-jets"+b.searchSelector+'="'+c[h]+'"]'+(b.invert?"":")");c=b.hideBy.split(";").filter(Boolean).map(function(a){return a+(b.addImportant?"!important":"")});
d=(d?"":this.content_param+">")+e.join(d?",":"")+"{"+c.join(";")+"}";this.styleTag.innerHTML=a.length?d:""},_addStyleTag:function(){this.styleTag=document.createElement("style");document.head.appendChild(this.styleTag)},_getText:function(b){return b&&(b.textContent||b.innerText)||""},_getContentTags:function(b){return Array.prototype.slice.call(this.content_tag).reduce(function(a,c){return a.concat(Array.prototype.slice.call(c.querySelectorAll(b||":scope > *")))},[])},_setJets:function(b,a){for(var c=
this,d=c._getContentTags(a?"":b),e,h=0,f;f=d[h];h++)if(!f.hasAttribute("data-jets")||a)e=this.options.manualContentHandling?this.options.manualContentHandling(f):c.options.columns&&c.options.columns.length?c.options.columns.map(function(a){return c._getText(f.children[a])}).join(" "):c._getText(f),f.setAttribute("data-jets",c.replaceDiacritics(e.trim().replace(/\s+/g," ").toLowerCase()))},replaceDiacritics:function(b){var a=this.options.diacriticsMap,c;for(c in a)if(a.hasOwnProperty(c))for(var d=
0,e=a[c].length;d<e;d++)b=b.replace(new RegExp(a[c][d],"g"),c);return b},update:function(b){this._setJets(":scope > :not([data-jets])",b)},_destroy:function(){this.styleTag.parentNode&&document.head.removeChild(this.styleTag);for(var b=this._getContentTags(),a=0,c;c=b[a];a++)c.removeAttribute("data-jets")}};(function(b,a){try{b.querySelector(":scope body")}catch(c){["querySelector","querySelectorAll"].forEach(function(c){var e=a[c];a[c]=function(a){if(/(^|,)\s*:scope/.test(a)){var f=this.id;this.id=
"ID_"+Date.now();a=a.replace(/((^|,)\s*):scope/g,"$1#"+this.id);a=b[c](a);this.id=f;return a}return e.call(this,a)}})}})(window.document,Element.prototype);return g});