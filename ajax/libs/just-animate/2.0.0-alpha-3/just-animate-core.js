...
var resolve = function (input, ctx) {
    ...
    if (isDefined(output) && !isNumber(output)) {
        ...
        if (match) {
            ...
            var step = stepTypeString && isDefined(ctx.index) ? (ctx.index || 0) + 1 : 1;
            var val = sign * num * step;
            output = unitTypeString ? val + unitTypeString : val;
        }
    }
    return output;
};
...
var Animator = (function () {
    function Animator(options) {
        var self = this;
        var $transition = options.$transition, css = options.css, delay = options.delay, easing = options.easing, endDelay = options.endDelay, from = options.from, index = options.index, target = options.target, targets = options.targets;
        var ctx = {
            index: index,
            target: target,
            targets: targets
...
var Timeline$$1 = (function () {
    ...
    Timeline$$1.prototype.to = function (toTime, opts) {
        ...
        var fromTime = isDefined(opts.from)
    ...
    Timeline$$1.prototype._insert = function (from, to, opts) {
        ...
        var $transition = opts.$transition, css$$1 = opts.css, delay = opts.delay, direction = opts.direction, endDelay = opts.endDelay, fill = opts.fill, iterationStart = opts.iterationStart, iterations = opts.iterations;
