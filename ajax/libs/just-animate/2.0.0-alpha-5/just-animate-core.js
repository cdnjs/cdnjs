...
var Animator = (function () {
    ...
    Animator.prototype.isActive = function (currentTime, playbackRate) {
        var self = this;
        var endTimeMs = self.endTimeMs, startTimeMs = self.startTimeMs;
        var isForward = playbackRate >= 0;
        if (isForward) {
            var paddedForwardTime = currentTime + framePadding;
            return startTimeMs <= paddedForwardTime && paddedForwardTime < endTimeMs;
        }
        var paddedBackwardTime = currentTime - framePadding;
        return startTimeMs < paddedBackwardTime && paddedBackwardTime <= endTimeMs;
    };
    Animator.prototype.seek = function (currentTime) {
        var _a = this, animator = _a.animator, startTimeMs = _a.startTimeMs, endTimeMs = _a.endTimeMs;
        var relativeTime = Math.min(Math.max(currentTime - startTimeMs, 0), endTimeMs - startTimeMs);
    ...
    Animator.prototype.tick = function (playbackRate, isLastFrame) {
        var self = this;
        if (isLastFrame) {
            self.restart();
        }
...
var Timeline$$1 = (function () {
    ...
    Timeline$$1.prototype.seek = function (currentTime) {
        var self = this;
        self.currentTime = currentTime;
        for (var i = 0, ilen = self._animations.length; i < ilen; i++) {
    ...
    Timeline$$1.prototype._calcDuration = function () {
        var self = this;
