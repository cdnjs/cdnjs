...
/***/ function(module, exports) {
    ...
    avalon.directive('controller', {
        ...
        parse: function (binding, num, vnode) {
            ...
            var vnodeStr = 'vnode' + num
            var a = 'var ' + vm + ' =  avalon.vmodels[' + avalon.quote($id) + ']\n'
            var b = 'var ' + vm + ' = ' + $id + '\n'
            vnode.isVoidTag = true
            var propsCopy = avalon.mix({'ms-controller': $id}, vnode.props)
            var template = vnode.template
            var _inheritVmodel
            vmodel.$$render = function(_vnode, inheritVmodel) {
                _vnode.props['ms-controller'] = $id
                inheritVmodel = _inheritVmodel = inheritVmodel || _inheritVmodel
                ...
                render = avalon.render(render) 
                _vnode.children = render(__vmodel__)
                _vnode.isVoidTag= false
                if (inheritVmodel) {
                    vmodel.$render = function() {
                        return [vmodel.$$render({
                            type: vnode.type,
...
/***/ function(module, exports, __webpack_require__) {
    ...
    avalon.directive('on', {
        priority: 3000,
        parse: function (binding, num, cur) {
...
/***/ function(module, exports) {
    ...
    function patch(nodes, vnodes, parent, steps) {
        ...
        for (var i = 0, vn = vnodes.length; i < vn; i++) {
            ...
            //ms-duplex
            var msc = vnode.props && vnode.props['ms-controller']
            if (msc) {
                avalon.vmodels[msc].$element = node 
                node.vtree = [vnode]
            }
            execHooks(node, vnode, parent, steps, 'afterChange')
        }
