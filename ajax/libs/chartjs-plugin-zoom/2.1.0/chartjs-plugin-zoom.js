...
function zoomScale(chart, scaleId, range, transition = 'none') {
  ...
  chart.update(transition);
  helpers.callback(state.options.zoom?.onZoom, [{chart}]);
}
...
function getPointPosition(event, chart) {
  ...
  if (!helpers._isPointInArea(event, canvasArea)) {
...
function startPinch(chart, state, event) {
  if (state.options.zoom.pinch.enabled) {
    const point = helpers.getRelativePosition(event, chart);
    helpers.callback(state.options.zoom.onZoomStart, [{chart, event, point}]);
    state.scale = 1;
  }
}
