...
${"[1;33m"+(l.info_validArgs||"Valid arguments are below")}.[0m`),g(["paramOptions","flags","infoCmds"]),process.exit(1)))});for(const $ of["length","qty"])m[$]&&(isNaN(m[$])||m[$]<1)&&(console.error(`
${a+(l.prefix_error||"ERROR")}: [${$}] `+`${l.error_nonPositiveNum||"argument can only be > 0"}.${t}`),f(),process.exit(1));if(process.argv.some(e=>d.infoCmds.help.test(e)))g();else if(process.argv.some(e=>d.infoCmds.version.test(e))){c=n(`npm view ${pkgName} version`).toString().trim()||"none";let e,s=process.cwd();for(;"/"!=s;){var u=o.join(s,"package.json");if(r.existsSync(u)){u=require(u);e=(u.dependencies?.[pkgName]||u.devDependencies?.[pkgName])?.match(/(\d+\.\d+\.\d+)/)[0]||"none";break}s=o.dirname(s)}console.info(`
${l.prefix_globalVer||"Global version"}: `+c),console.info(`${l.prefix_localVer||"Local version"}: `+e)}else{c={length:m.length||8,qty:m.qty||1,charset:m.charset,exclude:m.excludeChars,numbers:!!m.includeNums,symbols:!!m.includeSymbols,lowercase:!m.excludeLowerChars,uppercase:!m.excludeUpperChars,excludeSimilarChars:!!m.excludeSimilarChars,strict:!!m.strictMode,verbose:!m.quietMode},c=e(c);s=`
...
${i}o ${l.helpSection_usage||"Usage"}:[0m`," [1;97m» [1;92m"+cmdFormat+t],paramOptions:[`
