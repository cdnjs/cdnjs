...
${bw}o Usage:`+nc,` ${bw}» `+(bg+cmdFormat+nc)],paramOptions:[`
...
`+(bw+docURL+nc))}function copyToClipboard(e){e=e.replace(/\s+$/m,"").replace(/"/g,'""'),"darwin"==process.platform?execSync(`printf "${e}" | pbcopy`):"linux"==process.platform?execSync(`printf "${e}" | xclip -selection clipboard`):"win32"==process.platform&&execSync(`Set-Clipboard -Value "${e}"`,{shell:"powershell"})}