!function(a, b) {
    ...
    Jodit.prototype.setMode = function(b) {
        ...
        if (false === this.events.fire("beforeSetMode", [ c ])) return;
        this.syncCode();
        this.mode = c.mode === Jodit.MODE_TEXTAREA ? Jodit.MODE_TEXTAREA : Jodit.MODE_WYSIWYG;
        this.$toolbar.find(">li").each(a.proxy(function(b, c) {
            var d, e = c.control;
            d = void 0 === e || void 0 === e.mode ? Jodit.MODE_WYSIWYG : e.mode;
            if (d !== Jodit.MODE_TEXTAREA + Jodit.MODE_WYSIWYG) a(c).toggleClass("disabled", d !== this.mode);
        }, this));
        this.$wysiwyg.removeClass("wysiwyg area").addClass(b === Jodit.MODE_WYSIWYG ? "wysiwyg" : "area");
        setTimeout(a.proxy(function() {
            if (this.mode === Jodit.MODE_WYSIWYG && (d === Jodit.MODE_TEXTAREA || this.options.autofocus)) {
                this.$editor.focus();
                if (this.$editor.find("p").length) this.selection.moveToCursorPosition(this.$editor.find("p")[0]); else this.selection.moveToCursorPosition(this.editor);
            } else this.$area[0].focus();
        }, this), 300);
        if (this.getMode() !== Jodit.MODE_TEXTAREA) {
            if (this.observer) this.observer.changed();
        } else if (this.observerarea) this.observerarea.changed();
        this.events.fire("afterSetMode");
    };
    Jodit.prototype.toggleMode = function() {
        this.setMode(this.getMode() === Jodit.MODE_WYSIWYG ? Jodit.MODE_TEXTAREA : Jodit.MODE_WYSIWYG);
    };
    Jodit.prototype.val = function(a) {
        if (void 0 === a) return this.getElementValue();
        this.events.fire("beforeSetValue", a);
        this.setElementValue(a);
        this.setEditorValue(a);
        this.setAreaValue(a);
        this.events.fire("afterSetValue");
    };
    Jodit.prototype.syncCode = function() {
        if (false === this.events.fire("beforeSyncCode")) return;
        this.setElementValue(this.getMode() === Jodit.MODE_WYSIWYG ? this.getEditorValue() : this.getAreaValue());
        if (this.getMode() !== Jodit.MODE_WYSIWYG) this.setEditorValue(this.val()); else {
            if (!this.val() || "" === this.$editor.text() && !this.$editor.find("img,table,hr").length && this.$editor.find("br").length > 1) {
                if (!this.val()) {
                    var a = this.enterkey.createNewBlock(this.options.enter), b = this.selection.isFocused();
                    this.setEditorValue(a);
                    this.setElementValue(this.getEditorValue());
                    if (b) this.selection.moveToCaretPosition(a);
                }
            ...
            this.setAreaValue(this.val());
        }
        this.events.fire("afterSyncCode");
    };
    ...
    Jodit.prototype.setElementValue = function(a) {
        ...
        c = this.helper.trim(d.value).replace(Jodit.INVISIBLE_SPACE_REG_EXP, "");
        this.$element.val(c);
    ...
    Jodit.prototype.build = function() {
        var b = this, c;
        this.$wysiwyg = a('<div id="' + this.id + '" class="jodit"></div>');
        if (this.options.fullsize) this.$wysiwyg.addClass("jodit_fullsize");
        this.$workflow = a('<div class="jodit_workflow"></div>');
        this.$editor = a('<div class="jodit_editor" contenteditable="true" spellcheck="true" tabindex="' + this.options.tabIndex + '"/>');
        this.$placeholder = a('<span class="jodit_placeholder">Type something</span>');
        if (this.element.hasAttribute("placeholder")) this.$placeholder.html(this.$element.attr("placeholder"));
        this.$area = a('<textarea class="jodit_area_editor"/>');
        this.area = this.$area.get(0);
        this.$progress_bar = a('<div class="jodit_progress_bar"><div></div></div>');
        this.editor = this.$editor.get(0);
        this.$element.after(this.$wysiwyg);
        this.buildToolbar();
        this.$workflow.append(this.$placeholder);
        this.$workflow.append(this.$editor);
        this.$workflow.append(this.$area);
        this.$workflow.append(this.$progress_bar);
        this.loadModules();
        this.val(this.$element.val());
        this.$editor.css("min-height", this.helper.normalizeSize(this.options.minHeight));
        this.$wysiwyg.append(this.$workflow);
        this.$element.hide();
        a(window).on("scroll." + "jodit_" + this.id, a.proxy(function(a) {
            var b = window.pageYOffset || document.documentElement.scrollTop;
            if (b > this.$wysiwyg.offset().top && b < this.$wysiwyg.offset().top + this.$wysiwyg.outerHeight()) {
                this.$toolbar.addClass("jodit_sticky").css({
                    width: this.$wysiwyg.width() + "px"
                });
                this.$toolbar_dummy.css({
                    height: this.$toolbar.height() + "px"
                });
            } else if (this.$toolbar.hasClass("jodit_sticky")) this.$toolbar.removeClass("jodit_sticky");
        }, this));
        this.$editor.on("click", function(a) {
            b.checkTargets(a.target || a.srcElement);
        });
        this.$editor.on("keydown", a.proxy(function(b) {
            var c;
            switch (b.which) {
              case Jodit.keys.TAB:
                c = this.selection.current();
                if (this.node.hasParent(c, "TD|TH")) {
                    c = this.node.parentNode(c, "TD|TH");
                    if (!b.shiftKey) this.selection.focusTo(a(c).closest("table").data("table-processor").nextCell(c, true)); else this.selection.focusTo(a(c).closest("table").data("table-processor").prevCell(c));
                    return false;
                }
                if (this.node.hasParent(c, "PRE")) {
                    c = this.getDocument().createTextNode("	");
                    this.insertNode(c);
                    this.insertNode(this.getDocument().createTextNode("\xa0"));
                    return false;
                }
            }
        }, this));
        a().add(this.$area).add(this.$editor).on("keydown mousedown mouseup change update drop drag", a.proxy(function() {
            clearTimeout(c);
            c = setTimeout(a.proxy(function() {
                this.syncCode();
            }, this), 3);
        }, this));
        try {
    ...
    var a = {}, b = a.toString, c = a.hasOwnProperty, d = [], e = d.indexOf, f = function(a, b) {
        var c, d = [], e = document, g = /[\s]+/, h = Array.prototype.slice, i = function(a) {
            ...
            if (f.isPlainObject(a) || void 0 !== b) {
                e = function(a, b, c) {
                    if (/^left|top|bottom|right|width|height|margin|padding/i.test(b) && /^[0-9]+$/.test(c.toString())) c += "px";
                    a.style[b] = c;
                };
                i(function() {
            ...
            i(function() {
                var d = a ? a.split(g) : [], e = this.hasAttribute && this.hasAttribute("class") ? this.getAttribute("class").split(g) : [], f = e.join(" "), h;
            ...
            if (void 0 === b) y(a, function(a, b) {
                if (a.indexOf(b) !== -1) a.splice(a.indexOf(b), 1); else a.push(b);
            }); else if (b) A(a); else B(a);
            return this;
        }, D = f.type, E = function(a) {
            if (void 0 !== a) {
                i(function() {
            ...
            if (void 0 !== a) {
                if ("string" !== D(a)) this.empty().append(a); else i(function() {
                    this.innerHTML = a;
                });
                return this;
            }
            ...
            this.each(function() {
                ...
                if (b) do {
                    if (b && b.nodeType !== Node.TEXT_NODE) a.push(b);
                    b = b.nextSibling;
                } while (b);
            });
            return f(a);
            ...
            return this;
        }, Y = "_store_dataset", Z = function(a, b) {
            a = f.camelCase(a);
            if (d.length) {
                if (void 0 === b) {
                    if (void 0 !== d[0].dataset[a]) return d[0].dataset[a];
                    if (void 0 !== d[0][Y] && void 0 !== d[0][Y][a]) return d[0][Y][a];
                    return;
                }
                i(function() {
                    if (void 0 === this[Y]) this[Y] = {};
                    this[Y][a] = b;
                });
                return this;
            }
        ...
        if (a) if ("function" === typeof a) window.addEventListener("load", a, false); else {
            a = "string" === D(a) && !/^</.test(a) ? a ? a.split(",") : [] : [ a ];
            for (c = 0; c < a.length; c += 1) if (void 0 !== a[c]) d = d.concat(K(a[c], b || e.body));
        }
    ...
    Jodit.defaultOptions = {
        ...
        controls: {
            ...
            redo: {
                mode: Jodit.MODE_TEXTAREA + Jodit.MODE_WYSIWYG,
                exec: function() {
                    if (this.getMode() !== Jodit.MODE_TEXTAREA) this.observer.redo(); else this.observerarea.redo();
                },
                tooltip: "Redo"
            },
            undo: {
                mode: Jodit.MODE_TEXTAREA + Jodit.MODE_WYSIWYG,
                exec: function() {
                    if (this.getMode() !== Jodit.MODE_TEXTAREA) this.observer.undo(); else this.observerarea.undo();
                },
                tooltip: "Undo"
            },
            bold: {
        ...
        var c = {}, d = b.getDocument(), e = b.getWindow(), f = b.$editor, g = function() {
            ...
            try {
                ...
                if (!e && !j.collapsed) {
                    ...
                    if (g.parentNode && [ "TD", "TH" ].indexOf(g.parentNode.tagName) !== -1 && g.parentNode.previousSibling && !g.previousSibling) a(g.parentNode.previousSibling).append(g);
                }
                return g;
            } catch (k) {
                console.log(k);
...
!function(a) {
    ...
    Jodit.modules.TableProcessor = function(b, c) {
        ...
        var d, e = b.id + "_" + Jodit.modules.TableProcessor.unique, f = b.options.tableprocessor.accuracy, g = a(c), h = function() {
            ...
            var c, d, e, f, j, k, l = [], m = i(), n = h(), o = g.find("td,th"), p = o.get(0), q = 0, r = function(b, c, d, g) {
                if (void 0 === l[b]) l[b] = [];
                l[b][c] = d;
                ...
                j = 0;
                e = 0;
                if (d.hasAttribute("colspan")) j = parseInt(d.getAttribute("colspan"), 10);
                if (d.hasAttribute("rowspan")) e = parseInt(d.getAttribute("rowspan"), 10);
                if (j > 1 || e > 1) {
                    f = b;
                    k = c;
                    while (f < b + e - 1 || k < c + j - 1) {
                        if (k < c + j - 1) k += 1; else {
                            k = c;
                            f += 1;
                        }
                        if (false === r(f, k, d, true)) return false;
                    }
                }
            };
            for (c = 0; c < n; c += 1) for (d = 0; d < m; d += 1) if (void 0 === l[c] || void 0 === l[c][d]) {
                if (false === r(c, d, p)) return p;
                q += 1;
                p = o[q];
            }
            return l;
            ...
            m().each(function(b, d) {
                var e = j(), f = k(d, 2), g;
                for (g = 0; g < e.length; g += 1) if (!c) if (f[1] + 1 >= e[g].length || e[g][f[1]] !== e[g][f[1] + 1]) a(e[g][f[1]]).after("<td><br/></td>"); else o(e[g][f[1]], "colspan", parseInt(e[g][f[1]].getAttribute("colspan"), 10) + 1); else if (f[1] - 1 < 0 || e[g][f[1]] !== e[g][f[1] - 1]) a(e[g][f[1]]).before("<td><br/></td>"); else o(e[g][f[1]], "colspan", parseInt(e[g][f[1]].getAttribute("colspan"), 10) + 1);
                p();
            });
            ...
            m().each(function(d, e) {
                var f = j(), h = k(e, 2), i, l = a("<tr></tr>", b.getDocument(), b.getDocument()).get(0);
                for (i = 0; i < f[0].length; i += 1) if (!c) if (h[0] + 1 >= f.length || f[h[0]][i] !== f[h[0] + 1][i]) r(l, "<br>"); else o(f[h[0]][i], "rowspan", parseInt(f[h[0]][i].getAttribute("rowspan"), 10) + 1); else if (h[0] - 1 < 0 || f[h[0]][i] !== f[h[0] - 1][i]) r(l, "<br>"); else o(f[h[0]][i], "rowspan", parseInt(f[h[0]][i].getAttribute("rowspan"), 10) + 1);
                g.find("tr").eq(h[0])[c ? "before" : "after"](l);
                p();
            });
            a(b.getWindow()).trigger("updateresizer.jodit-resizer");
        }, t = function() {
            var c, d = a("<tr></tr>", b.getDocument());
            for (c = 1; c <= i(); c += 1) r(d, "<br>");
            g.append(d);
            return d.get(0);
        }, u = function() {
            m().each(function(a, b) {
                var c = k(b), d;
                d = j();
                for (a = 0; a < d.length; a += 1) if (c[1] - 1 >= 0 && d[a][c[1] - 1] === d[a][c[1]]) o(d[a][c[1] - 1], "colspan", parseInt(d[a][c[1] - 1].getAttribute("colspan"), 10) - 1); else if (c[1] + 1 < d[0].length && d[a][c[1] + 1] === d[a][c[1]]) o(d[a][c[1]], "colspan", parseInt(d[a][c[1]].getAttribute("colspan"), 10) - 1); else o(d[a][c[1]], "remove");
                p();
            });
        }, v = function() {
            var c = [];
            ...
            m().each(function(b, d) {
                var e = k(d), f, g, h, i = d.parentNode, l;
                if (c[e[0]]) return;
                c[e[0]] = true;
                g = j();
                for (f = 0; f < g[0].length; f += 1) if (e[0] - 1 >= 0 && g[e[0] - 1][f] === g[e[0]][f]) o(g[e[0] - 1][f], "rowspan", parseInt(g[e[0] - 1][f].getAttribute("rowspan"), 10) - 1); else if (e[0] + 1 < g.length && g[e[0] + 1][f] === g[e[0]][f]) {
                    h = f + 1;
                    l = false;
                    while (h < g[0].length && !l) {
                        if (g[e[0] + 1][h] !== g[e[0]][f] && i !== g[e[0] + 1][h].parentNode) {
                            o(g[e[0]][f], "rowspan", parseInt(g[e[0]][f].getAttribute("rowspan"), 10) - 1);
                            o(g[e[0]][f], "before", g[e[0] + 1][h]);
                            l = true;
                        }
                        h += 1;
                    }
                    if (!l) o(g[e[0]][f], "append", a(i).next());
                }
                p();
                a(i).remove();
            });
            ...
            for (h = 0; h < i.length; h += 1) if (i[h][b] !== i[h][d ? b - 1 : b + 1]) {
                k = a(i[h][b]).outerWidth();
                l = (k + c) / g.outerWidth() * 100;
                o(i[h][b], "width", l.toFixed(f) + "%");
            }
            if (!e) p();
        }, x = function() {
            var a, b, c, d, e, f;
            ...
            for (c = 0; c < a[0].length; c += 1) {
                ...
                if (!f) for (b = 0; b < a.length; b += 1) o(a[b][c], "colspan", d - e);
            }
            ...
            p();
            a = j();
            for (c = 0; c < a[0].length; c += 1) w(c, 0, false, true);
            p();
        }, y = function() {
            var b = n(), c = 0, d, e, h, i = [], k = 0, l = 0, m, q;
            if (b && (b[0] - b[2] || b[1] - b[3])) {
                j(function(f, g, j) {
                    if (f >= b[0] && f <= b[2]) if (g >= b[1] && g <= b[3]) {
                        ...
                        if ("" !== a.trim(j.innerHTML.replace(/<br(\/)?>/g, ""))) i.push(j.innerHTML + "<br/>");
                        m = 0;
                        q = 0;
                        if (j.hasAttribute("colspan")) m = parseInt(j.getAttribute("colspan"), 10);
                        if (j.hasAttribute("rowspan")) q = parseInt(j.getAttribute("rowspan"), 10);
                        if (m > 1) k += m - 1;
                        if (q > 1) l += q - 1;
                        if (!d) {
                            d = a(j);
                ...
                k = b[3] - b[1] + 1;
                l = b[2] - b[0] + 1;
                if (k > 1) d.attr("colspan", k);
                if (l > 1) d.attr("rowspan", l);
                o(d, "width", (c / g.outerWidth() * 100).toFixed(f) + "%");
            ...
            m().each(function(i, l) {
                h = a(l);
                b = k(h.get(0));
                if (!h.attr("colspan") || parseInt(h.attr("colspan"), 10) < 2) j(function(a, c, d) {
                    if (b[1] === c && b[0] !== a && d !== h.get(0)) o(d, "colspan", d.hasAttribute("colspan") ? parseInt(d.getAttribute("colspan"), 10) + 1 : 2);
                }); else o(h, "colspan", parseInt(h.attr("colspan"), 10) - 1);
                c = a("<td><br/></td>");
                if (h.attr("rowspan")) o(c, "rowspan", h.attr("rowspan"));
                h.after(c);
                d = h.outerWidth();
                e = d / g.outerWidth() / 2;
                o(h, "width", (100 * e).toFixed(f) + "%");
                o(c, "width", (100 * e).toFixed(f) + "%");
                p();
            ...
            m().each(function(i, l) {
                ...
                if (!h.attr("rowspan") || parseInt(h.attr("rowspan"), 10) < 2) {
                    j(function(a, c, d) {
                        if (b[0] === a && b[1] !== c && d !== h.get(0)) o(d, "rowspan", d.hasAttribute("rowspan") ? parseInt(d.getAttribute("rowspan"), 10) + 1 : 2);
                    });
                    h.closest("tr").after(d);
                    d.append(c);
                } else {
                    o(h, "rowspan", parseInt(h.attr("rowspan"), 10) - 1);
                    j(function(c, d, i) {
                ...
                if (h.attr("colspan")) o(c, "colspan", h.attr("colspan"));
                p();
                h.removeClass("jodit_focused_cell");
            });
            x();
            ...
            var d, f, h, i, m, n, o, p, q, r;
            if (!D) {
                D = a('<div class="border_resizer">' + "<div></div>" + "</div>");
                D.on("mousedown", function(c) {
                    d = true;
                    r = false;
                    f = parseInt(c.clientX, 10);
                    p = parseInt(D.css("left"), 10);
                    D.addClass("border_resizer_clicked");
                    b.$editor.addClass("non-selected");
                    i = k(D.targets[0], true);
                    m = k(D.targets[1]);
                    n = 1e8;
                    o = 0;
                    j(function(b, c, d) {
                        if (c === i[1]) n = Math.min(E(a(d)).left, n);
                        if (c === m[1]) o = Math.max(E(a(d)).left + a(d).outerWidth(), o);
                    });
                });
                a(b.getDocument()).off(".jodit-border-resizer" + e).on("mousemove.jodit-border-resizer" + e, function(a) {
                    ...
                    if (d) {
                        ...
                        if (p + q > n + b.options.tableprocessor.resizerOffset && p + q < o - b.options.tableprocessor.resizerOffset) D.css("left", p + q + "px");
                    }
                    ...
                    if (d && D.is(":visible")) {
                        D.removeClass("border_resizer_clicked");
                        D.hide();
                    }
                    if (r) {
                        p = parseInt(D.css("left"), 10);
            ...
            g.off(".jodit-border-resizer").on("mousemove.jodit-border-resizer", "td,th", function(b) {
                ...
                if (!D.hasClass("border_resizer_clicked") && ("TD" === c.tagName || "TH" === c.tagName)) {
                    d = a(c).offset();
                    h = a(c).outerWidth();
                    if (Math.abs(d.left - b.clientX) < 10 && a(c).prev().length || Math.abs(d.left + h - b.clientX) < 10 && a(c).next().length) {
                        D.target = a(c);
                        D.$table = a(c).closest("table");
                        D.targets = [];
                        if (Math.abs(d.left - b.clientX) < 10) {
                            i = k(c);
                            D.targets[0] = l(i[0], i[1] - 1);
                            D.targets[1] = c;
                        } else {
                            i = k(c, true);
                            D.targets[0] = c;
                            D.targets[1] = l(i[0], i[1] + 1);
                        }
                        D.css({
                            display: "block",
                            left: (Math.abs(d.left - b.clientX) < 10 ? E(c).left : E(c).left + h) + "px",
                            top: E(g).top + "px",
            ...
            if (!c.shiftKey && !H) {
                ...
                f = n();
                for (i = f[0]; i <= f[2]; i += 1) for (h = f[1]; h <= f[3]; h += 1) a(l(i, h)).addClass("jodit_focused_cell");
                if (!d) return J(c, true);
            }
            if (m().length) {
                j = a(l(f[2], f[3]));
                k = a(l(f[2], f[1]));
                b.events.fire("showPopap", [ g.get(0), E(k).left + Math.round((E(j).left + j.outerWidth() - E(k).left) / 2), E(j).top + j.outerHeight() ]);
            }
        };
        if (c) {
            G();
            g.on("mousedown", "td,th", function(a) {
                J(a);
                H = true;
                ...
                if (H && I !== c) {
                    m().removeClass("jodit_focused_cell");
                    a(I).addClass("jodit_focused_cell");
                    J(b);
                    b.preventDefault();
                }
            });
            ...
            if (g.length) a(b.getDocument()).on("keydown.jodit" + e, function(a) {
                ...
                if ((a.which === Jodit.keys.TOP || a.which === Jodit.keys.BOTTOM) && g && b.node.hasParent(g, "td|th") && h.caretAtStartOrEndOfBlock(a.which === Jodit.keys.TOP)) {
                    I = b.node.parentNode(g, "td|th");
                    d = k(I);
                    e = j();
                    f = d[0];
                    do {
                        f += a.which === Jodit.keys.TOP ? -1 : 1;
                        if (f >= 0 && f < e.length && e[f][d[1]] !== I) {
            ...
            switch (c) {
              ...
              case "removeFormat":
                ...
                g = function(a) {
                    ...
                    } else if (b.options.cleanHTML.replaceNBSP && a.nodeType === Node.TEXT_NODE && a.nodeValue.match(Jodit.NBSP)) a.nodeValue = a.nodeValue.replace(Jodit.NBSP, " ");
                    return a;
                };
...
!function(a) {
    ...
    Jodit.defaultOptions = a.extend(true, Jodit.defaultOptions, {
        ...
        codeMirrorUrlJS: "//cdnjs.cloudflare.com/ajax/libs/codemirror/5.3.0/codemirror.min.js",
        codeMirrorUrlCSS: "//cdnjs.cloudflare.com/ajax/libs/codemirror/5.3.0/codemirror.min.css",
        codeMirrorXMLUrl: "//cdnjs.cloudflare.com/ajax/libs/codemirror/5.3.0/mode/xml/xml.min.js"
    });
    Jodit.plugins.codeMirror = function(b) {
        if (b.options.useCodeMirror) {
            var c, d = "jodit_codemirror", e = function(a) {
            ...
            b.events.on("beforeSetMode afterResize", function(a) {
                if (c && b.getMode() === Jodit.MODE_TEXTAREA) b.setAreaValue(c.getValue());
            });
            b.events.on("afterSetMode afterResize", function() {
                if (b.getMode() !== Jodit.MODE_TEXTAREA) return;
                var a = function() {
                    if (void 0 === c) if (void 0 !== window.CodeMirror) c = window.CodeMirror.fromTextArea(b.area, b.options.codeMirrorOptions); else setTimeout(a, 300);
                    if (c) {
            ...
            if (void 0 === window.CodeMirror && !a("script." + d).length) {
                e(b.options.codeMirrorUrlJS);
                setTimeout(function() {
                    e(b.options.codeMirrorXMLUrl);
                }, 100);
...
!function(a) {
    ...
    Jodit.plugins.textProcessor = function(b) {
        if (b.options.wrapSingleTextNodes || b.options.removeEmptyNodes) b.events.on("changeElementValue", function() {
            if (b.getMode() === Jodit.MODE_WYSIWYG) {
                ...
                for (g = 0; c && g < c.length; g += 1) if (b.node.isBlock(c[g]) && "" === c[g].textContent.length && b.options.removeEmptyNodes) c[g].parentNode.removeChild(c[g]); else if (b.options.wrapSingleTextNodes) if (c[g].nodeType === Node.ELEMENT_NODE && "img" === c[g].nodeName.toLowerCase()) b.node.wrap(c[g], b.enterkey.createNewBlock(b.options.enter)); else if (c[g].nodeType === Node.TEXT_NODE && (0 !== b.helper.trim(c[g].nodeValue).length || d)) {
                    ...
                    f.push(c[g]);
                    e = c[g].nextSibling;
                    while (e && "SPAN" === e.tagName && "jodit_marker" === e.className) {
                        f.push(e);
                        d += a(e).prop("outerHTML");
                        e = e.nextSibling;
                    }
                } else if (d.length) {
