!function(a, b) {
    ...
    Jodit.prototype.getDocument = function() {
        if (this.iframe && this.iframe.contentWindow) return this.iframe.contentWindow.document;
    ...
    Jodit.prototype.setMode = function(b) {
        ...
        d.$toolbar.children().each(function(b, c) {
            var e, f = c.control;
    ...
    Jodit.prototype.syncCode = function() {
        ...
        if (!a.val() || "" === a.$editor.text() && !a.$editor.find("img,table,hr").length && a.$editor.find("br").length > 1) a.$placeholder.show(); else {
            a.$placeholder.hide();
            this.observe();
        }
    ...
    Jodit.prototype.buildToolbarButton = function(b, c, d) {
        ...
        if (c.list) {
            ...
            a.each(c.list, a.proxy(function(a, b) {
                ...
                }, c.template && c.template.call(this, a, b));
                f.append(d);
            }, this));
            e.append(f);
        ...
        if (c.tooltip) e.find(".jodit_tooltip").html(this.i18n(c.tooltip)); else e.find(".jodit_tooltip").remove();
    ...
    Jodit.prototype.buildToolbar = function() {
        var b = this;
        b.$toolbar = a('<ul class="toolbar"></ul>');
        if (!b.options.toolbar) b.$toolbar.hide();
        b.$toolbar_popap = a('<div class="jodit_toolbar_popap non-selected"></div>');
        b.$toolbar_dummy = a('<div class="jodit_toolbar_dummy"></div>');
        b.$toolbar_popap.on("mousedown", function(a) {
            a.stopImmediatePropagation();
        });
        a(b.getWindow()).add(b.$editor).off("mousedown.editor" + b.id).on("mousedown.editor" + b.id, function() {
            b.closeToolbarPopap();
        });
        a.each(b.options.buttons, function(c) {
            if (b.options.removeButtons.indexOf(b.options.buttons[c]) !== -1) return;
            if ("|" !== b.options.buttons[c]) {
                var d = b.options.buttons[c];
                if (!a.isPlainObject(d) && void 0 !== b.options.controls[d]) d = b.options.controls[d];
                b.$toolbar.append(b.buildToolbarButton(b.options.buttons[c], d));
            } else b.$toolbar.append('<li class="separator"></li>');
        });
        b.$workflow.before(b.$toolbar[0]);
        b.$workflow.before(b.$toolbar_dummy[0]);
    };
    ...
    Jodit.prototype.build = function() {
        ...
        if (!b.options.iframe) b.$editor = a('<div class="jodit_editor" contenteditable="true" spellcheck="true" tabindex="' + b.options.tabIndex + '"/>'); else b.$iframe = a('<iframe style="display:block;" src="about:blank" frameBorder="0"></iframe>');
        b.$placeholder = a('<span class="jodit_placeholder">' + b.i18n("Type something") + "</span>");
        ...
        if (!b.options.iframe) b.$wysiwyg_flow.append(b.$editor); else b.$wysiwyg_flow.append(b.$iframe);
        b.area = b.$area.get(0);
        b.$progress_bar = a('<div class="jodit_progress_bar"><div></div></div>');
        b.$workflow.append(b.$placeholder).append(b.$wysiwyg_flow).append(b.$area_flow).append(b.$progress_bar);
        ...
        if (b.options.iframe) {
            ...
            e.write('<html class="jodit"><head></head><body class="jodit_editor" style="outline:none" contenteditable="true"></body></html>');
            e.close();
            b.$editor = a(e.body);
            if (b.options.iframeIncludeJoditStyle) a("link[href*=jodit]").each(function() {
                var b = a('<link rel="stylesheet" href="' + this.getAttribute("href") + '">', e).get(0);
                e.head.appendChild(b);
            });
            if (b.options.iframeCSSLinks) b.options.iframeCSSLinks.forEach(function(b) {
                var c = a('<link rel="stylesheet" href="' + b + '">', e).get(0);
                e.head.appendChild(c);
            });
            if (b.options.iframeStyle) {
                d = e.createElement("style");
                d.innerHTML = b.options.iframeStyle;
                b.getDocument().head.appendChild(d);
            }
            b.$iframe.css({
                width: "auto" === b.options.width ? "100%" : b.options.width,
                height: b.options.height,
                minHeight: b.options.minHeight
            });
            b.$editor.css("minHeight", b.options.minHeight);
            b.windows.push(b.getWindow());
        } else b.$editor.css({
        ...
        if (b.options.iframe && "auto" === b.options.height) {
            b.getDocument().documentElement.style.overflowY = "hidden";
            b.events.on("changeElementValue afterInit", function() {
                b.$iframe.css("height", b.$editor.outerHeight());
            });
        }
        try {
    ...
    Jodit.prototype.i18n = function(a) {
        var b = void 0 !== Jodit.lang[this.options.language] ? Jodit.lang[this.options.language] : Jodit.lang.en, c = Array.prototype.slice.call(arguments), d = function(a) {
            return a.replace(/([0-9]{1})\$/gm, function(a) {
                a[1] = parseInt(a[1], 10);
                if (void 0 !== c[a[1]]) return c[a[1]];
                return "";
            });
        };
        if (void 0 !== this.options.i18n[this.options.language] && this.options.i18n[this.options.language][a]) return d(this.options.i18n[this.options.language][a]);
        if (b[a]) return d(b[a]);
        if (Jodit.lang.en[a]) return d(Jodit.lang.en[a]);
        return d(a);
    };
...
!function() {
    ...
    var a = {}, b = a.toString, c = a.hasOwnProperty, d = [], e = d.indexOf, f = function(a, b) {
        var c, d = [], e = document, g = /[\s]+/, h = Array.prototype.slice, i = function(a) {
            ...
            var b = a.getBoundingClientRect(), c = a.ownerDocument, d = c.body, e = c.documentElement, f = c.defaultView || c.parentWindow, g = f.pageYOffset || e.scrollTop || d.scrollTop, h = f.pageXOffset || e.scrollLeft || d.scrollLeft, i = e.clientTop || d.clientTop || 0, j = e.clientLeft || d.clientLeft || 0, k = b.top + g - i, l = b.left + h - j;
            return {
                top: Math.round(k),
                left: Math.round(l)
            };
            ...
            var d = this, e, h, j, k, l = function() {
                var g = this, h = function(a, b) {
                    ...
                    a = a || window.event;
                    a.originalEvent = a;
                    if (void 0 === a.which && void 0 !== a.keyCode) a.which = a.keyCode;
            ...
            if (f.isPlainObject(a) || void 0 !== b) {
                e = function(a, b, c) {
                    if (void 0 !== c && null !== c && /^left|top|bottom|right|width|min|max|height|margin|padding/i.test(b) && /^[0-9]+$/.test(c.toString())) c += "px";
                    a.style[b] = c;
                };
                i(function() {
            ...
            if (d.length) c = window.getComputedStyle(d[0]).getPropertyValue(f.fromCamelCase(a));
            return c;
        }, r = function(a) {
            ...
            return f();
        }, z = f.type, A = function(a, b, c) {
            i(function() {
                var d = a && "string" === z(a) ? a.split(g) : a || [], e = this.hasAttribute && this.hasAttribute("class") ? this.getAttribute("class").split(g) : [], f = e.join(" "), h;
            ...
            if (void 0 !== a) {
                if ("string" !== z(a)) this.empty().append(a); else i(function() {
                    this.innerHTML = a;
                });
                return this;
            }
            ...
            return this;
        }, _ = "_store_dataset", aa = function(a, b) {
            a = f.camelCase(a);
            if (d.length) {
                if (void 0 === b) {
                    if (void 0 !== d[0].dataset) {
                        if (void 0 !== d[0].dataset[a]) return d[0].dataset[a];
                    } else if (d[0].hasAttribute("data-" + a)) return d[0].getAttribute("data-" + a);
                    if (void 0 !== d[0][_] && void 0 !== d[0][_][a]) return d[0][_][a];
                    return;
                }
                i(function() {
                    if (void 0 === this[_]) this[_] = {};
                    this[_][a] = b;
                });
                return this;
            }
        ...
        if (a) if ("function" === typeof a) window.addEventListener("load", a, false); else {
            a = "string" === z(a) && !/^</.test(a) ? a ? a.split(",") : [] : [ a ];
            for (c = 0; c < a.length; c += 1) if (void 0 !== a[c]) d = d.concat(N(a[c], b || e.body));
        }
    ...
    f.ajax = function(a) {
        ...
        var b = a.xhr(), c = {
            ...
            __fire_error: function(a) {
                if (c.is_error && c.__error) {
                    b.getMessage = function() {
                        return c.status;
                    };
                    c.__error.call(b, c.response, c.status);
                }
            },
            done: function(a) {
                c.__done = a;
    ...
    if (void 0 !== window.Jodit) Jodit.modules.Dom = f; else window.$ = f;
}();
    ...
    Jodit.defaultOptions = {
        ...
        controls: {
            ...
            paragraph: {
                ...
                template: function(a, b) {
                    return "<" + a + ' class="jodit_list_element"><span>' + this.i18n(b) + "</span></" + a + "></li>";
                },
    ...
    Jodit.modules.Selection = function(b) {
        var c = {}, d = b.getDocument(), e = b.getWindow(), f = b.$editor, g = function() {
            ...
            if (/^(LI|DT|DD)$/.test(a.nodeName)) {
                i = k(a.firstChild);
                if (i && /^(UL|OL|DL)$/.test(i.nodeName)) a.insertBefore(d.createTextNode("\xa0"), a.firstChild);
            }
            e = d.createRange();
            ...
            if (a.hasChildNodes()) {
                ...
                while (f) {
                    if (f.nodeType === f.TEXT_NODE) {
                        e.setStart(f, !c ? 0 : f.nodeValue.length);
                        e.setEnd(f, !c ? 0 : f.nodeValue.length);
                        break;
                    }
                    h = f;
                    f = b.node.findSibling(f, a, "firstChild", "nextSibling", false);
                }
                if (!f) {
                    e.setStart(h, 0);
                    e.setEnd(h, 0);
            ...
            return b.getDocument().hasFocus() && b.$editor[0] === b.getDocument().activeElement;
        }, n = function(a) {
...
!function(a) {
    ...
    Jodit.modules.Browser = function() {
        return function(a) {
            var b = navigator.userAgent.toLowerCase(), c = (/(firefox)[\s\/]([\w.]+)/.exec(b) || /(chrome)[\s\/]([\w.]+)/.exec(b) || /(webkit)[\s\/]([\w.]+)/.exec(b) || /(opera)(?:.*version)[\s\/]([\w.]+)/.exec(b) || /(msie)[\s]([\w.]+)/.exec(b) || /(trident)\/([\w.]+)/.exec(b) || b.indexOf("compatible") < 0) && /(mozilla)(?:.*? rv:([\w.]+)|)/.exec(b) || [];
            if ("version" === a) return c[2];
            if ("webkit" === a) return "chrome" === c[1] || "webkit" === c[1];
            if ("ff" === a) return "mozilla" === c[1] || "firefox" === c[1];
...
!function(a) {
    ...
    Jodit.modules.EnterKey = function(b) {
        var c, d = b.getDocument(), e = function() {
            ...
            e.innerHTML = void 0 !== c ? c : Jodit.INVISIBLE_SPACE;
            return e;
        }, i = function(c) {
            ...
            var c = b.selection, d, e = c.current();
            if (e.nodeType === Node.TEXT_NODE && 0 === b.helper.trim(e.nodeValue).length && b.node.isBlock(e.parentNode) && e.parentNode !== b.editor && 0 === b.helper.trim(e.parentNode.innerHTML).length) {
                d = b.node.findSibling(e.parentNode, b.editor, false, "previousSibling", true, b.node.isBlock);
                if (d) c.moveToCaretPosition(d);
        ...
        b.$editor.on("keydown", function(a) {
            if (a.which === Jodit.keys.BACKSPACE) return j(a);
            if (a.which === Jodit.keys.ENTER) return k(a);
            if (a.which === Jodit.keys.RIGHT || a.which === Jodit.keys.BOTTOM || a.which === Jodit.keys.LEFT || a.which === Jodit.keys.TOP) return i(a.which === Jodit.keys.LEFT || a.which === Jodit.keys.TOP);
        });
        c = {
            createNewBlock: h,
            enter: k,
...
!function(a) {
    ...
    Jodit.modules.FileBrowser = function(b) {
        var c = {}, d = b.options.filebrowser, e = "", f = false, g = new Jodit.modules.Dialog(b), h = a('<div class="jodit_filebrowser_loader"><i class="icon-loader"></i></div>'), i = a('<div class="jodit_filebrowser non-selected">' + '<div class="jodit_filebrowser_tree"></div>' + '<div class="jodit_filebrowser_files"></div>' + '<div class="jodit_filebrowser_status"></div>' + "</div>"), j = i.find(".jodit_filebrowser_status"), k = {
            ...
            a.each(b, function(a, b) {
                if ("string" === typeof b) {
                    ...
                    if (b.thumb) g = b.thumb;
                }
                e.push('<a href="' + o(d + c + f) + '" data-path="' + p((c ? c + "/" : "") + f + "/") + '" data-name="' + f + '" data-url="' + o(d + c + f) + '">' + '<img src="' + o(d + c + g) + '" alt="' + f + '">' + "</a>");
            });
            n.html(e);
        }, v = function() {
            ...
            if (d.folder.url) {
                ...
                z = x("folder", function(a) {
                    ...
                    s(b.i18n("Error on load folders"));
                });
            ...
            if (d.isSuccess(c)) s(b.i18n("Files $1 was uploaded", a.join(",")), true); else s(d.getMsg(c));
            y();
        }, I = function() {
        ...
        k.remove.on("click", function() {
            if (n.find(">a.active").length) Jodit.Confirm(b.i18n("Are you shure?"), "", function(b) {
                if (b) {
                    n.find(">a.active").each(function() {
        ...
        m.on("click", "a>i.remove", function(c) {
            var d = this.parentNode, f = a(d).data("path");
            Jodit.Confirm(b.i18n("Are you shure?"), "", function(b) {
                if (b) {
                    D(f, a(d).data("name"));
                    A(e);
                }
            ...
            if (a(this).hasClass("addfolder")) Jodit.Promt(b.i18n("Enter Directory name"), b.i18n("Create directory"), function(a) {
                B(a, e);
            }, b.i18n("type name")); else A(a(this).data("path"));
...
!function(a) {
    ...
    Jodit.modules.Form = function(b) {
        var c, d, e = function(c, d) {
            ...
            var e = a('<div class="jodit_tabs"></div>'), f = a('<div class="jodit_tabs_wrapper"></div>'), g = a('<div class="jodit_tabs_buttons"></div>'), h = 0;
            e.append(g);
            e.append(f);
            a.each(d, function(d, e) {
                var i = a('<div class="jodit_tab"></div>'), j = a('<a href="javascript:void(0);"></a>');
                j.html(b.i18n(d));
                g.append(j);
                if (!a.isFunction(e)) i.append(e); else i.append('<div class="jodit_tab_empty"></div>');
                f.append(i);
                j.on("mousedown", function(b) {
                    g.find("a").removeClass("active");
                    f.find(".jodit_tab").removeClass("active");
                    j.addClass("active");
                    i.addClass("active");
                    if (a.isFunction(e)) e.call(c);
                    b.stopPropagation();
                    return false;
                });
                h += 1;
            ...
            if (!h) return null;
            g.find("a").css("width", (100 / h).toFixed(10) + "%");
            g.find("a:first-child").addClass("active");
            f.find(".jodit_tab:first-child").addClass("active");
            return e;
        }, g = function() {
            return d;
        }, h = function(c, e) {
            var g = {}, h, i;
            if (c.upload) {
                h = a('<div class="jodit_draganddrop_file_box">' + "<strong>" + b.i18n("Drop image") + "</strong>" + "<span><br> " + b.i18n("or click") + "</span>" + '<input type="file" accept="image/*" tabindex="-1" dir="auto" multiple=""/>' + "</div>");
                ...
                g['<i class="icon icon-upload"></i> ' + b.i18n("Upload")] = h;
            }
            if (c.url) {
                i = a("<div>" + '<input required name="url" placeholder="http://" type="text"/>' + '<input name="text" placeholder="' + b.i18n("Alternative text") + '" type="text"/>' + '<button type="submit">' + b.i18n("Insert") + "</button>" + "</div>");
                d = null;
            ...
            if (c.filebrowser) if (b.filebrowser && (b.options.filebrowser.url || b.options.filebrowser.ajax.url || b.options.filebrowser.items.url)) g['<i class="icon icon-folder"></i> ' + b.i18n("Browse")] = function() {
...
!function(a) {
    "use strict";
    Jodit.modules.Helper = function(b) {
        var c = function(a) {
            ...
            return f(a).replace(Jodit.INVISIBLE_SPACE_REG_EXP, "").replace(/[\s]*class=""/g, "");
        }, h = function(c, d) {
            var e = a(c).offset(), f = b.$workflow.offset();
            if (!b.options.iframe || d) return {
                left: e.left - f.left,
                top: e.top - f.top
            }; else return e;
        }, i = {
...
!function(a) {
    ...
    Jodit.modules.ImageProcessor = function(b, c) {
        ...
        if (c) {
            ...
            a(b.windows).on("keyup.jodit" + e + " mouseup.jodit" + e, function(d) {
                if (!c || !c.parentNode) {
                    g();
                    return;
                }
...
!function(a) {
    ...
    Jodit.modules.Popap = function(b) {
        var c, d, e, f, g, h = function() {
            ...
            if (c.content) l.html(b.i18n(c.content));
            if (c.list) {
            ...
            e.addClass("active");
            if (b.options.iframe) c += b.helper.offset(b.$iframe, true).left;
            e.css({
                left: c + "px",
                top: f + "px",
...
!function(a) {
    ...
    Jodit.modules.Resizer = function(b) {
        var c = {}, d = false, e = false, f, g, h, i, j, k, l, m, n, o, p = a('<div class="resizer">' + '<i class="topleft"></i>' + '<i class="topright"></i>' + '<i class="bottomright"></i>' + '<i class="bottomleft"></i>' + "</div>"), q = function(b) {
            b = a(b);
            b.off(".jodit-resizer").on("drag.jodit-resizer", function(a) {
                ...
                if (b.get(0).clicked) setTimeout(function() {
                    ...
                    p.$element = b;
                    p.show().trigger("updatesize");
                }, 50);
            });
        };
        p.find("i").on("mousedown", function(a) {
            p.$element.get(0).clicked = false;
            f = a.target || a.srcElement;
            a.preventDefault();
            a.stopImmediatePropagation();
            i = parseInt(p.$element.outerWidth(), 10);
            j = parseInt(p.$element.outerHeight(), 10);
            k = i / j;
            d = true;
            e = false;
        ...
        a(b.windows).off(".jodit-resizer" + b.id + " updateresizer.jodit-resizer" + b.id).on("mousemove.jodit-resizer" + b.id, function(a) {
            if (d) {
                e = true;
                n = parseInt(a.clientX, 10) - g;
                o = parseInt(a.clientY, 10) - h;
                if ("IMG" === p.$element[0].tagName) if (n) {
                ...
                if (m > b.options.resizer.min_width) if (m < p.parent().outerWidth()) p.$element.css("width", m + "px"); else p.$element.css("width", "100%");
                if (l > b.options.resizer.min_height) p.$element.css("height", l + "px");
                p.trigger("updatesize");
                a.stopImmediatePropagation();
            }
        }).on("resize.jodit-resizer" + b.id + " updateresizer.jodit-resizer", function(a) {
            if (p.is(":visible")) p.trigger("updatesize");
        }).on("mouseup.jodit-resizer" + b.id + " keydown.jodit-resizer" + b.id, function(a) {
            if (p.is(":visible")) if (d) {
                if (e) b.syncCode();
                d = false;
                e = false;
                a.stopImmediatePropagation();
            } else if (!p.$element.get(0).clicked) p.hide();
        });
        p.on("updatesize.jodit-resizer" + b.id, function() {
            if (p.is(":visible")) {
                var a = b.helper.offset(p.$element), c;
                if (b.options.iframe) {
                    c = b.helper.offset(b.$iframe, true);
                    a.left += c.left;
                }
                p.css({
                    top: a.top - 1 + "px",
                    left: a.left - 1 + "px",
                    width: p.$element.outerWidth() + "px",
                    height: p.$element.outerHeight() + "px"
                });
            }
        });
        b.$workflow.append(p);
...
!function(a) {
    ...
    Jodit.modules.TableProcessor = function(b, c) {
        ...
        var d, e = b.id + "_" + Jodit.modules.TableProcessor.unique, f = b.options.tableprocessor.accuracy, g = a(c), h = function() {
            ...
            var c, d, e, f, k, l, m = [], n = i(), o = h(), p = H && j ? j : g.find("td,th"), q = p.get(0), r = 0, s = function(b, c, d, g) {
                if (void 0 === m[b]) m[b] = [];
                m[b][c] = d;
            ...
            a(b.windows).trigger("updateresizer.jodit-resizer");
        }, s = function(c, d) {
            ...
            if (!E) {
                ...
                E.on("mousedown", function(c) {
                    ...
                    o = 0;
                    k(function(c, d, e) {
                        if (d === i[1]) n = Math.min(b.helper.offset(a(e)).left, n);
                        if (d === j[1]) o = Math.max(b.helper.offset(a(e)).left + a(e).outerWidth(), o);
                    });
                });
            ...
            g.off(".jodit-border-resizer").on("mousemove.jodit-border-resizer", "td,th", function(c) {
                var d = c.target || c.srcElement, e;
                if (!E.hasClass("border_resizer_clicked") && ("TD" === d.tagName || "TH" === d.tagName)) {
                    e = a(d).offset();
                    h = a(d).outerWidth();
                    if (Math.abs(e.left - c.clientX) < 10 && a(d).prev().length || Math.abs(e.left + h - c.clientX) < 10 && a(d).next().length) {
                        E.target = a(d);
                        E.$table = a(d).closest("table");
                        E.targets = [];
                        if (Math.abs(e.left - c.clientX) < 10) {
                            i = l(d);
                        ...
                        E.css({
                            display: "block",
                            left: (Math.abs(e.left - c.clientX) < 10 ? b.helper.offset(d).left : b.helper.offset(d).left + h) + "px",
                            top: b.helper.offset(g).top + "px",
            ...
            if (n().length) {
                j = a(m(f[2], f[3]));
                k = a(m(f[2], f[1]));
                b.events.fire("showPopap", [ g.get(0), b.helper.offset(k).left + Math.round((b.helper.offset(j).left + j.outerWidth() - b.helper.offset(k).left) / 2), b.helper.offset(j).top + j.outerHeight() ]);
            }
        };
        if (c) {
            G();
...
!function(a) {
    ...
    Jodit.modules.Uploader = function(b, c) {
        ...
        var d = "", e = c || b.options.uploader, f = function(b) {
            ...
            var j = c.length, k, l, m;
            if (!j) return false;
            l = new FormData();
            l.append(e.pathVariableName, d);
            for (k = 0; k < j; k += 1) {
                m = c[k].type.match(/\/([a-z0-9]+)/i)[1].toLowerCase();
                l.append(e.filesVariableName + "[" + k + "]", c[k], c[k].name || Math.random().toString().replace(".", "") + "." + m);
            }
            e.prepareData(l);
            if (i) i(l);
            g(l, function(c) {
            ...
            var b = atob(a.split(",")[1]), c = a.split(",")[0].split(":")[1].split(";")[0], d = new ArrayBuffer(b.length), e, f = new Uint8Array(d);
            for (e = 0; e < b.length; e += 1) f[e] = b.charCodeAt(e);
            return new Blob([ f ], {
                type: c
            });
        }, k = function(c, d, f) {
            ...
            c.on("paste", function(a) {
                var c, e, g, i, k = function(a) {
                    a.append("extension", g);
                    a.append("mimetype", e.type);
                };
                if (!b.browser("ff")) {
                    if (a.clipboardData && a.clipboardData.items && a.clipboardData.items.length) for (c = 0; c < a.clipboardData.items.length; c += 1) if ("file" === a.clipboardData.items[c].kind && "image/png" === a.clipboardData.items[c].type) {
                        e = a.clipboardData.items[c].getAsFile();
                        g = e.type.match(/\/([a-z0-9]+)/i)[1].toLowerCase();
                        h([ e ], d, f, k);
                        a.preventDefault();
                        break;
                    }
                } else if (!a.clipboardData.types.length && "text/plain" !== a.clipboardData.types[0]) {
                    i = b.getDocument().createElement("div");
                    b.selection.insertNode(i);
                    i.focus();
                    setTimeout(function() {
                        if (i.firstChild && i.firstChild.hasAttribute("src")) {
                            var b = i.firstChild.getAttribute("src");
                            i.parentNode.removeChild(i);
                            h([ j(b) ], d, f);
                            a.preventDefault();
                        }
            ...
            if (b.editor) if (b.options.enableDragAndDropFileToEditor) k(b.$editor);
        };
        return {
            init: l,
            bind: k,
...
!function(a) {
    ...
    Jodit.plugins.image = function(b) {
        Jodit.plugins.image.open = function(c) {
            ...
            h.find("#tabsbox").html(b.form.buildTabs(function() {
                var a = {};
                a[b.i18n("Image")] = j;
                a[b.i18n("Advansed")] = i;
                return a;
            ...
            e.setTitle([ b.i18n("Image properties"), g.remove ]);
            e.setContent(h);
            f = a('<a href="javascript:void(0)" style="float:right;" class="jodit_button"><i class="icon icon-cancel"></i>&nbsp;' + b.i18n("Cancel") + "</a>").on("click", function() {
                e.close();
            });
            e.setFooter([ a('<a href="javascript:void(0)" style="float:left;" class="jodit_button"><i class="icon icon-check"></i>&nbsp;' + b.i18n("Ok") + "</a>").on("click", function() {
                var c;
...
!function(a) {
    ...
    Jodit.plugins.textProcessor = function(b) {
        if (b.options.wrapSingleTextNodes || b.options.removeEmptyNodes || b.options.cleanWhitespace) b.events.on("changeElementValue afterInit", function() {
            if (b.getRealMode() === Jodit.MODE_WYSIWYG) {
                var c, d = "", e = "", f, g = [], h = 0;
                b.observer.block(1);
                b.selection.restore();
                b.selection.save();
                if (b.options.wrapSingleTextNodes || b.options.removeEmptyNodes) {
                    c = [].slice.call(b.editor.childNodes);
                    for (h = 0; c && h < c.length; h += 1) if (b.node.isBlock(c[h]) && "" === c[h].textContent.length && b.options.removeEmptyNodes) c[h].parentNode.removeChild(c[h]); else if (b.options.wrapSingleTextNodes) if (c[h].nodeType === Node.ELEMENT_NODE && "img" === c[h].nodeName.toLowerCase()) b.node.wrap(c[h], b.enterkey.createNewBlock(b.options.enter, false, "")); else if (c[h].nodeType === Node.TEXT_NODE && (0 !== b.helper.trim(c[h].nodeValue).length || e)) {
                        e += c[h].nodeValue;
                        g.push(c[h]);
                        f = c[h].nextSibling;
                        while (f && "SPAN" === f.tagName && "jodit_marker" === f.className) {
                            g.push(f);
                            e += a(f).prop("outerHTML");
                            f = f.nextSibling;
                            h += 1;
                        }
                    } else if ("SPAN" === c[h].tagName && "jodit_marker" === c[h].className) {
                        g.push(c[h]);
                        e += c[h].outerHTML;
                    } else if (e.length) {
                        b.node.replace(g[0], b.enterkey.createNewBlock(b.options.enter, false, e), false, true);
                        a(g).remove();
                        e = "";
                        g = [];
                    } else if (Jodit.INVISIBLE_SPACE_REG_EXP.test(c[h].nodeValue)) c[h].parentNode.removeChild(c[h]);
                    if (e.length) {
                        b.node.replace(g[0], b.enterkey.createNewBlock(b.options.enter, false, e), false, true);
                        a(g).remove();
                    }
                }
                if (b.options.cleanWhitespace) {
                    f = b.editor.firstChild;
                    while (f) {
                        if (f.nodeType === Node.TEXT_NODE && "jodit_marker" !== f.parentNode.className) {
                            d = f.nodeValue;
                            e = d.replace(Jodit.INVISIBLE_SPACE_REG_EXP, "");
                            if (e !== d && e.length) f.nodeValue = e;
                        }
                        f = b.node.findSibling(f, b.editor, "firstChild", "nextSibling", false);
                    }
                }
                b.selection.restore();
                b.observer.block(0);
