!function(a, b) {
    ...
    Jodit.prototype.setMode = function(b) {
        ...
        setTimeout(function() {
            if (d.getRealMode() === Jodit.MODE_WYSIWYG && (f === Jodit.MODE_TEXTAREA || d.options.autofocus)) {
                d.$editor.focus();
    ...
    Jodit.prototype.syncCode = function() {
        var a = this, b, c = this.getRealMode();
        if (false === this.events.fire("beforeSyncCode")) return;
        if (c === Jodit.MODE_WYSIWYG) b = this.getEditorValue(); else b = this.getAreaValue();
        this.setElementValue(b);
        b = this.val();
        if (c === Jodit.MODE_WYSIWYG) this.setAreaValue(b); else this.setEditorValue(b);
        if (!this.val() || "" === this.$editor.text() && !this.$editor.find("img,table,hr").length && this.$editor.find("br").length > 1) this.$placeholder.show(); else this.$placeholder.hide();
