...
// package.json
var version = "0.0.0-development.4";
...
function parseVariant(variant, variants) {
...
function parseCandidate(input, {
  ...
  for (let variant of rawVariants) {
    let parsedVariant = parseVariant(variant, variants);
    ...
    switch (variant) {
      ...
      case "placeholder":
      case "selection":
        parsedVariants.unshift(parsedVariant);
        break;
      default:
...
function createUtilities(theme) {
  ...
  colorUtility("accent", {
    themeKeys: ["--accent-color", "--colors"],
    handle: (value) => [decl("accent-color", value)]
  });
  colorUtility("caret", {
    themeKeys: ["--caret-color", "--colors"],
    handle: (value) => [decl("caret-color", value)]
  });
  colorUtility("divide", {
    themeKeys: ["--divide-color", "--colors"],
    handle: (value) => [
      rule(":where(& > :not([hidden]) ~ :not([hidden]))", [
  ...
  function borderSideUtility(classRoot, desc) {
    utilities.set(classRoot, (candidate) => {
      ...
      {
        let value = resolveThemeColor(candidate, theme, ["--border-color", "--colors"]);
        if (value) {
          return desc.color(value);
        }
      }
  ...
  utilities.set("bg", (candidate) => {
    ...
    {
      let value = resolveThemeColor(candidate, theme, ["--background-color", "--colors"]);
      if (value) {
        return [decl("background-color", value)];
      }
    }
  ...
  utilities.set("fill", (candidate) => {
    ...
    let value = resolveThemeColor(candidate, theme, ["--fill", "--colors"]);
    if (value) {
  ...
  utilities.set("stroke", (candidate) => {
    ...
    {
      let value = resolveThemeColor(candidate, theme, ["--stroke", "--colors"]);
      if (value) {
        return [decl("stroke", value)];
      }
    }
  ...
  colorUtility("placeholder", {
    themeKeys: ["--background-color", "--colors"],
    handle: (value) => [
  ...
  utilities.set("decoration", (candidate) => {
    ...
    {
      let value = resolveThemeColor(candidate, theme, ["--text-decoration-color", "--colors"]);
      if (value) {
        return [decl("text-decoration-color", value)];
      }
    }
  ...
  utilities.set("outline", (candidate) => {
    ...
    {
      let value = resolveThemeColor(candidate, theme, ["--outline-color", "--colors"]);
      if (value) {
        return [decl("outline-color", value)];
      }
    }
  ...
  utilities.set("text", (candidate) => {
    ...
    {
      let value = resolveThemeColor(candidate, theme, ["--text-color", "--colors"]);
      if (value) {
        return [decl("color", value)];
      }
    }
  ...
  });
  {
    ...
    utilities.set("shadow", (candidate) => {
      ...
      {
        let value = resolveThemeColor(candidate, theme, ["--box-shadow-color", "--colors"]);
        if (value) {
          return [
            boxShadowProperties(),
            decl("--tw-shadow-color", value),
  ...
  }
  {
    ...
    utilities.set("ring", (candidate) => {
      ...
      {
        let value = resolveThemeColor(candidate, theme, ["--ring-color", "--colors"]);
        if (value) {
          return [decl("--tw-ring-color", value)];
        }
      }
    ...
    utilities.set("ring-offset", (candidate) => {
      ...
      {
        let value = resolveThemeColor(candidate, theme, ["--ring-offset-color", "--colors"]);
        if (value) {
          return [decl("--tw-ring-offset-color", value)];
        }
      }
...
function compile(css, rawCandidates) {
  ...
  walk(ast, (node, { replaceWith }) => {
    if (node.kind === "rule" && node.selector === "@tailwind utilities") {
      replaceWith(parse2(rawCandidates, designSystem));
      return false;
    }
  });
...
function parse2(rawCandidates, designSystem, { throwOnInvalidCandidate = false } = {}) {
  ...
  let ast = [];
  let variantOrder = Array.from(designSystem.variants.keys());
    ...
    for (let rawCandidate of rawCandidates) {
      ...
      for (let variant of candidate.variants) {
        if (variant.kind === "named") {
          let applyVariant = designSystem.variants.get(variant.root);
    ...
    for (let variant of value.variants) {
      if (variant.kind === "named") {
        if (variant.root === "group" && variant.kind === "named" && variant.value !== null) {
          let nested = parseVariant(variant.value.value, designSystem.variants);
  ...
  ast.sort((a, z) => {
    ...
    return (
      ...
      (aSort.properties[offset] ?? Infinity) - (zSort.properties[offset] ?? Infinity) || // Sort by most properties first, then by least properties
      zSort.properties.length - aSort.properties.length
    );
  });
  return ast;
}
