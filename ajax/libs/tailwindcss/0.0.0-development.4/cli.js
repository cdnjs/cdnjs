...
function addWhitespaceAroundMathOperators(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    if (char === "(") {
      ...
      result2 += `, `;
      continue;
    } else if (char === " " && formattable[0] && result2[result2.length - 1] === " ") {
      continue;
    } else if ((char === "+" || char === "*" || char === "/" || char === "-") && formattable[0]) {
      let trimmed = result2.trimEnd();
...
function parseVariant(variant, variants) {
  ...
  }
  {
    ...
    let [root2, value] = findRoot(variantWithoutModifier, variants);
    if (root2 === null)
      return null;
    {
      let variant2 = {
...
function parseCandidate(input, {
  ...
  let [root2, value] = findRoot(base, utilities);
  if (root2 === null)
    return null;
  if (value === null) {
    return {
...
function toCss(ast) {
  let atRoots = [];
  let css2 = ast.map(function stringify(node) {
    let css3 = "";
    if (node.kind === "rule") {
      ...
      css3 += `${node.selector}{`;
      for (let child of node.nodes) {
        css3 += stringify(child);
      }
      css3 += "}";
    } else if (node.kind === "comment") {
      css3 += `/*${node.value}*/`;
    } else if (node.property !== "--tw-sort" && node.value !== void 0 && node.value !== null) {
      css3 += `${node.property}:${node.value}${node.important ? "!important" : ""};`;
    }
    return css3;
  }).concat(atRoots).join("\n");
  return `${license}${css2}`;
...
var result = oxide.scanDir({ base: root });
var pkgRoot = path__default.default.dirname(__require.resolve("tailwindcss/package.json"));
var preflight = fs__default.default.readFileSync(path__default.default.resolve(pkgRoot, "preflight.css"), "utf-8");
var defaultTheme = fs__default.default.readFileSync(path__default.default.resolve(pkgRoot, "theme.css"), "utf-8");
console.log(
  optimizeCss(
    compile(
      css`
