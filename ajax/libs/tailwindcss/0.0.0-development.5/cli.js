...
function applyVariant(node, variant, variants) {
  ...
  if (variant.kind === "compound") {
    let result3 = applyVariant(node, variant.variant, variants);
    if (result3 === null)
      return null;
    for (let child of node.nodes) {
      let result4 = applyVariantFn(child, variant);
      if (result4 === null)
        return null;
    }
    return;
  }
  let result2 = applyVariantFn(node, variant);
  if (result2 === null)
    return null;
}
  ...
  next:
    for (let [candidate, rawCandidate] of candidates) {
      ...
      for (let variant of candidate.variants) {
        let result2 = applyVariant(node, variant, designSystem.variants);
        if (result2 === null) {
          if (throwOnInvalidCandidate) {
            throw new Error(`Cannot apply unknown utility class: ${rawCandidate}`);
          } else {
