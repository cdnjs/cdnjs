	...
	var addProp = function (ref) {
		...
		if (typeValid(value)) { lastNode[lastKey] = value; }
			...
			if (syncTrigger ||
				...
				!value[0]) { addValListener({ctx: ctx, syncTrigger: syncTrigger, handlers: handlers, subscribers: subscribers, innerData: innerData, element: element, lastNode: lastNode, key: lastKey, expr: value[1], custom: custom}); }
			queue([_handler]);
		}
	};
