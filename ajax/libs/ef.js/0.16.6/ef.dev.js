(function (global, factory) {
	...
	var legacyAssign = function (ee) {
		var ers = [], len = arguments.length - 1;
		while ( len-- > 0 ) ers[ len ] = arguments[ len + 1 ];

		for (var i$1 = 0, list = ers; i$1 < list.length; i$1 += 1) {
			var er = list[i$1];
	...
	var insertBeforeNode = function (node, ref) {
	...
	var handleMountPoint = function (element, target, ref) {
		...
		if (ref) {
			if (ARR.isArray(node)) {
				for (var i$2 = 0, list = node; i$2 < list.length; i$2 += 1) {
					var i = list[i$2];

					insertBeforeNode(i, ref);
				}
			} else { insertBeforeNode(node, ref); }
		} else {
			// eslint-disable-next-line no-lonely-if
			if (ARR.isArray(node)) {
				for (var i$3 = 0, list$1 = node; i$3 < list$1.length; i$3 += 1) {
					var i$1 = list$1[i$3];

					appendNode(i$1, target);
				}
	...
	DOM.before = function (anchorNode) {
	...
	DOM.after = function (anchorNode) {
		var nodes = [], len = arguments.length - 1;
		while ( len-- > 0 ) nodes[ len ] = arguments[ len + 1 ];

		if (anchorNode.nextSibling) { return DOM.before.apply(DOM, [ anchorNode.nextSibling ].concat( nodes )) }
		return DOM.append.apply(DOM, [ anchorNode.parentNode ].concat( nodes ))
	};

	DOM.append = function (parentNode) {
		var nodes = [], len = arguments.length - 1;
		while ( len-- > 0 ) nodes[ len ] = arguments[ len + 1 ];
		...
		if (parentNode instanceof shared.EFBaseComponent) {
			if (!(ARR.isArray(parentNode.children))) {
				{ dbg.warn(parentNode, 'has no `children` list mount point! Child nodes are all ignored!'); }
				return
			}
		...
		// Handle fragment
		if (isInstance(parentNode, EFFragment)) { return parentNode.append.apply(parentNode, nodes) }
	};
	...
	var addEvent = function (ctx, ref) {
		...
		var callEventHandler = function (event) {
			var value = _handler();
			if (ctx.methods[m]) { ctx.methods[m]({e: event, event: event, value: value, state: ctx.state}); }
			else {
				{ dbg.warn(("Event handler '" + m + "' not found! Bubbling up...")); }
				event.data = value;
	...
	var resolveAST = function (ctx, ref) {
		...
		switch (nodeType) {
			...
			case 'object': {
				...
				// Append anchor
				{ DOM.append(element, DOM.document.createComment(("<MountPoint" + (node.t && ' type="list" ' || ' ') + "name=\"" + (node.n) + "\">"))); }
	...
	var scoped = function (component, initScope) {
		...
		return /*@__PURE__*/(function (component) {
			function Scoped(state, scope) {
				if (scope) { component.call(this, state, assign({}, _scope, scope)); }
				else { component.call(this, state, _scope); }
			}
