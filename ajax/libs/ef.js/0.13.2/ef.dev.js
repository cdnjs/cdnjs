(function (global, factory) {
	...
	var addProp = function (ref) {
		...
		if (typeValid(value)) { lastNode[lastKey] = value; }
			...
			if (propPath.length === 1 &&
				...
				!value[0]) { addValListener({ctx: ctx, syncTrigger: syncTrigger, handlers: handlers, subscribers: subscribers, innerData: innerData, element: element, key: lastKey, expr: value[1], custom: custom}); }
			queue([_handler]);
		}
	};
	...
	var createElement = function (ref) {
		...
		if (p) { for (var i = 0, list = p; i < list.length; i += 1) {
				...
				addProp({element: element, custom: custom, value: value, propPath: propPath, syncTrigger: syncTrigger, ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData});
			} }
