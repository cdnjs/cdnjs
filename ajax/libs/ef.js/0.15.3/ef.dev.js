(function (global, factory) {
	...
	var execUserQueue = function () {
		for (var i$1 = 0, list = userQueue; i$1 < list.length; i$1 += 1) {
			var i = list[i$1];
	...
	var initBinding = function (ref) {
		...
		if (!keyStatus || !(keyStatus.get || keyStatus.set)) { initDataNode({parentNode: parentNode, dataNode: dataNode, handlerNode: handlerNode, subscriberNode: subscriberNode, ctx: ctx, _key: _key}); }
	...
	var addProp = function (ref) {
		...
		if (typeValid(value)) { lastNode[lastKey] = value; }
			...
			if (trigger ||
				...
				!value[0]) { addValListener({ctx: ctx, trigger: trigger, updateLock: updateLock, handlers: handlers, subscribers: subscribers, innerData: innerData, element: element, lastNode: lastNode, key: lastKey, expr: value[1], custom: custom}); }
			queue([_handler]);
		}
	};
	...
	var createElement$1 = function (ref) {
		var info = ref.info;
		var ctx = ref.ctx;
		var innerData = ref.innerData;
		var refs = ref.refs;
		...
		if (p) { for (var i = 0, list = p; i < list.length; i += 1) {
				...
				addProp({element: element, custom: custom, value: value, propPath: propPath, trigger: trigger, updateOnly: updateOnly, ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData});
			} }
		if (e) { for (var i$1 = 0, list$1 = e; i$1 < list$1.length; i$1 += 1) {
				var trigger$1 = list$1[i$1];

				addEvent({element: element, custom: custom, trigger: trigger$1, ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData});
			} }
	...
	var DOMARR = {
		...
		splice: function splice(ref) {
			...
			if (args[2]) {
				...
				var insertItem = shared.toEFComponent(args[2]);
				insertItem.$mount({parent: ctx.state, key: key});
	...
	var create$2 = function (ref) {
		...
		var element = createElement$1({info: info, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, namespace: namespace, fragment: fragment, custom: custom});
		if (fragment && 'development' !== 'production') { element.append(DOM.document.createComment('<Fragment>')); }
		...
		for (var i = 0, list = childNodes; i < list.length; i += 1) {
			...
			else { resolveAST({node: node$1, nodeType: typeOf(node$1), element: element, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, namespace: namespace, create: create$2}); }
		}
		if (fragment && 'development' !== 'production') { element.append(DOM.document.createComment('</Fragment>')); }
	...
	var EFBaseComponent = /*@__PURE__*/(function () {
		...
		if ( scope === void 0 ) scope = {};
			...
			nodeInfo.element = create$2({node: ast, ctx: ctx, innerData: innerData, refs: refs, handlers: handlers, subscribers: subscribers, namespace: ''});
