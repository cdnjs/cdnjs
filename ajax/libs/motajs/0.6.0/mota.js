...
// Pass this if window is not defined yet
  ...
  */
  ...
  var eachAttr = function ( component , el , key , elemA , elemB ) {
    ...
    var i, className, eventName, style, styleRule, styleRules, prevStyle, newStyle;
    ...
    switch ( key ) {
      ...
      case "style":
        ...
        styleRules = motaKeys( newStyle );
        i = styleRules.length;

        while ( i-- ) {
          styleRule = styleRules[ i ];
          setStyle( style , styleRule , newStyle[ styleRule ] );
          // http://jsperf.com/for-delete-vs-for-undefined
          prevStyle[ styleRule ] = undefined;
        }

        styleRules = motaKeys( prevStyle );
        i = styleRules.length;

        while ( i-- ) {
          styleRule = styleRules[ i ];
          if ( prevStyle[ styleRule ] != null ) {
  ...
  update = function ( INIT ) {
    ...
    while ( i-- ) {
      ...
      eachAttr( this , thisEl , attrName ,
        prevAttrs[ attrName ] , ( newAttrs[ attrName ] = attrs[ attrName ] )
      );
      // http://jsperf.com/for-delete-vs-for-undefined
      prevAttrs[ attrName ] = undefined;
    }
