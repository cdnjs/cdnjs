...
( function( factory ) {
  ...
  var NOTHING = {};
  var NOTHING_FUNC = function() { return NOTHING; };
  ...
  extend( Collision.prototype, {
    ...
    modify: function( owner, shift, h, k, updater ) {
      ...
      exists = idx < len;
      value = updater.get( exists ? updater.oldValue : updater.notSetValue );
      removed = value === updater.notSetValue;
  ...
  extend( IndexedNode.prototype, {
    ...
    modify: function( owner, shift, h, k, updater ) {
      ...
      newChild = alter( child, owner, shift + SHIFT, h, k, updater, updater );
  ...
  MotaMapIterator.prototype[ ITERATOR_SYMBOL ] = function() {
    return this;
  };
  ...
  function MotaMapUpdater( func, notSetValue ) {
  ...
  MotaMapUpdater.prototype.get = function( oldValue ) {
    if ( this.resolved ) {
      return this.value;
    }
    this.resolved = true;
    return ( this.value = this.func( oldValue ) );
  };
  ...
  makeMap = function( mutable, size, root ) {
    var map = ObjectCreate( MotaMapProto );
    map.size = size || 0;
    map.__root = root;
  ...
  MotaMapProto = extend( MotaMap.prototype, events_main__events, {
    ...
    set: function( key, value ) {
      return this.update( key, function() { return value; }, NOTHING );
    },

    remove: function( key ) {
      return this.update( key, NOTHING_FUNC, NOTHING );
    },

    update: function( key, func, notSetValue, silent ) {

      var updater = new MotaMapUpdater( func, notSetValue ),
          isMutable = this.isMutable(),
          value, remove, map, change, changes;

      map = isMutable ? this : makeMap();
      map.__root = alter( this.__root, map.__owner, 0, hash( key ), key, updater );

      if ( !updater.changeValue ) {
        return silent === INIT ? updater : this;
      }

      value = updater.newValue;
      remove = value === updater.notSetValue;

      map.size = this.size + ( updater.changeSize ? remove ? -1 : 1 : 0 );
    ...
    merge: function( obj, silent ) {
      ...
      Map_main__forEach( obj, function( value, key ) {
        ...
        var updater, func = function() { return value; };

        if ( silent === INIT ) {

          updater = newMap.update( key, func, NOTHING, silent );
          ...
          newMap.update( key, func, NOTHING, true );
  ...
  MotaMapProto[ ITERATOR_SYMBOL ] = MotaMapProto.pairs = MotaMapProto.entries;
  ...
  defineProp( mota, "VERSION", "0.9.0", true );
