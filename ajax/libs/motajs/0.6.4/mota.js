...
// Pass this if window is not defined yet
  ...
  var key = "component",
  check = function ( el ) {
    return isElement( el ) || isDocument( el ) ? 1 : motaIsWindow( el ) ? 2 : 0;
  };
  ...
  update = function ( INIT ) {
    ...
    children = attrs.children || result.children,
    ...
    if ( voidElementTags[ type ] ) {
      if ( ( children != null && children.length > 0 ) || attrs.daugerousInnerHTML != null ) {
        err(
          "mota.view.Component.update",
    ...
    for ( ; i < len ; i++ ) {
      ...
      phType = placeholder.type || placeholder.tag;
      phProps = motaExtend( {} , placeholder.props || placeholder.attrs );
      phProps.children = motaExtend( [] , phProps.children || placeholder.children );
      phText = placeholder.text;

      key = phProps.key == null ? NOKEY + noKeyIdx++ : KEY + phProps.key;
  ...
  var render = function ( placeholder , parent ) {
    ...
    if ( !parentComponent || !isElement( parentComponent.el ) ) {
  ...
  MotaRouterRoute.prototype = {
    ...
    normalize: function ( fragment ) {
      ...
      return fragment ? fragment.replace( helper_rstripper , "/" ).replace( rleadslash , "" ) : "";
    },
  ...
  motaExtend( MotaRouter.prototype , Router_route.prototype , {
    ...
    start: function ( options ) {
      ...
      this.root = ( "/" + ( options && options.root || "" ) + "/" ).replace( helper_rstripper , "/" );
