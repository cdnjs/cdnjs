...
// Pass this if window is not defined yet
  ...
  var Model_constructor = MotaModel;
  ...
  Model_constructor.prototype.clear = clear;

  Model_constructor.prototype.clone = Model_clone;

  Model_constructor.prototype.get = get;

  Model_constructor.prototype.has = has;

  Model_constructor.prototype.set = Model_set;

  Model_constructor.prototype.setAll = setAll;

  Model_constructor.prototype.splice = Model_splice;

  Model_constructor.prototype.unset = Model_unset;

  Model_constructor.prototype.unsetAll = unsetAll;

  var Model = Model_constructor;
  ...
  var view_helper_data = {
    add: function ( component ) {
      var el = component.el, c = check( el );
      return c && helper_data.setup( el , key , component , c === 1 );
    },
    remove: function ( el ) {
      return check( el ) && helper_data.del( el , key );
    },
    get: function ( el ) {
      return check( el ) && helper_data.get( el , key );
    }
  };
  ...
  var eachAttr = function ( component , el , key , elemA , elemB ) {
    ...
    var i, len, className, eventName, style, styleRule, styleRules, newStyle;
    ...
    switch ( key ) {
      ...
      case "style":
        ...
        if ( motaIsString( elemB ) ) {
          if ( style.cssText != elemB ) {
            style.cssText = elemB;
          }
          return elemB;
        }
  ...
  var voidElementTags = {
    ...
    input: 1, keygen: 1, link: 1, menuitem: 1, meta: 1,
  ...
  update = function ( INIT ) {
    ...
    attrs    = result.props || result.attrs,
    ...
    placeholderType, placeholderProps,
    placeholderChildren, placeholderText,
    ...
    if ( voidElementTags[ type ] ) {
      if ( children != null || attrs.daugerousInnerHTML != null ) {
        err(
          "mota.view.Component.update",
    ...
    if ( voidElementTags[ type ] || ( attrs && attrs.daugerousInnerHTML != null ) ) {
      return;
    }
    ...
    for ( ; i < len ; i++ ) {
      ...
      placeholderType = placeholder.type || placeholder.tag;
      placeholderProps = placeholder.props || placeholder.attrs;
      placeholderChildren = placeholder.children;
      placeholderText = placeholder.text;

      key = placeholderProps ? placeholderProps.key : null;
      ...
      if ( cacheIndex != null ) {
        ...
        // !important
        if ( !isFunction( placeholder.type ) ) {
          prevComponent.props = {
          ...
          prevComponent.props = motaExtend( {} , placeholderProps );
        }
        ...
        if ( !isFunction( placeholder.type ) ) {
          ...
          component = new placeholder.type( placeholderProps );
        }
    ...
    while ( i-- ) {
      ...
      if ( !component || component.el ) {
        ...
        // Remove data
        view_helper_data.remove( prevComponent.el );
  ...
  if ( ROOT.addEventListener ) {
    ...
    var eventName, names, view_events__name, i;

    for ( view_events__name in ROOT ) {
      eventName = view_events__name.replace( helper_revent , "" );
      if ( view_events__name !== eventName ) {
  ...
  MotaRouterRoute.prototype = {
    ...
    normalize: function ( fragment ) {
      ...
      return fragment ? fragment.replace( Router_route__rstripper , "/" ).replace( rleadslash , "" ) : "";
    },
  ...
  var utilities = {
    ...
    isBoolean: isBoolean,
    isDate: isDate,
    isDocument: motaIsDocument,
    isElement: motaIsElement,
    isEmpty: isEmpty,
    isEqual: motaIsEqual,
    isError: isError,
  ...
  for ( main__name in core ) {
    mota[ main__name ] = core[ main__name ];
  }

  // Events

  mota.events = motaEvents;
  ...
  for ( main__name in utilities ) {
    mota[ main__name ] = utilities[ main__name ];
  }
