...
var motaType = function( obj ) {
  ...
  if ( type === "object" || type === "function" ) {
    return obj === null ? "null" : types[ _toString.call( obj ) ] || "object";
  }
  return type;
};
...
var isWindow = function( obj ) {
  ...
  var className = _toString.call( obj );
...
var isPlainObject = function( obj ) {
  ...
  if (
    ...
    _toString.call( obj ) !== "[object Object]" || has( obj, "constructor" )
  ) {
    return false;
  }
...
var isArguments = function( obj ) {
  return _toString.call( obj ) === "[object Arguments]";
};
...
var isElement = function( obj ) {
  ...
  if ( ISENVNODE ) {
    ...
    return _toString.call( obj ).indexOf( "Element" ) > -1;
...
var _err = function( funcName, msg ) {
...
var _getListeners = function( results, object, events, listener, action ) {
...
var _eventTypes = function( data, result ) {
...
var eventsApi = function( object, action, callback, data, arg ) {
  ...
  if ( action === "on" ) {
    ...
    for ( i = 0; i < types.length; i++ ) {
      _getListeners( eventListeners, object, events, types[ i ], action );
    }
...
function MotaViewUpdater() {
  ...
  this.dirtyComponents = new BinaryTree( _comparator );
...
MotaViewUpdater.prototype = {
  ...
  error: function( msg, owner ) {
    ...
    _err( "mota.view.Component.update", msg + errText );
  }

};

var _updater = new MotaViewUpdater();
...
var motaViewData = {
  add: function( component ) {
    ...
    if ( c ) {
      id = MotaData.get( el, __key );
      if ( !id ) {
        id = uid++;
        MotaData.setup( el, __key, id, c === 1 );
      }
      return ( _data[ id ] = component );
    }

  },
  remove: function( el ) {

    var id = check( el ) && MotaData.get( el, __key );
  ...
  get: function( el ) {
    return _data[ check( el ) && MotaData.get( el, __key ) ];
  }
};
...
var _once = function( eventType, handler, context ) {
...
var _forEach = function( obj, func, context, allowObject ) {
...
var flattenChildrenImpl = function( p, array, callback ) {
  ...
  if ( typeof p === "object" ) {
    return _forEach( p, callback, null, false );
  }
...
var flattenChildren = function( children, array ) {
...
var traverse = function(
  ...
  noKeyIdx, renderToString, depth
) {

  var i = 0, len = children.length,
  key, component, prevComponent,
  placeholder, pHTypeIsFunc, cacheIndex,
  parentEl = parentComponent.el;
  ...
  for ( ; i < len ; i++ ) {
    ...
    if ( cacheIndex !== undefined ) {
      ...
      component.__internal__.update(
        component,
        "update",
        pHTypeIsFunc ? placeholder.props : placeholder,
        null,
        parentEl,
        i
      );
      ...
      component.__internal__.update(
        ...
        "add",
        pHTypeIsFunc ? placeholder.props : placeholder,
        pHTypeIsFunc ? component : parentComponent.__internal__.owner,
        parentEl,
        i,
        renderToString,
        depth
...
var diffChildren = function(
  ...
  var newChildren = [], newChildrenMap = {}, noKeyIdx = {},
      flattenedChildren = [],
      depth = internal.depth + 1,
      prevComponent, i = prevChildren.length;

  flattenChildren( children, flattenedChildren );

  traverse(
    component, flattenedChildren,
    prevChildren, prevChildrenMap,
    newChildren, newChildrenMap,
    noKeyIdx, renderToString, depth
  );
  ...
  while ( i-- ) {
    ...
    if ( prevComponent ) {
      prevComponent.__internal__.update( prevComponent, "remove", null, null, component.el );
    }
  }
...
function isAttributeNameSafe( name ) {
  if ( validatedAttributeNameCache[ name ] === true ) {
    return true;
  }
  if ( illegalAttributeNameCache[ name ] === true ) {
    return false;
  }
...
var emptyStyle = document ? document.createElement( "div" ).style : {};
...
function setStyle( component, elem, _key, _prevValue, _value ) {
  ...
  if ( !key ) {
    if ( true ) {
      warn( "mota.view.Component.update", "`" + _key + "` is not a style property." );
    }
    return;
  }

  if ( _value == null || typeof _value === "boolean" || _value === "" ) {

    value = "";

  } else if ( _value === 0 || unitlessNumber[ key ] === 1 || isNaN( _value ) ) {
  ...
  if ( elem.style[ key ] !== value ) {
    _updater.queueAttrUpdate( elem, key, value, "style" );
  }

}

nativeKeys( unitlessNumber ).forEach( function( key ) {
  ...
  switch ( key ) {
    ...
    case "dangerouslySetInnerHTML":
      ...
      if ( elemB && el.innerHTML !== elemB.__html ) {
        _updater.queueAttrUpdate( el, "innerHTML", elemB.__html, "prop" );
      }
    ...
    case "className":
      ...
      if ( el.nodeName === "svg" ) {
        ...
        if ( el.getAttribute( "class" ) !== className ) {
          _updater.queueAttrUpdate( el, "class", className, "attr" );
        }

      } else {

        if ( el.className !== className ) {
          _updater.queueAttrUpdate( el, "className", className, "prop" );
        }
      ...
      if ( key !== eventName ) {
        ...
        if ( el[ key ] !== elemB ) {
          ...
          if ( key === "value" && el.nodeName === "SELECT" ) {
            _updater.queueEdgeCaseUpdate( el, key, elemB, "prop" );
          } else {
            _updater.queueAttrUpdate( el, key, elemB, "prop" );
          }
        ...
        if ( isAttributeNameSafe( key ) ) {
          _updater.queueAttrUpdate( el, key, elemB, "attr" );
        }
...
var updateComponent = function(
  component, action, owner, parentEl, index, renderToString, depth
) {

  var result, cache, thisEl, type, attrs, children;
  ...
  if ( action === "add" ) {
    ...
    component.el = createEl(
      type, attrs, parentEl.childNodes.item( index ), renderToString, this.owner
    );
  }
  ...
  if ( !thisEl ) {
    _updater.error( "The `el` property is missing.", this.owner );
  }
  ...
  if ( type === "#text" ) {
    return _updater.queueAttrUpdate( thisEl, "nodeValue", attrs.text, "prop" );
  }

  if ( type === "#comment" ) {
    return _updater.queueAttrUpdate( thisEl, "nodeValue", children.join( "\n" ), "prop" );
  }
...
var __update = function(
  component, action, nextProps, owner, parentEl, index, renderToString, depth
) {

  var prevProps, shouldUpdate;
  ...
  // Update component
  this.updateComponent( component, action, owner, parentEl, index, renderToString, depth );
  ...
  } else if ( action === "update" && parentEl ) {

    this.updater.queueElemMove( component.el, index, parentEl );
...
var MotaViewInternal = function() {
  ...
  this.children = frozenArr;
  this.childrenMap = frozenObj;
...
var _renderToString = function( placeholder ) {
...
var render = function( placeholder, parent, callback ) {
  ...
  if ( !parentComponent ) {
    _err( "mota.view.render", "`parent` should be an DOM element" );
  }
...
function MotaRouterRoute( name, path, callback, parent ) {
  ...
  if ( !path ) {
    _err( "mota.Router.route", "Provide a non-empty path" );
  }
...
MotaPromise.all = function( iterable ) {
  ...
  return new MotaPromise( function( resolve, reject ) {
    ...
    _forEach( iterable, function( val ) {
...
function _update( self, key, updater, silent ) {
...
var prototype = {
  ...
  set: function( key, value ) {
    ...
    updater.value = value;
    return _update( this, key, updater );
  },
  ...
  update: function( key, func, notSetValue ) {
    return _update( this, key, MotaMapUpdater( func, notSetValue ) );
  },
  ...
  merge: function( obj, silent ) {
    ...
    _forEach( obj, function( value, key ) {
      ...
      if ( silent === INIT ) {
        ...
        _update( newMap, key, updater, silent );
        ...
        _update( newMap, key, updater, true );
...
defineProp( mota, "VERSION", "0.10.0", true );
