...
function hashImmutableObj( obj ) {
  var h = 0;
  var size = obj.forEach( function( v, k ) {
...
History.prototype = {
  ...
  triggerRoutes: function( path ) {
    if ( this.router ) {
      this.router.handle( { url: path, method: "GET" }, {}, noop );
    }
  },
...
Router.prototype = {
  ...
  use: function( path, middleware ) {
    ...
    router.pattern = pathToRegExp( joinPaths( this.path, path ), {
      caseSensitive: this.caseSensitive,
      dot: this.dot,
      end: false
    } );
...
function setStyle( internal, elem, k, _prevValue, val ) {
  ...
  if ( !key ) {
    if ( true ) {
      warn( "mota.view.Component.update", "`" + k + "` is not a style property." );
    }
    return;
  }
