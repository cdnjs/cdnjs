...
( function( factory ) {
  ...
  helper_eventsApi__eventTypes = function( data ) {
    ...
    type, namespaces,
    isRegExp = motaIsRegExp( eventType );
  ...
  extend( Collision.prototype, {
    ...
    modify: function( owner, shift, h, k, v, changes ) {

      var removed = v === NOTHING,
          idx, len,
          exists, children,
      ...
      if ( h !== this.hash ) {
        ...
        changes.changeValue = true;
        changes.changeSize = true;
        return mergeLeaves( this, owner, shift, new Leaf( owner, h, k, v ) );
      }
      ...
      if ( exists ? child.value === v : removed ) {
        return this;
      }
      ...
      if ( exists ) {
        if ( removed ) {
          ...
          newChildren[ idx ] = new Leaf( owner, h, k, v );
        }
      } else {
        newChildren.push( new Leaf( owner, h, k, v ) );
      }
  ...
  var alter = function( child, owner, shift, h, k, v, changes ) {
    ...
    if ( !child ) {
      ...
      changes.changeValue = true;
      changes.changeSize = true;
      return new Leaf( owner, h, k, v );
    }

    return child.modify( owner, shift, h, k, v, changes );
  ...
  extend( ArrayNode.prototype, {
    ...
    modify: function( owner, shift, h, k, v, changes ) {
      ...
      newChild = alter( child, owner, shift + SHIFT, h, k, v, changes );
      if ( newChild === child ) {
        return this;
      }
  ...
  extend( Leaf.prototype, {
    ...
    modify: function( owner, shift, h, k, v, changes ) {
      ...
      if ( keyMatch ? this.value === v : removed ) {
        return this;
      }

      changes.changeValue = true;
      changes.oldValue = this.value;
      ...
      if ( keyMatch ) {
        ...
        return new Leaf( owner, this.hash, k, v );
  ...
  MotaMapPrototype = extend( MotaMap.prototype, events_main__events, {
    ...
    set: function( key, value, silent ) {
      ...
      var ref = { changeSize: false, changeValue: false },
          remove = value === NOTHING,
      ...
      if ( this.__root ) {
        ...
        map.__root = this.__root.modify( map.__owner, 0, hash( key ), key, value, ref );

      } else if ( !remove ) {

        ref.changeValue = true;
        ref.changeSize = true;
        map.__root = new Leaf( map.__owner, hash( key ), key, value );
    ...
    merge: function( obj, silent ) {
      ...
      Map_main__forEach( obj, function( value, key ) {
        ...
        if ( silent === INIT ) {
          ...
          if ( newMap.set( key, value, INIT ).changeValue ) {
            changed = true;
          }

        } else {

          newMap.set( key, value, true );
  ...
  var resolve = function( base, url ) {
    ...
    var i, p, baseArr, urlArr, isAbsolute;
    ...
    isAbsolute = url.pathname.charAt( 0 ) === "/";
  ...
  setStyle = function( style, _key, _value ) {
    ...
    if ( !key ) {
      return warn( "mota.view.Component.update", "`" + _key + "` is not a style property." );
    }
  ...
  var diffChildren = function(
    ...
    for ( ; i < len ; i++ ) {
      ...
      if ( cacheIndex != null ) {
        ...
        update.call( component, helper_INIT, thisElChildNodes[ index ] );
  ...
  setDataMota = function( el ) {
    if ( el && el.getAttribute ) {
      el.setAttribute( "data-mota", "client" );
    }
    return el;
  },

  exists = function( current, newEl ) {

    if ( !current || ( current.getAttribute && current.getAttribute( "data-mota" ) !== "server" ) ) {
      return setDataMota( newEl );
    }
  ...
  var update = function( ifInit, existingElem ) {

    if ( !this.__cache ) {
      this.__cache = {};
    }

    if ( ifInit === helper_INIT && this.componentWillMount ) {
    ...
    if ( ifInit === helper_INIT ) {
      ...
      if ( typeof type === "string" ) {
        ...
        thisEl = this.el = exists( existingElem, thisEl );
  ...
  var select = function( el ) {
    ...
    var component = motaViewData.get( el ) || new MotaViewComponent();
  ...
  var renderToString = function( placeholder ) {
    ...
    return parentComponent.el.innerHTML.replace( /data-mota="client"/g, "data-mota=\"server\"" );
