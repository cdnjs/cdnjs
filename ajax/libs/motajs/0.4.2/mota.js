...
mota.isDocument = function ( obj ) {
  ...
  if ( ISENVNODE ) {
    ...
    return mota.isWindow( obj.defaultView );
...
var getNext  = function ( current , event ) {
  if ( !current ) {
    return event.target;
  }
  ...
  if ( mota.isDocument( current ) ) {
    return current.defaultView;
  }
  return null;
},

eachTime = function ( event ) {
  ...
  while ( !event.isPropagationStopped && ( curTargetDOM = getNext( curTargetDOM , event ) ) ) {
    ...
    if ( !event.targetComponent ) {
      event.targetComponent = mota.view.select( event.target );
    }
