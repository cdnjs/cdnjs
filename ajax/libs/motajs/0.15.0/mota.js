...
var updaterQueueElemInsertion = function( compInternal ) {
  pendingElemInsertions.push( compInternal );
};

var updaterQueueElemMove = function( compInternal ) {
...
function baseEqual( a, b, customizer, key, aParent, bParent, stack ) {
  var comparison;
  if ( customizer ) {
    comparison = aParent ? customizer( a, b, key, aParent, bParent, stack ) : customizer( a, b );
...
var Subscription = function( subscriber, fn, _unsub ) {
  ...
  if ( !_unsub ) {
    ...
    try {
      cancel = initObservable( fn, observer );
    } catch ( e ) {
      error = e;
    }
    ...
    if ( currCompInternal && currCompInternal.key === key ) {
      ...
      if ( compInternal ) {
        ...
        if ( typeIsFunc ) {
          ...
          compInternal = component.__internal__ = new ViewInternal( component );
        } else {
          compInternal = new ViewInternal();
        }
...
var init = function( nextProps, parentInternal, index, key, renderToString ) {
  ...
  updaterQueueElemInsertion( this );
...
var createEl = function( type, attrs, currEl, renderToString, owner ) {
  ...
  if ( typeof type === "string" ) {
    ...
    if ( currEl && type === currEl.nodeName.toLowerCase() && !motaViewData.get( currEl ) ) {
      return currEl;
    }
...
function create$1( el ) {
  ...
  component.el = el;
  return motaViewData.add( el, component );
}
...
var renderToString = function( placeholder ) {
  ...
  var parentComponent = new ViewInternal();
  ...
  parentComponent.queueUpdate( true, true );
