...
ChildrenList.prototype = {
  ...
  remove: function( arr ) {
    var i = arr.length, componentInternal;
    while ( i-- ) {
      componentInternal = arr[ i ];
      if ( componentInternal ) {
        componentInternal.remove();
      }
    }
  },
  ...
  diffChildren: function( children, renderToString ) {
    ...
    for ( ; i < children.length; i++ ) {
      ...
      cacheIndex = prevMap && prevMap[ key ];
      ...
      if ( cacheIndex != null ) {
        ...
        componentInternal = prevArray[ cacheIndex ];
        prevArray[ cacheIndex ] = null;
        ...
        componentInternal = ( typeIsFunc ? new placeholder.type() : new DefaultClazz() ).__internal__;

        componentInternal.init(
          typeIsFunc ? placeholder.props : placeholder,
      ...
      this.array[ i ] = componentInternal;
      this.map[ key ] = i;

    }

    if ( prevArray ) {
      this.remove( prevArray );
    }
...
defineProp( mota, "VERSION", "0.10.2", true );
