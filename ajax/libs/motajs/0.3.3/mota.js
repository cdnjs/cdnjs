/*!
 * MotaJS v0.3.3
 * https://motapc97.github.io/motajs/
 *
 ...
 * that correspond with the type, namespaces, attrs and handler of that object
 *
 ...
 * attrs work in a different way: they all need to match
 *   e.g. if I trigger "change", it won't trigger "change:attr1"
 */
getListeners = function ( self , type , namespaces , attrs , handler ) {
  ...
  while ( i-- ) {
    ...
    if (
      ...
      // Remember, 'attrs' need always to match
      (
        (!attrs && !thisEvent.attrs) || // If was not provided and if event listener
        // doesn't have 'attrs', return true
        (attrs === true && thisEvent.attrs) ||
        m.isEqual( thisEvent.attrs || [] , attrs || [] )
      )
    ) {
...
this.__m__.events = [
  {
    ...
    attrs : [], // save the colons e.g. ":attr1",":attr2"
    handler : Function,
    data : {},
    index : 0
  }
...
m.events.off = function ( types , handler ) {
  ...
  var type, namespaces, attrs, t, eventIndexes, j, p = 0, noopArr = [];
  ...
  while ( t-- ) {
    ...
    attrs = types[ t ].match( rattrs );

    eventIndexes = getListeners( this , type , namespaces , attrs , handler );
...
m.events.on = function ( types , handler , data ) {
  ...
  while ( t-- ) {
    ...
    attrs = types[ t ].match( rattrs ) || undefined;
...
m.events.trigger = function ( types ) {
  ...
  var type, namespaces, attrs, t, eventIndexes, j,
  ...
  while ( t-- ) {
    ...
    eventIndexes = getListeners( this , type , namespaces , attrs );
...
diff_attrs_style = function ( el , prevStyle , newStyle ) {
  ...
  for ( key in prevStyle ) {
    ...
    if ( key in newStyle ) {
      ...
      el.style[ vendorPropName( key , el.style ) ] = undefined;
...
diff_dom_make = function ( component , elem , t , x , y , adjust ) {
  ...
  if ( elem === null ) { // !important (see render.js line 10)
    return;
  }

  var componentEl = component.el,
  componentElChildNodes = componentEl.childNodes,
  xx = x + adjust - 1, // !important (see render.js line 10)
...
var eachTime = function ( event ) {
  ...
  while (
    (curTargetDOM = start ? event.target : curTargetDOM.parentNode) &&
    ...
    if ( !curTargetCOMPONENT ) {
      // If we are in the start, and if the element has no component, fix the target
      if ( start ) {
        event.target = event.target.parentNode;
      }
      continue;
    ...
    // Only prevent if was not already prevented
    // and if the target has events to dispatch
...
(function ( doc ) {
  ...
  for ( name in document ) {
    if ( name.match( ron ) ) {
      doc.addEventListener( name.replace( ron , "" ) , eachTime );
    }
  }
})( document );
