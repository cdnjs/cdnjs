...
function equalObjects( a, b, customizer, stack ) {
  ...
  var aKeys = motaKeys( a );
  var bKeys = motaKeys( b );
...
Object.defineProperty( proto$2, "closed", {
  get: function() {
...
function insertChildAt( el, index, parentNode ) {
  ...
  var beforeChild = index >= childNodes.length ? null : childNodes.item( index );

  if ( el !== beforeChild ) {
    parentNode.insertBefore( el, beforeChild );
  }
}
...
function pendingElemDeletionsIteratee( elem ) {
  elem.parentNode.removeChild( elem );
}

function pendingElemInsertionsMovesIteratee( compInternal ) {
  compInternal.isNativeMounted = true;
  return insertChildAt( compInternal.el, compInternal.index, compInternal.parentEl );
}
...
// This method must only be called inside `updaterFlushUpdates`
...
function updaterQueueAttrUpdate( internal, name, value, type ) {
  var el = internal.el;
  if ( el.parentNode ) {
    if ( internal.root.isFirstRender && !needsUpdate( el, name, value, type ) ) {
      return;
    }
    queueAttrUpdate( pendingAttrUpdates, el, name, value, type );
  } else {
...
function updaterQueueElemDeletion( elem ) {
  pendingElemDeletions.push( elem );
}

function updaterQueueElemInsertion( compInternal ) {
  if ( compInternal.parentInternal.isNativeMounted ) {
    pendingElemInsertions.push( compInternal );
  } else {
    // Apply changes directly if the parent node is not in the DOM yet
    pendingElemInsertionsMovesIteratee( compInternal );
  }
}

function updaterQueueElemMove( compInternal ) {
  pendingElemMoves.push( compInternal );
}

function updaterQueueEdgeCaseUpdate( curr, name, value, type ) {
  queueAttrUpdate( pendingEdgeCaseUpdates, curr.el, name, value, type );
}

function updaterQueueCallback( component, name, arg ) {
...
function updaterQueueUpdate( internal ) {
  dirtyComponents.add( internal );
  updaterFlushUpdates();
}

function updaterStartIgnore() {
  ignore$1++;
}

function updaterEndIgnore() {
  ignore$1--;
  updaterFlushUpdates();
}
...
function updaterError( methodName, msg ) {
...
function init( type, nextProps, parentInternal, key, index ) {
  ...
  if ( !this.el ) {
    updaterError( "You must use `setType` inside the render method." );
  }
...
function update( _props ) {
  ...
  if ( this.type === "#text" || this.type === "#comment" ) {
    ...
    if ( this.el.nodeValue !== value ) {
      updaterQueueAttrUpdate( this, "nodeValue", value, "prop" );
    }
    return;
  }
...
ViewInternal.prototype = {
  ...
  get: function( index ) {
    ...
    return ( internal && internal.component ) || internal;
  },
...
function createEl( curr, type, props, currEl, renderToString ) {

  var namespace, doc, parentEl;

  if ( currEl ) {

    
    if ( type.toLowerCase() !== currEl.nodeName.toLowerCase() ) {
      updaterError( "The pre-rendered markup did not match the component being rendered." );
    }
...
function styleMutator( curr, newObj, prevStyle ) {
  ...
  if ( styleEmpty( newObj ) ) {
    if ( !styleEmpty( prevStyle ) ) {
      updaterQueueAttrUpdate( curr, "style", null, "attr" );
    }
    return;
  }
    ...
    if ( value ) {
      if ( prevStyle ) {
        if ( prevStyle[ name ] !== value ) {
          updaterQueueAttrUpdate( curr, name, value, "style" );
        }
  ...
  if ( prevStyle ) {
    for ( key in prevStyle ) {
      if ( prevStyle[ key ] ) {
        updaterQueueAttrUpdate( curr, key, "", "style" );
      }
    }
  }
...
var setAttribute = function( name, curr, val, prevValue ) {
  ...
  if ( value !== prevValue ) {
    updaterQueueAttrUpdate( curr, name, value, "attr" );
  }
...
var setSelectProp = function( name, curr, val, prevValue ) {
  ...
  if ( value !== prevValue ) {
    ...
    updaterQueueEdgeCaseUpdate( curr, name, value || "", "prop" );
  }
...
mutators = {
  ...
  _default: function( name, curr, val, prevValue ) {
    ...
    if ( el[ name ] !== undefined && !NOT_PROPS[ name ] ) {
      if ( val !== prevValue ) {
        updaterQueueAttrUpdate( curr, name, val, "prop" );
      }
      return val;
    }
  ...
  dangerousInnerHTML: function( curr, val, prevValue ) {
    ...
    if ( curr.isVoid ) {
      updaterError( curr.type + " is a void element and must not use dangerousInnerHTML." );
    }

    newHtml = normalizeStr( newHtml );

    if ( newHtml !== prevValue ) {
      updaterQueueAttrUpdate( curr, "innerHTML", newHtml || "", "prop" );
    }
    ...
    if ( nodeName === "INPUT" ) {
      ...
      if ( value !== prevValue ) {
        ...
        if ( curr.el.value !== str ) {
          updaterQueueAttrUpdate( curr, "value", str, "prop" );
        }
      }
      return value;
    }
...
function assertNotVoid() {
  if ( current.isVoid ) {
    updaterError( current.type + " is a void element." );
  }
}
...
function setCurrent( c ) {
  if ( c && current ) {
    updaterError( "Can't set a new context." );
  }
  current = c;
  if ( c ) {
    root$1 = c.root;
...
function elOpen( type, _key, props, isConst ) {

  var parent = current, array, map, i, typeIsFunction, key, noKeyIdx,
      freeSpot = 0, currCompInternal, compInternal, component;
  ...
  if ( !type ) {
    updaterError( "dom.open", "Provide a type." );
  }
  ...
  if ( compInternal ) {
    ...
    updaterQueueElemMove( compInternal );
    ...
    if ( typeIsFunction ) {
      /* eslint new-cap: 0 */
      component = new type();
      compInternal = component.__internal__ = new ViewInternal( root$1, component );
    } else {
      compInternal = new ViewInternal( root$1, null );
    }

    // Save in map
    map[ key ] = compInternal;

    current = compInternal;
    compInternal.init( type, props, parent, key, i );
...
function elConst( type, key, props ) {
  return elOpen( type, key, props, true );
}
...
function elClose() {
  ...
  if ( !curr.isNativeMounted ) {
    updaterQueueElemInsertion( curr );
  }
...
function elVoid( type, key, props ) {
  var component = elOpen( type, key, props );
  elClose();
  return component;
}
...
function attr( _name, value ) {
  ...
  var curr = current, name = _name, prevValue, newValue, prevAttrs, newAttrs, mutator;
  ...
  if ( curr.itIndex > 0 ) {
    updaterError( "dom.attr", "You can only call this before any .open() call." );
  }

  if ( !_name ) {
    updaterError( "dom.attr", "Provide a name." );
  }

  name = NORMALIZE_ATTR_NAME[ _name ] || _name;

  if ( NOT_ALLOWED[ name ] ) {
    updaterError( "dom.attr", "\"" + _name + "\" is not an accepted name." );
  }
  ...
  prevAttrs = curr.prevAttrs;
  newAttrs = curr.newAttrs || ( curr.newAttrs = new Blank() );

  mutator = mutators$1[ name ];
  prevValue = prevAttrs ? prevAttrs[ name ] : undefined;
...
function setType( type, props ) {
  ...
  if ( !curr.el ) {
    ...
    if ( typeof type !== "string" ) {
      updaterError( "dom.setType", "Provide a string." );
    }

    curr.type = type;
    curr.hasAttrs = type !== "#text" && type !== "#comment";
    curr.isVoid = VOID_ELEMENTS[ type ] || false;

    currEl = isFirstRender && curr.parentInternal.isNativeMounted && curr.parentEl.childNodes.item( curr.index );
    curr.el = createEl( curr, type, props, currEl, renderToString );
