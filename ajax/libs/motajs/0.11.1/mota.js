...
function hashImmutableObj( obj ) {
  ...
  var size = obj.forEach(
    keyed ?
      ordered ?
        function( v, k ) { h = 31 * h + hashMerge( hashCode( v ), hashCode( k ) ) | 0; } :
        function( v, k ) { h = h + hashMerge( hashCode( v ), hashCode( k ) ) | 0; } :
      ordered ?
        function( v ) { h = 31 * h + hashCode( v ) | 0; } :
        function( v ) { h = h + hashCode( v ) | 0; }
  );
...
var dispatchEvents = function( nativeEvent ) {
  ...
  var
  event = new ViewEvent( nativeEvent ),
  target = event.target,
  type = event.type,
  path = event.path,
  i = 0,
...
addNotEnumProp( proto, "multicast", function( observer ) {
  return new Multicast( this._subscriber );
} );
...
addNotEnumProp( proto, "map", function( fn ) {
  ...
  var self = this;
  var C = this.constructor[ getSymbol( "species" ) ] || this.constructor;
...
var Multicast = function( subscriber ) {
  this._state = PENDING;
  this._value = undefined;
  this._subs = new SubsList();
  this._subscription = null;
  Observable.call( this, subscriber );
};

Multicast.prototype = extend( ObjectCreate( Observable.prototype ), {
  ...
  subscribe: function( observer ) {
    ...
    if ( this._state === FULFILLED ) {
      ...
      sub = new Subscription( observer, null, function() {
        var subscription = self._subscription;
        var subs = self._subs;
        if ( subs ) {
          subs.remove( this );
          if ( subscription && !subs.length() ) {
  ...
  connect: function() {
    ...
    var observer = {
      next: function( v ) {
        self._subs.safeForEach( function( sub ) {
          sub._next( v );
        } );
      },
      error: function( e ) {
    ...
    this._subscription = this._subscription || new Subscription( observer, this._subscriber );
...
defineProp( mota, "VERSION", "0.11.1", true );
