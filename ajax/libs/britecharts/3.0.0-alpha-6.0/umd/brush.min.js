!(function (t, e) {
    'object' == typeof exports && 'object' == typeof module
        ? (module.exports = e(
              require('d3-time'),
              require('d3-shape'),
              require('d3-selection'),
              require('d3-format'),
              require('d3-array'),
              require('d3-scale'),
              require('d3-axis'),
              require('d3-time-format'),
              require('d3-transition'),
              require('d3-dispatch'),
              require('d3-brush')
          ))
        : 'function' == typeof define && define.amd
        ? define([
              'd3-time',
              'd3-shape',
              'd3-selection',
              'd3-format',
              'd3-array',
              'd3-scale',
              'd3-axis',
              'd3-time-format',
              'd3-transition',
              'd3-dispatch',
              'd3-brush',
          ], e)
        : 'object' == typeof exports
        ? (exports.brush = e(
              require('d3-time'),
              require('d3-shape'),
              require('d3-selection'),
              require('d3-format'),
              require('d3-array'),
              require('d3-scale'),
              require('d3-axis'),
              require('d3-time-format'),
              require('d3-transition'),
              require('d3-dispatch'),
              require('d3-brush')
          ))
        : ((t.britecharts = t.britecharts || {}),
          (t.britecharts.brush = e(
              t['d3-time'],
              t['d3-shape'],
              t['d3-selection'],
              t['d3-format'],
              t['d3-array'],
              t['d3-scale'],
              t['d3-axis'],
              t['d3-time-format'],
              t['d3-transition'],
              t['d3-dispatch'],
              t['d3-brush']
          )));
})(window, function (t, e, r, n, o, a, i, u, c, l, s) {
    return (function (t) {
        var e = {};
        function r(n) {
            if (e[n]) return e[n].exports;
            var o = (e[n] = { i: n, l: !1, exports: {} });
            return t[n].call(o.exports, o, o.exports, r), (o.l = !0), o.exports;
        }
        return (
            (r.m = t),
            (r.c = e),
            (r.d = function (t, e, n) {
                r.o(t, e) ||
                    Object.defineProperty(t, e, { enumerable: !0, get: n });
            }),
            (r.r = function (t) {
                'undefined' != typeof Symbol &&
                    Symbol.toStringTag &&
                    Object.defineProperty(t, Symbol.toStringTag, {
                        value: 'Module',
                    }),
                    Object.defineProperty(t, '__esModule', { value: !0 });
            }),
            (r.t = function (t, e) {
                if ((1 & e && (t = r(t)), 8 & e)) return t;
                if (4 & e && 'object' == typeof t && t && t.__esModule)
                    return t;
                var n = Object.create(null);
                if (
                    (r.r(n),
                    Object.defineProperty(n, 'default', {
                        enumerable: !0,
                        value: t,
                    }),
                    2 & e && 'string' != typeof t)
                )
                    for (var o in t)
                        r.d(
                            n,
                            o,
                            function (e) {
                                return t[e];
                            }.bind(null, o)
                        );
                return n;
            }),
            (r.n = function (t) {
                var e =
                    t && t.__esModule
                        ? function () {
                              return t.default;
                          }
                        : function () {
                              return t;
                          };
                return r.d(e, 'a', e), e;
            }),
            (r.o = function (t, e) {
                return Object.prototype.hasOwnProperty.call(t, e);
            }),
            (r.p = ''),
            r((r.s = 48))
        );
    })([
        function (e, r) {
            e.exports = t;
        },
        function (t, r) {
            t.exports = e;
        },
        function (t, e) {
            t.exports = r;
        },
        function (t, e) {
            t.exports = n;
        },
        function (t, e) {
            t.exports = o;
        },
        function (t, e, r) {
            'use strict';
            r.d(e, 'a', function () {
                return a;
            }),
                r.d(e, 'f', function () {
                    return i;
                }),
                r.d(e, 'd', function () {
                    return u;
                }),
                r.d(e, 'e', function () {
                    return c;
                }),
                r.d(e, 'b', function () {
                    return l;
                }),
                r.d(e, 'c', function () {
                    return s;
                }),
                r.d(e, 'g', function () {
                    return f;
                });
            var n = r(1),
                o = r(0),
                a = {
                    MINUTE_HOUR: 'minute-hour',
                    HOUR_DAY: 'hour-daymonth',
                    DAY_MONTH: 'day-month',
                    MONTH_YEAR: 'month-year',
                    CUSTOM: 'custom',
                },
                i = {
                    ONE_AND_A_HALF_YEARS: 47304e6,
                    ONE_YEAR: 31536000365,
                    ONE_DAY: 86400001,
                },
                u = [
                    '00h',
                    '01h',
                    '02h',
                    '03h',
                    '04h',
                    '05h',
                    '06h',
                    '07h',
                    '08h',
                    '09h',
                    '10h',
                    '11h',
                    '12h',
                    '13h',
                    '14h',
                    '15h',
                    '16h',
                    '17h',
                    '18h',
                    '19h',
                    '20h',
                    '21h',
                    '22h',
                    '23h',
                ],
                c = { duration: 1200 },
                l = {
                    linear: n.curveLinear,
                    basis: n.curveBasis,
                    cardinal: n.curveCardinal,
                    catmullRom: n.curveCatmullRom,
                    monotoneX: n.curveMonotoneX,
                    monotoneY: n.curveMonotoneY,
                    natural: n.curveNatural,
                    step: n.curveStep,
                    stepAfter: n.curveStepAfter,
                    stepBefore: n.curveStepBefore,
                },
                s = [{ quantity: 1, percentage: 100 }],
                f = {
                    timeMillisecond: o.timeMillisecond,
                    utcMillisecond: o.utcMillisecond,
                    timeSecond: o.timeSecond,
                    utcSecond: o.utcSecond,
                    timeMinute: o.timeMinute,
                    utcMinute: o.utcMinute,
                    timeHour: o.timeHour,
                    utcHour: o.utcHour,
                    timeDay: o.timeDay,
                    utcDay: o.utcDay,
                    timeWeek: o.timeWeek,
                    utcWeek: o.utcWeek,
                    timeSunday: o.timeSunday,
                    utcSunday: o.utcSunday,
                    timeMonday: o.timeMonday,
                    utcMonday: o.utcMonday,
                    timeTuesday: o.timeTuesday,
                    utcTuesday: o.utcTuesday,
                    timeWednesday: o.timeWednesday,
                    utcWednesday: o.utcWednesday,
                    timeThursday: o.timeThursday,
                    utcThursday: o.utcThursday,
                    timeFriday: o.timeFriday,
                    utcFriday: o.utcFriday,
                    timeSaturday: o.timeSaturday,
                    utcSaturday: o.utcSaturday,
                    timeMonth: o.timeMonth,
                    utcMonth: o.utcMonth,
                    timeYear: o.timeYear,
                    utcYear: o.utcYear,
                };
        },
        function (t, e, r) {
            'use strict';
            r.r(e),
                r.d(e, 'colorGradients', function () {
                    return n;
                }),
                r.d(e, 'colorGradientsHuman', function () {
                    return o;
                }),
                r.d(e, 'britecharts', function () {
                    return a;
                }),
                r.d(e, 'grey', function () {
                    return i;
                }),
                r.d(e, 'orange', function () {
                    return u;
                }),
                r.d(e, 'blueGreen', function () {
                    return c;
                }),
                r.d(e, 'teal', function () {
                    return l;
                }),
                r.d(e, 'green', function () {
                    return s;
                }),
                r.d(e, 'yellow', function () {
                    return f;
                }),
                r.d(e, 'pink', function () {
                    return d;
                }),
                r.d(e, 'purple', function () {
                    return p;
                }),
                r.d(e, 'red', function () {
                    return m;
                }),
                r.d(e, 'colorSchemas', function () {
                    return h;
                }),
                r.d(e, 'colorSchemasHuman', function () {
                    return g;
                }),
                r.d(e, 'aloeGreen', function () {
                    return y;
                }),
                r.d(e, 'greenColor', function () {
                    return b;
                }),
                r.d(e, 'blueColor', function () {
                    return x;
                }),
                r.d(e, 'yellowColor', function () {
                    return v;
                }),
                r.d(e, 'orangeColor', function () {
                    return M;
                });
            var n = {
                    greenBlue: ['#39C7EA', '#4CDCBA'],
                    orangePink: ['#FBC670', '#F766B8'],
                    bluePurple: ['#3DC3C9', '#824a9e'],
                },
                o = {
                    greenBlue: 'Green to Blue',
                    orangePink: 'Orange to Pink',
                    bluePurple: 'Blue to Purple',
                },
                a = [
                    '#6aedc7',
                    '#39c2c9',
                    '#ffce00',
                    '#ffa71a',
                    '#f866b9',
                    '#998ce3',
                ],
                i = [
                    '#F8F8FA',
                    '#EFF2F5',
                    '#D2D6DF',
                    '#C3C6CF',
                    '#ADB0B6',
                    '#666A73',
                    '#45494E',
                    '#363A43',
                    '#282C35',
                ],
                u = [
                    '#fcc870',
                    '#ffa71a',
                    '#fb8825',
                    '#f6682f',
                    '#db5a2c',
                    '#bf4c28',
                    '#a43b1c',
                    '#892a10',
                    '#f9e9c5',
                ],
                c = [
                    '#ccf7f6',
                    '#70e4e0',
                    '#00d8d2',
                    '#00acaf',
                    '#007f8c',
                    '#005e66',
                    '#003c3f',
                    '#002d2f',
                    '#0d2223',
                ],
                l = [
                    '#ccfffe',
                    '#94f7f4',
                    '#00fff8',
                    '#1de1e1',
                    '#39c2c9',
                    '#2e9a9d',
                    '#227270',
                    '#1a5957',
                    '#133f3e',
                ],
                s = [
                    '#edfff7',
                    '#d7ffef',
                    '#c0ffe7',
                    '#95f5d7',
                    '#6aedc7',
                    '#59c3a3',
                    '#479980',
                    '#34816a',
                    '#206953',
                ],
                f = [
                    '#f9f2b3',
                    '#fbe986',
                    '#fce05a',
                    '#fed72d',
                    '#ffce00',
                    '#fcc11c',
                    '#f9b438',
                    '#eda629',
                    '#e09819',
                ],
                d = [
                    '#fdd1ea',
                    '#fb9cd2',
                    '#f866b9',
                    '#fc40b6',
                    '#ff1ab3',
                    '#e3239d',
                    '#c62c86',
                    '#a62073',
                    '#85135f',
                ],
                p = [
                    '#ddd6fc',
                    '#bbb1f0',
                    '#998ce3',
                    '#8e6bc1',
                    '#824a9e',
                    '#77337f',
                    '#6b1c60',
                    '#591650',
                    '#470f3f',
                ],
                m = [
                    '#ffd8d4',
                    '#ffb5b0',
                    '#ff938c',
                    '#ff766c',
                    '#ff584c',
                    '#f04b42',
                    '#e03d38',
                    '#be2e29',
                    '#9c1e19',
                ],
                h = {
                    britecharts: a,
                    grey: i,
                    orange: u,
                    blueGreen: c,
                    teal: l,
                    green: s,
                    yellow: f,
                    pink: d,
                    purple: p,
                    red: m,
                },
                g = {
                    britecharts: 'Britecharts Default',
                    grey: 'Britecharts Grey',
                    orange: 'Orange',
                    blueGreen: 'Blue',
                    teal: 'Light Blue',
                    green: 'Green',
                    yellow: 'Yellow',
                    pink: 'Pink',
                    purple: 'Purple',
                    red: 'Red',
                },
                y = ['#7bdcc0'],
                b = ['#6aedc7'],
                x = ['#39c2c9'],
                v = ['#ffce00'],
                M = ['#ffa71a'],
                O = {
                    aloeGreen: y,
                    greenColor: b,
                    blueColor: x,
                    yellowColor: v,
                    orangeColor: M,
                    pinkColor: ['#f866b9'],
                    purpleColor: ['#998ce3'],
                };
            e.default = {
                colorSchemas: h,
                colorSchemasHuman: g,
                colorGradients: n,
                colorGradientsHuman: o,
                singleColors: O,
                singleColorsHuman: {
                    aloeGreen: 'Aloe Green',
                    greenColor: 'Green',
                    blueColor: 'Blue',
                    yellowColor: 'Yellow',
                    orangeColor: 'Orange',
                    pinkColor: 'Pink',
                    purpleColor: 'Purple',
                },
            };
        },
        function (t, e) {
            t.exports = a;
        },
        function (t, e) {
            (t.exports = function (t, e, r) {
                return (
                    e in t
                        ? Object.defineProperty(t, e, {
                              value: r,
                              enumerable: !0,
                              configurable: !0,
                              writable: !0,
                          })
                        : (t[e] = r),
                    t
                );
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e) {
            function r(e) {
                return (
                    'function' == typeof Symbol &&
                    'symbol' == typeof Symbol.iterator
                        ? ((t.exports = r = function (t) {
                              return typeof t;
                          }),
                          (t.exports.default = t.exports),
                          (t.exports.__esModule = !0))
                        : ((t.exports = r = function (t) {
                              return t &&
                                  'function' == typeof Symbol &&
                                  t.constructor === Symbol &&
                                  t !== Symbol.prototype
                                  ? 'symbol'
                                  : typeof t;
                          }),
                          (t.exports.default = t.exports),
                          (t.exports.__esModule = !0)),
                    r(e)
                );
            }
            (t.exports = r),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e, r) {
            var n = r(26),
                o = r(27),
                a = r(16),
                i = r(28);
            (t.exports = function (t) {
                return n(t) || o(t) || a(t) || i();
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e) {
            t.exports = i;
        },
        function (t, e) {
            (t.exports = function (t, e) {
                (null == e || e > t.length) && (e = t.length);
                for (var r = 0, n = new Array(e); r < e; r++) n[r] = t[r];
                return n;
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e, r) {
            var n = r(8);
            (t.exports = function (t) {
                for (var e = 1; e < arguments.length; e++) {
                    var r = null != arguments[e] ? Object(arguments[e]) : {},
                        o = Object.keys(r);
                    'function' == typeof Object.getOwnPropertySymbols &&
                        (o = o.concat(
                            Object.getOwnPropertySymbols(r).filter(function (
                                t
                            ) {
                                return Object.getOwnPropertyDescriptor(r, t)
                                    .enumerable;
                            })
                        )),
                        o.forEach(function (e) {
                            n(t, e, r[e]);
                        });
                }
                return t;
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        ,
        function (t, e) {
            t.exports = u;
        },
        function (t, e, r) {
            var n = r(12);
            (t.exports = function (t, e) {
                if (t) {
                    if ('string' == typeof t) return n(t, e);
                    var r = Object.prototype.toString.call(t).slice(8, -1);
                    return (
                        'Object' === r &&
                            t.constructor &&
                            (r = t.constructor.name),
                        'Map' === r || 'Set' === r
                            ? Array.from(t)
                            : 'Arguments' === r ||
                              /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r)
                            ? n(t, e)
                            : void 0
                    );
                }
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        ,
        function (t, e, r) {
            var n = r(33),
                o = r(34),
                a = r(16),
                i = r(35);
            (t.exports = function (t, e) {
                return n(t) || o(t, e) || a(t, e) || i();
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        ,
        ,
        ,
        function (t, e, r) {
            'use strict';
            r.d(e, 'a', function () {
                return l;
            }),
                r.d(e, 'b', function () {
                    return s;
                }),
                r.d(e, 'e', function () {
                    return f;
                }),
                r.d(e, 'c', function () {
                    return d;
                }),
                r.d(e, 'd', function () {
                    return p;
                }),
                r.d(e, 'f', function () {
                    return m;
                });
            var n = r(3),
                o = r(4),
                a = 0,
                i = {
                    small: { limit: 10, format: Object(n.format)('') },
                    medium: { limit: 1e3, format: Object(n.format)('') },
                    large: { limit: null, format: Object(n.format)('.2s') },
                },
                u = {
                    small: { limit: 10, format: Object(n.format)('.3f') },
                    medium: { limit: 100, format: Object(n.format)('.1f') },
                    large: { limit: null, format: Object(n.format)('.2s') },
                },
                c = function (t, e) {
                    var r = 'large';
                    return (
                        t < e.small.limit
                            ? (r = 'small')
                            : t < e.medium.limit && (r = 'medium'),
                        r
                    );
                },
                l = function (t) {
                    var e = t.length,
                        r = 0,
                        n = 0,
                        a = 0,
                        i = 0;
                    t.forEach(function (t) {
                        (r += t.x),
                            (n += t.y),
                            (a += t.x * t.y),
                            (i += t.x * t.x);
                    });
                    var u = e * i - r * r,
                        c = (n * i - r * a) / u,
                        l = (e * a - r * n) / u,
                        s = Object(o.min)(t, function (t) {
                            return t.x;
                        }),
                        f = Object(o.max)(t, function (t) {
                            return t.x;
                        });
                    return { x1: s, y1: l * e + c, x2: f, y2: l * f + c };
                },
                s = function (t, e, r) {
                    var o = e ? (t / e) * 100 : 0;
                    return Object(n.format)(r)(o);
                },
                f = function (t) {
                    return t % 1 == 0;
                },
                d = function (t) {
                    var e = c(t, u);
                    return (0, u[e].format)(t);
                },
                p = function (t) {
                    var e = c(t, i);
                    return (0, i[e].format)(t);
                },
                m = function (t) {
                    var e = ++a;
                    return ''.concat(t.toString(), '-').concat(e);
                };
        },
        function (t, e) {
            t.exports = c;
        },
        function (t, e) {
            t.exports = l;
        },
        function (t, e, r) {
            'use strict';
            r.r(e),
                r.d(e, 'linearGradient', function () {
                    return n;
                }),
                r.d(e, 'bar', function () {
                    return o;
                }),
                r.d(e, 'donut', function () {
                    return a;
                }),
                r.d(e, 'line', function () {
                    return i;
                }),
                r.d(e, 'stackedArea', function () {
                    return u;
                });
            var n =
                    '\n        <defs>\n            <linearGradient id="lgrad" x1="0%" y1="50%" x2="100%" y2="50%" >\n                <stop offset="0" stop-color="#ffffff" stop-opacity="0.8" />\n                <stop offset="33.33%" stop-color="#ffffff" stop-opacity="0.8" />\n                <stop offset="50%" stop-color="#ffffff" stop-opacity="0" />\n                <stop offset="66.66%" stop-color="#ffffff" stop-opacity="0.8" />\n                <stop offset="100%" stop-color="#ffffff" stop-opacity="0.8" />\n            </linearGradient>\n        </defs>\n    ',
                o = '\n        <svg class="load-state bar-load-state" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 711 325">\n            '.concat(
                    n,
                    '\n            <g fill="none" fill-rule="evenodd">\n                <g transform="translate(0 29)">\n                    <g stroke="#EFF2F5" stroke-dasharray="4 4">\n                        <path d="M.400592911 200.302477L710.674315 200.302477M.400592884 131.172748L710.674315 131.172748M.324410282 64.2071321L710.621499 64.2071321M.291004517.563888874L709.82431.563888889"/>\n                    </g>\n                    <g fill="#D2D6DF" transform="translate(63.08 11)">\n                        <polygon points="-.08 176 23.92 176 23.92 255 -.08 255"/>\n                        <polygon points="50.829 147 74.829 147 74.829 255 50.829 255"/>\n                        <polygon points="254.465 0 278.465 0 278.465 255 254.465 255"/>\n                        <polygon points="458.102 169 482.102 169 482.102 255 458.102 255"/>\n                        <polygon points="152.647 82 176.647 82 176.647 255 152.647 255"/>\n                        <polygon points="356.283 66 380.283 66 380.283 255 356.283 255"/>\n                        <polygon points="559.92 229 583.92 229 583.92 255 559.92 255"/>\n                        <polygon points="101.738 115 125.738 115 125.738 255 101.738 255"/>\n                        <polygon points="305.374 42 329.374 42 329.374 255 305.374 255"/>\n                        <polygon points="509.011 201 533.011 201 533.011 255 509.011 255"/>\n                        <polygon points="203.556 19 227.556 19 227.556 255 203.556 255"/>\n                        <polygon points="407.192 115 431.192 115 431.192 255 407.192 255"/>\n                    </g>\n                </g>\n                <polygon fill="#D2D6DF" fill-rule="nonzero" points="0 295 711 295 711 294 0 294"/>\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    '
                ),
                a = '\n        <svg class="load-state donut-load-state" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 376 331">\n            '.concat(
                    n,
                    '\n            <g fill="none" fill-rule="evenodd">\n                <g transform="translate(116 107)">\n                    <circle cx="72" cy="72" r="72" stroke="#EFF2F5" stroke-linecap="round" stroke-width="9.6"/>\n                    <path stroke="#D2D6DF" stroke-width="19.2" d="M126.153559,119.524055 C137.264629,106.845712 144,90.2321371 144,72.0444604 C144,32.2554036 111.764502,0 72,0"/>\n                    <circle cx="72" cy="72" r="67.2" fill="#FFF"/>\n                </g>\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    '
                ),
                i = '\n        <svg\n            class="load-state line-load-state"\n            xmlns="http://www.w3.org/2000/svg"\n            viewBox="0 0 711 325"\n        >\n            '.concat(
                    n,
                    '\n            <path\n                id="chart-bg"\n                class="chart-bg"\n                style="stroke:#C3C6CF;"\n                d="M3.4,216.5h707.3 M3.4,160.5h707.3 M3.3,103.5h707.3 M3.3,48.5h707.6 M0.4,276.6H710H0.4z"\n            />\n            <polyline\n                id="chart-line"\n                class="chart-line"\n                style="stroke:#C3C6CF;stroke-width:4;fill:none;"\n                points="8.8,175.8 62.4,237.7 116.1,184.7 169.7,175.8 223.3,57 277,176.8 330.6,176.8 384.3,122.5 437.9,176.8 491.6,176.8 545.2,218.4 598.8,122.5 652.5,184.7 706.1,135.1 "\n            />\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    '
                ),
                u = '\n        <svg\n            class="load-state stacked-area-load-state"\n            xmlns="http://www.w3.org/2000/svg"\n            viewBox="0 0 711 325"\n        >\n            '.concat(
                    n,
                    '\n            <path\n                id="chart-bg"\n                class="chart-bg"\n                style="stroke:#C3C6CF;"\n                d="M3.4,216.5h707.3 M3.4,160.5h707.3 M3.3,103.5h707.3 M3.3,48.5h707.6 M0.4,276.6H710H0.4z"\n            />\n            <g transform="translate(20 50)">\n                <path\n                    id="chart-area"\n                    strokeLinecap="square"\n                    d="M0.34233103,0.593688165 L709.977885,0.593688189"\n                    transform="translate(.01 227.976)"\n                />\n                <path fill="#C3C6CF" d="M0.528124801,224.014648 L0.528124801,177.734375 L53.3834796,177.734375 C71.5390789,177.734375 86.8277373,168.972754 101.240241,151.662202 C112.578335,138.044258 121.139826,123.110227 136.974507,91.596773 C137.343842,90.8617404 139.300293,86.9654028 139.856735,85.8583549 C155.041692,55.6476711 163.354313,41.0906306 174.319873,27.7179171 C188.951312,9.87459412 204.885845,0.5 223.830634,0.5 C242.123071,0.5 257.291724,8.27027858 270.907992,23.1359289 C281.228683,34.4036118 289.135925,47.1272372 302.542017,72.085092 C303.275893,73.4513345 306.289669,79.0766612 307.063369,80.5168656 C321.41025,107.222876 330.088083,120.97663 341.470704,132.92446 C355.88994,148.05969 371.908861,155.792969 391.654853,155.792969 C412.142049,155.792969 428.763593,152.325614 442.880698,145.765582 C454.197328,140.506893 463.373931,133.679865 473.786035,123.626931 C476.528659,120.978915 486.44777,110.911455 488.791866,108.6483 C502.907223,95.0203436 514.194325,88.9355469 530.135322,88.9355469 C546.532652,88.9355469 559.505909,97.338499 575.973261,115.41103 C579.723508,119.526837 593.103621,135.086814 592.915496,134.871799 C605.09738,148.794859 614.368835,157.635549 625.072091,164.58539 C638.386599,173.230769 652.701021,177.734375 669.279853,177.734375 L673.779853,177.734375 L673.779853,224.014648 L0.528124801,224.014648 Z" />\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    '
                );
            e.default = {
                linearGradient: n,
                bar: o,
                donut: a,
                line: i,
                stackedArea: u,
            };
        },
        function (t, e, r) {
            var n = r(12);
            (t.exports = function (t) {
                if (Array.isArray(t)) return n(t);
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e) {
            (t.exports = function (t) {
                if (
                    'undefined' != typeof Symbol &&
                    Symbol.iterator in Object(t)
                )
                    return Array.from(t);
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e) {
            (t.exports = function () {
                throw new TypeError(
                    'Invalid attempt to spread non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.'
                );
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        ,
        ,
        ,
        function (t, e, r) {
            'use strict';
            r.d(e, 'a', function () {
                return o;
            }),
                r.d(e, 'c', function () {
                    return a;
                }),
                r.d(e, 'b', function () {
                    return i;
                }),
                r.d(e, 'd', function () {
                    return u;
                });
            var n = {
                    minute: { minute: 'numeric' },
                    hour: { hour: 'numeric' },
                    day: { day: 'numeric' },
                    daymonth: { day: 'numeric', month: 'short' },
                    month: { month: 'short' },
                    year: { year: 'numeric' },
                },
                o = function (t, e) {
                    var r = new Date(t);
                    return r.setDate(r.getDate() + e), String(r);
                },
                a = function (t, e) {
                    return Math.ceil(
                        Math.abs(
                            (new Date(t).getTime() - new Date(e).getTime()) /
                                864e5
                        )
                    );
                },
                i = function (t) {
                    return Math.ceil(t / 864e5);
                },
                u = function (t) {
                    var e =
                            arguments.length > 1 && void 0 !== arguments[1]
                                ? arguments[1]
                                : 'day',
                        r = n[e],
                        o = new Intl.DateTimeFormat(t, r);
                    return function (t) {
                        return o.format(t);
                    };
                };
        },
        function (t, e) {
            (t.exports = function (t) {
                if (Array.isArray(t)) return t;
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e) {
            (t.exports = function (t, e) {
                if (
                    'undefined' != typeof Symbol &&
                    Symbol.iterator in Object(t)
                ) {
                    var r = [],
                        n = !0,
                        o = !1,
                        a = void 0;
                    try {
                        for (
                            var i, u = t[Symbol.iterator]();
                            !(n = (i = u.next()).done) &&
                            (r.push(i.value), !e || r.length !== e);
                            n = !0
                        );
                    } catch (t) {
                        (o = !0), (a = t);
                    } finally {
                        try {
                            n || null == u.return || u.return();
                        } finally {
                            if (o) throw a;
                        }
                    }
                    return r;
                }
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        function (t, e) {
            (t.exports = function () {
                throw new TypeError(
                    'Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.'
                );
            }),
                (t.exports.default = t.exports),
                (t.exports.__esModule = !0);
        },
        ,
        ,
        ,
        ,
        ,
        function (t, e, r) {
            'use strict';
            r.d(e, 'c', function () {
                return b;
            }),
                r.d(e, 'b', function () {
                    return x;
                });
            var n,
                o = r(18),
                a = r.n(o),
                i = r(9),
                u = r.n(i),
                c = r(8),
                l = r.n(c),
                s = r(0),
                f = r(15),
                d = r(5),
                p = r(32),
                m = {
                    minute: Object(f.timeFormat)('%M m'),
                    hour: Object(f.timeFormat)('%H %p'),
                    day: Object(f.timeFormat)('%e'),
                    daymonth: Object(f.timeFormat)('%d %b'),
                    month: Object(f.timeFormat)('%b'),
                    year: Object(f.timeFormat)('%Y'),
                },
                h =
                    ((n = {}),
                    l()(n, d.a.MINUTE_HOUR, s.timeHour.every(1)),
                    l()(n, d.a.HOUR_DAY, s.timeDay.every(1)),
                    l()(n, d.a.DAY_MONTH, s.timeMonth.every(1)),
                    l()(n, d.a.MONTH_YEAR, s.timeYear.every(1)),
                    n),
                g = function (t) {
                    var e = d.f.ONE_YEAR;
                    return t < d.f.ONE_DAY
                        ? d.a.HOUR_DAY
                        : t < e
                        ? d.a.DAY_MONTH
                        : d.a.MONTH_YEAR;
                },
                y = function (t, e) {
                    var r = Math.ceil(t / 70);
                    return e < 5 ? s.timeDay : Math.min(e, r);
                },
                b = function (t, e) {
                    var r =
                            arguments.length > 2 && void 0 !== arguments[2]
                                ? arguments[2]
                                : null,
                        n =
                            arguments.length > 3 && void 0 !== arguments[3]
                                ? arguments[3]
                                : null,
                        o = new Date(t[0].date),
                        i = new Date(t[t.length - 1].date),
                        c = i - o;
                    n &&
                        ('undefined' == typeof Intl ||
                            ('object' ===
                                ('undefined' == typeof Intl
                                    ? 'undefined'
                                    : u()(Intl)) &&
                                !Intl.DateTimeFormat)) &&
                        (n = null),
                        r || (r = g(c));
                    var l = r.split('-'),
                        s = a()(l, 2),
                        f = s[0],
                        d = s[1],
                        b = h[r],
                        x = y(e, Object(p.b)(c));
                    return {
                        minor: {
                            format: n ? Object(p.d)(n, f) : m[f],
                            tick: x,
                        },
                        major: {
                            format: n ? Object(p.d)(n, d) : m[d],
                            tick: b,
                        },
                    };
                },
                x = function (t, e) {
                    var r = t[0].date;
                    return {
                        tick: (function (t, e) {
                            var r = Math.ceil(t / 70);
                            return Math.min(e, r);
                        })(e, t[t.length - 1].date - r),
                    };
                };
            e.a = { getTimeSeriesAxis: b, getSortedNumberAxis: x };
        },
        ,
        ,
        function (t, e) {
            t.exports = s;
        },
        ,
        ,
        ,
        function (t, e, r) {
            'use strict';
            r.r(e),
                r.d(e, 'default', function () {
                    return t;
                });
            var n = r(13),
                o = r.n(n),
                a = r(10),
                i = r.n(a),
                u = r(4),
                c = r(11),
                l = r(44),
                s = r(7),
                f = r(1),
                d = r(24),
                p = r(2),
                m = r(15),
                h = (r(23), r(6)),
                g = r(41),
                y = r(5),
                b = r(22),
                x = r(25);
            function t() {
                var t,
                    e,
                    r,
                    n,
                    a,
                    v,
                    M,
                    O,
                    C,
                    w,
                    j,
                    S,
                    _,
                    A,
                    D = { top: 20, right: 20, bottom: 30, left: 20 },
                    k = 960,
                    F = 500,
                    T = x.line,
                    B = !1,
                    H = y.e.duration,
                    E = [null, null],
                    q = !1,
                    Y = null,
                    G = null,
                    N = null,
                    P = 'monotoneX',
                    L = h.default.colorGradients.greenBlue,
                    R = Object(b.f)('brush-area-gradient'),
                    I = 'timeDay',
                    U = Object(d.dispatch)(
                        'customBrushStart',
                        'customBrushEnd'
                    ),
                    W = function (t) {
                        return t.value;
                    },
                    z = function (t) {
                        return t.date;
                    };
                function X(d) {
                    d.each(function (d) {
                        var b, x;
                        (n = k - D.left - D.right),
                            (a = F - D.top - D.bottom),
                            (t = (function (t) {
                                var r = t.reduce(function (t, e) {
                                    var r;
                                    return (
                                        (e.date = new Date(e.date)),
                                        (e.value =
                                            null === (r = e.value) ? null : +r),
                                        [].concat(i()(t), [e])
                                    );
                                }, []);
                                return (
                                    (e = r.map(function (t) {
                                        return o()({}, t, {
                                            value: null === t.value ? null : 0,
                                        });
                                    })),
                                    r
                                );
                            })(((b = d), JSON.parse(JSON.stringify(b))))),
                            (v = Object(s.scaleTime)()
                                .domain(Object(u.extent)(t, z))
                                .range([0, n])),
                            (M = Object(s.scaleLinear)()
                                .domain([0, Object(u.max)(t, W)])
                                .range([a, 0])),
                            (function () {
                                var e, r;
                                if ('custom' === Y && 'string' == typeof N)
                                    (e = {
                                        tick: G,
                                        format: Object(m.timeFormat)(N),
                                    }),
                                        (r = null);
                                else {
                                    var n = g.a.getTimeSeriesAxis(t, k, Y);
                                    (e = n.minor),
                                        (r = n.major),
                                        (C = Object(c.axisBottom)(v)
                                            .ticks(r.tick)
                                            .tickSize(0, 0)
                                            .tickFormat(r.format));
                                }
                                O = Object(c.axisBottom)(v)
                                    .ticks(e.tick)
                                    .tickSize(10, 0)
                                    .tickPadding([5])
                                    .tickFormat(e.format);
                            })(),
                            (function (t) {
                                r ||
                                    ((r = Object(p.select)(t)
                                        .append('svg')
                                        .classed('britechart brush-chart', !0)),
                                    (function () {
                                        var t = r
                                            .append('g')
                                            .classed('container-group', !0)
                                            .attr(
                                                'transform',
                                                'translate('
                                                    .concat(D.left, ', ')
                                                    .concat(D.top, ')')
                                            );
                                        t
                                            .append('g')
                                            .classed('chart-group', !0),
                                            t
                                                .append('g')
                                                .classed('x-axis-group', !0)
                                                .append('g')
                                                .classed('x axis', !0),
                                            t
                                                .selectAll('.x-axis-group')
                                                .append('g')
                                                .classed('axis sub-x', !0),
                                            t
                                                .append('g')
                                                .classed('brush-group', !0),
                                            t
                                                .append('g')
                                                .classed('metadata-group', !0);
                                    })());
                                r.attr('width', k).attr('height', F);
                            })(this),
                            A ||
                                (A = r
                                    .select('.metadata-group')
                                    .append('linearGradient')
                                    .attr('id', R)
                                    .attr('gradientUnits', 'userSpaceOnUse')
                                    .attr('x1', 0)
                                    .attr('x2', v(t[t.length - 1].date))
                                    .attr('y1', 0)
                                    .attr('y2', 0)
                                    .selectAll('stop')
                                    .data([
                                        { offset: '0%', color: L[0] },
                                        { offset: '100%', color: L[1] },
                                    ])
                                    .enter()
                                    .append('stop')
                                    .attr('offset', function (t) {
                                        return t.offset;
                                    })
                                    .attr('stop-color', function (t) {
                                        return t.color;
                                    })),
                            (j = Object(l.brushX)()
                                .extent([
                                    [0, 0],
                                    [n, a],
                                ])
                                .on('brush', J)
                                .on('end', Z)),
                            (function () {
                                _ &&
                                    (r.selectAll('.brush-area').remove(),
                                    r
                                        .selectAll('.missing-brush-area')
                                        .remove());
                                (_ = Object(f.area)()
                                    .defined(function (t) {
                                        var e = t.value;
                                        return !isNaN(parseInt(e, 10));
                                    })
                                    .x(function (t) {
                                        var e = t.date;
                                        return v(e);
                                    })
                                    .y0(a)
                                    .y1(function (t) {
                                        var e = t.value;
                                        return M(e);
                                    })
                                    .curve(y.b[P])),
                                    B
                                        ? (e.filter(_.defined()).length !==
                                              e.length &&
                                              (r
                                                  .select('.chart-group')
                                                  .append('path')
                                                  .datum(e.filter(_.defined()))
                                                  .attr(
                                                      'class',
                                                      'missing-brush-area'
                                                  )
                                                  .attr('d', _),
                                              r
                                                  .select('.chart-group')
                                                  .selectAll(
                                                      '.missing-brush-area'
                                                  )
                                                  .datum(t.filter(_.defined()))
                                                  .transition()
                                                  .duration(H)
                                                  .attr('d', _)),
                                          r
                                              .select('.chart-group')
                                              .append('path')
                                              .datum(e)
                                              .attr('class', 'brush-area')
                                              .attr('d', _),
                                          r
                                              .select('.chart-group')
                                              .selectAll('.brush-area')
                                              .datum(t)
                                              .transition()
                                              .duration(H)
                                              .attr('d', _))
                                        : (t.filter(_.defined()).length !==
                                              t.length &&
                                              r
                                                  .select('.chart-group')
                                                  .append('path')
                                                  .datum(t.filter(_.defined()))
                                                  .attr(
                                                      'class',
                                                      'missing-brush-area'
                                                  )
                                                  .attr('d', _),
                                          r
                                              .select('.chart-group')
                                              .append('path')
                                              .datum(t)
                                              .attr('class', 'brush-area')
                                              .attr('d', _));
                            })(),
                            r
                                .select('.x-axis-group .axis.x')
                                .attr(
                                    'transform',
                                    'translate(0, '.concat(a, ')')
                                )
                                .call(O),
                            'custom' !== Y &&
                                r
                                    .select('.x-axis-group .axis.sub-x')
                                    .attr(
                                        'transform',
                                        'translate(0, '.concat(a + 30, ')')
                                    )
                                    .call(C),
                            (function () {
                                (S = r.select('.brush-group').call(j)),
                                    B &&
                                        (S.style('opacity', 0),
                                        setTimeout(function () {
                                            S.transition()
                                                .duration(H)
                                                .style('opacity', 1);
                                        }, 0));
                                S.selectAll('rect')
                                    .classed('brush-rect', !0)
                                    .attr('height', a),
                                    S.selectAll('.selection').attr(
                                        'fill',
                                        'url(#'.concat(R, ')')
                                    );
                            })(),
                            (x = h.default.colorSchemasHuman.grey[1]),
                            S.selectAll('.handle.brush-rect').style('fill', x);
                    });
                }
                function J() {
                    var t,
                        e = p.event.selection;
                    if (e) {
                        if (q) {
                            var r = Math.floor(
                                    v(new Date(E[1])) - v(new Date(E[0]))
                                ),
                                n = Math.floor(e[1] - e[0]);
                            n < r || n > r
                                ? ((t = [
                                      Math.floor(e[0]),
                                      Math.floor(e[0]) + r,
                                  ]),
                                  j.move(S, t))
                                : (t = e);
                        } else t = e;
                        U.call('customBrushStart', this, t.map(v.invert));
                    }
                }
                function Z() {
                    if (p.event.sourceEvent) {
                        var t = [null, null],
                            e = p.event.selection;
                        if (e) {
                            var r = e.map(v.invert);
                            (t = r.map(y.g[I].round))[0] >= t[1] &&
                                ((t[0] = y.g[I].floor(r[0])),
                                (t[1] = y.g[I].offset(t[0]))),
                                Object(p.select)(this)
                                    .transition()
                                    .call(p.event.target.move, t.map(v));
                        } else q && $.apply(void 0, i()(E));
                        U.call('customBrushEnd', this, t);
                    }
                }
                function $(t, e) {
                    var r = null;
                    null !== t &&
                        (new Date(t) < new Date(e)
                            ? (r = [v(new Date(t)), v(new Date(e))])
                            : console.error(
                                  'dateRange Error: End date should be posterior to startDate!'
                              )),
                        j.move(S, r);
                }
                return (
                    (X.animationDuration = function (t) {
                        return arguments.length ? ((H = t), this) : H;
                    }),
                    (X.areaCurve = function (t) {
                        return arguments.length ? ((P = t), this) : P;
                    }),
                    (X.axisTimeCombinations = y.a),
                    (X.dateRange = function (t) {
                        return arguments.length
                            ? ((E = t),
                              Array.isArray(E) && $.apply(void 0, i()(E)),
                              this)
                            : E;
                    }),
                    (X.gradient = function (t) {
                        return arguments.length ? ((L = t), this) : L;
                    }),
                    (X.height = function (t) {
                        return arguments.length ? ((F = t), this) : F;
                    }),
                    (X.isAnimated = function (t) {
                        return arguments.length ? ((B = t), this) : B;
                    }),
                    (X.isLocked = function (t) {
                        return arguments.length ? ((q = t), this) : q;
                    }),
                    (X.loadingState = function (t) {
                        return arguments.length ? ((T = t), this) : T;
                    }),
                    (X.locale = function (t) {
                        return arguments.length ? ((w = t), this) : w;
                    }),
                    (X.margin = function (t) {
                        return arguments.length
                            ? ((D = o()({}, D, t)), this)
                            : D;
                    }),
                    (X.on = function () {
                        var t = U.on.apply(U, arguments);
                        return t === U ? X : t;
                    }),
                    (X.width = function (t) {
                        return arguments.length ? ((k = t), this) : k;
                    }),
                    (X.xAxisCustomFormat = function (t) {
                        return arguments.length ? ((N = t), this) : N;
                    }),
                    (X.xAxisFormat = function (t) {
                        return arguments.length ? ((Y = t), this) : Y;
                    }),
                    (X.xTicks = function (t) {
                        return arguments.length ? ((G = t), this) : G;
                    }),
                    (X.roundingTimeInterval = function (t) {
                        return arguments.length ? ((I = t), this) : I;
                    }),
                    X
                );
            }
        },
    ]).default;
});
//# sourceMappingURL=brush.min.js.map
