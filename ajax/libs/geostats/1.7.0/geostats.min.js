...
function(a){if(!this._nodata()){var b=this.sorted(),c=this.getQuantiles(a);c.unshift(b[0]);c[b.length-1]!==b[b.length-1]&&c.push(b[b.length-1]);this.setBounds(c);this.setRanges();this.method="quantile ("+a+" classes)";return this.bounds}};this.getClassStdDeviation=function(a,b){if(!this._nodata()){var c=this.max(),d=this.min(),e=this.stddev(),g=this.mean(),f=[];if(1==a%2){var l=Math.floor(a/2),h=l+1;f[l]=g-e/2;f[h]=g+e/2;for(i=l-1;0<i;i--)g=f[i+1]-e,f[i]=g;i=h+1}else{h=a/2;f[h]=g;for(i=h-1;0<i;i--)g=
f[i+1]-e,f[i]=g;i=h+1}for(;i<a;i++)g=f[i-1]+e,f[i]=g;f[0]="undefined"===typeof b?f[1]-e:d;f[a]="undefined"===typeof b?f[a-1]+e:c;this.setBounds(f);this.setRanges();this.method="std deviation ("+a+" classes)";return this.bounds}};this.getClassGeometricProgression=function(a){if(!this._nodata())if(this._hasNegativeValue()||this._hasZeroValue())if(this.silent)this.log("[silent mode] geometric progression can't be applied with a serie containing negative or zero values.",!0);else throw new TypeError("geometric progression can't be applied with a serie containing negative or zero values.");
