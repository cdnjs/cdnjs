...
// UNUSED EXPORTS: OverlayAPI, class2job, isCEFSharp, job2class, mergeCombatant
...
;// CONCATENATED MODULE: ./src/utils/getter.ts
...
;// CONCATENATED MODULE: ./src/modules/injectExtendData.ts
...
function parsePlayer(data) {
  ...
  return {
    ...
    shieldPct: `${Math.round(
      (getInt(data.damageShield) / getInt(data.healed) || 0) * getPctNum(data["healed%"] || "")
    ) || 0}%`,
    maxHit,
    maxHitDamage,
    maxHeal,
...
function injectExtendData(data) {
  if (data.type === "CombatData") {
    ...
    const combatantValidKeys = combatantKeys.filter(
      (key) => Object.prototype.hasOwnProperty.call(data.Combatant, key)
    );
...
/* harmony default export */ const modules_injectExtendData = (injectExtendData);

;// CONCATENATED MODULE: ./src/modules/isCEFSharp.ts
function isCEFSharp() {
  return !!window.OverlayPluginApi;
}
/* harmony default export */ const modules_isCEFSharp = (isCEFSharp);

;// CONCATENATED MODULE: ./src/utils/adder.ts
...
;// CONCATENATED MODULE: ./src/modules/mergeCombatant.ts
...
/* harmony default export */ const modules_mergeCombatant = (mergeCombatant);

;// CONCATENATED MODULE: ./src/overlay.ts
...
const _OverlayAPI = class {
  constructor() {
    ...
    this._queue = [];
    this._wsURL = Array.from(
      /[?&]OVERLAY_WS=([^&]+)/.exec(window.location.href) || /[?&]HOST_PORT=([^&]+)/.exec(window.location.href) || []
    )[1] || "";
    this._ws = null;
    this._rseqCounter = 0;
  ...
  _triggerEvents(data) {
    if (this._eventCenter[data.type]) {
      for (const cb of this._eventCenter[data.type].listeners) {
        cb(modules_injectExtendData(data));
      }
    }
  }
  _sendMessage(msg, cb) {
  ...
  endEncounter() {
    return __async(this, null, function* () {
      if (this._status && modules_isCEFSharp()) {
...
let OverlayAPI = _OverlayAPI;
OverlayAPI._instance = null;
OverlayAPI.mergeCombatant = modules_mergeCombatant;
OverlayAPI.isCEFSharp = modules_isCEFSharp;
/* harmony default export */ const overlay = (OverlayAPI);

;// CONCATENATED MODULE: ./src/modules/jobClassConversion.ts
const classJobMap = {
  acn: "smn",
  arc: "brd",
  lnc: "drg",
...
/* harmony default export */ const src = (overlay);
