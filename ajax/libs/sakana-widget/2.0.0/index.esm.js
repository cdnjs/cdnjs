...
var _baseIsTypedArray_argsTag = "[object Arguments]", arrayTag = "[object Array]", boolTag = "[object Boolean]", dateTag = "[object Date]", errorTag = "[object Error]", _baseIsTypedArray_funcTag = "[object Function]", mapTag = "[object Map]", numberTag = "[object Number]", objectTag = "[object Object]", regexpTag = "[object RegExp]", setTag = "[object Set]", stringTag = "[object String]", weakMapTag = "[object WeakMap]";
...
function arrayLikeKeys(value, inherited) {
  ...
  for (var key in value) {
    if ((inherited || _arrayLikeKeys_hasOwnProperty.call(value, key)) && !(skipIndexes && (key == "length" || isBuff && (key == "offset" || key == "parent") || isType && (key == "buffer" || key == "byteLength" || key == "byteOffset") || _isIndex(key, length)))) {
      result.push(key);
    }
  }
  return result;
...
function assignMergeValue(object, key, value) {
  if (value !== void 0 && !lodash_es_eq(object[key], value) || value === void 0 && !(key in object)) {
    _baseAssignValue(object, key, value);
  }
}
...
Object.keys(characters).forEach((key) => {
  ...
  _characters[key] = lodash_es_cloneDeep(_char);
});
class SakanaWidget {
  constructor(options = {}) {
    ...
    this._options = lodash_es_cloneDeep(
      defaultOptions
    );
    lodash_es_merge(this._options, options);
  ...
  static registerCharacter(name, character) {
    _characters[name] = lodash_es_cloneDeep(character);
  }
  _updateLimit(size) {
    let maxR = size / 5;
    if (maxR < 30) {
  ...
  setState(state) {
    ...
    lodash_es_merge(this._state, lodash_es_cloneDeep(state));
    return this;
  }
  setCharacter(name) {
