...
wheat:"f5deb3",white:"fff",whitesmoke:"f5f5f5",yellow:"ff0",yellowgreen:"9acd32"};d[b]&&(b=d[b]);null!==b.match(/^#?([0-9a-f]{3}|[0-9a-f]{6})$/gi)?(b=b.replace("#",""),1===b.length%2?(c=String(b.substr(0,1))+b.substr(0,1),e=String(b.substr(1,1))+b.substr(1,1),b=String(b.substr(2,1))+b.substr(2,1),a.rgb={r:parseInt(c,16),g:parseInt(e,16),b:parseInt(b,16)},a.hex="#"+c+e+b):(a.rgb={r:parseInt(b.substr(0,2),16),g:parseInt(b.substr(2,2),16),b:parseInt(b.substr(4,2),16)},a.hex="#"+b),b=this.rgbToHsl(a.rgb.r,
...
a||"smallify"===a||"close"===a)b[a]()},exportPanels:function(){var a=0<arguments.length&&void 0!==arguments[0]?arguments[0]:".jsPanel",b=1<arguments.length&&void 0!==arguments[1]?arguments[1]:"jspanels",c=[],a=$(".jsPanel").not(".jsPanel-tooltip, .jsPanel-hint, .jsPanel-modal").filter(a);a.each(function(a,b){"normalized"!==$(b).data("status")&&$(".jsPanel-btn-normalize",b).trigger("click")});a.each(function(a,b){var f,h,l,m;f=$(b).data("container");h=$(b).offset();l=$(b).position();"minimized"===
...
$(document).trigger("jspanelmaximized",f);$(document).trigger("jspanelstatuschange",f);$.isFunction(a.option.onmaximized)&&a.option.onmaximized.call(a,a);b&&$.isFunction(b)&&b.call(a,a);return a},minimize:function(a,b){var c=a.attr("id");if("minimized"===a.data("status"))return a;$(document).trigger("jspanelbeforeminimize",c);if($.isFunction(a.option.onbeforeminimize)&&!1===a.option.onbeforeminimize.call(a,a))return a;var e=a.data("status");"normalized"===e?a.updateCachedData():"smallified"===e&&
(a.cachedData.top=a.css("top"),a.cachedData.left=a.css("left"));var e=this.createMinimizedReplacement(a),d=$(a.option.container).closest(".jsPanel-content");if(d.length){var f=".jsPanel-minimized-box";a.option.minimizeTo&&("string"===typeof a.option.minimizeTo?(f=a.option.minimizeTo,$(f).append(e)):$(f,d.parent()).append(e))}else d="#jsPanel-replacement-container",a.option.minimizeTo&&("string"===typeof a.option.minimizeTo&&(d=a.option.minimizeTo),$(d).append(e));$(document).trigger("jspanelminimized",
c);$(document).trigger("jspanelstatuschange",c);$.isFunction(a.option.onminimized)&&a.option.onminimized.call(a,a);b&&$.isFunction(b)&&b.call(a,a);$(".jsPanel-btn-normalize",e).css("display","block").on("click",function(){return a.normalize()});"disabled"===a[0].dataset.btnnormalize?$(".jsPanel-btn-normalize",e).css({pointerEvents:"none",opacity:.5,cursor:"default"}):"removed"===a[0].dataset.btnnormalize&&$(".jsPanel-btn-normalize",e).remove();$(".jsPanel-btn-maximize",e).on("click",function(){return a.maximize()});
"disabled"===a[0].dataset.btnmaximize?$(".jsPanel-btn-maximize",e).css({pointerEvents:"none",opacity:.5,cursor:"default"}):"removed"===a[0].dataset.btnmaximize&&$(".jsPanel-btn-maximize",e).remove();$(".jsPanel-btn-close",e).on("click",function(){return a.close()});"disabled"===a[0].dataset.btnclose&&$(".jsPanel-btn-close",e).css({pointerEvents:"none",opacity:.5,cursor:"default"});return a},normalize:function(a,b){var c=a.attr("id");if("normalized"===a.data("status"))return a;$(document).trigger("jspanelbeforenormalize",
c);if($.isFunction(a.option.onbeforenormalize)&&!1===a.option.onbeforenormalize.call(a,a))return a;if("smallified"===a.data("status"))return a.smallify(),$(document).trigger("jspanelnormalized",c),$(document).trigger("jspanelstatuschange",c),$.isFunction(a.option.onnormalized)&&a.option.onnormalized.call(a,a),a;a.css({left:a.cachedData.left,top:a.cachedData.top,width:a.cachedData.width,height:a.cachedData.height,zIndex:function(){jsPanel.setZi(a)},overflow:"visible"}).data("status","normalized").contentResize();
...
$.isFunction(a.option.onunsmallified)&&a.option.onunsmallified.call(a,a)}})}a.css("z-index",this.setZi(a));b&&$.isFunction(b)&&b.call(a,a);return a},toolbarAdd:function(a,b,c,e){"header"===b?(a.header.toolbar.addClass("active"),$.isArray(c)?this.configToolbar(c,a.header.toolbar,a):$.isFunction(c)?a.header.toolbar.append(c(a.header)):a.header.toolbar.append(c)):"footer"===b&&(a.content.removeClass("jsPanel-content-nofooter"),a.footer.addClass("active"),"none"===a.option.theme&&a.footer.css({background:"transparent",
borderTop:"none"}),$.isArray(c)?this.configToolbar(c,a.footer,a):$.isFunction(c)?a.footer.append(c(a.footer)):a.footer.append(c));this.contentResize(a);e&&$.isFunction(e)&&e.call(a,a);return a}};
...
function(a,b){return jsPanel.headerControl(d,a,b)};d.headerTitle=function(a){return jsPanel.headerTitle(d,a)};d.hideControls=function(b){var c=d.header.controls;a("div",c).css("display","block");a(b,c).css("display","none")};d.maximize=function(a){return jsPanel.maximize(d,a)};d.minimize=function(a){return jsPanel.minimize(d,a)};d.normalize=function(a){return jsPanel.normalize(d,a)};d.reposition=function(a,b){return jsPanel.reposition(d,a,b)};d.resize=function(){var b=0<arguments.length&&void 0!==
arguments[0]?arguments[0]:a.jsPanel.resizedefaults.width,c=1<arguments.length&&void 0!==arguments[1]?arguments[1]:a.jsPanel.resizedefaults.height,e=2<arguments.length&&void 0!==arguments[2]?arguments[2]:a.jsPanel.resizedefaults.callback,f={},g=b;a.isPlainObject(g)?"content"===g.resize&&(String(g.height).match(/[^0-9\.]/)||(g.height+="px"),String(g.width).match(/[^0-9\.]/)||(g.width+="px"),g.height="calc("+g.height+" + "+(d.header.outerHeight()+"px")+" + "+d.css("border-top-width")+" + "+d.css("border-bottom-width")+
...
b):jsPanel.themes.includes(b.color)?jsPanel.applyBuiltInTheme(d,b):jsPanel.applyArbitraryTheme(d,b);l?jsPanel.applyThemeBorder(d,b):d.css({borderWidth:"",borderStyle:"",borderColor:""});c&&a.isFunction(c)&&c.call(d,d);return d};d.smallify=function(a){return jsPanel.smallify(d,a)};d.toolbarAdd=function(a,b,c){return jsPanel.toolbarAdd(d,a,b,c)};d.updateCachedData=function(){d.cachedData.top=d.css("top");d.cachedData.left=d.css("left");d.cachedData.width=d.css("width");d.cachedData.height=d.css("height")};
...
v[a]?jsPanel.setControlStatus(d,a,"disable"):"remove"===v[a]?jsPanel.setControlStatus(d,a,"remove"):d[0].setAttribute("data-btn"+a,"enabled")}),u&&("string"===typeof u&&"<"!==u.substring(0,1)?d.header.logo.append('<img src="'+u+'" height="38" alt="logo">'):d.header.logo.append(u)));(q||1>a(".jsPanel-hdr").length)&&d.content.css("border","none");jsPanel.configIconfont(d);"modal"===y?(jsPanel.insertModalBackdrop(d),d.addClass("jsPanel-modal").css("z-index",jsPanel.modalcount+9999)):"hint"===y?d.addClass("jsPanel-hint").css("z-index",
1E4):y.tooltip&&(f=jsPanel.setTrigger(C),d.addClass("jsPanel-tooltip"),jsPanel.setTooltipClass(d),y.solo&&jsPanel.closeTooltips(),jsPanel.setTooltipMode(d,f));y.tooltip&&a(f).addClass("hasTooltip");G&&!q&&d.toolbarAdd("header",G);x&&d.toolbarAdd("footer",x);g&&(d.content.append(g),d.data("content",g));r&&("string"===typeof r&&(d.option.contentAjax={url:r,autoload:!0}),jsPanel.ajax(d));a.isPlainObject(p)&&(p.src||p.srcdoc)&&jsPanel.iframe(d);y.connector&&jsPanel.addConnector(d);"string"===typeof k&&
(f=k.trim().split(" "),k={width:f[0],height:f[f.length-1]});0===k.height&&(k.height="0");d.content.css({width:k.width||a.jsPanel.defaults.contentSize.width,height:k.height||a.jsPanel.defaults.contentSize.height});d.css({width:function(){return 0<a(".jsPanel-content",d).length?d.content.outerWidth()+"px":k.width||a.jsPanel.defaults.contentSize.width},zIndex:function(){jsPanel.setZi(d)}});d.content.css("width","100%");jsPanel.position(d,C);jsPanel.calcPositionFactors(d);d.data("status","normalized");
a(document).trigger("jspanelstatuschange",c);if(!y&&t){if(t.title)d.header.headerbar.on("dblclick",function(a){a.preventDefault();jsPanel.dblclickhelper(t.title,d)});if(t.content)d.content.on("dblclick",function(a){a.preventDefault();jsPanel.dblclickhelper(t.content,d)});if(t.footer)d.footer.on("dblclick",function(a){a.preventDefault();jsPanel.dblclickhelper(t.footer,d)})}"string"===typeof n?d.content.css("overflow",n):a.isPlainObject(n)&&d.content.css({"overflow-y":n.vertical||n["overflow-y"],"overflow-x":n.horizontal||
n["overflow-x"]});a.isPlainObject(w)?d.draggable(w):"disabled"===w?(a(".jsPanel-titlebar, .jsPanel-ftr",d).css("cursor","default"),d.draggable({disabled:!0})):a(".jsPanel-titlebar, .jsPanel-ftr",d).css("cursor","default");a.isPlainObject(D)?d.resizable(D):"disabled"===D&&(d.resizable({disabled:!0}),a(".ui-icon-gripsmall-diagonal-se, .ui-resizable-handle.ui-resizable-sw",d).css({"background-image":"none","text-indent":-9999}),a(".ui-resizable-handle",d).css({cursor:"inherit"}));!0===E.rtl&&(a(".jsPanel-hdr, .jsPanel-headerbar, .jsPanel-titlebar, .jsPanel-controlbar, .jsPanel-hdr-toolbar, .jsPanel-ftr",
...
headerControls:{controls:"closeonly"},position:{fixed:!1},resizable:!1};a.jsPanel.hintdefaults={autoclose:8E3,draggable:!1,headerControls:{controls:"closeonly"},resizable:!1};a.jsPanel.resizedefaults={width:null,height:null,minwidth:!1,maxwidth:!1,minheight:!1,maxheight:!1,resize:!1,callback:!1};a(document).ready(function(){document.body.addEventListener("click",function(b){1>a(b.target).closest(".jsPanel").length&&!a(b.target).hasClass("hasTooltip")&&(jsPanel.closeTooltips(),a(".hasTooltip").removeClass("hasTooltip"))},
!1);a("body").css("-ms-overflow-style","scrollbar").append('<div id="jsPanel-replacement-container">');window.addEventListener("keydown",function(a){a=a.key||a.code;("Escape"===a||"Esc"===a)&&jsPanel.closeOnEscape&&jsPanel.activePanels.getPanel(jsPanel.getTopmostPanel()).close()},!1)})})(jQuery);
