(function (global, factory) {
  ...
  const cer = function () {
    return rv.log && logErr(...arguments);
  };
  ...
  const defaultErrorHandler = function (err, silent) {
    ...
    const logFn = rv.isDebug ? logWarn : logErr; // 避免travis 发现 error 打印就导致 test 用例不通过
  ...
  function setPartialState(partialState, state, key) {
    ...
    if (rv.ignoreUndefined && value === UNDEFINED) {
      return false;
    }
  ...
  */
  ...
  function configStoreState(storeState) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      const moduleState = evalState$2(storeState[moduleName], `module[${moduleName}] `);
