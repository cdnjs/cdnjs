(function (global, factory) {
  ...
  var cer = function cer() {
    return runtimeVar.log && logErr.apply(void 0, arguments);
  };
  ...
  var defaultErrorHandler = function defaultErrorHandler(err, silent) {
    ...
    var logFn = runtimeVar.isDebug ? logWarn : logErr; // 避免travis 发现 error 打印就导致 test 用例不通过
  ...
  var UNDEFINED = void 0; // set成功则返回true
  ...
  function _executeCuInfo() {
    _executeCuInfo = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee(cuInfo) {
      ...
      if (curStateForComputeFn) {
        ...
        var ensureCommittedState = function ensureCommittedState(fnCommittedState) {
          ...
          if (ignoredStateKeys.length) {
            var reason = "they are not " + (sourceType === CATE_REF ? 'private' : 'module') + " keys, fn is " + sourceType + " " + fnType;
  ...
  */
  ...
  function changeRefState(inputState, _temp, callerRef) {
    ...
    var _syncCommittedStateTo = syncCommittedStateToStore(module, inputState, {
      ref: callerRef,
      callInfo: callInfo,
      noSave: true,
      force: force
        ...
        hasPrivState = _syncCommittedStateTo.hasPrivState; // 有 computed 提交了新的 state
  ...
  function broadcastState(callInfo, callerRef, partialSharedState, allowOriInsRender, moduleName, reactCallback, renderKeys, force) {
    ...
    var _callerRef$ctx2 = callerRef.ctx,
        currentCcUKey = _callerRef$ctx2.ccUniqueKey,
        ccClassKey = _callerRef$ctx2.ccClassKey;
    ...
    var _findUpdateRefs = findUpdateRefs(moduleName, partialSharedState, renderKeys, renderKeyClasses),
        ...
        connectRefKeys = _findUpdateRefs$resul.connect; // 引入更新队列，放入下一轮事件循环更新，避免一下警告
    // Warning: Cannot update a component (`XXX`) while rendering a different component (`YYY`)


    var updaterQueue = [];
    var oriCalledBy = callInfo.calledBy;

    var queuedOrUpdate = function queuedOrUpdate(refKey, updater) {
    ...
    belongRefKeys.forEach(function (refKey) {
      ...
      queuedOrUpdate(refKey, function () {
    ...
    setTimeout(function () {
      updaterQueue.forEach(function (updater) {
        return updater();
      });
    }, 0);
  }
  ...
  function configStoreState(storeState) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var moduleState = evalState$2(storeState[moduleName], "module[" + moduleName + "] ");
  ...
  function startup (_temp, _temp2) {
    ...
    var _ref2 = _temp2 === void 0 ? {} : _temp2,
        ...
        _ref2$ignoreUndefined = _ref2.ignoreUndefined,
        ignoreUndefined = _ref2$ignoreUndefined === void 0 ? false : _ref2$ignoreUndefined,
        _ref2$log = _ref2.log,
  ...
  function register(_temp, ccClassKey) {
    ...
    try {
      var _mapRegistrationInfo = mapRegInfo(module, ccClassKey, renderKeyClasses, CC_CLASS, watchedKeys, connect, __checkStartUp, __calledBy),
  ...
  function initCcFrag (ref, props) {
    ...
    if (props.__$$regDumb !== true) {
      ...
      var _mapRegistrationInfo = mapRegInfo(module, ccClassKey, renderKeyClasses, CC_CLASS, watchedKeys, connect, true),
          _ccClassKey = _mapRegistrationInfo._ccClassKey,
          _connect = _mapRegistrationInfo._connect,
          _watchedKeys = _mapRegistrationInfo._watchedKeys;
  ...
  var CcFragment = /*#__PURE__*/function (_React$Component) {
  ...
  function registerDumb (registerOption, ccClassKey) {
    ...
    var _mapRegistrationInfo = mapRegInfo(module, ccClassKey, renderKeyClasses, CC_FRAGMENT, watchedKeys, connect, true),
        _module = _mapRegistrationInfo._module,
        _ccClassKey = _mapRegistrationInfo._ccClassKey,
        _connect = _mapRegistrationInfo._connect,
        _watchedKeys = _mapRegistrationInfo._watchedKeys;
  ...
  function buildRef(ref, insType, hookCtx, rState, iState, regOpt, hookState, hookSetter, props, ccClassKey) {
    ...
    var _mapRegistrationInfo = mapRegInfo(module, ccClassKey, renderKeyClasses, CC_HOOK, watchedKeys, connect, true),
        _module = _mapRegistrationInfo._module,
        _ccClassKey = _mapRegistrationInfo._ccClassKey,
        _connect = _mapRegistrationInfo._connect,
        _watchedKeys = _mapRegistrationInfo._watchedKeys;
