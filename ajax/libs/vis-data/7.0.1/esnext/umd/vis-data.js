...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    class DataPipeUnderConstruction {
         ...
         */
        filter(callback) {
            __classPrivateFieldGet(this, _transformers_1).push((input) => input.filter(callback));
            return this;
        }
        /**
         ...
         */
        map(callback) {
            __classPrivateFieldGet(this, _transformers_1).push((input) => input.map(callback));
            return this;
        }
        /**
         ...
         */
        flatMap(callback) {
            __classPrivateFieldGet(this, _transformers_1).push((input) => input.flatMap(callback));
            return this;
        }
        /**
    ...
    class Queue {
        ...
        destroy() {
            ...
            if (__classPrivateFieldGet(this, _extended)) {
                const object = __classPrivateFieldGet(this, _extended).object;
                const methods = __classPrivateFieldGet(this, _extended).methods;
        ...
        queue(entry) {
            if (typeof entry === "function") {
                __classPrivateFieldGet(this, _queue).push({ fn: entry });
            }
            else {
         ...
         */
        flush() {
            __classPrivateFieldGet(this, _queue).splice(0).forEach((entry) => {
                entry.fn.apply(entry.context || entry.fn, entry.args || []);
            });
        }
    }
    _queue = new WeakMap(), _timeout = new WeakMap(), _extended = new WeakMap();
    ...
    class DataSetPart {
        ...
        _trigger(event, payload, senderId) {
            ...
            [...__classPrivateFieldGet(this, _subscribers)[event], ...__classPrivateFieldGet(this, _subscribers)["*"]].forEach((subscriber) => {
         ...
         */
        off(event, callback) {
            __classPrivateFieldGet(this, _subscribers)[event] = __classPrivateFieldGet(this, _subscribers)[event].filter((subscriber) => subscriber !== callback);
        }
    }
    ...
    class DataStream {
         ...
         */
        *[(_pairs = new WeakMap(), Symbol.iterator)]() {
        ...
        *entries() {
         ...
         */
        *keys() {
            for (const [id] of __classPrivateFieldGet(this, _pairs)) {
                yield id;
            }
        }
        /**
         ...
         */
        *values() {
            for (const [, item] of __classPrivateFieldGet(this, _pairs)) {
                yield item;
            }
        }
        /**
         ...
         */
        toIdArray() {
            return [...__classPrivateFieldGet(this, _pairs)].map((pair) => pair[0]);
        }
        /**
         ...
         */
        toItemArray() {
            return [...__classPrivateFieldGet(this, _pairs)].map((pair) => pair[1]);
        }
        /**
         ...
         */
        toEntryArray() {
            return [...__classPrivateFieldGet(this, _pairs)];
        }
        /**
         ...
         */
        toMap() {
            return new Map(__classPrivateFieldGet(this, _pairs));
        }
        /**
         ...
         */
        cache() {
            return new DataStream([...__classPrivateFieldGet(this, _pairs)]);
        }
        /**
         ...
         */
        filter(callback) {
            const pairs = __classPrivateFieldGet(this, _pairs);
         ...
         */
        max(callback) {
            const iter = __classPrivateFieldGet(this, _pairs)[Symbol.iterator]();
            let curr = iter.next();
            if (curr.done) {
                return null;
            }
        ...
        sort(callback) {
            return new DataStream({
                [Symbol.iterator]: () => [...__classPrivateFieldGet(this, _pairs)]
    ...
    class DataSet extends DataSetPart {
        ...
        constructor(data, options) {
            ...
            _idProp.set(this, void 0);
            _queue$1.set(this, null);
            ...
            __classPrivateFieldSet(this, _data, new Map()); // map with data indexed by id
            this.length = 0; // number of items in the DataSet
            __classPrivateFieldSet(// number of items in the DataSet
            this, _idProp, __classPrivateFieldGet(this, _options).fieldId || "id"); // name of the field containing id
        ...
        add(data, senderId) {
            ...
            if (Array.isArray(data)) {
                // Array
                const idsToAdd = data.map((d) => d[__classPrivateFieldGet(this, _idProp)]);
                if (idsToAdd.some((id) => __classPrivateFieldGet(this, _data).has(id))) {
        ...
        update(data, senderId) {
            ...
            const oldData = [];
            const updatedData = [];
            const idProp = __classPrivateFieldGet(this, _idProp);
            const addOrUpdate = (item) => {
                const origId = item[idProp];
                if (origId != null && __classPrivateFieldGet(this, _data).has(origId)) {
                    const fullItem = item; // it has an id, therefore it is a fullitem
                    const oldItem = Object.assign({}, __classPrivateFieldGet(this, _data).get(origId));
                    // update item
        ...
        updateOnly(data, senderId) {
            ...
            const updateEventData = data
                .map((update) => {
                const oldData = __classPrivateFieldGet(this, _data).get(update[__classPrivateFieldGet(this, _idProp)]);
                if (oldData == null) {
                ...
                .map(({ oldData, update }) => {
                const id = oldData[__classPrivateFieldGet(this, _idProp)];
                const updatedData = esnext.pureDeepObjectAssign(oldData, update);
                __classPrivateFieldGet(this, _data).set(id, updatedData);
                return {
                    id,
                    oldData: oldData,
                    updatedData,
        ...
        get(first, second) {
            ...
            if (id != null) {
                ...
                // return all items
                itemIds = [...__classPrivateFieldGet(this, _data).keys()];
            ...
            if (returnType == "Object") {
                ...
                for (let i = 0, len = items.length; i < len; i++) {
                    ...
                    const id = resultant[__classPrivateFieldGet(this, _idProp)];
                    result[id] = resultant;
                }
                return result;
            }
        ...
        getIds(options) {
            ...
            if (filter) {
                ...
                if (order) {
                    ...
                    for (let i = 0, len = items.length; i < len; i++) {
                        ids.push(items[i][__classPrivateFieldGet(this, _idProp)]);
                    }
                }
                else {
                    ...
                    for (let i = 0, len = itemIds.length; i < len; i++) {
                        ...
                        const item = __classPrivateFieldGet(this, _data).get(id);
                        if (item != null && filter(item)) {
                            ids.push(item[__classPrivateFieldGet(this, _idProp)]);
                        }
                    }
                }
            }
        ...
        forEach(callback, options) {
            ...
            if (options && options.order) {
                ...
                for (let i = 0, len = items.length; i < len; i++) {
                    const item = items[i];
                    const id = item[__classPrivateFieldGet(this, _idProp)];
                    callback(item, id);
                }
            }
            else {
        ...
        remove(id, senderId) {
            ...
            for (let i = 0, len = ids.length; i < len; i++) {
                ...
                if (item) {
                    const itemId = item[__classPrivateFieldGet(this, _idProp)];
                    if (itemId != null) {
        ...
        _remove(id) {
            ...
            if (ident != null && __classPrivateFieldGet(this, _data).has(ident)) {
                const item = __classPrivateFieldGet(this, _data).get(ident) || null;
         ...
         */
        clear(senderId) {
            const ids = [...__classPrivateFieldGet(this, _data).keys()];
            const items = [];
            for (let i = 0, len = ids.length; i < len; i++) {
        ...
        max(field) {
            let max = null;
            let maxField = null;
            for (const item of __classPrivateFieldGet(this, _data).values()) {
         ...
         */
        _addItem(item) {
            const fullItem = ensureFullItem(item, __classPrivateFieldGet(this, _idProp));
            const id = fullItem[__classPrivateFieldGet(this, _idProp)];
        ...
        stream(ids) {
            if (ids) {
                ...
                return new DataStream({
                    [Symbol.iterator]: __classPrivateFieldGet(this, _data).entries.bind(__classPrivateFieldGet(this, _data)),
                });
            }
        }
    }
    _options = new WeakMap(), _data = new WeakMap(), _idProp = new WeakMap(), _queue$1 = new WeakMap();
    ...
    class DataView extends DataSetPart {
        ...
        constructor(data, options) {
            ...
            _data$1.set(this, void 0); // constructor → setData
            _ids.set(this, new Set()); // ids of the items currently in memory (just contains a boolean true)
            _options$1.set(this, void 0);
            __classPrivateFieldSet(this, _options$1, options || {});
            __classPrivateFieldSet(this, _listener, this._onEvent.bind(this));
            this.setData(data);
        }
        ...
        setData(data) {
            if (__classPrivateFieldGet(this, _data$1)) {
                ...
                const ids = __classPrivateFieldGet(this, _data$1).getIds({ filter: __classPrivateFieldGet(this, _options$1).filter });
                const items = __classPrivateFieldGet(this, _data$1).get(ids);
                __classPrivateFieldGet(this, _ids).clear();
                this.length = 0;
                this._trigger("remove", { items: ids, oldData: items });
            }
            if (data != null) {
                __classPrivateFieldSet(this, _data$1, data);
        ...
        get(first, second) {
            ...
            const viewOptions = Object.assign({}, __classPrivateFieldGet(this, _options$1), options);
            // create a combined filter method when needed
            const thisFilter = __classPrivateFieldGet(this, _options$1).filter;
        ...
        getIds(options) {
            if (__classPrivateFieldGet(this, _data$1).length) {
                const defaultFilter = __classPrivateFieldGet(this, _options$1).filter;
        ...
        forEach(callback, options) {
            if (__classPrivateFieldGet(this, _data$1)) {
                ...
                __classPrivateFieldGet(this, _data$1).forEach(callback, {
                    filter: filter,
                    order: options && options.order,
                });
            }
        ...
        map(callback, options) {
            if (__classPrivateFieldGet(this, _data$1)) {
                ...
                return __classPrivateFieldGet(this, _data$1).map(callback, {
                    filter: filter,
                    order: options && options.order,
                });
            }
        ...
        /** @inheritDoc */
        stream(ids) {
            return __classPrivateFieldGet(this, _data$1).stream(ids || {
                [Symbol.iterator]: __classPrivateFieldGet(this, _ids).keys.bind(__classPrivateFieldGet(this, _ids)),
            });
        }
        /**
    ...
    _listener = new WeakMap(), _data$1 = new WeakMap(), _ids = new WeakMap(), _options$1 = new WeakMap();
