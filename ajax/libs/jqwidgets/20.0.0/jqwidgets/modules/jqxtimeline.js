...
(function () {
	...
	(function ($) {
		...
		$.jqx.jqxWidget("jqxTimeline", "", {});

		$.extend($.jqx._jqxTimeline.prototype, {
			defineInstance: function () {
				...
				if (this === $.jqx._jqxTimeline.prototype) {
					return settings;
				}
			...
			createInstance: function (args) {
				...
				that.element.classList.add('jqx-widget', 'jqx-timeline');
				container.classList.add('jqx-container');

				if (that.theme) {
					that.element.classList.add('jqx-widget-' + that.theme);
				}

				var nearItems = document.createElement('div');
				nearItems.classList.add('jqx-timeline-near-items');

				var track = document.createElement('div');
				track.classList.add('jqx-timeline-track');

				var farItems = document.createElement('div');
				farItems.classList.add('jqx-timeline-far-items');

				container.appendChild(nearItems);
				container.appendChild(track);
				container.appendChild(farItems);

				that.$ = {};

				that.$.container = container;
				that.$.nearItems = nearItems;
				that.$.track = track;
				that.$.farItems = farItems;
			},
			...
			render: function () {
				...
				that.element.setAttribute('role', 'navigation');
				that._render();
			},
			...
			*/
			 ...
			 */
			propertyChangedHandler: function (object, propertyName, oldvalue, value) {
				var that = this;

				switch (propertyName) {
					case 'theme':
						that.element.classList.remove('jqx-widget-' + oldvalue);
						break;
					case 'source':
						that._render();
						return;
			...
			_render: function () {
				...
				if (!that.source) {
					that.$.nearItems.innerHTML = '';
					that.$.farItems.innerHTML = '';
					return;
				}
				that.$.nearItems.innerHTML = '';
				that.$.farItems.innerHTML = '';
				that.$.track.innerHTML = '';
				that.$.nearItems.style.height = '';
				that.$.nearItems.style.width = '';
				that.$.farItems.style.height = '';
				that.$.farItems.style.width = '';

				var timelineItems = [];
				var dateItems = [];

				if (that.horizontal) {
					that.element.setAttribute('horizontal', '');
				}
				else {
					that.element.removeAttribute('horizontal');
				}

				for (var i = 0; i < that.source.length; i++) {
					var item = that.source[i];
					var createTimelineItem = function (item) {
						var timelineItem = document.createElement('div');
						timelineItem.classList.add('jqx-timeline-item');
						if (item.css) {
							timelineItem.classList.add(item.css);
						}
						if (that.collapsible) {
							timelineItem.setAttribute('collapsible', '');
						}

						timelineItem.innerHTML = '<div class="jqx-timeline-item-pointer"></div><div class="jqx-timeline-item-content"><div class="jqx-flex jqx-timeline-item-header"><div class="jqx-timeline-item-icon"><span class="' + item.icon + '"></span></div><div><div class="jqx-timeline-item-title">' + item.title + '</div><div class="jqx-timeline-item-subtitle">' + item.subtitle + '</div></div></div><div class="jqx-timeline-item-description">' + item.description + '</div></div>';

						timelineItem.onclick = function () {
							if (that.collapsible) {
								timelineItem.classList.toggle('expanded');
								if (timelineItem.cloneNode) {
									timelineItem.cloneNode.classList.toggle('expanded');
								}
								that._repositionDots();
							}
						}
						...
						date.classList.add('jqx-timeline-item-date');
						date.innerHTML = item.date;
						dateItems.push(date);
						var dot = document.createElement('div');
						dot.timelineItem = timelineItem;

						if (that.position === 'near') {
							that.$.nearItems.appendChild(timelineItem);
							that.$.farItems.appendChild(date);
						}
						else if (that.position === 'far') {
							that.$.farItems.appendChild(timelineItem);
							that.$.nearItems.appendChild(date);
						}
						else {
							that.$.nearItems.appendChild(timelineItem);

							var cloneNode = timelineItem.cloneNode(true);
							cloneNode.onclick = function () {
								if (that.collapsible) {
									cloneNode.classList.toggle('expanded');
									timelineItem.classList.toggle('expanded');
									that._repositionDots();
								}
							}

							that.$.farItems.appendChild(cloneNode);
							timelineItem.cloneNode = cloneNode;
							if (i % 2 === 0) {
								timelineItem.classList.add('jqx-visibility-hidden');
								that.$.nearItems.appendChild(date);
								dot.timelineItem = cloneNode;
							}
							else {
								cloneNode.classList.add('jqx-visibility-hidden');
								that.$.farItems.appendChild(date);
							}
						}

						dot.classList.add('jqx-timeline-dot');
						if (item.dotCSS) {
							dot.classList.add(item.dotCSS);
						}
						that.$.track.appendChild(dot);
						dot.date = dateItems[i];
					}
					createTimelineItem(item);
				}


				that._timelineItems = timelineItems;
				that._layout();
			},
			...
			_arrange: function () {
				var that = this;
				var timelineItems = that._timelineItems;
				if (that.horizontal === false) {
					var calculateWidth = function (element) {
						var width = 0;

						for (var i = 0; i < element.children.length; i++) {
							width = Math.max(width, element.children[i].offsetWidth);
						}

						return width + 20;
					}
					that.$.nearItems.style.width = calculateWidth(that.$.nearItems) + 'px';
					that.$.farItems.style.width = calculateWidth(that.$.farItems) + 'px';
				}
				else {
					var calculateWidth = function () {
						...
						for (var i = 0; i < timelineItems.length; i++) {
							width += timelineItems[i].offsetWidth;
						}
					...
					if (that.autoWidth) {
						...
						that.element.style.minWidth = width + 'px';
						var offset = (that.element.offsetWidth - width) / timelineItems.length;
						if (offset > 0) {
							$.each(that.$.farItems.children, function (index) {
								var child = that.$.farItems.children[index];
								child.style.marginLeft = child.style.marginRight = offset / 2 + 'px';
							});
							$.each(that.$.nearItems.children, function (index) {
								var child = that.$.nearItems.children[index];
								child.style.marginLeft = child.style.marginRight = offset / 2 + 'px';
							});
						}
					}
			...
			_repositionDots: function () {
				var that = this;
				for (var i = 0; i < this.$.track.children.length; i++) {
					var dot = this.$.track.children[i];
					var itemOffset = that.offset(dot.timelineItem);

					if (that.horizontal) {
						dot.style.left = itemOffset.left + (dot.timelineItem.offsetWidth / 2) - 16 - 10 + 'px';
						dot.date.style.marginLeft = dot.date.style.marginRight = '0px';
						dot.date.style.left = itemOffset.left + dot.timelineItem.offsetWidth / 2 - (dot.date.offsetWidth / 2) - 26 + 'px';
					}
					else {
						dot.style.top = itemOffset.top + (dot.timelineItem.offsetHeight / 2) - 30 + 'px';
						dot.date.style.top = itemOffset.top + (dot.timelineItem.offsetHeight / 2) - 30 + 'px';
					}
				}
			},
