    ...
    (function ($) {
        ...
        $.extend($.jqx._jqxSlider.prototype, {
            defineInstance: function () {
                ...
                if (this === $.jqx._jqxSlider.prototype) {
                    return settings;
                }
                $.extend(true, this, settings);
                return settings;
            ...
            createInstance: function (args) {
                ...
                if (that._helpers['element'].isRendered) {
                    if (that._helpers['element'].isRendered()) {
                        ...
                        if (that._helpers['element'].sizeChanged) {
                    ...
                    $(that._leftButton).jqxRepeatButton({
                        template: that.template, theme: that.theme, delay: 50, width: that.sliderButtonSize, height: that.sliderButtonSize
                    });
                    $(that._rightButton).jqxRepeatButton({ template: that.template, theme: that.theme, delay: 50, width: that.sliderButtonSize, height: that.sliderButtonSize });
                    that.render();
                }
            ...
            _addTicks: function (container, side) {
                ...
                if (that.tickMode === 'default' && that.niceInterval === true) { // special case for second tick (and label) when niceInterval is enabled
                    var first, second;
            ...
            _performLayout: function () {
                ...
                that._performTrackLayout(size - 8);
                ...
                if (that.orientation === 'horizontal') {
                    ...
                    if (that.showButtons && !that.rangeSlider) {
                        that._contentWrapper.style.left = 4 + that._helpers['leftButton'].outerWidth(true) + 'px';
                    ...
                    if (that.showButtons && !that.rangeSlider) {
                        that._contentWrapper.style.top = 1 + that._helpers['leftButton'].outerHeight(true) + 'px';
            ...
            _performTrackLayout: function (size) {
                ...
                if (that.showButtons && !that.rangeSlider) {
                    if (that.orientation === 'horizontal') {
                        trackSize -= (that._helpers['leftButton'].innerWidth() + that._helpers['rightButton'].innerWidth() + 4);
                    }
                    else {
                        trackSize -= (that._helpers['leftButton'].innerHeight() + that._helpers['rightButton'].innerHeight() + 4);
                    }
                }
                ...
                that.addHandler($(document), 'mouseup.arrow' + that.element.id, function () {
                    ...
                    if (that.sliderTooltip) {
                        ...
                        that.sliderTooltipTimer = setTimeout(function () {
                            ...
                            $(that.sliderTooltipObj).removeClass('show');
                            $(that.sliderTooltipObj).addClass('hide');
            ...
            _trackMouseDownHandler: function (event) {
                ...
                leftButtonSize = that._getSetting('size') === 'width' ? that._slider.left.offsetWidth : that._slider.left.offsetHeight;
            ...
            _stopDrag: function () {
                ...
                if (that.sliderTooltip) {
                    that.sliderTooltipTimer = setTimeout(function () {
                        ...
                        $(that.sliderTooltipObj).removeClass('show');
                        $(that.sliderTooltip).addClass('hide');
                    }, that.tooltipHideDelay);
                }
            },
            ...
            _setSliderPosition: function (value, thumb, position) {
                ...
                if (this.int64 === 's') {
                    ...
                    distance = trackSize * ratio - thumbSize / 2;
                    var currentThumbDistance = thumb.style[this._getSetting('left')];
                ...
                if (this.value > 0) {
                    $(thumb).addClass(this.toThemeProperty('jqx-slider-has-value'));
                }
                else {
                    $(thumb).removeClass(this.toThemeProperty('jqx-slider-has-value'));
                }
            ...
            _showTooltip: function (thumb, value) {
                ...
                if (that._slideStarted || that._capturedElement != null || that._mouseDown) {
                    ...
                    if (!that.toolTipCreated) {
                        ...
                        if (that.sliderTooltipObj.initAnimate) {
                            that.sliderTooltipObj.initAnimate();
                        }

                        $text.innerHTML = '<span>' + value + '</span>';

                        that.sliderTooltip.className = that.toThemeProperty('jqx-slider-tooltip jqx-tooltip jqx-popup ' + that.orientation);
                        if (that.rangeSlider) {
                            that.sliderTooltip.className += ' range';
                        }
                        if (that.template) {
                            that.sliderTooltip.className += " " + "jqx-" + that.template + "-slider";
                        }
                    ...
                    var thumbSize = new jqxHelper(thumb).outerWidth();
                    that.sliderTooltip.style.display = 'block';
                    that.sliderTooltip.style.visibility = 'visible';
                    $(that.sliderTooltip).addClass('init');
                    $(that.sliderTooltip).removeClass('hide');
                    setTimeout(function () {
                        $(that.sliderTooltip).addClass('show');
                    });

                    var size = that.sliderButtonSize + that.tickSize;

                    if (!that.rangeSlider) {
                        that.sliderTooltipContent.innerHTML = '<span>' + value.toString() + '</span>';
                    }
                    else {
                        var from = that.value ? that.value.rangeStart : '';
                        var to = that.value ? that.value.rangeEnd : '';
                        if (from !== '') {
                            that.sliderTooltipContent.innerHTML = '<span>' + from + ' - ' + to + '</span>';;
                        }
                        else {
                            if (!that.isMaterialized()) {
                            ...
                            $(that.sliderTooltip).removeClass('show');
                            $(that.sliderTooltip).addClass('hide');
                    ...
                    if (that.orientation === 'horizontal') {
                        left = 1 + thumbPosition.left + thumbSize / 2 - tooltipWidth / 2;
                        if (that.rangeSlider) {
                        ...
                        switch (that.tooltipPosition) {
                            case 'far':
                                top = thumbPosition.top + size + that.arrowSize + 6;
                                that.sliderTooltipObj.offset({ top: top, left: left });
                                that.sliderTooltipObj.addClass(that.toThemeProperty('far'));
                                ...
                                that.sliderTooltipArrowObj.offset({ top: top - that.arrowSize + 1, left: left - that.arrowSize / 2 - 1 + tooltipWidth / 2 });
                                break;
                            case 'near':
                                top = thumbPosition.top - that.arrowSize - that.sliderTooltipObj.innerHeight() - 4;
                                that.sliderTooltipObj.offset({ top: top, left: left });
                                that.sliderTooltipArrowObj.addClass(that.toThemeProperty('jqx-tooltip-arrow-t-b'));
                        ...
                        left = thumbPosition.left - tooltipWidth - that.arrowSize - that.tickSize - 2;
                        top = thumbPosition.top + that._thumbSize / 2 - tooltipHeight / 2 - 1;
                        if (that.rangeSlider) {
                            distance = (that._helpers['right'].offset().top - that._helpers['left'].offset().top - that._thumbSize) / 2;
                        ...
                        switch (that.tooltipPosition) {
                            case 'far':
                                ...
                                that.sliderTooltipArrowObj.offset({ top: top + that.sliderTooltipObj.innerHeight() / 2 - that.arrowSize / 2 - 2, left: left - that.arrowSize + 1 });
                                break;
                            case 'near':
                                that.sliderTooltipObj.offset({ top: top, left: left + 2 });
                                that.sliderTooltipArrowObj.addClass(that.toThemeProperty('jqx-tooltip-arrow-l-r'));
                                that.sliderTooltipArrow.style.borderTopWidth = that.arrowSize + 'px';
                                that.sliderTooltipArrow.style.borderRightWidth = '0px';
                                that.sliderTooltipArrow.style.borderBottomWidth = that.arrowSize + 'px';
                                that.sliderTooltipArrow.style.borderLeftWidth = that.arrowSize + 'px';
                                that.sliderTooltipArrowObj.offset({ top: top + that.sliderTooltipObj.innerHeight() / 2 - that.arrowSize / 2 - 2, left: left + tooltipWidth + 3 });
                                break;
                        }
                    }


                    $(that.sliderTooltip).removeClass('init');
                }
            },
