// Generated by CoffeeScript 1.10.0
(function() {
  var Authorization, Method, base64, clone, include, isFunction, isObject, isString, querystring, ref, request, resolve, resolve_url, resource,
    slice = [].slice;

  querystring = require("querystring");

  resolve_url = (ref = require("fairmont"), include = ref.include, clone = ref.clone, isString = ref.isString, isObject = ref.isObject, isFunction = ref.isFunction, base64 = ref.base64, Method = ref.Method, ref);

  request = require("./request").request;

  resolve = (function() {
    var URL, _resolve;
    URL = require("url");
    _resolve = URL.resolve;
    return function() {
      return decodeURIComponent(_resolve.apply(null, arguments));
    };
  })();

  Authorization = {
    basic: function(arg) {
      var password, username;
      username = arg.username, password = arg.password;
      return "Basic " + base64(username + ":" + password);
    },
    bearer: function(token) {
      return "Bearer " + token;
    }
  };

  resource = Method.create();

  Method.define(resource, isString, function(url) {
    return resource({
      url: url
    });
  });

  Method.define(resource, isString, isObject, function(url, description) {
    return resource({
      url: url,
      description: description
    });
  });

  Method.define(resource, isObject, function(arg) {
    var _resource, definition, description, fn1, from_parameters, from_path, make_request, name, url;
    url = arg.url, description = arg.description;
    from_path = function(path, _description) {
      if (_description == null) {
        _description = description;
      }
      return resource({
        url: resolve(url, path),
        description: _description
      });
    };
    from_parameters = (function() {
      var template;
      template = (require("url-template")).parse(url);
      return function(parameters, _description) {
        if (_description == null) {
          _description = description;
        }
        return resource({
          url: template.expand(parameters),
          description: _description
        });
      };
    })();
    make_request = function(definition) {
      var fn;
      if (definition.url == null) {
        definition.url = url;
      }
      if (definition.headers == null) {
        definition.headers = {};
      }
      fn = function() {
        return request.apply(null, [definition].concat(slice.call(arguments)));
      };
      include(fn, {
        invoke: function() {
          return fn.apply(null, arguments);
        },
        curl: function() {
          var headers, key, method, value;
          url = definition.url, method = definition.method, headers = definition.headers;
          return ("curl -v -X" + (method.toUpperCase()) + " " + url) + (function() {
            var results;
            results = [];
            for (key in headers) {
              value = headers[key];
              results.push(" -H'" + key + ": " + value + "'");
            }
            return results;
          })();
        },
        authorize: function(credentials) {
          var _definition, authorization, scheme, transform;
          scheme = Object.keys(credentials)[0];
          transform = (function() {
            var ref1;
            if (((ref1 = definition.authorization) != null ? ref1[scheme] : void 0) != null) {
              return definition.authorization[scheme];
            } else if (Authorization[scheme] != null) {
              return Authorization[scheme];
            } else {
              return function(arg1) {
                var k, v;
                k = arg1.k, v = arg1.v;
                return k + ": " + v;
              };
            }
          })();
          authorization = transform(credentials[scheme]);
          _definition = clone(definition);
          _definition.headers.authorization = authorization;
          return make_request(_definition);
        }
      });
      return fn;
    };
    _resource = Method.create();
    Method.define(_resource, isString, function(path) {
      return from_path(path);
    });
    Method.define(_resource, isString, isObject, function(path, description) {
      return from_path(path, description);
    });
    Method.define(_resource, isObject, function(parameters) {
      return from_parameters(parameters);
    });
    Method.define(_resource, isObject, isObject, function(parameters, description) {
      return from_parameters(parameters, description);
    });
    fn1 = function(name, definition) {
      return _resource[name] = (function() {
        switch (false) {
          case !(isObject(definition)):
            return make_request(clone(definition));
          case !(isFunction(definition)):
            return definition(function() {
              if (arguments.length === 1) {
                return _resource.apply(null, slice.call(arguments).concat([{}]));
              } else {
                return _resource.apply(null, arguments);
              }
            });
        }
      })();
    };
    for (name in description) {
      definition = description[name];
      fn1(name, definition);
    }
    return _resource;
  });

  module.exports = {
    resource: resource
  };

}).call(this);
