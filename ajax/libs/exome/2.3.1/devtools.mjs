var L = Object.defineProperty;
var O = Object.getOwnPropertySymbols;
var j = Object.prototype.hasOwnProperty, T = Object.prototype.propertyIsEnumerable;
var S = (e, n, t) => n in e ? L(e, n, { enumerable: !0, configurable: !0, writable: !0, value: t }) : e[n] = t, $ = (e, n) => {
  for (var t in n || (n = {}))
    j.call(n, t) && S(e, t, n[t]);
  if (O)
    for (var t of O(n))
      T.call(n, t) && S(e, t, n[t]);
  return e;
};
var k = (e, n) => {
  var t = {};
  for (var o in e)
    j.call(e, o) && n.indexOf(o) < 0 && (t[o] = e[o]);
  if (e != null && O)
    for (var o of O(e))
      n.indexOf(o) < 0 && T.call(e, o) && (t[o] = e[o]);
  return t;
};

// src/devtools-redux.ts
import { Exome as V, getExomeId as v, updateAll as X } from "exome";
var b = /* @__PURE__ */ new Map();
function D(e, n = []) {
  if (e == null || typeof e != "object")
    return e;
  if (e instanceof V && v(e)) {
    let o = v(e);
    if (n.indexOf(o) > -1)
      return {
        $$exome_id: o
      };
    let a = D($({}, e), n.concat(o));
    return $({
      $$exome_id: o
    }, a);
  }
  if (e.constructor !== Array && e.constructor !== Object && e.constructor !== Date)
    return {
      $$exome_class: e.constructor.name
    };
  let t = e.constructor() || {};
  for (let o of Object.keys(e))
    t[o] = D(e[o], n);
  return t;
}
var N = () => {
  let e = {};
  for (let [n, t] of b.entries())
    e[n] = Array.from(t.values());
  return D(e);
}, z = ({
  name: e,
  maxAge: n,
  actionsBlacklist: t
}) => {
  let o = "__REDUX_DEVTOOLS_EXTENSION__", a;
  try {
    a = window[o] || window.top[o];
  } catch (r) {
  }
  if (!a)
    return console.warn(
      `Please install Redux devtools extension
https://github.com/reduxjs/redux-devtools`
    ), () => {
    };
  let l = {
    name: e,
    maxAge: n,
    actionsBlacklist: t
  }, f = a.connect(l);
  return f.subscribe((r) => {
    r.type === "DISPATCH" && r.state && (JSON.parse(r.state, (i, y) => {
      var c;
      if (typeof y == "object" && y !== null && "$$exome_id" in y) {
        let s = y, { $$exome_id: u } = s, d = k(s, ["$$exome_id"]), [p] = u.split("-"), g = (c = b.get(p)) == null ? void 0 : c.get(u);
        return Object.assign(g, d), g;
      }
      return y;
    }), X());
  }), f.init(N()), (r, i, y) => {
    var d;
    let c = v(r).replace(/-.*$/, ""), u = `[${c}] ${i}`;
    if (c)
      return b.has(c) || b.set(c, /* @__PURE__ */ new Map()), i === "NEW" && ((d = b.get(c)) == null || d.set(v(r), r), f.send({ type: u, payload: void 0 }, N())), () => {
        let p = [];
        try {
          p = JSON.parse(JSON.stringify(y));
        } catch (g) {
        }
        f.send({ type: u, payload: p }, N());
      };
  };
};

// src/devtools-exome.ts
import { Exome as B, getExomeId as E, subscribe as G } from "exome";
var w = /* @__PURE__ */ new Map(), _ = [], I = Object.getOwnPropertyDescriptor, W = ({
  name: e = "Exome",
  maxAge: n = 20,
  ignoreListActions: t = [],
  ignoreListStores: o = []
}) => {
  let a = "__EXOME_DEVTOOLS_EXTENSION__", l;
  try {
    l = window[a] || window.top[a];
  } catch (s) {
  }
  if (!l)
    return console.warn(
      `Please install Exome devtools extension
https://github.com/Marcisbee/exome-devtools`
    ), () => {
    };
  let f = 0, r = {
    version: "6.6.6"
  }, i = l.connect({ name: e, maxAge: n, details: r });
  return window.addEventListener("beforeunload", i.disconnect, {
    once: !0
  }), i.subscribe(({ type: s }) => {
    s === "sync" && i.send({
      event: "update",
      type: "all",
      payload: {
        actions: _,
        states: [...w].flatMap(
          ([c, u]) => [...u].map(([d, p]) => [d, m(p)])
        )
      }
    });
  }), (s, c, u) => {
    var R, h;
    let d = E(s), p = d.replace(/-[a-z0-9]+$/gi, "");
    if (o.indexOf(p) > -1)
      return;
    if (w.has(p) || w.set(p, /* @__PURE__ */ new Map()), c === "NEW")
      return (R = w.get(p)) == null || R.set(d, s), i.send({
        event: "send",
        type: "state",
        payload: [d, m(s)]
      }), () => {
        i.send({
          event: "update",
          type: "state",
          payload: [d, m(s), E(s)]
        }), G(s, (M) => {
          i.send({
            event: "update",
            type: "state",
            payload: [d, m(M), E(M)]
          });
        });
      };
    if (c === "LOAD_STATE")
      return () => {
        i.send({
          event: "update",
          type: "state",
          payload: [d, m(s), E(s)]
        });
      };
    if (t.indexOf(`${p}.${c}`) > -1)
      return;
    let g = m(s), P = String(Math.random()), J = ((h = new Error().stack) == null ? void 0 : h.split(/\n/g)[6]) || "", A = performance.now();
    f += 1;
    let x = {
      id: P,
      name: c,
      instance: d,
      payload: u.map(C),
      now: A,
      depth: f,
      trace: J,
      before: g
    };
    return y(x), i.send({
      event: "send",
      type: "action",
      payload: x
    }), () => {
      x.time = performance.now() - A, x.after = m(s), f -= 1, i.send({
        event: "update",
        type: "action",
        payload: x
      });
    };
  };
  function y(s) {
    _.push(s), _.length > n && _.splice(0, n);
  }
};
function m(e) {
  var l, f;
  let n = Object.getPrototypeOf(e), t = Object.getOwnPropertyNames(n), o = Object.getOwnPropertyNames(e).filter(
    (r) => t.indexOf(r) === -1
  ), a = {};
  for (let r of t) {
    if (r === "constructor")
      continue;
    if (typeof ((l = I(n, r)) == null ? void 0 : l.get) == "function") {
      a[`$$exome_gt:${r}`] = null;
      continue;
    }
    a[`$$exome_ac:${r}`] = String("@TODO");
  }
  for (let r of o) {
    let i = (f = I(e, r)) == null ? void 0 : f.value;
    if (typeof i == "function") {
      a[`$$exome_sl:${r}`] = r;
      continue;
    }
    a[r] = C(i);
  }
  return a;
}
function C(e) {
  if (e === void 0)
    return e;
  try {
    return JSON.parse(
      JSON.stringify(
        e,
        (n, t) => t == null || typeof t != "object" ? t : t instanceof B ? {
          $$exome_id: E(t)
        } : t.constructor.name !== "Array" && t.constructor.name !== "Object" && t.constructor.name !== "Date" ? {
          $$exome_class: t.constructor.name
        } : t,
        2
      )
    );
  } catch (n) {
    return;
  }
}
export {
  z as exomeReduxDevtools,
  W as unstableExomeDevtools
};
