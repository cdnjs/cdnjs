document.addEventListener("DOMContentLoaded",()=>{var{indexName:e,appID:t,apiKey:a,hits:s}=CONFIG.algolia;const i=instantsearch({indexName:e,searchClient:algoliasearch(t,a),searchFunction:e=>{document.querySelector(".search-input").value&&e.search()}});window.pjax&&i.on("render",()=>{window.pjax.refresh(document.querySelector(".algolia-hits"))}),i.addWidgets([instantsearch.widgets.configure({hitsPerPage:s.per_page||10}),instantsearch.widgets.searchBox({container:".search-input-container",placeholder:CONFIG.i18n.placeholder,showReset:!1,showSubmit:!1,showLoadingIndicator:!1,cssClasses:{input:"search-input"}}),instantsearch.widgets.stats({container:".algolia-stats",templates:{text:e=>{return`<span>${CONFIG.i18n.hits_time.replace(/\$\{hits}/,e.nbHits).replace(/\$\{time}/,e.processingTimeMS)}</span>
            <img src="${CONFIG.images}/logo-algolia-nebula-blue-full.svg" alt="Algolia">`}},cssClasses:{text:"search-stats"}}),instantsearch.widgets.hits({container:".algolia-hits",escapeHTML:!1,templates:{item:e=>{var{title:t,excerpt:a,excerptStrip:s,contentStripTruncate:i}=e._highlightResult;let n=`<a href="${e.permalink}" class="search-result-title">${t.value}</a>`;i=a||s||i;if(i&&i.value){const r=document.createElement("div");r.innerHTML=i.value,n+=`<a href="${e.permalink}"><p class="search-result">${r.textContent.substr(0,100)}...</p></a>`}return n},empty:e=>`<div class="algolia-hits-empty">
