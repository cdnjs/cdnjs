...
function(){s()?e.val(a.getTextareaValue(!1)):e.val(a.getWysiwygEditorValue())};z=function(c){var b=d.extend({},a.options.emoticons.more,a.options.emoticons.dropdown,a.options.emoticons.hidden);d.each(b,function(d,b){var e=a.regexEscape(d)+"(?=([^\\<\\>]*?<(?!/code)|[^\\<\\>]*?$))",f="";a.options.emoticonsCompat&&(e="((>|^|\\s){1})"+e+"(?=(\\s|$|<){1})",f="$1");c=c.replace(RegExp(e,"gm"),f+'<img src="'+b+'" data-sceditor-emoticon="'+d+'" />')});return c};a.regexEscape=function(a){return a.replace(/[\$\?\[\]\.\*\(\)\|]/g,
"\\$&").replace("<","&lt;").replace(">","&gt;")};s=function(){return k.is(":visible")};a.toggleTextMode=function(){s()?a.setWysiwygEditorValue(a.getTextareaValue()):a.setTextareaValue(a.getWysiwygEditorValue());i.find(".sceditor-button").removeClass("disabled");m=null;k.toggle();g.toggle();i.find(".sceditor-button").each(function(){var a=d(this);s()&&!a.data("sceditor-txtmode")?a.addClass("disabled"):!s()&&!a.data("sceditor-wysiwygmode")&&a.addClass("disabled")})};A=function(c,b){s()?b.hasOwnProperty("txtExec")&&
b.txtExec.call(a,c):b.hasOwnProperty("exec")&&(d.isFunction(b.exec)?b.exec.call(a,c):a.execCommand(b.exec,b.hasOwnProperty("execParam")?b.execParam:null))};a.focus=function(){h.contentWindow.focus();null!==m&&(window.document.createRange?window.getSelection().addRange(m):window.document.selection&&m.select(),m=null)};B=function(){d.browser.msie&&(m=p())};a.execCommand=function(c,b){var e=!1;a.focus();if(!(d(o()).is("code")||0!==d(o()).parents("code").length)){if(l())try{e=l().execCommand(c,!1,b)}catch(f){}!e&&
...
b){return"undefined"!==typeof a[b-0+1]?a[b-0+1]:"{"+b+"}"})};I=function(){if(d.sceditor.locale[a.options.locale])r=d.sceditor.locale[a.options.locale];else{var c=a.options.locale.split("-");d.sceditor.locale[c[0]]&&(r=d.sceditor.locale[c[0]])}null!==r&&r.dateFormat&&(a.options.dateFormat=r.dateFormat)};(function(){e.data("sceditor",a);a.options=d.extend({},d.sceditor.defaultOptions,c);null!==a.options.locale&&"en"!==a.options.locale&&I();null!==a.options.height&&e.height(a.options.height);null!==
a.options.width&&e.width(a.options.width);(null!==a.options.height&&-1<a.options.height.toString().indexOf("%")||null!==a.options.width&&-1<a.options.width.toString().indexOf("%"))&&d(window).resize(y);f=d('<div class="sceditor-container" />').width(e.outerWidth()).height(e.outerHeight());e.after(f);E();D();F();a.options.resizeEnabled&&G();d(document).click(w);e.parents("form").attr("novalidate","novalidate").submit(x);var b=e.hide().val();a.options.getTextHandler&&(b=a.options.getTextHandler(b));
a.setWysiwygEditorValue(b);-1!=a.options.toolbar.indexOf("emoticon")&&H()})()};d.sceditor.locale={};d.sceditor.commands={bold:{exec:"bold",tooltip:"Bold"},italic:{exec:"italic",tooltip:"Italic"},underline:{exec:"underline",tooltip:"Underline"},strike:{exec:"strikethrough",tooltip:"Strikethrough"},subscript:{exec:"subscript",tooltip:"Subscript"},superscript:{exec:"superscript",tooltip:"Superscript"},left:{exec:"justifyleft",tooltip:"Align left"},center:{exec:"justifycenter",tooltip:"Center"},right:{exec:"justifyright",
tooltip:"Align right"},justify:{exec:"justifyfull",tooltip:"Justify"},font:{exec:function(b){for(var c=this,a=c.options.fonts.split(","),e=d("<div />"),f=function(a){c.execCommand("fontname",d(this).data("sceditor-font"));c.closeDropDown(!0);a.preventDefault()},i=0;i<a.length;i++)e.append(d('<a class="sceditor-font-option" href="#"><font face="'+a[i]+'">'+a[i]+"</font></a>").data("sceditor-font",a[i]).click(f));c.createDropDown(b,"font-picker",e)},tooltip:"Font Name"},size:{exec:function(b){for(var c=
this,a=d("<div />"),e=function(a){c.execCommand("fontsize",d(this).data("sceditor-fontsize"));c.closeDropDown(!0);a.preventDefault()},f=1;7>=f;f++)a.append(d('<a class="sceditor-fontsize-option" href="#"><font size="'+f+'">'+f+"</font></a>").data("sceditor-fontsize",f).click(e));c.createDropDown(b,"fontsize-picker",a)},tooltip:"Font Size"},color:{exec:function(b){for(var c=this,a={r:255,g:255,b:255},e=d("<div />"),f=this.options.colors?this.options.colors.split("|"):Array(21),i=[],g=0,h=0;h<f.length;++h){var k=
"undefined"!==typeof f[h]?f[h].split(","):Array(21);i[g++]='<div class="sceditor-color-column">';for(var j=0;j<k.length;++j){var n="undefined"!==typeof k[j]?k[j]:"#"+a.r.toString(16)+a.g.toString(16)+a.b.toString(16);i[g++]='<a href="#" class="sceditor-color-option" style="background-color: '+n+'" data-color="'+n+'"></a>';a=0===j%5?{r:a.r,g:a.g-51,b:255}:{r:a.r,g:a.g,b:a.b-51}}i[g++]="</div>";a=0===h%5?{r:a.r-51,g:255,b:255}:{r:a.r,g:255,b:255}}e.append(i.join("")).find("a").click(function(a){c.execCommand("forecolor",
d(this).attr("data-color"));c.closeDropDown(!0);a.preventDefault()});c.createDropDown(b,"color-picker",e)},tooltip:"Font Color"},removeformat:{exec:"removeformat",tooltip:"Remove Formatting"},cut:{exec:"cut",tooltip:"Cut",errorMessage:"Your browser does not allow the cut command. Please use the keyboard shortcut Ctrl/Cmd-X"},copy:{exec:"copy",tooltip:"Copy",errorMessage:"Your browser does not allow the copy command. Please use the keyboard shortcut Ctrl/Cmd-C"},paste:{exec:"paste",tooltip:"Paste",
errorMessage:"Your browser does not allow the paste command. Please use the keyboard shortcut Ctrl/Cmd-V"},pastetext:{exec:function(b){var c=this,a=d(this._('<form><div><label for="txt">{0}</label> <textarea cols="20" rows="7" id="txt"></textarea></div></form>',this._("Paste your text inside the following box:"))).submit(function(){return!1});a.append(d(this._('<div><input type="button" class="button" value="{0}" /></div>',this._("Insert"))).click(function(a){c.wysiwygEditorInsertText(d(this).parent("form").find("#txt").val());
c.closeDropDown(!0);a.preventDefault()}));c.createDropDown(b,"pastetext",a)},tooltip:"Paste Text"},bulletlist:{exec:"insertunorderedlist",tooltip:"Bullet list"},orderedlist:{exec:"insertorderedlist",tooltip:"Numbered list"},undo:{exec:"undo",tooltip:"Undo"},redo:{exec:"redo",tooltip:"Redo"},table:{exec:function(b){var c=this,a=d(this._('<form><div><label for="rows">{0}</label><input type="text" id="rows" value="2" /></div><div><label for="cols">{1}</label><input type="text" id="cols" value="2" /></div></form>',
