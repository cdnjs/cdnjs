...
var ResponseValidator = class {
  ...
  async _processClaims(response, skipUserInfo = false) {
    ...
    if (!response.isOpenId) {
      logger2.debug("response is not OIDC, skipping claims processing");
      return;
    }
    logger2.debug("response is OIDC, processing claims", response.profile);
    ...
    logger2.debug("_processClaims: user info claims received, updated profile:", response.profile);
  }
...
var OidcClient = class {
  ...
  async createSignoutRequest({
    ...
    logger2.debug("createSignoutRequest: Received end session endpoint", url);
