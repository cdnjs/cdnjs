...
(function (global, factory) {
  ...
  function createEl(tagName = 'div', properties = {}, attributes = {}, content) {
    ...
    Object.getOwnPropertyNames(properties).forEach(function (propName) {
      const val = properties[propName]; // Handle textContent since it's not supported everywhere and we have a
      // method for it.
  ...
  function getAttributes(tag) {
    const obj = {}; // known boolean attributes
    ...
    if (tag && tag.attributes && tag.attributes.length > 0) {
      ...
      for (let i = attrs.length - 1; i >= 0; i--) {
        ...
        let attrVal = attrs[i].value; // check for known booleans
  ...
  const autoSetup = function () {
    ...
    const mediaEls = vids.concat(audios, divs); // Check if any media elements exist

    if (mediaEls && mediaEls.length > 0) {
      for (let i = 0, e = mediaEls.length; i < e; i++) {
        const mediaEl = mediaEls[i]; // Check if element exists, has getAttribute func.

        if (mediaEl && mediaEl.getAttribute) {
          // Make sure this player hasn't already been set up.
          if (mediaEl.player === undefined) {
            const options = mediaEl.getAttribute('data-setup'); // Check if data-setup attr exists.
  ...
  function _cleanUpEvents(elem, type) {
    ...
    const data = DomData.get(elem); // Remove the events of a particular type if there are none left
  ...
  function on(elem, type, fn) {
    ...
    const data = DomData.get(elem); // We need a place to store all our handler data
  ...
  function off(elem, type, fn) {
    ...
    const data = DomData.get(elem); // If no events exist, nothing to unbind
    ...
    const handlers = data.handlers[type]; // If no handlers exist, nothing to unbind
  ...
  function trigger(elem, event, hash) {
    ...
    const parent = elem.parentNode || elem.ownerDocument; // type = event.type || event,
    // handler;
    ...
    if (parent && !event.isPropagationStopped() && event.bubbles === true) {
      ...
      const targetData = DomData.get(event.target); // Checks if the target has a default action for this event.
  ...
  EventTarget.prototype.trigger = function (event) {
    const type = event.type || event; // deprecation
  ...
  *         The evented object we are validating for
  ...
  const EventedMixin = {
    ...
    one(...args) {
      const {
        ...
        type,
        listener
      } = normalizeListenArgs(this, args, 'one'); // Targeting this evented object.
    ...
    any(...args) {
      const {
        ...
        type,
        listener
      } = normalizeListenArgs(this, args, 'any'); // Targeting this evented object.
    ...
    off(targetOrType, typeOrListener, listener) {
      ...
      if (!targetOrType || isValidEventType(targetOrType)) {
        ...
        const type = typeOrListener; // Fail fast and in a meaningful way!
  ...
  function evented(target, options = {}) {
    const {
      eventBusKey
    } = options; // Set or create the eventBusEl_.
  ...
  function stateful(target, defaultState) {
    Object.assign(target, StatefulMixin); // This happens after the mixing-in because we need to replace the `state`
    // added in that step.
  ...
  class Component {
    ...
    constructor(player, options, ready) {
      ...
      this.options_ = merge({}, this.options_); // Updated options with supplied options

      options = this.options_ = merge(this.options_, options); // Get ID from options or options element if one is supplied
    ...
    addChild(child, options = {}, index = this.children_.length) {
      let component;
      let componentName; // If child is a string, create component with options

      if (typeof child === 'string') {
        componentName = toTitleCase(child);
        const componentClassName = options.componentClass || componentName; // Set name through options
    ...
    initChildren() {
      ...
      if (children) {
        ...
        const handleAdd = child => {
          ...
          let opts = child.opts; // Allow options for children to be set at the parent options
    ...
    triggerReady() {
      ...
      this.setTimeout(function () {
        const readyQueue = this.readyQueue_; // Reset Ready Queue
    ...
    emitTapEvents() {
      ...
      let touchStart = 0;
      let firstTouch = null; // Maximum movement allowed during a touch event to still be considered a tap
      ...
      this.on('touchend', function (event) {
        ...
        if (couldBeTap === true) {
          ...
          const touchTime = window.performance.now() - touchStart; // Make sure the touch was less than the threshold to be considered a tap
    ...
    enableTouchActivity() {
      ...
      const report = bind(this.player(), this.player().reportUserActivity);
      let touchHolding;
    ...
    static registerComponent(name, ComponentToRegister) {
      ...
      const Tech = Component.getComponent('Tech'); // We need to make sure this check is only done if Tech has been registered.
      ...
      if (name === 'Player' && Player && Player.players) {
        ...
        const playerNames = Object.keys(players); // If we have players that were disposed, then their name will still be
  ...
  class ModalDialog extends Component {
    ...
    constructor(player, options) {
      ...
      this.content(this.options_.content); // Make sure the contentEl is defined AFTER any children are initialized
    ...
    description() {
      let desc = this.options_.description || this.localize('This is a modal window.'); // Append a universal closeability message if the modal is closeable.
    ...
    closeable(value) {
      if (typeof value === 'boolean') {
        ...
        let close = this.getChild('closeButton'); // If this is being made closeable and has no close button, add one.
  ...
  class AudioTrackList extends TrackList {
    ...
    addTrack(track) {
      ...
      super.addTrack(track); // native tracks don't have this
  ...
  class TextTrackList extends TrackList {
    ...
    removeTrack(rtrack) {
      super.removeTrack(rtrack); // manually remove the event handlers we added
  ...
  const isCrossOrigin = function (url, winLoc = window.location) {
    const urlInfo = parseUrl(url); // IE8 protocol relative urls will return ':' for protocol
  ...
  class TextTrack extends Track {
    ...
    constructor(options = {}) {
      ...
      let changed = false;
      this.timeupdateHandler = bind(this, function () {
  ...
  class Tech extends Component {
    ...
    */
    constructor(options = {}, ready = function () {}) {
      ...
      this.queuedHanders_ = new Set(); // keep track of whether the current source has played at all to
      ...
      this.initTrackListeners(); // Turn on component tap events only if not using native controls
    ...
    removeRemoteTextTrack(track) {
      const trackElement = this.remoteTextTrackEls().getTrackElementByTrack_(track); // remove HTMLTrackElement and TextTrack from remote list
  ...
  function mediate(middleware, tech, method, arg = null) {
    ...
    const terminated = middlewareValue === TERMINATOR; // deprecated. The `null` return value should instead return TERMINATOR to
  ...
  function setSourceHelper(src = {}, middleware = [], next, player, acc = [], lastRun = false) {
    const [mwFactory, ...mwrest] = middleware; // if mwFactory is a string, then we're at a fork in the road

    if (typeof mwFactory === 'string') {
      setSourceHelper(src, middlewares[mwFactory], next, player, acc, lastRun); // if we have an mwFactory, call it with the player to get the mw,
      // then call the mw's setSource method
    } else if (mwFactory) {
      const mw = getOrCreateFactory(player, mwFactory); // if setSource isn't present, implicitly select this middleware
  ...
  class MediaLoader extends Component {
    ...
    constructor(player, options, ready) {
      ...
      super(player, options_, ready); // If there are no sources when the player is initialized,
      // load the first supported playback technology.

      if (!options.playerOptions.sources || options.playerOptions.sources.length === 0) {
        for (let i = 0, j = options.playerOptions.techOrder; i < j.length; i++) {
          const techName = toTitleCase(j[i]);
          let tech = Tech.getTech(techName); // Support old behavior of techs being registered as components.
  ...
  class PosterImage extends ClickableComponent {
    ...
    setSrc(url) {
      let backgroundImage = ''; // Any falsy value should stay as an empty string, otherwise
  ...
  class TextTrackDisplay extends Component {
    ...
    constructor(player, options, ready) {
      ...
      player.on('loadedmetadata', e => this.preselectTrack(e)); // This used to be called during player init, but was causing an error
    ...
    */
    ...
    updateForTrack(tracks) {
      ...
      const cues = []; // push all active track cues
  ...
  class BigPlayButton extends Button {
    ...
    handleClick(event) {
      const playPromise = this.player_.play(); // exit early if clicked via the mouse
  ...
  class PlayToggle extends Button {
     ...
     */
    constructor(player, options = {}) {
      super(player, options); // show or hide replay icon
  ...
  class DurationDisplay extends TimeDisplay {
    ...
    constructor(player, options) {
      ...
      const updateContent = e => this.updateContent(e); // we do not want to/need to throttle duration changes,
  ...
  class RemainingTimeDisplay extends TimeDisplay {
    ...
    updateContent(event) {
      ...
      let time; // @deprecated We should only use remainingTimeDisplay
      // as of video.js 7
  ...
  class Slider extends Component {
    ...
    */
    constructor(player, options) {
      ...
      this.update_ = e => this.update(e); // Set property names to bar to match with the child Slider class is looking for
  ...
  class TimeTooltip extends Component {
    ...
    update(seekBarRect, seekBarPoint, content) {
      ...
      const seekBarPointPx = seekBarRect.width * seekBarPoint; // do nothing if either rect isn't available
  ...
  class SeekBar extends Slider {
    ...
    handleMouseUp(event) {
      super.handleMouseUp(event); // Stop event propagation to prevent double fire in progress-control.js
  ...
  class ProgressControl extends Component {
    ...
    handleMouseMove(event) {
      ...
      let seekBarPoint = getPointerPosition(seekBarEl, event).x; // The default skin has a gap on either side of the `SeekBar`. This means
  ...
  class VolumeControl extends Component {
    ...
    constructor(player, options = {}) {
      ...
      super(player, options); // hide this control if volume support is missing
      ...
      this.on('mousemove', e => this.handleMouseMove(e)); // while the slider is active (the mouse has been pressed down and
  ...
  class MuteToggle extends Button {
    ...
    updateIcon_() {
      const vol = this.player_.volume();
      let level = 3; // in iOS when a player is loaded with muted attribute
  ...
  class VolumePanel extends Component {
    ...
    constructor(player, options = {}) {
      ...
      super(player, options); // this handler is used by mouse handler methods below
      ...
      this.on('mouseout', e => this.handleMouseOut(e)); // while the slider is active (the mouse has been pressed down and
  ...
  class Menu extends Component {
    ...
    constructor(player, options) {
      ...
      this.on('keydown', e => this.handleKeyDown(e)); // All the menu item instances share the same blur handler provided by the menu container.
    ...
    handleBlur(event) {
      const relatedTarget = event.relatedTarget || document.activeElement; // Close menu popup when a user clicks outside the menu
  ...
  class MenuButton extends Component {
    ...
    constructor(player, options = {}) {
      ...
      this.menuButton_.el_.setAttribute('aria-haspopup', 'true'); // Add buildCSSClass values to the button, not the wrapper
    ...
    buildWrapperCSSClass() {
      let menuButtonClass = 'vjs-menu-button'; // If the inline option is passed, we want to use different styles altogether.
  ...
  class TrackButton extends MenuButton {
    ...
    constructor(player, options) {
      ...
      const updateHandler = bind(this, this.update);
  ...
  class TextTrackMenuItem extends MenuItem {
    ...
    constructor(player, options) {
      ...
      const tracks = player.textTracks(); // Modify options for parent MenuItem class's init.

      options.label = track.label || track.language || 'Unknown';
      options.selected = track.mode === 'showing';
      super(player, options);
      this.track = track; // Determine the relevant kind(s) of tracks for this component and filter
      // out empty kinds.
    ...
    handleClick(event) {
      ...
      for (let i = 0; i < tracks.length; i++) {
        const track = tracks[i]; // If the track from the text tracks list is not of the right kind,
    ...
    handleTracksChange(event) {
      const shouldBeSelected = this.track.mode === 'showing'; // Prevent redundant selected() calls because they may cause
    ...
    handleSelectedLanguageChange(event) {
      if (this.track.mode === 'showing') {
        const selectedLanguage = this.player_.cache_.selectedLanguage; // Don't replace the kind of track across the same language
  ...
  class TextTrackButton extends TrackButton {
    ...
    createItems(items = [], TrackMenuItem = TextTrackMenuItem) {
      ...
      for (let i = 0; i < tracks.length; i++) {
        const track = tracks[i]; // only add tracks that are of an appropriate kind and have a label
  ...
  class ChaptersTrackMenuItem extends MenuItem {
    ...
    constructor(player, options) {
      ...
      const currentTime = player.currentTime(); // Modify options for parent MenuItem class's init.
  ...
  class DescriptionsButton extends TextTrackButton {
    ...
    constructor(player, options, ready) {
      ...
      const changeHandler = bind(this, this.handleTracksChange);
      tracks.addEventListener('change', changeHandler);
      this.on('dispose', function () {
        tracks.removeEventListener('change', changeHandler);
      });
    ...
    handleTracksChange(event) {
      const tracks = this.player().textTracks();
      let disabled = false; // Check whether a track of a different kind is showing
  ...
  class SubsCapsButton extends TextTrackButton {
    constructor(player, options = {}) {
      super(player, options); // Although North America uses "captions" in most cases for
  ...
  class AudioTrackMenuItem extends MenuItem {
    ...
    constructor(player, options) {
      const track = options.track;
      const tracks = player.audioTracks(); // Modify options for parent MenuItem class's init.
    ...
    handleClick(event) {
      super.handleClick(event); // the audio track list will automatically toggle other tracks
      // off for us.
      ...
      if (this.player_.tech_.featuresNativeAudioTracks) {
        ...
        for (let i = 0; i < tracks.length; i++) {
          const track = tracks[i]; // skip the current track since we enabled it above
  ...
  class PlaybackRateMenuItem extends MenuItem {
    ...
    constructor(player, options) {
      ...
      const rate = parseFloat(label, 10); // Modify options for parent MenuItem class's init.
  ...
  const OPACITY_TRANS = ['0', 'Transparent']; // Configuration for the various <select> elements in the DOM of this component.
  //
  ...
  class TextTrackSettings extends ModalDialog {
    ...
    constructor(player, options) {
      ...
      super(player, options);
      this.updateDisplay = this.updateDisplay.bind(this); // fill the modal and pretend we have opened it
      ...
      this.setDefaults(); // Grab `persistTextTrackSettings` from the player options if not passed in child options
  ...
  class ResizeManager extends Component {
     ...
     */
    constructor(player, options) {
      let RESIZE_OBSERVER_AVAILABLE = options.ResizeObserver || window.ResizeObserver; // if `null` was passed, we want to disable the ResizeObserver
  ...
  class LiveTracker extends Component {
    ...
    constructor(player, options) {
      ...
      this.reset_();
      this.on(this.player_, 'durationchange', e => this.handleDurationchange(e)); // we should try to toggle tracking on canplay as native playback engines, like Safari
    ...
    trackLive_() {
      const seekable = this.player_.seekable(); // skip undefined seekable
      ...
      const currentTime = this.player_.currentTime(); // we are behind live if any are true
    ...
    liveWindow() {
      const liveCurrentTime = this.liveCurrentTime(); // if liveCurrenTime is Infinity then we don't have a liveWindow at all
  ...
  const sourcesetLoad = tech => {
    const el = tech.el(); // if `el.src` is set, that source will be loaded.
    ...
    const srcUrls = [];
    let src = ''; // if there are no sources, do not fire sourceset
  ...
  const firstSourceWatch = function (tech) {
    const el = tech.el(); // make sure firstSourceWatch isn't setup twice.
  ...
  const setupSourceset = function (tech) {
    ...
    const el = tech.el(); // make sure sourceset isn't setup twice.
    ...
    Object.defineProperty(el, 'src', merge(srcDescriptor, {
      set: v => {
        const retval = srcDescriptor.set.call(el, v); // we use the getter here to get the actual value set on src
    ...
    el.load = () => {
      const retval = oldLoad.call(el); // if load was called, but there was no source to fire
  ...
  class Html5 extends Tech {
    ...
    */
    constructor(options, ready) {
      ...
      let crossoriginTracks = false; // Set the source if one is provided
    ...
    restoreMetadataTracksInIOSNativePlayer_() {
      ...
      let metadataTracksPreFullscreenState; // captures a snapshot of every metadata track's current state
    ...
    createEl() {
      let el = this.options_.tag; // Check if this browser supports moving the element into the box.
    ...
    supportsFullScreen() {
      if (typeof this.el_.webkitEnterFullScreen === 'function') {
        const userAgent = window.navigator && window.navigator.userAgent || ''; // Seems to be broken in Chromium/Chrome && Safari in Leopard
  ...
  Html5.canControlVolume = function () {
    ...
    try {
      ...
      const canControl = volume !== Html5.TEST_VID.volume; // With the introduction of iOS 15, there are cases where the volume is read as
  ...
  Html5.canMuteVolume = function () {
    try {
      const muted = Html5.TEST_VID.muted; // in some versions of iOS muted property doesn't always
  ...
  const BREAKPOINT_CLASSES = {}; // grep: vjs-layout-tiny
  ...
  class Player extends Component {
    ...
    constructor(tag, options, ready) {
      ...
      this.boundHandleTechTap_ = e => this.handleTechTap_(e); // default isFullscreen_ to false
      ...
      this.playCallbacks_ = [];
      this.playTerminatedQueue_ = []; // the attribute overrides the option
      ...
      this.el_ = this.createEl(); // Make this an evented object and use `el_` as its event bus.
      ...
      const playerOptionsCopy = merge(this.options_); // Load plugins
      ...
      this.initChildren(); // Set isAudio based on whether or not an audio tag was used
      ...
      this.addClass(`vjs-v${majorVersion}`); // When the player is first initialized, trigger activity so components
      ...
      this.responsive(this.options_.responsive); // Calling both the audio mode methods after the player is fully
    ...
    dispose() {
      ...
      ALL.names.forEach(name => {
        const props = ALL[name];
        const list = this[props.getterName](); // if it is not a native list
    ...
    createEl() {
      ...
      attrs.tabindex = '-1'; // Workaround for #4583 on Chrome (on Windows) with JAWS.
    ...
    updateStyleEl_() {
      ...
      let height;
      let aspectRatio;
      let idClass; // The aspect ratio is either used directly or to calculate width and height.
    ...
    loadTech_(techName, source) {
      ...
      const camelTechName = techName.charAt(0).toLowerCase() + techName.slice(1); // get rid of the HTML5 video tag as soon as we are using another tech
      ...
      this.isReady_ = false;
      let autoplay = this.autoplay(); // if autoplay is a string (or `true` with normalizeAutoplay: true) we pass false to the tech
    ...
    manualAutoplay_(type) {
      ...
      let promise; // if muted defaults to true
    ...
    handleTechSourceset_(event) {
      ...
      if (!this.changingSrc_) {
        ...
        const eventSrc = event.src; // if we have a playerSrc that is not a blob, and a tech src that is a blob
    ...
    documentFullscreenChange_(e) {
      const targetPlayer = e.target.player; // if another player was fullscreen
    ...
    play_(callback = silencePromise) {
      ...
      const isSrcReady = Boolean(!this.changingSrc_ && (this.src() || this.currentSrc())); // treat calls to play_ somewhat like the `one` event function
    ...
    requestFullscreenHelper_(fullscreenOptions) {
      let fsOptions; // Only pass fullscreen options to requestFullscreen in spec-compliant browsers.
      ...
      if (this.fsApi_.requestFullscreen) {
        const promise = this.el_[this.fsApi_.requestFullscreen](fsOptions); // Even on browsers with promise support this may not return a promise
    ...
    exitFullscreenHelper_() {
      if (this.fsApi_.requestFullscreen) {
        const promise = document[this.fsApi_.exitFullscreen](); // Even on browsers with promise support this may not return a promise
    ...
    handleKeyDown(event) {
      const {
        userActions
      } = this.options_; // Bail out if hotkeys are not configured.
      ...
      const excludeElement = el => {
        const tagName = el.tagName.toLowerCase(); // The first and easiest test is for `contenteditable` elements.
    ...
    handleHotkeys(event) {
      const hotkeys = this.options_.userActions ? this.options_.userActions.hotkeys : {}; // set fullscreenKey, muteKey, playPauseKey from `hotkeys`, use defaults if not set
    ...
    canPlayType(type) {
      let can; // Loop through each playback technology in the options order
    ...
    handleSrc_(source, isRetry) {
      ...
      setSource(this, sources[0], (middlewareSource, mws) => {
        this.middleware_ = mws; // since sourceSet is async we have to update the cache again after we select a source since
        ...
        if (err) {
          ...
          this.changingSrc_ = false; // We need to wrap this in a timeout to give folks a chance to add error event handlers
    ...
    autoplay(value) {
      ...
      let techAutoplay; // if the value is a valid string set it to that, or normalize `true` to 'play', if need be
    ...
    listenForUserActivity_() {
      ...
      let lastMoveX;
      let lastMoveY;
      const handleActivity = bind(this, this.reportUserActivity);
      ...
      const controlBar = this.getChild('controlBar'); // Fixes bug on Android & iOS where when tapping progressBar (when control bar is displayed)
    ...
    enableAudioOnlyUI_() {
      ...
      const controlBarHeight = controlBar && controlBar.currentHeight(); // Hide all player components except the control bar. Control bar components
    ...
    audioOnlyMode(value) {
      ...
      this.audioOnlyMode_ = value; // Enable Audio Only Mode

      if (value) {
        const exitPromises = []; // Fullscreen and PiP are not supported in audioOnlyMode, so exit if we need to.
    ...
    toJSON() {
      ...
      for (let i = 0; i < tracks.length; i++) {
        let track = tracks[i]; // deep merge tracks and null out player so no circular references
    ...
    breakpoints(breakpoints) {
      ...
      this.breakpoint_ = '';
      this.breakpoints_ = Object.assign({}, DEFAULT_BREAKPOINTS, breakpoints); // When breakpoint definitions change, we need to update the currently
      // selected breakpoint.
    ...
    responsive(value) {
      ...
      value = Boolean(value);
      const current = this.responsive_; // Nothing changed.
    ...
    loadMedia(media, ready) {
      ...
      const {
        ...
        textTracks,
        title
      } = this.cache_.media; // If `artwork` is not given, create it using `poster`.
    ...
    static getTagSettings(tag) {
      ...
      Object.assign(baseOptions, tagOptions); // Get tag children settings

      if (tag.hasChildNodes()) {
        const children = tag.childNodes;

        for (let i = 0, j = children.length; i < j; i++) {
          const child = children[i]; // Change case needed: http://ejohn.org/blog/nodename-case-sensitivity/
  ...
  const createPluginFactory = (name, PluginSubClass) => {
    ...
    return function (...args) {
      ...
      const instance = new PluginSubClass(...[this, ...args]); // The plugin is replaced by a function that returns the current instance.
  ...
  videojs.getPlayer = id => {
    ...
    if (isEl(tag)) {
      const {
        player,
        playerId
      } = tag; // Element may have a `player` property referring to an already created
  ...
  videojs.bind = deprecateForMajor(9, 'videojs.bind', 'native Function.prototype.bind', bind);
