(function (global, factory) {
	...
	Euler.prototype = {
		...
		setFromRotationMatrix: function ( m, order ) {
			...
			if ( order === 'XYZ' ) {
				...
				this._y = _Math.asin( _Math.clamp( m13, - 1, 1 ) );
				...
				this._x = _Math.asin( - _Math.clamp( m23, - 1, 1 ) );
				...
				this._x = _Math.asin( _Math.clamp( m32, - 1, 1 ) );
				...
				this._y = _Math.asin( - _Math.clamp( m31, - 1, 1 ) );
				...
				this._z = _Math.asin( _Math.clamp( m21, - 1, 1 ) );
				...
				this._z = _Math.asin( - _Math.clamp( m12, - 1, 1 ) );
		...
		setFromQuaternion: function ( q, order, update ) {
			...
			if ( order === 'XYZ' ) {
				...
				this._x = _Math.atan2( 2 * ( q.x * q.w - q.y * q.z ), ( sqw - sqx - sqy + sqz ) );
				this._y = _Math.asin(  _Math.clamp( 2 * ( q.x * q.z + q.y * q.w ), - 1, 1 ) );
				this._z = _Math.atan2( 2 * ( q.z * q.w - q.x * q.y ), ( sqw + sqx - sqy - sqz ) );

			} else if ( order ===  'YXZ' ) {

				this._x = _Math.asin(  _Math.clamp( 2 * ( q.x * q.w - q.y * q.z ), - 1, 1 ) );
				this._y = _Math.atan2( 2 * ( q.x * q.z + q.y * q.w ), ( sqw - sqx - sqy + sqz ) );
				this._z = _Math.atan2( 2 * ( q.x * q.y + q.z * q.w ), ( sqw - sqx + sqy - sqz ) );

			} else if ( order === 'ZXY' ) {

				this._x = _Math.asin(  _Math.clamp( 2 * ( q.x * q.w + q.y * q.z ), - 1, 1 ) );
				this._y = _Math.atan2( 2 * ( q.y * q.w - q.z * q.x ), ( sqw - sqx - sqy + sqz ) );
				this._z = _Math.atan2( 2 * ( q.z * q.w - q.x * q.y ), ( sqw - sqx + sqy - sqz ) );

			} else if ( order === 'ZYX' ) {

				this._x = _Math.atan2( 2 * ( q.x * q.w + q.z * q.y ), ( sqw - sqx - sqy + sqz ) );
				this._y = _Math.asin(  _Math.clamp( 2 * ( q.y * q.w - q.x * q.z ), - 1, 1 ) );
				this._z = _Math.atan2( 2 * ( q.x * q.y + q.z * q.w ), ( sqw + sqx - sqy - sqz ) );

			} else if ( order === 'YZX' ) {

				this._x = _Math.atan2( 2 * ( q.x * q.w - q.z * q.y ), ( sqw - sqx + sqy - sqz ) );
				this._y = _Math.atan2( 2 * ( q.y * q.w - q.x * q.z ), ( sqw + sqx - sqy - sqz ) );
				this._z = _Math.asin(  _Math.clamp( 2 * ( q.x * q.y + q.z * q.w ), - 1, 1 ) );

			} else if ( order === 'XZY' ) {

				this._x = _Math.atan2( 2 * ( q.x * q.w + q.y * q.z ), ( sqw - sqx + sqy - sqz ) );
				this._y = _Math.atan2( 2 * ( q.x * q.z + q.y * q.w ), ( sqw + sqx - sqy - sqz ) );
				this._z = _Math.asin(  _Math.clamp( 2 * ( q.z * q.w - q.x * q.y ), - 1, 1 ) );

			} else {

				console.warn( '_Math.Euler: .setFromQuaternion() given unsupported order: ' + order );
	...
	*/
	...
	var count$1 = 0;
	function ShapeIdCount() { return count$1++; }
	...
	*/
	...
	RigidBody.prototype = {
	    ...
	    */
	    ...
	    setPosition: function ( pos ) {
	        ...
	        //this.newPosition.set(pos.x*OIMO.WorldScale.invScale,pos.y*OIMO.WorldScale.invScale,pos.z*OIMO.WorldScale.invScale);
	...
	exports.Mat44 = Mat44;
	exports.AABB = AABB;
	exports.BasicProxy = BasicProxy;
	exports.Pair = Pair;
	exports.BroadPhase = BroadPhase;
	exports.BruteForceBroadPhase = BruteForceBroadPhase;
	exports.DBVTNode = DBVTNode;
	exports.DBVT = DBVT;
	exports.DBVTProxy = DBVTProxy;
	exports.DBVTBroadPhase = DBVTBroadPhase;
	exports.SAPAxis = SAPAxis;
	exports.SAPElement = SAPElement;
	exports.SAPProxy = SAPProxy;
	exports.SAPBroadPhase = SAPBroadPhase;
	exports.CollisionDetector = CollisionDetector;
	exports.BoxBoxCollisionDetector = BoxBoxCollisionDetector;
	exports.BoxCylinderCollisionDetector = BoxCylinderCollisionDetector;
	exports.CylinderCylinderCollisionDetector = CylinderCylinderCollisionDetector;
	exports.SphereBoxCollisionDetector = SphereBoxCollisionDetector;
	exports.SphereCylinderCollisionDetector = SphereCylinderCollisionDetector;
	exports.SphereSphereCollisionDetector = SphereSphereCollisionDetector;
	exports.MassInfo = MassInfo;
	...
	exports.ImpulseDataBuffer = ImpulseDataBuffer;
	exports.ContactPointDataBuffer = ContactPointDataBuffer;
	...
	exports.LinearConstraint = LinearConstraint;
	exports.Rotational3Constraint = Rotational3Constraint;
	exports.RotationalConstraint = RotationalConstraint;
	exports.Translational3Constraint = Translational3Constraint;
	exports.TranslationalConstraint = TranslationalConstraint;
	exports.Joint = Joint;
	exports.JointLink = JointLink;
	...
	exports.ProxyIdCount = ProxyIdCount;
	exports.Proxy = Proxy;
