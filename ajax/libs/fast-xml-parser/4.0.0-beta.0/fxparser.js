...
var buildOptions=__webpack_require__(/*! ../util */ "./src/util.js").buildOptions;var buildFromOrderedJs=__webpack_require__(/*! ./orderedJs2Xml */ "./src/xmlbuilder/orderedJs2Xml.js");var defaultOptions={attributeNamePrefix:'@_',attributesGroupName:false,textNodeName:'#text',ignoreAttributes:true,cdataTagName:false,format:false,indentBy:'  ',suppressEmptyNode:false,tagValueProcessor:function tagValueProcessor(key,a){return a;},attributeValueProcessor:function attributeValueProcessor(attrName,a){return a;},preserveOrder:false};var props=['attributeNamePrefix','attributesGroupName','textNodeName','ignoreAttributes','cdataTagName','format','indentBy','suppressEmptyNode','tagValueProcessor','attributeValueProcessor','arrayNodeName',//when array as root
'preserveOrder'// 'rootNodeName', //when jsObject have multiple properties on root level
...
}}function buildTextValNode(val,key,attrStr,level){return this.indentate(level)+'<'+key+attrStr+'>'+this.options.tagValueProcessor(key,val)+'</'+key+this.tagEndChar;}function buildEmptyTextNode(val,key,attrStr,level){if(val!==''){return this.buildTextValNode(val,key,attrStr,level);}else{return this.indentate(level)+'<'+key+attrStr+'/'+this.tagEndChar;}}function indentate(level){return this.options.indentBy.repeat(level);}function isAttribute(name/*, options*/){if(name.startsWith(this.options.attributeNamePrefix)){return name.substr(this.attrPrefixLen);}else{return false;}}//formatting
//indentation
...
var _require=__webpack_require__(/*! os */ "./node_modules/os-browserify/browser.js"),EOL=_require.EOL;function toXml(jObj,options){return arrToStr([jObj],options,0);}function arrToStr(arr,options,level){var xmlStr="";var indentation="";if(options.format&&options.indentBy.length>0){//TODO: this logic can be avoided for each call
indentation=EOL+""+options.indentBy.repeat(level);}for(var i=0;i<arr.length;i++){var tagObj=arr[i];var tagName=propName(tagObj);if(tagName===options.textNodeName){xmlStr+=indentation+options.tagValueProcessor(tagName,tagObj[tagName]);continue;}else if(tagName===options.cdataTagName){xmlStr+=indentation+("<![CDATA["+tagObj[tagName][0][options.textNodeName]+"]]>");continue;}var attStr=attr_to_str(tagObj.attributes,options);var tagStart=indentation+("<"+tagName+attStr);var tagValue=arrToStr(tagObj[tagName],options,level+1);if((!tagValue||tagValue.length===0)&&options.suppressEmptyNode){xmlStr+=tagStart+"/>";}else{//TODO: node with only text value should not parse the text value in next line
...
cdataTagName:false,numberParseOptions:{hex:true,leadingZeros:true},tagValueProcessor:function tagValueProcessor(tagName,val){return val;},attributeValueProcessor:function attributeValueProcessor(attrName,val){return val;},stopNodes:[],//nested tags will not be parsed even for errors
...
if(options.cdataTagName){var _ref2;var val=parseValue(_tagExp,options,options.cdataTagName,jPath+"."+options.cdataTagName,true,false,true);if(!val)val="";currentNode.add(options.cdataTagName,[(_ref2={},_ref2[options.textNodeName]=val,_ref2)]);}else{var _val=parseValue(_tagExp,options,currentNode.tagname,jPath,true,false,true);if(!_val)_val="";currentNode.add(options.textNodeName,_val);}i=_closeIndex2+2;}else{//Opening tag
...
if(_tagName!==_tagExp2&&shouldBuildAttributesMap){_childNode.attrsMap=buildAttributesMap(_tagExp2,jPath,options);}currentNode.addChild(_childNode);currentNode=_childNode;}textData="";i=_closeIndex3;}}else{textData+=xmlData[i];}}return xmlObj.child[0];};//TODO: use jPath to simplify the logic
/**
 * 
