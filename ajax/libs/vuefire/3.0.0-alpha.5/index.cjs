...
function useFirebaseApp(name) {
  const firebaseApp = vue.getCurrentScope() && vue.inject(
...
function getDataSourcePath(source) {
  return isFirestoreDataReference(source) ? source.path : isDatabaseReference(source) ? source.toString() : null;
}
function usePendingPromises(name) {
  const app = useFirebaseApp(name);
  const pendingPromises = appPendingPromises.get(app);
  return pendingPromises ? Promise.all(
