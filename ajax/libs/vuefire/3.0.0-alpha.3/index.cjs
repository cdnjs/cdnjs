...
function rtdbBindAsObject({ target, document, resolve, reject, ops }, extraOptions = DEFAULT_OPTIONS$1) {
  ...
  return (reset) => {
    database.off(document, "value", listener);
    if (reset !== false) {
...
function rtdbBindAsArray({ target, collection, resolve, reject, ops }, extraOptions = DEFAULT_OPTIONS$1) {
  ...
  const childAdded = database.onChildAdded(
    collection,
    (snapshot, prevKey) => {
  ...
  const childRemoved = database.onChildRemoved(
    collection,
    (snapshot) => {
      const array = vueDemi.unref(arrayRef);
  ...
  const childChanged = database.onChildChanged(
    collection,
    (snapshot) => {
      const array = vueDemi.unref(arrayRef);
      ops.set(
  ...
  const childMoved = database.onChildMoved(
    collection,
    (snapshot, prevKey) => {
      const array = vueDemi.unref(arrayRef);
  ...
  return (reset) => {
    database.off(collection, "child_added", childAdded);
    database.off(collection, "child_removed", childRemoved);
    database.off(collection, "child_changed", childChanged);
    database.off(collection, "child_moved", childMoved);
    if (reset !== false) {
...
function bindCollection(target, collection, ops, resolve, reject, extraOptions = DEFAULT_OPTIONS) {
  ...
  const unbind = firestore.onSnapshot(
    collection,
    (snapshot) => {
