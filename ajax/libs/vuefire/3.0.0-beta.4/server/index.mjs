...
import { g as getGlobalScope, A as AppCheckTokenInjectSymbol, x as AppCheckMap, m as authUserMap, y as _setInitialUser } from '../shared/vuefire.168e85f4.mjs';
...
function VueFireAppCheckServer(app, adminApp, firebaseApp, {
  ...
  const appCheck = initializeAppCheck(firebaseApp, {
    provider: new CustomProvider({
      getToken: () => getAppCheck(adminApp).createToken(firebaseApp.options.appId, { ttlMillis }).then(({ token: token2, ttlMillis: expireTimeMillis }) => ({
        token: token2,
        expireTimeMillis
      }))
    }),
