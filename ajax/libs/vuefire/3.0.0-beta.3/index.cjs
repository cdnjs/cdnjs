...
function databasePlugin(app, pluginOptions, firebaseApp) {
  ...
  GlobalTarget[bindName] = function databaseBind(key, source, userOptions) {
    ...
    const scope = user.getGlobalScope(firebaseApp || user.useFirebaseApp(), app).run(
      () => vueDemi.effectScope()
    );
...
function useDatabase(name) {
  return database.getDatabase(user.useFirebaseApp(name));
}
...
function useFirestore(name) {
  return firestore.getFirestore(user.useFirebaseApp(name));
}
...
function VueFireAuth(initialUser) {
  return (firebaseApp, app) => {
    ...
    user.setupOnAuthStateChanged(user$1, firebaseApp);
  };
}
function useFirebaseAuth(name) {
  return isClient ? auth.getAuth(user.useFirebaseApp(name)) : null;
}
...
function VueFireAppCheck(options) {
  return (firebaseApp, app) => {
    const token = user.getGlobalScope(firebaseApp, app).run(() => vueDemi.ref());
...
function useAppCheck(name) {
  return AppCheckMap.get(user.useFirebaseApp(name));
}

function useFirebaseStorage(name) {
  return storage.getStorage(user.useFirebaseApp(name));
}
...
function VueFire(app, { firebaseApp, modules = [] }) {
  app.provide(user._FirebaseAppInjectionKey, firebaseApp);
  for (const firebaseModule of modules) {
    app.use(firebaseModule.bind(null, firebaseApp));
  }
}

exports.getCurrentUser = user.getCurrentUser;
exports.updateCurrentUserProfile = user.updateCurrentUserProfile;
exports.useCurrentUser = user.useCurrentUser;
exports.useFirebaseApp = user.useFirebaseApp;
