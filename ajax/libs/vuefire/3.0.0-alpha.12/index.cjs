...
function isSSR() {
  return !!(vueDemi.getCurrentInstance() && vueDemi.inject(vueDemi.ssrContextKey, null));
}
...
function extractRefs(doc, oldDoc, subs) {
  ...
  function recursiveExtract(doc2, oldDoc2, path, result) {
    ...
    for (const key in doc2) {
      const ref = doc2[key];
      if (ref == null || ref instanceof Date || ref instanceof firestore.Timestamp || ref instanceof firestore.GeoPoint) {
        data[key] = ref;
...
function useStorageUrl(storageRef) {
  ...
  const promise = vueDemi.ref(Promise.resolve(null));
  let removePendingPromise = noop;
  function refresh() {
