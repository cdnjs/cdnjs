...
import { getApps, initializeApp, cert, applicationDefault, getApp } from 'firebase-admin/app';
...
async function decodeUserToken(token, adminApp) {
  if (token) {
...
function getAdminApp(firebaseAdminOptions) {
  if (!getApps().length) {
    const {
...
export { AUTH_COOKIE_NAME, VueFireAppCheckServer, VueFireAuthServer, createServerUser, decodeUserToken, getAdminApp, isFirebaseError };
