...
const START_UPLOAD_EMPTY_VALUES = [null, null, null];
function useStorageTask(storageRef, data = vue.shallowRef(), metadata = {}) {
  ...
  let unsub = noop;
  function startUpload(_ = [], [previousStorageSource, previousData, previousMetadata] = START_UPLOAD_EMPTY_VALUES) {
    const storageSource = vue.unref(storageRef);
    const newData = vue.unref(data);
    const newMetadata = vue.unref(metadata);
    ...
    if (storageSource && (newData || previousData)) {
      if (previousData === newData && previousStorageSource === storageSource) {
        storage.updateMetadata(storageSource, newMetadata);
      } else if (!newData) {
        if (previousData) {
          storage.deleteObject(storageSource);
        } else {
          storage.getDownloadURL(storageSource).then((downloadUrl) => {
            url.value = downloadUrl;
          });
        }
      } else {
        const newTask = storage.uploadBytesResumable(
          storageSource,
          newData,
          newMetadata
        );
        ...
        newTask.then(() => {
          uploadTask.value = null;
          unsub();
          storage.getDownloadURL(newTask.snapshot.ref).then((downloadUrl) => {
            url.value = downloadUrl;
          });
        });
        newTask.catch((err) => {
  ...
  if (vue.isRef(storageRef) || vue.isRef(data) || vue.isRef(metadata)) {
    vue.watch(
      () => [vue.unref(storageRef), vue.unref(data), vue.unref(metadata)],
      startUpload,
      { immediate: true }
    );
  } else {
