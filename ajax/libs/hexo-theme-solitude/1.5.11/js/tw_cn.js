document.addEventListener('DOMContentLoaded', function () {
    ...
    let targetEncoding = Number(saveToLocal.get(targetEncodingCookie)) || defaultEncoding;
    ...
    const translateBody = (fobj) => {
        const objs = fobj ? fobj.childNodes : document.body.childNodes;
        for (let obj of objs) {
            if (obj.tagName === 'BR' || obj.tagName === 'HR' || obj === translateButtonObject) continue;
    ...
    const translateChar = (cc, source, target) => {
        ...
        for (let char of cc) {
            ...
            str += char.charCodeAt(0) > 10000 && index !== -1 ? target.charAt(index) : char;
        }
        return str;
    }

    const Traditionalized = (cc) => translateChar(cc, JTPYStr(), FTPYStr());
    const Simplized = (cc) => translateChar(cc, FTPYStr(), JTPYStr());
