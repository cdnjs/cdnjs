...
var toc = (function () {
  ...
  var $_9nzrzfr8jfjm4n4x = { create: create };

  var tocId = $_9nzrzfr8jfjm4n4x.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_e2pr0xr7jfjm4n4w.getTocClass(editor);
    var headerTag = $_e2pr0xr7jfjm4n4w.getTocHeader(editor);
    var selector = generateSelector($_e2pr0xr7jfjm4n4w.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_e2pr0xr7jfjm4n4w.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_e2pr0xr7jfjm4n4w.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_80ej1cr3jfjm4n4r.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_80ej1cr3jfjm4n4r.updateToc(editor);
    });
  };
  var $_1yu5ngr2jfjm4n4q = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_e2pr0xr7jfjm4n4w.getTocClass(editor);
  ...
  var $_g89q0wr9jfjm4n4y = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_80ej1cr3jfjm4n4r.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_e2pr0xr7jfjm4n4w.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_e3y3jqrajfjm4n50 = { register: register$1 };

  global.add('toc', function (editor) {
    $_1yu5ngr2jfjm4n4q.register(editor);
    $_e3y3jqrajfjm4n50.register(editor);
    $_g89q0wr9jfjm4n4y.setup(editor);
  });
  function Plugin () {
  }
