...
class RPCEstimateProvider extends OperationEmitter {
    ...
    transferTicket(_a) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (rest.source && validateAddress(rest.source) !== ValidationResult.VALID) {
                throw new InvalidAddressError((_b = rest.source) !== null && _b !== void 0 ? _b : '', 'param source');
            }
            const pkh = (yield this.getKeys()).publicKeyHash;
            const protocolConstants = yield this.context.readProvider.getProtocolConstants('head');
            const DEFAULT_PARAMS = yield this.getAccountLimits(pkh, protocolConstants);
            ...
            for (const param of params) {
                switch (param.kind) {
                    ...
                    case OpKind.INCREASE_PAID_STORAGE:
                        ...
                        break;
                    case OpKind.SMART_ROLLUP_ADD_MESSAGES:
                        operations.push(yield createSmartRollupAddMessagesOperation(Object.assign(Object.assign({}, param), mergeLimits(param, DEFAULT_PARAMS))));
                        break;
                    default:
...
class RpcContractProvider extends OperationEmitter {
    ...
    smartRollupAddMessages(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const operation = yield createSmartRollupAddMessagesOperation(Object.assign(Object.assign({}, params), estimate));
            const ops = yield this.addRevealOperationIfNeeded(operation, publicKeyHash);
            const prepared = yield this.prepareOperation({ operation: ops, source: params.source });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new SmartRollupAddMessagesOperation(hash, operation, publicKeyHash, forgedBytes, opResponse, context);
        });
    }
    at(address, contractAbstractionComposer = (x) => x) {
        return __awaiter(this, void 0, void 0, function* () {
...
class PrepareProvider {
     ...
     * @returns a PreparedOperation object
     */
    smartRollupAddMessages(params, source) {
        return __awaiter(this, void 0, void 0, function* () {
            const pkh = yield this.signer.publicKeyHash();
            const estimate = yield this.estimate.smartRollupAddMessages(params);
            const estimates = this.buildEstimates(estimate);
            const op = yield createSmartRollupAddMessagesOperation(Object.assign(Object.assign({}, params), estimates));
            const operation = yield this.addRevealOperationIfNeeded(op, pkh);
            const ops = this.convertIntoArray(operation);
            const hash = yield this.getBlockHash();
            const protocol = yield this.getProtocolHash();
