...
import { switchMap, timeoutWith, concatMap, endWith, tap, shareReplay, map, filter, first, catchError, publishReplay, refCount, distinctUntilChanged, takeWhile, startWith, mergeMap, takeUntil, retry, pluck, distinctUntilKeyChanged, publish } from 'rxjs/operators';
...
import { format, validateOperation, ValidationResult, InvalidOperationHashError, InvalidOperationKindError, DeprecationError, validateAddress, InvalidAddressError, validateContractAddress, InvalidContractAddressError, validateKeyHash, InvalidKeyHashError, validateChain, InvalidChainIdError, encodeExpr } from '@taquito/utils';
...
class Operation {
    ...
    constructor(hash, raw, results, context) {
        ...
        this.currentHead$ = this._pollingConfig$.pipe(switchMap((config) => {
            return new BehaviorSubject(config).pipe(timeoutWith(config.timeout * 1000, throwError(new Error('Confirmation polling timed out'))));
        }), switchMap(() => {
...
const createNewPollingBasedHeadObservable = (sharedHeadOb, context, _scheduler) => {
    return sharedHeadOb.pipe(timeoutAfter(context.config.confirmationPollingTimeoutSecond * 1000), publishReplay(1), refCount());
};
...
export { BallotOperation, BatchOperation, BigMapAbstraction, COST_PER_BYTE, ChainIds, CompositeForger, Context, ContractAbstraction, ContractMethod, ContractMethodObject, ContractView, DEFAULT_FEE, DEFAULT_GAS_LIMIT, DEFAULT_SMART_CONTRACT_METHOD_NAME, DEFAULT_STORAGE_LIMIT, DefaultGlobalConstantsProvider, DelegateOperation, DelegationWalletOperation, DrainDelegateOperation, Estimate, GlobalConstantNotFound, IntegerError, InvalidCodeParameter, InvalidDelegationSource, InvalidInitParameter, InvalidParameterError, InvalidViewParameterError, InvalidViewSimulationContext, LegacyWalletProvider, MANAGER_LAMBDA, MichelCodecPacker, MichelCodecParser, NoopParser, ObservableSubscription, Operation, OperationBatch, OriginationOperation, OriginationParameterError, OriginationWalletOperation, PollingSubscribeProvider, PrepareProvider, Protocols, RPCEstimateProvider, RevealEstimateError, RevealOperationError, RpcForger, RpcPacker, RpcReadAdapter, SaplingStateAbstraction, TaquitoLocalForger, TezosOperationError, TezosPreapplyFailureError, TezosToolkit, TransactionOperation, TransactionWalletOperation, UnconfiguredGlobalConstantsProviderError, VIEW_LAMBDA, ViewSimulationError, Wallet, WalletOperation, WalletOperationBatch, compose, createBallotOperation, createDrainDelegateOperation, createIncreasePaidStorageOperation, createOriginationOperation, createProposalsOperation, createRegisterDelegateOperation, createRegisterGlobalConstantOperation, createRevealOperation, createSetDelegateOperation, createTransferOperation, createTransferTicketOperation, createTxRollupBatchOperation, createTxRollupOriginationOperation, createUpdateConsensusKeyOperation, defaultConfigConfirmation, protocols, validateAndExtractFailwith };
