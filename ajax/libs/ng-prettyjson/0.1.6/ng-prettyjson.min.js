/** 
* @license ng-prettyjson - v0.1.5
* (c) 2013 Julien VALERY https://github.com/darul75/ng-prettyjson
* License: MIT 
**/
angular.module("ngPrettyJson",[]).directive("prettyJson",["ngPrettyJsonFunctions",function(a){"use strict";var b=angular.isDefined;return{restrict:"AE",scope:{json:"=",prettyJson:"=",onEdit:"&"},replace:!0,template:'<div><button ng-click="edit()" ng-show="edition && !editActivated">Edit</button><button ng-click="edit()" ng-show="edition && editActivated">Cancel</button><button ng-click="update()" ng-show="editActivated && parsable">Update</button><pre id="prettyjson"></pre></div>',link:function(c,d,e){var f={},g="prettyjson",h=null;c.editActivated=!1,c.edition=e.edition,c.aceEditor=void 0!==window.ace;// prefer the "json" attribute over the "prettyJson" one.
// the value on the scope might not be defined yet, so look at the markup.
var i,j=b(e.json)?"json":"prettyJson",k=function(c){var e=a.syntaxHighlight(c)||"";return e=e.replace(/\{/g,"<span class='sep'>{</span>").replace(/\}/g,"<span class='sep'>}</span>").replace(/\[/g,"<span class='sep'>[</span>").replace(/\]/g,"<span class='sep'>]</span>").replace(/\,/g,"<span class='sep'>,</span>"),b(c)?d.find("pre").html(e):d.find("pre").empty()};i=c.$watch(j,function(a){// BACKWARDS COMPATIBILITY:
// if newValue is an object, and we find a `json` property,
// then stop watching on `exp`.
angular.isObject(a)&&b(a.json)?(i(),c.$watch(j+".json",function(a){k(a),f=a},!0)):(k(a),f=a)},!0);var l=function(){try{f=JSON.parse(h.getValue()),c.parsable=!0}catch(a){c.parsable=!1}// trigger update
c.$apply(function(){})};c.edit=function(){return c.aceEditor?(c.editActivated?(h&&(document.getElementById(g).env=null),k(f)):(h=ace.edit(g),h.setOptions({maxLines:1/0}),h.on("change",l),h.getSession().setMode("ace/mode/json")),void(c.editActivated=!c.editActivated)):void(console&&console.log("'ace lib is missing'"))},c.update=function(){c.$emit("json-updated",f),c.onEdit&&c.onEdit({newJson:f}),this.edit()}}}}]).factory("ngPrettyJsonFunctions",function(){// cache some regular expressions
