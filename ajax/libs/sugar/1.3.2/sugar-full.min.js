...
function Mb(a,b,c){D(b)&&Lb(Y,b);Lb(Y,c);a.unshift({ka:b,pa:c})}function Nb(a,b){return a==b||a=="all"||!a}function Ob(a){return Y.some(function(b){return(new q("\\b"+b+"$","i")).test(a)})}function Pb(a,b){a=D(a)?a.toString():"";return a.isBlank()||Ob(a)?a:Qb(a,b?Gb:Hb)}function Qb(a,b){H(b,function(c,d){if(a.match(d.ka)){a=a.replace(d.ka,d.pa);return m}});return a}function Rb(a){return a.replace(/^\W*[a-z]/,function(b){return b.toUpperCase()})}
...
f),"i"),"$1"+d)}else{Z.plural(new q("{1}{2}$".assign(i,d)),g+f);Z.plural(new q("{1}{2}$".assign(h,d)),j+f);Z.plural(new q("{1}{2}$".assign(g,f)),g+f);Z.plural(new q("{1}{2}$".assign(j,f)),j+f);Z.singular(new q("{1}{2}$".assign(g,f)),i+d);Z.singular(new q("{1}{2}$".assign(j,f)),h+d)}},uncountable:function(a){var b=p.isArray(a)?a:G(arguments);Y=Y.concat(b)},human:function(a,b){Ib.unshift({ka:a,pa:b})},clear:function(a){if(Nb(a,"singulars"))Hb=[];if(Nb(a,"plurals"))Gb=[];if(Nb(a,"uncountables"))Y=[];
...
function fc(a,b,c,d){Xb||gc();var e=G(b).join(""),f=Xb[d];e=e.replace(/all/,"").replace(/(\w)lphabet|umbers?|atakana|paces?|unctuation/g,"$1");return a.replace(c,function(g){return f[g]&&(!e||e.has(f[g].type))?f[g].to:g})}
function gc(){var a;Xb={zenkaku:{},hankaku:{}};Wb.forEach(function(b){N(b.start,b.end,function(c){$(b.type,t.fromCharCode(c),t.fromCharCode(c+b.shift))})});ec.each(function(b,c){a=dc.charAt(c);$("k",a,b);b.match(bc)&&$("k",a+"\uff9e",b.shift(1));b.match(cc)&&$("k",a+"\uff9f",b.shift(2))});ac.each(function(b,c){$("p",$b.charAt(c),b)});$("k","\uff73\uff9e","\u30f4");$("k","\uff66\uff9e","\u30fa");$("s"," ","\u3000")}function $(a,b,c){Xb.zenkaku[b]={type:a,to:c};Xb.hankaku[c]={type:a,to:b}}
function hc(){Sb={};H(Vb,function(a,b){b.split("").forEach(function(c){Sb[c]=a});Tb+=b});Tb=q("["+Tb+"]","g")}
...
F(t,k,m,{normalize:function(){Sb||hc();return this.replace(Tb,function(a){return Sb[a]})},hankaku:function(){return fc(this,arguments,Zb,"hankaku")},zenkaku:function(){return fc(this,arguments,Yb,"zenkaku")},hiragana:function(a){var b=this;if(a!==m)b=b.zenkaku("k");return b.replace(/[\u30A1-\u30F6]/g,function(c){return c.shift(-96)})},katakana:function(){return this.replace(/[\u3041-\u3096]/g,function(a){return a.shift(96)})}});
...
{ca:["Thai"],source:"\u0e00-\u0e7f"}].forEach(function(a){var b=q("^["+a.source+"\\s]+$"),c=q("["+a.source+"]");a.ca.forEach(function(d){ja(t.prototype,"is"+d,function(){return b.test(this.trim())});ja(t.prototype,"has"+d,function(){return c.test(this)})})});
