    ...
    */
    ...
    Frame.prototype.toCSSObject = function (useCamelCase) {
        ...
        for (var name_4 in properties) {
            ...
            if (name_4 === TIMING_FUNCTION) {
                name_4 = TIMING_FUNCTION.replace("animation", utils.ANIMATION);
                value = (utils.isString(value) ? value : value[EASING_NAME]) || "initial";
            }
            if (useCamelCase) {
                name_4 = utils.camelize(name_4.replace(/^[-]+/g, ""));
            }
    ...
    Frame.prototype._set = function (args, value) {
        ...
        if (length === 1 && lastParam === TIMING_FUNCTION) {
            ...
            properties[lastParam] = utils.isString(value) && !isFixed(args)
    ...
    Scene.prototype.load = function (properties, options) {
        ...
        for (var name_1 in properties) {
            ...
            if (isScene(object) || isSceneItem(object)) {
                ...
                var elements = utils.IS_WINDOW
                    ? utils.$("".concat(utils.isFunction(selector) ? selector(name_1) : name_1), true)
                    : [];
