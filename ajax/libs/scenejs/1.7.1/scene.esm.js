...
*/
var setters = ["id", ITERATION_COUNT, DELAY, FILL_MODE,
    DIRECTION, PLAY_SPEED, DURATION, PLAY_SPEED, ITERATION_TIME, PLAY_STATE];
var getters = __spreadArray(__spreadArray([], setters, true), [EASING, EASING_NAME], false);
/**
    ...
    */
    Frame.prototype.set = function () {
        ...
        if (length === 1 && isFrame(value)) {
            ...
            if (isRole(params)) {
                self.set.apply(self, __spreadArray(__spreadArray([], params, false), [toObject(value)], false));
            }
            else {
            ...
            for (var name_2 in value) {
                self.set.apply(self, __spreadArray(__spreadArray([], params, false), [name_2, value[name_2]], false));
            }
        }
            ...
            if (isRole(params, true)) {
                if (isFixed(params) || !isRole(params)) {
                    ...
                    if (isObject(obj)) {
                        self.set.apply(self, __spreadArray(__spreadArray([], params, false), [obj], false));
                    }
                }
                return this;
            }
                ...
                for (var name_3 in styles) {
                    self.set.apply(self, __spreadArray(__spreadArray([], params, false), [name_3, styles[name_3]], false));
                }
                if (stylesLength) {
                    return this;
                }
    ...
    */
    ...
    Frame.prototype.toCSSObject = function (useCamelCase) {
        ...
        for (var name_4 in properties) {
            ...
            if (name_4 === TIMING_FUNCTION) {
                name_4 = TIMING_FUNCTION.replace("animation", ANIMATION);
                value = (isString(value) ? value : value[EASING_NAME]) || "initial";
            }
            if (useCamelCase) {
                name_4 = camelize(name_4.replace(/^[-]+/g, ""));
            }
    ...
    Scene.prototype.load = function (properties, options) {
        ...
        for (var name_1 in properties) {
            ...
            if (isScene(object) || isSceneItem(object)) {
                this.setItem(name_1, object);
                item = object;
            }
            else if (isFunction(object) && selector) {
                var elements = IS_WINDOW
                    ? $("".concat(isFunction(selector) ? selector(name_1) : name_1), true)
                    : [];
                ...
                this.setItem(name_1, scene);
                continue;
            }
            else {
                item = this.newItem(name_1);
                item.load(object);
            }
...
export { Animator, EASE, EASE_IN, EASE_IN_OUT, EASE_OUT, EVENTS, FIXED, Frame, LINEAR, NAME_SEPARATOR, OPTIONS, ROLES, STEP_END, STEP_START, SceneItem, animate, animateItem, bezier, Scene as default, isFrame, isRole, isScene, isSceneItem, setAlias, setRole, steps };
