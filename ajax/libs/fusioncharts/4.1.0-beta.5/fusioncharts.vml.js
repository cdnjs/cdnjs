...
function _default(R) {
  var UNDEF,
    ...
    _parseNode = function parseNode(node, defs) {
      var xmlSTR = BLANK,
        conf = {
          attrSTR: BLANK,
          styleSTR: BLANK,
      ...
      if (node.node.style.display !== NONE && !isShadow) {
        ...
        if (node.bottom) {
          childXMLSTR = _parseNode(node.bottom, defs);
        }
        // parse following sibling also
        if (node.next) {
          nextXMLSTR = _parseNode(node.next, defs);
        }
        nodeType = node.type;
        ...
        // parse following sibling also
        if (node.next) {
          xmlSTR += _parseNode(node.next, defs);
        }
      }
      return xmlSTR;
    };
  ...
  if (R.vml) {
    R.fn.toSVG = function (keepImages) {
      ...
      if (paper.bottom) {
        ...
        childXMLSTR = _parseNode(paper.bottom, defs);
      }
...
var _raphael = _interopRequireDefault(__webpack_require__(1660));
var _redraphaelExport = _interopRequireDefault(__webpack_require__(1661));
...
exports.__esModule = true;
exports["default"] = _default;
var _raphael = __webpack_require__(281);
var _trustedPolicy = _interopRequireDefault(__webpack_require__(302));
/**!
...
//# sourceMappingURL=http://localhost:3052/4.1.0-beta.5/map/eval/fusioncharts.vml.js.map