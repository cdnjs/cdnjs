(function (global, factory) {
	...
	var camelizeStyleName = lib$1.cached(function (name) {
	  return name.replace(/-([a-z])/gi, function (s, g) {
	    return g.toUpperCase();
	  });
	});
	var isDimensionalProp = lib$1.cached(function (prop) {
	  return !NON_DIMENSIONAL_REG.test(prop);
	});
	var isEventProp = lib$1.cached(function (prop) {
	  ...
	  for (var prop in style) {
	    ...
	    if (typeof value === 'number' && isDimensionalProp(prop)) {
	      if (__shouldConvertUnitlessToRpx) {
	        ...
	        convertedValue = lib$1.convertUnit(convertedValue);
	      } else {
	        convertedValue = value + 'px';
	      }
	    } else {
	      convertedValue = lib$1.convertUnit(value);
	...
	exports.setUnitPrecision = lib$1.setUnitPrecision;
	exports.setViewportWidth = lib$1.setViewportWidth;
