...
var _logicalStylePolyfill = _interopRequireDefault(require("./logicalStylePolyfill"));

exports.logicalStylePolyfill = _logicalStylePolyfill.default;
...
function atomic(style) {
  ...
  var compiledRules = [];
  Object.keys(style).sort().forEach(function (key) {
    var value = style[key];

    if (value != null) {
      var valueString = stringifyValueWithProperty(value, key);
      var cacheKey = key + valueString;
      ...
      if (cachedResult != null) {
        ...
        var order = _constants.STYLE_GROUPS.custom[key] || _constants.STYLE_GROUPS.atomic;
        var identifier = createIdentifier('r', key, value);
        var rules = createAtomicRules(identifier, key, value);
...
function classic(style, name) {
  ...
  return [compiledStyle, [[compiledRules, _constants.STYLE_GROUPS.classic]]];
}
/**
...
function inline(style) {
  return (0, _createReactDOMStyle.default)(style, true);
}
