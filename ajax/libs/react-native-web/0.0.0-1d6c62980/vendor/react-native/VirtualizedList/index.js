...
class VirtualizedList extends React.PureComponent {
  ...
  constructor(_props) {
    ...
    this._registerAsNestedChild = childList => {
      ...
      const childListsInCell = this._cellKeysToChildListKeys.get(childList.cellKey) || new Set();
    ...
    this._convertParentScrollMetrics = metrics => {
      ...
      const offset = metrics.offset - this._offsetFromParentVirtualizedList; // Child's visible length is the same as its parent's
    ...
    this._updateCellsToRender = () => {
      ...
      const onEndReachedThreshold = onEndReachedThresholdOrDefault(_onEndReachedThreshold);
      ...
      this.setState(state => {
        let newState;
        const _this$_scrollMetrics = this._scrollMetrics,
        ...
        if (!isVirtualizationDisabled) {
          ...
          const renderAhead = distanceFromEnd < onEndReachedThreshold * visibleLength ? maxToRenderPerBatchOrDefault(this.props.maxToRenderPerBatch) : 0;
          newState = {
            first: 0,
        ...
        if (newState && this._nestedChildLists.size > 0) {
          ...
          const newLast = newState.last; // If some cell in the new state has a child list in it, we should only render
          ...
          for (let ii = newFirst; ii <= newLast; ii++) {
            ...
            const childListKeys = cellKeyForIndex && this._cellKeysToChildListKeys.get(cellKeyForIndex);

            if (!childListKeys) {
              continue;
            }

            let someChildHasMore = false; // For each cell, need to check whether any child list in it has more elements to render

            for (var _iterator = _createForOfIteratorHelperLoose(childListKeys), _step; !(_step = _iterator()).done;) {
              let childKey = _step.value;
    ...
    this._getFrameMetricsApprox = index => {
      ...
      if (frame && frame.index === index) {
        ...
        const getItemLayout = this.props.getItemLayout;
  ...
  static getDerivedStateFromProps(newProps, prevState) {
    ...
    const maxToRenderPerBatch = maxToRenderPerBatchOrDefault(newProps.maxToRenderPerBatch); // first and last could be stale (e.g. if a new, shorter items props is passed in), so we make
  ...
  render() {
    ...
    if (ListHeaderComponent) {
      ...
      const element = /*#__PURE__*/React.isValidElement(ListHeaderComponent) ? ListHeaderComponent :
      /*#__PURE__*/
    ...
    if (itemCount > 0) {
      ...
      const lastInitialIndex = this.props.initialScrollIndex ? -1 : initialNumToRenderOrDefault(this.props.initialNumToRender) - 1;
      ...
      if (!isVirtualizationDisabled && first > lastInitialIndex + 1) {
        ...
        if (stickyIndicesFromProps.size > 0) {
          const stickyOffset = ListHeaderComponent ? 1 : 0; // See if there are any sticky headers in the virtualized space that we need to render.
          ...
          for (let ii = firstAfterInitial - 1; ii > lastInitialIndex; ii--) {
            if (stickyIndicesFromProps.has(ii + stickyOffset)) {
              ...
              const leadSpace = stickyBlock.offset - initBlock.offset - (this.props.initialScrollIndex ? 0 : initBlock.length);
      ...
      if (!isVirtualizationDisabled && last < itemCount - 1) {
        const lastFrame = this._getFrameMetricsApprox(last); // Without getItemLayout, we limit our tail spacer to the _highestMeasuredFrameIndex to
      ...
      const element = /*#__PURE__*/React.isValidElement(ListEmptyComponent) ? ListEmptyComponent :
      /*#__PURE__*/
      // $FlowFixMe[not-a-component]
      // $FlowFixMe[incompatible-type-arg]
      React.createElement(ListEmptyComponent, null);
      cells.push( /*#__PURE__*/React.cloneElement(element, {
        key: '$empty',
        onLayout: event => {
    ...
    if (ListFooterComponent) {
      const element = /*#__PURE__*/React.isValidElement(ListFooterComponent) ? ListFooterComponent :
      /*#__PURE__*/
      // $FlowFixMe[not-a-component]
      // $FlowFixMe[incompatible-type-arg]
    ...
    const innerRet = /*#__PURE__*/React.createElement(VirtualizedListContextProvider, {
      value: {
        cellKey: null,
  ...
  _maybeCallOnEndReached() {
    ...
    const _this$_scrollMetrics2 = this._scrollMetrics,
          contentLength = _this$_scrollMetrics2.contentLength,
          visibleLength = _this$_scrollMetrics2.visibleLength,
          offset = _this$_scrollMetrics2.offset;
    const distanceFromEnd = contentLength - visibleLength - offset;
    const threshold = onEndReachedThreshold != null ? onEndReachedThreshold * visibleLength : 2;
  ...
  _scheduleCellsToRenderUpdate() {
    ...
    const _this$_scrollMetrics3 = this._scrollMetrics,
    ...
    const scrollingThreshold = onEndReachedThreshold * visibleLength / 2; // Mark as high priority if we're close to the start of the first item
  ...
  _updateViewableItems(data) {
    const getItemCount = this.props.getItemCount;
...
class CellRenderer extends React.Component {
  ...
  render() {
    ...
    const renderItem = parentProps.renderItem,
    ...
    const itemSeparator = ItemSeparatorComponent && /*#__PURE__*/React.createElement(ItemSeparatorComponent, this.state.separatorProps);
...
function describeNestedLists(childList) {
  let trace = 'VirtualizedList trace:\n' + ("  Child (" + (childList.horizontal ? 'horizontal' : 'vertical') + "):\n") + ("    listKey: " + childList.key + "\n") + ("    cellKey: " + childList.cellKey);
