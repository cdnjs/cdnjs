...
const modulo = function modulo(a, modulus) {
  return new _AnimatedModulo.default(a, modulus);
};

const diffClamp = function diffClamp(a, min, max) {
  return new _AnimatedDiffClamp.default(a, min, max);
};

const _combineCallbacks = function _combineCallbacks(callback, config) {
...
const maybeVectorAnim = function maybeVectorAnim(value, config, anim) {
  if (value instanceof _AnimatedValueXY.default) {
    const configX = (0, _objectSpread2.default)({}, config);
    const configY = (0, _objectSpread2.default)({}, config);
    ...
    const aY = anim(value.y, configY); // We use `stopTogether: false` here because otherwise tracking will break
...
const spring = function spring(value, config) {
  const _start = function start(animatedValue, configuration, callback) {
  ...
  return maybeVectorAnim(value, config, spring) || {
    ...
    _startNativeLoop: function _startNativeLoop(iterations) {
      const singleConfig = (0, _objectSpread2.default)((0, _objectSpread2.default)({}, config), {}, {
        iterations
      });
...
const timing = function timing(value, config) {
  const _start2 = function start(animatedValue, configuration, callback) {
    callback = _combineCallbacks(callback, configuration);
    const singleValue = animatedValue;
    const singleConfig = configuration;
    singleValue.stopTracking();
...
const decay = function decay(value, config) {
  const _start3 = function start(animatedValue, configuration, callback) {
    callback = _combineCallbacks(callback, configuration);
    const singleValue = animatedValue;
    const singleConfig = configuration;
    singleValue.stopTracking();
...
const parallel = function parallel(animations, config) {
  let doneCount = 0; // Make sure we only call stop() at most once for each animation
...
const loop = function loop(animation, _temp) {
  ...
  return {
    start: function start(callback) {
      const restart = function restart(result) {
        if (result === void 0) {
          result = {
            finished: true
          };
...
const event = function event(argMapping, config) {
  const animatedEvent = new _AnimatedEvent.AnimatedEvent(argMapping, config);
