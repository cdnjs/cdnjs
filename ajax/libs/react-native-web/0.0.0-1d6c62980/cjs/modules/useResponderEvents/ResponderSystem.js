...
function eventListener(domEvent) {
  ...
  const isStartEvent = (0, _ResponderEventTypes.isStartish)(eventType) && (0, _utils.isPrimaryPointerDown)(domEvent);
  const isMoveEvent = (0, _ResponderEventTypes.isMoveish)(eventType);
  const isEndEvent = (0, _ResponderEventTypes.isEndish)(eventType);
  const isScrollEvent = (0, _ResponderEventTypes.isScroll)(eventType);
  const isSelectionChangeEvent = (0, _ResponderEventTypes.isSelectionChange)(eventType);
  const responderEvent = (0, _createResponderEvent.default)(domEvent);
  /**
  ...
  let eventPaths = (0, _utils.getResponderPaths)(domEvent);
  let wasNegotiated = false;
  let wantsResponder; // If an event occured that might change the current responder...
  ...
  if (isStartEvent || isMoveEvent || isScrollEvent && trackedTouchCount > 0) {
    ...
    if (currentResponderIdPath != null && eventIdPath != null) {
      const lowestCommonAncestor = (0, _utils.getLowestCommonAncestor)(currentResponderIdPath, eventIdPath);

      if (lowestCommonAncestor != null) {
        const indexOfLowestCommonAncestor = eventIdPath.indexOf(lowestCommonAncestor); // Skip the current responder so it doesn't receive unexpected "shouldSet" events.
  ...
  if (currentResponder.id != null && currentResponder.node != null) {
    const _currentResponder = currentResponder,
          id = _currentResponder.id,
          node = _currentResponder.node;

    const _getResponderConfig = getResponderConfig(id),
    ...
    if (isStartEvent) {
      ...
      const isTerminateEvent = (0, _ResponderEventTypes.isCancelish)(eventType) || // native context menu
      ...
      const isReleaseEvent = isEndEvent && !isTerminateEvent && !(0, _utils.hasTargetTouches)(node, domEvent.touches); // End
      ...
      if (isTerminateEvent) {
        let shouldTerminate = true; // Responders can still avoid termination but only for these events.
...
function findWantsResponder(eventPaths, domEvent, responderEvent) {
  ...
  if (shouldSetCallbacks != null) {
    const idPath = eventPaths.idPath,
          nodePath = eventPaths.nodePath;
    const shouldSetCallbackCaptureName = shouldSetCallbacks[0];
    const shouldSetCallbackBubbleName = shouldSetCallbacks[1];
    const bubbles = shouldSetCallbacks[2].bubbles;

    const check = function check(id, node, callbackName) {
...
function attemptTransfer(responderEvent, wantsResponder) {
  const _currentResponder2 = currentResponder,
        currentId = _currentResponder2.id,
        currentNode = _currentResponder2.node;
  const id = wantsResponder.id,
        node = wantsResponder.node;

  const _getResponderConfig2 = getResponderConfig(id),
  ...
  if (currentId == null) {
    ...
    const _getResponderConfig3 = getResponderConfig(currentId),
...
function terminateResponder() {
  const _currentResponder3 = currentResponder,
        id = _currentResponder3.id,
        node = _currentResponder3.node;

  if (id != null && node != null) {
    const _getResponderConfig4 = getResponderConfig(id),
          onResponderTerminate = _getResponderConfig4.onResponderTerminate;

    if (onResponderTerminate != null) {
      const event = (0, _createResponderEvent.default)({});
