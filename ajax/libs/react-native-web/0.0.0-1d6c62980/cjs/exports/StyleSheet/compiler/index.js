...
function classic(style, name) {
  ...
  const compiledRules = [];
  const animationKeyframes = style.animationKeyframes,
  ...
  if (animationKeyframes != null) {
    const _processKeyframesValu = processKeyframesValue(animationKeyframes),
  ...
  const block = createDeclarationBlock((0, _objectSpread2.default)((0, _objectSpread2.default)({}, rest), {}, {
    animationName
  }));
...
function createAtomicRules(identifier, property, value) {
  const rules = [];
  const selector = "." + identifier; // Handle non-standard properties and object values that require multiple
  // CSS rules to be created.

  switch (property) {
    case 'animationKeyframes':
      {
        const _processKeyframesValu2 = processKeyframesValue(value),
...
function processKeyframesValue(keyframesValue) {
  ...
  value.forEach(keyframes => {
    if (typeof keyframes === 'string') {
      ...
      const _createKeyframes = createKeyframes(keyframes),
