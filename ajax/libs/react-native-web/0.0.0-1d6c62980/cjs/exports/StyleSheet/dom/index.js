...
const sheets = [];
const initialRules = [// minimal top-level reset
...
function createSheet(root, id) {
  ...
  if (_ExecutionEnvironment.default.canUseDOM) {
    const rootNode = root != null ? root.getRootNode() : document; // Create the initial style sheet
    ...
    if (sheets.length === 0) {
      ...
      if (index == null) {
        const initialSheet = sheets[0]; // If we're creating a new sheet, populate it with existing styles

        const textContent = initialSheet != null ? initialSheet.getTextContent() : ''; // Cast rootNode to 'any' because Flow types for getRootNode are wrong
