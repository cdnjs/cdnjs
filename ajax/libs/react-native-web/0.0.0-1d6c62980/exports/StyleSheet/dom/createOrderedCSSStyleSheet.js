...
export default function createOrderedCSSStyleSheet(sheet) {
  ...
  if (sheet != null) {
    ...
    slice.call(sheet.cssRules).forEach((cssRule, i) => {
      const cssText = cssRule.cssText; // Create record of existing selectors and rules
  ...
  function sheetInsert(sheet, group, text) {
    ...
    const nextGroup = orderedGroups[nextGroupIndex]; // Insert rule before the next group, or at the end of the stylesheet
  ...
  const OrderedCSSStyleSheet = {
    ...
    getTextContent() {
      return getOrderedGroups(groups).map(group => {
        const rules = groups[group].rules; // Sorting provides deterministic order of styles in group for
    ...
    insert(cssText, groupValue) {
      const group = Number(groupValue); // Create a new group.

      if (groups[group] == null) {
        const markerRule = encodeGroupRule(group); // Create the internal record.
