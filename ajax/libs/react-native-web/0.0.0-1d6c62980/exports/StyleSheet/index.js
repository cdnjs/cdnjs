...
function compileAndInsertAtomic(style) {
  const _atomic = atomic(preprocess(style)),
...
function compileAndInsertReset(style, key) {
  const _classic = classic(style, key),
...
function create(styles) {
  Object.keys(styles).forEach(key => {
    const styleObj = styles[key]; // Only compile at runtime if the style is not already compiled
