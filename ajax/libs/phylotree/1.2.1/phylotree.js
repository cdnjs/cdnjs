  ...
  */
  ...
  function neighborJoining(distanceMatrixArr, n, nodeList) {
  	...
  	if(n<=2) {
  		...
  		let newRow = ___namespace$1.concat(0,___namespace$1.filter(___namespace$1.map(___namespace$1.range(n), k => {
  			if(k != i && k !=j) {
  		...
  		let m = "InternalNode0";
  		let internalNodes = ___namespace$1.filter(nodeList, x => ___namespace$1.includes(x, 'InternalNode'));

  		if(internalNodes.length) {
  			let highestNum = ___namespace$1.max(___namespace$1.map(internalNodes, label => ___namespace$1.split(label, 'InternalNode')[1]));
