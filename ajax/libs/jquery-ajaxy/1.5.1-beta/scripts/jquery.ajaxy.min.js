...
d.compatMode=="BackCompat"?d.body:d.documentElement})};c.fn.scrollTo=function(d,e,f){if(typeof e=="object"){f=e;e=0}if(typeof f=="function")f={onAfter:f};if(d=="max")d=9E9;f=c.extend({},b.defaults,f);e=e||f.speed||f.duration;f.queue=f.queue&&f.axis.length>1;if(f.queue)e/=2;f.offset=a(f.offset);f.over=a(f.over);return this._scrollable().each(function(){function g(l){j.animate(k,e,f.easing,l&&function(){l.call(this,d,f)})}var h=this,j=c(h),i=d,n,k={},m=j.is("html,body");switch(typeof i){case "number":case "string":if(/^([+-]=)?\d+(\.\d+)?(px|%)?$/.test(i)){i=
a(i);break}i=c(i,this);case "object":if(i.is||i.style)n=(i=c(i)).offset()}c.each(f.axis.split(""),function(l,r){var o=r=="x"?"Left":"Top",p=o.toLowerCase(),q="scroll"+o,s=h[q],t=b.max(h,r);if(n){k[q]=n[p]+(m?0:s-j.offset()[p]);if(f.margin){k[q]-=parseInt(i.css("margin"+o))||0;k[q]-=parseInt(i.css("border"+o+"Width"))||0}k[q]+=f.offset[p]||0;if(f.over[p])k[q]+=i[r=="x"?"width":"height"]()*f.over[p]}else{o=i[p];k[q]=o.slice&&o.slice(-1)=="%"?parseFloat(o)/100*t:o}if(/^\d+$/.test(k[q]))k[q]=k[q]<=0?
0:Math.min(k[q],t);if(!l&&f.queue){s!=k[q]&&g(f.onAfterFirst);delete k[q]}});g(f.onAfter)}).end()};b.max=function(d,e){var f=e=="x"?"Width":"Height",g="scroll"+f;if(!c(d).is("html,body"))return d[g]-c(d)[f.toLowerCase()]();f="client"+f;var h=d.ownerDocument.documentElement,j=d.ownerDocument.body;return Math.max(h[g],j[g])-Math.min(h[f],j[f])};c.fn.ScrollTo=function(d){for(var e=c(this),f=e.parent(),g=false;f.length===1&&!f.is("body");){var h=f.get(0);if(f.css("overflow")!=="visible"&&h.scrollHeight!==
...
c.extend(b.options,a);return true},domReadied:false,domReady:function(){var a=c.History;if(!a.domRedied){a.domRedied=true;a.$window=c(window);a.$window.bind("hashchange",this.hashchange);setTimeout(a.hashchangeLoader,200);return true}},hashchangeLoader:function(){var a=c.History;if(c.browser.msie&&parseInt(c.browser.version)>=8)a.getHash()&&a.$window.trigger("hashchange");else{var b;if(c.browser.msie){a.$iframe=c('<iframe id="jquery-history-iframe" style="display: none;"></$iframe>').prependTo(document.body)[0];
a.$iframe.contentWindow.document.open();a.$iframe.contentWindow.document.close();var d=false;b=function(){var e=a.getHash(),f=a.getState(),g=a.extractHash(a.$iframe.contentWindow.document.location.hash);if(f!==e){if(!d){a.$iframe.contentWindow.document.open();a.$iframe.contentWindow.document.close();a.$iframe.contentWindow.document.location.hash=e}d=false;a.$window.trigger("hashchange")}else if(f!==g){d=true;a.setHash(g)}}}else b=function(){var e=a.getHash();a.getState()!==e&&a.$window.trigger("hashchange")};
setInterval(b,200)}return true}};c.History.construct()}})(jQuery);
...
0)&&b.currentState.hash===f.hash&&g===h){if(b.currentState.anchor!==f.anchor){b.options.debug&&window.console.debug("Ajaxy.request: There has only been an anchor change",[this,arguments],[b.currentState,f,a]);b.stateCompleted(f)}b.currentState=f;b.options.debug&&window.console.debug("Ajaxy.request: There has been no considerable change",[this,arguments],[b.currentState,f,a]);return true}delete g;delete h;b.ajaxQueue.push(a);if(b.ajaxQueue.length!==1)return false;b.options.analytics&&b.track(a);b.currentState=
f;g=f.controller||b.match(a)||undefined;f.controller=g;f.Request.url=f.Request.url||b.options.root_url+b.options.base_url+(a||"?");b.storeState(f);b.trigger(g,"request");g={data:f.Request.data,url:f.Request.url,type:"post",success:function(m){b.options.debug&&window.console.debug("Ajaxy.request.success:",[this,arguments]);m=c.extend(true,{},b.defaults.State.Response.data,m);m.Ajaxy=m.Ajaxy||{};if(m.Ajaxy.redirected){var l=b.extractState(m.Ajaxy.redirected.to);b.redirected={status:true,from:a,to:l};
d.go(l)}b.ajaxQueue.shift();if((l=b.ajaxQueue.pop())&&l!==a){b.ajaxQueue=[];b.stateChange(l);return false}f.Response.data=m;f.Error.data={};l=m.controller||f.controller||null;if(l===null){l="_generic";window.console.warn("Ajaxy.request.success.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[m.controller,f.controller])}if(f.Response.callback)if(f.Response.callback.apply(f,arguments)||l==="callback")return true;return b.trigger(l,"response",f)},error:function(m,
l,r,o){b.options.debug&&window.console.debug("Ajaxy.request.error:",[this,arguments]);o||(o={responseText:m.responseText.trim()||false});b.ajaxQueue.shift();var p=b.ajaxQueue.pop();if(p&&p!==a){b.ajaxQueue=[];b.stateChange(p);return false}f.Request.XMLHttpRequest=m;f.Response.data=o;f.Error.data={};p=o.controller||f.controller||null;if(p===null){p="_generic";window.console.warn("Ajaxy.request.error.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[o.controller,
f.controller])}if(f.Error.callback)if(f.Error.callback.apply(f,arguments)||p==="callback")return true;return b.trigger(p,"error",f)},complete:function(m){b.options.debug&&window.console.debug("Ajaxy.request.complete:",[this,arguments]);f.Request.XMLHttpRequest=m}};if(f.form){var j=c(f.form);if(j.attr("enctype")==="multipart/form-data"){e="ajaxy_form_iframe_"+Math.floor(Math.random()*99999);var i=c('<iframe style="display:none" src="about:blank" id="'+e+'" name="'+e+'" >').appendTo("body").hide(),
n=c('<input type="hidden" name="ajax" value="true"/>').appendTo(j),k=c('<input type="hidden" name="Ajaxy[form]" value="true"/>').appendTo(j);i.bind("load",function(){if((this.document||this.currentDocument||this.contentWindow.document).location.href==="about:blank")return true;var m=i.contents().find(".response").val(),l=false;try{l=c.parseJSON(m)}catch(r){window.console.error("Ajaxy.request.form: Invalid Response.",[this,arguments],[m])}l?request.success(l):request.error(l);j.removeAttr("target");
i.remove();n.remove();k.remove()});j.attr("target",e);j.submit();h=j.values();g.data=c.extend(true,g.data,h||{});e=true}else{h=j.values();g.data=c.extend(true,g.data,h||{})}}h=true;f.Request=g;e||(h=b.ajax(g));return h}},ajax:function(a){var b=c.Ajaxy;b.options.debug&&window.console.debug("Ajaxy.ajax:",[this,arguments]);var d={};d.success=a.success||function(){b.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.success:",[this,arguments]);c(".error").empty()};d.error=a.error||function(f,g,
h){b.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.error:",[this,arguments]);c(".error").html(h)};d.complete=a.complete||function(){b.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.complete:",[this,arguments])};delete a.success;delete a.error;delete a.complete;var e={type:"post",dataType:b.options.support_text?"text":"json"};c.extend(true,e,a);e.success=function(f,g){b.options.debug&&window.console.debug("Ajaxy.ajax.request.success:",[this,arguments]);var h={};if(typeof f!==
"object"&&b.options.support_text&&f)try{h=c.parseJSON(f)}catch(j){var i=c(f.replace(/<(html|head|body|title)>/g,'<div id="ajaxy-$1">').replace(/<\/(html|head|body|title)/g,"</div>"));h=i.find("#ajaxy-body");var n=i.find("#ajaxy-title");i=i.find("#ajaxy-controller");n=n.length?n.text():null;h=h.length?h.html():f;i=i.length?i.text():null;h={title:n,content:h,controller:i,html:f}}else h=f;b.options.debug&&window.console.debug("Ajaxy.ajax.success:",[this,arguments]);if(typeof h.controller==="undefined"&&
...
stateChange:function(a){c.Ajaxy.request(a)},configure:function(a){var b=c.Ajaxy,d=c.History;a=a||{};var e=a.Controllers||a.controllers||a;b.options=c.extend(true,b.options,a.options||a);b.bind(e);b.options.root_url=(b.options.root_url||document.location.protocol.toString()+"//"+document.location.hostname.toString()).replace(/\/+$/,"")+"/";b.options.base_url=b.options.base_url||"";b.options.relative_url=b.extractState(b.options.relative_url||document.location.pathname.toString());if(b.options.relative_as_base)if(b.options.base_url.length===
0){b.options.base_url=b.options.relative_url;b.options.relative_url=""}b.options.root_url=b.options.root_url.replace(/\/+$/,"");b.options.base_url=b.options.base_url.replace(/\/+$/,"");b.options.relative_url=b.extractRelativeUrl(b.options.relative_url);if(b.options.root_url==="/")b.options.root_url="";if(b.options.base_url==="/")b.options.base_url="";if(b.options.relative_url==="/")b.options.relative_url="";b.options.debug&&window.console.debug("Ajaxy.configure:",[this,arguments]);if(b.options.redirect&&
b.options.relative_url&&b.options.relative_url!==null){b=b.options.root_url+b.options.base_url+"#"+b.options.relative_url;if(d=d.getHash())b+="?anchor="+d;document.location=b}return true},construct:function(){var a=c.Ajaxy,b=c.History;if(!a.constructed){a.constructed=true;b.bind(function(d){return a.stateChange(d)});c.fn.ajaxify=a.ajaxify;c.fn.ajaxy=a.ajaxify;c(function(){a.domReady();b.domReady()});return true}},domReady:function(){c.Sparkle&&c.Sparkle.addExtension("ajaxy",function(){c(this).ajaxify()});
return true},ajaxify:function(){var a=c.Ajaxy,b=c(this);c.each(a.Controllers,function(d){a.ajaxifyController(d)});a.options.track_all_internal_links&&b.findAndSelf("a[href^=/],a[href^=./]").filter(":not(.ajaxy,.no-ajaxy)").addClass("ajaxy");a.options.track_all_anchors&&b.findAndSelf("a[href^=#]:not(.ajaxy,.no-ajaxy)").addClass("ajaxy");b.findAndSelf("a.ajaxy").once("click",a.ajaxify_helpers.a);b.findAndSelf("form.ajaxy").once("submit",a.ajaxify_helpers.form);return b},ajaxifyController:function(a){var b=
c.Ajaxy,d=b.getController(a);d&&typeof d.selector!=="undefined"&&c(function(){c(d.selector).data("ajaxy-controller",a).once("click",b.ajaxify_helpers.a)});return true},ajaxify_helpers:{a:function(a){var b=c.Ajaxy,d=c(this),e=b.extractRelativeUrl(d.attr("href")).replace(/^\/?\.\//,"/"),f=b.extractState(e);e=b.extractAnchor(e);if("/"+e===f||e===f)e="";var g=!d.hasClass(b.options.no_log_class);d=d.data("ajaxy-controller")||null;b.go({state:f,controller:d,log:g,anchor:e});a.stopPropagation();a.preventDefault();
return false},form:function(a){var b=c.Ajaxy,d=c(this),e=d.attr("disabled");if(e=e||e==="false")return false;if(d.attr("target"))return true;d=b.extractState(d.attr("action"));b.go({state:d,form:this});a.stopPropagation();a.preventDefault();return false}}};c.Ajaxy.construct()}})(jQuery);
