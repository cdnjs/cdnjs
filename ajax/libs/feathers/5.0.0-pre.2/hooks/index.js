...
class FeathersHookManager extends hooks_1.HookManager {
    ...
    collectMiddleware(self, args) {
        const app = this.app;
        const appHooks = app.appHooks[hooks_1.HOOKS].concat(app.appHooks[this.method] || []);
...
function hookMixin(service, path, options) {
    ...
    hooks_1.hooks(service, serviceMethodHooks);
    service.hooks = function (hookOptions) {
        ...
        if (Array.isArray(hookOptions)) {
            return hooks_1.hooks(this, hookOptions);
        }
        Object.keys(hookOptions).forEach(method => {
            const manager = hooks_1.getManager(this[method]);
