 ...
 *
 */(function(f){mejs.MepDefaults={poster:"",defaultVideoWidth:480,defaultVideoHeight:270,videoWidth:-1,videoHeight:-1,audioWidth:400,audioHeight:30,startVolume:0.8,loop:false,enableAutosize:true,features:["playpause","progress","current","duration","tracks","volume","fullscreen"]};mejs.mepIndex=0;mejs.MediaElementPlayer=function(a,c){if(!(this instanceof mejs.MediaElementPlayer))return new mejs.MediaElementPlayer(a,c);var b=this,d=mejs.MediaFeatures;b.$media=f(a);b.options=f.extend({},mejs.MepDefaults,
...
k=c.find(".mejs-time-handle"),m=c.find(".mejs-time-float"),o=c.find(".mejs-time-float-current"),l=function(g){g=g.target;var n=null;if(g&&g.bytesTotal!=undefined&&g.bytesTotal>0&&g.bufferedBytes!=undefined)n=g.bufferedBytes/g.bytesTotal;else if(g&&g.buffered&&g.buffered.length>0&&g.buffered.end&&g.duration)n=g.buffered.end(0)/g.duration;n!==null&&h.width(e.width()*n)},j=function(g){g=g.pageX;var n=e.offset(),q=e.outerWidth();d.setCurrentTime((g-n.left)/q*d.duration)},p=false;e.bind("mousedown",function(g){j(g);
p=true;return false});f(document).bind("mouseup",function(){p=false}).bind("mousemove",function(g){p&&j(g)});d.addEventListener("progress",function(g){l(g)},false);d.addEventListener("timeupdate",function(g){l(g);if(d.currentTime&&d.duration){g=e.width()*d.currentTime/d.duration;var n=g-k.outerWidth(true)/2;i.width(g);k.css("left",n);m.css("left",n);o.html(mejs.Utility.secondsToTimeCode(d.currentTime))}},false)}})(jQuery);
...
j-k.height()/2);i.height(h.height()-j+parseInt(h.css("top").replace(/px/,""),10));i.css("top",j)},o=function(j){var p=h.height(),g=h.offset(),n=parseInt(h.css("top").replace(/px/,""),10);j=j.pageY-g.top;g=(p-j)/p;if(j<0)j=0;else if(j>p)j=p;k.css("top",j-k.height()/2+n);i.height(p-j);i.css("top",j+n);if(g==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}g=Math.max(0,g);g=Math.min(g,1);d.setVolume(g)},l=
false;c.bind("mousedown",function(j){o(j);l=true;return false});f(document).bind("mouseup",function(){l=false}).bind("mousemove",function(j){l&&o(j)});e.find("span").click(function(){if(d.muted){d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute");m(1)}else{d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute");m(0)}});d.addEventListener("volumechange",function(j){l||m(j.target.volume)},true);d.setVolume(a.options.startVolume)}})(jQuery);
(function(f){MediaElementPlayer.prototype.buildfullscreen=function(a,c,b,d){if(a.isVideo){var e=false,h=0,i=0,k=a.container,m=f('<div class="mejs-button mejs-fullscreen-button"><span></span></div>').appendTo(c).click(function(){o(!e)}),o=function(l){switch(d.pluginType){case "flash":case "silverlight":d.setFullscreen(l);break;case "native":if(mejs.MediaFeatures.hasNativeFullScreen)l?d.webkitEnterFullScreen():d.webkitExitFullScreen();else{if(l){h=a.$media.height();i=a.$media.width();k.addClass("mejs-container-fullscreen").width("100%").height("100%").css("z-index",
1E3);a.$media.width("100%").height("100%");b.children("div").width("100%").height("100%");m.removeClass("mejs-fullscreen").addClass("mejs-unfullscreen")}else{k.removeClass("mejs-container-fullscreen").width(i).height(h).css("z-index",1);a.$media.width(i).height(h);b.children("div").width(i).height(h);m.removeClass("mejs-unfullscreen").addClass("mejs-fullscreen")}a.setControlsSize()}}e=l};f(document).bind("keydown",function(l){e&&l.keyCode==27&&o(false)})}}})(jQuery);
