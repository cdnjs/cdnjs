...
this.events[a];if(!c)return true;if(!b){this.events[a]=[];return true}for(i=0;i<c.length;i++)if(c[i]===b){this.events[a].splice(i,1);return true}return false},dispatchEvent:function(a){var b,c,d=this.events[a];if(d){c=Array.prototype.slice.call(arguments,1);for(b=0;b<d.length;b++)d[b].apply(null,c)}}};
...
g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(b),j=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(b).click(function(){d.paused?d.play():d.pause()});d.addEventListener("play",function(){j.hide();e.hide();g.hide()},false);d.addEventListener("playing",function(){j.hide();e.hide();g.hide()},false);d.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||j.show()},false);
d.addEventListener("waiting",function(){e.show()},false);d.addEventListener("loadeddata",function(){e.show()},false);d.addEventListener("canplay",function(){e.hide()},false);d.addEventListener("error",function(){e.hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},findTracks:function(){var a=this,c=a.$media.find("track");a.tracks=[];c.each(function(){a.tracks.push({srclang:f(this).attr("srclang").toLowerCase(),src:f(this).attr("src"),kind:f(this).attr("kind"),
...
g=e.find(".mejs-volume-slider"),j=e.find(".mejs-volume-total"),i=e.find(".mejs-volume-current"),k=e.find(".mejs-volume-handle"),n=function(h){if(g.is(":visible")){var m=j.height(),q=j.position();h=m-m*h;k.css("top",q.top+h-k.height()/2);i.height(m-h);i.css("top",q.top+h)}else{g.show();n(h);g.hide()}},l=function(h){var m=j.height(),q=j.offset(),o=parseInt(j.css("top").replace(/px/,""),10);h=h.pageY-q.top;var p=(m-h)/m;if(q.top!=0){p=Math.max(0,p);p=Math.min(p,1);if(h<0)h=0;else if(h>m)h=m;k.css("top",
h-k.height()/2+o);i.height(m-h);i.css("top",h+o);if(p==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}p=Math.max(0,p);p=Math.min(p,1);d.setVolume(p)}},r=false;e.hover(function(){g.show()},function(){g.hide()});g.bind("mousedown",function(h){l(h);r=true;return false});f(document).bind("mouseup",function(){r=false}).bind("mousemove",function(h){r&&l(h)});e.find("button").click(function(){d.setMuted(!d.muted)});
d.addEventListener("volumechange",function(){if(!r)if(d.muted){n(0);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{n(d.volume);e.removeClass("mejs-unmute").addClass("mejs-mute")}},false);n(a.options.startVolume);d.pluginType==="native"&&d.setVolume(a.options.startVolume)}}})})(mejs.$);
