...
${br}Error: [qty] argument can only be > 0.`+nc),printHelpCmdAndDocURL(),process.exit(1));const p=ipv4.generate({qty:config.qty||1,verbose:!config.quietMode});config.quietMode||(1<config.qty&&console.info(`[ ${p.join(", ")} ]`),console.info("\nCopying to clipboard...")),copyToClipboard(Array.isArray(p)?p.join("\n"):p)}function printHelpSections(e=["header","usage","paramOptions","flags","infoCmds"]){var o=`[106m[30m ${pkgName} ${nc} `;const n={header:[`
├ `+o+copyright,o+"Source: "+srcURL,o+"Doc: "+docURL],usage:[`
${bw}o Usage:`+nc,` ${bw}»${nc} `+(bg+cmdFormat+nc)],paramOptions:[`
${bw}o ${bw}Parameter options:`+nc," --qty=n                     Generate n IP address(es)."],flags:[`
${bw}o ${bw}Boolean options:`+nc," -q, --quiet                 Suppress all logging except errors."],infoCmds:[`
${bw}o ${bw}Info commands:`+nc," -h, --help                  Display help screen."," -v, --version               Show version number."]};e.forEach(t=>{n[t]?.forEach(e=>{{var r=/header|usage/.test(t)?1:29;const s=process.stdout.columns||80,i=[],o=e.match(/\S+|\s+/g);let n="";o.forEach(e=>{var o=s-(0==i.length?0:r);n.length+"| ".length+e.length>o&&(i.push(0==i.length?n:n.trimStart()),n=""),n+=e}),i.push(0==i.length?n:n.trimStart()),i.forEach((e,o)=>console.info("| "+(0==o?e:" ".repeat(r)+e)))}})})}function printHelpCmdAndDocURL(){console.info(`
