(function (global, factory) {
  ...
  */
  ...
  function defaultSize(model) {
    ...
    const step = scaleRange && isVgRangeStep(scaleRange) ? scaleRange.step : getViewConfigDiscreteStep(config.view, vgSizeChannel);
    if (vega.isNumber(step) && vega.isNumber(tickBandPaddingInner)) {
      return {
        value: step * (1 - tickBandPaddingInner)
      };
    } else {
      return {
        signal: `${exprFromSignalRefOrValue(tickBandPaddingInner)} * ${exprFromSignalRefOrValue(step)}`
      };
    }
  }
