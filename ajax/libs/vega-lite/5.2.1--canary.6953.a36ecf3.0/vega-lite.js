(function (global, factory) {
  ...
  const INITIALIZE_GEO_INTERVAL = 'Interval selections over cartographic projections must be initialized with both longitude and latitude.'; // REPEAT
  ...
  const INIT = '_init'; // Separate type because the "fields" property is only used internally and we don't want to leak it to the schema.

  const interval = {
    defined: selCmpt => selCmpt.type === 'interval',
    parse: (model, selCmpt, selDef) => {
      if (model.hasProjection) {
        const def = { ...(vega.isObject(selDef.select) ? selDef.select : {})
        };
      ...
      if (selCmpt.translate && !scaleBindings.defined(selCmpt)) {
        const filterExpr = "!event.item || event.item.mark.name !== ".concat(vega.stringValue(selCmpt.name + BRUSH));
    ...
    signals: (model, selCmpt, signals) => {
      ...
      if (!model.hasProjection) {
        ...
        const bbox = "[" + "[".concat(xvname ? xvname + '[0]' : '0', ", ").concat(yvname ? yvname + '[0]' : '0', "],") + "[".concat(xvname ? xvname + '[1]' : 'width', ", ").concat(yvname ? yvname + '[1]' : 'height', "]") + "]";
        const intersect = "intersect(".concat(bbox, ", {markname: ").concat(vega.stringValue(model.getName('marks')), "}, unit.mark)");
        const base = "{unit: ".concat(unitName(model), "}");
        return [{
          name: name + INIT,
          init: init ? "[scale(".concat(projection, ", [").concat(xinit[0], ", ").concat(yinit[0], "]), scale(").concat(projection, ", [").concat(xinit[1], ", ").concat(yinit[1], "])]") : null
        }, ...signals, {
          name: tupleSg,
          update: "vlSelectionTuples(".concat(intersect, ", ").concat(base, ")")
        }];
      }
    },
