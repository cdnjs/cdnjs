...
"use strict";
  ...
  function parse_list ( src ) {
    ...
    while ( (m = re_list_item.exec( src )) ) {
      ...
      Array.prototype.push.apply( par.li, parse_inline( m[2].trim() ) );
  ...
  function parse_table ( src ) {
    ...
    while ( (m = re_table_row.exec( src )) ) {
      ...
      do {
        ...
        var mx = /^(==.*?==|[^\|])*/.exec( inner );
        cell = cell.concat( parse_inline( mx[0] ) );
        row.push( '\n\t\t', cell );
  ...
  function parse_inline ( src ) {
    ...
    do {
      ...
      if ( (m = re_phrase.exec( src )) && ( boundary || m[1] ) ) {
        ...
        if ( (m = rx.exec( src )) && m[1] ) {
          ...
          if ( code ) {
            ...
            list.add( [ phrase_type, pba ].concat( parse_inline( m[1] ) ) );
          }
          continue;
        }
        // else 
        ...
        if ( single ) { // single tag
          ...
          if ( (m = re_end_tag.exec( src )) ) {
            ...
            if ( tag === 'code' ) {
              ...
              list.merge( parse_inline( m[1] ) );
              continue;
            }
            else {
              element = element.concat( parse_inline( m[1] ) );
            }
            list.add( element );
            continue;
          }
      ...
      if ( (boundary && (m = re_link.exec( src ))) || (m = re_link_fenced.exec( src )) ) {
        ...
        list.add( [ 'a', pba ].concat( parse_inline( inner.replace( /^(\.?\s*)/, '' ) ) ) );
        continue;
      }
      ...
      , paragraph = function ( s, tag, pba, linebreak ) {
          ...
          s.split( /\n\n+/ ).forEach(function( bit, i ) {
            if ( tag === 'p' && /^\s/.test( bit ) ) {
              ...
              bit = bit.replace( /\n[\t ]/g, ' ' ).trim();
              out = out.concat( parse_inline( bit ) );
            }
            else {
              if ( linebreak && i ) { out.push( linebreak ); }
              out.push( pba ? [ tag, pba ].concat( parse_inline( bit ) )
                            : [ tag      ].concat( parse_inline( bit ) ) );
            }
          });
          return out;
        }
      ...
      if ( (m = re_block.exec( src )) ) {
        ...
        if ( (m = /\.(\.?)(?:\s|(?=:))/.exec( src )) ) {
          ...
          if ( block_type === 'bq' ) {
            ...
            pba['id'] = 'fn' + fnid;
            list.add( [ "p", pba, [ 'a', { 'href': '#fnr' + fnid }, [ 'sup', fnid ] ], ' ', m[1] ] );
          }
          ...
          if ( single ) { // single tag
            ...
            if ( (m = re_end_tag.exec( src )) ) {
              ...
              if ( tag === 'pre' ) {
                ...
                if ( /\n/.test( m[1] )  ) {
                  element = element.concat( parse_blocks( m[1] ) );
                }
                else {
                  element = element.concat( parse_inline( m[1].replace( /^ +/, '' ) ) );
                }
  ...
  textile.setOptions = textile.set_options = set_options;
