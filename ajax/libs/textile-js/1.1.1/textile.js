...
"use strict";
  ...
  // This "indiscriminately" parses HTML text into a list of JSON-ML element
  // No steps are taken however to prevent things like <table><p><td> - user can still create nonsensical but "well-formed" markup
  var mode_html = 1;
  var mode_textile = 1;
  var mode_htmltext = 1;

  function parse_html ( src, whitelist_tags, textile_mode ) {
    var org = src + ''
      , list = []
      , root = list
      , _stack = []
      , m
      , oktag = ( whitelist_tags )
                  ? function ( tag ) { return tag in whitelist_tags; }
    ...
    do {
      ...
      if ( !text_mode && (m = re_html_comment.exec( src )) && oktag('!') ) {
        src.advance( m[0] );
        list.push( [ '!', m[1] ] );
      }
      ...
      // open/void tag
        ...
        if ( single ) { // single tag
          ...
          if ( tag === 'notextile' ) {
            console.log( 'PROBLEM!' );
            // glob the rest of this
            var _inner = /((?:[^<]+?|[^\0])*?)\n*<\/notextile(?:\s[^>]*)?>/.exec( src );
            if ( _inner ) {
              console.log( 'notextile', [_inner[1]] );
              // parse internals as a separate block of html
              var xx = parse_html( _inner[1], whitelist_tags );
              _push.apply( list, xx );
              src.advance( _inner[0].length );
            }

          }
          else if ( tag === 'pre' ) {
            // glob the rest of this
            var _inner = /((?:[^<]+|[^\0])*?)<\/pre(?:\s[^>]*)?>/.exec( src );
            if ( _inner ) {
              console.log( 'pre:', [] );


              var xx = parse_html( (tail||'')+_inner[1], {code:1} );
              console.log( xx );
              _push.apply( element, xx );
              src.advance( _inner[0].length );
              list.push( element );
        ...
        if ( m ) {
          if ( textile_mode ) {
            // maintain reference
            _push.apply( list, parse_inline( m[0], textile_mode ) );
          }
          else {
            list.push( m[0] );
          }
      ...
      // in html parsing mode: we are done when we are through the string
      // in textile mode: we are done when the stack is empty (or no more string)
      not_done = src.valueOf();
      if ( textile_mode ) {
        not_done = _stack.length || not_done;
      }
    }
    while ( not_done );
    return root;
  ...
  function parse_inline ( src, options ) {
    ...
    do {
      ...
      if ( (m = re_html_tag.exec( src )) ) {
        list.merge( parse_html( src, undefined, options ) );
        continue;
        /*
        src.advance( m[0] );
        var tag = m[1]
        ...
        if ( single ) { // single tag
          ...
          console.log( "^(.*?)(</" + tag + "\\s*>)" );
            console.log( re_end_tag.exec( src ), String(src) );
  ...
  function textile ( txt, opt, _debug ) {
    // get a throw-away copy of options
    opt = merge( merge( {}, textile.defaults ), opt || {} );
    // run the converter
    var ml = parse_blocks( txt, opt );
    if ( _debug ) {
      return ml;
    }
    return ml.map( JSONML.toHTML ).join( '' );
  }
