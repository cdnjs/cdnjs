...
"use strict";
  ...
  function parse_html ( src, whitelist_tags ) {
    var org = src + '';
    var src = typeof src === 'string' ? ribbon( src ) : src
      , list = []
      , root = list
      , _stack = []
      , m
  ...
  function parse_attr ( input, element ) {
    input += '';
    ...
    do {
      ...
      // preventing a problem case: "before _(in parens)_ after"
      // TODO: can this be added as a (?!...) to padding?
      if ( /^\([^\n\)]*\s[^\n\)]*\)/.test( remaining ) ) {
        // this is pretty much how Redcloth does it (by using a special case)
        m = null;
        continue;
      }
      ...
      // only for blocks: 
      if ( is_img || is_block ) {
        var re_pba_align = ( is_img ) ? re_pba_align_img : re_pba_align_blk;
  ...
  function parse_list ( src ) {
    ...
    while ( m = re_list_item.exec( src ) ) {
      ...
      while ( stack.length < dest_level ) {
        var lst = [ type, pad( stack.length + 1 ), (new_li = [ 'li' ]) ]
  ...
  function parse_table ( src ) {
    ...
    var m = re_table_head.exec( src );
    if ( m ) {
      // parse and apply table attr
      src.advance( m[0] );
      var pba = parse_attr( m[2], 'table' );
      if ( pba ) {
        table.push( pba[1] );
      }
    }
    ...
    while ( m = re_table_row.exec( src ) ) {
      ...
      do {
        ...
        row.push( '\n\t\t', cell );
        var more = inner.valueOf().charAt( mx[0].length ) === '|';
    ...
    do {
      ...
      if ( (m = re_phrase.exec( src )) && ( boundary || m[1] ) ) {
        ...
        var tok = m[2]
          ...
          , pba = !code && parse_attr( src, phrase_type )
          ;
        if ( pba ) {
          src.advance( pba[0] );
          pba = pba[1];
      ...
      if ( (m = re_image.exec( src )) || (m = re_image_fenced.exec( src )) ) {
        ...
        var pba = m[1] && parse_attr( m[1], 'img' );
  ...
  function parse_blocks ( src ) {
      ...
      , paragraph = function ( s, tag, pba, linebreak ) {
          ...
          s.split( /\n\n+/ ).forEach(function( bit, i ) {
            if ( tag === 'p' && /^\s/.test( bit ) ) {
              ...
              (linebreak && i) && out.push( linebreak );
    ...
    while ( src.valueOf() ) {
      ...
      // paragraph
      var m = re_block_normal.exec( src );
